 ...
 */
const char *
fmtId_internal(const char *rawid, bool force_pg_mode)
{
	PQExpBuffer id_return = getLocalPQExpBuffer();

	const char *cp;
	bool		need_quotes = false;
	char startc = (PQinOraMode() && !force_pg_mode) ? 'A' : 'a';
	char endc = (PQinOraMode() && !force_pg_mode) ? 'Z' : 'z';
 ...
 */
const char *
fmtQualifiedId(int remoteVersion, const char *schema, const char *id, bool force_pg_mode)
{
	...
	appendPQExpBufferStr(lcl_pqexp, fmtId_internal(id, force_pg_mode));
					  ...
					  const char *prefix, int encoding, bool std_strings)
{
	...
	for (i = 0; i < noptions; i++)
	{
		...
		appendPQExpBuffer(buffer, "%s%s=", prefix, fmtId_internal(name, true));
					  ...
					  bool lower_object)
{
	...
	bool		need_lower_name = false;
	bool		in_ora_mode = false;

	/* Use the local variable for every front clients */
	if (pg_strcasecmp(PQSqlMode(conn), "opentenbase_ora") == 0)
		in_ora_mode = true;
...
#define WHEREAND() \
	...
	while (*cp)
	{
		...
		else if (!inquotes && isupper((unsigned char) ch) && !in_ora_mode)
		{
			appendPQExpBufferChar(&namebuf,
								  pg_tolower((unsigned char) ch));
			cp++;
		}
		else if (!inquotes && islower((unsigned char) ch) && in_ora_mode)
		{
			/* Just update the need_lower_name flag when it is a special object and has lowercase characters */
			need_lower_name = lower_object;
			appendPQExpBufferChar(&namebuf,
		...
		else if (!inquotes && ch == '.')
		{
			...
			appendPQExpBufferStr(&namebuf, "^(");
			/* Update the need_lower_name flag to false, and check it again */
			need_lower_name = false;
			cp++;
		}
		else if (ch == '$')
		{
	...
	if (namebuf.len > 2)
	{
		/* Convert to lowercase when it is a special object in opentenbase_ora mode */
		if (in_ora_mode && need_lower_name)
		{
			int	j;

			/* Skip the ^( and convert to downcase */
			for (j = 2; j < namebuf.len; j++)
				namebuf.data[j] = pg_tolower((unsigned char) (namebuf.data[j]));
		}
	...
	if (schemabuf.len > 2)
	{
		/* Convert to downcase when it is a system namespace */
		if (in_ora_mode && isSystemNamespaceName(schemabuf.data + 2))
		{
			int	j;

			/* Skip the ^( and convert to downcase */
			for (j = 2; j < schemabuf.len; j++)
				schemabuf.data[j] = pg_tolower((unsigned char) (schemabuf.data[j]));
		}
