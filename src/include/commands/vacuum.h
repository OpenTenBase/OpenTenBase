...
typedef struct AnalyzePgStats
{
	...
	char		statypalign[STATISTIC_NUM_SLOTS];
	float4	   *stanumbers[STATISTIC_NUM_SLOTS];
	int32		*stavalueslen[STATISTIC_NUM_SLOTS];
	Datum	   *stavalues[STATISTIC_NUM_SLOTS];
} AnalyzePgStats;

typedef struct AnalyzeExtStats
{
	char	*statname;
	int		distinct_len;
	char	*distinct;
...
typedef struct AnalyzeRelStats
{
	/* pg_class info */
	char		*relname;
	char		*namespacename;
	int32		class_info_num;
	int32		relallvisible;
	bool		relhasindex;
	bool		inh;
	bool		resetcounter;
	...
	int32		stat_info_num;
	int32		*stat_att_num;
	AnalyzePgStats	*pgstat;

	/* pg_statistic_ext info */
	int32			ext_info_num;
	AnalyzeExtStats	 *pgstatext;
} AnalyzeRelStats;
...
extern void vac_update_relstats(Relation relation,
					...
					bool in_outer_xact);
extern void online_update_relstats(Relation relation, double tuples_incr);
...
extern void xlog_reinit_extent_pages(RelFileNode rnode, ExtentID eid, bool checksum_enabled);
...
#endif
extern int update_analyze_stat(AnalyzeRelStats *analyzeStats);
extern int get_remote_relstat(char *nspname, char *relname, bool replicated,
                              int32 *pages, int32 *allvisiblepages, float4 *tuples,
                              TransactionId *frozenXid);
#endif							/* VACUUM_H */
