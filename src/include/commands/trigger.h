...
#define CALLED_AS_FUNC_CHECK(fcinfo) \
	((fcinfo)->context != NULL && IsA((fcinfo)->context, FuncCheckData))

#define CALLED_AS_TRIGGER_CHECK(fcinfo)                                    \
	((fcinfo)->context != NULL && IsA((fcinfo)->context, FuncCheckData) && \
	 ((FuncCheckData *)((fcinfo)->context))->tgtype > 0)
...
extern bool should_pgxc_fire_trigger(TriggerDesc *trigdesc, Relation rel, CmdType cmd);
#endif
