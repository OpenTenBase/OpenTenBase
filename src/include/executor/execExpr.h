...
typedef struct ExprEvalStep
{
	...
	union
	{
		...
		struct
		{
			...
			Datum		testvalue;	/* values of bool result */
#endif
		}			jump;
		...
		struct
		{
			...
			int			jumpdone;
#ifdef __OPENTENBASE_C__
			bool		is_first;	/* if the first step */
#endif
		}			rowcompare_step;
...
#ifdef __OPENTENBASE_C__
	Datum			vec_resvalue;	/* result vector for normal case */
#endif
	int32			*restypmod; /* Currently only for empty string */
} ExprEvalStep;
...
typedef struct ArrayRefState
{
	...
	Datum		upperindexdatum[MAXDIM];
	Oid			upperindextype[MAXDIM]; /* Array subscript type, used for subscript type verification */
...
extern bool disable_tochar_simplify;
...
extern void ExecEvalRownum(ExprState *state, ExprEvalStep *op);
extern void ExecEvalRowNull(ExprState *state, ExprEvalStep *op,
				ExprContext *econtext);
extern void ExecEvalRowNotNull(ExprState *state, ExprEvalStep *op,
				   ExprContext *econtext);
extern void ExecEvalArrayExpr(ExprState *state, ExprEvalStep *op);
extern Oid NestedTableGetDisguiseType(Oid type_id);
extern bool NestedTableDisguiseTypeEqual(Oid ltype, Oid rtype);
