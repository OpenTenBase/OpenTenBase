...
typedef struct HashJoinTupleNext
{
#define FIELDNO_HJTNEXT_UNS 0
...
{
#define FIELDNO_HJTUP_NEXTTUP 0
	/* link to next tuple in same bucket */
	HashJoinTupleNext next;
#define FIELDNO_HJTUP_HASHVAL 1
...
typedef struct ParallelHashJoinState
{
	...
	dsa_pointer bloomfilter;	/* merged bloomfilter */
	LWLock		filterlock;		/* lock protecting the bloom filter */
...
#define FIELDNO_HJTABLE_TUPDATA 5
	HashJoinTableBuckets buckets;
	...
	BlockBloomFilter 	bloomFilter;    /* bloom filter on the hash values */
	bool		try_jitted;				/* true for hashed by crc32 */
#endif
}			HashJoinTableData;

#ifdef __OPENTENBASE_C__
extern void MultiExecCompiledVecHash(HashJoinState *node,
									 bool *noresult, bool *backward);
#endif
