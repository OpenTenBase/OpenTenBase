...
typedef bool (*post_query_replace_function) (const char *i_query, char **o_rewrite_query);
extern PGDLLIMPORT post_query_replace_function post_query_replace_hook;
 ...
 */
static inline void
pq_writeint128(StringInfoData *pg_restrict buf, int128 i)
{
	int128		ni = i;

	Assert(buf->len + (int) sizeof(int128) <= buf->maxlen);
	memcpy((char *pg_restrict) (buf->data + buf->len), &ni, sizeof(int128));
	buf->len += sizeof(int128);
}
...
/* append a binary int128 to a StringInfo buffer */
static inline void
pq_sendint128(StringInfo buf, int128 i)
{
	enlargeStringInfo(buf, sizeof(int128));
	pq_writeint128(buf, 128);
}
