...
extern bool enable_filter_statistics;
extern double agg_estimate_factor;
#endif
...
typedef struct VariableStatData
{
	...
	List       *pvsd;           /* variable stat for leaf partition*/
} VariableStatData;
...
#define ReleaseVariableStats(vardata)                                                              \
	...
	{                                                                                              \
		...
		{                                                                                          \
			...
			{                                                                                      \
				VariableStatData *pvardata = (VariableStatData *) lfirst(lc);                      \
				if (HeapTupleIsValid(pvardata->statsTuple))                                        \
				{                                                                                  \
					pvardata->freefunc(pvardata->statsTuple);                                      \
					pvardata->statsTuple = NULL;                                                   \
				}                                                                                  \
			}                                                                                      \
			list_free_deep((vardata).pvsd);                                                        \
			(vardata).pvsd = NIL;                                                                  \
		}                                                                                          \
	} while (0)
