 ...
 *
 * IDENTIFICATION
 *	  src/include/utils/gdd_detectorpriv.h
 *
 *-------------------------------------------------------------------------
 */

#ifndef GDD_DETECTORPRIV_H
#define GDD_DETECTORPRIV_H

#include "access/twophase.h"
#include "gdd_detector.h"	/* for the typedefs of the structs */
...
/*
 * Get current key at iter.
 */
#define gdd_map_iter_get_key(iter) \
	((iter).map->pairs[(iter).pos].key)
...
struct GddStat
{
    ...
    int			original_indeg;	/* original in degree */
};
...
typedef bool (*is_atxact_transaction_cb)(GddCtx *ctx, int txid);
...
struct GddCtx
{
	...
	TransactionInfo *global_transaction; /* an array to map gxid(string) to txn_id(int) */
	int global_transaction_count;        /* transaction array count */
    int global_transaction_size;         /* transaction array size */

	is_atxact_transaction_cb	is_atxact_transaction;
};

/***************************************************************************/

#endif   /* GDD_DETECTORPRIV_H */
