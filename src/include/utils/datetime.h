...
extern int strtoint(const char *nptr, char **endptr, int base);
...
extern void EncodeInterval_WithTypmod(struct pg_tm *tm, fsec_t fsec, int style,
										char *str, int typmod);
extern Datum interval_out_withtypmod(PG_FUNCTION_ARGS);
extern Datum orcl_interval_typmode_tochar(PG_FUNCTION_ARGS);
...
extern void general_do_to_timestamp(char *date_txt, char *fmt, struct pg_tm *tm, fsec_t *fsec);

extern char *pre_process_rawstr(char* str, int len, int *ostrlen);
#endif							/* DATETIME_H */
