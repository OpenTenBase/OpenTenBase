...
#define GiSTOptionsGetStringData(_basePtr, _memberName, _defaultVal)                    \
	(((_basePtr) && (((GiSTOptions*)(_basePtr))->_memberName)) 						 \
			? (((char*)(_basePtr) + *(int*)&(((GiSTOptions*)(_basePtr))->_memberName))) \
			: (_defaultVal))

#ifdef __OPENTENBASE_C__

#define GiSTEnableChecksum(relation) \
	((IS_PGXC_DATANODE && (relation)->rd_rel->relkind == RELKIND_INDEX && \
	  (relation)->rd_rel->relam == GIST_AM_OID && (relation)->rd_options) ? \
	 ((GiSTOptions *)(relation)->rd_options)->checksum : false)
...
extern void GISTInitBuffer(Buffer b, uint32 f, bool checksum_enabled);
