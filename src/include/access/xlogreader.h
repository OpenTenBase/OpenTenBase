...
/* xlog for heap/estore insert, an efficient routine*/
typedef struct xl_heap_data_newpage
{
    RelFileNode node;
    BlockNumber blkno;
    ForkNumber forknum;
    ...
    Size offset;
    int32 block_size;
} xl_heap_data_newpage;

#define SizeOfHeapDataNewPage (offsetof(xl_heap_data_newpage, block_size) + sizeof(int32))
...
#ifdef __OPENTENBASE_C__
#define XLogRecGetChecksumInfo(decoder) ((decoder)->checksum_enabled)
#endif
...
extern bool XLogRecGetBlockShardID(XLogReaderState *record, uint8 block_id,
				   				ShardID *sid);
#ifdef __OPENTENBASE_C__
extern bool RestoreColBlockImage(XLogReaderState *record, uint8 block_id, char *page);
#endif

extern bool XLogRecGetEstoreInfo(XLogReaderState *record, RelFileNode *rnode, 
