...
typedef struct CookedConstraint
{
	...
	AttrNumber *keys;			/* which attrs (only for partial cluster constraint) */
	int			numkeys;		/* number of attrs (only for partial cluster constraint) */
...
#ifdef __OPENTENBASE_C__
extern List* RelationGetConstraintList(Oid relOid, char constrType);
#endif
...
extern void AddRelationDistribution_DN(Oid relid,
				...
				PGXCSubCluster *subcluster,
				TupleDesc	 descriptor);
extern Oid GetDistributeGroup(PGXCSubCluster *subcluster, DistributionType dtype);
...
#ifdef __OPENTENBASE_C__
extern void ValidateStrOptCompression(char *val);
extern void ValidateStrOptParallelDml(char * val);
#endif
