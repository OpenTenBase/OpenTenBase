 ...
 */
static int
PQcancelSendInternal(PGcancel *cancel, bool end_query,
					 char *errbuf, int errbufsize, int timeo, pgsocket *sock)
{
	...
	if ((ret = connect_timeout(tmpsock, (struct sockaddr *) &cancel->raddr.addr,
							   cancel->raddr.salen, timeo)) <= 0)
	{
 ...
 */
static int
internal_cancel(PGcancel* cancel, char *errbuf, int errbufsize, int timeo)
{
	pgsocket tmpsock = PGINVALID_SOCKET;

	int ret = PQcancelSendInternal(cancel, false, errbuf, errbufsize, timeo, &tmpsock);
...
{
	return PQcancelSendInternal(cancel, end_query, errbuf, errbufsize, timeout, sock);
}

#ifdef __OPENTENBASE_C__
static int
internal_end(PGcancel *cancel, char *errbuf, int errbufsize)
{
	pgsocket tmpsock = PGINVALID_SOCKET;

	int ret = PQcancelSendInternal(cancel, true, errbuf, errbufsize, 0, &tmpsock);
...
{
	return internal_cancel(cancel, errbuf, errbufsize, 0);
}

int
PQcancel_timeout(PGcancel* cancel, char* errbuf, int errbufsize, int timeout)
{
    return internal_cancel(cancel, errbuf, errbufsize, timeout);
}
...
{
	return internal_end(cancel, errbuf, errbufsize);
}
#endif
