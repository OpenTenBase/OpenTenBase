...
static bool conn_in_ora_mode = false;
 ...
 *
 * Parallel establishes a connection to a backend through the postmaster
 * using connection information in a string.
 *
 * The conninfo string is either a whitespace-separated list of
 *
 ...
 */
int
PQconnectdbParallel(char const* const* conninfo, int count, PGconn **conn)
{
	int conn_done = 0;
	int success   = 1;
...
connectDBComplete(PGconn *conn)
{
	...
	for (;;)
	{
		...
		{
			case PGRES_POLLING_OK:
				/* Set front client tools sql mode */
				if (conn->sql_mode && pg_strcasecmp(conn->sql_mode, "opentenbase_ora") == 0)
					conn_in_ora_mode = true;
				else
					conn_in_ora_mode = false;
 ...
 */
static int
connect_timeout(int sockfd, struct sockaddr *serv_addr, int addrlen, int timeo)
{
	int error;
	int flags;
	int ret;
	struct pollfd fd;
	int len = sizeof(int);
	int timeout = (timeo <= 0) ? -1 : (timeo * 1000);
...
/*
 * Sending a cancel message without waiting for a response
 */
static int
PQcancelSendInternal(SockAddr *raddr, int be_pid, int be_key, bool end_query,
					 char *errbuf, int errbufsize, int timeo, pgsocket *sock)
{
	...
	{
		strlcpy(errbuf, "internal_cancel() -- socket() failed: ", errbufsize);
	...
	{
		strlcpy(errbuf, "internal_cancel() -- setsockopt(SO_KEEPALIVE) failed: ", errbufsize);
		goto cancel_errReturn;
	}
	...
	if (timeo)
	{
		...
		{
			strlcpy(errbuf, "internal_cancel() -- setsockopt(SO_SNDTIMEO) failed: ", errbufsize);
			goto cancel_errReturn;
		}
		...
		{
			strlcpy(errbuf, "internal_cancel() -- setsockopt(SO_RCVTIMEO) failed: ", errbufsize);
			goto cancel_errReturn;
		}
	}

	if ((ret = connect_timeout(tmpsock, (struct sockaddr *) &raddr->addr,
							   raddr->salen, timeo)) <= 0)
	{
		snprintf(errbuf, errbufsize, "PQcancel() -- connect() failed: errno %d, ret %d", SOCK_ERRNO, ret);
		goto cancel_errReturn;
	}
	...
	crp.cp.cancelRequestCode = (MsgType) pg_hton32((end_query) ? END_QUERY_REQUEST_CODE : CANCEL_REQUEST_CODE);
...
/*
 * Receive response to cancel
 */
void
PQcancelRecv(pgsocket tmpsock)
{
	int			save_errno = SOCK_ERRNO;
...
internal_cancel(SockAddr *raddr, int be_pid, int be_key,
				char *errbuf, int errbufsize, int timeo)
{
	pgsocket tmpsock = PGINVALID_SOCKET;

	int ret = PQcancelSendInternal(raddr, be_pid, be_key, false, errbuf, errbufsize, timeo, &tmpsock);

	if (ret)
	{
		PQcancelRecv(tmpsock);
	}
 ...
 */
int
PQcancelSend(PGcancel* cancel, pqbool end_query, char* errbuf, int errbufsize, int timeout, pgsocket *sock)
{
	if (!cancel)
	{
	...
	return PQcancelSendInternal(&cancel->raddr, cancel->be_pid, cancel->be_key, end_query,
						   errbuf, errbufsize, timeout, sock);
}

#ifdef __OPENTENBASE_C__
static int
internal_end(SockAddr *raddr, int be_pid, int be_key, char *errbuf, int errbufsize)
{
	pgsocket tmpsock = PGINVALID_SOCKET;

	int ret = PQcancelSendInternal(raddr, be_pid, be_key, true, errbuf, errbufsize, 0, &tmpsock);
...
int
PQcancel_timeout(PGcancel* cancel, char* errbuf, int errbufsize, int timeout)
{
    if (!cancel)
	{
		strlcpy(errbuf, "PQcancel() -- no cancel object supplied", errbufsize);
    ...
    return internal_cancel(&cancel->raddr, cancel->be_pid, cancel->be_key,
						   errbuf, errbufsize, timeout);
}


#ifdef __OPENTENBASE_C__
int
PQend(PGcancel *cancel, char *errbuf, int errbufsize)
{
	if (!cancel)
	{
		strlcpy(errbuf, "PQcancel() -- no cancel object supplied", errbufsize);
	...
	return internal_end(&cancel->raddr, cancel->be_pid, cancel->be_key,
						   errbuf, errbufsize);
}
#endif
...
PQrequestCancel(PGconn *conn)
{
	...
	r = internal_cancel(&conn->raddr, conn->be_pid, conn->be_key,
						conn->errorMessage.data, conn->errorMessage.maxlen, 0);
...
int
PQopentenbaseVersion(const PGconn *conn)
{
	if (!conn)
		return 0;
	if (conn->status == CONNECTION_BAD)
		return 0;
	return conn->opentenbase_version;
}
...
char *
PQSqlMode(const PGconn *conn)
{
	if (!conn || conn->status != CONNECTION_OK || conn->sql_mode == NULL)
		return "postgresql";
	return conn->sql_mode;
}
