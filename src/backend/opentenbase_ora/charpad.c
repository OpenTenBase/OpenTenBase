...
orcl_lpad(PG_FUNCTION_ARGS)
{
	...
	if (s2blen < 0)
		s2blen = 0;
	ptr1oriend = VARDATA_ANY(string1) + s1blen;
	ptr2oriend = VARDATA_ANY(string2) + s2blen;

	/* if the filler length is zero disable filling */
	if (s2blen == 0)
	{
		s2_operate = OFF;	/* turn off string2 processing flag */
		output_width = 0;	/* same behavior as opentenbase_ora database */
	}
	...
	while (s1blen > 0)
	{
		...
		mlen = pg_mblen(ptr1);
		/* illegal characters is unchanged, s1blen > 0, so setting mblen = 1 is safe */
		if (mlen > (ptr1oriend - ptr1))
		{
			mlen = 1;
			dsplen = ISO10646_NORMAL_CHARACTER;
		}
		else
		{
			dsplen = pg_dsplen(ptr1);

			if (dsplen == ISO10646_C0C1_CTRL_CHARACTER ||
				dsplen == ISO10646_ZERO_LEN_CHARACTER)
			{
				dsplen = ISO10646_NORMAL_CHARACTER;
			}
		}
