...
Datum
lightweight_ora_timestamp_tochar(PG_FUNCTION_ARGS)
{
	text *fmt = PG_GETARG_TEXT_PP_IF_NULL(1);
...
Datum
lightweight_ora_timestamptz_tochar(PG_FUNCTION_ARGS)
{
	text *fmt = PG_GETARG_TEXT_PP_IF_NULL(1);
...
/*
 * A dispatcher function to process to_char(text, text[fmt]).
 */
Datum
orcl_to_char(PG_FUNCTION_ARGS)
{
	text  *val = PG_GETARG_TEXT_PP_IF_NULL(0);
	text  *fmt = PG_GETARG_TEXT_PP_IF_NULL(1);
	Datum  result;
	...
	PG_RETURN_NULL_IF_EMPTY_TEXT(val);
	PG_RETURN_NULL_IF_EMPTY_TEXT(fmt);

	result = DirectFunctionCall3(numeric_in,
								 CStringGetDatum(text_to_cstring(val)),
								 Int32GetDatum(0),
								 Int32GetDatum(-1));

	return DirectFunctionCall2(numeric_to_char, result, PG_GETARG_DATUM(1));
}
...
/*
 * opentenbase_ora function to_number(text, text)
...
orcl_to_single_byte(PG_FUNCTION_ARGS)
{
	...
	while ((s - VARDATA_ANY(src) < srclen) && *s)
	{
		char   *u = s;
		int		clen;
		int		mapindex;
...
Datum
clobtochar(PG_FUNCTION_ARGS)
{
#define ORA_CLOB_TO_CHAR_MAX_LENGTH 4000
	Datum	clob_datum = PG_GETARG_DATUM(0);
	int32	len;

	len = text_length(clob_datum);

	/* If the length of the clob is larger than 4000, cut it to 4000 */
	if (len > ORA_CLOB_TO_CHAR_MAX_LENGTH)
		clob_datum = DirectFunctionCall3(text_substr, clob_datum,
										 Int32GetDatum(1),
										 Int32GetDatum(ORA_CLOB_TO_CHAR_MAX_LENGTH));

	return DirectFunctionCall3(bpcharin,
								CStringGetDatum(TextDatumGetCString(clob_datum)),
								ObjectIdGetDatum(InvalidOid), PG_GETARG_DATUM(1));
}

Datum
blobtoraw(PG_FUNCTION_ARGS)
{
	int32	len;
	int32	typmod = PG_GETARG_INT32(1);
	...
	/*
	 * In opentenbase_ora, select cast('11111'::BLOB, raw) from dual will report error:
	 * ERROR:  missing left parenthesis.
	 * But it supports sql like : create function xxx(a raw), the raw type
	 * will be treated as raw(2000) in function arguments. If we support it,
	 * we must update in here.
	 * However, it will not report a error in sql like:
	 * select cast('11111'::BLOB, raw) from dual.
	 * It is different from opentenbase_ora, but we think it is harmless.
	 */
	if (typmod == -1)
		maxlen = MAX_RAW_SIZE;

	len = DatumGetInt32(DirectFunctionCall1(byteaoctetlen, PG_GETARG_DATUM(0)));

	if (len > maxlen)
		ereport(ERROR,
				(errcode(ERRCODE_STRING_DATA_RIGHT_TRUNCATION),
						errmsg("value too long for type raw(%d), actual(%d)", maxlen, len)));

	return DirectFunctionCall1(rawin,
								CStringGetDatum(TextDatumGetCString(PG_GETARG_DATUM(0))));
}
