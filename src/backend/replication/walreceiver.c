...
WalReceiverMain(void)
{
    ...
    elog(LOG, "walreceiver started walrcv->walRcvState %d", walrcv->walRcvState);
	/*
	...
	for (;;)
	{
		...
		if (walrcv_startstreaming(wrconn, &options))
		{
			...
			for (;;)
			{
				...
				(void)whitebox_trigger_generic(WAL_RECEIVER_PROCESS_DATA, WHITEBOX_TRIGGER_SLEEP,
...
WalRcvDie(int code, Datum arg)
{
	...
	walrcv->latch = NULL;
	elog(LOG, "WalRcvDie, received flush:%X/%X", (uint32)(LogstreamResult.Flush >> 32),
