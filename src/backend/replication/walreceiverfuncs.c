	...
	if (state == WALRCV_STARTING)
	{
		...
		if ((now - startTime) > WALRCV_STARTUP_TIMEOUT)
		{
			...
			if (walrcv->walRcvState == WALRCV_STARTING)
            {
                ...
                elog(DEBUG1, "set walrcv->walRcvState to WALRCV_STOPPED");
            }
...
WalRcvStreaming(void)
{
    ...
    elog(DEBUG1, "WalRcvStreaming walRcvState %d  startTime %ld", state, startTime);
	/*
	 * If it has taken too long for walreceiver to start up, give up. Setting
	 * the state to STOPPED ensures that if walreceiver later does start up
	 * after all, it will see that it's not supposed to be running and die
 ...
 */
void
ShutdownWalRcvInternal(char *file, int32 lineno)
{
	WalRcvData *walrcv = WalRcv;
	pid_t		walrcvpid = 0;

    elog(DEBUG1, "ShutdownWalRcv pid %d, walRcvState %d, %s:%d", walrcv->pid, walrcv->walRcvState, file, lineno);
	/*
					 ...
					 const char *slotname)
{
    ...
    elog(DEBUG1, "RequestXLogStreaming walRcvState %d, startTime %ld", walrcv->walRcvState, now);
	...
	if (walrcv->walRcvState == WALRCV_STOPPED)
	{
        ...
        elog(DEBUG1, "set walrcv->walRcvState to WALRCV_STARTING, startTime %ld", now);
	}
	else
