...
#ifdef _SHARDING_		
		reln->smgr_shard_targblocks = NULL;		
...
#else
		reln->smgr_which = SMgr_Row_Storage;
#endif
...
smgrclose(SMgrRelation reln)
{
...
#ifdef __OPENTENBASE_C__
	if (reln->smgr_shard_targblocks)
	{
		pfree(reln->smgr_shard_targblocks);
		reln->smgr_shard_targblocks = NULL;
	}
#endif
	...
	if (hash_search(SMgrRelationHash,
		...
		elog(LOG, "SMgrRelation hashtable corrupted");
...
static void
push_unlink_rel_one_fork_to_hashtbl(RelFileNode node, ForkNumber forknum)
{
	HTAB *relfilenode_hashtbl = UnlinkRelInfo->unlink_rel_fork_hashtbl;
	bool found = false;
	...
	entry = (RelFileNodeNFork*)hash_search(relfilenode_hashtbl, &(key), HASH_ENTER, &found);
	if (!found)
	{
		entry->rnode.spcNode = key.rnode.spcNode;
		entry->rnode.dbNode = key.rnode.dbNode;
		entry->rnode.relNode = key.rnode.relNode;
