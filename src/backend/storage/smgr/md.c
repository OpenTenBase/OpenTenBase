...
/*
 * Specify the detection period for whether Postmaster is alive
 */
#define FORWARD_FSYNC_REQUEST_RETRY		256
...
static void register_dirty_segment(SMgrRelation reln, ForkNumber forknum,
					   MdfdVec *seg);
static void register_unlink(RelFileNodeBackend rnode, ForkNumber forknum);
...
static bool
CheckUnlinkRelHTAB(RelFileNode rnode, ForkNumber forknum)
{
	HTAB* relfilenode_hashtbl = UnlinkRelInfo->unlink_rel_hashtbl;
	HTAB* relfilenode_fork_hashtbl = UnlinkRelInfo->unlink_rel_fork_hashtbl;
	RelFileNodeNFork entry_key;
	bool found = false;
	...
	if (!found)
	{
		...
		if (!found)
		{
			...
			LWLockAcquire(&UnlinkRelInfo->rel_hashtbl_lock, LW_SHARED);
			(void)hash_search(relfilenode_hashtbl, &key, HASH_FIND, &found);
			LWLockRelease(&UnlinkRelInfo->rel_hashtbl_lock);
		}
	}
...
bool
IsExistInUnlinkRelHTAB(RelFileNode *rnode)
{
	HTAB* relfilenode_hashtbl = UnlinkRelInfo->unlink_rel_hashtbl;
	bool found = false;
...
mdcreate(SMgrRelation reln, ForkNumber forkNum, bool isRedo)
{
	...
	if (fd < 0)
	{
		...
		if (isRedo || IsBootstrapProcessingMode() ||
			(IsInplaceUpgrade && reln->smgr_rnode.node.relNode < FirstNormalObjectId))
			fd = PathNameOpenFile(path, O_RDWR | PG_BINARY | (IsInplaceUpgrade ? O_TRUNC : 0));
		if (fd < 0)
		{
...
mdunlink(RelFileNodeBackend rnode, ForkNumber forkNum, bool isRedo)
{
	...
	{		
		for (forkNum = 0; forkNum <= EXTENT_FORKNUM; forkNum++)
		{
		 ...
		 char *buffer, bool skipFsync)
{
	...
	{
		if (CheckUnlinkRelHTAB(reln->smgr_rnode.node, forknum))
		{
			ereport(DEBUG1,
		...
		char *buffer, bool skipFsync)
{
	...
	if (FileSeek(v->mdfd_vfd, seekpos, SEEK_SET) != seekpos)
	{
		...
		{
			ereport(DEBUG1,
					(errmsg("could not write block %u in file \"%s\": %m, this relation has been removed",
							blocknum, FilePathName(v->mdfd_vfd))));
			return;
		}
		else
...
mdnblocks(SMgrRelation reln, ForkNumber forknum)
{
	...
	/* If relation in unlink HTAB, return directly */
	if (v == NULL)
		return 0;
	...
	while (curopensegs > 0)
	{
		...
		if (priorblocks > nblocks)
		{
			...
			if (FileTruncate(v->mdfd_vfd, 0, WAIT_EVENT_DATA_FILE_TRUNCATE) < 0)
			{
				...
				{
					ereport(DEBUG1,
							(errmsg("could not truncate file \"%s\": %m, this relation has been removed",
									FilePathName(v->mdfd_vfd))));
					FileClose(v->mdfd_vfd);
					_fdvec_resize(reln, forknum, 0);
					break;
		...
		for (forknum = 0; forknum <= MAX_FORKNUM; forknum++)
		{
			...
			while ((segno = bms_next_member(requests, segno)) >= 0)
			{
				...
				for (failures = 0;; failures++) /* loop exits at "break" */
				{
                    ...
                    {
                        /* 
                         * Make sure that the relfile has been deleted, but 
                         * still in unlink hash table 
                         */
                        if (CheckUnlinkRelHTAB(entry->rnode, forknum))
                        {
                            ereport(DEBUG1,
						...
						failures > 0)
					{
                        ...
                        /* when code run here it must be failed by ereport*/
...
register_dirty_segment(SMgrRelation reln, ForkNumber forknum, MdfdVec *seg)
{
	...
	else
	{
		...
		if (FileSync(seg->mdfd_vfd, WAIT_EVENT_DATA_FILE_SYNC) < 0)
		{
			...
			{
				ereport(DEBUG1,
						(errmsg("could not fsync file \"%s\": %m, has been unlink",
							FilePathName(seg->mdfd_vfd))));
			}
			else
			{
 ...
 */
static void
register_unlink(RelFileNodeBackend rnode, ForkNumber forknum)
{
	if (pendingOpsTable)
	{
		/* push it into local pending-ops table */
		RememberFsyncRequest(rnode.node, forknum,
	...
	else
	{
		...
		while (!ForwardFsyncRequest(rnode.node, forknum,
									UNLINK_RELATION_REQUEST))
		{
			if (sleep_times > FORWARD_FSYNC_REQUEST_RETRY)
			{
				sleep_times = 0;
				if (false == PostmasterIsAlive())
				{
					ereport(LOG,
							(errmsg("could not forward fsync request because postmaster is dead")));
					proc_exit(1);
				}
			}
			else
...
RememberFsyncRequest(RelFileNode rnode, ForkNumber forknum, BlockNumber segno)
{
	...
	else if (segno == UNLINK_RELATION_REQUEST)
	{
		...
		Assert(forknum == MAIN_FORKNUM || forknum == DICTIONARY_FORKNUM || forknum == SORT_FORKNUM);
...
_mdnblocks(SMgrRelation reln, ForkNumber forknum, MdfdVec *seg)
{
	...
	if (len < 0)
	{
		...
		{
			ereport(DEBUG1,
					(errmsg("could not seek to end of file \"%s\": %m, this relation has been removed",
							FilePathName(seg->mdfd_vfd))));
			return 0;
		}
		ereport(ERROR,
...
#ifdef __OPENTENBASE_C__
/*
 * Get block size of a single disk file
 */
int
mdblocksize(SMgrRelation reln)
{
	return BLCKSZ;
}
...
void mddealloc(SMgrRelation reln, ForkNumber forknum, BlockNumber from_blk)
{
	...
	seekpos = (off_t) blocksize * (from_blk % ((BlockNumber) RELSEG_SIZE));
	Assert(seekpos < (off_t) blocksize * RELSEG_SIZE);
	ExtentAssert(seekpos < (off_t) blocksize * RELSEG_SIZE);

	if(0 != FileDealloc(v->mdfd_vfd,
						seekpos, 
						PAGES_PER_EXTENTS * blocksize,
