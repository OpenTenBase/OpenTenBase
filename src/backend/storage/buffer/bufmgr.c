...
static bool validate_checksum_error_file(const char *name);
...
ReadBufferWithoutRelcache(RelFileNode rnode, ForkNumber forkNum,
						  ...
						  BufferAccessStrategy strategy, bool checksum_enabled)
{
	bool		hit;

	SMgrRelation smgr = smgropen(rnode, InvalidBackendId);
	smgr->smgr_haschecksum = checksum_enabled;
				  ...
				  Relation rel)
{
	...
	*hit = false;
	...
	/* determine whether to encrypt when write out */
	bufHdr->encrypt = smgr->encrypt;
	bufHdr->haschecksum = smgr->smgr_haschecksum;
	...
	else
	{
		...
		else
		{
			...
			if (!PageIsVerified((Page) bufBlock, blockNum, smgr->smgr_haschecksum))
			{
				...
				{
					WritePageCheckSumErrorFile(smgr->smgr_rnode, forkNum, blockNum);
					ereport(ERROR,
							(errcode(ERRCODE_DATA_CORRUPTED),
...
FlushBuffer(BufferDesc *buf, SMgrRelation reln)
{
	...
	else
	{
		 ...
		 * Note that if we may have encrypted page in private memory, so need not to copy again.
		 */
		PageSetChecksumFlag(bufBlock, buf->haschecksum);
...
RelationGetNumberOfBlocksInFork(Relation relation, ForkNumber forkNum)
{
	...
	if (RELATION_IS_META_GLOBAL_TEMP(relation) && !smgrexists(relation->rd_smgr, forkNum))
	{
		ereport(WARNING,
					((errmsg("Missing global temp table file for table \"%s\"",
					  RelationGetRelationName(relation))),
					 errhint("Please add an empty file or re-create the global temp table.")));
		return 0;
	}
...
BufferGetLSNAtomic(Buffer buffer)
{
	 ...
	 */
	if (!XLogHintBitIsNeeded(page) || BufferIsLocal(buffer))
...
void
DropRelFileNodeAllBuffersUsingHash(HTAB *relfilenode_hashtbl)
{
	int i;
	...
	for(i = 0; i < NBuffers; i++)
	{
		...
		uint32 buf_state;
		bool found = false;
		Oid dropDbNode = InvalidOid;

		hash_search(relfilenode_hashtbl, &rnode, HASH_FIND, &found);

		if (!found)
		{
			RelFileNode key;
			key.spcNode = InvalidOid;
			key.dbNode = rnode.dbNode;
			key.relNode = InvalidOid;
			hash_search(relfilenode_hashtbl, &key, HASH_FIND, &found);
			if (found)
				dropDbNode = rnode.dbNode;
			else
				continue;
		}
		...
		if (RelFileNodeEquals(buf_desc->tag.rnode, rnode) ||
				(OidIsValid(dropDbNode) && buf_desc->tag.rnode.dbNode == dropDbNode))
		{
...
void
DropRelFileNodeOneForkAllBuffersUsingHash(HTAB *relfilenode_hashtbl)
{
	int i;
	...
	{
		BufferDesc *buf_desc = GetBufferDescriptor(i);
		RelFileNodeNFork forkrnode;
		uint32 buf_state;
		bool found = false;

		forkrnode.rnode = buf_desc->tag.rnode;
		forkrnode.forknum = buf_desc->tag.forkNum;

		hash_search(relfilenode_hashtbl, &forkrnode, HASH_FIND, &found);
		...
		if (RelFileNodeEquals(buf_desc->tag.rnode, forkrnode.rnode) &&
			forkrnode.forknum == buf_desc->tag.forkNum)
		{
			InvalidateBuffer(buf_desc);  /* releases spinlock */
		}
...
bool
RelationHasChecksum(Relation rel)
{
	return (RelationEnableChecksum(rel) || GinEnableChecksum(rel) || 
	GiSTEnableChecksum(rel) || BrinEnableChecksum(rel) || 
	IsSystemClass(rel->rd_id, rel->rd_rel));
}

/*
 * Write Checksum Error file
 *
 * If checksum is incorrect, write to file.
 */
void
WritePageCheckSumErrorFile(RelFileNodeBackend rnode, ForkNumber forkNum, BlockNumber blockNumber)
{
	int			fd;
	int			ret;
	StringInfo	content;
	char	path[MAXPGPATH];
	char	*checksumdir = "checksum";
	char	filepath[MAXPGPATH];
	int		fileno = 0;
	int		offset = 0;

	snprintf(path, MAXPGPATH, "%s/%d.crc_error", checksumdir, MyProcPid);

	/*
	 * Write crc file for data
	 */
	fd = PathNameOpenFile(path, O_RDWR | O_CREAT | O_EXCL);
	if (fd < 0)
	{
		elog(ERROR, "write checksum error file %s error:%s",
		     path, strerror(errno));
	}

	fileno = (int) (blockNumber / RELSEG_SIZE);
	offset = (blockNumber % RELSEG_SIZE) * BLCKSZ;
	memset(filepath, 0, MAXPGPATH);
	if (fileno > 0)
		snprintf(filepath, MAXPGPATH, "%s.%d", relpath(rnode, forkNum), fileno);
	else
		snprintf(filepath, MAXPGPATH, "%s", relpath(rnode, forkNum));

	content = makeStringInfo();
	/* filepath:offset:size:checksum_enabled:checksum_verified */
	appendStringInfo(content, "%s:%d:%d:%d:%d\n", filepath, offset, BLCKSZ, 1, 0);
...
/*
 * check whether data directory contains checksum errors
 */
void
CheckPageCheckSumErrorFileExists(void)
{
	bool	error_exists = false;
	char	*checksumdir = "checksum";
	struct dirent* entry;

	DIR* dir = opendir(checksumdir);
	...
	{
		if (validate_checksum_error_file(entry->d_name))
		{
			ereport(WARNING, 
					(errmsg("Checksum error file %s exists", 
							entry->d_name)));
			error_exists = true;
		}
	}
	...
	if (error_exists && !ignore_checksum_failure)
		ereport(ERROR, 
				(errmsg("Checksum error exists, check and repaire pages")));
}

/*
 * validate checksum error file
 *
 * checksum file name is like "digits.crc_error"
 * return true if file name matches, false if not.
 */
static bool
validate_checksum_error_file(const char *name)
{
	...
	return strcmp(name + 1, "crc_error") == 0;
}
