...
static bool retryLockBufHdr(BufferDesc *desc, uint32 *buf_state)
{
	...
	for (;;)
	{
		...
		if (!retryLockBufHdr(buf, &local_buf_state)) {
			if (--try_get_loc_times == 0) {
				ereport(WARNING,
					(errmsg("try get buf headr lock times equal to NBuffers when StrategyGetBuffer")));
				try_get_loc_times = NBuffers;
			}
