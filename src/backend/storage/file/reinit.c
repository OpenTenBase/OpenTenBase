...
ResetUnloggedRelationsInDbspaceDir(const char *dbspacedirname, int op)
{
	...
	if ((op & UNLOGGED_RELATION_CLEANUP) != 0)
	{
		...
		while ((de = ReadDir(dbspace_dir, dbspacedirname)) != NULL)
		{
			...
			if (found)
			{
				...
				/* Coltable is a directory while rowtable is a file */
                if (S_ISDIR(statbuf.st_mode))
                {
                    retcode = rmtree(rm_path, true);
                }
                else
                {
                    retcode = unlink(rm_path);
                }

                if (retcode)
                {
                    elog(LOG, "could not remove file \"%s\": %m", rm_path);
                }
                else
                {
                    elog(DEBUG2, "remove file \"%s\"", rm_path);
                }
            }
        }
	...
	if ((op & UNLOGGED_RELATION_INIT) != 0)
	{
		...
		while ((de = ReadDir(dbspace_dir, dbspacedirname)) != NULL)
		{
            ...
            {
                (void)copy_file(srcpath, dstpath, ERROR);
            }
		}
