...
copydir(char *fromdir, char *todir, bool recurse)
{
	...
	while ((xlde = ReadDir(xldir, fromdir)) != NULL)
	{
		...
		if (S_ISDIR(fst.st_mode))
		{
			...
			(void)copy_file(fromfile, tofile, ERROR);
	}
	FreeDir(xldir);
...
#define COPY_BUF_SIZE (8 * BLCKSZ)
	...
	buffer = (char *) palloc_extended(COPY_BUF_SIZE, MCXT_ALLOC_NO_OOM);
	if (buffer == NULL)
	{
        ereport(elevel,
				(errcode_for_file_access(),
				 errmsg("alloc copy buf failed,size: %d", COPY_BUF_SIZE)));
        return false;
	}
	/*
	 * Open the files
...
bool
is_directory_exists(const char *dir)
{
	struct stat st;
