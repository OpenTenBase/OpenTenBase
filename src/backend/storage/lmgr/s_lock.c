 ...
 */
static void
s_lock_stuck_nopanic(const char *file, int line, const char *func)
{
	if (!func)
		func = "(unknown)";
#if defined(S_LOCK_TEST)
	fprintf(stderr,
			...
			func, file, line);
#else
	elog(WARNING, "stuck spinlock detected at %s, %s:%d",
		 func, file, line);
#endif
}
...
/*
 * s_lock_nopanic(lock) - platform-independent portion of waiting for a spinlock.
 */
int
s_lock_nopanic(volatile slock_t *lock, const char *file, int line, const char *func)
{
	SpinDelayStatus delayStatus;
 ...
 */
void
perform_spin_delay_nopanic(SpinDelayStatus *status)
{
	/* CPU-specific delay each time through the loop */
	SPIN_DELAY();
	...
	{
		if (++(status->delays) > NUM_DELAYS)
			s_lock_stuck_nopanic(status->file, status->line, status->func);

		if (status->cur_delay == 0) /* first time to delay? */
			status->cur_delay = MIN_DELAY_USEC;

		pg_usleep(status->cur_delay/10);
