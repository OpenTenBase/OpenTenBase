...
#ifdef LWLOCK_STATS
static HTAB *lwlock_stats_htab = NULL;
...
print_lwlock_stats(int code, Datum arg)
{
	...
	{
		fprintf(stderr,
				"PID %d lwlock %s %p: shacq %u exacq %u expid %d blk %u spindelay %u dequeue self %u line %d file %s\n",
				MyProcPid, LWLockTrancheArray[lwstats->key.tranche],
				lwstats->key.instance, lwstats->sh_acquire_count,
				lwstats->ex_acquire_count, lwstats->ex_accquire_pid, lwstats->block_count,
				lwstats->spin_delay_count, lwstats->dequeue_self_count,
				lwstats->lineno, lwstats->filename);
	}
...
InitializeLWLocks(void)
{
	...
	/* Initialize FN recv buffer table LWLocks in main array */
	lock = MainLWLockArray + NUM_INDIVIDUAL_LWLOCKS +
		   NUM_BUFFER_PARTITIONS + NUM_LOCK_PARTITIONS +
		   NUM_PREDICATELOCK_PARTITIONS + NUM_CACHE_2PC_PARTITIONS;
	for (id = 0; id < NUM_RECVBUF_PARTITIONS; id++, lock++)
		LWLockInitialize(&lock->lock, LWTRANCHE_RECVBUFFER_HTAB);

	/* Initialize SPM plan partition LWLocks in main array */
	lock = MainLWLockArray + SPM_PLAN_MAPPING_LWLOCK_OFFSET;
	for (id = 0; id < NUM_SPM_PLAN_PARTITIONS; id++, lock++)
		LWLockInitialize(&lock->lock, LWTRANCHE_SPMPLAN_HTAB);
	
	/* Initialize SPM plan id partition LWLocks in main array */
	lock = MainLWLockArray + SPM_PLAN_ID_MAPPING_LWLOCK_OFFSET;
	for (id = 0; id < NUM_SPM_PLAN_ID_PARTITIONS; id++, lock++)
		LWLockInitialize(&lock->lock, LWTRANCHE_SPMPLAN_HTAB);

	/* Initialize SPM history partition LWLocks in main array */
	lock = MainLWLockArray + SPM_HISTORY_MAPPING_LWLOCK_OFFSET;
	for (id = 0; id < NUM_SPM_HISTORY_PARTITIONS; id++, lock++)
		LWLockInitialize(&lock->lock, LWTRANCHE_SPMHISTORY_HTAB);
	
	/* Initialize SPM history id partition LWLocks in main array */
	lock = MainLWLockArray + SPM_HISTORY_ID_MAPPING_LWLOCK_OFFSET;
	for (id = 0; id < NUM_SPM_HISTORY_ID_PARTITIONS; id++, lock++)
		LWLockInitialize(&lock->lock, LWTRANCHE_SPMHISTORY_HTAB);
...
RegisterLWLockTranches(void)
{
	...
	LWLockRegisterTranche(LWTRANCHE_PARALLEL_SHARE_CTE, "parallel_share_cte");
	LWLockRegisterTranche(LWTRANCHE_RECVBUFFER_HTAB, "FnRcvBufferLock");
...
const char *
GetLWLockName(LWLock* lock)
{
	const char *lockName = T_NAME(lock);
	if (lockName == NULL)
	{
		return "unknown";
	}
...
bool
#ifdef LWLOCK_STATS
LWLockAcquire_internal(LWLock *lock, LWLockMode mode)
#else
...
#ifdef LWLOCK_STATS
	if (mode == LW_EXCLUSIVE)
		lwstats->ex_accquire_pid = MyProcPid;
#endif
