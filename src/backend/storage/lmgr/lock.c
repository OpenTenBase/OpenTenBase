...
LockHasWaiters(const LOCKTAG *locktag, LOCKMODE lockmode, bool sessionLock)
{
	...
	if (!locallock || locallock->nLocks <= 0)
	{
		...
		elog(LOG,
		     ...
		     "detail: lock [dboid: %u, reloid: %u], "
		     "tid [maintid: %lu, selftid: %lu]",
		     lockMethodTable->lockModeNames[lockmode],
...
bool
IsLockExists(const LOCKTAG *locktag, LOCKMODE lockmode)
{
	LOCKMETHODID lockmethodid = locktag->locktag_lockmethodid;
	LOCK	   *lock;
	bool		found;
	...
	if (!found)
		exist = false;
	else if(lock && lock->granted[lockmode])
		exist = true;
	...
	if (proc->coordPid > 0)
	{
		...
		while (nextplock)
		{
			...
			if (nextproc->coordPid == proc->coordPid &&
					...
					nextproc->coordlxid == proc->coordlxid)
			{
				/*
 ...
 */
bool
CheckWaitLockModeConflictHoldMask(LOCKTAG tag, LOCKMODE waitLockMode,
	LOCKMASK holderMask, LOCKMODE holderWaitLockMode)
{
	...
	if (holderWaitLockMode != 0 && holderMask != 0)
		holderMask = holderMask | LOCKBIT_ON(holderWaitLockMode);
