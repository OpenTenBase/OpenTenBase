 ...
 */
void
PageInit_shard(Page page, Size pageSize, Size specialSize, ShardID sid, bool nohole, bool checksum_enabled)
{
 ...
 */
bool
PageIsVerified(Page page, BlockNumber blkno, bool has_checksum)
{
	PageHeader	p = (PageHeader) page;
	size_t	   *pagebytes;
	int			i;
	...
	if (!PageIsNew(page))
	{
		if (DataChecksumsEnabled(page))
		{
			/*
			 * Do verify page if checksum is on for the current heap.
			 */
			if (has_checksum)
			{
				...
				elog(DEBUG3,"Relation has checksum %X", p->pd_checksum);
			}
			else
			{
				/*
				 * Clear checksum flag if heap disables checksum.
				 * we don't mark the page dirty right now for performance issue
				 * and the operation will be synced to disk when it gets dirty.
				 */
				elog(DEBUG3,"Relation has checksum,clear now");
				PageClearCheckSum(page);
				p->pd_checksum = 0;
			}
		}
		else if (page_checksums && p->pd_checksum != 0)
		{
			checksum_failure = true;
			elog(DEBUG3,"Page has no checksum but page crc is not zero");
		}
 ...
 */
Page
PageGetTempPageCopySpecial(Page page, bool checksum_enabled)
{
	Size		pageSize;
	Page		temp;
	...
	PageInit(temp, pageSize, PageGetSpecialSize(page), checksum_enabled);
...
PageSetChecksumCopy(Page page, BlockNumber blkno)
{
	...
	if (PageIsNew(page) || !DataChecksumsEnabled(page))
		return (char *) page;
	...
	elog(DEBUG1,"Set page checksum %X",((PageHeader)pageCopy)->pd_checksum);
	return pageCopy;
}
...
PageSetChecksumInplace(Page page, BlockNumber blkno)
{
	...
	elog(DEBUG1,"Set page checksum inplace %X",((PageHeader)page)->pd_checksum);
}
