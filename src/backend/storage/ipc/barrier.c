 ...
 */
void
BarrierInit(Barrier *barrier, int participants, int max_participants)
{
	...
	barrier->phase = 0;
	barrier->elected = 0;
	barrier->max_parts = max_participants;
...
BarrierArriveAndWait(Barrier *barrier, uint32 wait_event_info)
{
	...
	/*
	 * Building hashtable require collaboration between workers, but Due to the
	 * current design of asynchronously receiving data in Forward Node, the 
	 * barrier needs to wait for any other potential participant who has not
	 * yet attached. So wait for "max_parts" instead of "participants". If
	 * there is some workers never launched, information will be noticed by
	 * BarrierRefreshParticipants.
	 */
	if (barrier->max_parts > 0 &&
		(wait_event_info == WAIT_EVENT_HASH_BUILD_HASHING_OUTER ||
		 wait_event_info == WAIT_EVENT_HASH_BUILD_HASHING_INNER ||
		 wait_event_info == WAIT_EVENT_SIP_FILTER_FINISHED))
		participants = barrier->max_parts;
	else
...
BarrierDetachImpl(Barrier *barrier, bool arrive)
{
	 ...
	 */
	if ((arrive || participants > 0) &&
		barrier->arrived == participants)
	{
		release = true;
		barrier->arrived = 0;
		++barrier->phase;
...
/*
 * Due to the current design of asynchronously receiving data in Forward Node,
 * the barrier may wait for a potential participant who has not yet attached.
 * Unless this worker has not been forked at all (probably due to the
 * limitation of max_parallel_workers), so we need to update max_participants
 * here to notify the waiting participants to continue.
 */
void
BarrierRefreshParticipants(Barrier *barrier, int max_participants)
{
	bool		release = false;
	int			start_phase;
	int			next_phase;
	...
	{
		participants = barrier->max_parts = max_participants;
	}
	else
	{
		/* end query requested, disable max parts */
