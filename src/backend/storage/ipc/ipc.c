...
#ifdef _PG_ORCL_
	/* avoid adding exit functions repeatedly */
	if (ORA_MODE && before_shmem_exit_index > 0)
	{
		for (i = before_shmem_exit_index - 1; i >= 0 ; i--)
		{
			if (before_shmem_exit_list[i].function == function && before_shmem_exit_list[i].arg == arg)
			{
				found = true;
				break;
			}
