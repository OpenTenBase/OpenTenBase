...
#ifdef __OPENTENBASE_C__
		/* force to finish material if we have to */
		if (pstate->plan->remote_flag == -1 && tuplestorestate)
		{
			...
			tuplestore_gettupleslot(tuplestorestate, forward, false, slot);
			return slot;
		}
#endif
...
#ifdef __OPENTENBASE_C__
	else if (((Plan *) node)->remote_flag == -1)
		matstate->eflags |= EXEC_FLAG_REWIND;
#endif
...
ExecReScanMaterial(MaterialState *node)
{
	...
	if (node->eflags != 0)
	{
		...
		if (outerPlan->chgParam != NULL ||
			...
			(outerPlan->state->es_rewinding && contain_remote_subplan(outerPlan->plan)))
		{
			tuplestore_end(node->tuplestorestate);
			node->tuplestorestate = NULL;
			if (outerPlan->chgParam == NULL)
