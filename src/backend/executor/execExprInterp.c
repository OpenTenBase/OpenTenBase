...
#include "utils/orcl_datetime_func.h"
...
ExecReadyInterpretedExpr(ExprState *state)
{
	...
	/* ExecJustConstVec that does not currently implement vectorization will follow the default process */
...
/*
 * *************************************************************************
 *  Important ! change ExecInterpExpr should also change llvm_compile_expr
 * *************************************************************************
 *
...
ExecInterpExpr(ExprState *state, ExprContext *econtext, bool *isnull)
{
	...
	static const void *const dispatch_table[] = {
		...
		&&CASE_EEOP_LEVELEXPR,
		&&CASE_EEOP_CBISLEAFEXPR,
		&&CASE_EEOP_CBISCYCLEEXPR,
		&&CASE_EEOP_LAST
	};
	...
	EEO_SWITCH()
	{
		...
		EEO_CASE(EEOP_SCAN_FETCHSOME)
		{
			...
			/* In case of vectorslot, tuple should have already been deformed while scanning */
		...
		EEO_CASE(EEOP_FUNCEXPR)
		{
			...
			if (enable_scalar_subquery_cache && 
				...
				state->parent && 
				func_result_cachable(fcinfo->flinfo->fn_oid))
			{
				ResultCacheValue *result = NULL;

				rc = GetScalarSubResultCache(fcinfo->flinfo->fn_oid, state->parent->state);
				result = SearchResultCacheInternal(rc, fcinfo->nargs, fcinfo->arg, fcinfo->argnull);
				if (result != NULL)
				{
					bool    rettypByVal;
					int16	rettypLen;
					Oid     rettype = rc->rc_rettype;

					get_typlenbyval(rettype, &rettypLen, &rettypByVal);
					*op->resnull = result->valuenull;
					if (!*op->resnull)
						*op->resvalue = datumCopy(result->value,
												rettypByVal,
												rettypLen);
					else
			...
			*op->resnull = fcinfo->isnull;
			if (rc)
			{
				(void)ResultCacheCreateEntry(state->parent->state->es_query_cxt, 
											&state->parent->state->es_rc_total_memory, 
											rc, fcinfo->arg, fcinfo->argnull,
		...
		EEO_CASE(EEOP_FUNCEXPR_STRICT)
		{
			 ...
			 * For vectorize execution, such check is performed in function.
			 */
	...
	strictfail:
		...
		EEO_CASE(EEOP_IOCOERCE)
		{
			...
			if (enable_lightweight_ora_syntax && str && strlen(str) == 0)
			{
				str = NULL;
				*op->resnull = true;
			}
		...
		EEO_CASE(EEOP_NULLIF)
		{
			...
			/* expr1 is literal null is not allowed in opentenbase_ora mode */
			if (ORA_MODE && fcinfo->argnull[0])
			{
				List *args = ((FuncExpr *) fcinfo->flinfo->fn_expr)->args;
				Node *arg = NULL;

				if (list_length(args) == 2)
					arg = (Node *) linitial(args);

				/* check if first arg is var and no records in the right tree */
				if (arg && IsA(arg, Var) && state->parent && state->parent->righttree
						&& state->parent->righttree->plan && IsA(state->parent->righttree->plan, Result))
				{
					Result *r = (Result *) state->parent->righttree->plan;
					if (r->resconstantqual && IsA(r->resconstantqual, List))
					{
						List *qual = (List*) r->resconstantqual;
						if (list_length(qual) == 1)
						{
							Node *e = (Node *) linitial(qual);
							if (e && IsA(e, Const) && ((Const *) e)->constvalue == 0)
								elog(ERROR, "inconsistent datatypes: expected - got CHAR");
						}
					}
				}
			}
		...
		EEO_CASE(EEOP_ROWCOMPARE_FINAL)
		{
			...
			cmpresult = DatumGetInt32(*op->resvalue);
			*op->resnull = false;
			switch (rctype)
			{
		 ...
		 */
		/*
		 * When performing vectorize execution:
		 * For strict function with no initial value, ExecAggInitGroup is already called to init it,
		 * while strictness check is enforced in detailed implementation of corresponding function.
		 * Note that the inital value is already processed, which should be skipped during aggregation.
		 * For strict function with an initial value, strictness check is enforced in corresponding
		 * function as above.
		 * For non-strict function, we should keep it always same as the non-vectorize one.
		 * Aggregation functions should be carefully handled in terms of above cases, refer to
		 * pg_aggregation and pg_proc for decision making.
		 */
		...
		EEO_CASE(EEOP_LEVELEXPR)
		{
			*op->resnull = false;
			if (ORA_MODE && enable_opentenbase_ora_return_type)
				*op->resvalue = DirectFunctionCall1(int8_numeric, econtext->level);
			else
				*op->resvalue = Int64GetDatum(econtext->level);
			EEO_NEXT();
		}

		EEO_CASE(EEOP_CBISLEAFEXPR)
		{
			*op->resnull = false;
			if (ORA_MODE && enable_opentenbase_ora_return_type)
				*op->resvalue = DirectFunctionCall1(int2_numeric, BoolGetDatum(econtext->isleaf));
			else
				*op->resvalue = BoolGetDatum(econtext->isleaf);
			EEO_NEXT();
		}

		EEO_CASE(EEOP_CBISCYCLEEXPR)
		{
			*op->resnull = false;
			*op->resvalue = DirectFunctionCall1(int2_numeric, BoolGetDatum(econtext->iscycle));
			EEO_NEXT();
		}
...
CheckVarSlotCompatibility(TupleTableSlot *slot, int attnum, Oid vartype)
{
	...
	if (attnum > 0)
	{
		...
		if (vartype != attr->atttypid &&
			!NestedTableDisguiseTypeEqual(vartype, attr->atttypid))
			ereport(ERROR,
			   ...
			   paramId > 0 && paramId <= paramInfo->numParams))
	{
		...
		if (likely(OidIsValid(prm->ptype)))
		{
			...
			if (unlikely(prm->ptype != op->d.param.paramtype) &&
				!(ORA_MODE &&
				  (NestedTableGetDisguiseType(prm->ptype) == op->d.param.paramtype ||
				   prm->ptype == NestedTableGetDisguiseType(op->d.param.paramtype))))
				ereport(ERROR,
						(errcode(ERRCODE_DATATYPE_MISMATCH),
...
#ifdef _PG_ORCL_
	if (ORA_MODE && paramInfo && paramId > paramInfo->numParams)
	{
		if (paramInfo->paramFetch != NULL)
		{
			ParamExternData *prm;
	...
	*op->resnull = false;
	...
	switch (svf->op)
	{
		...
		case SVFOP_USER:
		{
			...
			InitFunctionCallInfoData(fcinfo, NULL, 0, InvalidOid, NULL, NULL);
			name = current_user(&fcinfo);

			if (ORA_MODE && !fcinfo.isnull)
			{
				Datum oldname = name;
				char *oldname_str = NULL;
				int   typmod;

				oldname_str = NameStr(*DatumGetName(oldname));
				typmod = strlen(oldname_str) + VARHDRSZ;
				name = DirectFunctionCall3(varchar2in,
				                           CStringGetDatum(oldname_str),
				                           Int32GetDatum(InvalidOid),
				                           Int32GetDatum(typmod));

				pfree(DatumGetPointer(oldname));
			}
			*op->resvalue = name;
			*op->resnull = fcinfo.isnull;
			break;
		...
		case SVFOP_SESSION_USER:
			...
			*op->resnull = fcinfo.isnull;
			break;
		case SVFOP_USER_ID:
			*op->resvalue = Int64GetDatum((int64) GetUserId());
			*op->resnull = false;
			break;
...
{
	int128		newval = nextval_internal(op->d.nextvalueexpr.seqid, false);
	int32		typmod;
	Numeric		num;
	...
	switch (op->d.nextvalueexpr.seqtypid)
	{
		...
		case INT8OID:
			*op->resvalue = Int64GetDatum((int64) newval);
			break;
		case NUMERICOID:
			typmod = op->d.nextvalueexpr.typmod;
			num = convert_int128_to_numeric(newval, 0);
			*op->resvalue = DirectFunctionCall2(numeric,
												NumericGetDatum(num),
												Int32GetDatum(typmod));
			break;
		default:
...
/*
 * get the disguise type of the nested table type
 */
Oid
NestedTableGetDisguiseType(Oid type_id)
{
	Type	typtup = typeidType(type_id);
	Oid		disguisetypid = InvalidOid;

	if (OidIsValid(typeTypeRelid(typtup)) &&
		get_rel_relkind(typeTypeRelid(typtup)) == RELKIND_NESTED_TABLE_TYPE)
	{
		Oid		elemtypid;
		Oid		elemTypNamespaceOid;
		int32	typmod;
		char	*etypName;
		char	*namespaceName;
		Type	elemTyp;
		StringInfoData	typNameStr;

		elemtypid = get_nesttable_elemtype((Form_pg_type) GETSTRUCT(typtup), NULL);

		Assert(OidIsValid(elemtypid));
		elemTyp = typeidType(elemtypid);
		etypName = NameStr(((Form_pg_type)GETSTRUCT(elemTyp))->typname);
		elemTypNamespaceOid = ((Form_pg_type)GETSTRUCT(elemTyp))->typnamespace;
		namespaceName = get_namespace_name(elemTypNamespaceOid);

		initStringInfo(&typNameStr);
		if (!is_integer_type(etypName))
			appendStringInfo(&typNameStr, "%s.%s[%d]", namespaceName, etypName, 1);
		else
			appendStringInfo(&typNameStr, "integer[%d]", 1);

		ReleaseSysCache(elemTyp);
		parseTypeString(typNameStr.data, &disguisetypid, &typmod, false);
	}
	ReleaseSysCache(typtup);
	return disguisetypid;
}

/*
 * Compare with the pg array type corresponding to the nested table type.
 *
 * TODO:Can binary implicit array types also should return true?
 */
bool
NestedTableDisguiseTypeEqual(Oid ltype, Oid rtype)
{
	Oid		lDisguiseOid = InvalidOid;
	Oid		rDisguiseOid = InvalidOid;

	if (!ORA_MODE)
		return false;

	/*
	 * find the nested table type corresponding to the
	 * pg array type.
	 */
	lDisguiseOid = NestedTableGetDisguiseType(ltype);
	rDisguiseOid = NestedTableGetDisguiseType(rtype);

	if (lDisguiseOid != InvalidOid)
		ltype = lDisguiseOid;
	if (rDisguiseOid != InvalidOid)
		rtype = rDisguiseOid;
...
ExecEvalMinMax(ExprState *state, ExprEvalStep *op)
{
	...
	*op->resnull = true;
	...
	for (off = 0; off < op->d.minmax.nelems; off++)
	{
		...
		if (nulls[off])
		{
			/* In opentenbase_ora, any null value will cause the result become NULL */
			if (ORA_MODE)
			{
				*op->resnull = true;
				break;
...
ExecEvalConvertRowtype(ExprState *state, ExprEvalStep *op, ExprContext *econtext)
{
	...
	if (op->d.convert_rowtype.map != NULL)
	{
		...
		result = execute_attr_map_tuple(&tmptup, op->d.convert_rowtype.map, NULL);
		...
		case IS_XMLPARSE:
			{
				...
				bool	   *argnull = op->d.xmlexpr.argnull;
				text	   *data;
				int			preserve_whitespace_or_wellformed;
				...
				value = argvalue[1];
				if (ORA_MODE)
					preserve_whitespace_or_wellformed = DatumGetInt32(value);
				else
					preserve_whitespace_or_wellformed = DatumGetBool(value);

				*op->resvalue = PointerGetDatum(xmlparse(data,
														 xexpr->xmloption,
														 preserve_whitespace_or_wellformed));
				*op->resnull = false;
			}
			break;
...
ExecEvalRownum(ExprState *state, ExprEvalStep *op)
{
	*op->resnull  = false;
	if (ORA_MODE && enable_opentenbase_ora_return_type)
		*op->resvalue = DirectFunctionCall1(int8_numeric, Int64GetDatum(state->rownum));
	else
