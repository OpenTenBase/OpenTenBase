...
static WindowStatePerAggData *initialize_peragg(WindowAggState *winstate,
				  WindowFunc *wfunc,
				  WindowStatePerAgg peraggstate);
static Datum GetAggInitVal(Datum textInitVal, Oid transtype, Oid aggOwner,
						   Oid inputcollid, WindowAggState *winstate,
						   WindowStatePerAgg peraggstate);
						   ...
						   WindowStatePerAgg peraggstate)
{
	...
	if (perfuncstate->wfunc->windistinct)
	{
		...
		{
			if (peraggstate->distinct_typbyval)
				peraggstate->distinctArr = palloc0(sizeof(int64) * 16);
			else
				peraggstate->distinctArr = palloc0(peraggstate->distinct_typlen * 16);
						...
						WindowStatePerAgg peraggstate)
{
	...
	if (wfuncstate->wfunc->windistinct && !fcinfo->argnull[1])
	{
		...
		if (peraggstate->distinct_typlen > 0)
		{
			ele_size = peraggstate->distinct_typbyval ? sizeof(int64) : peraggstate->distinct_typlen;
			for (i = 0; i < peraggstate->distinctCount; i++)
			{
				Datum arg2;

				if (peraggstate->distinct_typbyval)
					arg2 = *((int64 *)peraggstate->distinctArr + i);
				else
					arg2 = PointerGetDatum((char *) peraggstate->distinctArr + i * ele_size);

				if (DatumGetInt32(FunctionCall2Coll(&peraggstate->distinct_cmpfn,
			...
			if (peraggstate->distinctCount == peraggstate->distinctSize)
			{
				...
				peraggstate->distinctArr = (int64 *) repalloc(peraggstate->distinctArr,
						ele_size * peraggstate->distinctSize);
			}
			if (peraggstate->distinct_typbyval)
				*((int64 *)peraggstate->distinctArr + peraggstate->distinctCount) = fcinfo->arg[1];
			else
				memcpy((char *) peraggstate->distinctArr + peraggstate->distinctCount * ele_size,
						(char *) fcinfo->arg[1], ele_size);
		}
		else if (peraggstate->distinct_typlen == -1)
		{
			uint64 len, tmp_len = 0;
			int8 *arr = (int8 *) peraggstate->distinctArr;
			int8 *arg = (int8 *) DatumGetPointer(fcinfo->arg[1]);
			int64 varlen = VARSIZE_ANY(fcinfo->arg[1]);
	...
	else if (frameOptions & FRAMEOPTION_START_OFFSET)
	{
		...
		else if (frameOptions & FRAMEOPTION_RANGE)
		{
			...
			while (!TupIsNull(winstate->framehead_slot))
			{
				...
				{
					if (ORA_MODE && winstate->start_contain_vars)
					{
						ExprContext *econtext;
						Datum value;
						bool  isnull;
						...
						econtext->ecxt_scantuple = winstate->ss.ss_ScanTupleSlot;
	...
	else if (frameOptions & FRAMEOPTION_END_OFFSET)
	{
		...
		else if (frameOptions & FRAMEOPTION_RANGE)
		{
			...
			while (!TupIsNull(winstate->frametail_slot))
			{
				...
				{
					if (ORA_MODE && winstate->end_contain_vars)
					{
						ExprContext *econtext;
						Datum value;
						bool  isnull;
...
ExecWindowAgg(PlanState *pstate)
{
	...
	if (winstate->all_first)
	{
		...
		if ((frameOptions & FRAMEOPTION_START_OFFSET) &&
			(PG_MODE || !winstate->start_contain_vars))
		{
		...
		if ((frameOptions & FRAMEOPTION_END_OFFSET) &&
			(PG_MODE || !winstate->end_contain_vars))
		{
...
ExecInitWindowAgg(WindowAgg *node, EState *estate, int eflags)
{
	 ...
	 */
	winstate->first_part_slot = ExecInitExtraTupleSlot(estate, NULL);
	winstate->agg_row_slot = ExecInitExtraTupleSlot(estate, NULL);
	winstate->temp_slot_1 = ExecInitExtraTupleSlot(estate, NULL);
	winstate->temp_slot_2 = ExecInitExtraTupleSlot(estate, NULL);
	...
	if (frameOptions & FRAMEOPTION_RANGE)
	{
		if (((frameOptions & FRAMEOPTION_START_CURRENT_ROW) &&
					node->ordNumCols != 0) ||
				(frameOptions & FRAMEOPTION_START_OFFSET))
			winstate->framehead_slot = ExecInitExtraTupleSlot(estate, NULL);
		if (((frameOptions & FRAMEOPTION_END_CURRENT_ROW) &&
					node->ordNumCols != 0) ||
				(frameOptions & FRAMEOPTION_END_OFFSET))
			winstate->frametail_slot = ExecInitExtraTupleSlot(estate, NULL);
	}
	...
	if (ORA_MODE)
	{
		winstate->start_contain_vars = node->start_contain_vars;
		winstate->end_contain_vars = node->end_contain_vars;
	}
	else
	{
		winstate->start_contain_vars = false;
		winstate->end_contain_vars = false;
	}
				  ...
				  WindowStatePerAgg peraggstate)
{
	...
	if (wfunc->windistinct)
	{
		...
		Oid			argtyp = wfunc->winaggargtype;
		...
		cmp_func_oid = get_opfamily_proc(opfamily, argtyp, argtyp, BTORDER_PROC);
		...
		peraggstate->distinct_typbyval = typeform->typbyval;
...
static Datum
GetAggInitVal(Datum textInitVal, Oid transtype, Oid aggOwner, Oid inputcollid,
			  WindowAggState *winstate, WindowStatePerAgg peraggstate)
{
	Oid			typinput,
				typioparam;
	char	   *strInitVal;
	...
	/* opentenbase_ora Agg function gets initval from initialization function */
	if (ORA_MODE &&
		pg_strncasecmp(strInitVal, INITVAL_MAGIC_HEADER,
					   strlen(INITVAL_MAGIC_HEADER)) == 0)
	{
		...
		initfn_oid = atooid(strInitVal + strlen(INITVAL_MAGIC_HEADER));
		if (!OidIsValid(initfn_oid))
			elog(ERROR, "unrecognized Agg init function OID: %d", (int) initfn_oid);

		/*
		 * Check that aggregate owner has permission to call
		 * initialization function.
		 */
		aclresult = pg_proc_aclcheck(initfn_oid, aggOwner,
										ACL_EXECUTE);
		if (aclresult != ACLCHECK_OK)
			aclcheck_error(aclresult, ACL_KIND_PROC,
							get_func_name(initfn_oid));
		InvokeFunctionExecuteHook(initfn_oid);

		/* Execute initialization function */
		fmgr_info(initfn_oid, &finfo);
		build_aggregate_initfn_expr(transtype,
									inputcollid,
									initfn_oid,
									&initfnexpr);
		fmgr_info_set_expr((Node *) initfnexpr, &finfo);

		InitFunctionCallInfoData(fcinfo, &finfo, 1,
									inputcollid,
									(void *) winstate, NULL);
		fcinfo.argnull[0] = true;
		initVal = FunctionCallInvoke(&fcinfo);

		/* ODCI style final function has 3 parameters. */
		peraggstate->numFinalArgs = 3;
	} else
	{
		/*
		 * Regular window Agg function gets initval from
		 * parentheses-quoted plain text.
		 */
...
/*
 * Implementation of window_ratio_to_report
 * on the first time through, accumulate all datum in the partition, store
 * sum, then div each value in current pos. Consider zero as NULL.
 */
Datum
WinRatioToReportImpl(WindowObject winobj, bool *isnull)
{
	WindowAggState *winstate = winobj->winstate;
	Datum sum = 0;
	Datum current;
	...
	*isnull = true;
	...
	if (winstate->currentpos)
		sum = winobj->temp_value;
	else
	{
		oldcontext = MemoryContextSwitchTo(queryContext);
		sum = DirectFunctionCall1(int8_numeric, Int64GetDatum(0));
		MemoryContextSwitchTo(oldcontext);
	}

	/* first time through, accumulate all values in this partition */
	if (winstate->currentpos == 0)
	{
		int64  i;
		Datum  datum;
		...
		for (i = 0; i < WinGetPartitionRowCount(winobj); i++)
		{
			datum = WinGetFuncArgInPartition(winobj, 0, i, WINDOW_SEEK_HEAD,
											 false, &null, NULL);
			if (!null)
			{
				oldcontext = MemoryContextSwitchTo(queryContext);
				sum = DirectFunctionCall2(numeric_add, sum, datum);
				MemoryContextSwitchTo(oldcontext);
			}
		}
		winobj->temp_value = sum;
	}

	if (numeric_is_zero_fast(DatumGetNumeric(sum)))
		return 0;

	current = WinGetFuncArgInPartition(winobj, 0, 0, WINDOW_SEEK_CURRENT,
									   false, isnull, NULL);

	if (*isnull)
		return 0;

	return DirectFunctionCall2(numeric_div, current, sum);
}
