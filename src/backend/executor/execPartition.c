...
ExecSetupPartitionTupleRouting(ModifyTableState *mtstate, Relation rel)
{
	...
	if (mtstate && mtstate->operation == CMD_UPDATE)
	{
		...
		num_update_rri = list_length(node->resultRelations);
				  ...
				  TupleTableSlot *slot, EState *estate)
{
	...
	while (true)
	{
		...
		if (myslot != NULL && map != NULL)
			slot = execute_attr_map_slot(map, slot, myslot, NULL);
...
ExecInitPartitionInfo(ModifyTableState *mtstate,
					  ...
					  PartitionTupleRouting *proute,
					  EState *estate, int partidx, Oid oldPartitionOid)
{
	...
	Oid childOid = proute->partition_oids[partidx];

	if (oldPartitionOid != InvalidOid)
		childOid = oldPartitionOid;
	partrel = heap_open(childOid, RowExclusiveLock);
	...
	if (node && node->withCheckOptionLists != NIL)
	{
		 ...
		 */
		Assert(node->operation == CMD_INSERT || node->operation == CMD_UPDATE || node->operation == CMD_MERGE);
		...
		Assert(node->operation != CMD_MERGE ||
				(list_length(node->withCheckOptionLists) ==
				list_length(node->resultRelations)));
