...
#ifdef __OPENTENBASE_C__
	node->typlen = sizeof(bool);
	node->typbyval = true;
	node->typid = BOOLOID;
#endif
	/* bein ora_compatible */
	if (ORA_MODE && subplan->subLinkType == INSERT_SUBLINK)
		elog(ERROR, "INSERT subplans should not be executed via ExecSubPlan");
	/* end ora_compatible */
				...
				bool *isNull)
{
	...
	/* scalar subquery parameters */
	Datum	rc_args[RESULTCACHE_MAXKEYS];
	bool	rc_argnull[RESULTCACHE_MAXKEYS];
	int		i = 0;
	short	rc_narg = 0;
	Oid		*rc_argtypes = NULL;
	bool	cachable = false;
	ResultCache			*rc = NULL;
	ResultCacheValue	*rc_result = NULL;
	...
	cachable = SubLinkResultCachable(subplan->subLinkType, subplan->args, &rc_narg, &rc_argtypes);
	...
	if (cachable)
	{
		rc = GetScalarSubResultCache(subplan->plan_id, planstate->state);
		rc_result = SearchResultCacheInternal(rc, rc_narg, rc_args, rc_argnull);
		pfree(rc_argtypes);
		if (rc_result != NULL)
		{
			result = rc_result->value;
			*isNull = rc_result->valuenull;
	...
	*isNull = false;
...
#ifdef __OPENTENBASE_C__
			node->typlen = TupleDescAttr(tdesc, 0)->attlen;
			node->typbyval = TupleDescAttr(tdesc, 0)->attbyval;
			node->typid = TupleDescAttr(tdesc, 0)->atttypid;
#endif
	...
	{
#ifdef __OPENTENBASE_C__
		if (OidIsValid(get_element_type(subplan->firstColType)))
		{
			get_typlenbyval(astate->arraystate->element_type, &node->typlen, &node->typbyval);
		}
		else
		{
			node->typlen = astate->scalarstate->typlen;
			node->typbyval = astate->scalarstate->typbyval;
		}
#endif
	...
	if (rc)
	{
		(void)ResultCacheCreateEntry(planstate->state->es_query_cxt, 
									&planstate->state->es_rc_total_memory, 
									rc, rc_args, rc_argnull, result,
									*isNull);
	}
...
ExecSetParamPlan(SubPlanState *node, ExprContext *econtext)
{
	...
	if (ORA_MODE && subLinkType == INSERT_SUBLINK)
		elog(ERROR, "INSERT subplans should not be executed via ExecSetParamPlan");
	/* end ora_compatible */
...
ExecReScanSetParamPlan(SubPlanState *node, PlanState *parent)
{
	...
	foreach(l, subplan->setParam)
	{
		...
		/* ora_compatible */
		if (subplan->subLinkType != CTE_SUBLINK && (PG_MODE ||
			subplan->subLinkType != INSERT_SUBLINK))
			prm->execPlan = node;
