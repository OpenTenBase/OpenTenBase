	...
	foreach(lc, node->lr_arowMarks)
	{
		...
		*testTuple = NULL;
		...
		if (erm->relation->rd_rel->relkind == RELKIND_FOREIGN_TABLE)
		{
			...
			if (erm->waitPolicy == LockWaitBlock && erm->waitTimeout > 0)
				ereport(ERROR,
						(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
						 errmsg("cannot set timeout for a row lock in foreign table \"%s\"",
								RelationGetRelationName(erm->relation))));
		...
		test = heap_lock_tuple_with_timeout(erm->relation,
											&tuple,
		...
		switch (test)
		{
			...
			case HeapTupleUpdated:
				...
				copyTuple = EvalPlanQualFetchWithTimeout(estate,
														 erm->relation,
														 lockmode,
														 erm->waitPolicy,
														 erm->waitTimeout,
...
ExecInitLockRows(LockRows *node, EState *estate, int eflags)
{
	...
	foreach(lc, node->rowMarks)
	{
		 ...
		 * all DNs. However, only one DN needs to return the data. In this case,
		 * the first DN is selected.
		 */
		if (IS_PGXC_DATANODE && erm->strength != LCS_NONE && erm->relation != NULL &&
			RelationIsReplication(erm->relation) && PGXCNodeId != 1)
			estate->es_not_send_tuple = true;
