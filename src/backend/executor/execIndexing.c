					  ...
					  List *arbiterIndexes)
{
	...
	for (i = 0; i < numIndices; i++)
	{
		...
		if (!indexInfo->ii_ReadyForInserts)
			continue;
		/* If the index is global, ignore it */
		if (OidIsValid(indexInfo->gindex_oid))
			continue;
...
ExecCheckIndexConstraints(TupleTableSlot *slot,
						  ...
						  List *arbiterIndexes, bool *isStash)
{
	ResultRelInfo *resultRelInfo;
	int			i;
	int			numIndices;
	...
	for (i = 0; i < numIndices; i++)
	{
		...
		satisfiesConstraint = check_exclusion_or_unique_constraint(
			heapRelation, indexRelation, indexInfo, &invalidItemPtr, values, isnull, estate,
