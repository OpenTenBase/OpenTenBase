...
bool	disable_tochar_simplify = false;
...
ExecInitExpr(Expr *node, PlanState *parent)
{
	...
	if (ORA_MODE)
		state->steps[state->steps_len - 1].restypmod = &state->restypmod;
...
enum CMPTYPE {NONECMP, CHARCMP, INT2CMP, INT4CMP, INT8CMP, FLOAT4CMP, FLOAT8CMP};

/*
 *		ExecBuildRIDProjectionInfo
 *
 * Build a ProjectionInfo node for evaluating the given targetRelids in the given
 * econtext, and storing the result into the tuple slot.
 */
ProjectionInfo *
ExecBuildRIDProjectionInfo(ProjectionInfo *projInfo,
						List *targetRelids,
						TupleTableSlot *slot,
						PlanState *parent)
{
	...
	projInfo->pi_rid_state.tag.type = T_ExprState;
	state = &projInfo->pi_rid_state;
	state->expr = (Expr *) targetRelids;
	state->parent = parent;
	...
	projInfo->pi_rid_projection = true;
	return projInfo;
}
						  ...
						  PlanState *parent)
{
	...
	/*
	 * Examine the targetList to see how many non-junk columns there are,
	...
	/*
	 * Special for row table in row-clomun mixed partition table.
	 * Caller must make sure the target list has been checked by
	 * ExecCheckPlanOutput already.
	 */
	if (targetColnos == NULL)
	{
		if (nAssignableCols != relDesc->natts)
			elog(ERROR, "subplan target list is out of order");

		deform.last_outer = nAssignableCols;
		ExecPushExprSlots(state, &deform);

		for (attnum = 1; attnum <= relDesc->natts; attnum++)
		{
	...
	for (attnum = relDesc->natts; attnum > 0; attnum--)
	{
				...
				Datum *resv, bool *resnull)
{
	...
	switch (nodeTag(node))
	{
		...
		case T_Const:
			{
				...
				if (ORA_MODE && con->constisnull && con->constlen == NULL_TYPE_STR_LEN &&
					IS_STRING_TYPE(con->consttype))
				{
					scratch.d.constval.typmod = NULL_TYPE_STR_LEN;
					scratch.d.constval.typoid = con->consttype;
				}
				ExprEvalPushStep(state, &scratch);
				break;
			}
		...
		case T_FuncExpr:
			{
				...
				if (enable_scalar_subquery_cache && 
					...
					func_result_cachable(func->funcid))
				{
					int			nargs = 0;
					Oid			rettype;
					Oid			*argtypes = NULL;
					...
					/* NULL means that the function may have been deleted in concurrent scenarios */
					if (name == NULL)
					{
						break;
					}
					rettype = get_func_signature(func->funcid, &argtypes, &nargs);

					if (state->parent && state->parent->state)
					{
						RecordScalarSubInResultCache(func->funcid, name, nargs, 
													argtypes, rettype, 
													state->parent->state);
					}
		...
		case T_SubPlan:
			{
				...
				old = MemoryContextSwitchTo(state->parent->state->es_query_cxt);
				if (!state->parent)
				...
				if (SubLinkResultCachable(subplan->subLinkType, subplan->args, &nargs, &argtypes))
				{
					RecordScalarSubInResultCache((Oid)subplan->plan_id, 
													subplan->plan_name,
													nargs, 
													argtypes, 
													exprType((Node*)subplan), 
													state->parent->state);
					pfree(argtypes);
				}
		...
		case T_CoerceViaIO:
			{
                ...
                if (ORA_MODE && (iocoerce->resulttype == NUMERICOID || iocoerce->resulttype == RECORDOID))
                    typmod = iocoerce->resulttypmod;
                else
                    typmod = -1;
				/*
				...
				fcinfo_in->arg[2] = Int32GetDatum(typmod);
		...
		case T_NextValueExpr:
			{
				...
				/* Used in numeric(x,0) about identity to convert numeric type. */
				if (nve->typeId == NUMERICOID)
					scratch.d.nextvalueexpr.typmod = nve->typmod;
		...
		case T_TargetEntry:
			{
				...
				ExecInitExprRec(tle->expr, state, resv, resnull);
				break;
			}
#ifdef _PG_ORCL_
		case T_RownumExpr:
		...
		case T_LevelExpr:
			{
				scratch.opcode = EEOP_LEVELEXPR;
				ExprEvalPushStep(state, &scratch);
				break;
			}
		case T_CBIsLeafExpr:
			{
				scratch.opcode = EEOP_CBISLEAFEXPR;
				ExprEvalPushStep(state, &scratch);
				break;
			}
		case T_CBIsCycleExpr:
			{
				scratch.opcode = EEOP_CBISCYCLEEXPR;
				ExprEvalPushStep(state, &scratch);
				break;
			}
		case T_CBPathExpr:
			{
				ExecInitExprRec((Expr * )castNode(CBPathExpr, node)->expr,
								state, resv, resnull);
				break;
			}
		case T_PriorExpr:
		case T_CBRootExpr:
			{
				/*
				 * Prior should be transformed to Param. And CBRoot was handled
				 * in set_plan_refs, it was transformed to scan inner(root)
				 * slot of connect-by node.
				 * 
				 * All of them shouldn't be here. 
				 */
				Assert(0);
				/* fallthrough */
			}
			 ...
			 Oid inputcollid, ExprState *state)
{
	...
	ListCell   *lc;
#ifdef _PG_ORCL_
	bool	iswith_func = false;

	iswith_func = get_fn_from_withfunc((fmNodePtr) node);
	if (!iswith_func)
	{
#endif
		/* Check permission to call function */
...
#ifdef _PG_ORCL_
	if (iswith_func)
		fmgr_info_withfuncs(InvalidOid, flinfo, (fmNodePtr) node);
	else
	{
#endif
		/* Set up the primary fmgr lookup information */
	...
	foreach(lc, args)
	{
		...
		if (IsA(arg, Const))
		{
			...
			if (ORA_MODE && con->constlen == NULL_TYPE_STR_LEN)
				fcinfo->argtypmod[argno] = NULL_TYPE_STR_LEN;
		}
		else
		{
			ExecInitExprRec(arg, state,
							&fcinfo->arg[argno], &fcinfo->argnull[argno]);
			if (ORA_MODE && state->steps_len > 0)
				state->steps[state->steps_len - 1].restypmod = &fcinfo->argtypmod[argno];
		}
		argno++;
	}
...
static bool contain_system_var_walker(Node *node, void *context);

/* Check if the expression contains system column */
bool
contain_system_var(Node *node)
{
	return contain_system_var_walker(node, NULL);
}

static bool
contain_system_var_walker(Node *node, void *context)
{
	if (node == NULL)
		return false;
	...
	return expression_tree_walker(node, contain_system_var_walker, context);
}
#endif
				 ...
				 ExprState *state, Datum *resv, bool *resnull)
{
	...
	if (exprType((Node *) aref) == aref->refarraytype)
		arefstate->multidim = true;
	...
	foreach(lc, aref->refupperindexpr)
	{
		...
		if (ORA_MODE)
			arefstate->upperindextype[i] = exprType((Node *) e);
				  ...
				  bool doSort, bool doHash, bool nullcheck)
{
	...
	for (transno = 0; transno < aggstate->numtrans; transno++)
	{
		 ...
		 *
		 * opentenbase_ora retains the NULL datum in its Agg function inputs, which
		 * is why we do not perform strict input checks when using the KEEP
		 * syntax.
		 */
		if (trans_fcinfo->flinfo->fn_strict && 
				pertrans->numTransInputs > 0 && 
				!(ORA_MODE && pertrans->aggref->aggkeep))
		{
