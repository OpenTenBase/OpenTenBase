...
/* guc variable */
bool innerhashjoin_optimizer = true;
#endif
...
#ifdef __OPENTENBASE_C__
static bool ExecHashJoinBloomFilter(HashJoinState *hjstate,
									HashJoinTable hashtable,
									uint32 hashvalue, bool parallel);
#endif
...
ExecHashJoinImpl(PlanState *pstate, bool parallel)
{
	...
	if (IS_PGXC_COORDINATOR && !node->is_cn_exec)
	{
		int st_userid = InvalidOid;
		char *query_id = pgstat_report_queryid(&st_userid);
		
		/* 
			1) super user execute some maintenance commands through direct exec on CN, filter them out
			2) normal user execute metadata join, filter them out
		*/
		if (!superuser_arg(st_userid) && pstate->plan->remote_flag != 0)
			elog(LOG,"early warning queryid %s pid %d exec HashJoin on CN query detail: %s", 
						query_id ? query_id: "", MyProcPid, 
						node->js.ps.state->es_sourceText ? node->js.ps.state->es_sourceText : "");
		
		node->is_cn_exec = true;
	}
	...
	/* if we need to run stable node in case of disconnect */
	forceouter = outerNode->plan->extParam == NULL &&
				 hashNode->ps.plan->extParam != NULL &&
				 outerNode->plan->remote_flag;
	forceinner = outerNode->plan->extParam != NULL &&
				 hashNode->ps.plan->extParam == NULL &&
				 hashNode->ps.plan->remote_flag;
#endif
	...
	for (;;)
	{
		...
		switch (node->hj_JoinState)
		{
			case HJ_BUILD_HASHTABLE:
				...
				if (HJ_FILL_INNER(node) || forceinner)
				{
				...
				else if (HJ_FILL_OUTER(node) || forceouter ||
				...
				hashtable = ExecHashTableCreate(hashNode,
												...
												HJ_FILL_INNER(node) || hashJoin->nonequijoin,
												jointype);
#else
				hashtable = ExecHashTableCreate(hashNode,
												node->hj_HashOperators,
				...
				{
					if (forceouter && !node->hj_OuterInited)
					{
						node->hj_OuterInited = true;
						(void) ExecProcNode(outerNode);
					}
				...
				if (parallel)
				{
					...
					if (BarrierPhase(build_barrier) == PHJ_BUILD_HASHING_OUTER)
					{
						...
						/* Need to wait all workers if remote fragment exists */
			...
			case HJ_NEED_NEW_OUTER:
				...
				if (hashtable->curbatch == 0 &&
					ExecHashJoinBloomFilter(node, hashtable, hashvalue, parallel))
				{
					continue;
				}
#endif
			...
			case HJ_SCAN_BUCKET:
				...
				{
					if ((node->js.jointype == JOIN_LEFT_SEMI_SCALAR ||
						 node->js.jointype == JOIN_SEMI_SCALAR) &&
					 ...
					 *
					 * Do not support parallel temporarily.
					 */
					if (jointype == JOIN_ANTI_RIGHT || jointype == JOIN_SEMI_RIGHT)
					{
						if (node->hj_PreTuple)
						{
							node->hj_PreTuple->next.unshared =
									node->hj_CurTuple->next.unshared;
						}
						else if (node->hj_CurSkewBucketNo != INVALID_SKEW_BUCKET_NO)
						{
							hashtable->skewBucket[node->hj_CurSkewBucketNo]->tuples =
									node->hj_CurTuple->next.unshared;
						}
						else
						{
							hashtable->buckets.unshared[node->hj_CurBucketNo] =
									node->hj_CurTuple->next.unshared;
						}
					...
					if (otherqual == NULL || ExecQual(otherqual, econtext))
					{
						...
						elog(DEBUG5, "Hash Join return matching slot %zu",
							 node->matched_tuples++);
...
ExecHashJoin(PlanState *pstate)
{
	...
	if (unlikely(TupIsNull(result)))
		ExecEagerFreeHashJoin(pstate);
...
ExecInitHashJoin(HashJoin *node, EState *estate, int eflags)
{
	...
	hjstate->hj_Eagerfree = ((eflags & EXEC_FLAG_EAGER_FREE) != 0);
	...
	jit_compile_hashjoin(hjstate);

	/*
	 * If there are non-replicated tables involved in the join, it is necessary
	 * to send tuples to the upper-level logic, and duplicate data will not occur.
	 */
	if (estate->es_not_send_tuple)
	{
		ListCell *cell;

		foreach(cell, estate->es_range_table)
		{
			RangeTblEntry *rte = (RangeTblEntry*)lfirst(cell);
			if (rte->rtekind == RTE_RELATION)
			{
				Relation rel = RelationIdGetRelation(rte->relid);
				if (!RelationIsReplication(rel))
				{
					estate->es_not_send_tuple = false;
					RelationClose(rel);
					break;
				}
				RelationClose(rel);
...
ExecEndHashJoin(HashJoinState *node)
{
	...
	if (node->hj_HashTable)
	{
		...
		{
				BlockBloomFilter filter = hashtable->bloomFilter;
				elog(DEBUG1, "bloom filter nbatch=%d lookups=%lu matches=%lu eliminated=%lu%%",
							node->hj_HashTable->nbatch, filter->nlookups, filter->nmatches,
							100 - (100 * filter->nmatches) / Max(1,filter->nlookups));
		}
	...
	if (curbatch == 0)			/* if it is the first pass */
	{
		...
		while (!TupIsNull(slot))
		{
			...
			if (ExecHashGetHashValue(hashtable, econtext,
									 ...
									 true,	/* outer tuple */
									 HJ_FILL_OUTER(hjstate) || ((HashJoin*)hjstate->js.ps.plan)->nonequijoin,
									 hashvalue))
#else
			if (ExecHashGetHashValue(hashtable, econtext,
									 hjstate->hj_OuterHashKeys,
					  ...
					  BufFile **fileptr)
{
	...
	if (file == NULL)
	{
		...
		file = BufFileCreateTemp("HashJoin", false);
		*fileptr = file;
	}
...
ExecReScanHashJoin(HashJoinState *node)
{
	...
	if (node->hj_HashTable != NULL)
	{
		if (node->hj_HashTable->nbatch == 1 &&
			...
			node->js.jointype != JOIN_SEMI_RIGHT &&
			node->js.jointype != JOIN_ANTI_RIGHT)
		{
			/*
...
ExecHashJoinInitializeDSM(HashJoinState *state, ParallelContext *pcxt)
{
	...
	bool		waitAll = state->js.ps.plan->remote_flag == 1;
	HashState  *hashNode;
	...
	pstate->bloomfilter = InvalidDsaPointer;
	LWLockInitialize(&pstate->filterlock,
					 LWTRANCHE_PARALLEL_WORKER_DSA);
	LWLockInitialize(&pstate->lock,
					 LWTRANCHE_PARALLEL_HASH_JOIN);
	BarrierInit(&pstate->build_barrier, 0, (waitAll ? pstate->nparticipants : 0));
	BarrierInit(&pstate->grow_batches_barrier, 0, (waitAll ? pstate->nparticipants : 0));
	BarrierInit(&pstate->grow_buckets_barrier, 0, (waitAll ? pstate->nparticipants : 0));
...
ExecHashJoinReInitializeDSM(HashJoinState *state, ParallelContext *cxt)
{
	...
	BarrierInit(&pstate->build_barrier, 0,
				state->js.ps.plan->remote_flag == 1 ? pstate->nparticipants : 0);
}
...
/*
 * Parallel hashjoin uses the barrier mechanism to work collaboratively between
 * multiple workers and leaders (if leader_participate is true), but due to the
 * current design of asynchronously receiving data in Forward Node, the barrier
 * may wait for a potential participant who has not yet attached. Unless this
 * worker has not been forked at all (probably due to the limitation of
 * max_parallel_workers), so we need to update max_participants here to notify
 * the waiting participants to continue.
 */
void
ExecHashJoinAdjustDSM(HashJoinState *state, ParallelContext *pcxt)
{
	int			plan_node_id = state->js.ps.plan->plan_node_id;
	int			max_participants = pcxt->nworkers_launched;
	ParallelHashJoinState *pstate;

	if (state->js.ps.plan->remote_flag != 1)
		return;
	...
	BarrierRefreshParticipants(&pstate->build_barrier, max_participants);
}

#ifdef __OPENTENBASE_C__
void
ExecEagerFreeHashJoin(PlanState *pstate)
{
	HashJoinState *node = castNode(HashJoinState, pstate);

	if (node->hj_Eagerfree && node->hj_HashTable)
	{
		HashState  *hashNode = (HashState *) innerPlanState(node);

		ExecShutdownHash(hashNode);
...
/*
 * Fast path to skip tuples by the BloomFilter we built.
 */
static bool
ExecHashJoinBloomFilter(HashJoinState *hjstate, HashJoinTable hashtable,
						uint32 hashvalue, bool parallel)
{
	ParallelHashJoinState  *phj_state = hashtable->parallel_state;

	if (!parallel && hashtable->bloomFilter)
	{
		BlockBloomFilter filter = hashtable->bloomFilter;
		if (!BlockBloomFilterFind(filter, hashvalue,
								  &filter->nlookups, &filter->nmatches))
		{
		...
		/*
		 * Give up using bloom filter if it's not efficient. If it's hybird
		 * hashjoin, then we still enable bloom filter since it could early
		 * filter it without spilling to disk.
		 */
		if (hashtable->nbatch == 1 && !BlockBloomFilterIsEfficient(filter))
		{
			elog(DEBUG1, "Bloom filter abandoned, nbatch=%d lookups=%lu"
				 "matches=%lu eliminated=%lu%%", hjstate->hj_HashTable->nbatch,
				 filter->nlookups, filter->nmatches,
				 100 - (100 * filter->nmatches) / Max(1,filter->nlookups));
			pfree(filter);
	...
	else if (phj_state && phj_state->bloomfilter != InvalidDsaPointer)
	{
		BlockBloomFilter mergedFilter =
				(BlockBloomFilter) dsa_get_address(hashtable->area,
		 										   phj_state->bloomfilter);
		BlockBloomFilter filter = hashtable->bloomFilter;

		if (!BlockBloomFilterFind(mergedFilter, hashvalue,
				 	 	 	 	  &filter->nlookups, &filter->nmatches))
		{
			/* no matches; check for possible outer-join fill */
			hjstate->hj_JoinState = HJ_FILL_OUTER_TUPLE;
		...
		/*
		 * TODO: When checking if it's efficient in parallel mode, we should
		 * check the final Filter. But the lookup/matched counter update
		 * could lead to contention. So we did not check efficiency here.
		 */
	}
