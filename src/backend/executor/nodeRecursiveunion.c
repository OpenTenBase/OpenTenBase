...
ExecInitRecursiveUnion(RecursiveUnion *node, EState *estate, int eflags)
{
	...
	/* inner should not eager free */
	eflags &= ~EXEC_FLAG_EAGER_FREE;
	innerPlanState(rustate) = ExecInitNode(innerPlan(node), estate, eflags);

    node->isDuplicateRmByAddr = false;
...
{
    /*
     * duplicate removal by physical address, it should reset numCols to zero and free the memory
     * which allocate in function ExecInitRecursiveUnion
     */
    RecursiveUnion *plan = (RecursiveUnion *) node->ps.plan;
    if (plan->isDuplicateRmByAddr)
    {
        plan->numCols = 0;
        pfree(plan->dupColIdx);
        plan->dupColIdx = NULL;
        pfree(plan->dupOperators);
        plan->dupOperators = NULL;
    }
