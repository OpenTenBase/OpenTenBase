...
MultiExecHash(HashState *node)
{
	...
	{	
		InstrStartFragment(node->ps.instrument, &(node->ps.state->executor_starttime));
	...
	if (node->ps.instrument)
	{
		InstrStopFragment(node->ps.instrument, &(node->ps.state->executor_starttime));
...
MultiExecPrivateHash(HashState *node)
{
	...
	for (;;)
	{
								 ...
								 &hashvalue))
		{
			 ...
			 * bloom filter.
			 *
			 * We may still not build any of the two things (HLL, bloom),
			 * e.g. if we decide not to based on estimates or at runtime.
			 */
			if (hashtable->bloomFilter)
			{
				BlockBloomFilterInsert(hashtable->bloomFilter, hashvalue);
			}
#endif
...
MultiExecParallelHash(HashState *node)
{
	...
	int			i;
	BlockBloomFilter mergeFilter = NULL;
	bool		elected;
		...
		case PHJ_BUILD_HASHING_INNER:
			...
			for (;;)
			{
										 ...
										 &hashvalue))
				{
					...
					/* Add hash value to local bloom filter */
					if (hashtable->bloomFilter)
					{
						BlockBloomFilterInsert(hashtable->bloomFilter, hashvalue);
					}
			...
			/* Merge local bloom filter result to parallel final filter */
			if (hashtable->bloomFilter)
			{
				mergeFilter = (BlockBloomFilter) dsa_get_address(hashtable->area,
										pstate->bloomfilter);

				LWLockAcquire(&pstate->filterlock, LW_EXCLUSIVE);
				MergeParallelBloomFilter(mergeFilter, hashtable->bloomFilter);
				LWLockRelease(&pstate->filterlock);
			}
			 ...
			 */
			elected = BarrierArriveAndWait(build_barrier,
...
void
HashFuncAssign(Oid functionId, FmgrInfo *finfo)
{
	if (!enable_newhash)
	{
	...
	{
	case F_HASHINT4:
		finfo->fn_addr = hashint4new;
		break;
	case F_HASHINT8:
		finfo->fn_addr = hashint8new;
		break;
	case F_HASHINT2:
		finfo->fn_addr = hashint2new;
		break;
	case F_HASHOID:
		finfo->fn_addr = hashoidnew;
		break;
	case F_HASHFLOAT4:
		finfo->fn_addr = hashfloat4new;
		break;
	case F_HASHFLOAT8:
		finfo->fn_addr = hashfloat8new;
		break;
	case F_JSONB_HASH:
		finfo->fn_addr = jsonb_hash_new;
		break;
	case F_ROWID_HASH:
		finfo->fn_addr = rowid_hash_new;
		break;
	case F_HASHCHAR:
		finfo->fn_addr = hashcharnew;
		break;
	case F_HASHNAME:
		finfo->fn_addr = hashnamenew;
		break;
	case F_HASHTEXT:
		finfo->fn_addr = hashtextnew;
		break;
	case F_HASHVARLENA:
		finfo->fn_addr = hashvarlenanew;
		break;
	case F_TIME_HASH:
		finfo->fn_addr = time_hash_new;
		break;
	case F_TIMESTAMP_HASH:
		finfo->fn_addr = timestamp_hash_new;
		break;
	case F_INTERVAL_HASH:
		finfo->fn_addr = interval_hash_new;
		break;
	case F_TIMETZ_HASH:
		finfo->fn_addr = timetz_hash_new;
		break;
	case F_HASH_NUMERIC:
		finfo->fn_addr = hash_numeric_new;
		break;
	case F_UUID_HASH:
		finfo->fn_addr = uuid_hash_new;
		break;
	case F_HASHBPCHAR:
		finfo->fn_addr = hashbpcharnew;
		break;
	default:
		fmgr_info(functionId, finfo);
		break;
...
ExecHashTableCreate(HashState *state, List *hashOperators,
					bool keepNulls, JoinType jointype)
{
	Hash	   *node;
	HashJoinTable hashtable;
	Plan	   *outerNode;
	 ...
	 */
	if (jointype != JOIN_ANTI && jointype != JOIN_ANTI_RIGHT &&
	...
	{
		/*
		 * Building the bloom filter right away. Assume all rows are
		 * distinct, although it might be an inefficient bloom filter.
		 */
		hashtable->bloomFilter = BlockBloomFilterInit(outerNode->plan_rows,
													  BLOOM_ERROR_RATE);
	}
#endif
	...
	foreach(ho, hashOperators)
	{
		...
		if (!get_op_hash_functions(hashop, &left_hashfn, &right_hashfn))
			elog(ERROR, "could not find hash function for hash operator %u",
				 hashop);
		HashFuncAssign(left_hashfn, &hashtable->outer_hashfunctions[i]);
		HashFuncAssign(right_hashfn, &hashtable->inner_hashfunctions[i]);
	...
	if (hashtable->parallel_state)
	{
			...
			BarrierArriveAndWait(build_barrier, WAIT_EVENT_HASH_BUILD_ELECTING))
		{
...
#ifdef __OPENTENBASE_C__
			/* Build the shared bloom filter for final merge phase */
			if (jointype != JOIN_ANTI && jointype != JOIN_ANTI_RIGHT &&
				jointype != JOIN_LEFT && jointype != JOIN_FULL &&
				jointype != JOIN_LEFT_SEMI_SCALAR &&
				jointype != JOIN_LEFT_SEMI &&
			...
			{
				pstate->bloomfilter =
						SharedBloomFilterInit(outerNode->plan_rows,
											  BLOOM_ERROR_RATE, hashtable->area);
			}
#endif
		}
...
ExecHashIncreaseNumBatches(HashJoinTable hashtable)
{
	...
	{
		/*
		 * If we reach a point where all or no tuples have been exported, it
		 * means we've hit a limitation due to identical hash values causing
		 * overflow. In this case, increasing the number of batches (`nbatch`)
		 * won't help, as we can't further divide the group of tuples with the
		 * same hash value.
		 * The only option is to continue with the current setting and hope
		 * the server has enough RAM to handle the operation.
		 * But if the memory used exceeds the limit in this situation, we throw
		 * an error.
		 */
		if (hashtable->spaceUsed >
			work_mem * hash_mem_multiplier * 1024L * work_mem_limit_multiplier)
			elog(ERROR, "hash join exceed work_mem limit due to identical hash values");
		return;
	}
					 ...
					 uint32 *hashvalue)
{
	...
	foreach(hk, hashkeys)
	{
		...
		else
		{
			...
			if (hashfunctions[i].fn_oid == InvalidOid)
			{
				hkey = DatumGetUInt32(DirectFunctionCall1(hashfunctions[i].fn_addr, keyval));
			}
			else
			{
...
#ifdef __OPENTENBASE__
		/*
		 * For Right_Semi/Right_Anti join, we delete matched tuples in HashTable
		 * to make next matching faster, so pointer hj_PreTuple is designed to
		 * follow the hj_CurTuple and to help us to clear the HashTable.
		 */
		if (hjstate->js.jointype == JOIN_SEMI_RIGHT ||
			hjstate->js.jointype == JOIN_ANTI_RIGHT)
		{
...
ExecHashBuildSkewHash(HashJoinTable hashtable, Hash *node, int mcvsToUse)
{
	...
	{
		statsTuple = LocalStatisticGetAttrStats(node->skewTable, node->skewColumn, node->skewInherit);
	}
						 ...
						 ATTSTATSSLOT_VALUES | ATTSTATSSLOT_NUMBERS))
	{
		...
		for (i = 0; i < mcvsToUse; i++)
		{
			...
			if (hashfunctions[0].fn_oid == InvalidOid)
			{
				hashvalue = DatumGetUInt32(DirectFunctionCall1(hashfunctions[0].fn_addr,
													 sslot.values[i]));
			}
			else
			{
...
ExecHashEstimate(HashState *node, ParallelContext *pcxt)
{
	...
	/*
	 * We need to know the number of total workers to wait all of them finished
	 * during the hash build phase.
	 */
							 ...
							 HashJoinTable hashtable)
{
	...
	instrument->space_peak = Max(instrument->space_peak,
								 hashtable->spacePeak);
	instrument->rowsFiltered = hashtable->bloomFilter ?
			hashtable->bloomFilter->nlookups -
			hashtable->bloomFilter->nmatches : -1;
}
...
ExecParallelHashJoinSetUpBatches(HashJoinTable hashtable, int nbatch)
{
	...
	for (i = 0; i < hashtable->nbatch; ++i)
	{
		 ...
		 * up the Barrier.
		 */
		BarrierInit(&shared->batch_barrier, 0, pstate->build_barrier.max_parts);
		if (i == 0)
		{
...
ExecHashTableDetach(HashJoinTable hashtable)
{
	...
	if (pstate && BarrierPhase(&pstate->build_barrier) == PHJ_BUILD_RUNNING)
	{
		...
		if (BarrierArriveAndDetach(&pstate->build_barrier))
		{
			...
			/* Clean up shared bloom filter */
			if (DsaPointerIsValid(pstate->bloomfilter))
			{
				dsa_free(hashtable->area, pstate->bloomfilter);
