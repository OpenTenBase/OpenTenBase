...
#define MAX_NUM_SHARED_CTE 4096
#define MAX_NUM_BUF_FILE 2

#define NOTIFY_DONE   0x01
#define NOTIFY_DATA   0x02
...
void
ExecEagerFreeCteScan(PlanState *pstate)
{
	return;
}

/* ----------------------------------------------------------------
 *					Parallel Shared CteScan Support
 * ----------------------------------------------------------------
 */
void
ExecCteScanEstimate(CteScanState *node, ParallelContext *pcxt)
{
	return;
}

void
ExecCteScanInitializeDSM(CteScanState *node, ParallelContext *pcxt)
{
	elog(ERROR, "Parallel ctescan should be shared");
}

void
ExecCteScanReInitializeDSM(CteScanState *node, ParallelContext *pcxt)
{
	elog(ERROR, "Parallel ctescan do not support reinitialize");
}

void
ExecCteScanInitializeWorker(CteScanState *node, ParallelWorkerContext *pwcxt)
{
	return;
}
