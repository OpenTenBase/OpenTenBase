...
sql_fn_parser_setup(struct ParseState *pstate, SQLFunctionParseInfoPtr pinfo)
{
	...
	pstate->multisetRetOid = pinfo->multisetRetOid;
}
					 ...
					 bool lazyEvalOK)
{
	...
	foreach(lc1, queryTree_list)
	{
		...
		foreach(lc2, qtlist)
		{
			...
			if (queryTree->stmt_location >= 0 && queryTree->stmt_len >= 0 &&
				queryTree->stmt_len + queryTree->stmt_location <= strlen(fcache->src))
				stmt->queryString = pnstrdup(fcache->src + queryTree->stmt_location, queryTree->stmt_len);
...
init_sql_fcache(FmgrInfo *finfo, Oid collation, bool lazyEvalOK)
{
	...
	iswith_func = get_fn_from_withfunc((fmNodePtr) finfo->fn_expr);
#endif
	...
	{
		int     with_funcid = get_fnid_from_withfunc(finfo->fn_expr);

		procedureTuple
			= GetWithFunctionTupleById(get_fnnsp_from_fmgr(finfo), with_funcid);
	}
#endif
	...
	fcache->pinfo = prepare_sql_fn_parse_info(procedureTuple,
											  finfo->fn_expr,
											  collation);
#define MULISETEXPR_FOID (9123)
	if (finfo->fn_expr &&
		((FuncExpr *)(finfo->fn_expr))->funcid == MULISETEXPR_FOID)
		fcache->pinfo->multisetRetOid = ((FuncExpr *)(finfo->fn_expr))->funcresulttype;
	else
		fcache->pinfo->multisetRetOid = InvalidOid;
#undef MULISETEXPR_FOID
	/*
