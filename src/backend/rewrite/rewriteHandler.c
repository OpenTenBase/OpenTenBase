...
static List        *rewriteTargetListMergeInto(List *targetList, CmdType commandType,
                                               OverridingKind override, Relation target_relation,
                                               List **attrno_list);
static Query *rewriteTargetSubquery(Query *parsetree, Query *subquery, Relation view);
		...
		switch (rte->rtekind)
		{
			...
			case RTE_JOIN:
				...
				foreach(ll, rte->joinaliasvars)
				{
					...
					if (aliasitem)
						newaliasvars = lappend(newaliasvars, copyObject(aliasitem));
					else
						newaliasvars = lappend(newaliasvars, NULL);
				}
...
static void
delForeignTargetListUD(Query *parsetree, RangeTblEntry *target_rte,
					Relation target_relation)
{
	Var		   *var = NULL;
	const char *attrname;
	...
	if (ORA_MODE && target_relation->rd_rel->relkind == RELKIND_FOREIGN_TABLE)
	{
		/*
				  ...
				  bool forUpdatePushedDown)
{
	...
	if (rc != NULL)
		markQueryForLocking(rule_action, (Node *) rule_action->jointree,
							rc->strength, rc->waitPolicy, true, rc->waitTimeout);
					...
					bool pushedDown, int waitTimeout)
{
	...
	if (IsA(jtnode, RangeTblRef))
	{
		...
		{
			applyLockingClause(qry, rti, strength, waitPolicy, pushedDown, waitTimeout);
		...
		else if (rte->rtekind == RTE_SUBQUERY)
		{
			...
			markQueryForLocking(rte->subquery, (Node *) rte->subquery->jointree,
								strength, waitPolicy, true, waitTimeout);
		}
	...
	else if (IsA(jtnode, FromExpr))
	{
		...
		foreach(l, f->fromlist)
			markQueryForLocking(qry, lfirst(l), strength, waitPolicy, pushedDown, waitTimeout);
	}
	else if (IsA(jtnode, JoinExpr))
	{
		JoinExpr   *j = (JoinExpr *) jtnode;

		markQueryForLocking(qry, j->larg, strength, waitPolicy, pushedDown, waitTimeout);
		markQueryForLocking(qry, j->rarg, strength, waitPolicy, pushedDown, waitTimeout);
	}
	else
...
fireRIRrules(Query *parsetree, List *activeRIRs, bool forUpdatePushedDown)
{
	...
	if (ORA_MODE && parsetree->multi_inserts != NULL)
	{
		/* process sub-insert */
		ListCell	*l;

		foreach(l, parsetree->multi_inserts)
		{
			MultiInsertInto	*mii = lfirst_node(MultiInsertInto, l);
			ListCell	*ll;

			foreach(ll, mii->sub_intos)
			{
				Query	*subq = lfirst_node(Query, ll);
				Query	*new_subq;

				new_subq = fireRIRrules(subq, activeRIRs, false);
				if (new_subq->commandType != CMD_INSERT)
					elog(ERROR, "A not INSERT operation is generated for multiple insert during fire rule");
...
view_has_instead_trigger(Relation view, CmdType event)
{
	...
	int     i;
	bool trig_insert_instead_row = false;
	bool trig_update_instead_row = false;
	bool trig_delete_instead_row = false;
	...
	for (i = 0; i < trigDesc->numtriggers; i++)
	{
		...
		if (trigger.tgenabled != TRIGGER_DISABLED)
		{
			trig_insert_instead_row |=
					TRIGGER_TYPE_MATCHES(trigger.tgtype, TRIGGER_TYPE_ROW,
	...
	{
		case CMD_INSERT:
			if (trig_insert_instead_row)
				return true;
			break;
		case CMD_UPDATE:
			if (trig_update_instead_row)
				return true;
			break;
		case CMD_DELETE:
			if (trig_delete_instead_row)
				return true;
			break;
		case CMD_MERGE:
			if (trigDesc &&
			    (trigDesc->trig_insert_instead_row || trigDesc->trig_update_instead_row ||
			     trigDesc->trig_delete_instead_row))
				elog(ERROR, "MERGE not supported on join view or view with INSTEAD OF trigger.");
			break;
		default:
...
view_col_is_auto_updatable(RangeTblRef *rtr, TargetEntry *tle)
{
	...
	if (!IsA(var, Var) ||
		...
		var->varlevelsup != 0)
		return gettext_noop("%s columns that are not columns of their base relation are not updatable.");

	if (var->varattno < 0)
		return gettext_noop("%s columns that refer to system columns are not updatable.");

	if (var->varattno == 0)
		return gettext_noop("%s columns that return whole-row references are not updatable.");
 ...
 */
const char *
view_query_is_auto_updatable(Query *viewquery, CmdType cmdType)
{
	RangeTblRef *rtr;
	...
	if (viewquery->distinctClause != NIL)
		return gettext_noop("%s containing DISTINCT are not automatically updatable.");

	if (viewquery->groupClause != NIL || viewquery->groupingSets)
		return gettext_noop("%s containing GROUP BY are not automatically updatable.");

	if (viewquery->havingQual != NULL)
		return gettext_noop("%s containing HAVING are not automatically updatable.");

	if (viewquery->setOperations != NULL)
		return gettext_noop("%s containing UNION, INTERSECT, or EXCEPT are not automatically updatable.");

	if (viewquery->cteList != NIL)
		return gettext_noop("%s containing WITH are not automatically updatable.");

	if (viewquery->limitOffset != NULL || viewquery->limitCount != NULL)
		return gettext_noop("%s containing LIMIT or OFFSET are not automatically updatable.");

	if (viewquery->connectByExpr != NULL)
		return gettext_noop("%s containing CONNECT BY are not automatically updatable.");

	if (contain_rownum(viewquery->jointree->quals))
		return gettext_noop("%s containing ROWNUM are not automatically updatable.");
	/*
	...
	if (viewquery->hasAggs)
		return gettext_noop("%s that return aggregate functions are not automatically updatable.");

	if (viewquery->hasWindowFuncs)
		return gettext_noop("%s that return window functions are not automatically updatable.");

	if (viewquery->hasTargetSRFs)
		return gettext_noop("%s that return set-returning functions are not automatically updatable.");
	...
	if (list_length(viewquery->jointree->fromlist) != 1)
		return gettext_noop("%s that do not select from a single table or view are not automatically updatable.");
	...
	if ((cmdType != CMD_MERGE || base_rte->rtekind != RTE_SUBQUERY) &&
	    (base_rte->rtekind != RTE_RELATION ||
	     (base_rte->relkind != RELKIND_RELATION && base_rte->relkind != RELKIND_FOREIGN_TABLE &&
	      base_rte->relkind != RELKIND_VIEW && base_rte->relkind != RELKIND_PARTITIONED_TABLE)))
		return gettext_noop(
			"%s that do not select from a single table or view are not automatically updatable.");

	if (base_rte->tablesample)
		return gettext_noop("%s containing TABLESAMPLE are not automatically updatable.");
	...
	if (cmdType != CMD_DELETE)
	{
		...
		if (!found)
			return gettext_noop("%s that have no updatable columns are not automatically updatable.");
	}
...
#define ALL_EVENTS ((1 << CMD_INSERT) | (1 << CMD_UPDATE) | (1 << CMD_DELETE))
	...
	if (rel->rd_rel->relkind == RELKIND_VIEW)
	{
		...
		/* CMD_DELETE: the view's columns are not checked for updatability*/
		if (view_query_is_auto_updatable(viewquery, CMD_DELETE) == NULL)
		{
			Bitmapset  *updatable_cols;
			int			auto_events;
			RangeTblRef *rtr;
...
/* BEGIN_OPENTENBASE_ORA */
	if (ORA_MODE && RelationIsReadonlyView(view))
		elog(ERROR, "could not perform DML operations on a read-only view");
/* END_OPENTENBASE_ORA */

	return rewriteTargetSubquery(parsetree, viewquery, view);
}

/*
 * rewriteTargetSubquery
 *     Attempt to rewrite a query where the target is a view or subquery, so that
 *	  the base relation or subquery becomes the target.
 */
static Query*
rewriteTargetSubquery(Query *parsetree, Query *subquery, Relation view)
{
	...
	Relation	base_rel = NULL;
	List	   *view_targetlist;
	ListCell   *lc;
	bool isTargetView = true;

	/* Locate RTE describing the view in the outer query */
	subquery_rte = rt_fetch(parsetree->resultRelation, parsetree->rtable);

	if (subquery_rte->rtekind == RTE_SUBQUERY)
		isTargetView = false;

	/* The view must be updatable, else fail */
	auto_update_detail =
		view_query_is_auto_updatable(subquery,
		...
		switch (parsetree->commandType)
		{
				...
				if (isTargetView)
					ereport(
						...
						(errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
					     ...
					     errdetail_internal(_(auto_update_detail), "Views"),
					     errhint("To enable inserting into the view, provide an INSTEAD OF INSERT "
					             "trigger or an unconditional ON INSERT DO INSTEAD rule.")));
				else
					ereport(
						ERROR,
						(errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
					     errmsg("cannot insert into subquery"),
					     errdetail_internal(_(auto_update_detail), "Subquery"),
					     errhint("To enable inserting into the view, provide an INSTEAD OF INSERT "
					             "trigger or an unconditional ON INSERT DO INSTEAD rule.")));
			}
			break;
				...
				if (isTargetView)
					ereport(
						...
						(errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
					     ...
					     errdetail_internal(_(auto_update_detail), "Views"),
					     errhint("To enable deleting from the view, provide an INSTEAD OF DELETE "
					             "trigger or an unconditional ON DELETE DO INSTEAD rule.")));
				else
					ereport(
						ERROR,
						(errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
					     errmsg("cannot delete from subquery"),
					     errdetail_internal(_(auto_update_detail), "Subquery"),
					     errhint("To enable deleting from the view, provide an INSTEAD OF DELETE "
					             "trigger or an unconditional ON DELETE DO INSTEAD rule.")));
			}
			...
			case CMD_MERGE:
			{
				if (isTargetView)
					ereport(ERROR,
					        (errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
					         errmsg("cannot merge into view \"%s\"", RelationGetRelationName(view)),
					         errdetail_internal(_(auto_update_detail), "Views")));
				else
					ereport(ERROR,
					        (errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
					         errmsg("cannot merge into subquery"),
					         errdetail_internal(_(auto_update_detail), "Subquery")));
			}
			break;
			default:
	 ...
	 * rewriteTargetListIU/rewriteTargetListMergeInto may have added additional targetlist entries for
	...
	if (parsetree->commandType != CMD_DELETE)
	{
		...
		ADD_MODIFIED_COLS(modified_cols, parsetree->targetList);

		if (parsetree->commandType == CMD_MERGE)
		{
			ListCell *l = NULL;
			foreach (l, parsetree->mergeActionList)
			{
				MergeAction *action = (MergeAction *) lfirst(l);
				List        *tlist = action->targetList;
				ADD_MODIFIED_COLS(modified_cols, tlist);
			}
		}
		if (parsetree->onConflict)
		{
			ADD_MODIFIED_COLS(modified_cols, parsetree->onConflict->onConflictSet);
		}

		auto_update_detail = view_cols_are_auto_updatable(subquery,
														  modified_cols,
														  NULL,
														  &non_updatable_col);
		if (auto_update_detail)
			...
			switch (parsetree->commandType)
			{
						...
						ereport(ERROR,
						        (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
						         errmsg("cannot insert into column \"%s\" of view \"%s\"",
						                non_updatable_col,
						                RelationGetRelationName(view)),
						         errdetail_internal(_(auto_update_detail), "View")));
					else
						ereport(ERROR,
						        (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
						         errmsg("cannot insert into column \"%s\" of Subquery",
						                non_updatable_col),
						         errdetail_internal(_(auto_update_detail), "Subquery")));
				}
				break;
				case CMD_UPDATE:
				{
					if (isTargetView)
						...
						ereport(
							ERROR,
							(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
						     errmsg("cannot update column \"%s\" of Subquery", non_updatable_col),
						     errdetail_internal(_(auto_update_detail), "Subquery")));
				}
				break;
				case CMD_MERGE:
				{
					if (isTargetView)
						ereport(ERROR,
						        (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
						         errmsg("cannot merge column \"%s\" of view \"%s\"",
						                non_updatable_col,
						                RelationGetRelationName(view)),
						         errdetail_internal(_(auto_update_detail), "View")));
					else
						ereport(
							ERROR,
							(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
						     errmsg("cannot merge column \"%s\" of Subquery", non_updatable_col),
						     errdetail_internal(_(auto_update_detail), "Subquery")));
				}
				break;
				default:
	...
	rtr = linitial_node(RangeTblRef, subquery->jointree->fromlist);

	base_rt_index = rtr->rtindex;
	base_rte = rt_fetch(base_rt_index, subquery->rtable);
	if (base_rte->rtekind == RTE_RELATION)
	{
		...
		if (parsetree->commandType == CMD_MERGE)
			CheckMergeTargetValid(base_rel);
		/*
	...
	/*
	 * If the subquery contains any sublink subqueries then we need to also
	...
	if (subquery->hasSubLinks)
	{
		...
		context.for_execute = true;
		query_tree_walker(subquery, acquireLocksOnSubLinks, &context,
						  QTW_IGNORE_RC_SUBQUERIES);
	}
	...
	/*
	 * INSERTs never inherit.  For UPDATE/DELETE/MERGE, we use the view query's
	 ...
	 * below.
	 */
	view_targetlist = subquery->targetList;
	...
	new_rte->requiredPerms = subquery_rte->requiredPerms;
	...
	new_rte->insertedCols = adjust_view_column_set(subquery_rte->insertedCols,
												   view_targetlist);

	new_rte->updatedCols = adjust_view_column_set(subquery_rte->updatedCols,
												  view_targetlist);
	...
	if (isTargetView)
	{
		new_rte->securityQuals = subquery_rte->securityQuals;
		subquery_rte->securityQuals = NIL;
	}
	...
	/*
	 * For INSERT/UPDATE/MERGE we must also update resnos in the targetlist to refer
	...
	if (parsetree->commandType != CMD_DELETE)
	{
		foreach(lc, parsetree->targetList)
		{
			...
			if (view_tle != NULL && !view_tle->resjunk && IsA(view_tle->expr, Var))
				...
				elog(ERROR, "attribute number %d not found in view targetlist",
					 tle->resno);
			/* check if update cols are legal*/
			if (IS_PGXC_COORDINATOR && parsetree->commandType == CMD_UPDATE)
			{
				checkUpdateColValid(tle, tle->resno, base_rel, parsetree->rtable);
			}
		}
	}
	...
	if (parsetree->commandType == CMD_MERGE)
	{
		foreach (lc, parsetree->mergeActionList)
		{
			...
			if (action->commandType != CMD_DELETE)
			{

				ListCell *l;
				foreach (l, action->targetList)
				{
					...
					/* check if update cols are legal*/
					if (IS_PGXC_COORDINATOR && action->commandType == CMD_UPDATE &&
					    base_rte->rtekind == RTE_RELATION)
					{
						checkUpdateColValid(tle, tle->resno, base_rel, parsetree->rtable);
					}
		...
		parsetree->onConflict->action == ONCONFLICT_UPDATE)
	{
		...
		parsetree->onConflict = (OnConflictExpr *)
			ReplaceVarsFromTargetList((Node *) parsetree->onConflict,
									  ...
									  &parsetree->hasSubLinks);
		/*
		 * after update targeList for INSERT .. ON CONFLICT DO UPDATE,
		 * check if update cols are legal
		 */
		if (IS_PGXC_COORDINATOR && isTargetView)
		{
			foreach (lc, parsetree->onConflict->onConflictSet)
			{
				TargetEntry *tle = (TargetEntry *) lfirst(lc);
	...
	{
		Node	   *viewqual = (Node *) subquery->jointree->quals;
		...
		if (isTargetView && RelationIsSecurityView(view))
		{
			/*
	 ...
	 */
	if (parsetree->commandType != CMD_DELETE && isTargetView)
	{
		 ...
		 */
		if (has_wco && (cascaded || subquery->jointree->quals != NULL))
		{
			WithCheckOption *wco;
			...
			{
				wco->qual = (Node *) subquery->jointree->quals;
 ...
 */
static List*
rewriteTargetListMergeInto(
        List* targetList, CmdType commandType, OverridingKind override, Relation target_relation, List** attrno_list)
{
    TargetEntry** new_tles;
    List* new_tlist = NIL;
    List* junk_tlist = NIL;
            ...
            if (new_expr == NULL) {
                if (commandType == CMD_INSERT)
                    ...
                    new_expr = coerce_to_domain(
                            new_expr, InvalidOid, -1, att_tup->atttypid,
							COERCION_IMPLICIT, COERCE_IMPLICIT_CAST, -1, false);
                }
            }
...
RewriteQuery(Query *parsetree, List *rewrite_events)
{
	...
	if (event != CMD_SELECT && event != CMD_UTILITY &&
		(PG_MODE || parsetree->multi_inserts == NULL))
	{
		int			result_relation;
		...
		if (event == CMD_MERGE && rt_entry->rtekind == RTE_SUBQUERY)
		{
			List *newstuff;
			parsetree = rewriteTargetSubquery(parsetree, rt_entry->subquery, NULL);

			newstuff = RewriteQuery(parsetree, rewrite_events);
		...
		else
		{
			...
			{
				delForeignTargetListUD(parsetree, rt_entry, rt_entry_relation);
			}
			...
			if (parsetree_list == NIL)
			{
				 ...
				 */
				locks = matchLocks(event,
				                   rt_entry_relation->rd_rules,
				                   result_relation,
				                   parsetree,
				                   &hasUpdate);
				 ...
				 */
				if (parsetree->onConflict && (product_queries != NIL || hasUpdate) &&
				    !updatableview)
					ereport(ERROR,
					        (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
					         errmsg("INSERT with ON CONFLICT clause cannot be used with table that "
						...
						if (product_queries != NIL)
						{
							...
							foreach (n, rewrite_events)
							{
								...
								if (rev->relation == RelationGetRelid(rt_entry_relation) &&
									...
									ereport(ERROR,
									        (errcode(ERRCODE_INVALID_OBJECT_DEFINITION),
									         errmsg("infinite recursion detected in rules for "
									                "relation \"%s\"",
									                RelationGetRelationName(rt_entry_relation))));
							}
                 ...
                 int numParams)
{
	...
	foreach(col, tlist)
	{
		...
		/* support CTAS by function which return RAW type. */
		if (typename->typeOid == RAWOID && typename->typemod == -1)
			typename->typemod = MAX_RAW_SIZE + VARHDRSZ;
	...
	if (create_stmt->oncommit != ONCOMMIT_NOOP
			&& !(create_stmt->relation->relpersistence == RELPERSISTENCE_TEMP || create_stmt->relation->relpersistence == RELPERSISTENCE_GLOBAL_TEMP))
		ereport(ERROR,
	...
	ProcessUtility(wrapper, cquery.data, false, PROCESS_UTILITY_QUERY,
	...
	if (parserSetup != NULL)
		stmt_list = pg_analyze_and_rewrite_params(linitial(raw_parsetree_list), cquery.data,
		                                     parserSetup, parserSetupArg, queryEnv, true, false);
	else
		stmt_list =
			pg_analyze_and_rewrite(linitial(raw_parsetree_list), cquery.data, NULL, 0, queryEnv);

	if (IS_PGXC_COORDINATOR && list_length(raw_parsetree_list) == 1 && list_length(stmt_list) == 1)
	{
		RawStmt *rawparsetree = (RawStmt *) linitial(raw_parsetree_list);
		...
		{
			InsertStmt *insert_stmt = (InsertStmt *) rawparsetree->stmt;

			stmt_list =
				transformInsertValuesIntoCopyFromQuery(insert_stmt, parse->copy_filename, parse);
		}
	}
	return stmt_list;
}
