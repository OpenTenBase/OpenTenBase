...
ChangeVarNodes(Node *node, int rt_index, int new_index, int sublevels_up)
{
	...
	else if (node != NULL && IS_PLAN_NODE(node))
		plan_tree_walker((Plan *) node, ChangeVarNodes_walker, &context);
	else
...
getInsertSelectQuery(Query *parsetree, Query ***subquery_ptr)
{
	...
	if (list_length(parsetree->rtable) >= 2 &&
		strcmpfunc(rt_fetch(PRS2_OLD_VARNO, parsetree->rtable)->eref->aliasname,
			   "old") == 0 &&
		strcmpfunc(rt_fetch(PRS2_NEW_VARNO, parsetree->rtable)->eref->aliasname,
			   "new") == 0)
		return parsetree;
	...
	if (list_length(selectquery->rtable) >= 2 &&
		strcmpfunc(rt_fetch(PRS2_OLD_VARNO, selectquery->rtable)->eref->aliasname,
			   "old") == 0 &&
		strcmpfunc(rt_fetch(PRS2_NEW_VARNO, selectquery->rtable)->eref->aliasname,
			   "new") == 0)
	{
		if (subquery_ptr)
