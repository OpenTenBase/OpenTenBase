 ...
 *	  GLOBAL MEMORY															 *
 *****************************************************************************/
__thread bool am_sub_thread = false;	/* Am I a sub thread? */
...
__thread MemoryContext SubThreadLocalContext = NULL;
...
inline void
InsertMemoryAllocInfo(const void *pointer, MemoryContext context, const char *file, int line,
                      Size size)
{
	if (MemCxtShouldTrack(context))
	{
		InsertTrackMemAllocInfo(pointer, context, file, line, size);
	}
}

inline void
RemoveMemoryAllocInfo(const void *pointer, const MemoryContext context, const char *file, int line,
                      Size size)
{
	if (MemCxtShouldTrack(context))
	{
		RemoveTrackMemAllocInfo(pointer, context, file, line, size);
	}
}

static inline void
RemoveMemoryContextInfo(MemoryContext context)
{
	if (MemCxtShouldTrack(context))
	{
		RemoveTrackMemCxt(context);
	}
}
...
static Size
MemoryContextMemAllocatedInternal(MemoryContext context, bool recurse, HTAB *cxt_detail_hash)
{
	...
	if (cxt_detail_hash != NULL)
	{
		MemCxtDetail *entry_cxt_total = NULL;
		const char   *name = context->ident ? context->ident : context->name;
		char          entry_cxt_key[NAMEDATALEN] = {0};
		bool          found = false;

		strncpy(entry_cxt_key, name, NAMEDATALEN - 1);
		entry_cxt_total =
			(MemCxtDetail *) hash_search(cxt_detail_hash, entry_cxt_key, HASH_ENTER, &found);
		if (entry_cxt_total)
		{
			if (found)
			{
				entry_cxt_total->total_size += total;
			}
			else
			{
				entry_cxt_total->total_size = total;
		...
		{
			elog(ERROR, "MemoryContextMemAllocatedInternal: entry_cxt_total HASH_ENTER failed");
		}
	}
	...
	if (recurse)
	{
		...
		for (child = context->firstchild;
			...
			total += MemoryContextMemAllocatedInternal(child, true, cxt_detail_hash);
	}
...
{
	return MemoryContextMemAllocatedInternal(context, recurse, NULL);
}

Size
SummarizeCxtMemUsage(HTAB *cxt_detail_hash)
{
	return MemoryContextMemAllocatedInternal(TopMemoryContext, true, cxt_detail_hash);
}
...
void
MemoryContextStatsAll(MemoryContext context)
{
	/* A hard-wired limit on the number of children is usually good enough */
	MemoryContextStatsDetail(context, -1, true);
}
						   ...
						   bool print_to_stderr)
{
	...
	{
		if ((ichild < max_children) || (max_children == -1))
...
MemoryContextCheck(MemoryContext context)
{
	...
	for (child = context->firstchild; child != NULL; child = child->nextchild)
	{
		if (strcmp(child->name, "Thread TupleQueue Context") != 0)
 ...
 */
void *
MemoryContextAllocInternal(MemoryContext context, Size size, const char* file, int line)
{
	void	   *ret;
	...
	ret = context->methods->alloc(context, size, file, line);
 ...
 */
void *
MemoryContextAllocZeroInternal(MemoryContext context, Size size, const char* file, int line)
{
	void	   *ret;
 ...
 */
void *
MemoryContextAllocZeroAlignedIternal(MemoryContext context, Size size, const char* file, int line)
{
	void	   *ret;
 ...
 */
void *
MemoryContextAllocExtendedIternal(MemoryContext context, Size size, int flags, const char* file, int line)
{
	void	   *ret;
...
void
HandleGetMemoryContextDetailInterrupt(void)
{
	InterruptPending = true;
	GetMemoryContextDetailPending = true;
	/* latch will be set by procsignal_sigusr1_handler */
}

void
HandleGetMemoryDetailInterrupt(void)
{
	InterruptPending = true;
	GetMemoryDetailPending = true;
	/* latch will be set by procsignal_sigusr1_handler */
}
...
ProcessLogMemoryContextInterrupt(void)
{
	 ...
	 */
	MemoryContextStatsDetail(TopMemoryContext, 100, true);

	DoMemTrackAllocInfoPrint();
}

void
ProcessGetMemoryDetailInterrupt(void)
{
	GetMemoryDetailPending = false;

	DoMemoryDetailBooking();
}
...
repallocInternal(void *pointer, Size size, const char* file, int line)
{
	...
	if (!AllocSizeIsValid(size))
		elog(ERROR, "invalid memory alloc request size %zu in %s:%d", size, file, line);
	...
	ret = context->methods->realloc(context, pointer, size, file, line);
	if (unlikely(ret == NULL))
	{
		MemoryContextStats(TopMemoryContext);
		ereport(ERROR,
 ...
 */
void *
MemoryContextAllocHugeInternal(MemoryContext context, Size size, const char* file, int line)
{
	void	   *ret;
 ...
 *		
 * Same with MemoryContextAllocHuge but with output of file and line of usage.
 */
void *
MemoryContextAllocHugeDebugInternal(MemoryContext context, Size size, const char* file, int line)
{
	void	   *ret;
 ...
 */
void *
repallocHugeInternal(void *pointer, Size size, const char* file, int line)
{
	MemoryContext context = GetMemoryChunkContext(pointer);
	void	   *ret;
...
MemoryContextStrdup(MemoryContext context, const char *string)
{
	...
	nstr = (char *) MemoryContextAllocInternal(context, len, __FILE__, __LINE__);
...
void *allocTopCxt(size_t s)
{
	return MemoryContextAllocInternal(TopMemoryContext, (Size)s, __FILE__, __LINE__);
}

Gen_Alloc genAlloc_class = {(void *)MemoryContextAllocInternal,
							(void *)MemoryContextAllocZeroInternal,
							(void *)repallocInternal,
							(void *)pfree,
							(void *)current_memcontext,
							(void *)allocTopCxt};
 ...
 * 		Execute the archiving of memory tracking information statistics.
 */
void
ProcessMemTrackAchieveInterrupt(void)
{
	if (!AchieveMemTrackInfoPending)
		return;

	AchieveMemTrackInfoPending = false;
	DoMemTrackInfoAchieve();
}
#endif

static void
GetMemoryDetailRecursive(MemoryContext context, int level, MemoryDetail *arr, int *index, int max_size)
{
	MemoryContext child;
	int			ichild;
	long total_space, free_space;
	MemoryDetail *dst;
	...
	/* Examine the context itself */
	dst = &arr[*index];
	strncpy(dst->cxt_name, context->name, NAMEDATALEN - 1);
	
	AllocSetStats_Output(context, &total_space, &free_space);
	dst->total_size = total_space;
	...
	*index = *index + 1;
	...
	for (child = context->firstchild, ichild = 0;
		 ...
		 child = child->nextchild, ichild++)
	{
		GetMemoryDetailRecursive(child, level + 1, arr, index, max_size);
	}
}

void
GetMemoryDetail(MemoryDetail *arr, int *arr_len)
{
	int index = 0;
	GetMemoryDetailRecursive(TopMemoryContext, 0, arr, &index, 50);
	*arr_len = index;
}

static void
RecursiveMemoryContextAlloc(const MemoryContext context,
    const char* ctx_name, StringInfoData* buf)
{
	...
	if (pg_strcasecmp(ctx_name, context->name) == 0) {
#ifndef ENABLE_MEMORY_CHECK
		GetAllocBlockInfo(context, buf);
#else
#endif
	}
	...
	for (child = context->firstchild; child != NULL; child = child->nextchild) {
		RecursiveMemoryContextAlloc(child, ctx_name, buf);
	}
	return;
}

void
GetMemoryContextDetail(MemCtxDetail *arr, int *arr_len, const char *cxtname)
{
	StringInfoData mem_info;
	int memctx_info_len;
	initStringInfo(&mem_info);
	RecursiveMemoryContextAlloc(TopMemoryContext, cxtname, &mem_info);

	memctx_info_len = 0;
	CollateMemctxInfoInternal(&mem_info, &memctx_info_len, arr);
	*arr_len = memctx_info_len;
	pfree(mem_info.data);
}
/*
 * CheckSubThreadContextInternal
 * Check sub thread context.
 */
extern void
CheckSubThreadContextInternal(MemoryContext context, const char *func)
{
	if (!am_sub_thread)
		return;

	if (context == NULL)
		elog(ERROR, "%s: context is null", func);

	if (context != SubThreadLocalContext && context != ErrorContext)
	{
		elog(ERROR, "can not call %s in sub thread, context: %s",
			func, context->name);
	}
}
