...
extern MemoryContext get_current_dynahashcxt(void);
...
/*
 * An interface is provided to reset CurrentDynaHashCxt, because the memory tracking function
 * will enter InsertTrackMemAllocInfo when other people execute hash_create, thus executing a
 * new hash_create operation. Failure to restore CurrentDynaHashCxt in time can cause many 
 * unexpected problems, so an interface is provided here for recovery.
 */
MemoryContext
get_current_dynahashcxt(void)
{
	return CurrentDynaHashCxt;
}

void
set_current_dynahashcxt(MemoryContext cxt)
{
	CurrentDynaHashCxt = cxt;
}
...
MemoryContext
hash_get_memcxt(HTAB *hashp)
{
	return hashp->hcxt;
}

void*
buf_hash_operate(HTAB* hashp, const BufferTag* keyPtr, uint32 hashvalue,
	bool* foundPtr, HASHACTION action)
{
	HASHHDR* hctl = hashp->hctl;
	uint32 bucket;
	long segment_num;
	...
	{
		if (currBucket->hashvalue == hashvalue &&
			BUFFERTAGS_PTR_EQUAL((BufferTag*)(ELEMENTKEY(currBucket)), keyPtr))
			 break;
	...
	ereport(ERROR, (errmsg("unrecognized hash action code: %d", (int)action)));
