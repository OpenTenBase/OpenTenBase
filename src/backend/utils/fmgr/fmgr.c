...
#ifdef _PG_ORCL_
static List *get_fnnsp_from_withfunc(fmNodePtr expr);
#endif
...
#define FUNCTION_OID_CHANGE_GRAND_VERSION 502
#define VERSION_502_FUNCTION_OID_CHANGE_MIN 8200
#define VERSION_502_FUNCTION_OID_CHANGE_MAX 9230
#define FUNCTION_PROSRC_CHANGE_GRAND_VERSION 500

#define SPECIAL_PROSRC_NUM_V3_TO_V521 (16)
const char *special_prosrc[SPECIAL_PROSRC_NUM_V3_TO_V521] = {"orcl_num_to_date","orcl_todate","orcl_timestamptz_tochar","orcl_totimestamp_default","orcl_timestamp_tochar","orcl_totimestamptz_default","orcl_timestamp_mi","orcl_timestamp_pl_day_i2","orcl_timestamp_pl_day_i4","orcl_timestamp_pl_day_i8","orcl_timestamp_pl_day_n","orcl_timestamp_mi_day_i2","orcl_timestamp_mi_day_i4","orcl_timestamp_mi_day_i8","orcl_timestamp_mi_day_n","orcl_todate_default"};
...
fmgr_isbuiltin(Oid id)
{
	...
	/*
	 * During the upgrade to version 502, some function OIDs have changed, and it
	 * is not allowed to search for them through hardcoding.
	 */
	if (unlikely(upgrade_mode != NON_UPGRADE))
	{
		if (WorkingGrandVersionNum <= FUNCTION_OID_CHANGE_GRAND_VERSION &&
			OidIsValid(MyDatabaseId) &&
			id >= VERSION_502_FUNCTION_OID_CHANGE_MIN && id <= VERSION_502_FUNCTION_OID_CHANGE_MAX)
			return NULL;
	}
...
fmgr_lookupByName(const char *name)
{
	...
	/*
	 * During the upgrade to v521, some functions prosrcs have been changed and it
	 * is not allowed to search for them through hardcoding.
	 */
	if (unlikely(upgrade_mode != NON_UPGRADE && WorkingGrandVersionNum < FUNCTION_PROSRC_CHANGE_GRAND_VERSION))
	{
		int	j;

		for (j = 0; j < SPECIAL_PROSRC_NUM_V3_TO_V521; j++)
		{
			if (strcmp(name, special_prosrc[j]) == 0)
				return NULL;
		}
	}
...
#ifdef _PG_ORCL_
void
fmgr_info_withfuncs(Oid functionId, FmgrInfo *finfo, fmNodePtr expr)
{
	fmgr_info_cxt_security(functionId, finfo, CurrentMemoryContext, false, expr);
}

void
fmgr_info_cxt_withfuncs(Oid functionId, FmgrInfo *finfo,
							fmNodePtr expr, MemoryContext mcxt)
{
	fmgr_info_cxt_security(functionId, finfo, mcxt, false , expr);
}
#endif
					   ...
					   )
{
	...
	from_withfuncs = get_fn_from_withfunc(expr);
	Assert(!from_withfuncs || (from_withfuncs && functionId == InvalidOid));
#endif
	...
	if (from_withfuncs)
	{
		int	with_funcid = get_fnid_from_withfunc(expr);

		procedureTuple = GetWithFunctionTupleById(get_fnnsp_from_withfunc(expr),
													with_funcid);
	}
	else
	{
...
#ifdef _PG_ORCL_
			if (from_withfuncs)
				elog(ERROR, "WITH FUCTION can not declare a INTERNAL function");
#endif
			/*
...
fmgr_security_definer(PG_FUNCTION_ARGS)
{
	...
	bool		saveSecContextIsSet = false;
	...
	if (OidIsValid(fcache->userid))
	{
		if (IS_PGXC_LOCAL_COORDINATOR)
		{
			...
			{
				saveSecContextIsSet = SecContextIsSet;
				SecContextIsSet = true;
			}
		}
		...
		if (OidIsValid(fcache->userid))
		{
			...
			if (IS_PGXC_LOCAL_COORDINATOR)
			{
				if (PG_MODE)
				{
					...
					SecContextIsSet = saveSecContextIsSet;
...
Datum
DirectFunctionCall1CollWithSubTran(PGFunction func, Oid collation, Datum arg1, bool *hasError)
{
	Datum	ret = (Datum) 0;
	int		nesting_level;
	...
	*hasError = false;
	...
	nesting_level = GetCurrentTransactionNestLevel();

	PG_TRY();
	{
		ret = DirectFunctionCall1Coll(func, collation, arg1);
	...
	{
		ErrorData *edata;
		bool	   handles_invalid;

		*hasError = true;
		handles_invalid = validate_handles();

		MemoryContextSwitchTo(oldcontext);
		edata = CopyErrorData();
		if (edata->message)
			elog(LOG, "An error occurred while calling the function, msg:%s", edata->message);

		/* Abort the inner transaction */
		RollbackAndReleaseCurrentSubTransaction();

		/*
		 * After popup exception(rollback subxact), we expect a 'safe' transaction
		 * state, else do any further works will cause some unexpected behaviors.
		 */
		if (!IsTransactionState())
		{
			FlushErrorState();
			elog(ERROR, "invalid transaction state and terminated");
		}
		...
		if (handles_invalid)
		{
			elog(WARNING, "Found broken connection, nesting level: %d", nesting_level);
			PG_RE_THROW();
		}
...
Datum
DirectFunctionCall2CollWithSubTran(PGFunction func, Oid collation, Datum arg1, Datum arg2, bool *hasError)
{
	Datum	ret = (Datum) 0;
	int		nesting_level;
	...
	*hasError = false;
	...
	PG_TRY();
	{
		ret = DirectFunctionCall2Coll(func, collation, arg1, arg2);
...
Datum
DirectFunctionCall5CollWithSubTran(PGFunction func, Oid collation, Datum arg1, Datum arg2, Datum arg3, Datum arg4, Datum arg5, bool *hasError)
{
	Datum	ret = (Datum) 0;
	int		nesting_level;
	...
	*hasError = false;
	...
	PG_TRY();
	{
		ret = DirectFunctionCall5Coll(func, collation, arg1, arg2, arg3, arg4, arg5);
...
Datum
DirectFunctionCall3CollWithSubTran(PGFunction func, Oid collation, Datum arg1, Datum arg2, Datum arg3, bool *hasError)
{
	Datum	ret = (Datum) 0;
	int		nesting_level;
	...
	*hasError = false;
	...
	PG_TRY();
	{
		ret = DirectFunctionCall3Coll(func, collation, arg1, arg2, arg3);
...
Datum
DirectFunctionCall4CollWithSubTran(PGFunction func, Oid collation, Datum arg1, Datum arg2, Datum arg3, Datum arg4, bool *hasError)
{
	Datum	ret = (Datum) 0;
	int		nesting_level;
	...
	*hasError = false;
	...
	PG_TRY();
	{
		ret = DirectFunctionCall4Coll(func, collation, arg1, arg2, arg3, arg4);
...
/* Allow returning null to facilitate inter-function calls. */
Datum
DirectCall1(bool *is_retnull, PGFunction func, Oid collation, Datum arg1)
{
	FunctionCallInfoData fcinfo;
	Datum                result = 0;
...
Datum
DirectCall2(bool *is_retnull, PGFunction func, Oid collation, Datum arg1, Datum arg2)
{
	FunctionCallInfoData fcinfo;
	Datum                result = 0;
...
Datum
DirectCall3(bool *is_retnull, PGFunction func, Oid collation, Datum arg1, Datum arg2, Datum arg3)
{
	FunctionCallInfoData fcinfo;
	Datum                result = 0;
...
Datum
XmlInputFunctionCall(FmgrInfo *flinfo, char *str, Oid typioparam, int32 typmod, bool need_check)
{
	FunctionCallInfoData fcinfo;
	Datum		result;
	...
	fcinfo.arg[1] = ObjectIdGetDatum(typioparam);
	fcinfo.arg[2] = Int32GetDatum(typmod);
	fcinfo.arg[3] = BoolGetDatum(need_check);
	fcinfo.argnull[0] = (str == NULL);
	fcinfo.argnull[1] = false;
	fcinfo.argnull[2] = false;
	fcinfo.argnull[3] = false;
...
bool
get_fn_from_withfunc(fmNodePtr expr)
{
	if (!expr)
		return false;
	...
	if (IsA(expr, FuncExpr))
	{
		...
		return funcexpr->withfuncnsp != NULL;
	}
	else
		return false;
}
...
/*
 * Whether the current FuncExpr is a local function/procedure
 */
bool
is_local_func_proc(fmNodePtr expr)
{
	if (!expr)
		return false;
	...
	if (IsA(expr, FuncExpr))
	{
		...
		if (funcexpr->withfuncnsp == NULL || funcexpr->withfuncid == -1)
			return false;

		tp = GetWithFunctionTupleById(funcexpr->withfuncnsp, funcexpr->withfuncid);
		if (tp && ((Form_pg_proc) GETSTRUCT(tp))->pronamespace != InvalidOid)
			return true;
	}
...
int
get_fnid_from_withfunc(fmNodePtr expr)
{
	FuncExpr        *funcexpr;
	...
	return funcexpr->withfuncid;
}

bool
is_local_function(FmgrInfo *flinfo)
{
	if (get_fn_from_withfunc(flinfo->fn_expr))
	{
		HeapTuple		procTup = NULL;

		procTup = get_functuple_by_fmgr(flinfo,
						get_fnid_from_withfunc(flinfo->fn_expr));
		...
		if (procTup)
		{
			...
			procStruct = (Form_pg_proc) GETSTRUCT(procTup);
			return procStruct->prokind == PROKIND_FUNCTION &&
				   OidIsValid(procStruct->pronamespace);
		}
	}
...
static List *
get_fnnsp_from_withfunc(fmNodePtr expr)
{
	FuncExpr        *funcexpr;
	...
	return funcexpr->withfuncnsp;
}

List *
get_fnnsp_from_fmgr(FmgrInfo *flinfo)
{
	FuncExpr        *funcexpr;

	Assert(flinfo != NULL && flinfo->fn_expr != NULL);
	Assert(IsA(flinfo->fn_expr, FuncExpr));

	funcexpr = (FuncExpr *) flinfo->fn_expr;

	return funcexpr->withfuncnsp;
}

HeapTuple
get_functuple_by_fmgr(FmgrInfo *flinfo, int id)
{
	Assert(flinfo != NULL && flinfo->fn_expr != NULL
			&& IsA(flinfo->fn_expr, FuncExpr));
	return GetWithFunctionTupleById(get_fnnsp_from_withfunc(flinfo->fn_expr), id);
}
