...
static SharedInvalidationMessage *SharedInvalidMessagesArray = NULL;
						 ...
						 HeapTuple newtuple)
{
    ...
    else if(DataMaskMapRelationId == tupleRelId)
    {
        ...
        databaseId = MyDatabaseId;
        CacheInvalidateRelcacheAllPartitions(databaseId, data_mask_tup->relid);
    ...
    else if(ClsTableRelationId == tupleRelId)
    {
        ...
        databaseId = MyDatabaseId;
        CacheInvalidateRelcacheAllPartitions(databaseId, cls_table_tup->relid);
...
void
RegisterRelcacheInvalidMsg(Oid dbId, Oid relId)
{
	PrepareInvalidationState();
