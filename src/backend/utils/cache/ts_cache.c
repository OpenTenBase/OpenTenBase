...
getTSCurrentConfig(bool emitError)
{
	...
	TSCurrentConfigCache =
		get_ts_config_oid(stringToQualifiedNameListAsPG(TSCurrentConfig),
						  !emitError);
...
check_TSCurrentConfig(char **newval, void **extra, GucSource source)
{
	...
	if (IsTransactionState())
	{
		...
		cfgId = get_ts_config_oid(stringToQualifiedNameListAsPG(*newval), true);
		...
		buf = quote_qualified_identifier_as_pg(get_namespace_name(cfg->cfgnamespace),
										 NameStr(cfg->cfgname));
