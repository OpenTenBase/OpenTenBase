...
static inline HeapTuple SearchCatCacheInternalOnlyWithCache(CatCache *cache,
					   bool *neg_entry,
					   int nkeys,
					   Datum v1, Datum v2,
					   Datum v3, Datum v4);
...
HeapTuple
SearchCatCache1OnlyCache(CatCache *cache,
				Datum v1)
{
	return SearchCatCacheInternalOnlyWithCache(cache, NULL, 1, v1, 0, 0, 0);
}
...
SearchCatCacheInternalOnlyWithCache(CatCache *cache,
					   bool *neg_entry,
					   int nkeys,
					   Datum v1,
					   Datum v2,
					   ...
					   Datum v4)
{
	...
	if (!IsTransactionState())
	{
		if (IsAbortedTransactionBlockState())
			...
			ereport(ERROR,
                (errcode(ERRCODE_IN_FAILED_SQL_TRANSACTION),
                 errmsg("search catcache not in transaction state")));
	}
					   ...
					   Datum v4)
{
	...
	Index		hashIndex;
	HeapTuple ret = NULL;
	bool neg_entry = false;
	ret = SearchCatCacheInternalOnlyWithCache(cache, &neg_entry, nkeys, v1, v2, v3, v4);
	if (ret != NULL || neg_entry == true)
	{
		return ret;
	}
	/*
