...
#ifdef __OPENTENBASE_C__
static const FormData_pg_attribute Desc_pg_fn_register_info[Natts_pg_fn_register_info] = {Schema_pg_fn_register_info};
#endif
...
bool enable_threadsafety_check = false;
...
Oid OraGetFirstDisValConstraint(Relation relation);
...
RelationBuildTupleDesc(Relation relation)
{
	...
	{
		int i;
		StringInfo missing_attnums;

		missing_attnums = makeStringInfo();
		for (i = 0; i < RelationGetNumberOfAttributes(relation); i++)
		{
			if (TupleDescAttr(relation->rd_att, i)->attnum == 0)
			{
	 ...
	 */
	if (constr->has_not_null || ndef > 0 || relation->rd_rel->relchecks ||attrmiss)
	{
	...
	if (ORA_MODE &&
	    !IsSystemObjOid(RelationGetRelid(relation)) &&
	    OraGetFirstDisValConstraint(relation) != InvalidOid)
	{
		relation->cons_disablevalidate = true;
	}

#ifdef _MLS_
	datamask_assign_relation_tupledesc_field(relation);
 #endif
...
#ifdef __OPENTENBASE_C__
	{
		...
		for (i = 0; i < relation->rd_rel->relnatts; i++)
		{
			assign_attribute_alignedlen(relation->rd_att, i);
		}
	}
#endif
}
...
RelationBuildDesc(Oid targetRelId, bool insertIt)
{
	...
	relation->rd_firstcid = InvalidCommandId;

	/* Record the current thread information. */
	if (unlikely(MyLocalThreadId == 0))
	{
		MyLocalThreadId = pthread_self();
	}
	relation->rd_threadid = MyLocalThreadId;
		...
		case RELPERSISTENCE_GLOBAL_TEMP:
			if (isTempOrTempToastNamespace(relation->rd_rel->relnamespace))
			{
				...
				relation->rd_backend = OidIsValid(MyCoordId) ?
												MyFirstBackendId : BackendIdForTempRelations();
#else
	 ...
	 */
	if (OidIsValid(relation->rd_rel->relam) || RelkindIsCrossNodeIndex(relation->rd_rel->relkind))
...
RelationInitIndexAccessInfo(Relation relation)
{
	...
	{
		/* during index creating, pg_index has no record*/
		if (RelationIsCrossNodeIndex(relation->rd_rel))
			return ;
	...
	/*
	 * global index table has no relam
	*/
	if (!OidIsValid(relation->rd_rel->relam) && RelationIsCrossNodeIndex(relation->rd_rel))
	{
		/* only support btree */
		  ...
		  int natts, const FormData_pg_attribute *attrs)
{
	...
	/*
	 * do not record for nailed relation
	 */
	relation->rd_firstcid = InvalidCommandId;
...
RelationIncrementReferenceCount(Relation rel)
{
	...
	/*
	 * Here, we add protective checks to detect and prevent any potential concurrent locking issues,
	 * which are not expected to occur.
	 */
	if (enable_threadsafety_check && unlikely(rel->rd_threadid != MyLocalThreadId))
	{
		elog(WARNING,
		     "[RelationIncrementReferenceCount] Internal error, there is a concurrent locking "
		     "issue. detail: rel[%d], tid [owntid: "
		     "%lu, maintid: %lu, selftid: %lu]",
	...
	/*
	 * Remember which statement using this relation first.
	 * If there are two or more statement using it then Invalid
	 */
	if (rel->rd_refcnt == 0)
		rel->rd_firstcid = cid;
	else if (rel->rd_firstcid != cid)
		rel->rd_firstcid = InvalidCommandId;
...
RelationDecrementReferenceCount(Relation rel)
{
	...
	{
		elog(WARNING,
		     "[RelationDecrementReferenceCount] Internal error, there is a concurrent locking "
		     "issue. detail: rel[%d], tid [owntid: "
		     "%lu, maintid: %lu, selftid: %lu]",
		     RelationGetRelid(rel),
		     rel->rd_threadid,
...
RelationClearRelation(Relation relation, bool rebuild)
{
	 ...
	 * During inplace upgrade, need rebuild the system relation, skip the
	...
	if (relation->rd_isnailed)
	{
		...
		if (!IsInplaceUpgrade ||
			(IsInplaceUpgrade && (relation->rd_id == RelationRelationId ||
			relation->rd_id == AttributeRelationId || relation->rd_id == ProcedureRelationId)))
			return;
	}
...
#define SWAPFIELD(fldtype, fldname) \
		...
		SWAPFIELD(struct XactRelStats *, xact_relstat_info);
...
void
RelationCacheCheckEntry(Oid relationId, const char * filename, int lineno, const char * funcname)
{
	Relation	relation;
	...
	if (PointerIsValid(relation))
	{
		elog(LOG, "Relcache of relation %d is relname %s, rd_id %d, refcount is %d at %s/%d/%s",
			relationId, RelationGetRelationName(relation), relation->rd_id, relation->rd_refcnt,
			filename, lineno, funcname);
	}
	else
	{
		elog(LOG, "Relcache of relation %d not valid anymore at %s/%d/%s",
			relationId, filename, lineno, funcname);	
	}
}
						   ...
						   char relkind)
{
	...
	rel->rd_threadid = MyLocalThreadId;
						  ...
						  TransactionId freezeXid, MultiXactId minmulti)
{
	...
	elog(LOG, "Relation %s(%u) set newfilenode %u oldfilenode %u xid %u",
			RelationGetRelationName(relation),
			RelationGetRelid(relation),
			newrelfilenode,
			relation->rd_node.relNode,
	...
	RelationCreateStorage(newrnode.node, persistence, relation);
	smgrclosenode(newrnode);
#else
	if (RelationIsRowStore(relation))
	{
		RelationCreateStorage(newrnode.node, persistence, relation);
		smgrclosenode(newrnode);		
	}
...
#endif
#ifdef __OPENTENBASE_C__
		formrdesc("pg_fn_register_info", PgFnRegisterInfoRelation_Rowtype_Id, true,
				  false, Natts_pg_fn_register_info, Desc_pg_fn_register_info);
#endif
...
load_critical_index(Oid indexoid, Oid heapoid)
{
	...
	ird->rd_threadid = MyLocalThreadId;
...
#ifdef __OPENTENBASE__
		check[found].ccenable = conform->conisenable;
#endif
...
/*
 * opentenbase_ora compatibility, constraint checking
 *
 * eg. ALTER TABLE const_test DISABLE VALIDATE CONSTRAINT aaaa;
 * the current table does not allow insertions, deletions, or modifications.
 */
Oid
OraGetFirstDisValConstraint(Relation relation)
{
	Relation           conrel;
	SysScanDesc        conscan;
	ScanKeyData        skey[1];
	...
	while (HeapTupleIsValid(htup = systable_getnext(conscan)))
	{
		conform = (Form_pg_constraint) GETSTRUCT(htup);
		...
		if (!conform->conisenable && conform->convalidated)
		{
			conid = HeapTupleGetOid(htup);
			break;
		}
	}
...
load_relcache_init_file(bool shared)
{
	...
	for (relno = 0;; relno++)
	{
		...
		MemSet(&rel->xact_relstat_info, 0, sizeof(rel->xact_relstat_info));
...
#ifdef __OPENTENBASE_C__
List *
RelationGetValidIndexList(Relation relation)
{
	Relation	indrel;
	SysScanDesc indscan;
	ScanKeyData skey;
...
List *
RelationGetInvalidIndexList(Relation relation)
{
	Relation	indrel;
	SysScanDesc indscan;
	ScanKeyData skey;
...
/*
 * RelationGetGlobalIndexInfoList -- get a list of global index info
 */
List *
RelationGetGlobalIndexInfoList(Relation relation)
{
	List* index_info_list = NIL;
	List* index_oid_list = NIL;
	ListCell* l = NULL;
	...
	/*
	 * Get cached list of global index OIDs
	 */
	index_oid_list = RelationBuildCrossNodeIndexList(relation, true);
	...
	foreach (l, index_oid_list)
	{
		...
		/* Only global index is considered */
		index_info_list = lappend(index_info_list, index_info);
		index_close(index_desc, AccessShareLock);
	}
...
/*
 * RelationIdGetGlobalIndexInfoList -- get a list of global index info
 */
List*
RelationIdGetGlobalIndexInfoList(Oid relid)
{
    List* index_info_list = NIL;
...
/*
 * Check for table/partition table: return -1 means all row tables
 */
int16
check_rel_for_uppdate(Relation rel)
{
	if (rel->rd_rel->relkind == RELKIND_PARTITIONED_TABLE)
	{
		ListCell   *lc;
		List *inhoids = find_all_inheritors(RelationGetRelid(rel), NoLock, NULL);
		...
		foreach(lc, inhoids)
		{
			...
			tp = SearchSysCache1(RELOID, ObjectIdGetDatum(inhrelid));
			if (!HeapTupleIsValid(tp))
