...
date_in(PG_FUNCTION_ARGS)
{
	...
	if (4 == PG_NARGS())
	{
		char *date_fmt = PG_GETARG_CSTRING(3);
		if (NULL == date_fmt)
		{
			ereport(ERROR,
					(errcode(ERRCODE_INVALID_DATETIME_FORMAT),
							errmsg("specified date format is null")));
		}

		/* the following logic shared from to_date(). */
		general_do_to_timestamp(str, date_fmt, tm, &fsec);
	}
	else
	{
...
#ifdef XCP
		/*
		 * We want other nodes could parse encoded dates correctly in
		 * distributed mode. ISO date style is best suitable for that.
		 */
		EncodeDateOnly(tm, IS_DISTRIBUTED_DATANODE ? USE_ISO_DATES : DateStyle,
					   buf);
#else
...
time_in(PG_FUNCTION_ARGS)
{
	...
	if (4 == PG_NARGS())
	{
		char *time_fmt = PG_GETARG_CSTRING(3);
		if (time_fmt == NULL)
		{
			ereport(ERROR,
					(errcode(ERRCODE_INVALID_DATETIME_FORMAT),
							errmsg("specified time format is null")));
		}

		/* the following logic shared from to_timestamp(). */
		general_do_to_timestamp(str, time_fmt, tm, &fsec);
	}
	else
	{
		dterr = ParseDateTime(str, workbuf, sizeof(workbuf),
...
Datum
time_hash_new(PG_FUNCTION_ARGS)
{
...
Datum
timetz_hash_new(PG_FUNCTION_ARGS)
{
	TimeTzADT  *key = PG_GETARG_TIMETZADT_P(0);
	uint32		thash;
	 ...
	 * field hashes separately and XOR them.
	 */
	thash = DatumGetUInt32(DirectFunctionCall1(hashint8new,
											   Int64GetDatumFast(key->time)));
	thash ^= DatumGetUInt32(hash_uint32_new(key->zone));
	PG_RETURN_UINT32(thash);
}
...
Datum
text_to_date(PG_FUNCTION_ARGS)
{
	char *str = TextDatumGetCString(PG_GETARG_DATUM(0));

	return DirectFunctionCall1(date_in, CStringGetDatum(str));
}
