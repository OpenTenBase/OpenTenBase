...
static inline Datum FetchDatum(Datum vectorStartPtr, int pos, int attlen, bool attbyval)
{
	Datum value;
	...
	if(attbyval)
	{
		...
		{
			case sizeof(char):
				value = CharGetDatum(*((char *) DatumGetPointer(vectorStartPtr) + pos));
				break;
			case sizeof(int16):
				value = Int16GetDatum(*((int16 *) DatumGetPointer(vectorStartPtr) + pos));
				break;
			case sizeof(int32):
				value = Int32GetDatum(*((int32 *) DatumGetPointer(vectorStartPtr) + pos));
				break;
			case sizeof(Datum):
				value = *((Datum *) DatumGetPointer(vectorStartPtr) + pos);
				break;
			default:
				elog(ERROR, "unsupported type len %d", attlen);
				break;
		}
	}
	else
