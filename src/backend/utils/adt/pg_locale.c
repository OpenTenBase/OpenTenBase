...
db_encoding_convert(int encoding, char **str)
{
	...
	pstr = pg_any_to_server(*str, strlen(*str), encoding, true, false);
	if (pstr == *str)
			   ...
			   const char *format, const struct tm *tm)
{
	...
	{
		char	   *convstr = pg_any_to_server(dst, len, PG_UTF8, false, false);
				...
				ereport(WARNING,
						(errmsg("collation \"%s\" has version mismatch",
						 ...
						 errhint("Rebuild all objects affected by this collation and run "
								 ...
								 quote_qualified_identifier_as_pg(get_namespace_name(collform->collnamespace),
															NameStr(collform->collname)))));
		}
...
pg_locale_t
get_newlocale_from_collation_secure(Oid collid)
{
	if (collid == DEFAULT_COLLATION_OID)
	{
		return NULL;
	...
	return pg_newlocale_from_collation(collid);
}
