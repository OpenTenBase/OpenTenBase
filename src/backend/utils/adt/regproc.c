...
static Oid get_ora_system_class_oid(char *class_name, Oid view_oid);
...
Oid
regprocedurein_ext(char *pro_name_or_oid, bool is_missing)
{
	List	   *names;
	int			nargs;
...
regprocedurein(PG_FUNCTION_ARGS)
{
	...
	result = regprocedurein_ext(pro_name_or_oid, false);
	PG_RETURN_OID(result);
}
...
regclassin(PG_FUNCTION_ARGS)
{
	...
	/*
	 * In opentenbase_ora mode, there are upper-case system views for each lower-case
	 * system class like system view PG_CLASS for system class pg_class.
	 * The SQL 'pg_class'::regclass will return PG_CLASS view but we want to
	 * get pg_class system class, so convert it here.
	 */
	if (OidIsValid(result) && ORA_MODE)
		result = get_ora_system_class_oid(class_name_or_oid, result);
...
to_regclass(PG_FUNCTION_ARGS)
{
	...
	if (OidIsValid(result))
	{
		/* Try to get the catalog class while it is a system view */
		if (ORA_MODE)
			result = get_ora_system_class_oid(class_name, result);
...
regclassout(PG_FUNCTION_ARGS)
{
	...
	if (HeapTupleIsValid(classtup))
	{
		...
		else
		{
			 ...
			 */
			if (portable_output && nspname != NULL && strncmp(nspname, "pg_temp", 7) == 0)
				nspname = "pg_temp";
			else if (RelationIsVisible(classid))
				nspname = NULL;
...
regconfigin(PG_FUNCTION_ARGS)
{
	 ...
	 */
	names = stringToQualifiedNameListAsPG(cfg_name_or_oid);
...
regconfigout(PG_FUNCTION_ARGS)
{
	...
	if (HeapTupleIsValid(cfgtup))
	{
		...
		result = quote_qualified_identifier_as_pg(nspname, cfgname);
...
regdictionaryin(PG_FUNCTION_ARGS)
{
	 ...
	 */
	names = stringToQualifiedNameListAsPG(dict_name_or_oid);
...
regdictionaryout(PG_FUNCTION_ARGS)
{
	...
	if (HeapTupleIsValid(dicttup))
	{
		...
		result = quote_qualified_identifier_as_pg(nspname, dictname);
...
regrolein(PG_FUNCTION_ARGS)
{
	...
	result = get_role_oid(strVal(linitial(names)), ORA_MODE);

	/* Downcase the role name while we can't find it */
	if (ORA_MODE && !OidIsValid(result))
	{
		char *qualified_name = strVal(linitial(names));
		char *lower_role_name;

		/* Skip if the qualified role name in a double quotes */
		if (!qualified_name || qualified_name[0] == '"')
			PG_RETURN_OID(result);
		else
		{
			lower_role_name = downcase_truncate_identifier(qualified_name,
															strlen(qualified_name),
															false);

			result = get_role_oid(lower_role_name, true);
			pfree(lower_role_name);
		}
	}
...
text_regclass(PG_FUNCTION_ARGS)
{
	...
	if (OidIsValid(result) && ORA_MODE)
	{
		char *class_name = text_to_cstring(relname);
...
List *
stringToQualifiedNameListAsPG(const char *string)
{
	if (ORA_MODE)
	{
		List *result;
		 ...
		 * But the stringToQualifiedNameList may be failed,
		 * so catch it to reset the sql mode to opentenbase_ora.
		 */
		set_sql_mode(SQLMODE_POSTGRESQL);
		PG_TRY();
		{
			result = stringToQualifiedNameList(string);
		}
		PG_CATCH();
		{
...
/* Try to get the catalog class while it is a system view */
static Oid
get_ora_system_class_oid(char *class_name, Oid view_oid)
{
	HeapTuple tuple;
	...
	/* Skip to get the catalog class while its name is in the double quotes */
	if (!convert_sysview_to_sysclass ||
		(class_name && class_name[0] == '"') ||
		!OidIsValid(result))
		return result;

	tuple = SearchSysCache1(RELOID, ObjectIdGetDatum(view_oid));
	if (!HeapTupleIsValid(tuple))
		elog(ERROR, "failed to lookup cache for view %u", view_oid);
	class_form = (Form_pg_class) GETSTRUCT(tuple);

	/* It is a system view */
	if (class_form->relnamespace == PG_CATALOG_NAMESPACE &&
		view_oid < FirstNormalObjectId &&
		class_form->relkind == RELKIND_VIEW)
	{
		names = stringToQualifiedNameListAsPG(class_name);
		result = RangeVarGetRelid(makeRangeVarFromNameList(names), NoLock, true);
		list_free(names);
	}
