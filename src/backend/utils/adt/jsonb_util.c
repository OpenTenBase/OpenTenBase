...
void
JsonbHashScalarValueNew(const JsonbValue *scalarVal, uint32 *hash)
{
	uint32		tmp;
	...
	switch (scalarVal->type)
	{
		case jbvNull:
			...
			break;
		case jbvString:
			tmp = DatumGetUInt32(hash_any_new((const unsigned char *) scalarVal->val.string.val,
										  scalarVal->val.string.len));
			break;
		case jbvNumeric:
			/* Must hash equal numerics to equal hash codes */
			tmp = DatumGetUInt32(DirectFunctionCall1(hash_numeric_new,
													 NumericGetDatum(scalarVal->val.numeric)));
			break;
		case jbvBool:
			tmp = scalarVal->val.boolean ? 0x02 : 0x04;
