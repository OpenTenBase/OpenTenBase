...
#define AARR_FREE_DATUM_IF_COPY(array, data) \
	do { \
		if (!VARATT_IS_EXPANDED_HEADER(array)) \
			PG_FREE_DATUM_IF_COPY(array, data); \
	} while (0)
...
static void ReadArrayStr(char *arrayStr, const char *origStr,
			 ...
			 bool *hasnulls, bool *exists,
			 bool *hasexists, int32 *nbytes);
...
static Datum array_set_element_expanded(Datum arraydatum,
						   ...
						   bool isExist, int arraytyplen,
						   int elmlen, bool elmbyval, char elmalign);
static bool array_get_isnull(const bits8 *nullbitmap, int offset);
static void array_set_isnull(bits8 *nullbitmap, int offset, bool isNull);
static void array_set_isexist(bits8 *existbitmap, int offset, bool isExist);
...
array_in(PG_FUNCTION_ARGS)
{
	...
	if (ndim == 0)
	{
		if (*p != '{')
		{
			/* To be compatible with opentenbase_ora, convert string with {%s} format */
			if (ORA_MODE)
			{
				int len = strlen(p) + 1;
				/* new string with {%s} format, so new length should add 2 for {} */
				string_assembled = (char *) palloc0(len + 2);
				snprintf(string_assembled, len + 2, "{%s}", p);
				p = string_assembled;
			}
			else
			{
	...
	existsPtr = (bool *) palloc0(nitems * sizeof(bool));
	...
	if (ORA_MODE && hasexists)
	{
		dataoffset = ARR_OVERHEAD_WITHEXISTS(ndim, nitems);
		nbytes += dataoffset;
	}
	else if (hasnulls)
	{
	...
	CopyArrayEls(retval,
				 dataPtr, nullsPtr, existsPtr,
				 nitems, typlen, typbyval, typalign,
				 true);
...
ArrayCount(const char *str, int *dim, char typdelim)
{
	...
	bool		reconstruct_array = false;
	const char *ptr;
	ArrayParseState parse_state = ARRAY_NO_LEVEL;

	for (i = 0; i < MAXDIM; ++i)
	{
		temp[i] = temp_max[i] = dim[i] = nelems_last[i] = 0;
		nelems[i] = 1;
	}
			...
			switch (*ptr)
			{
				...
				case '{':
					if (!in_quotes)
					{
						...
						if (temp_max[nest_level] < temp[nest_level])
							temp_max[nest_level] = temp[nest_level];
				...
				default:
					if (!in_quotes)
					{
						...
						else if (!array_isspace(*ptr))
						{
							...
							if (ORA_MODE && array_elem_level != -1 &&
								nest_level != array_elem_level)
								ereport(ERROR,
									(errcode(ERRCODE_INVALID_TEXT_REPRESENTATION),
											 errmsg("malformed array literal: \"%s\"", str),
											 errdetail("Multidimensional arrays must have "
			 ...
			 int32 *nbytes)
{
	...
	memset(exists, false, nitems * sizeof(bool));
			 ...
			 bool freedata)
{
	...
	int			bitval = 0;
	int			bitmask = 1;
	bits8	   *bitmap_exists = NULL;
	int			bitval_exists = 0;
	int			bitmask_exists = 1;
	int			i;

	if (ORA_MODE)
		bitmap_exists = ARR_EXISTBITMAP(array, nitems);
	...
	for (i = 0; i < nitems; i++)
	{
		if (ORA_MODE && exists)
		{
			...
			{
				if (!nulls || !nulls[i])
					elog(ERROR, "The corresponding position of the non-existent value in the array is not null");

				if (!bitmap_exists)
					elog(ERROR, "Mismatched dimensions of subarrays of multidimensional arrays are not supported");
			}
			else
				bitval_exists |= bitmask_exists;

			if (bitmap_exists)
			{
				bitmask_exists <<= 1;
				if (bitmask_exists == 0x100)
				{
					*bitmap_exists++ = bitval_exists;
					bitval_exists = 0;
					bitmask_exists = 1;
				}
			}
	...
	if (bitmap_exists && bitmask_exists != 1)
		*bitmap_exists = bitval_exists;
}
...
array_out(PG_FUNCTION_ARGS)
{
	...
	exists = (bool *) palloc(nitems * sizeof(bool));
	memset(exists, true, nitems * sizeof(bool));
	...
	for (i = 0; i < nitems; i++)
	{
		...
		itemvalue = array_iter_next(&iter, &isnull, &isexist, i,
									typlen, typbyval, typalign);
...
array_recv(PG_FUNCTION_ARGS)
{
	...
	{
		/*
		 * Limit type checking to built_in types. The oid of user-defined
		 * types may be different on different datanodes.
		 */
		if (element_type < FirstBootstrapObjectId &&
			spec_element_type < FirstBootstrapObjectId)
				ereport(ERROR,
	...
	dataPtr = (Datum *) palloc(nitems * sizeof(Datum));
	nullsPtr = (bool *) palloc(nitems * sizeof(bool));
	existsPtr = (bool *) palloc(nitems * sizeof(bool));
	...
	if (auxdata)
	{
		auxdata_len = VARSIZE(auxdata);
		dataoffset = ARR_OVERHEAD_WITHAUXDATA(ndim, nitems, auxdata_len);
		nbytes += dataoffset;
	}
	else if (hasexists)
	{
	...
	if (ARR_HASAUXDATA(retval, nitems) && auxdata)
	{
		bytea	*retval_auxdata = ARR_AUXDATA(retval, nitems);

		if (retval_auxdata)
			memcpy(retval_auxdata, auxdata, auxdata_len);

		pfree(auxdata);
	}

	CopyArrayEls(retval, dataPtr, nullsPtr,
				 existsPtr, nitems, typlen,
				 typbyval, typalign, true);
				...
				int32 *nbytes)
{
	...
	for (i = 0; i < nitems; i++)
	{
		...
		if (itemlen < -2 || itemlen > (buf->len - buf->cursor))
			ereport(ERROR,
					(errcode(ERRCODE_INVALID_BINARY_REPRESENTATION),
 ...
 *	*isExist is set to indicate whether the element exists.
 */
Datum
				  ...
				  bool *isExist)
{
	...
	else
	{
		...
		arrayexistsptr =  ARR_EXISTBITMAP(array, nelems);
	}
	...
	/*
	 * Check for Exist array element
	 */
	if (isExist)
		*isExist = array_get_isexist(arrayexistsptr, offset);
				  ...
				  char elmalign)
{
	...
	newhasexists = (ARR_HASEXIST(array, ArrayGetNItems(ndim, dim)) || !isExist);
	 ...
	 */
	if (newhasexists)
		overheadlen = ARR_OVERHEAD_WITHEXISTS(ndim, newnitems);
	else if (newhasnulls)
	...
	oldexistbitmap = ARR_EXISTBITMAP(array, oldnitems);
	...
	newarray->ndim = ndim;
	newarray->dataoffset = (newhasnulls || newhasexists) ? overheadlen : 0;
	...
	if (newhasexists)
	{
		bits8	   *newexistbitmap;

		newexistbitmap = ARR_EXISTBITMAP(newarray, newnitems);

		/* bitmap_exist does not exist in one-dimensional array */
		MemSet(newexistbitmap, 0, (newnitems + 7) / 8);
		array_set_isexist(newexistbitmap, offset, isExist);

		/*
		 * If oldexistbitmap is empty, the value corresponding to newexistbitmap
		 * is set to 1 by default.
		 */
		array_bitmap_copy(newexistbitmap, 0,
						  oldexistbitmap, 0,
						  offset);
		array_bitmap_copy(newexistbitmap, offset + 1,
						  oldexistbitmap, offset + 1,
						  oldnitems - offset - 1);
	}
						   ...
						   int elmlen, bool elmbyval, char elmalign)
{
	...
	newhasexists = ((dexists != NULL) || !isExist);
	addedbefore = addedafter = 0;
	...
	/*
	 * If we need a nulls/exists bitmap and don't already have one, create it, being
	...
	if (newhasexists && dexists == NULL)
	{
		eah->dexists = dexists = (bool *)
			MemoryContextAllocZero(eah->hdr.eoh_context,
								   eah->dvalueslen * sizeof(bool));
		MemSet(dexists, true, eah->dvalueslen * sizeof(bool));
	}
		  ...
		  Oid retType, ArrayMapState *amstate)
{
	...
	for (i = 0; i < nitems; i++)
	{
		/* Get source element, checking for NULL */
		*transform_source =
			array_iter_next(&iter, transform_source_isnull, &exists[i], i,
		...
		if (ORA_MODE && !exists[i])
		{
			...
			/*
			 * When !exists[i], the value corresponding to
			 * values[i] should be NULL.
			 */
			if (!nulls[i])
				elog(ERROR, "Elements that do not exist in the array have non-NULL values");
		}
...
ArrayType *
construct_array_with_all_notexist(Oid elmtype, int ndim,
								  int *dims, int *lbs)
{
	int			nbytes;
	ArrayType	*result;

	nbytes = ARR_OVERHEAD_WITHEXISTS(ndim, ArrayGetNItems(ndim, dims));
	result = (ArrayType *) palloc0(nbytes);
	SET_VARSIZE(result, nbytes);

	result->ndim = ndim;
	result->dataoffset = nbytes;
	result->elemtype = elmtype;
	memcpy(ARR_DIMS(result), dims, ndim * sizeof(int));
	memcpy(ARR_LBOUND(result), lbs, ndim * sizeof(int));
...
/*
 * deconstruct_array_exists --- parse bitmap_exist in array into existssp.
 */
void
deconstruct_array_exists(ArrayType *array, bool **existsp)
{
	int			i;
	int			nelems;
	bool	   *exists;
	...
	nelems = ArrayGetNItems(ARR_NDIM(array), ARR_DIMS(array));
	bitmap_exist = ARR_EXISTBITMAP(array, nelems);
	if (bitmap_exist == NULL || existsp == NULL)
		return;

	*existsp = exists = (bool *) palloc0(nelems * sizeof(bool));

	for (i = 0; i < nelems; i++)
	{
		if (bitmap_exist && (*bitmap_exist & bitmask_exist) != 0)
			exists[i] = true;
...
array_eq(PG_FUNCTION_ARGS)
{
	...
	else
	{
		...
		for (i = 0; i < nitems; i++)
		{
			...
			elt1 = array_iter_next(&it1, &isnull1, NULL, i,
								   typlen, typbyval, typalign);
			elt2 = array_iter_next(&it2, &isnull2, NULL, i,
								   typlen, typbyval, typalign);
...
array_cmp(FunctionCallInfo fcinfo)
{
	...
	for (i = 0; i < min_nitems; i++)
	{
		...
		/* Get elements, checking for NULL */
		elt1 = array_iter_next(&it1, &isnull1, NULL, i, typlen, typbyval, typalign);
		elt2 = array_iter_next(&it2, &isnull2, NULL, i, typlen, typbyval, typalign);
...
hash_array(PG_FUNCTION_ARGS)
{
	...
	for (i = 0; i < nitems; i++)
	{
		...
		elt = array_iter_next(&iter, &isnull, NULL, i, typlen, typbyval, typalign);
...
/*
 * array_subscript_is_valid :
 *		returns true if the subscript is valid, otherwise false
 */
Datum
array_subscript_is_valid(PG_FUNCTION_ARGS)
{
    int     subscript = PG_GETARG_INT32(2);	
    int			lower, upper;

    lower = array_lower(fcinfo);
    upper = array_upper(fcinfo);

    if ((lower <= subscript) && (subscript <= upper))
       PG_RETURN_BOOL(true); 
...
/*
 * Check whether a specific array element is Exist
 *
 * existbitmap: pointer to array's exist bitmap (NULL if none)
 * offset: 0-based linear element number of array element
 */
bool
array_get_isexist(const bits8 *existbitmap, int offset)
{
	if (existbitmap == NULL)
		return true;			/* assume exist */
	if (existbitmap[offset / 8] & (1 << (offset % 8)))
		return true;			/* exist */
	return false;
}

/*
 * Set a specific array element's exist-bitmap entry
 *
 * existbitmap: pointer to array's exist bitmap (mustn't be NULL)
 * offset: 0-based linear element number of array element
 * isExist: exist status to set
 */
static void
array_set_isexist(bits8 *existbitmap, int offset, bool isExist)
{
	int			bitmask;

	existbitmap += offset / 8;
	bitmask = 1 << (offset % 8);
...
array_unnest(PG_FUNCTION_ARGS)
{
	...
	if (fctx->nextelem < fctx->numelems)
	{
		...
		elem = array_iter_next(&fctx->iter, &fcinfo->isnull, NULL, offset,
...
Datum
array_unnest_dim(PG_FUNCTION_ARGS)
{
	typedef struct
	{
		array_iter	iter;
	...
	MemoryContext oldcontext;
	int				ndim = 0;
	AnyArrayType	*arr = NULL;
	ArrayBuildState *astate = NULL;

	if (PG_ARGISNULL(0))
		return (Datum) 0;

	if (PG_ARGISNULL(1) || ((ndim = PG_GETARG_INT32(1)) <= 0))
		elog(ERROR, "parameter ndim only support 1 right now.");

	arr = PG_GETARG_ANY_ARRAY_P(0);

	if (AARR_NDIM(arr) != 2)
		elog(ERROR, "unnest_dim only support two dim array.");
	...
	if (SRF_IS_FIRSTCALL())
	{
		...
		fctx->nextelem = 0;
		fctx->numelems = ArrayGetNItems(AARR_NDIM(arr), AARR_DIMS(arr));
		fctx->numdimelems = ArrayGetNItems(ndim, AARR_DIMS(arr));
		fctx->numdimelems = fctx->numelems / fctx->numdimelems;
	...
	{
		Datum result;
		int		off = fctx->nextelem;

		while (fctx->nextelem < fctx->numdimelems + off)
		{
			int			offset = fctx->nextelem++;
			Datum		elem;
			bool		isnull;

			/* Doesn't care about lbound, which is used by arr slice in pg mode */
			elem = array_iter_next(&fctx->iter, &isnull, NULL, offset,
								fctx->elmlen, fctx->elmbyval, fctx->elmalign);
		...
		/*
		 * Consider the multidim array, reserve the ablilty of construct an 
		 * subarray of multi-dimensions. if want to unnest dim one by one of
		 * an array who's dimension more than 2, just call this function 
		 * recursively.
		 */
		result = makeMdArrayResult(astate,
								AARR_NDIM(arr) - ndim, /* unnest one dim */
								AARR_DIMS(arr) + ndim, /* the remain dims of the origin arr */
								AARR_LBOUND(arr) + ndim, /* the remain lbound of the origin arr */
								CurrentMemoryContext,
								astate->private_cxt);
...
Datum
multiset_expr(PG_FUNCTION_ARGS)
{
	Datum			lnt;
	Datum			rnt;
	SetOperation	setop;
	...
	lnt = PG_GETARG_DATUM(0);
	rnt = PG_GETARG_DATUM(1);
	setop = PG_GETARG_INT32(2);
	all = !!PG_GETARG_INT32(3);

	if (PG_ARGISNULL(0) || PG_ARGISNULL(1))
		PG_RETURN_ARRAYTYPE_P(construct_empty_array(fcinfo->flinfo->fn_rettype));

#define CALLMUTLISETFN(foid, arg0, arg1, lnull, rnull) \
do \
{ \
	fmgr_info(foid, &flinfo); \
	InitFunctionCallInfoData(fcinfo1, &flinfo, 2, InvalidOid, NULL, NULL); \
	fcinfo1.arg[0] = arg0; \
	fcinfo1.arg[1] = arg1; \
	fcinfo1.argnull[0] = lnull; \
	fcinfo1.argnull[1] = rnull; \
	fcinfo1.flinfo->fn_expr = fcinfo->flinfo->fn_expr; \
	...
	{
		if (all)
			CALLMUTLISETFN(9124, lnt, rnt, PG_ARGISNULL(0), PG_ARGISNULL(1));
		else
			CALLMUTLISETFN(9125, lnt, rnt, PG_ARGISNULL(0), PG_ARGISNULL(1));
	}
	else if (setop == SETOP_INTERSECT)
	{
		if (all)
			CALLMUTLISETFN(9126, lnt, rnt, PG_ARGISNULL(0), PG_ARGISNULL(1));
		else
			CALLMUTLISETFN(9127, lnt, rnt, PG_ARGISNULL(0), PG_ARGISNULL(1));
	}
	else if (setop == SETOP_EXCEPT)
	{
		if (all)
			CALLMUTLISETFN(9128, lnt, rnt, PG_ARGISNULL(0), PG_ARGISNULL(1));
		else
			CALLMUTLISETFN(9129, lnt, rnt, PG_ARGISNULL(0), PG_ARGISNULL(1));
	}
	else
		elog(ERROR, "unsupproted set operator, %d", setop);

#undef CALLMUTLISETFN
