			 ...
			 ArrayMetaState *metacache)
{
	...
	/*
	 * aux_data is set to null and is only used
	 * when obtaining the subscript.
	 */
	eah->aux_data = NULL;
						  ...
						  ExpandedArrayHeader *oldeah)
{
	...
	if (oldeah->dexists)
	{
		eah->dexists = (bool *) MemoryContextAlloc(objcxt,
												   dvalueslen * sizeof(bool));
		memcpy(eah->dexists, oldeah->dexists, dvalueslen * sizeof(bool));
	}
	else
		eah->dexists = NULL;
...
EA_get_flat_size(ExpandedObjectHeader *eohptr)
{
	...
	if (dexists)
		nhead = ARR_OVERHEAD_WITHEXISTS(ndims, nelems);
	else if (dnulls)
		nhead = ARR_OVERHEAD_WITHNULLS(ndims, nelems);
	else
		nhead = ARR_OVERHEAD_NONULLS(ndims);
				...
				void *result, Size allocated_size)
{
	...
	if (eah->dexists)
		dataoffset = ARR_OVERHEAD_WITHEXISTS(ndims, nelems);
	else if (eah->dnulls)
	...
	if (ARR_HASEXIST(aresult, nelems))
	{
		bits8	*existbitmap = ARR_EXISTBITMAP(aresult, nelems);

		if (existbitmap)
		{
			/* ARR_NULLBITMAP and ARR_EXISTBITMAP are the same size. */
			memset(existbitmap, 0xff, ARR_NULLBITMAP_SIZE(nelems));
		}
	}

	if (flatted_aux_data && ARR_HASAUXDATA(aresult, nelems))
	{
		bytea	*auxdata = ARR_AUXDATA(aresult, nelems);

		if (auxdata)
			memcpy(auxdata, flatted_aux_data, auxdata_len);
	}

	if (flatted_aux_data)
		pfree(flatted_aux_data);

	CopyArrayEls(aresult,
				 eah->dvalues, eah->dnulls, eah->dexists, nelems,
...
deconstruct_expanded_array(ExpandedArrayHeader *eah)
{
	if (eah->dvalues == NULL)
	{
		...
		deconstruct_array(eah->fvalue,
						  ...
						  &nelems);
		deconstruct_array_exists(eah->fvalue, &dexists);
