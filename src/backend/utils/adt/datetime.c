	...
	if (fsec != 0)
	{
		...
		*cp++ = '.';
		...
		while (precision--)
		{
			...
			if (gotnonzero || ORA_MODE)
				cp[precision] = DIG(remainder);
			else
				end = &cp[precision];
		}
		...
		switch (ftype[i])
		{
			case DTK_DATE:
						  ...
						  (DTK_M(MONTH) | DTK_M(DAY))))
				{
					...
					{
						namedTz = pg_tzset(field[i], false);
						if (!namedTz)
						{
							/*
		   ...
		   int *tmask, struct pg_tm *tm, fsec_t *fsec)
{
	...
	*tmask = DTK_TIME_M;
	...
	if (ORA_MODE && IS_LOCAL_ACCESS_NODE &&
		...
		*cp != ':')
	{
		elog(ERROR, "The interval is invalid");
	}
	...
	if (ORA_MODE && (tm->tm_hour > HOURS_PER_DAY - 1 ||
...
DecodeTimezoneName(const char *tzname, int *offset, pg_tz **tz)
{
	...
	type = DecodeTimezoneAbbrev(0, lowzone, offset, tz);
...
static void
CheckIntervalTimeFormat(int *ftype, int nf, int range)
{
	if (!ORA_MODE || !IS_LOCAL_ACCESS_NODE ||
		IntervalStyle != INTSTYLE_SQL_STANDARD)
		return;

	 /* DAY TO SECOND must in format of "DAY HOUR:MINUTE:SECOND.[FF]" */
	if ((range == (INTERVAL_MASK(DAY) | INTERVAL_MASK(HOUR) |
		INTERVAL_MASK(MINUTE) | INTERVAL_MASK(SECOND))) &&
		(nf != 2 || !((ftype[0] == DTK_TIME) || (ftype[1] == DTK_TIME))))
	{
		elog(ERROR, "The interval is invalid");
	}
	/* DAY TO MINUTE must in format of "DAY HOUR:MINUTE" */
	if ((range == (INTERVAL_MASK(DAY) | INTERVAL_MASK(HOUR) |
	...
	/* HOUR TO SECOND must in format of "HOUR:MINUTE:SECOND"*/
	if ((range == (INTERVAL_MASK(HOUR) | INTERVAL_MASK(MINUTE) |
		INTERVAL_MASK(SECOND))) && (nf != 1 || ftype[0] != DTK_TIME))
	{
		elog(ERROR, "The interval is invalid");
	}
	/* HOUR TO MINUTE must in format of "HOUR:MINUTE"*/
	if ((range == (INTERVAL_MASK(HOUR) | INTERVAL_MASK(MINUTE))) &&
		(nf != 1 || ftype[0] != DTK_TIME))
	{
		elog(ERROR, "The interval is invalid");
	}
	/* MINUTE TO SECOND must in format of "MINUTE:SECOND"*/
	if ((range == (INTERVAL_MASK(MINUTE) | INTERVAL_MASK(SECOND))) &&
		(nf != 1 || ftype[0] != DTK_TIME))
	{
		elog(ERROR, "The interval is invalid");
			   ...
			   int *dtype, struct pg_tm *tm, fsec_t *fsec)
{
	...
	*dtype = DTK_DELTA;
	...
	for (i = nf - 1; i >= 0; i--)
	{
		...
		{
			case DTK_TIME:
				CheckIntervalTimeFormat(ftype, nf, range);
			...
			case DTK_NUMBER:
				...
				if (ORA_MODE && IS_LOCAL_ACCESS_NODE &&
					IntervalStyle == INTSTYLE_SQL_STANDARD && val)
				{
					/* YEAR TO MONTH format must be "year-month" */
					if (range == (INTERVAL_MASK(YEAR) | INTERVAL_MASK(MONTH)) && *cp != '-')
						elog(ERROR, "The interval is invalid");
					/* DAY TO HOUR must in format of "DAY HOUR" */
					if (range == (INTERVAL_MASK(DAY) | INTERVAL_MASK(HOUR)) && nf != 2)
					{
						elog(ERROR, "The interval is invalid");
					}
					CheckIntervalTimeFormat(ftype, nf, range);
...
EncodeDateOnly(struct pg_tm *tm, int style, char *str)
{
	...
	switch (style)
	{
		...
		case USE_SQL_DATES:
			/* compatible with opentenbase_ora/Ingres date formats */
...
{
	EncodeInterval_WithTypmod(tm, fsec, style, str, -1);
}

/* For Year-Month */
static void
EncodeYearMon(char *cp, int typmod, int year, int mon, bool is_neg)
{
	int		year_w = INTERVAL_YEAR_PRECISION(typmod);

	if (year_w < 0 || year_w > MAX_INTERVAL_YEAR_PRECISION)
		year_w = INTERVAL_DEFAULT_YEAR_PRECISION;

	if (!is_neg)
		*cp++ = '+';
	sprintf(cp, "%0*d-%02d", year_w, year, mon);
}

/* For Day Hour:Minute:00 */
static char *
EncodeDayMin(char *cp, int typmod, int day, int hour, int min, bool is_neg)
{
	int		day_w = INTERVAL_DAY_PRECISION(typmod);

	if (day_w < 0 || day_w > MAX_INTERVAL_DAY_PRECISION)
		day_w = INTERVAL_DEFAULT_YEAR_PRECISION;

	if (!is_neg)
		*cp++ = '+';
	sprintf(cp, "%0*d %02d:%02d:00", day_w, day, hour, min);
	cp += strlen(cp);
	*cp = '\0';
...
/* For Day Hour:Minute:Second.9(n) */
static char *
EncodeDaySec(char *cp, int typmod, int day, int hour, int min,
						int sec, fsec_t fsec, bool is_neg)
{
	int		day_w = INTERVAL_DAY_PRECISION(typmod);
	int		second_prec = INTERVAL_SEC_PRECISION(typmod);
	int		loop = 0;
	int		factor = 1;
	int		count = 0;
	int		tmp_fsec = fsec;
	...
	if (day_w < 0 || day_w > MAX_INTERVAL_DAY_PRECISION)
		day_w = MAX_INTERVAL_DAY_PRECISION;
	if (second_prec < 0 || second_prec > MAX_INTERVAL_PRECISION)
		second_prec = MAX_INTERVAL_PRECISION;

	if (!is_neg)
		*cp++ = '+';
	sprintf(cp, "%0*d %02d:%02d:", day_w, day, hour, min);
	cp += strlen(cp);
	...
	loop = count - second_prec;
	while (loop > 0)
	{
		factor *= 10;
		loop--;
	}
	cp = AppendSeconds(cp, sec, fsec / factor, second_prec, true);

	if (second_prec && fsec == 0)
	{
		*cp++ = '.';
		while (second_prec--)
		{
...
void
EncodeInterval_WithTypmod(struct pg_tm *tm, fsec_t fsec, int style, char *str,
							int typmod)
{
	char	   *cp = str;
	int			year = tm->tm_year;
		...
		case INTSTYLE_SQL_STANDARD:
			{
				...
				{
					if (ORA_MODE && typmod != -1)
					{
						if ((fields & INTERVAL_MASK(YEAR)) ||
								(fields & INTERVAL_MASK(MONTH)))
						{
							/* output YEAR-MONTH */
							EncodeYearMon(cp, typmod, 0, 0, is_neg);
						}
						else if (!(fields & INTERVAL_MASK(SECOND)))
						{
							/* output DAY HOUR:MIN:SEC */
							cp = EncodeDayMin(cp, typmod, 0, 0, 0, is_neg);
						}
						else
						{
							/* output DAY HOUR:MIN:SEC.9(n) */
							cp = EncodeDaySec(cp, typmod, 0, 0, 0, 0, 0, is_neg);
						}
					}
					else if (ORA_MODE)
						sprintf(cp, "+000000000 00:00:00.000000");
					else
						sprintf(cp, "0");
				}
				else if (!sql_standard_value && ORA_MODE)
				{
					if (has_negative && has_positive)
					{
						int64 tm_secs = 0;
						int64 remainer = 0;
						struct pg_tm tt;
						struct pg_tm *tm_new = &tt;
						tm_secs = ((tm->tm_hour * MINS_PER_HOUR + tm->tm_min) * SECS_PER_MINUTE) + tm->tm_sec;
						tm_secs += tm->tm_year * SECS_PER_YEAR + ((tm->tm_mon * DAYS_PER_MONTH) + tm->tm_mday) * SECS_PER_DAY;
						remainer = tm_secs;
						tm_new->tm_year = remainer / (SECS_PER_YEAR);
						remainer -= (tm_new->tm_year * SECS_PER_YEAR);
						tm_new->tm_mon = remainer / (DAYS_PER_MONTH * SECS_PER_DAY);
						remainer -= (tm_new->tm_mon * DAYS_PER_MONTH * SECS_PER_DAY);
						tm_new->tm_mday = remainer / SECS_PER_DAY;
						remainer -= (tm_new->tm_mday * SECS_PER_DAY);
						tm_new->tm_hour = remainer / (MINS_PER_HOUR * SECS_PER_MINUTE);
						remainer -= (tm_new->tm_hour  * MINS_PER_HOUR * SECS_PER_MINUTE);
						tm_new->tm_min = remainer / SECS_PER_MINUTE;
						remainer -= (tm_new->tm_min  * SECS_PER_MINUTE);
						tm_new->tm_sec = remainer;
						EncodeInterval_WithTypmod(tm_new, fsec, style, str, typmod);
					}
					else if (has_year_month && has_day_time)
					{
						if (!is_neg)
							sprintf(cp, "+%09d-%02d", year, mon);
						else
							sprintf(cp, "%09d-%02d", year, mon);
					}
				}
				...
				else if (has_year_month)
				{
					if (ORA_MODE && typmod != -1)
					{
						EncodeYearMon(cp, typmod, year, mon, is_neg);
						break;
					}
					else if (ORA_MODE)
					{
				...
				else if (has_day)
				{
					if (ORA_MODE && typmod != -1)
					{
						...
						{
							/* output DAY HOUR:MIN:SEC */
							cp = EncodeDayMin(cp, typmod, mday, hour, min, is_neg);
						}
						else
						{
							/* output DAY HOUR:MIN:SEC.9(n) */
							cp = EncodeDaySec(cp, typmod, mday, hour, min,
													sec, fsec, is_neg);
						}
						break;
					}
					...
					{
						if (!is_neg)
							sprintf(cp, "+%09d %02d:%02d:%02d.%06d", mday, hour, min, sec, fsec);
						else
							sprintf(cp, "%09d %02d:%02d:%02d.%06d", mday, hour, min, sec, fsec);
						break;
					}
				...
				else
				{
					if (ORA_MODE && typmod != -1)
					{
						...
						{
							/* output DAY HOUR:MIN:SEC */
							cp = EncodeDayMin(cp, typmod, 0, hour, min, is_neg);
						}
						else
						{
							/* output DAY HOUR:MIN:SEC.9(n) */
							cp = EncodeDaySec(cp, typmod, 0, hour, min,
													sec, fsec, is_neg);
						}
						break;
					}
...
FetchDynamicTimeZone(TimeZoneAbbrevTable *tbl, const datetkn *tp)
{
	...
	if (dtza->tz == NULL)
	{
		dtza->tz = pg_tzset(dtza->zone, false);
