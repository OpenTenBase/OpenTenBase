...
int2in(PG_FUNCTION_ARGS)
{
    ...
    if (ORA_MODE)
    {
	    ...
	    /* check for overflow of int, if integer too large, treat it as a float */
	    if (*endptr != '\0' || errno == ERANGE || val < SHRT_MIN || val > SHRT_MAX)
	    {
		    /* support insert numeric-type data into int2/in4/int8-type column */
		    return DirectFunctionCall1(numeric_int2,
		                               DirectFunctionCall3(numeric_in,
		                                                   PG_GETARG_DATUM(0),
		                                                   PG_GETARG_OID(InvalidOid),
		                                                   Int32GetDatum(-1)));
	    }
    }
#endif
...
int4in(PG_FUNCTION_ARGS)
{
    ...
    if (ORA_MODE)
    {
	    ...
	    val = strtol(num, &endptr, 10);
	    /* check for overflow of int, if integer too large, treat it as a float */
	    if (*endptr != '\0' || errno == ERANGE || val != (int32) val)
	    {        /* support insert numeric-type data into int2/in4/int8-type column */
...
Datum
int4remainder(PG_FUNCTION_ARGS)
{
	int32		arg1 = PG_GETARG_INT32(0);
	int32		arg2 = PG_GETARG_INT32(1);
	...
	PG_RETURN_INT32(arg1 - pg_banker_round((double)arg1 / arg2) * arg2);
}

Datum
int2remainder(PG_FUNCTION_ARGS)
{
	int16 arg1 = PG_GETARG_INT16(0);
	int16 arg2 = PG_GETARG_INT16(1);
	...
	PG_RETURN_INT16(arg1 - pg_banker_round((double) arg1 / arg2) * arg2);
}
...
Datum
varchar_to_int4(PG_FUNCTION_ARGS)
{
	char *str = TextDatumGetCString(PG_GETARG_DATUM(0));

	return DirectFunctionCall1(int4in, CStringGetDatum(str));
}

/* Cast int2 -> interval */
Datum
int2_interval(PG_FUNCTION_ARGS)
{
	PG_RETURN_DATUM(DirectFunctionCall1(numeric_interval,
	                                    DirectFunctionCall1(int2_numeric, PG_GETARG_DATUM(0))));
}

/* Cast int4 -> interval */
Datum
int4_interval(PG_FUNCTION_ARGS)
{
	PG_RETURN_DATUM(DirectFunctionCall1(numeric_interval,
