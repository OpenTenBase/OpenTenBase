...
static unsigned
b64_encode_base(const char *src, unsigned srclen, char *dst, unsigned dstlen, bool safe_check)
{
	char	   *p,
			   *lend = dst + 76;
	const char *s,
	...
	while (s < end)
	{
		...
		if (pos < 0)
		{
			if (safe_check && (p - dst + 4 > dstlen))
				ereport(ERROR,
						(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
						 errmsg("target length is not long enough")));
...
unsigned
b64_encode_safe(const char *src, unsigned srclen, char *dst, unsigned dstlen)
{
	return b64_encode_base(src, srclen, dst, dstlen, true);
}

static unsigned
b64_encode(const char *src, unsigned len, char *dst)
{
	return b64_encode_base(src, len, dst, 0, false);
}

static unsigned
b64_decode_base(const char *src, unsigned srclen, char *dst, unsigned dstlen, bool safe_check)
{
...
unsigned
b64_decode_safe(const char *src, unsigned srclen, char *dst, unsigned dstlen)
{
	return b64_decode_base(src, srclen, dst, dstlen, true);
}

static unsigned
b64_decode(const char *src, unsigned len, char *dst)
{
	return b64_decode_base(src, len, dst, 0, false);
}
