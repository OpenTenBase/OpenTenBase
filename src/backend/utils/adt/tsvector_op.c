...
{
	TSVector    in1 = PG_ARGISNULL(0) ? NULL : PG_GETARG_TSVECTOR(0);
	TSVector    in2 = PG_ARGISNULL(1) ? NULL : PG_GETARG_TSVECTOR(1);
	TSVector	out;
	WordEntry  *ptr;
	WordEntry  *ptr1,
			   *ptr2;
	...
	if (ORA_MODE || enable_lightweight_ora_syntax)
	{
		...
		{
			out = PG_GETARG_TSVECTOR_COPY(1);
			PG_RETURN_POINTER(out);
		}
		else if (in2 == NULL)
		{
			out = PG_GETARG_TSVECTOR_COPY(0);
			PG_RETURN_POINTER(out);
		}
	}
