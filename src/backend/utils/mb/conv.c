		  ...
		  int lc, int encoding)
{
	int			c1;
	mbverifier  mbverify;
	char        replace_char = copy_illegal_conv_char == 0 ? '?' : copy_illegal_conv_char;
	...
	while (len > 0)
	{
		...
		if (c1 == 0)
		{
			if (copy_illegal_chars_conversion || enable_null_string)
			{
				CONVERT_ILLEGAL_CHAR(p, mic, len, ' ');
				continue;
			}
			else
		...
		else
		{
			...
			if (len < l)
			{
				if (copy_illegal_chars_conversion || enable_null_string)
				{
					CONVERT_ILLEGAL_CHAR(p, mic, len, replace_char);
					continue;
				}
				else
					report_invalid_encoding(PG_MULE_INTERNAL, (const char *) mic, len);
			...
			if (l != 2 || c1 != lc || !IS_HIGHBIT_SET(mic[1]))
			{
				...
				{
					/*
					 * Here we need call pg_mule_verifier() to be sure whether the current mic byte
					 * sequence is valid not not. If it's invalid, p to be placed
					 * copy_illegal_conv_char and mic go to the next byte for next check. If it's
					 * valid, p to be placed copy_illegal_conv_char and mic can safely skip l bytes
					 * for next check. fetch function pointer just once
					 */
					...
					{
						/*
						 * use copy_illegal_conv_char as conversion.
						 */
						*p++ = replace_char;
						mic++;
						len--;
		   ...
		   int encoding)
{
	...
	for (; len > 0; len -= l)
	{
		...
		if (*utf == '\0')
		{
			if (copy_illegal_chars_conversion || enable_null_string)
			{
				CONVERT_ILLEGAL_CHAR_DELTA(iso, utf, l, ' ');
				continue;
			}
			else
		...
		if (len < l || !pg_utf8_islegal(utf, l))
		{
			if (copy_illegal_chars_conversion || enable_null_string)
			{
				CONVERT_ILLEGAL_CHAR_DELTA(iso, utf, l, replace_char);
				continue;
			}
			else
				report_invalid_encoding(PG_UTF8, (const char *) utf, len);
		   ...
		   int encoding)
{
	...
	for (; len > 0; len -= l)
	{
		...
		if (*iso == '\0')
		{
			if (copy_illegal_chars_conversion || enable_null_string)
			{
				CONVERT_ILLEGAL_CHAR_DELTA(utf, iso, l, ' ');
				continue;
			}
			else
		...
		if (l < 0)
		{
			if (copy_illegal_chars_conversion || enable_null_string)
			{
				CONVERT_ILLEGAL_CHAR_DELTA(utf, iso, l, replace_char);
				continue;
			}
			else
				report_invalid_encoding(encoding, (const char *) iso, len);
