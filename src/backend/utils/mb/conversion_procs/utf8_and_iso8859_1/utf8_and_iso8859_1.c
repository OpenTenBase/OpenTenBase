...
iso8859_1_to_utf8(PG_FUNCTION_ARGS)
{
	...
	while (len > 0)
	{
		...
		if (c == 0)
		{
			...
			{
				CONVERT_ILLEGAL_CHAR(dest, src, len, ' ');
				continue;
			}
			else
	...
	while (len > 0)
	{
		...
		else
		{
			...
			if (l > len || !pg_utf8_islegal(src, l))
			{
				if (copy_illegal_chars_conversion || enable_null_string)
				{
					CONVERT_ILLEGAL_CHAR_SKIP(dest, src, len, 1, replace_char);
					continue;
				}
				else
					report_invalid_encoding(PG_UTF8, (const char *) src, len);
			...
			if (l != 2)
			{
				if (copy_illegal_chars_conversion || enable_null_string)
				{
					CONVERT_ILLEGAL_CHAR_SKIP(dest, src, len, l, replace_char);
					continue;
				}
				else
			...
			else
			{
				if (copy_illegal_chars_conversion || enable_null_string)
				{
					CONVERT_ILLEGAL_CHAR_SKIP(dest, src, len, 2, replace_char);
					continue;
				}
				else
					report_untranslatable_char(PG_UTF8, PG_LATIN1, (const char *) src, len);
