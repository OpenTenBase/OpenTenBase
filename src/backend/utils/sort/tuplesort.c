...
#endif
int			hybrid_sort_mem = 65535;
bool		enable_hybrid_sort = false;
int			hybrid_sort_slab_size = 32;
#define		HYBRID_SORT_MIN_BATCH_NUMBER 4
#define GET_TUPLE_FROM_MEMTUPLES(state, index)											  \
	((state)->hybrid_sort_work ? &((state)->memtuples[(state)->hybrid_sort_idx_sorted[(index)]]) : \
	&((state)->memtuples[index]))
...
typedef enum
{
	...
	TSS_SORTEDINMEM_PARTIAL		/* Sort partial in memory */
} TupSortStatus;
...
struct Tuplesortstate
{
...
#ifdef __OPENTENBASE_C__
	RemoteFragmentState	*fstate;	/* tuple source, alternate to tapeset */
#endif
	/*
	...
	/*
	 * Split memory to slice which size near to L3 CACHE will be cpu cache friendly.
	 */
	int			*hybrid_sort_idx_sorted; /* array of SortTuple sorted index */
	int			hybrid_sort_idx_sorted_size; /* allocated length of hybrid_sort_idx_sorted array */
	int			*hybrid_sort_idx_partial;
	SortTuple  *hybrid_sort_stash_memtuples;		/* array of SortTuple structs */
	bool		hybrid_sort_work;
	int			hybrid_sort_partial_size;
	Tuplesortstate *hybrid_sort_heap_state; /* use for mix sort */
	bool		use_hybrid_sort; /* a runtime setting */
...
static void tuplesort_sort_memtuples_hybrid_sort(Tuplesortstate *state);
...
#endif
#ifdef __OPENTENBASE_C__
static unsigned int getlen_remote(Tuplesortstate *state, int tapenum,
								  bool eofOK);
static void readtup_remote(Tuplesortstate *state, SortTuple *stup,
						   int tapenum, unsigned int len);
#endif
...
static int tuplesort_sort_get_partial_size(Tuplesortstate *state);
static void copy_tuple_for_hybrid_sort(Tuplesortstate *state, int src_part, int size_for_one_partial);
...
tuplesort_begin_common(int workMem, bool randomAccess)
{
	...
	state->hybrid_sort_work = false;
	state->hybrid_sort_idx_sorted = NULL;
	state->hybrid_sort_idx_sorted_size = 0;
	state->hybrid_sort_idx_partial = NULL;
	state->hybrid_sort_stash_memtuples = NULL;
	state->hybrid_sort_heap_state = NULL;
	state->hybrid_sort_partial_size = -1;
	/*
 ...
 *		tuplesort_copy
 *
 * Initialize for a tuple sort construction refer to another one.
 * copy all field and alloc memory independently.Caller should free it.
 */

static Tuplesortstate *
tuplesort_copy(Tuplesortstate *ref_state)
{
	Tuplesortstate *state;
	MemoryContext sortcontext;
	MemoryContext tuplecontext;
	...
	state->status = TSS_INITIAL;
	state->randomAccess = ref_state->randomAccess;
	state->bounded = ref_state->bounded;
	state->tuples = ref_state->tuples;
	state->boundUsed = false;
	state->allowedMem = ref_state->allowedMem;
	state->availMem = state->allowedMem;
	state->sortcontext = sortcontext;
	state->tuplecontext = tuplecontext;
	state->tapeset = NULL;
	...
	state->comparetup = ref_state->comparetup;
	state->copytup = ref_state->copytup;
	state->writetup = ref_state->writetup;
	state->readtup = ref_state->readtup;
#ifdef PGXC
	state->getlen = getlen;
#endif

	state->tupDesc = ref_state->tupDesc;
	state->abbrevNext = 10;

	state->sortKeys = ref_state->sortKeys;
	state->indexInfo = ref_state->indexInfo;
	state->heapRel = ref_state->heapRel;
	state->indexRel = ref_state->indexRel;
	state->enforceUnique = ref_state->enforceUnique;

	state->high_mask = ref_state->high_mask;
	state->low_mask = ref_state->low_mask;
	state->max_buckets = ref_state->max_buckets;

	state->datumTypeLen = ref_state->datumTypeLen;
	state->tuples = ref_state->tuples;
							...
							int workMem, bool randomAccess)
{
	...
	if (RelationIsCrossNodeIndex(indexRel->rd_rel))
		state->nKeys -= CNI_META_NUM;
	/* Prepare SortSupport data for each column */
	state->sortKeys = (SortSupport) palloc0(state->nKeys *
											sizeof(SortSupportData));
					 ...
					 int workMem)
{
	...
	state->copytup = NULL;
	state->writetup = NULL;
	state->combiner = &fstate->combiner;
	...
	{
		state->readtup = readtup_remote;
		state->getlen = getlen_remote;
	}
	 ...
	 */
	state->maxTapes = num_tapes;
	state->tapeRange = num_tapes;

	state->mergeactive = (bool *) palloc0(num_tapes * sizeof(bool));
	state->tp_runs = (int *) palloc0(num_tapes * sizeof(int));
	state->tp_dummy = (int *) palloc0(num_tapes * sizeof(int));
	state->tp_tapenum = (int *) palloc0(num_tapes * sizeof(int));
...
#endif
	if (state->hybrid_sort_heap_state)
	{
		tuplesort_end(state->hybrid_sort_heap_state);
	}
	...
	switch (state->status)
	{
		case TSS_SORTEDINMEM:
			...
			if (forward)
			{
				...
				{
					memcpy(stup, GET_TUPLE_FROM_MEMTUPLES(state, state->current), sizeof(SortTuple));
					state->current++;
					return true;
				}
			...
			else
			{
				...
				memcpy(stup, GET_TUPLE_FROM_MEMTUPLES(state, state->current - 1), sizeof(SortTuple));
				return true;
			}
			break;
		...
		case TSS_SORTEDINMEM_PARTIAL:
			size_for_one_partial = tuplesort_sort_get_partial_size(state);
			batch_number = state->memtupcount / size_for_one_partial;

			if (state->hybrid_sort_heap_state->lastReturnedTuple)
			{
				RELEASE_SLAB_SLOT(state->hybrid_sort_heap_state, state->hybrid_sort_heap_state->lastReturnedTuple);
				state->hybrid_sort_heap_state->lastReturnedTuple = NULL;
			}

			if (state->memtupcount > state->current)
			{
				SortTuple	newtup;
				*stup = state->hybrid_sort_heap_state->memtuples[0];
				state->hybrid_sort_heap_state->lastReturnedTuple = stup->tuple;
				src_part = state->hybrid_sort_heap_state->memtuples[0].tupindex;
				part_length = Min(size_for_one_partial, state->memtupcount - src_part * size_for_one_partial);
				/* last one */
				if (src_part == (batch_number - 1))
					part_length = state->memtupcount - src_part * size_for_one_partial;


				next_idx = src_part * hybrid_sort_slab_size + 
					state->hybrid_sort_heap_state->hybrid_sort_idx_partial[src_part];
				if (state->hybrid_sort_heap_state->hybrid_sort_idx_partial[src_part] == hybrid_sort_slab_size )
				{
					copy_tuple_for_hybrid_sort(state, src_part, size_for_one_partial);
					next_idx = src_part * hybrid_sort_slab_size + 
						state->hybrid_sort_heap_state->hybrid_sort_idx_partial[src_part];
				}

				/*
				 * pull next tuple from the partial, and the idx point to next
				 */
				if ((state->hybrid_sort_heap_state->hybrid_sort_idx_partial[src_part] + state->hybrid_sort_idx_partial[src_part] - hybrid_sort_slab_size) < part_length )
				{
					newtup = state->hybrid_sort_heap_state->hybrid_sort_stash_memtuples[next_idx];
					newtup.tupindex = src_part;
					tuplesort_heap_replace_top(state->hybrid_sort_heap_state, &newtup);
					state->hybrid_sort_heap_state->hybrid_sort_idx_partial[src_part] += 1;
				}
				else
				{
					tuplesort_heap_delete_top(state->hybrid_sort_heap_state);
				}
...
/*
 * copy data from state to hybrid_sort_heap_state, 
 * for datum in state is very random memory,accessing them will not a CPU cache friendly method.
*/						  
static void 
copy_tuple_for_hybrid_sort(Tuplesortstate *state, int src_part, 
											int size_for_one_partial)
{
	int i = 0;
	int curr_idx;
	...
	curr_idx = src_part * size_for_one_partial + 
		state->hybrid_sort_idx_partial[src_part];

	for (i = 0; i < hybrid_sort_slab_size && 
		(state->hybrid_sort_idx_partial[src_part] + i) < part_length; i++)
	{
		COPYTUP(state->hybrid_sort_heap_state, 
			&state->hybrid_sort_heap_state->hybrid_sort_stash_memtuples[src_part * hybrid_sort_slab_size + i], 
			(void *)&state->memtuples[curr_idx]);
		curr_idx ++;
	}
	state->hybrid_sort_heap_state->hybrid_sort_idx_partial[src_part] = 0;
	state->hybrid_sort_idx_partial[src_part] += hybrid_sort_slab_size;
}
...
dumptuples(Tuplesortstate *state, bool alltuples)
{
	...
	for (i = 0; i < memtupwrite; i++)
	{
		WRITETUP(state, state->tp_tapenum[state->destTape], 
				GET_TUPLE_FROM_MEMTUPLES(state, i));
		state->memtupcount--;
	}
	/* data in disk is in order */
	state->hybrid_sort_work = false;
	state->hybrid_sort_partial_size = -1;
	/*
					...
					TuplesortInstrumentation *stats)
{
	...
	switch (state->status)
	{
		...
		case TSS_FINALMERGE:
			...
			break;
		case TSS_SORTEDINMEM_PARTIAL:
			stats->sortMethod = SORT_TYPE_HYBRID_SORT;
			break;
		default:
...
/*
* get mix sort every partial`s size refer to guc hybrid_sort_mem and datum number
*/
static int
tuplesort_sort_get_partial_size(Tuplesortstate *state)
{
	int batch_number = 0;
	int64 used_mem = 0;
	if (state->hybrid_sort_partial_size != -1)
		return state->hybrid_sort_partial_size;
	used_mem = state->allowedMem - state->availMem;
	batch_number = used_mem / 1024 / hybrid_sort_mem;
	if (batch_number < HYBRID_SORT_MIN_BATCH_NUMBER)
	{
		state->hybrid_sort_partial_size = state->memtupcount;
		return state->hybrid_sort_partial_size;
	}
	state->hybrid_sort_partial_size = state->memtupcount / batch_number;

	return state->hybrid_sort_partial_size;
}

static bool
tuplesort_sort_use_hybrid_sort(Tuplesortstate *state)
{
	int size_for_one_partial;
	if (!enable_hybrid_sort || !state->use_hybrid_sort)
		return false;
	state->hybrid_sort_partial_size = -1;
	size_for_one_partial = tuplesort_sort_get_partial_size(state);
	return size_for_one_partial != state->memtupcount;
}
 ...
 */
static void
tuplesort_sort_memtuples_partial(Tuplesortstate *state)
{
	int i = 0;
	int size_for_one_partial = tuplesort_sort_get_partial_size(state);
	int batch_number = state->memtupcount / size_for_one_partial;
	if (state->memtupcount <= 0)
		return;
#ifdef TRACE_SORT
	if (trace_sort)
		elog(LOG, "begin quicksort partial of run %d: %s",
			 state->currentRun, pg_rusage_show(&state->ru_start));
#endif
	do {
		int qsort_length = Min(size_for_one_partial, state->memtupcount - i * size_for_one_partial);
		int qsort_offset = i * size_for_one_partial;
		/*
		 * if all batch is full, than the last one deal more datum(hybrid_sort_batch_number - 1 datum at most)
		 * else the last one deal less datum, equal to (memtupcount - i * size_for_one_partial) which less than size_for_one_partial
		*/
		/* last one */
		if (i == (batch_number - 1))
			qsort_length = state->memtupcount - i * size_for_one_partial;
		/* Can we use the single-key sort function? */
		if (state->onlyKey != NULL)
			qsort_ssup(state->memtuples + qsort_offset, 
						qsort_length,
						state->onlyKey);
		else
			qsort_tuple(state->memtuples + qsort_offset,
						qsort_length,
						state->comparetup,
						state);
		i ++;
	} while((i * size_for_one_partial) < state->memtupcount && i < batch_number);
#ifdef TRACE_SORT
	if (trace_sort)
		elog(LOG, "end quicksort partial of run %d: %s",
			 state->currentRun, pg_rusage_show(&state->ru_start));
#endif
...
static void
tuplesort_sort_memtuples_begin_merge(Tuplesortstate *state)
{
	int i = 0;
	int size_for_one_partial = tuplesort_sort_get_partial_size(state);
	int batch_number = state->memtupcount / size_for_one_partial;
	Assert(size_for_one_partial <= state->memtupcount);
	if (size_for_one_partial == state->memtupcount)
		return ;
#ifdef TRACE_SORT
	if (trace_sort)
		elog(LOG, "begin quicksort begin merge of run %d: %s",
			 state->currentRun, pg_rusage_show(&state->ru_start));
#endif
	if (state->hybrid_sort_idx_sorted == NULL)
	{
		state->hybrid_sort_idx_sorted = (int *) palloc0(state->memtupcount * sizeof(int));
		USEMEM(state, GetMemoryChunkSpace(state->hybrid_sort_idx_sorted));
		state->hybrid_sort_idx_sorted_size = state->memtupcount;
	}
	if (state->hybrid_sort_idx_sorted != NULL && state->hybrid_sort_idx_sorted_size < state->memtupcount)
	{
		FREEMEM(state, GetMemoryChunkSpace(state->hybrid_sort_idx_sorted));
		state->memtupsize = state->memtupcount;
		state->hybrid_sort_idx_sorted = (int *)
			repalloc(state->hybrid_sort_idx_sorted,
						  state->memtupcount * sizeof(int));
		USEMEM(state, GetMemoryChunkSpace(state->memtuples));
	}
	if (state->hybrid_sort_idx_partial == NULL)
	{
		state->hybrid_sort_idx_partial = (int *)palloc0(batch_number * sizeof(int));
	}
	else
	{
		memset(state->hybrid_sort_idx_partial, 0, batch_number * sizeof(int));
	}
	state->hybrid_sort_work = true;
	if (state->hybrid_sort_heap_state == NULL)
	{
		state->hybrid_sort_heap_state = tuplesort_copy(state);
	}
	if (state->status == TSS_INITIAL && state->randomAccess == false)
	{
		init_slab_allocator(state->hybrid_sort_heap_state, batch_number * hybrid_sort_slab_size);
		state->hybrid_sort_heap_state->hybrid_sort_stash_memtuples = (SortTuple *) palloc0(batch_number * hybrid_sort_slab_size * sizeof(SortTuple));
		USEMEM(state, GetMemoryChunkSpace(state->hybrid_sort_heap_state->hybrid_sort_stash_memtuples));
		state->hybrid_sort_heap_state->hybrid_sort_idx_partial = (int *)palloc0(batch_number * sizeof(int));
		do {
			SortTuple	stup;
			/* insert first tuple of every partial */
			copy_tuple_for_hybrid_sort(state, i, size_for_one_partial);
			stup = state->hybrid_sort_heap_state->hybrid_sort_stash_memtuples[i * hybrid_sort_slab_size];
			stup.tupindex = i;
			if (state->hybrid_sort_heap_state->memtupcount >= state->hybrid_sort_heap_state->memtupsize - 1)
			{
				(void) grow_memtuples(state->hybrid_sort_heap_state);
				Assert(state->hybrid_sort_heap_state->memtupcount < state->hybrid_sort_heap_state->memtupsize);
			}
			tuplesort_heap_insert(state->hybrid_sort_heap_state, &stup);
			state->hybrid_sort_heap_state->hybrid_sort_idx_partial[i] = 1;
			i ++;
		} while((i * size_for_one_partial) < state->memtupcount && i < batch_number);
	}
	else {
		do {
			/* insert first tuple of every partial */
			SortTuple		stup = state->memtuples[i * size_for_one_partial];
			stup.tupindex = i;
			if (state->hybrid_sort_heap_state->memtupcount >= state->hybrid_sort_heap_state->memtupsize - 1)
			{
				(void) grow_memtuples(state->hybrid_sort_heap_state);
			...
			tuplesort_heap_insert(state->hybrid_sort_heap_state, &stup);
			state->hybrid_sort_idx_partial[i] = 0;
			i ++;
		} while((i * size_for_one_partial) < state->memtupcount && i < batch_number);
	}
...
#ifdef TRACE_SORT
	if (trace_sort)
		elog(LOG, "end quicksort begin merge of run %d: %s",
			 state->currentRun, pg_rusage_show(&state->ru_start));
#endif
}

static void
tuplesort_sort_memtuples_merge(Tuplesortstate *state)
{
	int size_for_one_partial;
	int i = 0;
	int src_part;
...
#ifdef TRACE_SORT
	if (trace_sort)
		elog(LOG, "begin quicksort merge of run %d: %s",
			 state->currentRun, pg_rusage_show(&state->ru_start));
#endif
	batch_number = state->memtupcount / size_for_one_partial;

	while (state->memtupcount > i)
	{
		SortTuple	stup;
		int curr_idx;
		...
		curr_idx = state->hybrid_sort_idx_sorted[i] = src_part * size_for_one_partial + 
			state->hybrid_sort_idx_partial[src_part];
		state->hybrid_sort_idx_partial[src_part] += 1;

		/*
		 * pull next tuple from the tape, and replace the written-out tuple in
		 * the heap with it.
		 */
		if (state->hybrid_sort_idx_partial[src_part] < part_length)
		{
			
			stup = state->memtuples[curr_idx + 1];
			stup.tupindex = src_part;
			tuplesort_heap_replace_top(state->hybrid_sort_heap_state, &stup);
		}
		else
			tuplesort_heap_delete_top(state->hybrid_sort_heap_state);
		i ++;
...
#ifdef TRACE_SORT
			if (trace_sort)
				elog(LOG, "end quicksort merge of run %d: %s",
					 state->currentRun, pg_rusage_show(&state->ru_start));
#endif
}

/*
 * Sort all memtuples using specialized qsort() routines and merge sort.
 *
 * Quicksort is used for small in-memory sorts, and merge sort runs.
 */
static void
tuplesort_sort_memtuples_hybrid_sort(Tuplesortstate *state)
{
	if (state->memtupcount <= 1)
	{
		return;
	}
	tuplesort_sort_memtuples_partial(state);
	tuplesort_sort_memtuples_begin_merge(state);
	if (state->status == TSS_INITIAL && state->randomAccess == false)
		state->status = TSS_SORTEDINMEM_PARTIAL;
	else
		tuplesort_sort_memtuples_merge(state);
}
...
tuplesort_sort_memtuples(Tuplesortstate *state)
{
	if (state->memtupcount > 1)
	{
		if (tuplesort_sort_use_hybrid_sort(state))
			return tuplesort_sort_memtuples_hybrid_sort(state);
		/* Can we use the single-key sort function? */
		if (state->onlyKey != NULL)
...
static void
copytup_heap_with_slab(Tuplesortstate *state, SortTuple *stup, void *tup)
{
	SortTuple *srctup = (SortTuple *)tup;
	MinimalTuple tuple_src = (MinimalTuple)(srctup->tuple);
	MinimalTuple tuple = (MinimalTuple) readtup_alloc(state, tuple_src->t_len);
	HeapTupleData htup;
	Datum		original;
	MemoryContext oldcontext;
	memcpy(stup, srctup, sizeof(SortTuple));
	memcpy(tuple, tuple_src, tuple_src->t_len);
	...
	oldcontext = MemoryContextSwitchTo(state->tuplecontext);
...
copytup_heap(Tuplesortstate *state, SortTuple *stup, void *tup)
{
	...
	HeapTupleData htup;
	MemoryContext oldcontext;
	if (state->hybrid_sort_stash_memtuples != NULL)
		return copytup_heap_with_slab(state, stup, tup);
...
#ifdef __OPENTENBASE_C__
static unsigned int
getlen_remote(Tuplesortstate *state, int tapenum, bool eofOK)
{
	RecvBuffer      *recvBuffer = state->fstate->recvBuffer;
	TupleTableSlot  *slot = state->fstate->combiner.ss.ps.ps_ResultTupleSlot;
	void			*msg;
	...
	if (IS_PGXC_LOCAL_COORDINATOR)
		RunRemoteControllerWithFN(recvBuffer->queue, tapenum,
								  recvBuffer->controller);

	msg = TupleQueueReceiveMessage(recvBuffer->queue, tapenum, NULL);
...
static void
readtup_remote(Tuplesortstate *state, SortTuple *stup,
			   int tapenum, unsigned int len /* useless */)
{
	TupleTableSlot *slot = state->fstate->combiner.ss.ps.ps_ResultTupleSlot;
	MinimalTuple tuple;
	HeapTupleData htup;
