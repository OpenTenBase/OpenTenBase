...
extern __thread volatile bool resource_queue_no_backoff;

#ifdef __OPENTENBASE__
/* for error code contrib */
__thread ErrcodeHookType g_pfErrcodeHook = NULL;
#endif
...
static __thread ErrorHintData error_hint_data;
...
/*
 * To avoid to error report the out of memory in function
 * enlargeStringInfo, we must control the query in the log, but
 * user want to get the total query. So we limit the query size
 * to 500MB which we think it is safe.
 *
 * Notice: Now we have many guc variable, so we choose to make
 * a macro definition here.
 */
#define LOG_QUERY_STRING_LIMIT (524288000)
...
#ifdef __OPENTENBASE__
/* Macros for stacktrace support functions */
...
#define errordata                       (GetTDXThreadInfo->thr_error_data)
#define recursion_depth                 (GetTDXThreadInfo->thr_error_recursion_depth)
#define errordata_stack_depth           (GetTDXThreadInfo->thr_error_stack_depth)
#endif

#ifdef _PG_ORCL_
static __thread List *errctxtstack = NIL;
#endif
...
/* opentenbase_ora: The current error data */
static __thread ErrorData *curr_error_data = NULL;
...
static void append_stacktrace(StringInfo buf, void *const *stackarray, int stacksize);
...
static void append_with_tabs_limit(StringInfo buf, const char *str, int limit);
 ...
 */
void
push_err_ctxt(void *ctxt)
{
    errctxtstack = lcons(ctxt, errctxtstack);
}
 ...
 */
void
trunc_errctxtstack(void)
{
    list_free_deep(errctxtstack);
    errctxtstack = NIL;
}
		 ...
		 const char *funcname, const char *domain)
{
	...
	edata->spi_connected = SPI_connect_level();
	edata->elevel = elevel;
	...
	if (elevel >= ERROR && error_stacktrace_level == STACKTRACE_CORE)
	{
		/* Watch out! I'm dying!! */
		abort();
	}
	...
	/* Get stacktrace, maximum to 30 frames is enough. */
	if (elevel >= ERROR && error_stacktrace_level == STACKTRACE_LOG)
	{
		HOLD_INTERRUPTS();
...
errfinish(int dummy,...)
{
	 ...
	 */
#ifdef __OPENTENBASE_C__
	resource_queue_no_backoff = true;
#endif
	CHECK_FOR_INTERRUPTS();
#ifdef __OPENTENBASE_C__
	resource_queue_no_backoff = false;
#endif
}
...
#ifdef _PG_ORCL_
/*
 * errname --- setup current error short name.
 */
int
errname(char *name)
{
	...
	StrNCpy(NameStr(edata->errname), name, NAMEDATALEN);
...
errnode(PGXCNodeHandle *conn)
{
	...
	edata->nodename = pstrdup(conn->nodename);
	edata->nodepid = conn->backend_pid;
...
int
errnodepid(const char *nodename, int pid)
{
	ErrorData  *edata = &errordata[errordata_stack_depth];
	MemoryContext oldcontext;
	...
	edata->nodename = pstrdup(nodename);
	edata->nodepid = pid;
...
EmitErrorReport(void)
{
	...
	if (edata->output_to_client)
	{
		...
		{
			int old_elevel = edata->elevel;

			/*
			 * When 2PC start to commit prepared, send WARNING
			 * instead of ERROR to client
			 */
...
CopyErrorData(void)
{
	...
	if (newedata->nodename)
		newedata->nodename = pstrdup(newedata->nodename);
	if (newedata->detail)
...
ThrowErrorData(ErrorData *edata)
{
	...
	if (edata->nodename)
	{
		newedata->nodename = pstrdup(edata->nodename);
		newedata->nodepid = edata->nodepid;
	}
	if (edata->detail)
...
#ifdef __OPENTENBASE_C__
							remoteConnType == REMOTE_CONN_FORWARDNODE ? 'F' :
#endif
...
/*
 * Just like appendCSVLiteral, but limit the data size.
 */
static inline void
appendCSVLiteralLimitSize(StringInfo buf, const char *data, Size limit)
{
	const char *p = data;
	char		c;
	...
	/* Break while the size of the data larger than the limit size. */
	while (((c = *p++) != '\0') && (p - data < limit))
	{
		if (c == '"')
			appendStringInfoCharMacro(buf, '"');
		appendStringInfoCharMacro(buf, c);
...
write_csvlog(ErrorData *edata)
{
	...
	/* global trace id */
	appendCSVLiteral(&buf, pgxc_get_global_trace_id());
	appendStringInfoChar(&buf, ',');
	...
	/* global session id */
	appendStringInfo(&buf, "\"coord(%d,%u)\"", MyCoordId, MyCoordPid);
	appendStringInfoChar(&buf, ',');
	...
	if (is_log_level_output(edata->elevel,
		...
		print_stmt = true;
	if (print_stmt)
		appendCSVLiteralLimitSize(&buf, debug_query_string, LOG_QUERY_STRING_LIMIT);
	...
	/* print stacktrace */
	if (error_stacktrace_level == STACKTRACE_LOG &&
		edata->elevel >= ERROR &&
		edata->stacktracesize > 0)
	{
		HOLD_INTERRUPTS();
		...
		/* add line number to stacktrace */
		append_stacktrace(&buf, edata->stacktracearray, edata->stacktracesize);
		RESUME_INTERRUPTS();
	}
 ...
 */
static void
append_stacktrace(StringInfo buf, void *const *stackarray, int stacksize)
{
	...
	appendStringInfoString(buf, "Use addr2line to get pretty function name and line, "
							"e.g., addr2line -e path_to_postgres symbol_address -f\n");
#endif
}

/*
 * To trace and identify certain unexpected issues, print the current stack and relevant location
 * information.
 */
void
print_stacktrace(void)
{
	...
	appendStringInfoString(&buf, "Stack trace:\n");
	/* add line number to stacktrace */
	append_stacktrace(&buf, stacktracearray, stacktracesize);
...
send_message_to_server_log(ErrorData *edata)
{
	...
	if (edata->nodename)
		appendStringInfo(&buf, _(" on %s pid %d"),
						 edata->nodename, edata->nodepid);
		...
		!edata->hide_stmt)
	{
		...
		append_with_tabs_limit(&buf, debug_query_string, LOG_QUERY_STRING_LIMIT);
		appendStringInfoChar(&buf, '\n');
	}
#ifdef __OPENTENBASE__
    /* for error code contrib */
    if (g_pfErrcodeHook && (false == g_is_in_init_phase) && (NULL != MyProc) &&
        (!(IsForwardSenderProcess() || IsForwardRecverProcess())))
    {
...
send_message_to_frontend(ErrorData *edata)
{
	StringInfoData msgbuf;
	 ...
	 /* Do not send message to frontend from sub thread */
	if (am_sub_thread)
		return;

    if (!IsConnFromApp() && (edata->elevel >= ERROR))
	{
		pq_putmessage('9', (const char *)&internal_cmd_sequence, 1);
		if (enable_distri_print)
			elog(DEBUG5, "pid %d send internal_cmd_sequence %u", MyProcPid, internal_cmd_sequence);
	}
...
#ifdef _PG_ORCL_
		if (ORA_MODE && IS_ORA_SQLSTATE(edata->sqlerrcode))
		{
			tbuf[0] = '-';
			ti = 1;
		}
			...
			if (debug_message_log)
			{
				...
				if (edata->nodename)
					appendStringInfo(&buf, "(%s:%d) %s",
									 edata->nodename, edata->nodepid, edata->message);
				else
					appendStringInfo(&buf, "(%s:%d) %s",
									 PGXCNodeName, MyProcPid, edata->message);
...
/*
 * Just like append_with_tabs, but limit the string size.
 */
static void
append_with_tabs_limit(StringInfo buf, const char *str, int limit)
{
	char		ch;
	const char 		*start = str;

	while (((ch = *str++) != '\0') && (str - start < limit))
	{
		appendStringInfoCharMacro(buf, ch);
		if (ch == '\n')
			appendStringInfoCharMacro(buf, '\t');
...
pg_msgmodule_change(PG_FUNCTION_ARGS)
{
	...
	if ((change < (DEBUG5 - LOG)) || (change > (LOG - DEBUG5)))
		ereport(ERROR,
				(errcode(ERRCODE_CHARACTER_NOT_IN_REPERTOIRE),
				 (errmsg("accepted values are between %d and +%d",
						 DEBUG5 - LOG, LOG - DEBUG5))));
...
/*
 * opentenbase_ora: Get the current error data.
 */
ErrorData *
get_curr_error_data(void)
{
	return curr_error_data;
}

/*
 * opentenbase_ora: Set the current error data.
 */
void
set_curr_error_data(ErrorData *curr)
{
	curr_error_data = curr;
}

void
elog_hint(int hintType, const char *hintmsg)
{

	ErrorHintData *errorHintData;

	if (hintmsg == NULL)
		return;

	errorHintData = &error_hint_data;

	switch (hintType)
	{
		case HINT_COLUMN:
			strncpy(errorHintData->column_name, hintmsg, NAMEDATALEN);
			break;
		default:
			break;
	}
...
/*
 * append some message to the end of hint
 */
void
append_hint_message(ErrorData *edata)
{
	StringInfoData	buf;
	ErrorHintData *errorHintData;
	errorHintData = &error_hint_data;
	...
	/* skip the null column name or default column name */
	if (strlen(errorHintData->column_name) != 0 &&
					pg_strcasecmp(errorHintData->column_name, "?column?") != 0)
	{
		appendStringInfoString(&buf,
							   psprintf("%s while processing column %s",
										edata->hint ? "\n": "",
										quote_identifier(errorHintData->column_name)));
	}


	if (buf.len > 0)
	{
		edata->hint = pstrdup(buf.data);
	}
	pfree(buf.data);

	/* all steps end, need to clean this data */
	elog_hint_reset();
}

void elog_hint_reset(void)
{
