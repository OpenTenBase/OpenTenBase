...
extern int send_snapshot_times;
extern char send_latest_snapshot_dn[NAMEDATALEN];
...
SnapshotData UseLatestGtsSnapshotData = {HeapTupleSatisfiesMVCC};
...
GetNonHistoricCatalogSnapshot(Oid relid)
{
	...
	if (CatalogSnapshot == NULL)
	{
        ...
        CatalogSnapshot->is_catalog = true;
		/*
					   ...
					   int sourcepid, PGPROC *sourceproc)
{
	 ...
	 * 
	 * Use latest true to avoid get gts from gtm when GetSnapshotData.
	 */
    ...
    if ((sourcesnap->xcnt || sourcesnap->subxcnt) && CurrentSnapshot->xip == NULL)
    {
        /*
         * First call for this snapshot. Snapshot is same size whether or not
         * we are in recovery, see later comments.
         */
        CurrentSnapshot->xip = (TransactionId *)
                malloc(GetMaxSnapshotXidCount() * sizeof(TransactionId));
        if (CurrentSnapshot->xip == NULL)
            ereport(ERROR,
                    (errcode(ERRCODE_OUT_OF_MEMORY),
                            errmsg("out of memory")));
        Assert(CurrentSnapshot->subxip == NULL);
        CurrentSnapshot->subxip = (TransactionId *)
                malloc(GetMaxSnapshotSubxidCount() * sizeof(TransactionId));
        if (CurrentSnapshot->subxip == NULL)
            ereport(ERROR,
                    (errcode(ERRCODE_OUT_OF_MEMORY),
                            errmsg("out of memory")));
    }
    ...
    CurrentSnapshot->is_catalog = sourcesnap->is_catalog;
#endif
	...
	if (sourceproc != NULL)
	{
		if (CurrentSnapshot->local && !ProcArrayInstallRestoredXmin(CurrentSnapshot->xmin, sourceproc))
			ereport(ERROR,
...
CopySnapshot(Snapshot snapshot)
{
	...
	if(IS_PGXC_DATANODE && newsnap->groupsize > 0)
	{
		...
		SnapshotGetShardClusterTable(newsnap) = (Bitmapset *)newsnap->sc_filler;
		memcpy(SnapshotGetShardClusterTable(newsnap), SnapshotGetShardClusterTable(snapshot), SHARD_CLUSTER_BITMAP_SIZE);
	}
	else
	{
		newsnap->groupsize = 0;
		SnapshotGetShardTable(newsnap) = NULL;
		SnapshotGetShardClusterTable(newsnap) = NULL;
	}
#endif
...
GetActiveSnapshot(void)
{
	 ...
	 * if it is, a new one will be generated.
	 */
	if (!ActiveSnapshot && (IS_PGXC_LOCAL_COORDINATOR || IS_CENTRALIZED_MODE))
		return NULL;
#endif
	Assert(ActiveSnapshot != NULL);
...
ExportSnapshot(Snapshot snapshot)
{
    ...
    appendStringInfo(&buf, "is_catalog:%d\n", snapshot->is_catalog);
#endif
...
ImportSnapshot(const char *idstr)
{
    ...
    snapshot.is_catalog = parseIntFromText("is_catalog:", &filebuf, path);
#endif
...
/*
 * Push HistoricSnapshot to active snapshot.
 * Just used for logical decode.
 */
void
PushHistoricSnapToActiveSnap()
{
	PushActiveSnapshot(HistoricSnapshot);
}
...
SerializeSnapshot(Snapshot snapshot, char *start_address)
{
    ...
    serialized_snapshot.is_catalog	= snapshot->is_catalog;
#endif
...
RestoreSnapshot(char *start_address)
{
    ...
    snapshot->is_catalog = serialized_snapshot.is_catalog;
	if(enable_distri_print)
	{
...
/*
 * Get a snapshot to nodify dn to use the local latest gts.
 */
Snapshot
GetUseLatestGtsSnapshot(void)
{
    if (unlikely(ReadWithLocalTs))
        UseLatestGtsSnapshotData.start_ts = GetSharedLatestCommitTS() + 1;
	else if (!CSN_IS_USE_LATEST(UseLatestGtsSnapshotData.start_ts))
		UseLatestGtsSnapshotData.start_ts = CSN_USE_LATEST;

	send_snapshot_times = 0;
	send_latest_snapshot_dn[0] = '\0';

	UseLatestGtsSnapshotData.curcid = GetCurrentCommandId(false);

	elog(DEBUG1, "GetUseLatestGtsSnapshot, cid: %u, ts:%lu",
		UseLatestGtsSnapshotData.curcid, UseLatestGtsSnapshotData.start_ts);

	return &UseLatestGtsSnapshotData;
}
