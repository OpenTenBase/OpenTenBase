...
/* max size of char *fields in audit log*/
int AuditLog_log_query_str_size = 32 * 1024;
int AuditLog_log_appname_size = 100;
int AuditLog_log_objectname_size = 100;
...
typedef struct AuditResultInfo
{
	...
	char			*node_osuser;						/* current os user name (<64B)*/
...
static AuditStmtMap gAuditStmtActionMap[] =
{
	...
	},
	{
		...
		{Audit_InvalidEnum},
		{
			...
			OBJECT_SEQUENCE,		/* ACL_OBJECT_SEQUENCE */
			OBJECT_FUNCTION,		/* ACL_OBJECT_FUNCTION */
			OBJECT_PACKAGE,		    /* ACL_OBJECT_PACKAGE */
			Audit_InvalidEnum
		},
		{
			T_GrantStmt,
...
static void
audit_build_idx_for_stmt_action_map(void)
{
	Size i = 0;
...
char* 
audit_get_type_string(AuditType type)
{
...
char* 
audit_get_mode_string(AuditMode mode)
{
...
Size
audit_get_stmt_map_size(void)
{
	return (sizeof(gAuditStmtActionMap) / sizeof(gAuditStmtActionMap[0]));
}

AuditStmtMap*
audit_get_stmt_map_by_action(AuditSQL action_id)
{
	Size i = 0;
	int32 idx = -1;
...
AuditStmtMap*
audit_get_stmt_map_by_index(int32 idx)
{
	if (idx >= 0 && idx < (int32)audit_get_stmt_map_size())
	{
 ...
 */
static bool
isa_shortcut_audit_stmt(AuditStmtMap * item)
{
...
/* Audit xxx */
static bool
isa_statement_action_id(AuditSQL action_id)
{
	if (action_id > AuditSql_ShortcutBegin &&
		action_id < AuditSql_ShortcutEnd)
	{
...
/* Audit xxx BY xxx */
static bool
isa_user_action_id(AuditSQL action_id)
{
	if (action_id > AuditSql_ShortcutBegin &&
		action_id < AuditSql_ShortcutEnd)
	{
...
/* Audit xxx ON xxx */
static bool
isa_object_action_id(AuditSQL action_id)
{
...
static bool
isa_statement_audit_type(AuditType type)
{
...
static bool
isa_user_audit_type(AuditType type)
{
...
static bool
isa_object_audit_type(AuditType type)
{
...
static void
audit_get_cacheid_pg_audit_o(int32 * rel_cacheid, 
									  	 int32 * oid_cacheid)
{
	if (rel_cacheid != NULL)
	{
...
static void
audit_get_cacheid_pg_audit_d(int32 * rel_cacheid, 
									  	 int32 * oid_cacheid)
{
	if (rel_cacheid != NULL)
	{
...
static void
audit_get_cacheid_pg_audit_u(int32 * rel_cacheid, 
									  	 int32 * oid_cacheid)
{
	if (rel_cacheid != NULL)
	{
...
static void
audit_get_cacheid_pg_audit_s(int32 * rel_cacheid, 
									  	 int32 * oid_cacheid)
{
	if (rel_cacheid != NULL)
	{
 ...
 */
static bool
audit_check_action_type(AuditType audit_type, 
							  		AuditSQL action_id)
{
	switch (audit_type)
	{
 ...
 */
static void
audit_expand_AuditSql_All(AuditStmt * stmt,
									  AuditStmtContext * ctxt)
{
	Size i = 0;
	AuditSQL action_id = AuditSQL_Ivalid;
...
static bool
audit_check_relkind(Relation relation,
						        List * object_name,
								ObjectType object_type)
{
...
static void
audit_process_object_internal(List * object_name, 
								 		  ObjectType object_type,
								 		  ObjectAddress * object_address)
...
static void
audit_process_object(AuditStmt * stmt, 
								 AuditStmtContext * ctxt)
...
static void
audit_clean_process_object(CleanAuditStmt * stmt, 
								 	   CleanAuditStmtContext * ctxt)
{
	audit_process_object_internal(stmt->object_name,
								  stmt->object_type,
...
static void
audit_free_stmt_context(AuditStmtContext * ctxt)
{
	if (ctxt == NULL)
	{
		return;
...
static void
audit_clean_free_stmt_context(CleanAuditStmtContext * ctxt)
{
	if (ctxt == NULL)
	{
		return;
...
static AuditModeMi
audit_mode_minus(AuditMode first, AuditMode second)
{
	if (first == second)
	{
...
static void
audit_append_unique_action_item(AuditStmtContext * ctxt,
											AuditStmtMap * action_item)
{
...
static void
audit_process_action_list(AuditStmt * stmt,
									  AuditStmtContext * ctxt)
{
	ListCell *l = NULL;
...
static void
audit_process_user_list_internal(List * user_list,
											 List ** l_user_oids)
{
	List * luid = NIL;
	ListCell *l = NULL;
...
static void
audit_process_user_list(AuditStmt * stmt,
									AuditStmtContext * ctxt)
{
...
static void
audit_clean_process_user_list(CleanAuditStmt * stmt,
										  CleanAuditStmtContext * ctxt)
...
static char*
audit_object_type_string(ObjectType object_type, bool report_error)
{
	switch (object_type)
	{
		case OBJECT_INDEX:
...
static void
audit_final_to_string(AuditStmt * stmt, 
								  AuditStmtContext * ctxt)
{
	switch (stmt->audit_type)
	{
...
static void
audit_generate_one_record(AuditStmt * stmt,
									  AuditStmtContext * ctxt,
									  AuditStmtMap * item,
									  Oid user_id,
									  AuditStmtRecord ** record)
...
static void
audit_generate_record_list(AuditStmt * stmt,
									   AuditStmtContext * ctxt,
									   List ** lrecord)
{
	ListCell * l = NULL;
...
static void
audit_write_sysrel_stmt_conf(AuditStmt * stmt,
										 AuditStmtContext * ctxt,
										 AuditRecordWriter * writer,
										 AuditStmtRecord * record,
										 Relation pg_sysrel)
...
static void
audit_write_sysrel_user_conf(AuditStmt * stmt,
										 AuditStmtContext * ctxt,
										 AuditRecordWriter * writer,
										 AuditStmtRecord * record,
										 Relation pg_sysrel)
...
static void
audit_write_sysrel_obj_conf(AuditStmt * stmt,
										AuditStmtContext * ctxt,
										AuditRecordWriter * writer,
										AuditStmtRecord * record,
										Relation pg_sysrel)
...
static void
audit_write_sysrel_obj_def_opts(AuditStmt * stmt,
										 	AuditStmtContext * ctxt,
										 	AuditRecordWriter * writer,
										 	AuditStmtRecord * record,
										 	Relation pg_sysrel)
...
static void
audit_get_record_writer(AuditStmt * stmt,
									AuditRecordWriter * writer)
{
...
static void
audit_write_recod_list(AuditStmt * stmt,
								   AuditStmtContext * ctxt,
								   List * lrecord)
{
...
static void
audit_remove_tuple_by_oid(int32 cacheid, Oid audit_id)
{
...
static void
audit_clean_for_statement(CleanAuditStmt * stmt, 
								  	  CleanAuditStmtContext * ctxt)
{
...
static void
audit_clean_for_user_by_oids(List * l_user_oids)
{
...
static void
audit_clean_for_user(CleanAuditStmt * stmt, 
						         CleanAuditStmtContext * ctxt)
{
	if (ctxt->l_user_oids != NIL)
	{
...
static void
audit_clean_for_object(CleanAuditStmt * stmt, 
							   	   CleanAuditStmtContext * ctxt)
{
	int32 sys_cacheid = InvalidSysCacheID;
	Oid sys_reloid = InvalidOid;
...
static void
audit_clean_for_object_default(CleanAuditStmt * stmt, 
								  	   	   CleanAuditStmtContext * ctxt)
{
	Assert(stmt->clean_type == CleanAuditType_All ||
...
static void
audit_clean_for_unknown(CleanAuditStmt * stmt, 
									CleanAuditStmtContext * ctxt)
{
...
static void
audit_clean_process_delete(CleanAuditStmt * stmt, 
								  	   CleanAuditStmtContext * ctxt)
{
...
void
RemoveStmtAuditById(Oid audit_id)
{
	int32 sys_cacheid = InvalidSysCacheID;
...
void
RemoveUserAuditById(Oid audit_id)
{
	int32 sys_cacheid = InvalidSysCacheID;
...
void
RemoveObjectAuditById(Oid audit_id)
{
	int32 sys_cacheid = InvalidSysCacheID;
...
void
RemoveObjectDefaultAuditById(Oid audit_id)
{
	int32 sys_cacheid = InvalidSysCacheID;
...
/* "statement audit %s" */
char*
get_stmt_audit_desc(Oid audit_id, bool just_identity)
{
	int32 sys_cacheid = InvalidSysCacheID;
	HeapTuple tup = NULL;
...
/* "user audit %s by %s" */
char*
get_user_audit_desc(Oid audit_id, bool just_identity)
{
	int32 sys_cacheid = InvalidSysCacheID;
	HeapTuple tup = NULL;
	StringInfoData str_desc;
...
/* "object audit %s on %s" */
char*
get_obj_audit_desc(Oid audit_id, bool just_identity)
{
	int32 sys_cacheid = InvalidSysCacheID;
	HeapTuple tup = NULL;
	StringInfoData str_desc;
...
char*
get_obj_default_audit_desc(Oid audit_id, bool just_identity)
{
	int32 sys_cacheid = InvalidSysCacheID;
	HeapTuple tup = NULL;
	StringInfoData str_desc;
...
void
AuditDefine(AuditStmt * stmt, const char * queryString, char ** auditString)
{
...
void
AuditClean(CleanAuditStmt * stmt, const char * queryString, char ** cleanString)
{
...
void
AuditReadQueryList(const char * query_sring, List * l_parsetree)
{
	/*
...
void
AuditProcessResultInfo(bool is_success)
{
	if (enable_audit && 
		is_audit_environment())
	{
...
void
AuditCheckPerms(Oid table_oid, Oid roleid, AclMode mask)
{
...
static AuditResultInfo*
audit_hit_init_result_info(void)
{
	...
	{
		if (strlen(application_name) > AuditLog_log_appname_size)
			audit_ret->app_name = pnstrdup(application_name, AuditLog_log_appname_size);
		else
			audit_ret->app_name = pstrdup(application_name);
	}
	else
	{
		audit_ret->app_name = pstrdup("unknown");
	}
...
static void
audit_hit_free_result_info(AuditResultInfo * audit_ret)
{
	if (audit_ret == NULL)
	{
		return;
...
static AuditResultInfo*
audit_hit_get_result_info(void)
{
	return gAuditResultInfo;
}

static void
audit_hit_set_result_info(AuditResultInfo * audit_ret)
{
	gAuditResultInfo = audit_ret;
}

static void
audit_hit_find_action_index(AuditHitInfo * hit_info)
{
	int32 i = 0;
...
static void
audit_hit_free_hit_info(AuditHitInfo * hit_info)
{
	if (hit_info == NULL)
	{
		return;
...
static AuditHitInfo*
audit_hit_make_hit_info(NodeTag stmt_tag,
											  Oid class_id,
											  Oid object_id,
											  int32 object_sub_id,
											  ObjectType obj_type,
											  char * obj_name)
{
	...
	if (obj_name && strlen(obj_name) > AuditLog_log_objectname_size)
		hit_info->obj_name = pnstrdup(obj_name, AuditLog_log_objectname_size);
	else
...
static AuditHitInfo*
audit_hit_make_hit_info_2(AuditSQL id,
												NodeTag stmt_tag,
												Oid class_id,
												Oid object_id,
												int32 object_sub_id,
...
static bool
audit_hit_find_seq_walker(Node * node, void * context)
{
...
#ifdef _PG_ORCL_
		/* No audit for functions in WITH FUNCTION */
#endif
...
static List*
audit_hit_find_object_info(Query * audit_query, 
										 NodeTag stmt_tag)
{
	ListCell * l = NULL;
	...
	foreach(l, audit_query->rtable)
	{
		...
		switch (rtekind)
		{
			case RTE_RELATION:
			{
				...
				if (rel_kind == RELKIND_RELATION || rel_kind == RELKIND_PARTITIONED_TABLE)
				{
...
static bool
audit_hit_object_missing_ok(bool src_missing_ok)
{
...
static List*
audit_hit_read_query_utility(Query * audit_query)
{
	...
	switch (nodeTag(audit_stmt))
	{
		...
		case T_AlterFunctionStmt:
		{
			...
			funcOid = LookupFuncWithArgs(OBJECT_FUNCTION, stmt->func, audit_hit_object_missing_ok(false));
		...
		case T_AlterOwnerStmt:
		{
			 ...
			 * ALTER opt_public SYNONYM name OWNER TO RoleSpec
			 */
		...
		case T_CommentStmt:
		{
			...
			if (stmt->objtype == OBJECT_COLUMN)
			{
				...
				if (relation == NULL && list_length(list_name) >= 2)
				{
					...
					RangeVar *r_var = makeRangeVarFromNameList(relname);
					relation = relation_openrv_extended(r_var,
														AccessShareLock, 
														audit_hit_object_missing_ok(false));
		...
		case T_CreateDirStmt:
		{
			 ...
			 */
			AuditHitInfo * hit_info = NULL;
			CreateDirStmt * stmt = (CreateDirStmt *) audit_stmt;
			char * object_name = stmt->dirname;

			/* Audit for CREATE DIRECTORY */
			hit_info = audit_hit_make_hit_info(hit_ctxt.stmt_tag,
											   DirectoryId,
											   InvalidOid,
											   InvalidOid,
		...
		case T_DropDirStmt:
		{
			/* Drop directory */
			/*
			 * DROP DIRECTORY directory_name AS directory_path
			 */
			AuditHitInfo * hit_info = NULL;
			DropDirStmt * stmt = (DropDirStmt *) audit_stmt;
			char * object_name = stmt->dirname;
		...
		{
			CreateSynonymStmt	*stmt = (CreateSynonymStmt *) audit_stmt;
			StringInfoData	string;
			AuditHitInfo	*hit_info;

			initStringInfo(&string);
			if (stmt->ispublic)
				appendStringInfoString(&string, "public.");
			else if (stmt->synname->schemaname != NULL)
				appendStringInfo(&string, "%s.", stmt->synname->schemaname);

			appendStringInfoString(&string, stmt->synname->relname);
			hit_info = audit_hit_make_hit_info(hit_ctxt.stmt_tag,
											   SynonymRelationId,
											   InvalidOid,
											   InvalidOid,
					...
					switch (objtype)
					{
						...
						case ACL_OBJECT_FUNCTION:
						{
							foreach(cell, objnames)
							{
								...
								Oid				  funcid = LookupFuncWithArgs(OBJECT_FUNCTION, func, audit_hit_object_missing_ok(false));
...
static List*
audit_hit_read_query_tree(Query * audit_query)
{
	List * l_hit_info = NIL;
	...
	switch (audit_query->commandType)
	{
        ...
        case CMD_MERGE:
        {
            l_hit_info = audit_hit_find_object_info(audit_query, T_MergeStmt);
            break;
        }
		case CMD_UTILITY:
		{
...
static void
audit_hit_remove_dup_object(AuditResultInfo * audit_ret)
{
	List * l_src = NIL;
	List * l_des = NIL;
...
static void
audit_hit_read_query_list(Port * port,
									  const char * query_sring,
									  List * l_parsetree)
{
	if (audit_hit_get_result_info() == NULL)
	{
		...
		if (strlen(query_sring) > AuditLog_log_query_str_size)
			audit_ret->qry_string = pnstrdup(query_sring, AuditLog_log_query_str_size);
		else
...
static AuditMode
audit_hit_get_reverse_mode(bool is_success)
{
	if (is_success)
	{
...
static bool
audit_hit_match_in_pg_audit_o(AuditHitInfo * audit_hit,
									   	  AuditSQL action_id,
									   	  AuditMode reverse_mode)
{
	int32 sys_cacheid = InvalidSysCacheID;
...
static bool
audit_hit_match_in_pg_audit_d(AuditHitInfo * audit_hit,
									   	  AuditSQL action_id,
									   	  AuditMode reverse_mode)
{
	int32 sys_cacheid = InvalidSysCacheID;
...
static bool
audit_hit_match_in_pg_audit_u(AuditHitInfo * audit_hit,
									   	  AuditSQL action_id,
									   	  AuditMode reverse_mode)
{
	int32 sys_cacheid = InvalidSysCacheID;
...
static bool
audit_hit_match_in_pg_audit_s(AuditHitInfo * audit_hit,
									   	  AuditSQL action_id,
									   	  AuditMode reverse_mode)
{
	int32 sys_cacheid = InvalidSysCacheID;
...
static void
audit_hit_rebuild_hit_info(AuditHitInfo * hit_info,
									   int hit_index,
									   AuditStmtMap * hit_action,
									   int hit_match,
									   char * hit_audit,
...
static void
audit_hit_match_in_catalog(AuditHitInfo * audit_hit,
									   bool is_success)
{
	ListCell * lc1 = NULL;
	ListCell * lc2 = NULL;
...
static void
format_query_string(StringInfoData *buf, char *string)
{
	char escapec = '"';
	char quotec = '"';
	char *ptr;
	...
	appendStringInfoCharMacro(buf, quotec);
	while ((c = *ptr) != '\0')
	{
		...
		if (c == quotec || c == escapec)
		{
			...
			appendStringInfoCharMacro(buf, escapec);
...
static void 
audit_hit_print_result_log(void)
{
	ListCell * l = NULL;
	...
	foreach(l, audit_ret->l_hit_info)
	{
		...
		forthree(lc1, audit_hit->l_hit_action, lc2, audit_hit->l_hit_match, lc3, audit_hit->l_hit_audit)
		{
			...
			format_query_string(&buf, audit_ret->qry_string);
			/*
			 * audit log message:
			 * AuditType, QueryString, TopCommandTag, DatabaseID, DatabaseName, 
			 * DatabaseUserID, DatabaseUserName, NodeOid, NodeName, NodeType, 
			 * NodeHost, NodePort ,NodeOSUser, PostgresPID, PostmasterPID, 
			 * BackendStartTime, QueryBeginTime, QueryEndTime, QueryIsSuccess,
			 * ClientHost, ClientHostname, ClientPort, AppName, ObjectClassID,
			 * ObjectId, ObjectSubId, ObjectType, ObjectName, ActionID, ActionName,
			 * ExecStatus
			 */
			audit_log(
				"%s, %s, %s, %u, %s, %u, %s, %u, %s, %s, %s, %d, %s, %u, %u, "
				"%s, %s, %s, %d, %s, %s, %s, %s, %u, %u, %d, %s, %s, %d, %s, %s",

				hit_audit,									// audit type
				buf.data,		// query_string
...
static void
audit_hit_process_result_info(bool is_success)
{
	ListCell * l = NULL;
	AuditResultInfo * audit_ret = audit_hit_get_result_info();
