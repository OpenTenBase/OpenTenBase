...
static double dependency_degree(int numrows, HeapTuple *rows, TupleTableSlot *slot,
					int k, AttrNumber *dependency, VacAttrStats **stats, Bitmapset *attrs);
 ...
 */
static double
dependency_degree(int numrows, HeapTuple *rows, TupleTableSlot *slot, int k, 
...
statext_dependencies_build(int numrows, HeapTuple *rows, 
					TupleTableSlot *slot, Bitmapset *attrs, VacAttrStats **stats)
{
	int			i,
				j,
				k;
	...
	for (k = 2; k <= numattrs; k++)
	{
		...
		while ((dependency = DependencyGenerator_next(DependencyGenerator)))
		{
			...
			degree = dependency_degree(numrows, rows, slot, k, dependency, stats, attrs);
