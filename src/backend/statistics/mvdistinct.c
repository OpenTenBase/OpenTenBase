...
static double ndistinct_for_combination(double totalrows, int numrows,
						  HeapTuple *rows, TupleTableSlot *slot, VacAttrStats **stats,
 ...
 */
MVNDistinct *
statext_ndistinct_build(double totalrows, int numrows, HeapTuple *rows, TupleTableSlot *slot,
						Bitmapset *attrs, VacAttrStats **stats)
{
	...
	for (k = 2; k <= numattrs; k++)
	{
		...
		while ((combination = generator_next(generator)))
		{
			...
			item->ndistinct =
				ndistinct_for_combination(totalrows, numrows, rows, slot,
										  stats, k, combination);
...
ndistinct_for_combination(double totalrows, int numrows, HeapTuple *rows,
						  TupleTableSlot *slot, VacAttrStats **stats, int k, int *combination)
{
	int			i,
				j;
	int			f1,
