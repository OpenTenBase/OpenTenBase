...
static bool flatten_rtes_to_list_walker(Node *node, List **rtelist);
static void add_rte_to_flat_rtable_list(List **rtelist, RangeTblEntry *rte);
...
static void set_remotetidscan_references(PlannerInfo *root, TidScan *plan, int rtoffset);
#endif
...
/*
 * Extract RangeTblEntries from a query that was never planned at all, for pgxc_planner
 */
void
flatten_unplanned_query(List **rtelist, Query *query)
{
	/* Use query_tree_walker to find all RTEs in the parse tree */
	(void) query_tree_walker(query,
							 flatten_rtes_to_list_walker,
							 (void *) rtelist,
							 QTW_EXAMINE_RTES_BEFORE | QTW_IGNORE_DUMMY_RTE);
}

static bool
flatten_rtes_to_list_walker(Node *node, List **rtelist)
{
	if (node == NULL)
		return false;
	if (IsA(node, RangeTblEntry))
	{
		...
		if (rte->rtekind == RTE_RELATION)
			add_rte_to_flat_rtable_list(rtelist, rte);
		return false;
	}
	if (IsA(node, Query))
	{
	...
	return expression_tree_walker(node, flatten_rtes_to_list_walker,
								  (void *) rtelist);
}

static void
add_rte_to_flat_rtable_list(List **rtelist, RangeTblEntry *rte)
{
	...
	*rtelist = lappend(*rtelist, newrte);
	 ...
	 * values.
	 */
	if (IS_SPECIAL_VARNO(list_length(*rtelist)))
		ereport(ERROR,
...
set_plan_refs(PlannerInfo *root, Plan *plan, int rtoffset)
{
	...
	switch (nodeTag(plan))
	{
		...
		case T_TidScan:
			{
				...
				if (splan->remote)
					set_remotetidscan_references(root, splan, rtoffset);
			}
			break;
		case T_SubqueryScan:
...
#ifdef __OPENTENBASE_C__
				if (distinct_optimizer)
				{
					long dnum = plan->lefttree->plan_rows / agg->numGroups;

					set_distinct_aggrefs((Node *) plan->targetlist, &dnum);
					set_distinct_aggrefs((Node *) plan->qual, &dnum);
				}
#endif
				set_upper_references(root, plan, rtoffset);
			}
		...
		case T_WindowAgg:
			{
				 ...
				 * opentenbase_ora mode can contain variable refs, it will be processed
				 * in set_upper_references, so here can skip it.
				 */
				if (!ORA_MODE)
				{
					wplan->startOffset =
		...
		case T_Result:
			{
				...
				if (splan->plan.lefttree != NULL)
					/* 
					 * SORT operation delays the execution of volatile functions,
					 * expecting the RESULT node to handle them. However, the
					 * current use of set_dummy_tlist_references in the RESULT
					 * node leads to erroneous references to non-existent function
					 * targets in sub-node, resulting in column misalignment and
					 * various errors. Restore original logic.
					 */
					set_upper_references(root, plan, rtoffset);
				else
				{
							...
							lcr, splan->resultRelations)
					{
						...
						foreach(l, mergeActionList)
						{
							...
							if (ORA_MODE && action->deleteAction)
							{
								...
								delAction->qual = (Node *) fix_join_expr(root,
							                                             (List *) delAction->qual,
							                                             NULL,
							                                             itlist,
							                                             resultrel,
		...
		case T_RemoteModifyTable:
			{
				RemoteModifyTable *rmt = castNode(RemoteModifyTable, plan);
				ModifyTable *mt;
				ListCell    *lc;
				ListCell    *tle_lc;
				
				if (IsA(plan->lefttree, RemoteQuery))
					break; /* just break, already done by standard_planner */
				else if (IsA(plan->lefttree, RemoteSubplan))
					mt = (ModifyTable *) plan->lefttree->lefttree;
				else if (IS_CENTRALIZED_MODE)
					mt = (ModifyTable *) plan->lefttree;
				else
					elog(ERROR, "error in RemoteModifyTable construction");
				
				/* remote lock stmt may have projection or qualification, set refs for them */
				foreach(lc, rmt->dist_update_locks)
				{
					RemoteQuery *rq = lfirst_node(RemoteQuery, lc);
					
					if (rq->remote_target != NIL)
					{
						indexed_tlist *subplan_itlist;
						List	   *output_targetlist = NIL;
						
						subplan_itlist = build_tlist_index(rq->remote_query->targetList);
						
						foreach(tle_lc, rq->remote_target)
						{
						...
						rq->remote_target = output_targetlist;
						rq->remote_quals = (List *)
							fix_upper_expr(root,
							               (Node *) rq->remote_quals,
							               subplan_itlist,
							               OUTER_VAR,
							               rtoffset);
				...
				set_upper_references(root, plan, rtoffset);
				if (rmt->returningLists)
				{
					List *newRL = NIL;
					ListCell *lcrl, *lcrr;
					Plan *subplan = mt->plan.lefttree;
					/*
					 * Pass each per-subplan returningList through
					 * set_returning_clause_references().
					 */
					forboth(lcrl, rmt->returningLists,
					         lcrr, mt->resultRelations)
					{
						List *rlist = (List *) lfirst(lcrl);
						Index resultrel = lfirst_int(lcrr);
                 ...
                 * are available from the scan tuple.
                 */
                if (rmt->remoteMergeActionList != NIL)
                {
                    /*
                     * mergeSourceTargetList is already setup correctly to
                     * include all Vars coming from the source relation. So we
                     ...
                     */
					indexed_tlist *itlist = NULL;
					Plan          *reSubplan = outerPlan(rmt);
					ModifyTable   *mt = (ModifyTable *) outerPlan(reSubplan);
					Plan          *subplan = outerPlan(mt);
					itlist = build_tlist_index(subplan->targetlist);
					foreach (l, rmt->remoteMergeActionList)
					{
						MergeAction *action = (MergeAction *) lfirst(l);
		...
		case T_MergeQualProj:
			{
				MergeQualProj *splan = (MergeQualProj *) plan;
				set_dummy_tlist_references(plan, rtoffset);
				if (splan->mergeActionList != NIL)
				{
					indexed_tlist *itlist = NULL;
					Plan          *subplan = splan->plan.lefttree;
					itlist = build_tlist_index(subplan->targetlist);
					foreach (l, splan->mergeActionList)
					{
						...
						action->targetList =
							fix_join_expr(root,
										  ...
										  NULL, itlist,
										  (root->glob->resultRelations ?
										  linitial_int(root->glob->resultRelations):root->parse->resultRelation),
										  rtoffset);
						...
						{
							MergeAction *delAction = action->deleteAction;
							delAction->targetList = fix_join_expr(root,
	...
	/*
	 * After processing the child for PartIterator, it is necessary to bump up
	 * plan_node_id to ensure that DSM can be correctly set if parallel aware.
	 */
	if (IsA(plan, PartIterator))
		root->glob->lastPlanNodeId += ((PartIterator *) plan)->leafNum - 1;
	...
	if (IsA(plan, Gather) &&
		...
		castNode(ModifyTable, plan->lefttree)->returningLists != NIL)
	{
							...
							int rtoffset)
{
	...
	if (trivial_subqueryscan(plan))
	{
		...
		if (result->remote_flag == 0)
			result->remote_flag = plan->scan.plan.remote_flag;
		result->is_subplan = plan->scan.plan.is_subplan;
	}
	else
	{
		/*
	...
	/*
	 * In OpenTenBase, when processing the initplan in SS_set_subplan, it is
	 * necessary to collect the initplan in the root of the subquery to the
	 * upper level, otherwise the root will not be found later.
	 */
	foreach(lc, rel->subroot->init_plans)
		root->init_plans = list_append_unique_ptr(root->init_plans, lfirst(lc));
...
fix_expr_common(PlannerInfo *root, Node *node)
{
	...
	else if (IsA(node, GroupingFunc))
	{
		...
		/* TODO: now, the path is created by create_agg_path in create_grouping_append_path, not created by
		 * create_groupingsets_path. So the plan node is created by create_agg_plan, not by create_groupingsets_plan.
		 * So, grouping_map is NULL, and g->cols is not null
		 *
...
set_upper_references(PlannerInfo *root, Plan *plan, int rtoffset)
{
	...
	plan->qual = (List *)
		fix_upper_expr(root,
					   ...
					   OUTER_VAR,
					   rtoffset);
	if (ORA_MODE && IsA(plan, WindowAgg))
	{
		WindowAgg *wplan = (WindowAgg *)plan;

		if (wplan->start_contain_vars)
			wplan->startOffset = (Node *) fix_upper_expr(root,
														 wplan->startOffset,
														 subplan_itlist,
														 OUTER_VAR,
														 rtoffset);
		...
		if (wplan->end_contain_vars)
			wplan->endOffset = (Node *) fix_upper_expr(root,
													   wplan->endOffset,
													   subplan_itlist,
													   OUTER_VAR,
													   rtoffset);
...
#ifdef __OPENTENBASE_C__
			newvar->varrelid = oldvar->varrelid;
			newvar->varrelcolid = oldvar->varrelcolid;
			newvar->varseq = oldvar->varseq;
#endif
		}
		else
		{
...
fix_upper_expr_mutator(Node *node, fix_upper_expr_context *context)
{
	...
	if (IsA(node, CBRootExpr))
	{
		/*
		 * CBRootExpr should only appear in target list of CONNECT BY plan,
		 * so the context->newvarno is INNER_VAR definitely. and we change
		 * it to OUTER_VAR for expr inside and when processing projection
		 * during execution of CONNECT BY, we consider the ecxt_outertuple
		 * of econtext as "root" slot, and fetch root value from it.
		 *
		 * This require CONNECT BY do not use the ecxt_outertuple or we need
		 * to add another field like ecxt_roottuple in econtext, thankfully
		 * it do not.
		 */
		Index   save = context->newvarno;
		CBRootExpr *root = castNode(CBRootExpr, node);

		/*
		 * cn-expr inside connect_by_root will still be evaluated rather
		 * than referred, not support yet...
		 */
		if (contain_cn_expr(root->expr))
			elog(ERROR, "cn-expr blended in connect_by_root is not yet supported");

		Assert(save == INNER_VAR);
		context->newvarno = OUTER_VAR;
		root->expr = fix_upper_expr_mutator(root->expr, context);
		context->newvarno = save;
		/* return the expr inside, we don't need CBRootExpr for execution */
		return root->expr;
	}
	fix_expr_common(context->root, node);
	return expression_tree_mutator(node,
...
{
	/*
	 * Assign fid now, set_plan_refs is a top-bottom progress, so the fid from
	 * top to bottom is in order as before.
	 */
	((RemoteSubplan *)plan)->fid = ++root->glob->fragmentNum;

	/* normal references process */
...
#ifdef __OPENTENBASE__
/*
 * set_remotetidscan_references
 * 
 * This is pretty much like set_upper_references. But we consider
 * TidScan->remote_targets as the "outer" plan, not our left tree
 * (for global index, it's an indexscan).
 */
static void
set_remotetidscan_references(PlannerInfo *root, TidScan *plan, int rtoffset)
{
	indexed_tlist *subplan_itlist;
	List	   *output_targetlist;
	ListCell   *l;
	...
	Assert(plan->remote);
	Assert(plan->tidquals == NULL);
	Assert(plan->remote_targets != NIL);
	
	plan->remote_targets =
		fix_scan_list(root, plan->remote_targets, rtoffset);
	plan->remote_indexqual =
		fix_scan_list(root, plan->remote_indexqual, rtoffset);
	
	subplan_itlist = build_tlist_index(plan->remote_targets);
	
	output_targetlist = NIL;
	foreach(l, plan->scan.plan.targetlist)
	{
		TargetEntry *tle = (TargetEntry *) lfirst(l);
		Node	   *newexpr;
	...
	plan->scan.plan.targetlist = output_targetlist;
	...
	/* also the remote index cond need upper reference too */
	plan->remote_indexqual = (List *)
		fix_upper_expr(root,
		               (Node *) plan->remote_indexqual,
		               subplan_itlist,
		               OUTER_VAR,
		               rtoffset);
