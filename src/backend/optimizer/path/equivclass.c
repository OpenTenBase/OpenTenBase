...
static bool reconsider_full_join_clause(PlannerInfo *root,
							RestrictInfo *rinfo);
static bool check_predicate_during_join(Expr *item1, Expr *item2,
					...
					bool below_outer_join)
{
	...
	/* Avoid subplan calcualte twice */
	if (check_predicate_during_join(item1, item2, item1_relids, item2_relids))
		return false;
						 ...
						 bool create_it)
{
	 ...
	 * correct.
	 *
	 * also, we check prior params when there is CONNECT BY.
	 */
	if (newec->ec_has_const)
	{
		if (newec->ec_has_volatile ||
			...
			contain_prior_params((Node *) expr, root->glob->cb_params))
		{
			newec->ec_has_const = false;
...
/*
 check_predicate_during_join
	The subplan may be calculate twice during single table access and also join.
	It may cause error when the subplan is in the different remote fragments.

	Currently, a small scope is issued to avoid the problem, and extend in the future.
	The function could be removed if the subplan could be sent to more than one target.
 */
static bool
check_predicate_during_join(Expr *item1, Expr *item2, Relids item1_relids, Relids item2_relids)
{
	if (bms_equal(item1_relids, item2_relids))
		return false;

	if (bms_num_members(item1_relids) == 1 &&
		IsA(item1, CaseExpr) &&
		contain_subplans((Node*)item1))
		return true;
	
	if (bms_num_members(item2_relids) == 1 &&
		IsA(item2, CaseExpr) &&
		contain_subplans((Node*)item2))
		return true;
