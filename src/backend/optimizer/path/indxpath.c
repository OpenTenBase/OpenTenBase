...
#define ADJUST_FACTOR_COLUMN_STORE_INDEX 500
#define ADJUST_BAR_ROWS 25
#define ADJUST_THRESHOLD_ROWS 10000
...
/*
 * adjust_column_store_index
 *		Adjust the cost for index on column store table.
 *
 * Background:
 * When index on column store table accesses tuples, it opens the tuples
 * one by one. Comparing with bitmap access, the cost for index scan on
 * column store table is underestimated.
 * 
 * Solution:
 * Give a punishment factor for the index scan on column store table.
 * 
 * Formula:
 * rows      <= 25    : No push for index scan access
 * 25 < rows <= 10000 : 1 + (rows - 25)/(10000 - 25)*(500-1)
 * 10000     < rows   : 500
 * 
 * Condition:
 * 1. The index is build up on column store table.
 * 2. Use index scan access not including index only scan 
 */
static void
adjust_column_store_index(IndexPath *ipath)
{
	/*
	 * If there is smaller than ADJUST_BAR_ROWS, the index scan access 
	 * is no worse than bitmap access.
	 * 
	 * ADJUST_BAR_ROWS is 25 currently depending on test.
	 */
	if (ipath->path.rows <= ADJUST_BAR_ROWS)
		return;
				...
				List **bitindexpaths)
{
	...
	foreach(lc, indexpaths)
	{
		...
		{
			adjust_column_store_index(ipath);
...
check_index_only(RelOptInfo *rel, IndexOptInfo *index)
{
	...
	if (!enable_indexonlyscan)
		return false;
	idx_rel = index_open(index->indexoid, NoLock);
	if (RelationIsCrossNodeIndex(idx_rel->rd_rel))
	{
...
approximate_joinrel_size(PlannerInfo *root, Relids relids)
{
	...
	while ((relid = bms_next_member(relids, relid)) >= 0)
	{
		...
		num_nodes = get_rel_num_data_nodes(root, rel);
#endif

		/* Accumulate product */
		rowcount *= ROWS_PER_DN(rel->rows);
	}
	return rowcount;
}
...
bool
allow_global_index_path(Query *query)
{
	Node    *quals;
	...
	/* if guc is not allowed */
	if (!enable_global_indexscan)
		return false;
	
	/* SELECT FOR UPDATE with global index is not supported for now */
	if (query->rowMarks && query->commandType == CMD_SELECT)
		return false;

	quals = query->jointree ? query->jointree->quals : NULL;
	if (unsupport_quals_for_global_index(quals))
		return false;
