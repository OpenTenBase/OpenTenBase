...
static Node *pull_up_sublinks_jointree_recurse(PlannerInfo *root, Node *jtnode,
								  Relids *relids, Node *allQuals);
static Node *pull_up_sublinks_qual_recurse(PlannerInfo *root, Node *node,
							  Node **jtlink1, Relids available_rels1,
							  Node **jtlink2, Relids available_rels2,
							  Node *allQuals, Relids all_rels);
...
#endif
extern bool enable_lateral_pull_up;
/*
...
transform_MERGE_to_join(Query *parse)
{
	...
	foreach (l, parse->mergeActionList)
	{
		...
		/**
		 * Checks if the target relation is a replication relation and if the action contains
		 * volatile functions. If both conditions are met and the system is not a centralized data
		 * node, an error is reported.
		 *
		 */
		if (RelationIsReplication(target_relation) &&
		    contain_volatile_functions((Node *) action) && !IS_CENTRALIZED_DATANODE)
		{
			ereport(
				ERROR,
				(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
			     ...
			     errhint(
					 "MERGE is not supported for replicated relations with volatile function.")));
		}
	...
	Assert(IsA(linitial(parse->jointree->fromlist), FromExpr));
	fromExpr = linitial_node(FromExpr, parse->jointree->fromlist);
	/* Manufacture a join RTE to use. */
	joinrte = makeNode(RangeTblEntry);
	joinrte->rtekind = RTE_JOIN;
	/* Get new join type from transformMergeStmt by (+). */
	if (list_length(fromExpr->fromlist) != 1)
	{
		Assert(list_length(fromExpr->fromlist) == 2);
		newJoinExpr = lsecond_node(JoinExpr, fromExpr->fromlist);
		jointype = newJoinExpr->jointype;
		Assert(jointype != JOIN_RIGHT || jointype != JOIN_LEFT);
	}
	...
	/*
	 * When the target is a view or subquery, "quals" represents the qualifier condition in it,
	 * otherwise, NULL
	 */
	if (parse->jointree->quals != NULL)
	{
		RangeTblRef *targetRTR = makeNode(RangeTblRef);
		targetRTR->rtindex = parse->resultRelation;
		joinexpr->larg = (Node *) makeFromExpr(list_make1(targetRTR), parse->jointree->quals);
	...
	joinexpr->rarg = linitial(fromExpr->fromlist);	/* original join */
	joinexpr->usingClause = NIL;
	/* The quals are removed from the jointree and into this specific join */
	joinexpr->quals = fromExpr->quals;
	joinexpr->alias = NULL;
...
static bool
check_having_quals_tagetlist_pushdown(Query *parse)
{
	if (parse == NULL ||
	...
	if ((parse->groupClause && parse->groupingSets) ||
		contain_aggs_of_level(parse->havingQual, 0) ||
		contain_volatile_functions(parse->havingQual) ||
		contain_subplans(parse->havingQual))
	{
			return false;
	}
...
pull_up_sublinks(PlannerInfo *root)
{
    ...
    if (enable_pullup_subquery &&
		check_having_quals_tagetlist_pushdown(root->parse))
    {
		...
		List		*sublink_case_list = NULL; /* Support reducing duplicated sublinks*/
		List		*qual_case_list = NULL;    /* Support reducing duplicated sublinks*/
        ...
        foreach(lc, root->parse->targetList)
        {
			...
			new_entry = convert_TargetList_sublink_to_join(root, entry, &is_pull_up,
														   &sublink_case_list, &qual_case_list);
            if (new_entry)
                new_targetList = lappend(new_targetList, new_entry);
			/*
			 * Avoid add multi-expresion sublink back if the sublink is already
			 * pulled up.
			 */
            else if (!is_pull_up || !IsA(entry->expr, SubLink) ||
					 root->parse->commandType != CMD_UPDATE ||
		 			 ((SubLink *)entry->expr)->subLinkType != MULTIEXPR_SUBLINK)
                new_targetList = lappend(new_targetList, entry);
        }
		list_free(sublink_case_list);
...
/*
 * Assign qualify clauses after pull up.
 */
static Node*
assign_qual_clause(Node* new_node, Node* old_node, Node* qual, Relids old_node_relids)
{
	Relids qual_varnos;
								  ...
								  Relids *relids, Node *allQuals)
{
	...
	else if (IsA(jtnode, FromExpr))
	{
		...
		quals = pull_up_sublinks_qual_recurse(root, f->quals,
													&jtlink, frelids,
													NULL, NULL, f->quals, frelids);
		 ...
		 */
		jtlink = assign_qual_clause(jtlink, (Node*)newf, quals, current_relids);
	...
	else if (IsA(jtnode, JoinExpr))
	{
		...
		{
			case JOIN_INNER:
				quals = pull_up_sublinks_qual_recurse(root, j->quals,
														 &jtlink,
...
/*
 * build_exists_or_sublink_list
 *	Build up sublink list for OR clause.
 */
static bool
build_exists_or_sublink_list(Node* node, List *rtable, List **or_list)
{
	if (node == NULL)
		return false;
	...
	{
		SetOperationStmt* setOperationStmt = (SetOperationStmt *)node;

		if (setOperationStmt->op != SETOP_UNION)
			return false;

		return build_exists_or_sublink_list(setOperationStmt->larg,
											rtable, or_list) &&
			   build_exists_or_sublink_list(setOperationStmt->rarg,
											rtable, or_list);
	}
	else if (IsA(node, RangeTblRef))
	{
		...
		rtindex = ((RangeTblRef*)node)->rtindex;

		if (rtindex < 0 || rtindex > list_length(rtable))
			return false;

		rte = (RangeTblEntry*) list_nth(rtable, rtindex - 1);

		if (rte->subquery == NULL ||
			!IsA(rte->subquery, Query))
			return false;

		sub_link = makeNode(SubLink);
		sub_link->subLinkType = EXISTS_SUBLINK;

		sub_link->subselect = (Node *) copyObject((Query *) rte->subquery);
		sub_link->testexpr = NULL;

		IncrementVarSublevelsUp(sub_link->subselect, -1, 2);
		*or_list = lappend(*or_list, sub_link);
...
/*
 * check_exists_sublink_with_union
 *	Convert exist with union (all) to or exists.
 */
static Node *

check_exists_sublink_with_union(PlannerInfo *root, SubLink *sublink)
{
	Node		*or_node;
	List		*or_list = NULL;
	...
	if (sublink->subLinkType != EXISTS_SUBLINK ||
		...
		((Query*)sublink->subselect)->setOperations == NULL ||
		((Query*)sublink->subselect)->rtable == NULL ||
		((Query*)sublink->subselect)->limitOffset != NULL ||
		((Query*)sublink->subselect)->cteList != NULL)
		return NULL;

	if (((Query*)sublink->subselect)->limitCount != NULL)
	{
		Node   *limitCount;
		int64	limit_count = 0;

		limitCount = eval_const_expressions(root,
							((Query*)sublink->subselect)->limitCount);

		if (limitCount && IsA(limitCount, Const))
		{
			if (((Const *) limitCount)->constisnull)
			{
			...
			{
				limit_count = DatumGetInt64(((Const *) limitCount)->constvalue);
			}
		}
		else
			limit_count = -1;	/* can't evaluate */

		if (limit_count <= 0)
			return NULL;
	}

	if (!build_exists_or_sublink_list(((Query*)sublink->subselect)->setOperations,
				((Query*)sublink->subselect)->rtable, &or_list))
	{
		list_free(or_list);
		return NULL;
	}

	or_node = (Node*)make_orclause(or_list);
							  ...
							  Node *allQuals, Relids all_rels)
{
	...
	if (IsA(node, SubLink))
	{
		...
		if (sublink->subLinkType == ANY_SUBLINK ||
			sublink->subLinkType == ROWCOMPARE_SUBLINK)
		{
			bool sclar_join = (sublink->subLinkType == ROWCOMPARE_SUBLINK);

			if ((j = convert_ANY_sublink_to_join(root, sublink, false,
												 available_rels1, sclar_join, all_rels)) != NULL)
			{
			...
			if (available_rels2 != NULL &&
				(j = convert_ANY_sublink_to_join(root, sublink, false,
												 available_rels2, sclar_join, all_rels)) != NULL)
			{
				/* Yes; insert the new join node into the join tree */
				j->larg = *jtlink2;
				*jtlink2 = (Node *) j;
		...
		else if (sublink->subLinkType == EXISTS_SUBLINK)
		{
			...
			/*
			 * Try convert exist with union (all) to or exists.
			 */
			or_node = check_exists_sublink_with_union(root, sublink);
			...
			if (or_node != NULL)
			{
				convert_ORCLAUSE_to_join(root, (BoolExpr*)or_node, jtlink1, &available_rels1);

				if (available_rels2 != NULL)
				{
					convert_ORCLAUSE_to_join(root, (BoolExpr *)or_node, jtlink2, &available_rels2);
				}
		...
		if (sublink && IsA(sublink, SubLink))
		{
			...
			else if (sublink->subLinkType == ANY_SUBLINK)
			{
				if ((j = convert_ANY_sublink_to_join(root, sublink, true,
													 available_rels1, false, all_rels)) != NULL)
				{
					/* Yes; insert the new join node into the join tree */
					j->larg = *jtlink1;
					*jtlink1 = (Node *) j;
				...
				if (available_rels2 != NULL &&
					(j = convert_ANY_sublink_to_join(root, sublink, true,
												 available_rels2, false, all_rels)) != NULL)
				{
					/* Yes; insert the new join node into the join tree */
					j->larg = *jtlink2;
					*jtlink2 = (Node *) j;
	...
	if (OpExpr_clause(node))
	{
		...
		Node       *filter = NULL;
		Relids		child_rels;
		bool		contains_case = false;
		ListCell	*lc = NULL;
		bool		contains_nonstrict = false;
		...
		contains_nonstrict = contain_nonstrict_functions_before_sublink((Node *)node);
		
		if ( !contains_case &&
			((j = convert_EXPR_sublink_to_join(root, jtlink1, (OpExpr *)node,
										available_rels1, &filter, allQuals, contains_nonstrict)) != NULL))
		{
			/* Yes; insert the new join node into the join tree */
			j->larg = *jtlink1;
			...
			*jtlink1 = (Node *)from;
			...
			if (contains_nonstrict)
				return filter;
			else
				return NULL;
		}
		else if (contains_case)
		{
			/*
			 * Check sublink under CASE expression
			 */
			convert_CASE_to_join(root, (OpExpr*)node, jtlink1, &available_rels1);
		}

		if (available_rels2 != NULL &&
					(j = convert_EXPR_sublink_to_join(root, jtlink1, (OpExpr *)node,
										available_rels2, &filter, allQuals, contains_nonstrict)) != NULL)
		{
			/* Yes; insert the new join node into the join tree */
			j->larg = *jtlink2;
						...
						bool deletion_ok)
{
    ...
    if (parse->mergeActionList)
	{
		foreach (lc, parse->mergeActionList)
		{
			...
			action->targetList =
				(List *) pullup_replace_vars((Node *) action->targetList, &rvcontext);
			if (action->deleteAction)
			{
				Assert(ORA_MODE && action->commandType == CMD_UPDATE);
				action->deleteAction->qual =
					pullup_replace_vars(action->deleteAction->qual, &rvcontext);
			}
		}
	}
	...
	if (parse->connectByExpr)
	{
		parse->connectByExpr->expr =
			pullup_replace_vars(parse->connectByExpr->expr, &rvcontext);
		parse->connectByExpr->start =
			pullup_replace_vars(parse->connectByExpr->start, &rvcontext);
	}
				   ...
				   bool deletion_ok)
{
	 ...
	 *
	 * OpenTenBase: can't pull up a subquery involving CONNECT BY or rownum too.
	 *
	...
	if (subquery->jointree->fromlist == NIL &&
		(subquery->jointree->quals != NULL ||
		 ...
		 lowest_outer_join != NULL))
		return false;
	if (rte->lateral && !enable_lateral_pull_up)
		return false;
	/*
