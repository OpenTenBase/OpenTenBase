				  ...
				  RelOptInfo *rel)
{
	 ...
	 */
	if ((inhparent && !(enable_partition_iterator && !enable_partitionwise_join)) ||
	    (IgnoreSystemIndexes && IsSystemRelation(relation)))
		hasindex = false;
	else if (relation->rd_rel->relkind == RELKIND_PARTITIONED_TABLE &&
            !relation->rd_rel->relispartition && !enable_partition_iterator)
    {
        /* If partition table has not partitioned, treat like no index */
        hasindex = false;
    }
	else
	...
	if (hasindex)
	{
		...
		foreach(l, indexoidlist)
		{
			...
			/* Skip this index if it miss storage file */
			if (RELATION_IS_META_GLOBAL_TEMP(indexRelation))
			{
				RelationOpenSmgr(indexRelation);
				if (!smgrexists(indexRelation->rd_smgr, MAIN_FORKNUM))
				{
					ereport(WARNING,
						((errmsg("Missing global temp table file for index \"%s\" on table \"%s\"",
								RelationGetRelationName(indexRelation), RelationGetRelationName(relation))),
							errhint("Please reindex the table.")));
					index_close(indexRelation, NoLock);
					continue;
				}
			}
			...
			info->amhasgetbitmap = (amroutine->amgetbitmap != NULL 
				&& !(RelationIsCrossNodeIndex(indexRelation->rd_rel)));
			...
			if (info->indpred == NIL)
			{
				 ...
				 */
				if ((IS_PGXC_COORDINATOR || RELATION_IS_PG_PARTITION(relation)) &&
			...
			if (info->relam == BTREE_AM_OID && !(RelationIsCrossNodeIndex(indexRelation->rd_rel)) &&
			    !RELATION_IS_PG_PARTITION(relation))
			{
	...
	switch (rel->rd_rel->relkind)
	{
		...
		case RELKIND_PARTITIONED_GLOBAL_INDEX:
			...
			if (ENABLE_AUTO_ANALYZE_IN_XACT())
			{
				...
				if (relation_need_autoanalyze(rel->rd_id, true))
				{
				...
				if (LocalStatisticGetRelStats(rel->rd_id, &pages, NULL, &tuples, NULL))
				{
					if (IS_PGXC_COORDINATOR && rel->rd_locator_info != NULL)
					{
						curpages = pages;
								 ...
								 RelOptInfo *rel, RangeTblEntry *rte)
{
	...
	/* If limit 0, the relation should exclude */
	if (limitCount && IsA(limitCount, Const) &&
		(!((Const *) limitCount)->constisnull) &&
		((Const *) limitCount)->constvalue == 0)
		return true;
