					  ...
					  bool create_new_ph)
{
	...
	if (bms_is_empty(phinfo->ph_eval_at))
	{
		...
		/*
		 * Postgresql add a new RTE type as RTE_RESULT as dummy table
		 * when the fromlist is empty. There is no such RTE type currently,
		 * and the relation list is empty.
		 */
