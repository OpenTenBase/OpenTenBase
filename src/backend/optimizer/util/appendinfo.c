...
static bool make_inh_translation_list(Relation oldrelation,
					 ...
					 Index parentRTindex, Index childRTindex)
{
	...
	if (make_inh_translation_list(parentrel, childrel, childRTindex, &appinfo->translated_vars))
		appinfo->isPartHeterogeneous = true;
						  ...
						  List **translated_vars)
{
	...
	for (old_attno = 0; old_attno < oldnatts; old_attno++)
	{
		...
		vars = lappend(vars, makeVar(newvarno, (AttrNumber) (new_attno + 1), atttypid, atttypmod,
		                             attcollation, 0));
		if (!needTrans && new_attno != old_attno)
			needTrans = true;
					 ...
					 Index rtindex, const char *rowid_name)
{
	...
	/* To make equalVar work as pg */
	rowid_var->varnoold = 0;
						 ...
						 Relation target_relation)
{
	...
	bool		has_global_index = relation_has_cross_node_index(target_relation);

	/*
	 * If relation is non-replicated, we need also to identify the Datanode
	 * from where tuple is fetched.
	 */
	if ((IS_PGXC_COORDINATOR || has_global_index) &&
	...
	{
		if (RelationIsRowStore(target_relation))
		{
			var = makeVar(rtindex,
						  ...
						  InvalidOid,
						  0);
			add_row_identity_var(root, var, rtindex, "shardid");
		}
	}
#endif

	Assert(commandType == CMD_UPDATE ||
...
distribute_row_identity_vars(PlannerInfo *root)
{
	...
	foreach(lc, root->processed_tlist)
	{
		...
		{
			if (CAN_PARTITERATOR(target_rel, target_rte))
			{
				int                 ndx;
				RowIdentityVarInfo *ridinfo =
					(RowIdentityVarInfo *) list_nth(root->row_identity_vars, var->varattno - 1);
				var->varattno = ridinfo->rowidvar->varattno;
				var->varno = target_rel->relid;
				var->varnoold = 0;
				var->varoattno = 0;

				ndx = var->varattno - target_rel->min_attr;
				target_rel->attr_needed[ndx] =
					bms_add_members(target_rel->attr_needed[ndx], bms_make_singleton(0));
			}

			target_rel->reltarget->exprs = lappend(target_rel->reltarget->exprs, copyObject(var));
