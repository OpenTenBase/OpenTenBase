						...
						RangeTblEntry *rte, Index rti)
{
	...
	if (oldrelation->rd_rel->relkind == RELKIND_PARTITIONED_TABLE)
	{
		 ...
		 */
		expand_partitioned_rtentry(root, rel, rte, rti, oldrelation, oldrc,
									lockmode);
	}
	else
	{
						   ...
						   PlanRowMark *top_parentrc, LOCKMODE lockmode)
{
	...
	Assert(parentrte->inh || ((!parentrte->inh) && root->parse->commandType == CMD_MERGE));
	...
	relinfo->num_live_parts = num_live_parts;
	/*
	...
	while ((i = bms_next_member(live_parts, i)) >= 0)
	{
		...
		elog(DEBUG5,
			 "partprune lock child %s of %s, lockmode %d",
			 RelationGetRelationName(childrel), RelationGetRelationName(parentrel), lockmode);
		...
		if (!relinfo->isPartHeterogeneous &&
		    (childrelinfo->isPartHeterogeneous ||
		     root->append_rel_array[childRTindex]->isPartHeterogeneous))
		{
			relinfo->isPartHeterogeneous = true;
		}
	...
	elog(DEBUG5,
		 "partprune partitioned table %s, total parts %d, live part %d",
		 RelationGetRelationName(parentrel), partdesc->nparts, num_live_parts);
}
								...
								Index *childRTindex_p)
{
	...
	*childRTindex_p = childRTindex;
	...
	if (top_parentrc)
	{
		...
		childrc->waitTimeout  = top_parentrc->waitTimeout;
