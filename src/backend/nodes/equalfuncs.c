...
#ifdef _PG_ORCL_
	COMPARE_LOCATION_FIELD(dblinkname);
	COMPARE_LOCATION_FIELD(childtablename);
#endif
...
_equalParam(const Param *a, const Param *b)
{
	...
	COMPARE_LOCATION_FIELD(location);
	COMPARE_NODE_FIELD(paramindextype);
...
_equalFuncExpr(const FuncExpr *a, const FuncExpr *b)
{
	...
	COMPARE_NODE_FIELD(args);
#ifdef _PG_ORCL_
	COMPARE_NODE_FIELD(withfuncnsp);
	COMPARE_SCALAR_FIELD(withfuncid);
#endif
	COMPARE_LOCATION_FIELD(location);
...
_equalMergeAction(const MergeAction* a, const MergeAction* b)
{
	...
	COMPARE_NODE_FIELD(deleteAction);
...
_equalQuery(const Query *a, const Query *b)
{
	...
	/* we intentionally ignore sql_id, since it might not be set */
	...
	COMPARE_SCALAR_FIELD(hasRowNumExpr);
	COMPARE_LOCATION_FIELD(stmt_location);
	COMPARE_LOCATION_FIELD(stmt_len);
	/* begin ora_compatible */
	COMPARE_SCALAR_FIELD(isall);
	COMPARE_NODE_FIELD(multi_inserts);
	/* end ora_compatible */

#ifdef _PG_ORCL_
	COMPARE_NODE_FIELD(with_funcs);
#endif
	return true;
}
...
static bool _equalMergeWhenClause(const MergeWhenClause* a, const MergeWhenClause* b)
{
	...
	COMPARE_NODE_FIELD(deleteClause);
...
_equalSelectStmt(const SelectStmt *a, const SelectStmt *b)
{
	...
	COMPARE_NODE_FIELD(withClause);
	COMPARE_NODE_FIELD(connectClause);
	COMPARE_SCALAR_FIELD(op);
...
#ifdef _PG_ORCL_
	COMPARE_NODE_FIELD(withFuncClause);
#endif
	return true;
}
...
_equalCopyStmt(const CopyStmt *a, const CopyStmt *b)
{
	...
	COMPARE_NODE_FIELD(attinfolist);
	COMPARE_SCALAR_FIELD(fixed_position);
...
_equalDropStmt(const DropStmt *a, const DropStmt *b)
{
	...
	COMPARE_NODE_FIELD(parent_r);
	COMPARE_SCALAR_FIELD(drop_subpart);
...
{
	COMPARE_NODE_FIELD(paretablename);
	COMPARE_NODE_FIELD(relations);
	COMPARE_SCALAR_FIELD(restart_seqs);
	COMPARE_SCALAR_FIELD(behavior);
	COMPARE_SCALAR_FIELD(sc_id);
	COMPARE_SCALAR_FIELD(trunc_subpart);
...
_equalIndexStmt(const IndexStmt *a, const IndexStmt *b)
{
	...
	COMPARE_SCALAR_FIELD(cross_node);
...
static bool
_equalCreateTypeObjectStmt(const CreateTypeObjectStmt *a, const CreateTypeObjectStmt *b)
{
    COMPARE_NODE_FIELD(typevar);
    COMPARE_NODE_FIELD(coldeflist);
    COMPARE_SCALAR_FIELD(replace);
	COMPARE_STRING_FIELD(objheadersrc);
	COMPARE_NODE_FIELD(options);
	COMPARE_NODE_FIELD(objfunclist);
	COMPARE_STRING_FIELD(func_spec);
	COMPARE_SCALAR_FIELD(reparse_spec);
	COMPARE_SCALAR_FIELD(has_cons);
	COMPARE_SCALAR_FIELD(force);
	COMPARE_SCALAR_FIELD(check_dup);

    return true;
}

static bool
_equalCreateTypeObjectBodyStmt(const CreateTypeObjectBodyStmt *a, const CreateTypeObjectBodyStmt *b)
{
	COMPARE_NODE_FIELD(typevar);
    COMPARE_SCALAR_FIELD(replace);
	COMPARE_STRING_FIELD(objheadersrc);
	COMPARE_STRING_FIELD(objbodysrc);
	COMPARE_NODE_FIELD(options);
	COMPARE_NODE_FIELD(objfunclist);
	COMPARE_NODE_FIELD(objtypelist);
	COMPARE_STRING_FIELD(objfundef);
	COMPARE_SCALAR_FIELD(reparse_body);
	COMPARE_SCALAR_FIELD(force);
...
static bool
_equalCreateNestedTableStmt(const CreateNestedTableStmt *a, const CreateNestedTableStmt *b)
{
    COMPARE_NODE_FIELD(typevar);
    COMPARE_NODE_FIELD(subtypename);
    COMPARE_SCALAR_FIELD(replace);
...
static bool
_equalCreateDirStmt(const CreateDirStmt *a, const CreateDirStmt *b)
{
	COMPARE_STRING_FIELD(dirname);
	COMPARE_STRING_FIELD(dirpath);

	return true;
}

static bool
_equalDropDirStmt(const DropDirStmt *a, const DropDirStmt *b)
{
	COMPARE_STRING_FIELD(dirname);
...
_equalFuncCall(const FuncCall *a, const FuncCall *b)
{
	...
	COMPARE_LOCATION_FIELD(location);
	COMPARE_NODE_FIELD(return_columnref);
...
_equalLockingClause(const LockingClause *a, const LockingClause *b)
{
	...
	COMPARE_SCALAR_FIELD(waitTimeout);
...
_equalWindowClause(const WindowClause *a, const WindowClause *b)
{
	...
	COMPARE_SCALAR_FIELD(start_contain_vars);
	COMPARE_SCALAR_FIELD(end_contain_vars);
...
_equalDistribution(const Distribution *a, const Distribution *b)
{
	...
	COMPARE_BITMAPSET_FIELD(restrictNodes);
	COMPARE_SCALAR_FIELD(nExprs);
	for (i = 0; i < a->nExprs; i++)
	{
		COMPARE_NODE_FIELD(disExprs[i]);
	}
...
bool
IsSimilarDistribution(const Distribution *a, const Distribution *b)
{
	int i;
	...
	COMPARE_SCALAR_FIELD(distributionType);
	COMPARE_BITMAPSET_FIELD(nodes);
	/* restrictNodes need not to be checked if distributed by value */
	if (!IsLocatorDistributedByValue(a->distributionType))
		COMPARE_BITMAPSET_FIELD(restrictNodes);

	COMPARE_SCALAR_FIELD(nExprs);
	for (i = 0; i < a->nExprs; i++)
	{
		if (a->disExprs[i] && IsA(a->disExprs[i], Var) &&
			b->disExprs[i] && IsA(b->disExprs[i], Var))
		{
			if (!equalVarExceptVarno(a->disExprs[i], b->disExprs[i]))
				return false;
		}
		else
		{
...
{
	COMPARE_NODE_FIELD(part_rv);
	COMPARE_NODE_FIELD(bound_spec);
	COMPARE_NODE_FIELD(subpart_defs);
...
_equalPartitionSpec(const PartitionSpec *a, const PartitionSpec *b)
{
	...
	COMPARE_NODE_FIELD(subPartSpec);
...
_equalPartitionCmd(const PartitionCmd *a, const PartitionCmd *b)
{
	...
	COMPARE_NODE_FIELD(ex_tbl);

	return true;
}

static bool 
_equalFdwFileSegment(const FdwFileSegment* a, const FdwFileSegment* b)
{
    COMPARE_STRING_FIELD(filename);
    COMPARE_SCALAR_FIELD(begin);
    COMPARE_SCALAR_FIELD(end);
	COMPARE_SCALAR_FIELD(fdwfileSize);
	COMPARE_NODE_FIELD(hudi_delta_log);
    return true;
}

static bool 
_equalFdwDataNodeTask(const FdwDataNodeTask* a, const FdwDataNodeTask* b)
{
    COMPARE_STRING_FIELD(dnName);
	COMPARE_STRING_FIELD(instant_time);
    COMPARE_NODE_FIELD(task);
    return true;
}

static bool
_equalFragmentDest(const FragmentDest *a, const FragmentDest *b)
{
	COMPARE_SCALAR_FIELD(fid);
	COMPARE_SCALAR_FIELD(recvfid);
	COMPARE_SCALAR_FIELD(targetNodeType);
	COMPARE_NODE_FIELD(targetNodes);
	return true;
}

static bool
_equalLevelExpr(const LevelExpr *a, const LevelExpr *b)
{
	return true;
}

static bool
_equalPriorExpr(const PriorExpr *a, const PriorExpr *b)
{
	COMPARE_NODE_FIELD(expr);
	COMPARE_SCALAR_FIELD(for_path);
	return true;
}

static bool
_equalCBIsCycleExpr(const CBIsCycleExpr *a, const CBIsCycleExpr *b)
{
	return true;
}

static bool
_equalCBIsLeafExpr(const CBIsLeafExpr *a, const CBIsLeafExpr *b)
{
	return true;
}

static bool
_equalCBRootExpr(const CBRootExpr *a, const CBRootExpr *b)
{
	COMPARE_NODE_FIELD(expr);
	return true;
}
...
static bool
_equalCBPathExpr(const CBPathExpr *a, const CBPathExpr *b)
{
	COMPARE_NODE_FIELD(expr);
	return true;
}
...
static bool
_equalMultiSetExpr(const MultiSetExpr *a, const MultiSetExpr *b)
{
	COMPARE_NODE_FIELD(lexpr);
	COMPARE_NODE_FIELD(rexpr);
...
static bool
_equalDisKeyAttr(const DisKeyAttr *a, const DisKeyAttr *b)
{
	bool res = false;
	int  i = 0;
	...
	if (!a || !b || a->nDisAttrs != b->nDisAttrs)
		return false;

	for (i = 0; i < a->nDisAttrs; i++)
	{
		if (a->disAttrNums[i] != b->disAttrNums[i] || 
			a->disAttrTypes[i] != b->disAttrTypes[i] || 
			a->disAttrTypMods[i] != b->disAttrTypMods[i])
			break;
	}
...
_equalAlterNodeStmt(const AlterNodeStmt *a, const AlterNodeStmt *b)
{
	...
	COMPARE_STRING_FIELD(plane_name);
	COMPARE_NODE_FIELD(options);
	return true;
}

static bool
_equalQueryTermStmt(const QueryTermStmt *a, const QueryTermStmt *b)
{
	return true;
}

static bool
_equalUpdateTermStmt(const UpdateTermStmt *a, const UpdateTermStmt *b)
{
	COMPARE_SCALAR_FIELD(term);
	return true;
}
...
{
       COMPARE_STRING_FIELD(name);
       COMPARE_NODE_FIELD(profile_args);
       return true;
}
static bool
...
static bool
_equalAlterResourceCostStmt(const AlterResourceCostStmt *a, const AlterResourceCostStmt *b)
{
       COMPARE_NODE_FIELD(resource_args);
       return true;
}
static bool
_equalProfileParamter(const ProfileParameter *a, const ProfileParameter *b)
{
       COMPARE_STRING_FIELD(name);
       COMPARE_NODE_FIELD(value);
       return true;
...
static bool
_equalCreateResourceQueueStmt(const CreateResourceQueueStmt *a, const CreateResourceQueueStmt *b)
{
...
static bool
_equalAlterResourceQueueStmt(const AlterResourceQueueStmt *a, const AlterResourceQueueStmt *b)
{
	COMPARE_STRING_FIELD(queue);
	COMPARE_NODE_FIELD(options);
	return true;
...
static bool
_equalDropResourceQueueStmt(const DropResourceQueueStmt *a, const DropResourceQueueStmt *b)
{
	COMPARE_STRING_FIELD(queue);
	return true;
}
...
static bool
_equalCreateDatabaseLinkStmt(const CreateDatabaseLinkStmt *a, const CreateDatabaseLinkStmt *b)
{
    COMPARE_SCALAR_FIELD(dblinkkind);
    COMPARE_STRING_FIELD(name);
    COMPARE_SCALAR_FIELD(iscurrentuser);
    COMPARE_STRING_FIELD(username);
    COMPARE_STRING_FIELD(passwd);
    COMPARE_STRING_FIELD(connstr);
    return true;
}

static bool
_equalDropDatabaseLinkStmt(const DropDatabaseLinkStmt *a, const DropDatabaseLinkStmt *b)
{
    COMPARE_SCALAR_FIELD(dblinkkind);
    COMPARE_STRING_FIELD(name);
    return true;
...
static bool
_equalCreateSynonymStmt(const CreateSynonymStmt *a, const CreateSynonymStmt *b)
{
	COMPARE_SCALAR_FIELD(replace);
	COMPARE_SCALAR_FIELD(ispublic);
	COMPARE_NODE_FIELD(synname);
	COMPARE_NODE_FIELD(objname);
	COMPARE_STRING_FIELD(dblink);

	return true;
}

static bool
_equalCreatePackageStmt(const CreatePackageStmt *a, const CreatePackageStmt *b)
{
	COMPARE_SCALAR_FIELD(replace);
	COMPARE_SCALAR_FIELD(edition);
	COMPARE_SCALAR_FIELD(body);

	COMPARE_NODE_FIELD(pkgname);
	COMPARE_STRING_FIELD(pkgheadersrc);
	COMPARE_NODE_FIELD(options);
	COMPARE_NODE_FIELD(pkgfunclist);
	COMPARE_NODE_FIELD(pkgtypelist);
	COMPARE_NODE_FIELD(pkgvarlist);
	COMPARE_STRING_FIELD(func_spec);
	COMPARE_SCALAR_FIELD(reparse_spec);
...
static bool
_equalDropPackageStmt(const DropPackageStmt *a, const DropPackageStmt *b)
{
	COMPARE_NODE_FIELD(pkgname);
	COMPARE_SCALAR_FIELD(removeType);
	return true;
...
static bool
_equalCreatePackageBodyStmt(const CreatePackageBodyStmt *a, const CreatePackageBodyStmt *b)
{
	COMPARE_NODE_FIELD(pkgname);
	COMPARE_STRING_FIELD(pkgbodysrc);
	...
	COMPARE_STRING_FIELD(priv_decl);
	COMPARE_STRING_FIELD(func_def);
	COMPARE_STRING_FIELD(initial_part);
...
equal(const void *a, const void *b)
{
	...
	switch (nodeTag(a))
	{
		...
		case T_CreateDirStmt:
			retval = _equalCreateDirStmt(a, b);
			break;
		...
		case T_AlterResourceCostStmt:
				retval = _equalAlterResourceCostStmt(a, b);
				break;
		case T_ProfileParameter:
				retval = _equalProfileParamter(a, b);
				break;	
#endif
		...
		case T_CreateResourceQueueStmt:
			retval = _equalCreateResourceQueueStmt(a, b);
			break;
		case T_AlterResourceQueueStmt:
			retval = _equalAlterResourceQueueStmt(a, b);
			break;
		case T_DropResourceQueueStmt:
			retval = _equalDropResourceQueueStmt(a, b);
			break;
		case T_CreateSynonymStmt:
			retval = _equalCreateSynonymStmt(a, b);
			break;
		case T_CreatePackageStmt:
			retval =_equalCreatePackageStmt(a, b);
			break;
    	case T_CreatePackageBodyStmt:
			retval =_equalCreatePackageBodyStmt(a, b);
			break;
    	case T_DropPackageStmt:
			retval =_equalDropPackageStmt(a, b);
			break;
		case T_CreateTypeObjectBodyStmt:
			retval = _equalCreateTypeObjectBodyStmt(a, b);
			break;
        case T_CreateDatabaseLinkStmt:
            retval = _equalCreateDatabaseLinkStmt(a, b);
            break;
        case T_DropDatabaseLinkStmt:
            retval = _equalDropDatabaseLinkStmt(a, b);
            break;
		case T_ColumnRefJoin:
			retval = _equalColumRefJoin(a, b);
			break;
		case T_CreateTypeObjectStmt:
			retval =_equalCreateTypeObjectStmt(a, b);
			break;
		case T_CreateNestedTableStmt:
			retval =_equalCreateNestedTableStmt(a, b);
			break;
#endif
		case T_FdwDataNodeTask:
            retval = _equalFdwDataNodeTask(a, b);
            break;
        case T_FdwFileSegment:
            retval = _equalFdwFileSegment(a, b);
            break;
		case T_FragmentDest:
			retval = _equalFragmentDest(a, b);
			break;
		case T_LevelExpr:
		...
		case T_CBIsCycleExpr:
			retval = _equalCBIsCycleExpr(a, b);
			break;
		case T_CBIsLeafExpr:
			retval = _equalCBIsLeafExpr(a, b);
			break;
		case T_CBRootExpr:
			retval = _equalCBRootExpr(a, b);
			break;
		case T_CBPathExpr:
			retval = _equalCBPathExpr(a, b);
			break;
		case T_PartitionDef:
			retval = _equalPartitionDef(a, b);
			break;
		case T_QueryTermStmt:
			retval = _equalQueryTermStmt(a, b);
			break;
		case T_UpdateTermStmt:
			retval = _equalUpdateTermStmt(a, b);
			break;
		case T_MultiSetExpr:
			retval = _equalMultiSetExpr(a, b);
			break;
		default:
