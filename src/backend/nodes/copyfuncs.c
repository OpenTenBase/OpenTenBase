 ...
 *src/backend/nodes/copyfuncs.csrc/backend/nodes/copyfuncs.c
 *-------------------------------------------------------------------------
 */
...
_copyPlannedStmt(const PlannedStmt *from)
{
    ...
    COPY_SCALAR_FIELD(canNextvalParallel);
	COPY_SCALAR_FIELD(remote);
	...
	COPY_NODE_FIELD(sharedCtePlans);
	COPY_BITMAPSET_FIELD(sharedCtePlanIds);
	COPY_SCALAR_FIELD(planned_query_mem);
	COPY_SCALAR_FIELD(planned_work_mem);
	COPY_SCALAR_FIELD(fragmentNum);
#endif

	/* begin ora_compatible */
	COPY_NODE_FIELD(multi_dist);
	COPY_SCALAR_FIELD(p_startno);
	COPY_SCALAR_FIELD(p_endno);
	COPY_SCALAR_FIELD(p_has_else);
	COPY_SCALAR_FIELD(p_has_val_func);
	/* end ora_compatible */
	COPY_SCALAR_FIELD(queryid.timestamp_nodeid);
	COPY_SCALAR_FIELD(queryid.sequence);
	if (from->guc_str)
	{
		newnode->guc_str = makeStringInfo();
		copyStringInfo(newnode->guc_str, from->guc_str);
	}
	return newnode;
}
...
CopyPlanFields(const Plan *from, Plan *newnode)
{
	...
	COPY_NODE_FIELD(exec_subplan);
	COPY_BITMAPSET_FIELD(extParam);
	COPY_BITMAPSET_FIELD(allParam);
#ifdef __OPENTENBASE_C__
	COPY_SCALAR_FIELD(parallel_num);
	COPY_SCALAR_FIELD(is_subplan);
	COPY_SCALAR_FIELD(operator_mem);
#endif
#ifdef __OPENTENBASE__
	COPY_SCALAR_FIELD(remote_flag);
#endif
#ifdef __AUDIT_FGA__
...
_copyModifyTable(const ModifyTable *from)
{
	...
	COPY_SCALAR_FIELD(update_gindex_key);
#endif
	COPY_SCALAR_FIELD(is_submod);	/* ora_compatible */
 ...
 */
static PartIterator *
_copyPartIterator(const PartIterator *from)
{
	 ...
	 * copy remainder of node
	 */
	COPY_SCALAR_FIELD(leafNum);
	COPY_SCALAR_FIELD(direction);
	COPY_SCALAR_FIELD(partParamno);
...
CopyScanFields(const Scan *from, Scan *newnode)
{
	...
	COPY_NODE_FIELD(partition_leaf_rels);
	COPY_NODE_FIELD(partition_leaf_rels_idx);
	COPY_SCALAR_FIELD(partIterParamno);
...
_copyTidScan(const TidScan *from)
{
	...
	/*
	 * copy fields for remote tid scan
	 */
	COPY_SCALAR_FIELD(remote);
	COPY_NODE_FIELD(remote_targets);
	COPY_STRING_FIELD(remote_sql);
	COPY_NODE_FIELD(remote_indexqual);
	COPY_NODE_FIELD(remote_qual);
...
CopyJoinFields(const Join *from, Join *newnode)
{
	...
	COPY_SCALAR_FIELD(plannerinfo_id);
}
...
_copyHashJoin(const HashJoin *from)
{
	...
	COPY_SCALAR_FIELD(nonequijoin);

	COPY_SCALAR_FIELD(num_batches);
	COPY_SCALAR_FIELD(num_buckets);
	COPY_SCALAR_FIELD(innerbucketsize);
	COPY_SCALAR_FIELD(hashjointuples);
...
_copyGroup(const Group *from)
{
	...
	COPY_SCALAR_FIELD(plannerinfo_seq_no);
	COPY_SCALAR_FIELD(agg_type);
...
#ifdef __OPENTENBASE_C__
	COPY_SCALAR_FIELD(groupingFunc);
#endif
	COPY_SCALAR_FIELD(plannerinfo_seq_no);
	COPY_SCALAR_FIELD(agg_type);
...
_copyWindowAgg(const WindowAgg *from)
{
	...
	COPY_SCALAR_FIELD(start_contain_vars);
	COPY_SCALAR_FIELD(end_contain_vars);
...
_copyRemoteQuery(const RemoteQuery *from)
{
	...
	COPY_SCALAR_FIELD(exec_direct_dn_allow);
	...
	COPY_NODE_FIELD(remote_mt);
	COPY_NODE_FIELD(remote_target);
	COPY_NODE_FIELD(remote_quals);
	COPY_SCALAR_FIELD(rcmdtype);
	COPY_SCALAR_FIELD(ignore_tuple_desc);
	COPY_SCALAR_FIELD(fqs_mode);
#endif
	return newnode;
}
...
_copyExecNodes(const ExecNodes *from)
{
	...
	{
		int i = 0;
		newnode->dis_exprs = (Expr **)palloc(sizeof(Expr *) * newnode->nExprs);
		for (i = 0; i < newnode->nExprs; i++)
		{
			newnode->dis_exprs[i] = copyObject(from->dis_exprs[i]);
		}

		if (from->en_param_expr)
		{
			newnode->en_param_expr = (Expr **)palloc(sizeof(Expr *) * newnode->nExprs);
			for (i = 0; i < newnode->nExprs; i++)
			{
				newnode->en_param_expr[i] = copyObject(from->en_param_expr[i]);
			}
		}
	}
#endif
	...
	COPY_STRING_FIELD(g_index_table_name);
	COPY_NODE_FIELD(dist_attno);
...
_copyRemoteSubplan(const RemoteSubplan *from)
{
...
#ifdef __OPENTENBASE_C__
	COPY_SCALAR_FIELD(fid);
	COPY_SCALAR_FIELD(param_fid);
	COPY_SCALAR_FIELD(flevel);
	COPY_NODE_FIELD(targetNodes);
	COPY_SCALAR_FIELD(targetNodeType);
	COPY_SCALAR_FIELD(protocol);
	COPY_SCALAR_FIELD(localSend);
	COPY_SCALAR_FIELD(under_subplan);
#endif
	/*
	 * copy remainder of node
	 */
...
#ifdef __OPENTENBASE_C__
	COPY_SCALAR_FIELD(ndiskeys);
	if (newnode->ndiskeys > 0)
	{
		newnode->diskeys = (AttrNumber *)palloc(sizeof(AttrNumber) * newnode->ndiskeys);
		memcpy(newnode->diskeys, from->diskeys, sizeof(AttrNumber) * newnode->ndiskeys);
	}
#endif
	...
	COPY_NODE_FIELD(sort);
	COPY_STRING_FIELD(cursor);
	COPY_SCALAR_FIELD(roundrobin_distributed);
	COPY_SCALAR_FIELD(roundrobin_replicate);
	COPY_NODE_FIELD(retain_list);
	COPY_NODE_FIELD(replicated_list);
#ifdef __OPENTENBASE__
	COPY_SCALAR_FIELD(num_workers);
	COPY_SCALAR_FIELD(num_virtualdop);
	COPY_SCALAR_FIELD(cacheSend);
	COPY_SCALAR_FIELD(transfer_datarow);
	COPY_BITMAPSET_FIELD(initParam);
	COPY_NODE_FIELD(dests);
#endif

	/* begin ora_compatible */
	COPY_NODE_FIELD(multi_dist);
	COPY_SCALAR_FIELD(startno);
	COPY_SCALAR_FIELD(endno);
	COPY_SCALAR_FIELD(has_else);
	COPY_SCALAR_FIELD(only_one_send_tuple);
	/* end ora_compatible */
	return newnode;
}
...
_copyDistribution(const Distribution *from)
{
	...
	COPY_SCALAR_FIELD(distributionType);
	COPY_SCALAR_FIELD(replication_for_update);
	COPY_SCALAR_FIELD(replication_for_limit);
#ifdef __OPENTENBASE_C__
	COPY_SCALAR_FIELD(nExprs);
	if (newnode->nExprs > 0)
	{
		int i = 0;
		newnode->disExprs = (Node **)palloc(sizeof(Node *) * newnode->nExprs);
		for (i = 0; i < newnode->nExprs; i++)
		{
			newnode->disExprs[i] = copyObject(from->disExprs[i]);
		}
	}
#endif
...
#endif
#ifdef _PG_ORCL_
	COPY_STRING_FIELD(dblinkname);
	COPY_STRING_FIELD(childtablename);
#endif
...
_copyVar(const Var *from)
{
	...
	COPY_LOCATION_FIELD(location);
#ifdef __OPENTENBASE_C__
	COPY_SCALAR_FIELD(varrelid);
	COPY_SCALAR_FIELD(varrelcolid);
	COPY_SCALAR_FIELD(varseq);
#endif
...
_copyParam(const Param *from)
{
	...
	COPY_LOCATION_FIELD(location);
	COPY_NODE_FIELD(paramindextype);
...
#endif /* PGXC */
#ifdef __OPENTENBASE_C__
	COPY_NODE_FIELD(distinct_args);
	COPY_SCALAR_FIELD(distinct_num);
#endif
...
_copyGroupingFunc(const GroupingFunc *from)
{
	...
	COPY_LOCATION_FIELD(location);
#ifdef __OPENTENBASE_C__
	COPY_SCALAR_FIELD(groupingSets);
#endif
...
_copyFuncExpr(const FuncExpr *from)
{
	...
	COPY_NODE_FIELD(args);
#ifdef _PG_ORCL_
	COPY_NODE_FIELD(withfuncnsp);
	COPY_SCALAR_FIELD(withfuncid);
#endif
	COPY_LOCATION_FIELD(location);
...
_copyRestrictInfo(const RestrictInfo *from)
{
	...
	COPY_NODE_FIELD(retain_value_left);
	COPY_NODE_FIELD(retain_value_right);
...
#endif
	COPY_SCALAR_FIELD(plannerinfo_seq_no);
	COPY_SCALAR_FIELD(rte_array_id);
	COPY_SCALAR_FIELD(parent_plannerinfo_seq_no);
	COPY_SCALAR_FIELD(parent_rte_array_id);
	return newnode;
}
...
_copyAConst(const A_Const *from)
{
	...
	/* Lower ident is used to set search path parse in opentenbase_ora mode and just used in memory */
	COPY_SCALAR_FIELD(lower_ident);
	COPY_LOCATION_FIELD(location);
...
_copyFuncCall(const FuncCall *from)
{
	...
	COPY_LOCATION_FIELD(location);
	COPY_NODE_FIELD(return_columnref);
...
_copyCopyFormatter(const CopyFormatter *from)
{
	CopyFormatter *newnode = makeNode(CopyFormatter);

	COPY_NODE_FIELD(fixed_attrs);
	COPY_SCALAR_FIELD(with_cut);

	return newnode;
}

static CopyAttrFixed *
_copyCopyAttrFixed(const CopyAttrFixed *from)
{
	CopyAttrFixed *newnode = makeNode(CopyAttrFixed);

	COPY_STRING_FIELD(attname);
	COPY_SCALAR_FIELD(fix_len);
	COPY_SCALAR_FIELD(attnum);
	COPY_SCALAR_FIELD(align);
	COPY_SCALAR_FIELD(with_cut);
...
_copyMergeAction(const MergeAction* from)
{
	...
	COPY_SCALAR_FIELD(actionOnly);
	COPY_NODE_FIELD(deleteAction);
...
_copyInsertStmt(const InsertStmt *from)
{
	...
	COPY_SCALAR_FIELD(is_preparestmt);
#endif
...
#ifdef _PG_ORCL_
	COPY_NODE_FIELD(withFuncClause);
#endif
	return newnode;
}
...
_copyCopyStmt(const CopyStmt *from)
{
	...
	COPY_NODE_FIELD(attinfolist);
	COPY_SCALAR_FIELD(fixed_position);
	...
	newnode->data_list = (char ***) palloc(sizeof(char **) * from->ndatarows);
	for (row = 0; row < from->ndatarows; row++)
	{
		newnode->data_list[row] = (char **) palloc(sizeof(char *) * from->ncolumns);
		for (column = 0; column < from->ncolumns; column++)
		{
			if (from->data_list[row][column])
				newnode->data_list[row][column] = pstrdup(from->data_list[row][column]);
			else
				newnode->data_list[row][column] = NULL;
		}
	}
	COPY_SCALAR_FIELD(ncolumns);
...
static AttInfo *
_copyCopyAttrInfo(const AttInfo *from)
{
	AttInfo *newnode = makeNode(AttInfo);
	COPY_NODE_FIELD(attname);
	COPY_NODE_FIELD(attrpos);
	COPY_NODE_FIELD(custom_expr);
	return newnode;
}

static Position *
_copyCopyAttrPos(const Position *from)
{
	Position *newnode = makeNode(Position);
	COPY_SCALAR_FIELD(flag);
	COPY_SCALAR_FIELD(start_offset);
	COPY_SCALAR_FIELD(end_offset);
	return newnode;
}
...
#endif
#ifdef _PG_ORCL_
	COPY_SCALAR_FIELD(istransformed);
	COPY_NODE_FIELD(merge_partspec);
#endif
}
...
_copyDropStmt(const DropStmt *from)
{
	...
	COPY_NODE_FIELD(parent_r);
	COPY_SCALAR_FIELD(drop_subpart);
...
_copyTruncateStmt(const TruncateStmt *from)
{
	...
	COPY_NODE_FIELD(paretablename);
	COPY_NODE_FIELD(relations);
	COPY_SCALAR_FIELD(restart_seqs);
	COPY_SCALAR_FIELD(behavior);
	COPY_SCALAR_FIELD(sc_id);
	COPY_SCALAR_FIELD(trunc_subpart);
...
static CreateTypeObjectStmt *
_copyCreateTypeObjectStmt(const CreateTypeObjectStmt *from)
{
    CreateTypeObjectStmt *newnode = makeNode(CreateTypeObjectStmt);

    COPY_NODE_FIELD(typevar);
    COPY_NODE_FIELD(coldeflist);
    COPY_SCALAR_FIELD(replace);
	COPY_STRING_FIELD(objheadersrc);
	COPY_NODE_FIELD(options);
	COPY_NODE_FIELD(objfunclist);
	COPY_STRING_FIELD(func_spec);
	COPY_SCALAR_FIELD(reparse_spec);
	COPY_SCALAR_FIELD(has_cons);
	COPY_SCALAR_FIELD(force);
	COPY_SCALAR_FIELD(check_dup);

    return newnode;
}

static CreateTypeObjectBodyStmt *
_copyCreateTypeObjectBodyStmt(const CreateTypeObjectBodyStmt *from)
{
    CreateTypeObjectBodyStmt *newnode = makeNode(CreateTypeObjectBodyStmt);

    COPY_NODE_FIELD(typevar);
    COPY_SCALAR_FIELD(replace);
	COPY_STRING_FIELD(objheadersrc);
	COPY_STRING_FIELD(objbodysrc);
	COPY_NODE_FIELD(options);
	COPY_NODE_FIELD(objfunclist);
	COPY_NODE_FIELD(objtypelist);
	COPY_STRING_FIELD(objfundef);
	COPY_SCALAR_FIELD(reparse_body);
	COPY_SCALAR_FIELD(force);

    return newnode;
}

static CreateNestedTableStmt *
_copyCreateNestedTableStmt(const CreateNestedTableStmt *from)
{
    CreateNestedTableStmt *newnode = makeNode(CreateNestedTableStmt);

    COPY_NODE_FIELD(typevar);
    COPY_NODE_FIELD(subtypename);
    COPY_SCALAR_FIELD(replace);
...
static CreateDirStmt *
_copyCreateDirStmt(const CreateDirStmt *from)
{
	CreateDirStmt *newnode = makeNode(CreateDirStmt);

	COPY_STRING_FIELD(dirname);
	COPY_STRING_FIELD(dirpath);
...
static DropDirStmt *
_copyDropDirStmt(const DropDirStmt *from)
{
	DropDirStmt *newnode = makeNode(DropDirStmt);
...
#ifdef _PG_ORCL_
	COPY_SCALAR_FIELD(withproc);
	COPY_SCALAR_FIELD(replace);
	COPY_NODE_FIELD(ddlevents);
...
_copyPartitionDef(const PartitionDef *from)
{
	...
	COPY_NODE_FIELD(part_rv);
	COPY_NODE_FIELD(bound_spec);
	COPY_NODE_FIELD(subpart_defs);
...
_copyPartitionInto(const PartitionInto *from)
{
	PartitionInto	*newnode = makeNode(PartitionInto);

	COPY_NODE_FIELD(part_name);
	COPY_STRING_FIELD(part_tablespace);

	return newnode;
}

static PartitionMergeSplit *
_copyPartitionMergeSplit(const PartitionMergeSplit *from)
{
	PartitionMergeSplit	*newnode = makeNode(PartitionMergeSplit);

	COPY_NODE_FIELD(part_ext_names);
	COPY_NODE_FIELD(into_parts);
	COPY_SCALAR_FIELD(stp);
	COPY_NODE_FIELD(from_rel);
	COPY_SCALAR_FIELD(is_merge);
	COPY_SCALAR_FIELD(is_partition);
	COPY_NODE_FIELD(bound);
...
_copyPartitionMergeSpec(const PartitionMergeSpec *from)
{
	PartitionMergeSpec	*newnode = makeNode(PartitionMergeSpec);

	COPY_SCALAR_FIELD(strategy);
	COPY_SCALAR_FIELD(partnatts);

	if (from->partnatts)
	{
		COPY_POINTER_FIELD(partopclass, sizeof(Oid) * from->partnatts);
		COPY_POINTER_FIELD(partcollation, sizeof(Oid) * from->partnatts);
		COPY_POINTER_FIELD(partattrs, sizeof(AttrNumber) * from->partnatts);
	}

	COPY_NODE_FIELD(partexprs);

	return newnode;
}

static PauseTransactionStmt *
_copyPauseTransactionStmt(const PauseTransactionStmt *from)
{
	PauseTransactionStmt	*newnode = makeNode(PauseTransactionStmt);

	COPY_STRING_FIELD(xact_name);
	COPY_SCALAR_FIELD(pause);
...
static DisKeyAttr *
_copyDisKeyAttr(const DisKeyAttr *from)
{
	int i;
	DisKeyAttr *new = makeNode(DisKeyAttr);

	Assert(from->nDisAttrs);
	new->nDisAttrs = from->nDisAttrs;
	new->disAttrNums = (AttrNumber *) palloc0(sizeof(AttrNumber) * from->nDisAttrs);
	new->disAttrTypes = (Oid *) palloc0(sizeof(Oid) * from->nDisAttrs);
	new->disAttrTypMods = (int32 *) palloc0(sizeof(int32) * from->nDisAttrs);

	for (i = 0; i < from->nDisAttrs; i++)
	{
		new->disAttrNums[i] = from->disAttrNums[i];
		new->disAttrTypes[i] = from->disAttrTypes[i];
		new->disAttrTypMods[i] = from->disAttrTypMods[i];
	}
...
static CreateSynonymStmt *
_copyCreateSynonymStmt(const CreateSynonymStmt *from)
{
	...
	COPY_STRING_FIELD(dblink);

	return newnode;
}

static AlterPackageStmt *
_copyAlterPackageStmt(const AlterPackageStmt *from)
{
	AlterPackageStmt	*newnode = makeNode(AlterPackageStmt);

	COPY_NODE_FIELD(pkgname);
	COPY_SCALAR_FIELD(debug);
	COPY_SCALAR_FIELD(pkg_part);
	COPY_NODE_FIELD(comp_items);
	COPY_SCALAR_FIELD(reuse_settings);
	COPY_SCALAR_FIELD(edition);

	return newnode;
}

static CreatePackageStmt *
_copyCreatePackageStmt(const CreatePackageStmt *from)
{
	CreatePackageStmt *newnode = makeNode(CreatePackageStmt);

	COPY_SCALAR_FIELD(replace);
	COPY_SCALAR_FIELD(edition);
	COPY_SCALAR_FIELD(body);

	COPY_NODE_FIELD(pkgname);
	COPY_STRING_FIELD(pkgheadersrc);
	COPY_NODE_FIELD(options);
	COPY_NODE_FIELD(pkgfunclist);
	COPY_NODE_FIELD(pkgtypelist);
	COPY_NODE_FIELD(pkgvarlist);
	COPY_STRING_FIELD(func_spec);
	COPY_SCALAR_FIELD(reparse_spec);
...
static CreatePackageBodyStmt *
_copyCreatePackageBodyStmt(const CreatePackageBodyStmt *from)
{
	...
	COPY_STRING_FIELD(priv_decl);
	COPY_STRING_FIELD(func_def);
	COPY_STRING_FIELD(initial_part);
	COPY_SCALAR_FIELD(reparse_body);

	return newnode;
}

static DropPackageStmt *
_copyDropPackageStmt(const DropPackageStmt *from)
{
	DropPackageStmt *newnode = makeNode(DropPackageStmt);
...
static AlterResourceCostStmt*
_copyAlterResourceCostStmt(const AlterResourceCostStmt *from)
{
       AlterResourceCostStmt *newnode = makeNode(AlterResourceCostStmt);

       COPY_NODE_FIELD(resource_args);

       return newnode;
}
static ProfileParameter*
_copyProfileParameter(const ProfileParameter *from)
{
       ProfileParameter *newnode = makeNode(ProfileParameter);
...
_copyCreateResourceQueueStmt(const CreateResourceQueueStmt *from)
{
	CreateResourceQueueStmt *newnode = makeNode(CreateResourceQueueStmt);
...
_copyAlterResourceQueueStmt(const AlterResourceQueueStmt *from)
{
	AlterResourceQueueStmt *newnode = makeNode(AlterResourceQueueStmt);
...
_copyDropResourceQueueStmt(const DropResourceQueueStmt *from)
{
	DropResourceQueueStmt *newnode = makeNode(DropResourceQueueStmt);
...
static CreateDatabaseLinkStmt *
_copyCreateDatabaseLinkStmt(const CreateDatabaseLinkStmt *from)
{
    CreateDatabaseLinkStmt *newnode = makeNode(CreateDatabaseLinkStmt);

    COPY_SCALAR_FIELD(dblinkkind);
    COPY_STRING_FIELD(name);
    COPY_SCALAR_FIELD(iscurrentuser);
    COPY_STRING_FIELD(username);
    COPY_STRING_FIELD(passwd);
    COPY_STRING_FIELD(connstr);

    return newnode;
}

static DropDatabaseLinkStmt *
_copyDropDatabaseLinkStmt(const DropDatabaseLinkStmt *from)
{
    DropDatabaseLinkStmt *newnode = makeNode(DropDatabaseLinkStmt);
...
static RemoteModifyTable *
_copyRemoteModifyTable(const RemoteModifyTable *from)
{
	RemoteModifyTable *newnode = makeNode(RemoteModifyTable);
	
	CopyPlanFields((const Plan *) from, (Plan *) newnode);
	
	COPY_SCALAR_FIELD(operation);
	COPY_NODE_FIELD(returningLists);
	COPY_NODE_FIELD(dist_update_inserts);
	COPY_NODE_FIELD(dist_update_deletes);
	COPY_NODE_FIELD(dist_update_updates);
	COPY_NODE_FIELD(dist_update_locks);
...
_copyMergeQualProj(const MergeQualProj *from)
{
	MergeQualProj *newnode = makeNode(MergeQualProj);
...
_copyFdwDataNodeTask(const FdwDataNodeTask* from)
{
    FdwDataNodeTask* newnode = makeNode(FdwDataNodeTask);

    COPY_STRING_FIELD(dnName);
	COPY_STRING_FIELD(instant_time);
...
_copyFdwFileSegment(const FdwFileSegment* from)
{
    FdwFileSegment* newnode = makeNode(FdwFileSegment);

    COPY_STRING_FIELD(filename);
    COPY_SCALAR_FIELD(begin);
    COPY_SCALAR_FIELD(end);
    COPY_SCALAR_FIELD(fdwfileSize);
	COPY_NODE_FIELD(hudi_delta_log);

	return newnode;
}

static FragmentDest *
_copyFragmentDest(const FragmentDest *from)
{
	FragmentDest *newnode = makeNode(FragmentDest);

	COPY_SCALAR_FIELD(fid);
	COPY_SCALAR_FIELD(recvfid);
	COPY_SCALAR_FIELD(targetNodeType);
	COPY_NODE_FIELD(targetNodes);
...
static QueryTermStmt *
_copyQueryTermStmt(const QueryTermStmt *from)
{
	QueryTermStmt *newnode = makeNode(QueryTermStmt);
	return newnode;
}

static UpdateTermStmt *
_copyUpdateTermStmt(const UpdateTermStmt *from)
{
	UpdateTermStmt *newnode = makeNode(UpdateTermStmt);
	COPY_SCALAR_FIELD(term);
	return newnode;
}

static MultiSetExpr *
_copyMultiSetExpr(const MultiSetExpr *from)
{
	MultiSetExpr *newnode = makeNode(MultiSetExpr);
...
copyObjectImpl(const void *from)
{
	...
	switch (nodeTag(from))
	{
		...
		case T_MergeQualProj:
			retval = _copyMergeQualProj(from);
			break;
#ifdef PGXC
			/*
			 * PGXC SPECIFIC NODES
		...
		case T_RemoteModifyTable:
			retval = _copyRemoteModifyTable(from);
			break;
        case T_MergeStmt:
		...
		case T_CreateDirStmt:
			retval = _copyCreateDirStmt(from);
			break;
		...
		case T_QueryTermStmt:
			retval = _copyQueryTermStmt(from);
			break;
		case T_UpdateTermStmt:
			retval = _copyUpdateTermStmt(from);
			break;
		case T_DropNodeStmt:
		...
		case T_PartitionInto:
			retval = _copyPartitionInto(from);
			break;
		case T_PartitionMergeSplit:
			retval = _copyPartitionMergeSplit(from);
			break;
		case T_PartitionMergeSpec:
			retval = _copyPartitionMergeSpec(from);
			break;
		case T_PauseTransactionStmt:
			retval = _copyPauseTransactionStmt(from);
			break;
#endif
#ifdef __OPENTENBASE__
		case T_CreateSynonymStmt:
			retval = _copyCreateSynonymStmt(from);
			break;
		case T_CreatePackageStmt:
			retval =_copyCreatePackageStmt(from);
			break;
    	case T_CreatePackageBodyStmt:
			retval =_copyCreatePackageBodyStmt(from);
			break;
    	case T_DropPackageStmt:
			retval =_copyDropPackageStmt(from);
			break;
#endif
		/* opentenbase_ora begin */
		case T_AlterPackageStmt:
			retval = _copyAlterPackageStmt(from);
			break;
		...
		case T_AlterResourceCostStmt:
				retval = _copyAlterResourceCostStmt(from);
				break;
		case T_ProfileParameter:
				retval = _copyProfileParameter(from);
				break;
		case T_CreateTypeObjectStmt:
			retval = _copyCreateTypeObjectStmt(from);
			break;
		case T_CreateTypeObjectBodyStmt:
			retval = _copyCreateTypeObjectBodyStmt(from);
			break;
		case T_CreateNestedTableStmt:
			retval = _copyCreateNestedTableStmt(from);
			break;

	    case T_CreateDatabaseLinkStmt:
            retval = _copyCreateDatabaseLinkStmt(from);
            break;
	    case T_DropDatabaseLinkStmt:
            retval = _copyDropDatabaseLinkStmt(from);
            break;
		...
		case T_SampleStmt:
			...
			case T_CreateResourceQueueStmt:
				retval = _copyCreateResourceQueueStmt(from);
				break;
			case T_AlterResourceQueueStmt:
				retval = _copyAlterResourceQueueStmt(from);
				break;
			case T_DropResourceQueueStmt:
				retval = _copyDropResourceQueueStmt(from);
				break;
#endif
		...
		case T_AttInfo:
			retval = _copyCopyAttrInfo(from);
			break;
		case T_Position:
			retval = _copyCopyAttrPos(from);
			break;
		case T_FdwDataNodeTask:
            retval = _copyFdwDataNodeTask(from);
            break;
        case T_FdwFileSegment:
            retval = _copyFdwFileSegment(from);
            break;
		case T_FragmentDest:
			retval = _copyFragmentDest(from);
			break;
		case T_CopyFormatter:
			retval = _copyCopyFormatter(from);
			break;
		case T_CopyAttrFixed:
			retval = _copyCopyAttrFixed(from);
			break;
		case T_MultiSetExpr:
			retval = _copyMultiSetExpr(from);
			break;
		case T_PartitionDef:
