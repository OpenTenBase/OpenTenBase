...
LLVMTypeRef StructVectorTableSlot;
LLVMTypeRef StructVectorDatumData;
LLVMTypeRef StructPlanState;
LLVMTypeRef StructHashJoinState;
LLVMTypeRef StructHashJoinTableData;
LLVMTypeRef StructHashJoinTupleData;
LLVMTypeRef StructHashJoinTableBuckets;
LLVMTypeRef StructHashJoinTupleNext;
#endif
...
#ifdef __OPENTENBASE_C__
LLVMValueRef FuncLLVMDebug;
LLVMValueRef FuncExecParallelHashFirstTuple;
LLVMValueRef FuncExecParallelHashNextTuple;
#endif

extern void llvm_debug_func(Datum arg1, Datum arg2);
...
#ifdef __OPENTENBASE_C__
	cb->compile_hashjoin = llvm_compile_hashjoin;
#endif
}
...
llvm_create_types(void)
{
	...
	StructMinimalTupleData = load_type(mod, "StructMinimalTupleData");
...
#ifdef __OPENTENBASE_C__
	StructHeapTupleHeaderData = load_type(mod, "StructHeapTupleHeaderData");
	StructVectorTableSlot = load_type(mod, "StructVectorTableSlot");
	StructVectorDatumData = load_type(mod, "StructVectorDatumData");
	StructPlanState = load_type(mod, "StructPlanState");
	StructHashJoinState = load_type(mod, "StructHashJoinState");
	StructHashJoinTableData = load_type(mod, "StructHashJoinTableData");
	StructHashJoinTupleData = load_type(mod, "StructHashJoinTupleData");
	StructHashJoinTableBuckets = load_type(mod, "StructHashJoinTableBuckets");
	StructHashJoinTupleNext = load_type(mod, "StructHashJoinTupleNext");
#endif
...
#ifdef __OPENTENBASE_C__
	FuncLLVMDebug = LLVMGetNamedFunction(mod, "llvm_debug_func");
	FuncExecParallelHashFirstTuple = LLVMGetNamedFunction(mod, "ExecParallelHashFirstTuple");
	FuncExecParallelHashNextTuple = LLVMGetNamedFunction(mod, "ExecParallelHashNextTuple");
#endif
...
#ifdef __OPENTENBASE_C__
LLVMValueRef
llvm_crc_reference(LLVMJitContext *context)
{
	char	   *funcname;

	LLVMModuleRef mod;
	LLVMTypeRef deform_sig;
	LLVMValueRef v_crc_fn;

	mod = llvm_mutable_module(context);

#if defined(__aarch64__) || defined(__aarch64)
	funcname = "llvm.aarch64.crc32cx";
	{
		LLVMTypeRef param_types[2];

		param_types[0] = LLVMInt32Type();
		param_types[1] = LLVMInt64Type();
		deform_sig = LLVMFunctionType(LLVMInt32Type(), param_types,
									  lengthof(param_types), 0);
	}
#else
	funcname = "llvm.x86.sse42.crc32.64.64";
	{
		LLVMTypeRef param_types[2];

		param_types[0] = LLVMInt64Type();
		param_types[1] = LLVMInt64Type();
		deform_sig = LLVMFunctionType(LLVMInt64Type(), param_types,
									  lengthof(param_types), 0);
	}
#endif

	v_crc_fn = LLVMAddFunction(mod, funcname, deform_sig);
 ...
 */
void
llvm_debug_func(Datum arg1, Datum arg2)
{
	bool wait = true;
