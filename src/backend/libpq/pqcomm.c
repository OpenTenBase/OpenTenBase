...
is_disconnection(Port *port)
{
	if (port)
	{
		...
		/* check sock */
		if (getsockopt(port->sock, IPPROTO_TCP, TCP_INFO, &info, (socklen_t *)&len) < 0)
		{
			return true;
		}
 ...
 */
void
SetSockKeepAlive(int sock, int keepalives_interval, int keepalives_count, int keepalives_idle)
{
	int keepalive = 1;
	/* user_timeout in ms */
	...
	if (tcp_user_timeout == 0)
	{
		if (keepalives_idle > 0)
		{
			 ...
			 */
			if (keepalives_count >= 1)
				time_offset = (int)(((float)keepalives_count - 0.5) * keepalives_interval);
			else
				time_offset = 1;

			user_timeout = time_offset + keepalives_idle;
