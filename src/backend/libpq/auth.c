...
#ifdef _PG_ORCL_
	if ((profile_password_limit || profile_resource_limit) && port->user_name)
	{
		if (IsBootstrapProcessingMode() || IsAutoVacuumWorkerProcess() ||
			!IsUnderPostmaster || IsBackgroundWorker || am_walsender)
		{
			/* ... */
		}
		else
		{
			GetMyProfile(port->user_name);
			GetMyProfCostWeight();
		}
	}
#endif
...
#define HOSTNAME_LOOKUP_DETAIL(port) \
	...
	if (ClientAuthentication_hook)
       ...
       if (profile_password_limit && MyProfile && IS_PGXC_LOCAL_COORDINATOR)
       {
               /* on error, check before auth_failed */
               if(status == STATUS_ERROR)
               {
                       ProfileAuthentication(status, port->user_name);
               }
       }
#endif
	...
	{
		/*
		 * on success, we may send warning message to frontent, frontent 
		 * wait for authentication request now, so check after sendAuthRequest
	     */
		if(status == STATUS_OK)
		{
			ProfileAuthentication(status, port->user_name);
    ...
    /* profile sessions auth */
    if (IS_PGXC_LOCAL_COORDINATOR && profile_resource_limit && MyProfile)
    {
        if (MyProfile->sessions != 0 && CountUserBackends(get_role_oid(port->user_name, false)) > MyProfile->sessions)
            ereport(FATAL,
                    (errcode(ERRCODE_TOO_MANY_CONNECTIONS),
                            errmsg("exceeded simultaneous SESSIONS_PER_USER limit")));
    }
#endif
}
