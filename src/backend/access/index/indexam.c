...
index_endscan(IndexScanDesc scan)
{
	...
	if(enable_distri_debug)
	{
		if(scan->heapRelation && scan->xs_snapshot)
		{
			...
			elog(LOG, "[index_endscan] xid:%u, start ts " UINT64_FORMAT ", rel :%s, "
				...
				GetTopTransactionIdIfAny(), scan->xs_snapshot->start_ts, RelationGetRelationName(scan->heapRelation),
				scan->xs_scan_number, scan->xs_snapshot->number_visible_tuples,
				scan->xs_snapshot->scanned_tuples_before_prepare, scan->xs_snapshot->scanned_tuples_after_prepare,
				scan->xs_snapshot->scanned_tuples_after_committed, scan->xs_snapshot->scanned_tuples_after_abort);
		}
 ...
 */
HeapTuple
index_getnext(IndexScanDesc scan, ScanDirection direction, void* plan_node)
{
	HeapTuple	heapTuple;
	ItemPointer tid;
