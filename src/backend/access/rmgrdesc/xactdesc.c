...
ParseCommitRecord(uint8 info, xl_xact_commit *xlrec, xl_xact_parsed_commit *parsed)
{
	...
	if (parsed->xinfo & XACT_XINFO_HAS_RELFILENODES)
	{
		...
		data += xl_relfilenodes->nrels * sizeof(RelFileNodeNFork);
	}
	...
	{
	    TransactionId xid;
	    memcpy(&xid, data, sizeof(TransactionId));
	    parsed->oldest_active_xid = xid;
...
xact_desc_commit(StringInfo buf, uint8 info, xl_xact_commit *xlrec, RepOriginId origin_id)
{
    ...
    appendStringInfo(buf, ", gts: "UINT64_FORMAT, xlrec->gts);
	...
	if (parsed.nrels > 0)
	{
		...
		{
			char *path = relpathperm_client(parsed.xnodes[i].rnode, MAIN_FORKNUM, "");

#ifdef __OPENTENBASE_C__
			appendStringInfo(buf, " %s %u:%u:%u ,",
								path,
								parsed.xnodes[i].rnode.spcNode,
								parsed.xnodes[i].rnode.dbNode,
								parsed.xnodes[i].rnode.relNode);
#else
	...
	if (parsed.xinfo & XACT_XINFO_HAS_OLDEST_ACTIVE_XID)
	{
	    appendStringInfo(buf, "; oldest active xid: %u", parsed.oldest_active_xid);
	}
}
