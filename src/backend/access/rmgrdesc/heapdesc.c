...
heap2_desc(StringInfo buf, XLogReaderState *record)
{
	...
	if (info == XLOG_HEAP2_CLEAN)
	{
		...
		appendStringInfo(buf, "remxid %u prunets %lu hasdata %d", xlrec->latestRemovedXid, 
                         xlrec->pruneTs, xlrec->hasData);
		...
		if (need_prune)
		{
			...
			for (i = 0; i < nredirected; i ++)
			{
				...
				appendStringInfo(buf, " %u", redirected[i]);
			}
			for (i = 0; i < ndead; i ++)
			{
				if (i == 0)
				{	
					appendStringInfo(buf, " nowdead: ");
				}
				appendStringInfo(buf, " %u", nowdead[i]);
			}
			...
			for (i = 0; i < nunused; i ++)
			{
				if (i == 0)
				{	
					appendStringInfo(buf, " nowunused: ");
				}
				appendStringInfo(buf, " %u", nowunused[i]);
			}
		}
		else
		{
			appendStringInfo(buf, " has no data");
		}
	...
	else if (info == XLOG_HEAP2_FREEZE_PAGE)
	{
		...
		if (!XLogRecHasBlockImage(record, 0) && xlrec->ntuples > 0)
		{
			...
			for (ntup = 0; ntup < xlrec->ntuples; ntup++)
			{
				...
				appendStringInfo(buf, " %u ", xlrec_tp->offset);
			}
