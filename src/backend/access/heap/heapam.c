...
/* inplace upgrade */
Oid			inplace_upgrade_next_general_oid = InvalidOid;
...
#ifdef __OPENTENBASE__
	/* should use mvcc instead of visibility map */
	all_visible = false;
#endif
...
relation_open(Oid relationId, LOCKMODE lockmode)
{
	...
	if (isTempNamespace(RelationGetNamespace(r)))
	{
		MyXactFlags |= XACT_FLAGS_ACCESSEDTEMPREL;
		record_acitve_temp_table(relationId);
	}
...
heap_endscan(HeapScanDesc scan)
{
	...
	if(enable_distri_debug)
	{
		...
		elog(LOG, "[heap_endscan] xid:%u, start ts " UINT64_FORMAT ", rel :%s, "
			"rs_scan_number:" INT64_FORMAT "number_visible_tuples:" INT64_FORMAT
			"scanned_tuples_before_prepare:" INT64_FORMAT "scanned_tuples_after_prepare:" INT64_FORMAT
			"scanned_tuples_after_committed:" INT64_FORMAT "scanned_tuples_after_abort:" INT64_FORMAT,
			GetTopTransactionIdIfAny(), scan->rs_snapshot->start_ts, RelationGetRelationName(scan->rs_rd),
			scan->rs_scan_number, scan->rs_snapshot->number_visible_tuples,
			scan->rs_snapshot->scanned_tuples_before_prepare, scan->rs_snapshot->scanned_tuples_after_prepare,
			scan->rs_snapshot->scanned_tuples_after_committed, scan->rs_snapshot->scanned_tuples_after_abort);
	}
#endif
	/*
	 * unpin scan buffers
							 ...
							 Snapshot snapshot)
{
	...
	target->phs_syncscan = synchronize_seqscans && (target->phs_nblocks > NBuffers / 4);
	SpinLockInit(&target->phs_mutex);
	target->phs_startblock = InvalidBlockNumber;
	pg_atomic_init_u64(&target->phs_nallocated, 0);
	pg_atomic_init_flag(&target->phs_utilflag);
...
heap_parallelscan_reinitialize(ParallelHeapScanDesc parallel_scan)
{
	...
	pg_atomic_clear_flag(&parallel_scan->phs_utilflag);
}
...
heap_parallelscan_startblock_init(HeapScanDesc scan)
{
	...
	pbscanwork = scan->rs_parallelworkerdata;

	/* Reset the state we use for controlling allocation size. */
	memset(pbscanwork, 0, sizeof(ParallelBlockTableScanWorkerData));
	 ...
	 */
	pbscanwork->phsw_chunk_size = pg_nextpower2_32(Max(parallel_scan->phs_nblocks /
...
heap_parallelscan_nextpage(HeapScanDesc scan)
{
	...
	else
	{
		...
		if (pbscanwork->phsw_chunk_size > 1 &&
			pbscanwork->phsw_nallocated > parallel_scan->phs_nblocks -
			(pbscanwork->phsw_chunk_size * PARALLEL_SEQSCAN_RAMPDOWN_CHUNKS))
			pbscanwork->phsw_chunk_size >>= 1;

		nallocated = pbscanwork->phsw_nallocated =
			pg_atomic_fetch_add_u64(&parallel_scan->phs_nallocated,
					   ...
					   bool *all_dead, bool first_call)
{
	...
	bool		valid;
	bool		skip;
	bool		kill_on_transaction = false;

	/* If this is not the first call, previous call returned a (live!) tuple */
	if (all_dead)
		*all_dead = first_call;

	/* Only statement scanning this relation */
	if (relation->rd_firstcid == GetCurrentCommandId(false))
		kill_on_transaction = true;
	...
	for (;;)
	{
		...
		if (all_dead && *all_dead &&
			!HeapTupleIsSurelyDead(heapTuple,
								   GetRecentGlobalXmin(),
								   kill_on_transaction))
		{
			*all_dead = false;
			...
			int options, BulkInsertState bistate)
{
	...
	if (RELATION_IS_META_GLOBAL_TEMP(relation))
		elog(ERROR, "cannot insert data to meta global temp table");
...
#ifdef __OPENTENBASE__
	LightLockCheck(CMD_INSERT, RelationGetRelid(relation), HeapTupleGetShardId(tup), InvalidShardClusterId);
#endif
	/*
	...
	heap_tuple_validate_shardid(relation, heaptup);
...
#ifdef _PG_ORCL_
		xlhdr.t_infomask3 = heaptup->t_data->t_infomask3;
#endif
#ifdef _MIGRATE_
					...
					CommandId cid, int options)
{
		...
		if (trigdesc != NULL)
		{
			...
			ereport(ERROR,
					(errcode(ERRCODE_INVALID_TRANSACTION_STATE),
						errmsg("cannot insert to tables with trigger in a parallel worker")));
		}
	}
	...
	if (relation->rd_rel->relhasoids)
	{
		...
		if (IsInplaceUpgrade && OidIsValid(inplace_upgrade_next_general_oid))
		{
			HeapTupleSetOid(tup, inplace_upgrade_next_general_oid);
			inplace_upgrade_next_general_oid = InvalidOid;
		}
		/*
	...
	HeapTupleHeaderSetXmaxTimestamp(tup->t_data, 0); /* for cleanliness of xmax_gts */
...
#ifdef __OPENTENBASE__
		LightLockCheck(CMD_INSERT, RelationGetRelid(relation), tuple_sid, InvalidShardClusterId);
#endif
...
#ifdef _PG_ORCL_
				tuphdr->t_infomask3 = heaptup->t_data->t_infomask3;
#endif
#ifdef _MIGRATE_
...
#ifdef __OPENTENBASE__
	tup = &tp;
	LightLockCheck(CMD_DELETE, RelationGetRelid(relation), HeapTupleGetShardId(tup), InvalidShardClusterId);
#endif
...
l1:
	...
	if (result == HeapTupleInvisible)
	{
		...
		if (tp.t_data->t_infomask & HEAP_GTSXMAX_COMBOCID)
		{
			elog(WARNING, "heap xmax_gts as combocid");
			heap_combocid = true;
		}
		...
		if(TransactionIdGetCSNAndCommitStat(xmin, &committs, NULL))
		{
        ...
        if (ActiveSnapshotSet())
        {
            ...
            elog(WARNING, "active snapshot cid %d start_ts "INT64_FORMAT " local %d snap xmin %d",
                    active_snap->curcid,
		...
		ereport(ERROR,
				(errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
				 errmsg("attempted to delete invisible tuple in rel %s xid %u xmin %u xmax %u is-current %d transaction status %d "
				 		"cid %d tuple cmin %d cmax %d getcurcid %d "
                        ...
                        TransactionIdGetStatusCSN(xmin), 
				 		cid, 
						HeapTupleHeaderGetCmin(tp.t_data),
						HeapTupleHeaderGetCmax(tp.t_data),
						GetCurrentCommandId(false),
				 		HeapTupleHeaderGetXminTimestampAtomic(tp.t_data),
				 		committs,
				 		xmax_invalid,
				 		locked,
				 		multi,
...
#ifdef _PG_ORCL_
			xlhdr.t_infomask3 = old_key_tuple->t_data->t_infomask3;
#endif
...
static void
CheckPgClassTupleUpdate(Relation rel, HeapTuple newtup, HeapTuple oldtup, ItemId lp)
{
	Oid reloid = HeapTupleGetOid(newtup);
	Oid page_reloid = HeapTupleGetOid(oldtup);

	if (!ItemIdIsNormal(lp))
	{
		/*
		 * Register the tuple obtained from the cache, and trigger a transaction
		 * rollback upon error to clean up the error cache tuple.
		 */
		CacheInvalidateHeapTuple(rel, newtup, NULL);
		ereport(ERROR,
			(errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
			 errmsg("attempted to update invlaid pg_class tuple, line-pointer is not normal")));
	}

	if (OidIsValid(reloid) && reloid != page_reloid)
	{
		/*
		 * Register the tuple obtained from the cache, and trigger a transaction
		 * rollback upon error to clean up the error cache tuple.
		...
		ereport(ERROR,
			(errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
			 errmsg("attempted to update wrong pg_class tuple, catche rel oid is %u, page rel oid is %u",
			 reloid, page_reloid)));
	}
}
			...
			HeapUpdateFailureData *hufd, LockTupleMode *lockmode, HeapTuple oldtup)
{
...
#ifdef __OPENTENBASE__
	LightLockCheck(CMD_UPDATE, RelationGetRelid(relation), HeapTupleGetShardId(newtup), InvalidShardClusterId);
#endif
	...
	/*
	 * Fill in enough data in oldtupData for HeapDetermineModifiedColumns to work
	 * properly.
	 */
	...
	if (relation->rd_id == RelationRelationId && !IsBootstrapProcessingMode())
	{
		CheckPgClassTupleUpdate(relation, newtup, oldtup, lp);
	}

	LightLockCheck(CMD_UPDATE, RelationGetRelid(relation), HeapTupleGetShardId(oldtup), InvalidShardClusterId);
	...
	/*
	 * Note: beyond this point, use oldtupData not otid to refer to old tuple.
...
l2:
	...
	if (result == HeapTupleInvisible)
	{
		...
		{
			elog(WARNING, "xmax is multi");
			xmax = HeapTupleGetUpdateXid(oldtup->t_data);
			multi = true;
		}
		else
		{
		...
		ereport(ERROR,
				(errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
				 errmsg("attempted to update invisible tuple in rel %s xid %u xmin %u xmax %u is-current %d in-progress %d committted %d "
						...
						HeapTupleHeaderGetCmax(oldtup->t_data),
						active_snap->curcid,
						active_snap->start_ts,
						active_snap->local,
						active_snap->xmin,
						GetCurrentCommandId(false),
				 		HeapTupleHeaderGetXminTimestampAtomic(oldtup->t_data),
				 		committs,
				 		xmax_invalid,
				 		locked,
				 		multi,
	 ...
	 * since both tuples are in the same relation and the check for oldtupData
	 ...
	 * have to do this before releasing the buffer because oldtupData is in the
	...
	/* 
	 * old tuple not null, if we release buffer, pointer of t_data will change,
	 * so make a copy is necessary.
	*/
	if (oldtup != &oldtupData && oldtup->t_len != 0)
	{
		void *t_data = palloc(oldtup->t_len);
		memcpy(t_data, oldtup->t_data, oldtup->t_len);
		oldtup->t_data = (HeapTupleHeader)t_data;
	}
...
/*
 * Lock a tuple in shared or exclusive mode before the specified timeout.
 *
 * Input parameters:
 * relation: relation containing tuple (caller must hold suitable lock)
 * tuple->t_self: TID of tuple to lock (rest of struct need not be valid)
 ...
 * mode: indicates if shared or exclusive tuple lock is desired
 * wait_policy: what to do if tuple lock is not available
 * wait_timeout: maximum time to wait lock the tuple
 * follow_updates: if true, follow the update chain to also lock descendant
 * tuples.
 *
 * Output parameters:
 ...
 * HeapTupleWouldBlock: lock couldn't be acquired and wait_policy is skip
 *
 * This is a wrapper function, whose main purpose is to call heap_lock_tuple
 * after setting RowLockTimeout to wait_timeout. The function will be called by
 * ExeclockRows for each row and each row mark, so must reset RowLockTimeout to
 * zero avoid affecting other row mark.
 *
 * See comments for heap_lock_tuple for additional info.
 */
HTSU_Result
heap_lock_tuple_with_timeout(Relation				relation,
							 HeapTuple				tuple,
							 CommandId				cid,
							 LockTupleMode			mode,
							 ...
							 HeapUpdateFailureData *hufd)
{
	...
	{
		RelationLocInfo *rd_locator_info = relation->rd_locator_info;
		if (rd_locator_info && (IsRelationDistributedByValue(rd_locator_info) ||
								IsRelationReplicated(rd_locator_info)))
		{
			elog(ERROR, "locking tuple is not supported in coordinator");
		}
	}
...
l5:
	...
	{
		TransactionIdStatus xidstatus = TransactionIdGetStatusCSN(xmax);
						   ...
						   LockTupleMode mode, bool *needwait)
{
	...
	*needwait = false;
	...
	{
		TransactionIdStatus xidstatus = TransactionIdGetStatusCSN(xid);
		if (xidstatus == XID_INPROGRESS)
		{
			/*
...
heap_abort_speculative(Relation relation, HeapTuple tuple)
{
    ...
    Assert(TransactionIdIsValid(GetRecentGlobalXmin()));
#ifdef __SUPPORT_DISTRIBUTED_TRANSACTION__
    PageSetPrunable(page, GetRecentGlobalXmin(), InvalidGlobalTimestamp);
#else
				  ...
				  uint16 *flags)
{
	...
	*flags = 0;
		...
		else
		{
			...
			if (TransactionIdPrecedes(xid, cutoff_xid))
			{
#ifdef __SUPPORT_DISTRIBUTED_TRANSACTION__
				{
					...
					if(TransactionIdGetCSNAndCommitStat(xid, &committs, NULL))
					{
						if(TestForOldTimestampForOriginRecentDataTs(committs, false))
						{
						...
						{
							elog(DEBUG1, "tuple cannot be frozen now, "
								"please try later, xid: %u, cutoff xid: %u, "
								"committs: " INT64_FORMAT ", "
								"RecentDataTs: " INT64_FORMAT ", "
								"GetRecentGlobalXmin(): %u, "
								"GetRecentGlobalDataXmin(): %u",
								xid, cutoff_xid, committs,
								GetRecentRecentDataTs(), GetRecentGlobalXmin(),
	...
	for (i = 0; i < nmembers; i++)
	{
		...
		{
			TransactionIdStatus xidstatus = TransactionIdGetStatusCSN(members[i].xid);
						  ...
						  xl_heap_freeze_tuple *frz, bool *totally_frozen_p)
{
...
#ifdef __SUPPORT_DISTRIBUTED_TRANSACTION__
			{
                CommitSeqNo committs = HeapTupleHeaderGetXminTimestampAtomic(tuple);
				...
				else
				{
					elog(DEBUG1, "tuple cannot be frozen now, "
						...
						"GetRecentGlobalXmin(): %u, "
						"GetRecentGlobalDataXmin(): %u",
						xid, cutoff_xid, committs, GetRecentRecentDataTs(),
						GetRecentGlobalXmin(), GetRecentGlobalDataXmin());
				}
	...
	else if (TransactionIdIsNormal(xid))
	{
		if (TransactionIdPrecedes(xid, cutoff_xid))
		{
			GlobalTimestamp committs = HeapTupleHeaderGetXmaxTimestampAtomic(tuple);

			if(!CSN_IS_COMMITTED(committs))
				TransactionIdGetCSNAndCommitStat(xid, &committs, NULL);

			if(CSN_IS_COMMITTED(committs))
			{
				freeze_xmax = false;
				/* if committs is bigger than RecentDataTs, not report error */
				if (!(tuple->t_infomask & HEAP_XMAX_LOCK_ONLY) &&
					TestForOldTimestampForOriginRecentDataTs(committs, false))
					ereport(ERROR,
							(errcode(ERRCODE_DATA_CORRUPTED),
							  errmsg_internal("cannot freeze committed xmax %u, committs is %lu, RecentDataTs is %lu",
														xid, committs, RecentDataTs)));
			}
						...
						LockTupleMode lockmode, bool *current_is_member)
{
	...
	if (nmembers >= 0)
	{
		...
		for (i = 0; i < nmembers; i++)
		{
			...
			xidstatus = TransactionIdGetStatusCSN(memxid);
				   ...
				   int *remaining)
{
	...
	if (nmembers >= 0)
	{
		...
		for (i = 0; i < nmembers; i++)
		{
			...
			{
				TransactionIdStatus status = TransactionIdGetStatusCSN(memxid);

				if (remaining && status == XID_INPROGRESS)
					remain++;
				continue;
			}
...
log_heap_visible(RelFileNode rnode, Buffer heap_buffer, Buffer vm_buffer,
				 TransactionId cutoff_xid, uint8 vmflags, bool enable_checksum)
{
	...
	if (!XLogHintBitIsNeeded(BufferGetPage(vm_buffer)))
...
#ifdef _PG_ORCL_
	xlhdr.t_infomask3 = newtup->t_data->t_infomask3;
#endif
#ifdef _MIGRATE_
...
#ifdef _PG_ORCL_
		xlhdr_idx.t_infomask3 = old_key_tuple->t_data->t_infomask3;
#endif
...
log_heap_new_cid(Relation relation, HeapTuple tup)
{
	...
	/* get Raw Cmin, Cmax, combocid is always invalid now */
	xlrec.cmin = HeapTupleHeaderGetRawCmin(hdr);
	xlrec.cmax = HeapTupleHeaderGetRawCmax(hdr);
...
heap_xlog_clean(XLogReaderState *record)
{
	...
	if (BufferIsValid(buffer))
	{
		 ...
		 */
		XLogRecordPageWithFreeSpace(rnode, blkno, freespace, record->checksum_enabled);
	}
}
...
heap_xlog_visible(XLogReaderState *record)
{
	...
	if (BufferIsValid(buffer))
	{
		...
		if (xlrec->flags & VISIBILITYMAP_VALID_BITS)
			XLogRecordPageWithFreeSpace(rnode, blkno, space, record->checksum_enabled);
	}
									  ...
									  &vmbuffer) == BLK_NEEDS_REDO)
	{
		...
		if (PageIsNew(vmpage))
			PageInit(vmpage, BLCKSZ, 0, record->checksum_enabled);
...
heap_xlog_insert(XLogReaderState *record)
{
	...
	XLogRedoAction action;
#ifdef __OPENTENBASE_C__
	bool		checksum_enabled = XLogRecGetChecksumInfo(record);
#endif
	...
	if (XLogRecGetInfo(record) & XLOG_HEAP_INIT_PAGE)
	{
		...
		page = BufferGetPage(buffer);
		PageInit_shard(page, BufferGetPageSize(buffer), 0, sid, false, checksum_enabled);
...
#ifdef _PG_ORCL_
		htup->t_infomask3 = xlhdr.t_infomask3;
#endif
#ifdef _MIGRATE_
	...
	if (action == BLK_NEEDS_REDO && freespace < BLCKSZ / 5)
		XLogRecordPageWithFreeSpace(target_node, blkno, freespace, checksum_enabled);
}
...
heap_xlog_multi_insert(XLogReaderState *record)
{
	 ...
	 */
	if (action == BLK_NEEDS_REDO && freespace < BLCKSZ / 5)
		XLogRecordPageWithFreeSpace(rnode, blkno, freespace, checksum_enabled);
}
...
heap_xlog_update(XLogReaderState *record, bool hot_update)
{
	...
	else if (XLogRecGetInfo(record) & XLOG_HEAP_INIT_PAGE)
	{
		...
		PageInit_shard(page, BufferGetPageSize(nbuffer), 0, sid, false, checksum_enabled);
	...
	if (newaction == BLK_NEEDS_REDO && !hot_update && freespace < BLCKSZ / 5)
		XLogRecordPageWithFreeSpace(rnode, newblk, freespace, checksum_enabled);
}
...
/* 
 * compare cmmitts and RecentDataTs, if committs is bigger than RecentDataTs, 
 * than check if current database is template or is allowconn, adjust it as default_freeze_min_age
 */
bool 
TestForOldTimestampForOriginRecentDataTs(GlobalTimestamp currentTimestamp, bool skip_template_db)
{
	HeapTuple tuple;
	Form_pg_database dbForm;
	GlobalTimestamp originRecentDataTs;
	if (likely(TestForOldCSN(currentTimestamp, GetRecentRecentDataTs()))) 
	{ 
		return true;
	}

	if (true == skip_template_db)
	{
		return false; 
	}
	...
	tuple = SearchSysCache1OnlyCache(DATABASEOID, ObjectIdGetDatum(MyDatabaseId));
	if (!HeapTupleIsValid(tuple))
	{
#ifndef _PG_REGRESS_
		elog(DEBUG1, "cache lookup failed for database %u", MyDatabaseId);
#endif
		return false;
	}
	...
	{
		/* default_freeze_min_age is set to 0, so gts should also adjust */
		originRecentDataTs = GetRecentRecentDataTs() + (vacuum_delta * TIMESTAMP_SHIFT);
		ReleaseSysCache(tuple);
		return TestForOldCSN(currentTimestamp, originRecentDataTs);
	}
	ReleaseSysCache(tuple);
	return false;
}
