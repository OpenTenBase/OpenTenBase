	...
	if (flags != (map[mapByte] >> mapOffset & VISIBILITYMAP_VALID_BITS))
	{
		...
		if (RelationNeedsWAL(rel))
		{
			if (XLogRecPtrIsInvalid(recptr))
			{
				 ...
				 */
				if (XLogHintBitIsNeeded(heapPage))
				{
...
visibilitymap_truncate(Relation rel, BlockNumber nheapblocks)
{
	...
	if (truncByte != 0 || truncOffset != 0)
	{
		...
		if (!InRecovery && RelationNeedsWAL(rel) &&
		    XLogHintBitIsNeeded(BufferGetPage(mapBuffer)))
		{
			log_newpage_buffer(mapBuffer, false, RelationHasChecksum(rel));
		}
...
vm_extend(Relation rel, BlockNumber vm_nblocks)
{
	...
	{
		PageSetChecksumInplaceAll(pg, vm_nblocks_now,
							RelationHasChecksum(rel));
