...
heap_page_prune_opt(Relation relation, Buffer buffer)
{
	...
	/* we can't prune page during inplace upgrade */
	 ...
	 */
#ifdef __SUPPORT_DISTRIBUTED_TRANSACTION__
	if(!PageIsPrunable(page, OldestXmin, GetRecentRecentDataTs()))
	{
		return;
	}
#else
	...
	for (;;)
	{
		...
		switch (HeapTupleSatisfiesVacuum(&tup, OldestXmin, buffer))
		{
			...
			case HEAPTUPLE_RECENTLY_DEAD:
				...
				{
                    committs = TransactionIdGetCSN(HeapTupleHeaderGetUpdateXid(htup));
				}
				else
				{
					committs = HeapTupleHeaderGetXmaxTimestampAtomic(htup);
				}
