...
end_heap_rewrite(RewriteState state)
{
	...
	{
		bool checksum_enabled = RelationHasChecksum(state->rs_new_rel);

		PageSetChecksumFlag(state->rs_buffer, checksum_enabled);
		if (state->rs_use_wal)
		{
				   ...
				   HeapTuple old_tuple, HeapTuple new_tuple)
{
    ...
    new_tuple->t_data->t_infomask2 |= old_tuple->t_data->t_infomask2 & 
        (HEAP_XMIN_TIMESTAMP_UPDATED | HEAP_XMAX_TIMESTAMP_UPDATED);
...
raw_heap_insert(RewriteState state, HeapTuple tup)
{
	...
	{
		/*
		 * Here acquire available buffer page of the relation to store new tuples,
		 * instead of the original way, which is to write the page immediately
		 * and reset for use. This way may avoid frequent IO for my understanding.
		 * As a result, no encryption is needed here, refer to FlushBuffer instead.
		 */
#ifdef _SHARDING_		
	...
	/* check validation of shardid */
	heap_tuple_validate_shardid(state->rs_new_rel, heaptup);
