...
found:
	...
	else
	{
		...
		{
			_hash_initbitmapbuffer(newmapbuf, metap->hashm_bmsize, false, RelationHasChecksum(rel));
				   ...
				   BufferAccessStrategy bstrategy)
{
	 ...
	 */
	_hash_pageinit(ovflpage, BufferGetPageSize(ovflbuf), RelationHasChecksum(rel));
 ...
 */
void
_hash_initbitmapbuffer(Buffer buf, uint16 bmsize, bool initpage, bool checksum_enabled)
{
	Page		pg;
	HashPageOpaque op;
	uint32	   *freep;
	...
	/* initialize the page */
	if (initpage)
		_hash_pageinit(pg, BufferGetPageSize(buf), checksum_enabled);
