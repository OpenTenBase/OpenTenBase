...
_hash_getinitbuf(Relation rel, BlockNumber blkno)
{
	...
	/* initialize the page */
	_hash_pageinit(BufferGetPage(buf), BufferGetPageSize(buf), RelationHasChecksum(rel));
...
_hash_initbuf(Buffer buf, uint32 max_bucket, uint32 num_bucket, uint32 flag,
			  bool initpage, bool checksum_enabled)
{
	...
	/* initialize the page */
	if (initpage)
		_hash_pageinit(page, BufferGetPageSize(buf), checksum_enabled);
...
_hash_init(Relation rel, double num_tuples, ForkNumber forkNum)
{
	 ...
	 * logged.
	 */
	use_wal = RelationNeedsWAL(rel) || forkNum == INIT_FORKNUM || RELATION_IS_META_GLOBAL_TEMP(rel);
	...
	_hash_init_metabuffer(metabuf, num_tuples, procid, ffactor, false, RelationHasChecksum(rel));
	...
	for (i = 0; i < num_buckets; i++)
	{
		...
		checksum_enabled = RelationHasChecksum(rel);
		_hash_initbuf(buf, metap->hashm_maxbucket, i, LH_BUCKET_PAGE, false, checksum_enabled);
		MarkBufferDirty(buf);
	...
	_hash_initbitmapbuffer(bitmapbuf, metap->hashm_bmsize, false, RelationHasChecksum(rel));
...
_hash_init_metabuffer(Buffer buf, double num_tuples, RegProcedure procid,
					  uint16 ffactor, bool initpage, bool checksum_enabled)
{
	HashMetaPage metap;
	HashPageOpaque pageopaque;
	Page		page;
 ...
 */
void
_hash_pageinit(Page page, Size size, bool checksum_enabled)
{
	PageInit(page, size, sizeof(HashPageOpaqueData), checksum_enabled);
}
...
_hash_alloc_buckets(Relation rel, BlockNumber firstblock, uint32 nblocks)
{
	 ...
	 */
	checksum_enabled = RelationHasChecksum(rel);
	_hash_pageinit(page, BLCKSZ, checksum_enabled);
