...
gistbuildempty(Relation index)
{
	...
	START_CRIT_SECTION();
	checksum_enabled = RelationHasChecksum(index);

	GISTInitBuffer(buffer, F_LEAF, checksum_enabled);
	...
	*splitinfo = NIL;
	...
	if (is_split)
	{
		...
		if (!is_rootsplit)
		{
			...
			dist->page = PageGetTempPageCopySpecial(BufferGetPage(buffer), RelationHasChecksum(rel));
		...
		for (; ptr; ptr = ptr->next)
		{
			...
			GISTInitBuffer(ptr->buffer, (is_leaf) ? F_LEAF : 0, RelationHasChecksum(rel));
		...
		if (is_rootsplit)
		{
			...
			rootpg.page = PageGetTempPageCopySpecial(BufferGetPage(rootpg.buffer), RelationHasChecksum(rel));
		...
		if (RelationNeedsWAL(rel))
			recptr = gistXLogSplit(is_leaf,
								   ...
								   markfollowright, RelationHasChecksum(rel));
		else
