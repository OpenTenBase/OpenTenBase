...
#define COPY_HEADER_FIELD(_dst, _size)			\
	...
	while (remaining > datatotal)
	{
		...
		else if (BKID_GET_BKID(block_id) <= XLR_MAX_BLOCK_ID)
		{
			...
			uint8		fork_flags;
#ifdef __OPENTENBASE_C__
			uint32 BLOCK_SIZE = BLCKSZ;
#endif

#ifdef __OPENTENBASE_C__
			state->checksum_enabled = (block_id & BKID_HAS_CHECKSUM_PAGE) != 0;
...
#else
			COPY_HEADER_FIELD(&blk->data_len, sizeof(blk->data_len));
#endif
			...
			if (blk->has_image)
			{
...
#else
				COPY_HEADER_FIELD(&blk->bimg_len, sizeof(blk->bimg_len));
				COPY_HEADER_FIELD(&blk->hole_offset, sizeof(blk->hole_offset));
#endif
...
#else
						COPY_HEADER_FIELD(&blk->hole_length, sizeof(blk->hole_length));
#endif
					else
						blk->hole_length = 0;
				}
				 ...
				 * IS_COMPRESSED flag is set except for colstore
				 */
				if (!(blk->bimg_info & BKPIMAGE_HAS_HOLE) &&
...
/*
 * Restore a full-page image for column store from a backup block attached to an XLOG record.
 *
 * Returns the buffer number containing the page.
 */
bool
RestoreColBlockImage(XLogReaderState *record, uint8 block_id, char *page)
{
    DecodedBkpBlock *bkpb;
    char	   *ptr;
    char		tmp[COL_BLCKSZ];
    ...
    else
    {
        ...
        memcpy(page + (bkpb->hole_offset + bkpb->hole_length),
               ptr + bkpb->hole_offset,
               COL_BLCKSZ - (bkpb->hole_offset + bkpb->hole_length));
    }
...
bool
XLogRecGetEstoreInfo(XLogReaderState *record, RelFileNode *rnode, 
					ForkNumber *forknum, uint32 *attnum, 
					uint32 *offset, uint32 *blocksz)
{
	xl_heap_data_newpage* xlrec = (xl_heap_data_newpage*)XLogRecGetData(record);
	if(!xlrec->hasdata)
	{
		return false;
	}

	(*rnode).spcNode = xlrec->node.spcNode;
	(*rnode).dbNode = xlrec->node.dbNode;
	(*rnode).relNode = xlrec->node.relNode;

	*forknum = xlrec->forknum;
	*attnum = xlrec->attid;
	*offset = xlrec->offset;
	*blocksz = xlrec->block_size;
