...
typedef struct
{
	...
	Buffer		bufferid;		/* shared buffer id */
...
#else
static bool XLogCompressBackupBlock(char *page, uint32 hole_offset, uint32 hole_length,
						char *dest, uint32 *dlen);
#endif
 ...
 */
void
XLogRegisterBuffer_TDE(uint8 block_id, Buffer buffer, uint8 flags, bool checksum_enabled)
{
	...
	regbuf->rdata_len = 0;
	regbuf->bufferid = buffer;

#ifdef __OPENTENBASE_C__

	regbuf->checksum_enabled = checksum_enabled;
				   ...
				   XLogRecPtr *fpw_lsn)
{
	...
	*fpw_lsn = InvalidXLogRecPtr;
	for (block_id = 0; block_id < max_registered_block_id; block_id++)
	{
		...
		if (regbuf->checksum_enabled)
		{
			bkpb.id |= BKID_HAS_CHECKSUM_PAGE;
		}
...
#else
static bool 
XLogCompressBackupBlock(char *page, uint32 hole_offset, uint32 hole_length,
						char *dest, uint32 *dlen)
...
#else
		*dlen = len;	/* successful compression */
#endif
		return true;
	}
	return false;
...
XLogSaveBufferForHint(Buffer buffer, bool buffer_std)
{
	...
	if (lsn <= RedoRecPtr)
	{
		...
		XLogRegisterBlock(0, &rnode, forkno, blkno, copied_buffer, flags, checksum_enabled);
...
log_newpage(RelFileNode *rnode, ForkNumber forkNum, BlockNumber blkno,
			Page page, bool page_std, bool checksum_enabled)
{
	int			flags;
	XLogRecPtr	recptr;
	...
	XLogBeginInsert();
	XLogRegisterBlock(0, rnode, forkNum, blkno, page, flags, checksum_enabled);
 ...
 */
XLogRecPtr
log_newpage_buffer(Buffer buffer, bool page_std, bool checksum_enabled)
{
	Page		page = BufferGetPage(buffer);
	RelFileNode rnode;
	ForkNumber	forkNum;
	...
	return log_newpage(&rnode, forkNum, blkno, page, page_std, checksum_enabled);
}
