...
bool force_ignore_redo_missing = false;
...
int	        gts_track_segment = 0;
#ifdef WAL_DEBUG
bool		XLOG_DEBUG = false;
#endif
bool		DoingStartupXLOG = false;
						  ...
						  XLogRecPtr *PrevPtr)
{
	...
	if (size <= SizeOfXLogRecord)
	{
			elog(PANIC, "ReserveXLogInsertLocation error size");
	}
	 ...
	 */
#if defined(__x86_64__) || defined(__aarch64__)
	compare = pg_atomic_read_u128((uint128_u*)&Insert->CurrBytePos);
...
loop:
	...
	current = pg_atomic_compare_and_swap_u128((uint128_u*)&Insert->CurrBytePos, compare, exchange);
...
ReserveXLogSwitch(XLogRecPtr *StartPos, XLogRecPtr *EndPos, XLogRecPtr *PrevPtr)
{
	...
	uint128_u exchange;
	uint128_u current;
	uint128_u compare = pg_atomic_read_u128((uint128_u*)&Insert->CurrBytePos);
...
XLogFlush(XLogRecPtr record)
{
	...
	(void)whitebox_trigger_generic(XLOG_FLUSH, WHITEBOX_TRIGGER_SLEEP,
...
XLogFileInit(XLogSegNo logsegno, bool *use_existent, bool use_lock)
{
	 ...
	 */
	rc = pg_pwrite_zeros(fd, (size_t)XLogSegSize, 0);
			 ...
			 int upto)
{
	...
	elog(LOG, "copy file segno %lu, src tli %u, dest tli %u", destsegno, srcTLI, ThisTimeLineID);
}
...
ReadControlFile(void)
{
	...
	SetConfigOption("data_checksums", ControlDataChecksumsEnabled() ? "yes" : "no",
...
/*
 * Are checksums enabled for data pages in controldata?
 */
bool
ControlDataChecksumsEnabled()
{
	Assert(ControlFile != NULL);
 ...
 */
bool
DataChecksumsEnabled(Page page)
{
	Assert(ControlFile != NULL);
	return (ControlFile->data_checksum_version > 0 ||
			(page_checksums && PageHasCheckSum(page)));
}
...
XLOGShmemSize(void)
{
	...
	/* unlogged col table cleanup flag */
...
BootStrapXLOG(void)
{
	...
	checkPoint.latestGTS = CSN_FIRST_NORMAL;
	checkPoint.latestCommitTs = CSN_FIRST_NORMAL;

    SpinLockInit(&ShmemVariableCache->ts_lock);
    
    SetSharedMaxCommitTs(checkPoint.latestCommitTs);
    SetSharedLatestCommitTS(checkPoint.latestGTS);
...
HandleRecoveryForReload(void)
{
	...
	{
        elog(DEBUG1, "HandleRecoveryForReload %s: %s", item->name, item->value);
...
recoveryStopsBefore(XLogReaderState *record)
{
	...
	if (XLogRecGetRmid(record) == RM_XACT_ID)
	{
		...
		if (xact_info == XLOG_XACT_COMMIT)
		{
			...
			if (XLogArchivingActive())
			{
				BackendTrackGTS(XLByteToSegNo((XLogRecGetEndLsn(record))), xlrec->gts);
			}
#endif
		}
...
StartupXLOG(void)
{
	...
	TransactionId oldestActiveXIDAfterRedo;
	...
	/*
	 * If the checksum failure file exists, it indicates that some pages have  
	 * been corrupted. In such cases, the DBA needs to perform an inspection  
	 * and repair of the damaged pages before proceeding with the startup.
	 */
	...
	ereport(LOG,
			(errmsg_internal("redo record is at %X/%X; loc %X/%X; shutdown %s;"
							" prev tli %u, curr tli %u;"
							" next transaction ID: %u:%u; next OID: %u;"
							" next MultiXactId: %u; next MultiXactOffset: %u;"
							" oldest unfrozen transaction ID: %u, in database %u;"
							" oldest MultiXactId: %u, in database %u;"
							" commit timestamp Xid oldest/newest: %u/%u",
							(uint32) (checkPoint.redo >> 32), (uint32) checkPoint.redo,
							(uint32) (checkPointLoc >> 32), (uint32) checkPointLoc,
							wasShutdown ? "TRUE" : "FALSE",
							checkPoint.PrevTimeLineID, checkPoint.ThisTimeLineID,
							checkPoint.nextXidEpoch, checkPoint.nextXid, checkPoint.nextOid,
    ...
    SetSharedMaxCommitTs(checkPoint.latestCommitTs);
    SetSharedLatestCommitTS(checkPoint.latestGTS);
	elog(LOG, "read maxCommitTs " INT64_FORMAT " read latestGTS " INT64_FORMAT, GetSharedMaxCommitTs(), GetSharedLatestCommitTS());	
#endif
	...
	if (RECOVERY_TARGET_GTS == recoveryTarget)
	{
        GlobalTimestamp maxGts = GetSharedMaxCommitTs();
		if (maxGts > recoveryTargetGTS)
		{
			ereport(FATAL,
					(errmsg("invalid recovery target global timestamp:%lu less than latestCommitTs:%lu", recoveryTargetGTS, maxGts)));
		}
	}
#endif
	...
	/* For shut checkpoint checkPoint.oldestActiveXid is InvalidTransactionId but it does not matter */
	pg_atomic_write_u32(&ShmemVariableCache->oldestActiveXid, checkPoint.oldestActiveXid);
	...
	if (XLogArchivingActive())
	{
        ...
        GlobalTimestamp latestGts = GetSharedLatestCommitTS();
        
		StartupXlogTrack(XLByteToSegNo(RedoRecPtr), latestGts);
		BackendTrackGTS(XLByteToSegNo(RedoRecPtr), latestGts);
		XLogFileName(xlogfile, ThisTimeLineID, XLByteToSegNo(RedoRecPtr));	
		elog(LOG, "database first track global timestamp:%lu xlog:%s", latestGts, xlogfile);	
	}
#endif
	...
	if (InRecovery)
	{
		...
		if (checkPoint.redo < RecPtr)
		{
			...
			if (!record)
			{
				emode = force_ignore_redo_missing ? WARNING : PANIC;
				ereport(emode,
						(errmsg("could not read redo %X/%X of checkpoint %X/%X",
								(uint32) (checkPoint.redo >> 32), (uint32) checkPoint.redo,
								(uint32) (checkPointLoc >> 32), (uint32) checkPointLoc)));
			}
		}
...
#ifdef USE_WHITEBOX_INJECTION
					/* 
					 * If PRIMARY_CHECKPOINT_KEEP_XLOG or STANDBY_XLOGREDO_KEEP_XLOG is specified
					 * in whitebox setting, do not recycle and remove xlog files, and keep it 
					 * for analysis, even if we are switching timeline.
					 */
					if (!whitebox_trigger_setting(PRIMARY_CHECKPOINT_KEEP_XLOG, WHITEBOX_TRIGGER_SETTING)
						&& (!whitebox_trigger_setting(STANDBY_XLOGREDO_KEEP_XLOG, WHITEBOX_TRIGGER_SETTING)))
	...
	oldestActiveXIDAfterRedo = pg_atomic_read_u32(&ShmemVariableCache->oldestActiveXid);
	
	if ( oldestActiveXIDAfterRedo != InvalidTransactionId)
		oldestActiveXID = oldestActiveXIDAfterRedo;
     ...
     */
    TrimCSNLOG();
    elog(LOG, "max commit ts after redo " UINT64_FORMAT, GetSharedLatestCommitTS());
...
LogCheckpointEnd(bool restartpoint)
{
	...
	elog(LOG, "%s complete: wrote %d buffers (%.1f%%); "
		 ...
		 "pre-point=%X/%X, cur-point=%X/%X; "
		 "oldestactivexid:%u, nextxid:%u, oldestxid:%u, "
		 "oldestCommitTsXid:%u, newestCommitTsXid:%u",
		 restartpoint ? "restartpoint" : "checkpoint",
		 ...
		 (uint32)(ControlFile->prevCheckPoint >> 32), (uint32)ControlFile->prevCheckPoint,
		 (uint32)(ControlFile->checkPoint >> 32), (uint32)ControlFile->checkPoint,
		 oldest_active_xid, ControlFile->checkPointCopy.nextXid, ControlFile->checkPointCopy.oldestXid,
		 ControlFile->checkPointCopy.oldestCommitTsXid, ControlFile->checkPointCopy.newestCommitTsXid);
}
...
CreateCheckPoint(int flags)
{
	...
	checkPoint.latestCommitTs = GetSharedMaxCommitTs();
	checkPoint.latestGTS = GetSharedLatestCommitTS();
	...
	/*
	 * Do not sync nextoid in initdb for postgres template to avoid
	 * opentenbase_ora template oid inflation.
	 */
	if(tempOid)
		checkPoint.nextOid = ControlFile->checkPointCopy.nextOid;
	else
...
#ifdef USE_WHITEBOX_INJECTION
		/* 
		 * If PRIMARY_CHECKPOINT_KEEP_XLOG is specified in whitebox setting,
		 * do not recycle and remove xlog files, and keep it for analysis.
		 */
		if (whitebox_trigger_setting(PRIMARY_CHECKPOINT_KEEP_XLOG, WHITEBOX_TRIGGER_SETTING))
			_logSegNo = 1;
#endif
...
CreateEndOfRecoveryRecord(void)
{
	...
	LocalSetXLogInsertAllowed();
	elog(LOG, "log end of recovery: prev tli:%u, cur tli:%u", xlrec.PrevTimeLineID, xlrec.ThisTimeLineID);
...
CreateRestartPoint(int flags)
{
		...
		lastCheckPoint.redo <= ControlFile->checkPointCopy.redo)
	{
		...
		ereport(trace_recovery(LOG),
				(errmsg("skipping restartpoint, already performed at %X/%X according to XlogCtl, "
						"whereas the last checkpoint performed is at %X/%X from control file, and "
						"last replay LSN at %X/%X in XlogCtl.",
						(uint32) (lastCheckPoint.redo >> 32),
						(uint32) lastCheckPoint.redo,
						(uint32) (ControlFile->checkPointCopy.redo >> 32),
						(uint32) ControlFile->checkPointCopy.redo,
						(uint32) (XLogCtl->replayEndRecPtr >> 32),
						(uint32) XLogCtl->replayEndRecPtr)));
#else
		ereport(DEBUG2,
	...
	{
		ereport(trace_recovery(LOG),
				(errmsg("performing restartpoint %X/%X, "
						"when last replay LSN at %X/%X according to XlogCtl.",
						(uint32) (lastCheckPointRecPtr >> 32),
...
#ifdef USE_WHITEBOX_INJECTION
		/* 
		 * If STANDBY_XLOGREDO_KEEP_XLOG is specified in whitebox setting,
		 * do not recycle and remove xlog files, and keep it for analysis.
		 */
		if (whitebox_trigger_setting(STANDBY_XLOGREDO_KEEP_XLOG, WHITEBOX_TRIGGER_SETTING))
			_logSegNo = 1;
#endif
    ...
    else if (info == XLOG_CREATE_2PC_FILE)
    {
        ...
        if (0 != strcmp(type, "readonly"))
        {
            ...
            else
            {
                ...
                elog(LOG, "upgrade from format v1");
            ...
            if (FILE_XLOG_EXISTED == twophase_exception_case)
            {
                ...
                record_2pc_involved_nodes_xid(gid, startnode, startxid, nodestring,
												xid, NULL, database, user);
            }
#endif
            /*
							...
							bool fetching_ckpt, XLogRecPtr tliRecPtr)
{
    ...
    elog(DEBUG1, "WaitForWALToBecomeAvailable currentSource %s, lastSourceFailed %s", xlogSourceNames[currentSource],
	...
	for (;;)
	{
			...
			switch (currentSource)
			{
				...
				case XLOG_FROM_PG_WAL:
					...
					if (PrimaryConnInfo)
					{
						...
						curFileTLI = tli;
                        launch = RequestXLogStreaming(tli, ptr, PrimaryConnInfo,
											 PrimarySlotName);
                        
                        /* 
                         * if PMSIGNAL_START_WALRECEIVER is requested, wait for it to avoid 
                         * fast shutdown of walreceiver just after it is started
                         */
                        if (launch)
                        {
                            WaitLatch(&XLogCtl->recoveryWakeupLatch,
                                      WL_LATCH_SET | WL_TIMEOUT | WL_POSTMASTER_DEATH,
                                      wal_retrieve_retry_interval, WAIT_EVENT_RECOVERY_WAL_STREAM);
                            ResetLatch(&XLogCtl->recoveryWakeupLatch);
                        }
						receivedUpto = 0;
					}
		...
		switch (currentSource)
		{
			...
			case XLOG_FROM_PG_WAL:
				...
				if (readFile >= 0)
                {
                    elog(DEBUG1, "WaitForWALToBecomeAvailable currentSource %s, lastSourceFailed %s readFile true", xlogSourceNames[currentSource],
                         lastSourceFailed ? "failure" : "success");
                    return true;	/* success! */
                }
			...
			case XLOG_FROM_STREAM:
				{
                    ...
                    elog(DEBUG1, "WaitForWALToBecomeAvailable havedata %d, RecPtr %lu, receivedUpto %lu, "
                                 "receiveTLI %u, curFileTLI %u, currentSource %s, lastSourceFailed %s",
                         havedata, RecPtr, receivedUpto, receiveTLI, curFileTLI,
					...
					if (havedata)
					{
						...
						else
						{
                            ...
                            elog(DEBUG1, "WaitForWALToBecomeAvailable havedata return true, currentSource %s, lastSourceFailed %s", xlogSourceNames[currentSource],
                                 lastSourceFailed ? "failure" : "success");
							return true;
						}
                        elog(DEBUG1, "WaitForWALToBecomeAvailable havedata break, currentSource %s, lastSourceFailed %s", xlogSourceNames[currentSource],
                             lastSourceFailed ? "failure" : "success");
						break;
					}
					...
					if (CheckForStandbyTrigger())
					{
						 ...
						 */
						lastSourceFailed = true;
                        elog(DEBUG1, "WaitForWALToBecomeAvailable CheckForStandbyTrigger, currentSource %s, lastSourceFailed %s", 
                    ...
                    elog(DEBUG1, "WaitForWALToBecomeAvailable WaitLatch start, currentSource %s, lastSourceFailed %s",
                         xlogSourceNames[currentSource], lastSourceFailed ? "failure" : "success");
					...
					ResetLatch(&XLogCtl->recoveryWakeupLatch);
                    elog(DEBUG1, "WaitForWALToBecomeAvailable WaitLatch end, currentSource %s, lastSourceFailed %s",
                         xlogSourceNames[currentSource], lastSourceFailed ? "failure" : "success");
					break;
				}
 ...
 */
void
WakeupRecoveryInternal(char *file, int32 lineno)
{
	SetLatch(&XLogCtl->recoveryWakeupLatch);
    elog(DEBUG1, "WakeupRecovery %s:%d", file, lineno);
}
	...
	if (RECOVERY_TARGET_GTS == recoveryTarget && recoveryGTMHost != NULL)
	{
		...
		do 
		{
            ...
            if (*p == '[') {
                ...
                p = strstr(p, "]");
                if (p == NULL) {
                    elog(ERROR, "Invalid IPv6 address format");
                    break;
                }
                *p = '\0';
