...
brin_xlog_createidx(XLogReaderState *record)
{
	...
	brin_metapage_init(page, xlrec->pagesPerRange, xlrec->version, record->checksum_enabled);
	PageSetLSN(page, lsn);
	MarkBufferDirty(buf);
						...
						xl_brin_insert *xlrec)
{
	...
	if (XLogRecGetInfo(record) & XLOG_BRIN_INIT_PAGE)
	{
		...
		brin_page_init(page, BRIN_PAGETYPE_REGULAR, record->checksum_enabled);
...
brin_xlog_revmap_extend(XLogReaderState *record)
{
	...
	page = (Page) BufferGetPage(buf);
	brin_page_init(page, BRIN_PAGETYPE_REVMAP, record->checksum_enabled);
