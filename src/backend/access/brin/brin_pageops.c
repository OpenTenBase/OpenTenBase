			  ...
			  bool samepage)
{
	...
	else
	{
		...
		if (extended)
		{
			brin_page_init(BufferGetPage(newbuf), BRIN_PAGETYPE_REGULAR, RelationHasChecksum(idxrel));
		}
			  ...
			  BrinTuple *tup, Size itemsz)
{
	...
	if (extended)
	{
		brin_page_init(BufferGetPage(*buffer), BRIN_PAGETYPE_REGULAR, RelationHasChecksum(idxrel));
	}
	...
	/* XLOG stuff */
	if (RelationNeedsWAL(idxrel) || RELATION_IS_META_GLOBAL_TEMP(idxrel))
	{
 ...
 */
void
brin_page_init(Page page, uint16 type, bool checksum_enabled)
{
	PageInit(page, BLCKSZ, sizeof(BrinSpecialSpace), checksum_enabled);
 ...
 */
void
brin_metapage_init(Page page, BlockNumber pagesPerRange, uint16 version, bool checksum_enabled)
{
	BrinMetaPageData *metadata;

	brin_page_init(page, BRIN_PAGETYPE_META, checksum_enabled);
...
brin_initialize_empty_new_buffer(Relation idxrel, Buffer buffer)
{
	...
	START_CRIT_SECTION();
	page = BufferGetPage(buffer);
	checksum_enabled = RelationHasChecksum(idxrel);

	brin_page_init(page, BRIN_PAGETYPE_REGULAR, checksum_enabled);
	MarkBufferDirty(buffer);
	log_newpage_buffer(buffer, true, checksum_enabled);
	END_CRIT_SECTION();
