...
spgRedoCreateIndex(XLogReaderState *record)
{
	...
	SpGistInitMetapage(page, record->checksum_enabled);
	PageSetLSN(page, lsn);
	...
	SpGistInitBuffer(buffer, SPGIST_LEAF, record->checksum_enabled);
	page = (Page) BufferGetPage(buffer);
	PageSetLSN(page, lsn);
	MarkBufferDirty(buffer);
	UnlockReleaseBuffer(buffer);
	...
	SpGistInitBuffer(buffer, SPGIST_LEAF | SPGIST_NULLS, record->checksum_enabled);
	page = (Page) BufferGetPage(buffer);
	PageSetLSN(page, lsn);
...
spgRedoAddNode(XLogReaderState *record)
{
	...
	else
	{
		...
		if (xldata->newPage)
		{
			...
			buffer = XLogInitBufferForRedo(record, 1);
			SpGistInitBuffer(buffer, 0, record->checksum_enabled);
			action = BLK_NEEDS_REDO;
		}
		else
			action = XLogReadBufferForRedo(record, 1, &buffer);
...
spgRedoPickSplit(XLogReaderState *record)
{
	...
	if (xldata->initInner)
	{
		...
		SpGistInitBuffer(innerBuffer, (xldata->storesNulls ? SPGIST_NULLS : 0), record->checksum_enabled);
		action = BLK_NEEDS_REDO;
	}
	else
