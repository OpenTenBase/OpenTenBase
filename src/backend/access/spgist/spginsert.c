...
spgbuild(Relation heap, Relation index, IndexInfo *indexInfo)
{
	...
	SpGistInitMetapage(BufferGetPage(metabuffer), RelationHasChecksum(index));
	MarkBufferDirty(metabuffer);
	SpGistInitBuffer(rootbuffer, SPGIST_LEAF, RelationHasChecksum(index));
	MarkBufferDirty(rootbuffer);
	SpGistInitBuffer(nullbuffer, SPGIST_LEAF | SPGIST_NULLS, RelationHasChecksum(index));
	MarkBufferDirty(nullbuffer);

	if (RelationNeedsWAL(index) || RELATION_IS_META_GLOBAL_TEMP(index))
	{
		XLogRecPtr	recptr;
...
{
	Page		page;
	bool		checksum_enabled = RelationHasChecksum(index);

	/* Construct metapage. */
	page = (Page) palloc(BLCKSZ);
	SpGistInitMetapage(page, checksum_enabled);
	...
	log_newpage(&RelationGetSmgr(index)->smgr_rnode.node, INIT_FORKNUM,
				SPGIST_METAPAGE_BLKNO, page, false, checksum_enabled);

	/* Likewise for the root page. */
	SpGistInitPage(page, SPGIST_LEAF, checksum_enabled);

	PageSetChecksumInplace(page, SPGIST_ROOT_BLKNO);
	smgrwrite(RelationGetSmgr(index), INIT_FORKNUM, SPGIST_ROOT_BLKNO,
			  (char *) page, true);
	log_newpage(&RelationGetSmgr(index)->smgr_rnode.node, INIT_FORKNUM,
				SPGIST_ROOT_BLKNO, page, true, checksum_enabled);

	/* Likewise for the null-tuples root page. */
	SpGistInitPage(page, SPGIST_LEAF | SPGIST_NULLS, checksum_enabled);

	PageSetChecksumInplace(page, SPGIST_NULL_BLKNO);
	smgrwrite(RelationGetSmgr(index), INIT_FORKNUM, SPGIST_NULL_BLKNO,
			  (char *) page, true);
	log_newpage(&RelationGetSmgr(index)->smgr_rnode.node, INIT_FORKNUM,
				SPGIST_NULL_BLKNO, page, true, checksum_enabled);
