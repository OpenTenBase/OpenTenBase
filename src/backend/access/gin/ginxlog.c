...
ginRedoCreateIndex(XLogReaderState *record)
{
	...
	GinInitMetabuffer(MetaBuffer, record->checksum_enabled);
	...
	GinInitBuffer(RootBuffer, GIN_LEAF, record->checksum_enabled);
...
ginRedoCreatePTree(XLogReaderState *record)
{
	...
	GinInitBuffer(buffer, GIN_DATA | GIN_LEAF | GIN_COMPRESSED, record->checksum_enabled);
...
ginRedoUpdateMetapage(XLogReaderState *record)
{
	...
	GinInitPage(metapage, GIN_META, BufferGetPageSize(metabuffer), record->checksum_enabled);
...
ginRedoInsertListPage(XLogReaderState *record)
{
	...
	GinInitBuffer(buffer, GIN_LIST, record->checksum_enabled);
	GinPageGetOpaque(page)->rightlink = data->rightlink;
...
ginRedoDeleteListPages(XLogReaderState *record)
{
	...
	for (i = 0; i < data->ndeleted; i++)
	{
		...
		page = BufferGetPage(buffer);
		GinInitBuffer(buffer, GIN_DELETED, record->checksum_enabled);
