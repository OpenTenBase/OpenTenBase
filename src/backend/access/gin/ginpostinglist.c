...
/*
 * define MAXHeapTuplesPerSILOBits: number of bits to encode offset number to 16.
 * estore can get ctid with offset number that utilize the full bits of 16bits ip_posid.
 */
#define MaxHeapTuplesPerPageBits		11
#define MAXHeapTuplesPerSILOBits 		16

static inline uint64
itemptr_to_uint64(const ItemPointer iptr, GinPostingListType type)
{
	uint64		val;
	uint16 		perBits = (type == ROW_STORE_TYPE) ? 
							MaxHeapTuplesPerPageBits : MAXHeapTuplesPerSILOBits;

	Assert(ItemPointerIsValid(iptr));
	Assert(GinItemPointerGetOffsetNumber(iptr) < (1 << perBits));
...
static inline void
uint64_to_itemptr(uint64 val, ItemPointer iptr, GinPostingListType type)
{
	uint16 		perBits = (type == ROW_STORE_TYPE) ? 
						MaxHeapTuplesPerPageBits : MAXHeapTuplesPerSILOBits;

	GinItemPointerSetOffsetNumber(iptr, val & ((1 << perBits) - 1));
	val = val >> perBits;
 ...
 */
static uint64
decode_varbyte(unsigned char **ptr, GinPostingListType type)
{
	uint64		val;
	unsigned char *p = *ptr;
	uint64		c;
			...
			if (c & 0x80)
			{
				...
				if (c & 0x80)
				{
					...
					if (c & 0x80)
					{
						...
						{
							c = *(p++);
							if (type == ROW_STORE_TYPE)
							{ 
								/* last byte for row store, no continuation bit */
								val |= c << 42;
							}
							else
							{
								...
								if (c & 0x80)
								{
									...
									if (c & 0x80)
									{
										...
										if (c & 0x80)
										{
											/* last byte for estore ,no continuation bit */
											c = *(p++);
											val |= c << 63;
										}
									}
