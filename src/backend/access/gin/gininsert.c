...
ginbuild(Relation heap, Relation index, IndexInfo *indexInfo)
{
	...
	START_CRIT_SECTION();
	GinInitMetabuffer(MetaBuffer, RelationHasChecksum(index));
	MarkBufferDirty(MetaBuffer);
	GinInitBuffer(RootBuffer, GIN_LEAF, RelationHasChecksum(index));
...
ginbuildempty(Relation index)
{
	...
	GinInitMetabuffer(MetaBuffer, checksum_enabled);
	MarkBufferDirty(MetaBuffer);
	log_newpage_buffer(MetaBuffer, false, checksum_enabled);
	GinInitBuffer(RootBuffer, GIN_LEAF, checksum_enabled);
	MarkBufferDirty(RootBuffer);
	log_newpage_buffer(RootBuffer, false, checksum_enabled);
	END_CRIT_SECTION();
