...
void
GinInitPage(Page page, uint32 f, Size pageSize, bool checksum_enabled)
{
	GinPageOpaque opaque;

	PageInit(page, pageSize, sizeof(GinPageOpaqueData), checksum_enabled);
...
void
GinInitBuffer(Buffer b, uint32 f, bool checksum_enabled)
{
	GinInitPage(BufferGetPage(b), f, BufferGetPageSize(b), checksum_enabled);
}

void
GinInitMetabuffer(Buffer b, bool checksum_enabled)
{
	GinMetaPageData *metadata;
	Page		page = BufferGetPage(b);

	GinInitPage(page, GIN_META, BufferGetPageSize(b), checksum_enabled);
...
ginoptions(Datum reloptions, bool validate)
{
	...
	static const relopt_parse_elt tab[] = {
		...
		{"checksum", RELOPT_TYPE_BOOL, offsetof(GinOptions, checksum)}
	};
