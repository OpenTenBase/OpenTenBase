...
static void dataPlaceToPageLeafSplit(disassembledLeaf *leaf,
						 ...
						 Page lpage, Page rpage, bool checksum_enabled);
	...
	else
	{
		...
		*newrpage = palloc(BLCKSZ);
		...
		dataPlaceToPageLeafSplit(leaf, lbound, rbound,
								 *newlpage, *newrpage,
								 RelationHasChecksum(btree->index));
						 ...
						 Page lpage, Page rpage, bool checksum_enabled)
{
	...
	GinInitPage(lpage, GIN_DATA | GIN_LEAF | GIN_COMPRESSED, BLCKSZ, checksum_enabled);
	GinInitPage(rpage, GIN_DATA | GIN_LEAF | GIN_COMPRESSED, BLCKSZ, checksum_enabled);
					  ...
					  Page *newlpage, Page *newrpage)
{
	...
	GinInitPage(lpage, GinPageGetOpaque(oldpage)->flags, pageSize, RelationHasChecksum(btree->index));
	GinInitPage(rpage, GinPageGetOpaque(oldpage)->flags, pageSize, RelationHasChecksum(btree->index));
				  ...
				  GinStatsData *buildStats)
{
	...
	GinInitPage(tmppage, GIN_DATA | GIN_LEAF | GIN_COMPRESSED, BLCKSZ, RelationHasChecksum(index));
