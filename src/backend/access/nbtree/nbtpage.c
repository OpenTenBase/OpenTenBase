 ...
 */
void
_bt_initmetapage(Page page, BlockNumber rootbknum, uint32 level, bool checksum_enabled)
{
	BTMetaPageData *metad;
	BTPageOpaque metaopaque;

	_bt_pageinit(page, BLCKSZ, false, checksum_enabled);
		...
		for (;;)
		{
			...
			if (ConditionalLockBuffer(buf))
			{
				...
				if (PageIsNew(page))
				{
					...
					_bt_pageinit(page, BufferGetPageSize(buf), false, RelationHasChecksum(rel));
					return buf;
				}
 ...
 */
void
_bt_pageinit(Page page, Size size, bool tde, bool checksum_enabled)
{
	PageInit(page, size, sizeof(BTPageOpaqueData), checksum_enabled);
}
...
_bt_page_recyclable(Page page)
{
	 ...
	 *
	 * For now we are only expecting new page in this function because 
	 * opaque status is not checked during recursion in btvacuumpage. 
	 * It is recommended to directly deal with new page there
	 * instead of relying on checks here. Should add "Assert(!PageIsNew(page));"
	 * in here after the rework of btvacuumpage.
	 */
	if (PageIsNew(page))
		return true;
					...
					Relation heapRel)
{
	...
	if (RelationNeedsWAL(rel))
	{
		...
		xlrec_delete.h_relstore = RelationGetRelStore(heapRel);
	...
	*rightsib_empty = (P_FIRSTDATAKEY(opaque) > PageGetMaxOffsetNumber(page));
	...
	if (target != leafblkno)
	{
		if (nextchild == InvalidBlockNumber)
			ItemPointerSetInvalid(&((leafhikey)->t_tid));
		else
