...
static TransactionId _bt_check_unique(Relation rel, IndexTuple itup,
				 ...
				 uint32 *speculativeToken, List *update_ctid, 
				 bool violationOK, ItemPointer conflictTid);
...
static bool _bt_scankey_is_all_null(ScanKey scankey, int keysz);
...
_bt_doinsert(Relation rel, IndexTuple itup,
			 IndexUniqueCheck checkUnique, Relation heapRel, List *update_ctid)
{
	...
	is_unique = SearchBufferAndCheckUnique(rel, itup, checkUnique, heapRel, &element,
	                                       update_ctid, false, NULL);

	itup_scankey = element.itupScanKey;
	stack = element.btStack;
	buf = element.buffer;
	offset = element.offset;
	natts = element.indnkeyatts;
				 ...
				 bool violationOK, ItemPointer conflictTid)
{
	...
	*is_unique = true;
	...
	/*
	 * All NULLs tuple like (NULL, NULL) will not cause conflict
	 * for both PG and opentenbase_ora
	 */
	if (_bt_scankey_is_all_null(itup_scankey, natts))
	...
	for (;;)
	{
		...
		if (offset <= maxoff)
		{
			...
			if (!ItemIdIsDead(curitemid))
			{
				...
				/*
				 * The composite unique index is compatible with opentenbase_ora's behavior,
				 * two (1,NULL) will cause conflict.
				 */
				if (ORA_MODE && natts > 1)
				{
					if (_bt_compare(rel, natts, itup_scankey, page, offset) != 0)
				 ...
				 */
				else if (!_bt_isequal(itupdesc, page, offset, natts, itup_scankey))
					break;		/* we're past all the equal tuples */
		...
		else
		{
			...
			if (ORA_MODE && natts > 1)
			{
				if (_bt_compare(rel, natts, itup_scankey, page, P_HIKEY) != 0)
					break;		/* we're past all the equal tuples */
			}
			else if (!_bt_isequal(itupdesc, page, P_HIKEY,
							 natts, itup_scankey))
				break;
		  ...
		  bool newitemonleft)
{
	...
	_bt_pageinit(leftpage, BufferGetPageSize(buf), false, RelationHasChecksum(rel));
...
bool 
SearchBufferAndCheckUnique(Relation rel, IndexTuple itup, IndexUniqueCheck checkUnique, Relation heapRel,
   		BTCheckElement* element, List *update_ctid, bool violationOK, ItemPointer conflictTid)
{
    bool		is_unique = false;
	int			natts = rel->rd_rel->relnatts;
	ScanKey		itup_scankey;
...
top:
	...
	if (checkUnique != UNIQUE_CHECK_NO)
	{
		...
		xwait = _bt_check_unique(rel, itup, heapRel, buf, offset, itup_scankey, checkUnique,
		                         &is_unique, &speculativeToken, update_ctid,
		                         violationOK, conflictTid);
...
static bool
_bt_scankey_is_all_null(ScanKey scankey, int keysz)
{
	int			i;
	...
	/* if we get here, the keys are all NULLs */
	return true;
}
