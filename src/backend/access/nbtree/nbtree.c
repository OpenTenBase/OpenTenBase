...
btbuild(Relation heap, Relation index, IndexInfo *indexInfo)
{
	 ...
	 * unless we are in boostrap mode, index creatation overrides.
	 */
	if (!IsBootstrapProcessingMode() && RelationGetNumberOfBlocks(index) != 0)
...
btbuildempty(Relation index)
{
	...
	_bt_initmetapage(metapage, P_NONE, 0, checksum_enabled);
	...
	log_newpage(&RelationGetSmgr(index)->smgr_rnode.node, INIT_FORKNUM,
				BTREE_METAPAGE, metapage, false, checksum_enabled);
		 ...
		 IndexInfo *indexInfo)
{
	...
	result = _bt_doinsert(rel, itup, checkUnique, heapRel, 
						indexInfo ? indexInfo->eu_update_ctid : NULL);
