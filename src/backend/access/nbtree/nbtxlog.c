...
_bt_restore_meta(XLogReaderState *record, uint8 block_id)
{
	...
	_bt_pageinit(metapg, BufferGetPageSize(metabuf), false, record->checksum_enabled);
...
btree_xlog_split(bool onleft, bool isroot, XLogReaderState *record)
{
	...
	_bt_pageinit(rpage, BufferGetPageSize(rbuf), false, record->checksum_enabled);
	...
	if (XLogReadBufferForRedo(record, 0, &lbuf) == BLK_NEEDS_REDO)
	{
		...
		newlpage = PageGetTempPageCopySpecial(lpage, record->checksum_enabled);
...
btree_xlog_vacuum(XLogReaderState *record)
{
	...
	if (HotStandbyActiveInReplay() && BlockNumberIsValid(xlrec->lastBlockVacuumed))
	{
		...
		for (blkno = xlrec->lastBlockVacuumed + 1; blkno < thisblkno; blkno++)
		{
			 ...
			 */
			buffer = XLogReadBufferExtended_TDE(thisrnode, MAIN_FORKNUM, blkno,
											RBM_NORMAL_NO_LOG, record->isTDE, record->checksum_enabled);
			if (BufferIsValid(buffer))
			{
...
btree_xlog_delete_get_latestRemovedXid(XLogReaderState *record)
{
	...
	ibuffer = XLogReadBufferExtended(rnode, MAIN_FORKNUM, blkno, RBM_NORMAL, record->checksum_enabled);
	...
	for (i = 0; i < xlrec->nitems; i++)
	{
		...
		hbuffer = XLogReadBufferExtended(xlrec->hnode, MAIN_FORKNUM, hblkno, RBM_NORMAL_NO_LOG, record->checksum_enabled);
...
btree_xlog_mark_page_halfdead(uint8 info, XLogReaderState *record)
{
	...
	_bt_pageinit(page, BufferGetPageSize(buffer), false, record->checksum_enabled);
	pageop = (BTPageOpaque) PageGetSpecialPointer(page);
...
btree_xlog_unlink_page(uint8 info, XLogReaderState *record)
{
	...
	_bt_pageinit(page, BufferGetPageSize(target), false, record->checksum_enabled);
	pageop = (BTPageOpaque) PageGetSpecialPointer(page);
	...
	if (XLogRecHasBlockRef(record, 3))
	{
		...
		_bt_pageinit(page, BufferGetPageSize(leafbuf), false, record->checksum_enabled);
		pageop = (BTPageOpaque) PageGetSpecialPointer(page);
