...
static Page _bt_blnewpage(uint32 level, bool tde, bool checksum_enabled);
...
_bt_leafbuild(BTSpool *btspool, BTSpool *btspool2)
{
	...
	wstate.btws_use_wal = XLogIsNeeded() && (RelationNeedsWAL(wstate.index) ||
							RELATION_IS_GLOBAL_TEMP(wstate.index));
 ...
 */
static Page
_bt_blnewpage(uint32 level, bool tde, bool checksum_enabled)
{
	Page		page;
	BTPageOpaque opaque;
	...
	_bt_pageinit(page, BLCKSZ, tde, checksum_enabled);
...
{
	char	   *bufToWrite;
	bool		checksum_enabled = RelationHasChecksum(wstate->index);

	/* Ensure rd_smgr is open (could have been closed by relcache flush!) */
	RelationOpenSmgr(wstate->index);

	PageSetChecksumFlag(page, checksum_enabled);
	...
	PageSetChecksumInplace(bufToWrite, blkno);
...
_bt_pagestate(BTWriteState *wstate, uint32 level)
{
	...
	state->btps_page = _bt_blnewpage(level, false, RelationHasChecksum(wstate->index));
...
_bt_buildadd(BTWriteState *wstate, BTPageState *state, IndexTuple itup)
{
	...
	if (pgspc < itupsz || (pgspc < state->btps_full && last_off > P_FIRSTKEY))
	{
		...
		npage = _bt_blnewpage(state->btps_level, false, RelationHasChecksum(wstate->index));
...
_bt_uppershutdown(BTWriteState *wstate, BTPageState *state)
{
	...
	_bt_initmetapage(metapage, rootblkno, rootlevel, RelationHasChecksum(wstate->index));
