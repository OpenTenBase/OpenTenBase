...
int advance_verification_action = VERIFY_ACTION_ERROR;
	...
	*isnull = false;
	...
	switch (attnum)
	{
		...
		case MaxTransactionIdAttributeNumber:
			...
			break;
		case MinCommandIdAttributeNumber:
			result = CommandIdGetDatum(HeapTupleHeaderGetRawCmin(tup->t_data));
			break;
		case MaxCommandIdAttributeNumber:
			result = CommandIdGetDatum(HeapTupleHeaderGetRawCmax(tup->t_data));
			break;
		case TableOidAttributeNumber:
		...
		case GlobalTidAttributeNumber:
			elog(ERROR, "GlobalTidAttributeNumber %d does not support yet!", attnum);
			break;
...
heap_copytuple(HeapTuple tuple)
{
	...
	/* Avoid inconsistent values ​​due to alignment */
	memset(newTuple, 0, HEAPTUPLESIZE);
			 ...
			 TupleDesc tupleDesc)
{
	...
	if (targetHeapTuple)
	{
		...
		*targetHeapTuple = (HeapTuple) palloc0(HEAPTUPLESIZE + len);
		...
		(*targetHeapTuple)->t_xc_node_id = sourceTuple->t_xc_node_id;
		ItemPointerSetInvalid(&((*targetHeapTuple)->t_self));

		targetTHeader->t_infomask = sourceTHeader->t_infomask;
#ifdef _PG_ORCL_
		targetTHeader->t_infomask3 = sourceTHeader->t_infomask3;
#endif
		...
		HeapTupleHeaderSetShardId(targetTHeader, HeapTupleGetShardId(sourceTuple));
		if (targetNullLen > 0)
...
#ifdef _PG_ORCL_
		(*targetMinimalTuple)->t_infomask3 = sourceTHeader->t_infomask3;
#endif
		(*targetMinimalTuple)->t_shardid = sourceTHeader->t_shardid;
					...
					)
{
	...
	/*
	 * T_bits describes the bitmap relationship of NULL, and its actual length is variable
	 * depending on the number of columns (attributes & table with oid or not & table with rowid or not).
	 * Therefore, it is necessary to determine whether the current hoff is out of bounds to prevent accidents.
	 */
	if (len > UINT8_MAX)
	{
		ereport(ERROR,
				(errcode(ERRCODE_TOO_MANY_COLUMNS),
				 errmsg("number of columns exceeds limit (%d) when a null field is included.",
					 numberOfAttributes)));
	}
		...
		case SetFlag_PlainShard:
			{
				...
				Oid   *type = (Oid *)palloc(sizeof(Oid) * ndiscols);
				
				if (ndiscols > 0)
				{
					for (i = 0; i < ndiscols; i++)
					{
						if (discolnums[i] < 1 || discolnums[i] > tupleDescriptor->natts)
						{
							elog(ERROR, "AttrNum[%d] of distributed key is invalid, ", discolnums[i]);
						}
						type[i] = TupleDescAttr(tupleDescriptor, discolnums[i] - 1)->atttypid;
						value[i] = values[discolnums[i] - 1];
						isdisnull[i] = isnull[discolnums[i] - 1];
					}
				}
		
				shardId = EvaluateShardId(type, isdisnull, value, ndiscols);
						  ...
						  bool *replIsnull)
{
	...
	if (IS_CENTRALIZED_MODE && ShardIDIsValid(HeapTupleGetShardId(tuple)))
		HeapTupleSetShardId(newTuple, HeapTupleGetShardId(tuple));
 ...
 * The result is allocated in the current memory context.
 * 
 * Notice: if you intend to change this function, do the same thing for
 *      heap_minimal_tuple_header_size and heap_form_minimal_tuple_ptr below.
 */
MinimalTuple
						...
						bool *isnull)
{
	...
	/*
	 * T_bits describes the bitmap relationship of NULL, and its actual length is variable
	 * depending on the number of columns (attributes).
	 * Therefore, it is necessary to determine whether the current hoff is out of bounds to prevent accidents.
	 */
	if (len + MINIMAL_TUPLE_OFFSET > UINT8_MAX)
	{
		ereport(ERROR,
			(errcode(ERRCODE_TOO_MANY_COLUMNS),
			 errmsg("number of columns exceeds limit (%d) when a null field is included.",
...
/*
 * Calculate header offset of a minimal tuple.
 * 
 * Notice: KEEP THIS FUNCTION SYNC WITH heap_form_minimal_tuple
 */
Size
heap_minimal_tuple_header_size(TupleDesc tupleDescriptor, bool hasnull)
{
	Size        len;
	int			numberOfAttributes = tupleDescriptor->natts;
...
/*
 * heap_form_minimal_tuple_ptr
 *      like heap_form_minimal_tuple but given a pointer as result, and then
 *		construct a MinimalTuple from the given values[] and isnull[] arrays,
 *		which are of the length indicated by tupleDescriptor->natts.
 *		
 *		hasnull, total size, header offset are also given as args.
 *
 * tuple_ptr should allocated by the caller. For now, it's used for forward
 * node sending data. Construct minimal tuple directly into share-memory buffer
 * can obviously reduce memory copy. So for transferring purpose, we need to
 * ensure the page will not go beyond BLCKSZ, calculate total size outside this
 * function and pass in as another arg.
 * 
 * Notice: KEEP THIS FUNCTION SYNC WITH heap_form_minimal_tuple
 */
void
heap_form_minimal_tuple_ptr(TupleDesc tupleDescriptor,
                            Datum *values,
                            bool *isnull,
                            bool hasnull,
                            Size len,   /* total length including hoff */
                            int hoff,   /* offset of header */
                            MinimalTuple *result)
{
	MinimalTuple tuple = *result;
	int			numberOfAttributes = tupleDescriptor->natts;
...
#ifdef _MLS_
void
slot_deform_tuple_extern(void *slot, int natts)
{
...
void
heap_tuple_set_shardid(HeapTuple tup, void *tupleslot, AttrNumber *discolnums, int ndiscols, Oid relid)
{
	int   shardId;
	Datum *value = (Datum *)palloc(sizeof(Datum) * ndiscols);
	bool  *isdisnull = (bool *)palloc(sizeof(bool) * ndiscols);
	...
	if (ndiscols > 0)
	{
		...
		for (i = 0; i < ndiscols; i++)
		{
			if (discolnums[i] < 1 || discolnums[i] > slot->tts_tupleDescriptor->natts)
			{
				elog(ERROR, "AttrNum[%d] of distributed key is invalid, ", discolnums[i]);
			}
			
			type[i] = TupleDescAttr(slot->tts_tupleDescriptor, discolnums[i] - 1)->atttypid;
			value[i] = slot->tts_values[discolnums[i] - 1];
			isdisnull[i] = slot->tts_isnull[discolnums[i] - 1];
		}
	}	
...
/*
 * Validate the shardid before the tuple is written
 * to page.
 *
 * raise an error when an invalid shardid is found
 */
void
heap_tuple_validate_shardid(Relation relation, HeapTuple tuple)
{
	Oid	nodeOid = 0;

	if (advance_verification_action == VERIFY_ACTION_NONE)
		return;

	if (relation == NULL || tuple == NULL)
	{
		elog(WARNING, "Found NULL pointer when validating the shardid, relation: %p, tuple: %p",
			 relation, tuple);
		return;
	}

	/* only the shard table should be checked */
	if (!RelationIsSharded(relation))
		return;

	if (HeapTupleGetShardId(tuple) == InvalidShardID ||
		advance_verification_action == VERIFY_ACTION_TEST)
	{
		int error_level = 0;
		int saved_stacktrace_level = error_stacktrace_level;

		/* 
		 * Test mode uses LOG to print test messages, this option is 
		 * only for developing purposes to test the output format and 
		 * the potential pointer issues, and it is not open to the customer. 
		 */
		if (advance_verification_action == VERIFY_ACTION_TEST)
			error_level = LOG;
		else if (advance_verification_action >= VERIFY_ACTION_WARNING)
			error_level =  advance_verification_action;

		/*
		 * For printing stack when error_level is ERROR
		 * save/restore the stack trace level before/after change it.
		 */
		error_stacktrace_level = STACKTRACE_LOG;

		/* Report invalid sharidid issue */
		ereport(error_level,
				(errcode(ERRCODE_DATA_CORRUPTED),
						errmsg("The shardID of the tuple being written is invalid!\n"),
						errdetail("\n=== FOUND AN ISSUE WHEN STORING THE TUPLE ===\n"
								  "Shardid is [**%u**], table oid: %u, spcNode: %u,\n"
								  "dbNode: %u, relNode: %u.\n"
								  "=== PLEASE REPORT THIS ERROR TO TENCENT!! ===\n",
								  HeapTupleGetShardId(tuple),
								  tuple->t_tableOid,
	...
	else if (IS_PGXC_DATANODE && MyLogicalRepWorker == NULL)
	{
		...
		nodeOid = GetNodeOidByShardid(HeapTupleGetShardId(tuple));

		if (PGXCNodeOid == InvalidOid)
		...
		if (nodeOid != PGXCNodeOid)
		{
			...
			elog(advance_verification_action,
				"shardId %d, should not be inserted to this node, correct nodeoid is %u, relation is %s",
					HeapTupleGetShardId(tuple), nodeOid, RelationGetRelationName(relation));
 ...
 */
void
slot_deform_datarow_internal(TupleTableSlot *slot, char *cur, bool encoding)
{
	int natts;
	int i;
	int 		col_count;
	...
	oldcontext = MemoryContextSwitchTo(slot->tts_drowcxt);
	...
	for (i = 0; i < natts; i++)
	{
		...
		else
		{
			...
			/* convert buffer data to server encoding if needed */
			if (encoding)
				pstring = pg_client_to_server(buffer->data, len);
			else
				pstring = buffer->data;

			/*
			 * XML is a somewhat special data type. It is non-idempotent.
			 * That is to say, inputfunction(outputfunction(inputfunction(xmlstring))) does not always succeed,
			 * because xml_in always checks the integrity of xmlstring,
			 * but some functions returned xml is not wellformed, such as xmlagg.
			 * Therefore, when we get the XML data returned by DN,
			 * there is no need to perform integrity verification on it.
			 */
			if (ORA_MODE && (slot->tts_attinmeta->attinfuncs + i)->fn_oid == XMLINOID)
			{

				slot->tts_values[i] = XmlInputFunctionCall(slot->tts_attinmeta->attinfuncs + i,
									 pstring,
...
slot_deform_datarow(TupleTableSlot *slot)
{
	...
	slot_deform_datarow_internal(slot, slot->tts_datarow->msg, false);
}

void
slot_deform_datarow_encoding(TupleTableSlot *slot)
{
	Assert(slot->tts_datarow);
	slot_deform_datarow_internal(slot, slot->tts_datarow->msg, true);
}
#endif
