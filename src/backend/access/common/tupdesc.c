...
/*
 * tupleDescIsSame
 *   If the signature of tuple descriptor is same on attribute number, attribute
 * name/type, etc.
 */
bool
tupleDescIsSame(TupleDesc tupdesc1, TupleDesc tupdesc2, bool ignore_attname)
{
	int			i;
				   ...
				   int attdim)
{
	...
	att->atttranscomp = COMPRESSION_TYPE_NO;
	att->attuniquestore		= false;
	assign_attribute_alignedlen(desc, attributeNumber - 1);
#endif
...
static int
UpgradeAdaptAttr(Oid atttypid, ColumnDef *entry)
{
	int attdim;
	if (upgrade_mode != NON_UPGRADE &&
		OidIsValid(inplace_upgrade_next_heap_pg_class_oid) &&
		inplace_upgrade_next_heap_pg_class_oid < FirstBootstrapObjectId)
	{
		char typcategory;
		bool type_tmp = false;
 ...
 */
TupleDesc
BuildDescForRelation(List *schema, char	relstore, int default_compress_id, bool inh)
{
	int			natts;
	AttrNumber	attnum;
	ListCell   *l;
	...
	foreach(l, schema)
	{
		...
		{
			/*
			 * The 'entry->ptr' refers to a dropped column from a parent table,
			 * either for table partitions or a meta global temporary table. Copying
			 * it to the new table ensures that the schema remains consistent with
			 * the table being imitated.
			 */
			if (entry->ptr)
			{
				...
				ereport(ERROR,
					(errcode(ERRCODE_INVALID_TABLE_DEFINITION),
					 errmsg("dropped column cannot be copied due to an invalid pointer")));
			continue;
		}
#endif
		attname = entry->colname;
		typenameTypeIdAndMod(NULL, entry->typeName, &atttypid, &atttypmod, true);
...
#ifdef __OPENTENBASE_C__
		ret = GetColumnEncodingInfo(entry->encoding, &att->atttranscomp);
		if (!ret)
		{
			att->atttranscomp = default_compress_id;
		}
		if (RELSTORE_ROW == relstore && att->atttranscomp)
		{
			if (inh)
			{
				/* attr inherits from parent table, skip compression */
				att->atttranscomp = COMPRESSION_TYPE_NO;
			}
			else
			{
				/* when create row table, compress is not supported */
				ereport(ERROR,
						(errcode(ERRCODE_INVALID_TABLE_DEFINITION),
						errmsg("column compress not supported in noncolumn table")));
			}
		}
#endif
	}
...
bool 
GetColumnEncodingInfo(List *encoding, char *compression_id)
{
	if (encoding)
	{
		ListCell   *cell = NULL;
		*compression_id = COMPRESSION_TYPE_LOW;
		
		foreach(cell, encoding)
		{
			DefElem    *defel = (DefElem *) lfirst(cell);			
			
			elog(ERROR, "encoding option \"%s\" not recognized", defel->defname);
		}
...
void
BitmapClear(int bit, Bitmap *bitmap)
{
	Bitmap		mask = ~(((Bitmap) 1) << (bit % BITMAP_RADIX));

	bitmap[bit / BITMAP_RADIX] &= mask;
}

bool
BitmapTest(int bit, Bitmap *bitmap)
{
	int			index = (bit) >> BITMAP_RADIX_BIT;
	int			pos = (bit) & BITMAP_RADIX_MASK;

	return ((bitmap[index] >> pos) & 1);
}

void
assign_attribute_alignedlen(TupleDesc tupleDesc, int attno)
{
	Form_pg_attribute att = TupleDescAttr(tupleDesc, attno);
	int			attlen = att->attlen;

	if (attlen < 0)
	{
		attlen = AttrMaxLength(att);
	}

	if (attlen > 0 && attlen < MaxFixedDataLen)
	{
		att->attcollen = AttAligned(attlen);
	}
	else if (attlen > INT16_MAX)
	{
		att->attcollen = INT16_MAX;
	}
	else
	{
		att->attcollen = attlen;
	}

}

void
SerializeDescriptor(StringInfo buf, TupleDesc typeinfo)
{
	int i, valid_natts;

	valid_natts = typeinfo->natts;
	...
	appendStringInfo(buf, ":%d", valid_natts);
	for (i = 0; i < typeinfo->natts; i++)
	{
		Form_pg_attribute att = TupleDescAttr(typeinfo, i);
		...
		/*
		 * Send the type name from a Postgres-XC backend node. This preserves
		 * from OID inconsistencies as architecture is shared nothing.
		 *
		 * We need to add the namespace name as typename 's prefix for opentenbase_ora
		 * compatibility, so we could distinguish types with the same name in
		 * different namespaces.
		 */
		typename = get_typenamespace_typename(atttypid, true);
		appendStringInfo(buf, ":%s", quote_identifier(NameStr(att->attname)));
		appendStringInfo(buf, ":%s", quote_identifier(typename));
		appendStringInfo(buf, ":%d", atttypmod);
	}
...
TupleDesc
DeserializeDescriptor(char *str, Size *len)
{
	TupleDesc		result;
	StringInfoData	buf;
	char   *ptr = str;
	...
	result = CreateTemplateTupleDesc(natts, false);
	for (i = 0; i < natts; i++)
	{
		char	*attname;
		char	*typname;
		...
		get_next;
		attname = pstrdup(buf.data);
		get_next;
		typname = pstrdup(buf.data);
		get_next;
		atttypmod = atoi(buf.data);

		/* Get the OID type and mode type from typename */
		parseTypeString(typname, &oidtypeid, NULL, false);

		TupleDescInitEntry(result, i + 1, attname, oidtypeid, atttypmod, 0);
	}
...
error:
	ereport(ERROR,
			(errcode(ERRCODE_INVALID_TEXT_REPRESENTATION),
				errmsg("malformed tuple desc literal: \"%s\"", ptr)));
	return NULL;
}
