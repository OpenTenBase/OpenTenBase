...
ResetSession(void)
{
	if (CurrentSession)
	{
		if (CurrentSession->segment && CurrentSession->area)
		{
			/*
			 * some RecordCache's context were allocated from CurrentSession's shared_typmod_registry
			 * need to destroy them when reset session
			 */
			destroy_record_cache_context();
			DetachSession();
		}
		else
		{
