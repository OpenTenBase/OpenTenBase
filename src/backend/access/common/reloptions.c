...
/* END_OPENTENBASE_ORA */
	{
		{
			"extent",
			"enable a table organized with extent.",
	...
	},
	{
		{
			"checksum",
			"enable a table with checksum.",
			RELOPT_KIND_HEAP | RELOPT_KIND_PARTITIONED | RELOPT_KIND_INDEX,
	...
	{
		{
			...
			"Minimum number of tuple bulk loads prior to gathering.",
			RELOPT_KIND_HEAP,
			ShareUpdateExclusiveLock
		},
		-1, 0, INT_MAX
...
static relopt_string stringRelOpts[] =
{
	...
	{
		"compression",
			"which compression level applied to, or not compressed", 
			RELOPT_KIND_HEAP | RELOPT_KIND_PARTITIONED,
			AccessExclusiveLock
		},
		3,
...
#endif
	{
		{
			"rel_parallel_dml",
			"rel parallel dml safety",
			RELOPT_KIND_HEAP | RELOPT_KIND_PARTITIONED,
			AccessExclusiveLock
		},
		10,
		false,
		ValidateStrOptParallelDml,
		"default",
	},
	/* list terminator */
	{{NULL}}
	...
	if (PointerIsValid(DatumGetPointer(oldOptions)))
	{
		...
		for (i = 0; i < noldoptions; i++)
		{
			...
			foreach(cell, defList)
			{
				...
				{
					if (pg_strncasecmp(text_str, "checksum", kw_len) == 0)
					{
						int		oldvalue_len = text_len - kw_len - 1;
						char	*oldvalue = (char *) palloc0(oldvalue_len + 1);
						char 	*newvalue;

						snprintf(oldvalue, oldvalue_len + 1, "%s", text_str + kw_len + 1);
						if (def->arg != NULL)
							newvalue = defGetString(def);
						else
							newvalue = "true";

						if ((pg_strcasecmp(oldvalue, "off") == 0 ||
							pg_strcasecmp(oldvalue, "no") == 0 ||
							pg_strcasecmp(oldvalue, "false") == 0) &&
							(pg_strcasecmp(newvalue, "on") == 0 ||
							pg_strcasecmp(newvalue, "yes") == 0 ||
							pg_strcasecmp(newvalue, "true") == 0))
						{
							pfree(oldvalue);
							ereport(ERROR,
									(errcode(ERRCODE_SYNTAX_ERROR),
									errmsg("Checksum can not be altered from OFF to ON")));
						}
						pfree(oldvalue);
					}
					break;
...
#endif
#ifdef __OPENTENBASE_C__
		if(pg_strcasecmp(options[i].gen->name, "orientation") == 0)
			continue;

		if(pg_strcasecmp(options[i].gen->name, "compression") == 0)
			continue;
		
		if(pg_strcasecmp(options[i].gen->name, "compress_level") == 0)
			continue;
#endif
...
#ifdef __OPENTENBASE_C__
bool
is_valid_network_address(char *address)
{
	char *ip_str = strtok(address, ":");
...
/*
 * fill options for checksum relations
 */
List *
CheckAndAddChecksumOptions(List *options)
{
	ListCell	*cell = NULL;
	...
	{
		DefElem *def = (DefElem *)lfirst(cell);
		if (pg_strcasecmp(def->defname, "checksum") == 0)
		{
			hasChecksum = true;
			break;
		}
	...
	if (!page_checksums && hasChecksum)
	{
		ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
			errmsg("set relation option checksum failed"),
			errdetail("guc parameter page_checksums must be set to enable checksum feature")));
	}

	if (page_checksums && !hasChecksum)
	{
		char *default_checksum = "on";
		DefElem *def = makeDefElem("checksum",
				(Node *)makeString(default_checksum), -1);
		res = lappend(res, def);
	}
...
default_reloptions(Datum reloptions, bool validate, relopt_kind kind)
{
	...
	static const relopt_parse_elt tab[] = {
		...
		{"online_gathering_threshold", RELOPT_TYPE_INT, offsetof(StdRdOptions, online_gathering_threshold)},
		...
		{"rel_parallel_dml", RELOPT_TYPE_STRING,
		offsetof(StdRdOptions, rel_parallel_dml)},
		{"checksum", RELOPT_TYPE_BOOL,
		offsetof(StdRdOptions, checksum)}
	};
...
/* BEGIN_OPENTENBASE_ORA */
		, {"readonly", RELOPT_TYPE_BOOL,
		offsetof(ViewOptions, readonly)}
/* END_OPENTENBASE_ORA */
	};
...
/*
 * Given user options, find the first invalid option from
 * invalidOptions[invalidOptionsNum]. 
 * firstInvalidOpt will remember position of the first one.
 */
static bool
FindInvalidOption(List *userOptions, const char *invalidOptions[], 
					int invalidOptionsNum, int *firstInvalidOpt)
{
	ListCell *opt = NULL;
	int i = 0;

	for (i = 0; i < invalidOptionsNum; ++i)
	{
...
/*
 * forbid to set un-supported options
 */
static void
ForbidToSetUnsupportedOptions(List *userOptions, const char *unsupported[], 
								int unsupportedNum, const char *errorDetail)
{
	if (userOptions != NIL)
	{
...
/*
 * can not set relation options for estore table
 */
void
ForbidUserToSetDefinedOptions(List *options)
{
	...
	ForbidToSetUnsupportedOptions(options, unsupported, lengthof(unsupported), "relation");
}

char
reloptions_get_parallel_dml_safety(bytea *reloptions)
{
	char	*val;
	char	hazard = PROPARALLEL_DEFAULT;

	val = StdRdOptionsGetStringData(reloptions, rel_parallel_dml, REL_PARALLEL_DML_DEFAULT);

	if (pg_strcasecmp(val, REL_PARALLEL_DML_UNSAFE) == 0)
		hazard = PROPARALLEL_UNSAFE;
	else if (pg_strcasecmp(val, REL_PARALLEL_DML_SAFE) == 0)
		hazard = PROPARALLEL_SAFE;
