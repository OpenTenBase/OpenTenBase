 ...
 * AggregateCreate
 *
 * Note that ODCI functions should be mutual exclusive to agg functions.
 */
ObjectAddress
AggregateCreate(const char *aggName,
				...
				Oid variadicArgType,
				Oid odciinitfnOid,  	/* odci init function */
				Oid odciiterfnOid,   	/* odci iterate function */
				Oid odciterminatefnOid, /* odci terminate function */
				Oid odcimergefnOid,	 	/* odci merge function */
				List *aggtransfnName, 	/* agg trans function */
				List *aggfinalfnName,   /* agg final function */
				List *aggcombinefnName, /* agg combine function */
				List *aggserialfnName,	/* agg serial function */
				List *aggdeserialfnName, /* agg deserial function */
				List *aggmtransfnName,  /* moving agg trans function */
				List *aggminvtransfnName, /* moving agg invert trans function */
				List *aggmfinalfnName,  /* moving agg final function */
				...
				TypeName *returnType, /* odci agg return type */
				char proparallel)
{
	Relation	aggdesc;
	HeapTuple	tup;
	...
	Form_pg_proc proc;
	Oid			transfn;
	Oid			initfn = InvalidOid;	/* can be omitted, but required for odci agg */
	Oid			finalfn = InvalidOid;	/* can be omitted, but required for odci agg */
	Oid			combinefn = InvalidOid; /* can be omitted, but required for odci agg */
	...
	/* opentenbase_ora style aggregates can have at most 1 arg */
	else
	...
	/*
	 * TODO: Given the fact that plpgsql function handler cannot pass
	 *       OUT parameter out of function, slightly tweak the ODCI
	 * 		 function signature so that the aggragation context will
	 * 		 be passed out as return value. The original status code
	 *       returned by the function is omitted.
	 *
	 * ODCI function signatures:
	 * 
	 ...
	 * 		) RETURN <return_type>;
	 */
	if (odciinitfnOid != InvalidOid && odciiterfnOid != InvalidOid)
	{
		rettype = get_func_rettype(odciinitfnOid);
		if (rettype != aggTransType)
			ereport(ERROR,
					(errcode(ERRCODE_DATATYPE_MISMATCH),
					errmsg("return type of ODCI init function %s is not %s",
							get_func_name(odciiterfnOid),
							format_type_be(aggTransType))));
		
		initfn = odciinitfnOid;
		transfn = odciiterfnOid;
	}
	else
	{
	...
	else if (odciterminatefnOid != InvalidOid)
	{
		/* Supplied is ODCI style terminate function */
		fnArgs[0] = aggTransType;
		memcpy(fnArgs + 1, aggArgTypes, numArgs * sizeof(Oid));

		finalfn = odciterminatefnOid;
		finaltype = typenameTypeId(NULL, returnType);
		rettype = get_func_rettype(finalfn);
		if (rettype != finaltype)
			ereport(ERROR,
				    (errcode(ERRCODE_DATATYPE_MISMATCH),
					errmsg("return type of final function %s is not %s",
						get_func_name(finalfn),
		...
		if (finalfnExtraArgs && func_strict(finalfn))
			ereport(ERROR,
					(errcode(ERRCODE_INVALID_FUNCTION_DEFINITION),
					 errmsg("ODCI terminate function with extra arguments must not be declared STRICT")));
	}
	else
	{
		/*
		 * If no finalfn or teminatefn, aggregate result type is type of the state value
		 */
	...
	/* handle the ODCI mergefn, if supplied */
	if (odcimergefnOid != InvalidOid)
	{
		Oid			combineType;

		/*
		 * Merge function must have 2 argument, each of which is the trans
		 * type
		 */
		fnArgs[0] = aggTransType;
		fnArgs[1] = aggTransType;

		combinefn = odcimergefnOid;
		combineType = get_func_rettype(combinefn);
		...
		if (combineType != aggTransType)
			ereport(ERROR,
					(errcode(ERRCODE_DATATYPE_MISMATCH),
					 errmsg("return type of combine function %s is not %s",
							get_func_name(combinefn),
							format_type_be(aggTransType))));

		/*
		 * A merge function to combine INTERNAL states must accept nulls and
		 * ensure that the returned state is in the correct memory context.
		 */
		if (aggTransType == INTERNALOID && func_strict(combinefn))
			ereport(ERROR,
					(errcode(ERRCODE_INVALID_FUNCTION_DEFINITION),
					 errmsg("ODCI merge function with transition type %s must not be declared STRICT",
							format_type_be(aggTransType))));
	}
	 ...
	 */
	if (initfn != InvalidOid)
	{
		/* Depends on initialize function */
		referenced.classId = ProcedureRelationId;
		referenced.objectId = initfn;
