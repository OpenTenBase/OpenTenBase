...
#ifdef _PG_ORCL_
static List *getFuncsInSchema(Oid namespaceId, char type);
static List *GetPackagesInSchema(Oid namespaceId);
static List *getDeclaredPkgFuncsInSchema(List *schemas, List **pkg_nsp);
static List *getDeclaredFuncsInPkg(List *pkgnames,  List **pkg_nsp);
static void ExecGrantPackage_namespace(List *pkg_nsp, InternalGrant *pkg_istmt);
#endif
...
ExecuteGrantStmt(GrantStmt *stmt)
{
	...
	switch (stmt->targtype)
	{
...
#ifdef _PG_ORCL_
			if (ORA_MODE && stmt->objtype == ACL_OBJECT_PACKAGE)
			{
				istmt.objects = getDeclaredFuncsInPkg(stmt->objects, &pkg_nsp);
				istmt.objtype = ACL_OBJECT_PROCEDURE;
			}
			else
#endif
		...
		case ACL_TARGET_ALL_IN_SCHEMA:
			...
			if (ORA_MODE && stmt->objtype == ACL_OBJECT_PACKAGE)
			{
				istmt.objects = getDeclaredPkgFuncsInSchema(stmt->objects, &pkg_nsp);
				istmt.objtype = ACL_OBJECT_PROCEDURE;
			}
			else
#endif
	...
	ExecGrantPackage_namespace(pkg_nsp, &istmt);
}
...
static void
ExecGrantPackage_namespace(List *pkg_nsp, InternalGrant *pkg_istmt)
{
	InternalGrant	nsp_istmt;

	if (list_length(pkg_nsp) == 0)
		return;
	
	memset((char *) &nsp_istmt, 0, sizeof(InternalGrant));
	nsp_istmt.is_grant = pkg_istmt->is_grant;
	nsp_istmt.objtype = ACL_OBJECT_NAMESPACE;
	nsp_istmt.objects = pkg_nsp;
	nsp_istmt.col_privs = NIL;
	nsp_istmt.grantees = pkg_istmt->grantees;
	nsp_istmt.grant_option = pkg_istmt->grant_option;
	nsp_istmt.behavior = pkg_istmt->behavior;
	nsp_istmt.all_privs = false;
	nsp_istmt.privileges = ACL_USAGE;

	ExecGrantStmt_oids(&nsp_istmt);
}

static List *
getFuncsInSchema(Oid namespaceId, char type)
{
	ScanKeyData key[2];
	Relation	rel;
	HeapScanDesc scan;
...
static List *
GetPackagesInSchema(Oid namespaceId)
{
	List	   *pkgs = NIL;
	ScanKeyData key[1];
	Relation	rel;
	...
	while ((tuple = heap_getnext(scan, ForwardScanDirection)) != NULL)
	{
		pkgs = lappend_oid(pkgs, HeapTupleGetOid(tuple));
	}
...
static List *
getDeclaredPkgFuncsInSchema(List *schemas, List **pkg_nsp)
{
	ListCell	*l;
	List		*objs = NIL;
	...
	*pkg_nsp = NIL;
	...
	foreach(l, schemas)
	{
		Oid		namespaceId;
		List	*pkgIds;
		char	*nspname = strVal(lfirst(l));
		ListCell	*ll;

		namespaceId = LookupExplicitNamespace(nspname, false);

		pkgIds = GetPackagesInSchema(namespaceId);
		foreach(ll, pkgIds)
		{
			Oid		nspid;

			nspid = get_pkgcrrspndingns_by_id(lfirst_oid(ll));
			objs = list_concat(objs, getFuncsInSchema(nspid, PROKIND_PROCEDURE));
			objs = list_concat(objs, getFuncsInSchema(nspid, PROKIND_FUNCTION));

			*pkg_nsp = lappend_oid(*pkg_nsp, nspid);
		}
	}
...
static List *
getDeclaredFuncsInPkg(List *pkgnames, List **pkg_nsp)
{
	ListCell	*l;
	List		*objs = NIL;
	...
	*pkg_nsp = NIL;
	...
	foreach(l, pkgnames)
	{
		RangeVar	*pkgvar = (RangeVar *) lfirst(l);
		Oid		pkgId = GetPackageOid(pkgvar, false);
		Oid		nspid;

		/* Then lookup all functions under this pkgcrrspndingns */
		nspid = get_pkgcrrspndingns_by_id(pkgId);
		objs = list_concat(objs, getFuncsInSchema(nspid, PROKIND_PROCEDURE));
		objs = list_concat(objs, getFuncsInSchema(nspid, PROKIND_FUNCTION));
	...
	switch (objtype)
	{
		...
		case ACL_OBJECT_SEQUENCE:
			foreach(cell, objnames)
			{
				...
				{
					/*
					 * Support synonym on GRANT/REVOKE.
					 */
					...
					if (!OidIsValid(relOid))
					{
						...
						resolve_synonym(relvar->schemaname, relvar->relname, &objspc, &objname, &dblink);
						if (objname != NULL && dblink == NULL)
						{
							/* local object */
							relvar = (RangeVar *) copyObject(relvar);

							relvar->schemaname = objspc;
							relvar->relname = objname;
							relOid = RangeVarGetRelid(relvar, NoLock, true);
						}
					}
					if (!OidIsValid(relOid))
							...
							ereport(ERROR,
									(errcode(ERRCODE_UNDEFINED_TABLE),
									 errmsg("relation \"%s.%s\" does not exist",
											relvar->schemaname, relvar->relname)));
						else
							ereport(ERROR,
									(errcode(ERRCODE_UNDEFINED_TABLE),
				...
				seqOid = get_rel_related_seq_oid(relOid);
				if (OidIsValid(seqOid) && ORA_MODE)
					objects = lappend_oid(objects, seqOid);
				...
				if (enable_lightweight_ora_syntax && objtype == ACL_OBJECT_RELATION && relvar->inh)
				{
					Relation  rel;
					List     *inheritor_oids = NIL;

					/* already locked by RangeVarGetRelidExtended */
					rel = heap_open(relOid, AccessShareLock);
					if (rel->rd_rel->relkind == RELKIND_PARTITIONED_TABLE)
						inheritor_oids = find_all_inheritors(relOid, AccessShareLock, NULL);
					heap_close(rel, AccessShareLock);

					/* recording parent and child oids. */
					objects = list_union_oid(objects, inheritor_oids);
				}
			}
			break;
		case ACL_OBJECT_DATABASE:
...
#ifdef _PG_ORCL_
				if (ORA_MODE)
					funcid = LookupSynonFuncWithArgs(OBJECT_FUNCTION, func);
				else
#endif
		...
		case ACL_OBJECT_PROCEDURE:
			foreach(cell, objnames)
			{
				...
				if (ORA_MODE)
					procid = LookupSynonFuncWithArgs(OBJECT_PROCEDURE, func);
				else
		...
		case ACL_OBJECT_ROUTINE:
			foreach(cell, objnames)
			{
				...
				if (ORA_MODE)
					routid = LookupSynonFuncWithArgs(OBJECT_FUNCTION, func);
				else
						  ...
						  int num_col_privileges)
{
		 ...
		 curr_att++)
	{
		...
		if (curr_att == ORCL_RowIdAttributeNumber
			)
			continue;
...
static const char *const no_priv_msg[MAX_ACL_KIND] =
{
	...
	/* ACL_KIND_SYNONYM */
	gettext_noop("permission denied for synonym %s"),
};
...
static const char *const not_owner_msg[MAX_ACL_KIND] =
{
	...
	/* ACL_KIND_SYNONYM */
	gettext_noop("must be owner of synonym %s"),
	/* ACL_KIND_SPMPLAN */
	gettext_noop("must be owner of spmplan %s"),
	/* ACL_KIND_SPMPLAN_HISTORY */
	gettext_noop("must be owner of spmplan history %s")
};
...
bool
pg_synonym_ownercheck(Oid syn_oid, Oid roleid, List *name)
{
	bool	ispublic = (intVal(linitial(name)) == 1);
	...
	char		*syname;
	bool		isnull;
	Relation	rel = heap_open(SynonymRelationId, AccessShareLock);

	namespaceId = RangeVarGetCreationNamespace(rname);
	syname = rname->relname;

	if (ispublic && namespaceId != PG_PUBLIC_NAMESPACE)
        ereport(ERROR,
                (errcode(ERRCODE_UNDEFINED_OBJECT),
	...
	if (!HeapTupleIsValid(tup))
        ereport(ERROR,
                (errcode(ERRCODE_UNDEFINED_OBJECT),
                        errmsg("synonym \"%s\" does not exist",
                               syname)));
	ownerId = DatumGetObjectId(heap_getattr(tup, Anum_pg_synonym_synowner,
...
#ifdef _PG_ORCL_
bool
has_createprofile_privilege(Oid roleid)
{
	bool		result = false;
	HeapTuple	utup;
	...
	if (HeapTupleIsValid(utup))
	{
		result = ((Form_pg_authid) GETSTRUCT(utup))->rolcreateprof;
		ReleaseSysCache(utup);
	}
	return result;
}
