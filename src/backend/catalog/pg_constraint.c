...
/*
 * Test whether a constraint name is currently already used by an existing
 * constraint for current namespace.
 *
 * This is used to decide whether to accept a user-specified constraint name.
 * It is deliberately not the same test as ChooseConstraintName uses to decide
 * whether an auto-generated name is OK.
 *
 */
bool
ConstraintNameIsUsedInObjNamespace(ConstraintCategory conCat,
		Oid objNamespace, const char *conname)
{
	bool		found;
	Relation	conDesc;
	...
	while (HeapTupleIsValid(tup = systable_getnext(conscan)))
	{
		if (conCat == CONSTRAINT_RELATION)
		{
			found = true;
			break;
		}
		else if (conCat == CONSTRAINT_DOMAIN)
		{
			found = true;
			break;
		}
 ...
 */
Bitmapset *
get_relation_constraint_attnos(Oid relid, const char *conname, bool missing_ok,
							   Oid *constraintOid)
{
	...
	*constraintOid = InvalidOid;
	...
	scan = systable_beginscan(pg_constraint, ConstraintRelidIndexId, true, NULL,
							  1, skey);
	...
	while (HeapTupleIsValid(tuple = systable_getnext(scan)))
	{
		...
		*constraintOid = HeapTupleGetOid(tuple);
		...
		if (ARR_NDIM(arr) != 1 || numcols < 0 || ARR_HASNULL(arr) ||
		...
		for (i = 0; i < numcols; i++)
		{
			conattnos = bms_add_member(conattnos, attnums[i] -
