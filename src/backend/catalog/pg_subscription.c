...
GetSubscription(Oid subid, bool missing_ok)
{
	...
	sub->col_chunks = subform->subcolchunks;
	sub->dst_scno = subform->subdstscno;
	...
	if (IS_ACCESS_NODE)
	{
		...
		do
		{
			...
			{
				elog(ERROR, "subid %d not found, please check opentenbase_subscription_parallel data, if during upgrade or rollback, please ignore", subid);
			}
		} while (0);
		...
		do
		{
			...
			{
				elog(ERROR, "subid %d not found, please check opentenbase_subscription data, if during upgrade or rollback, please ignore", subid);
			}
		} while (0);
