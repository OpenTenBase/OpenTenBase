		...
		while ((tuple = systable_getnext(scan)) != NULL)
		{
			...
			switch (sdepForm->classid)
			{
				...
				case RelationRelationId:
					 ...
					 */
					ATExecChangeOwner(sdepForm->objid, newrole, true, AccessExclusiveLock, NULL);
					break;
...
GroupHasRelations(Oid group)
{
	...
	ScanKeyInit(&key[0],
				Anum_pg_shdepend_refclassid,
				BTEqualStrategyNumber, F_OIDEQ,
				ObjectIdGetDatum(PgxcGroupRelationId));
	ScanKeyInit(&key[1],
				Anum_pg_shdepend_refobjid,
				BTEqualStrategyNumber, F_OIDEQ,
	...
	{
		/* Ignore if not of the target dependency type */
		if (((Form_pg_shdepend) GETSTRUCT(tuple))->deptype != SHARED_DEPENDENCY_GROUP)
		{
			continue;
		}
		else
...
void
RecordGroupDependency(Oid relId, Oid group)
{
	Relation	sdepRel;
	TupleDesc	sdepDesc;
	HeapTuple	tup;
	...
	values[Anum_pg_shdepend_dbid - 1] = ObjectIdGetDatum(MyDatabaseId);
	values[Anum_pg_shdepend_classid - 1] = ObjectIdGetDatum(RelationRelationId);
	values[Anum_pg_shdepend_objid - 1] = ObjectIdGetDatum(relId);
	values[Anum_pg_shdepend_objsubid - 1] = Int32GetDatum(InvalidOid);

	values[Anum_pg_shdepend_refclassid - 1] = ObjectIdGetDatum(PgxcGroupRelationId);
	values[Anum_pg_shdepend_refobjid - 1] = ObjectIdGetDatum(group);
	values[Anum_pg_shdepend_deptype - 1] = CharGetDatum((char)SHARED_DEPENDENCY_GROUP);

	tup = heap_form_tuple(sdepDesc, values, nulls);
...
void
ClearGroupDependency(Oid relId, Oid group)
{
	Relation	sdepRel;

	sdepRel = heap_open(SharedDependRelationId, RowExclusiveLock);

	shdepDropDependency(sdepRel, RelationRelationId, relId, InvalidOid, false,
						PgxcGroupRelationId, group, SHARED_DEPENDENCY_GROUP);
