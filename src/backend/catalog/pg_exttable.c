		...
		Datum	locationUris)
{
	...
	/* do all the indexing work for a new catalog tuple */
	{
		CatalogIndexState indstate;

		indstate = CatalogOpenIndexes(pg_exttable_rel);
		CatalogIndexInsert(indstate, pg_exttable_tuple);
...
GetExtTableEntryIfExists(Oid relid)
{
	...
	if (isNull)
	{
		Assert(false); /* location list is always populated (url or ON X) */
	}
	else
	{
		Datum	   *elems;
		...
		if(loc_str && (IS_FILE_URI(loc_str) || IS_TDX_URI(loc_str) || IS_TDXS_URI(loc_str)))
	...
	Assert(extentry->fmtcode == 'c' || extentry->fmtcode == 't'
	...
	if (!isNull)
		Assert(extentry->rejectlimittype == 'r' || extentry->rejectlimittype == 'p');
	else
	...
	Assert(PG_VALID_ENCODING(extentry->encoding));
...
RemoveExtTableEntry(Oid relid)
{
	 ...
	 *
	 * Normal foreign table has no record in pg_exttable.
	 */
