...
static const ObjectPropertyType ObjectProperty[] =
{
    ...
    },
	{
		...
		SPMPLANOID,
		-1,
		Anum_pg_spm_plan_spmname,
		Anum_pg_spm_plan_spmnsp,
		Anum_pg_spm_plan_ownerid,
		InvalidAttrNumber,
		-1,
		false
	},
	{
		SPMPlanHistoryRelationId,
		SPMPlanHistoryOidIndexId,
		-1,
		-1,
		Anum_pg_spm_plan_history_spmname,
		Anum_pg_spm_plan_history_namespace,
		Anum_pg_spm_plan_history_ownerid,
		InvalidAttrNumber,
		-1,
		false
	},
...
#endif
	{
		...
		Anum_pg_type_object_owner,
		Anum_pg_type_object_acl,
		-1,
		true
	}
};
	...
	for (;;)
	{
		...
		switch (objtype)
		{
			...
			case OBJECT_STATISTIC_EXT:
				...
				break;
			case OBJECT_SYNONYM:
				address.classId = SynonymRelationId;
				address.objectId = get_synonym_oid(castNode(List, object), missing_ok);
				address.objectSubId = 0;
				break;
			case OBJECT_PACKAGE:
			case OBJECT_PACKAGE_BODY:
				address.classId = PackageRelationId;
				address.objectId = GetPackageOid(castNode(RangeVar, object), missing_ok);
				address.objectSubId = 0;
				break;
			default:
...
#ifdef _PG_ORCL_
/*
 * get_synonym_object_address
 *    Found the object address from synonym replacement.
 */
ObjectAddress
get_synonym_object_address(ObjectType objtype, Node *object,
				   Relation *relp, LOCKMODE lockmode, bool missing_ok)
{
	*relp = NULL;
	...
	switch (objtype)
	{
		...
		case OBJECT_MATVIEW:
			{
				List	*relname = castNode(List, object);
				ObjectAddress	addr;
				RangeVar	*relation;
				char	*objspc = NULL,
						*objname = NULL,
				...
				addr = get_object_address(objtype, object, relp, lockmode, true);
				if (addr.objectId != InvalidOid)
					return addr;

				/* Use synonym replacement */
				relation = makeRangeVarFromNameList(relname);
				resolve_synonym(relation->schemaname, relation->relname, &objspc, &objname, &dblink);
				if (objname != NULL && dblink == NULL)
				{
					if (objspc == NULL)
						relname = list_make1(makeString(objname));
					else
						relname = list_make2(makeString(objspc), makeString(objname));

					addr = get_object_address(objtype, (Node *) relname, relp, lockmode, true);
					if (addr.objectId != InvalidOid)
						return addr;
				}

				return get_object_address(objtype, object, relp, lockmode, false);
			}
		case OBJECT_FUNCTION:
		case OBJECT_PROCEDURE:
		case OBJECT_ROUTINE:
			{
				...
				addr.classId = ProcedureRelationId;
				addr.objectId = LookupSynonFuncWithArgs(objtype, castNode(ObjectWithArgs, object));
				addr.objectSubId = 0;

				return addr;
			}
		default:
			return get_object_address(objtype, object, relp, lockmode, missing_ok);
	}
}
#endif
...
/* BEGIN_OPENTENBASE_ORA */
	r_var = makeRangeVarFromNameList(object);
/* END_OPENTENBASE_ORA */
...
/* BEGIN_OPENTENBASE_ORA */
	r_var = makeRangeVarFromNameList(relname);
/* END_OPENTENBASE_ORA */

	relation = heap_openrv_extended(r_var,
									AccessShareLock,
									missing_ok);
...
pg_get_object_address(PG_FUNCTION_ARGS)
{
	...
	switch (type)
	{
        ...
        case OBJECT_PACKAGE_BODY:
            {
                objnode = (Node *) makeRangeVarFromNameList(name);
                break;
            }
		case OBJECT_LARGEOBJECT:
					   ...
					   Node *object, Relation relation)
{
	switch (objtype)
	{
		...
		case OBJECT_TSCONFIGURATION:
			...
			break;
		case OBJECT_SYNONYM:
			if (!pg_synonym_ownercheck(address.objectId, roleid, castNode(List, object)))
				aclcheck_error(ACLCHECK_NOT_OWNER, ACL_KIND_SYNONYM,
							   lsecond_node(RangeVar, castNode(List, object))->relname);
			break;
		case OBJECT_ROLE:
		...
		case OBJECT_STATISTIC_EXT:
			...
			break;
		case OBJECT_PACKAGE:
			if (!pg_pkg_ownercheck(address.objectId, roleid))
				aclcheck_error(ACLCHECK_NOT_OWNER, ACL_KIND_PACKAGE,
							   castNode(RangeVar, object)->relname);
			break;
		default:
...
getObjectDescription(const ObjectAddress *object)
{
	...
	switch (getObjectClass(object))
	{
		...
		case OCLASS_OPCLASS:
			{
				...
				appendStringInfo(&buffer, _("operator class %s for access method %s"),
								 quote_qualified_identifier_as_pg(nspname,
		...
		case OCLASS_SYNONYM:
			{
				appendStringInfo(&buffer, _("synonsym %s"),
        ...
        case OCLASS_PACKAGE:
            appendStringInfo(&buffer, _("package %s"),
                             format_package(object->objectId));
            break;
        case OCLASS_CHILDALIAS:
            appendStringInfo(&buffer, _("childalias %s"),
                             format_childalias(object->objectId));
            break;
        case OCLASS_SPMPLAN:
            appendStringInfo(&buffer, _("pg_spm_plan %s"),
                             FormatSpmplan(object->objectId));
            break;
        case OCLASS_SPMPLAN_HISTORY:
            appendStringInfo(&buffer, _("pg_spm_plan_history %s"),
                             FormatSpmplanHistory(object->objectId));
            break;
...
getObjectTypeDescription(const ObjectAddress *object)
{
	...
	switch (getObjectClass(object))
	{
		...
		case OCLASS_PGXC_GROUP:
			...
			break;
		case OCLASS_SPMPLAN:
			appendStringInfoString(&buffer, "pg_spm_plan");
			break;
		case OCLASS_SPMPLAN_HISTORY:
			appendStringInfoString(&buffer, "pg_spm_plan_history");
			break;
		case OCLASS_SYNONYM:
        ...
        case OCLASS_PACKAGE:
            ...
            break;
        case OCLASS_CHILDALIAS:
            appendStringInfoString(&buffer, "childalias");
            break;
#endif
#ifdef __AUDIT__
		case OCLASS_AUDIT_STMT:
...
#endif
		case OCLASS_TYPE_OBJECT:
			appendStringInfoString(&buffer, "type object metatable");
			break;
					   ...
					   List **objname, List **objargs)
{
	...
	switch (getObjectClass(object))
	{
		...
		case OCLASS_COLLATION:
			{
				...
				appendStringInfoString(&buffer,
									   quote_qualified_identifier_as_pg(schema,
																  NameStr(coll->collname)));
				if (objname)
		...
		case OCLASS_LANGUAGE:
			{
				...
				appendStringInfoString(&buffer,
									   quote_identifier_as_pg(NameStr(langForm->lanname)));
				if (objname)
		...
		case OCLASS_OPCLASS:
			{
				...
				appendStringInfo(&buffer, "%s USING %s",
								 quote_qualified_identifier_as_pg(schema,
															NameStr(opcForm->opcname)),
								 quote_identifier_as_pg(NameStr(amForm->amname)));
				if (objname)
		...
		case OCLASS_AM:
			{
				...
				if (!amname)
					elog(ERROR, "cache lookup failed for access method %u",
						 object->objectId);
				appendStringInfoString(&buffer, quote_identifier_as_pg(amname));
				if (objname)
		...
		case OCLASS_EXTENSION:
			{
				...
				if (!extname)
					elog(ERROR, "cache lookup failed for extension %u",
						 object->objectId);
				appendStringInfoString(&buffer, quote_identifier_as_pg(extname));
				if (objname)
		...
		case OCLASS_SYNONYM:
			{
				...
				tup = SearchSysCache1(SYNONYMOID,
									  ObjectIdGetDatum(object->objectId));
				if (!HeapTupleIsValid(tup))
					elog(ERROR, "cache lookup failed for synonym %u", object->objectId);

				syn = (Form_pg_synonym) GETSTRUCT(tup);
				appendStringInfoString(&buffer, quote_identifier(NameStr(syn->synname)));

				if (objname)
					*objname = lappend(*objname, pstrdup(NameStr(syn->synname)));
		...
		case OCLASS_SPMPLAN:
			{
				Relation             rel;
				HeapTuple            tup;
				Form_pg_spm_plan spmplan;

				rel = heap_open(SPMPlanRelationId, AccessShareLock);
				tup = SearchSysCache1(SPMPLANOID, ObjectIdGetDatum(object->objectId));
				if (!HeapTupleIsValid(tup))
					elog(ERROR, "cache lookup failed for pg_spm_plan %u", object->objectId);

				spmplan = (Form_pg_spm_plan) GETSTRUCT(tup);
				appendStringInfoString(&buffer, quote_identifier(NameStr(spmplan->spmname)));

				if (objname)
					*objname = lappend(*objname, pstrdup(NameStr(spmplan->spmname)));
		...
		case OCLASS_SPMPLAN_HISTORY:
			{
				Relation                 rel;
				HeapTuple                tup;
				Form_pg_spm_plan_history spmplan;

				rel = heap_open(SPMPlanHistoryRelationId, AccessShareLock);

				tup = get_catalog_object_by_oid(rel, object->objectId);

				if (!HeapTupleIsValid(tup))
					elog(ERROR, "could not find tuple for spmplan %u", object->objectId);

				spmplan = (Form_pg_spm_plan_history) GETSTRUCT(tup);

				appendStringInfo(&buffer, "%s on ", quote_identifier(NameStr(spmplan->spmname)));
				if (objname)
					*objname = lappend(*objname, pstrdup(NameStr(spmplan->spmname)));
		...
		case OCLASS_PGXC_CLASS:
			/* 
			 * XXX: look it up in pg_class
			 */
			{
				char    *desc;
				Relation rel = relation_open(object->objectId, NoLock);
				
				desc = pstrdup(rel->rd_rel->relname.data);
				appendStringInfoString(&buffer,
				                       desc);
		...
		case OCLASS_PGXC_NODE:
			{
				...
				appendStringInfoString(&buffer,
									   quote_identifier_as_pg(nodename));
				break;
			}
		...
		case OCLASS_PGXC_GROUP:
			{
				...
				appendStringInfoString(&buffer,
									   quote_identifier_as_pg(groupname));
				break;
			}
		case OCLASS_PACKAGE:
			{
				...
				packname = format_package(object->objectId);
				if (objname)
					*objname = list_make1(packname);
				appendStringInfoString(&buffer,
									   quote_identifier_as_pg(packname));
				break;
			}
		case OCLASS_CHILDALIAS:
			{
				...
				childalias = format_childalias(object->objectId);
				if (objname)
					*objname = list_make1(childalias);
				appendStringInfoString(&buffer,
									   quote_identifier_as_pg(childalias));
				break;
			}
#ifdef __AUDIT__
		case OCLASS_AUDIT_STMT:
...
get_object_name(ObjectType objtype, Node *object)
{
	...
	switch (objtype)
	{
		...
		case OBJECT_DOMCONSTRAINT:
			{
				...
				if (list_length(objlist) > 1)
				{
					Assert(list_length(objlist) == 2);
					obj_name = strVal(lsecond(objlist));
				}
				else
				{
					Assert(list_length(objlist) == 1);
					obj_name = strVal(linitial(objlist));
				}
			}
			break;
		case OBJECT_DATABASE:
		...
		case OBJECT_LARGEOBJECT:
			{
				char buff[64] = { 0 };
				sprintf(buff, "%u", oidparse(object));
