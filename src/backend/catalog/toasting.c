				   ...
				   Datum reloptions, LOCKMODE lockmode, bool check)
{
	...
	if (shared_relation && !IsInplaceUpgrade && !IsBootstrapProcessingMode())
		ereport(ERROR,
	...
	if (new_catalog_need_storage || !IsInplaceUpgrade)
	{
		index_create(toast_rel, toast_idxname, toastIndexOid, InvalidOid,
				 ...
				 (IsInplaceUpgrade ? 0 : INDEX_CREATE_IS_PRIMARY), 0, true, true, NULL, NULL);
	}
	else
	{
		index_create(toast_rel, toast_idxname, toastIndexOid, InvalidOid,
				 ...
				 rel->rd_rel->reltablespace,
				 collationObjectId, classObjectId, coloptions, (Datum) 0,
				 INDEX_CREATE_SKIP_BUILD, 0, true, true, NULL, NULL);
	}
	...
	if (!IsBootstrapProcessingMode())
	{
		...
		if (IsInplaceUpgrade && toastobject.objectId < FirstBootstrapObjectId)
		{
...
needs_toast_table(Relation rel)
{
	 ...
	 */
	if (IsInplaceUpgrade && rel->rd_id < FirstBootstrapObjectId)
	{
		if (OidIsValid(inplace_upgrade_next_toast_pg_class_oid))
			return true;
		else
			return false;
	}
