...
/* inplace upgrade */
Oid			inplace_upgrade_next_heap_pg_class_oid = InvalidOid;
Oid			inplace_upgrade_next_toast_pg_class_oid = InvalidOid;
bool		new_catalog_need_storage = true;
...
static FormData_pg_attribute a12 = {
	0, {"ROWID"}, RIDOID, 0, 16,
	ORCL_RowIdAttributeNumber, 0, -1, -1,
	false, 'p', 's', true, false,
#ifdef _MLS_
        false,
#endif
...
static FormData_pg_attribute a13 = {
	0, {"gtid"}, INT8OID, 0, sizeof(int64),
	GlobalTidAttributeNumber, 0, -1, -1,
	true, 'p', 'd', true, false,
#ifdef _MLS_
    false,
#endif
...
static const Form_pg_attribute SysAtt[] = {&a1, &a2, &a3, &a4, &a5, &a6, &a7, &a8, &a9 , &a10 , &a11, &a12, &a13};
#else
						...
						)
{
	...
	for (j = 0; j < (int) lengthof(SysAtt); j++)
	{
		...
		{
			/* Support to use uppercase system column name to get system attribute */
			if (ORA_MODE && att->attnum != ORCL_RowIdAttributeNumber)
			{
				if (downcase_ora_ident_strcmp(attname, NameStr(att->attname)) == 0)
					return att;
			}
			else if (strcmp(NameStr(att->attname), attname) == 0)
				return att;
		}
	}
			...
			char gtt_type)
{
	...
	if (IsInplaceUpgrade && !new_catalog_need_storage)
		create_storage = false;
...
#ifdef __OPENTENBASE_C__
						   TupleDescAttr(tupdesc, i)->attuniquestore,
#endif
...
#ifdef __OPENTENBASE_C__
	values[Anum_pg_attribute_attcollen - 1]      = Int16GetDatum(new_attribute->attcollen);
	values[Anum_pg_attribute_atttranscomp - 1]      = CharGetDatum(new_attribute->atttranscomp);
	values[Anum_pg_aggregate_attuniquestore - 1]    = BoolGetDatum(new_attribute->attuniquestore);
#endif
					  ...
					  )
{
	...
	if (relkind != RELKIND_VIEW && relkind != RELKIND_COMPOSITE_TYPE)
	{
		for (i = 0; i < (int) lengthof(SysAtt); i++)
		{
			...
			/* skip ROWID where appropriate */
			if (
				SysAtt[i]->attnum == ORCL_RowIdAttributeNumber)
				continue;
				   ...
				   Datum reloptions)
{
	...
	values[Anum_pg_class_relstashrelid - 1] = ObjectIdGetDatum(rd_rel->relstashrelid);
    values[Anum_pg_class_relstashidx - 1] = ObjectIdGetDatum(rd_rel->relstashidxid);
    values[Anum_pg_class_relregistryrelid - 1] = ObjectIdGetDatum(rd_rel->relregistryrelid);
    values[Anum_pg_class_relregistryidx - 1] = ObjectIdGetDatum(rd_rel->relregistryidxid);	
...
#endif
#ifdef __OPENTENBASE_C__
	values[Anum_pg_class_relstore - 1] = CharGetDatum(rd_rel->relstore);
	values[Anum_pg_class_relflags - 1] = Int16GetDatum(rd_rel->relflags);
#endif
	...
	values[Anum_pg_class_relpartkind - 1] = 'n';
	values[Anum_pg_class_relparent - 1] = 0;
					...
					Datum reloptions)
{
	...
	switch (relkind)
	{
		...
		case RELKIND_PARTITIONED_GLOBAL_INDEX:
			...
			new_rel_reltup->relallvisible = 0;
			/* hard code */
			new_rel_reltup->relhasindex = true;
			break;
		default:
	 ...
	 * TODO: The transaction information of the global temporary table is not in catalog now.
	 */
		...
		relkind == RELKIND_TOASTVALUE)
	{
		 ...
		 * that will do.
		 */
		new_rel_reltup->relfrozenxid = GetRecentXmin();
				...
				TupleDesc	 descriptor)
{
	...
	Oid	*nodeoids;
#ifdef _MIGRATE_
	Oid group = InvalidOid; /* max 1 groups to distribute */
#endif
#ifdef __OPENTENBASE_C__
	int ndiscols = 0;
	...
	if (distributeby)
	{
		group = GetDistributeGroup(subcluster, distributeby->disttype);	
		if(DISTTYPE_SHARD == distributeby->disttype)
		{
			if (OidIsValid(group))
			{
				if (IS_PGXC_COORDINATOR && !is_group_sharding_inited(group))
				{
					elog(ERROR, "please initialize group:%u sharding map first", group);
				}
			}
		}
	}
	...
	if (!subcluster)
	{
		if (LOCATOR_TYPE_SHARD == locatortype)
		{
			...
			group = GetDistributeGroup(subcluster, DISTTYPE_SHARD);
			if (OidIsValid(group))
			{
				n->clustertype = SUBCLUSTER_GROUP;
				n->members	   = list_make1(makeString(get_pgxc_groupname(group)));				
			}
			subcluster	   = n; 		
		}
	}
...
#ifdef _MIGRATE_
	PgxcClassCreate(relid, locatortype,
#ifdef __OPENTENBASE_C__
					ndiscols, (int *)discolnums,
#endif
	                hashalgorithm, hashbuckets, numnodes, nodeoids, group);
#else
	...
	if (LOCATOR_TYPE_SHARD == locatortype)
		RecordGroupDependency(relid, group);
				...
				TupleDesc	 descriptor)
{
	...
	GetRelationDistributionItems(relid,
								 ...
								 &hashbuckets,
#ifdef __OPENTENBASE_C__
								 &ndiscols, discolnums, 0
#endif
								 );

	/* Now OK to insert data in catalog */
	RegisterDistributeKey(relid, locatortype, ndiscols, discolnums, hashalgorithm, hashbuckets);
...
Oid 
GetDistributeGroup(PGXCSubCluster *subcluster, DistributionType dtype)
{
    Oid group_oid;
	...
	else
	{
		...
		else if (DISTTYPE_REPLICATION == dtype)
		{
			group_oid = GetDefaultGroup();
			if (!OidIsValid(group_oid))
					ereport(ERROR,
			...
			{
				Oid *dnoids = NULL;
				int nmembers = get_pgxc_groupmembers(group_oid, &dnoids);
				pfree(dnoids);
				if (nmembers == NumDataNodes)
					return group_oid;
			}
		}
	}
                            ...
                            )
{
...
#endif
		if (discolnums && ndiscols)
		{
			discolnums[0] = local_attnum;
			*ndiscols = 1;
		}
	}
	else
	{
		...
		switch (distributeby->disttype)
		{
			...
			case DISTTYPE_SHARD:
#ifdef __OPENTENBASE_C__
				len = list_length(distributeby->colname);
#else
...
#ifdef __OPENTENBASE_C__
				/* get second or more distributed column if exist */
				if (len > 0)
				{
					/* Save results */
					if (discolnums && ndiscols)
					{
						...
						for (i = 0; i < len; i++)
						{
							colname = strVal(list_nth(distributeby->colname, i));
							discolnums[i] = get_attnum(relid, colname);
							if (discolnums[i] <= 0 && discolnums[i] >= -(int) lengthof(SysAtt))
							{
								ereport(ERROR,
								        (errcode(ERRCODE_INVALID_TABLE_DEFINITION),
							...
							if (!IS_CENTRALIZED_MODE &&
								!IsTypeDistributable(TupleDescAttr(descriptor, discolnums[i] - 1)->atttypid))
							{
								ereport(ERROR,
			...
			case DISTTYPE_HASH:
				...
				if (!IS_CENTRALIZED_MODE &&
					!IsTypeHashDistributable(TupleDescAttr(descriptor, local_attnum - 1)->atttypid))
				{
					ereport(ERROR,
						(errcode(ERRCODE_WRONG_OBJECT_TYPE),
			...
			case DISTTYPE_MODULO:
				...
				if (!IS_CENTRALIZED_MODE &&
					!IsTypeModuloDistributable(TupleDescAttr(descriptor, local_attnum - 1)->atttypid))
				{
					ereport(ERROR,
						(errcode(ERRCODE_WRONG_OBJECT_TYPE),
			...
			case DISTTYPE_ROUNDROBIN:
				if (foreignDistributeType > 0)
					local_locatortype = foreignDistributeType;
				else
					local_locatortype = LOCATOR_TYPE_RROBIN;
				break;
...
GetRelationDistributionNodes(PGXCSubCluster *subcluster, int *numnodes)
{
	...
	*numnodes = 0;
	...
	if (!nodes && subcluster && subcluster->clustertype == SUBCLUSTER_GROUP)
	{
 ...
 *
 * ORA Arguments:
 *	viewId: used for previously created force view Oid
 *
 * Output parameters:
						 ...
						 ObjectAddress *typaddress)
{
	...
	{
		/*
		 * Global temp table with same name cannot be created under different
		 * schemas
		 */
			...
			!isTempOrTempToastNamespace(relnamespace))
		{
			if (!isTempOrTempToastNamespace(relnamespace))
				if (IsTempTableNameDuplicated(relname, false))
					ereport(ERROR,
							(errcode(ERRCODE_DUPLICATE_TABLE),
							 errmsg("temporary relation \"%s\" already exists in other schemas", relname)));
		}
		else if (relpersistence == RELPERSISTENCE_TEMP)
		{
			Assert(isTempOrTempToastNamespace(relnamespace));
			if (IsTempTableNameDuplicated(relname, true))
				ereport(ERROR,
						(errcode(ERRCODE_DUPLICATE_TABLE),
						 errmsg("temporary relation \"%s\" already exists in other schemas", relname)));
		}
	...
	if (!OidIsValid(relid))
	{
		...
		else if (IsInplaceUpgrade &&
				OidIsValid(inplace_upgrade_next_heap_pg_class_oid) &&
				(relkind == RELKIND_RELATION || relkind == RELKIND_SEQUENCE ||
				relkind == RELKIND_VIEW || relkind == RELKIND_COMPOSITE_TYPE ||
				relkind == RELKIND_FOREIGN_TABLE))
		{
			relid = inplace_upgrade_next_heap_pg_class_oid;
			inplace_upgrade_next_heap_pg_class_oid = InvalidOid;
		}
		else if (IsInplaceUpgrade &&
				OidIsValid(inplace_upgrade_next_toast_pg_class_oid) &&
				relkind == RELKIND_TOASTVALUE)
		{
			relid = inplace_upgrade_next_toast_pg_class_oid;
			inplace_upgrade_next_toast_pg_class_oid = InvalidOid;
		}
		else
...
#endif
#ifdef __OPENTENBASE_C__
	new_rel_desc->rd_rel->relstore   = RELSTORE_ROW;
	new_rel_desc->rd_rel->relflags   = RELFLAGS_NORMAL;
#endif
	/*
	 ...
	 */
	if (IsUnderPostmaster && !IsInplaceUpgrade && (relkind == RELKIND_RELATION ||
...
/* BEGIN_OPENTENBASE_ORA */
			/*
			 * If 'viewId' is valid, we have recorded these dependencies when first
			 * creation of the force view.
			 */
				recordDependencyOn(&myself, &referenced, DEPENDENCY_NORMAL);
...
/* END_OPENTENBASE_ORA */
	 ...
	 * on-commit action of gtt would be registered in init_gtt_storage while insert/update.
	 */
	if (oncommit != ONCOMMIT_NOOP && isTempNamespace(relnamespace))
...
heap_drop_with_catalog(Oid relid)
{
	...
	/*
	 * If the gtt is activated, then avoid drop it. This is mainly to protect
	 * drop schema cascade to remove it.
	 */
...
)
{
	...
	if (!attStruct->atthasdef)
	{
		...
		if ((rel->rd_rel->relkind == RELKIND_RELATION || rel->rd_rel->relkind == RELKIND_PARTITIONED_TABLE) && add_column_mode)
		{
						  ...
						  bool need_check_volatile)
{
	...
	foreach(cell, newColDefaults)
	{
		...
		expr = cookDefault(pstate, colDef->raw_default,
						   ...
						   NameStr(atp->attname), need_check_volatile);
			...
			bool need_check_volatile)
{
	...
	if (need_check_volatile && contain_volatile_functions(expr))
		ereport(ERROR,
				(errcode(ERRCODE_INVALID_COLUMN_REFERENCE),
				 errmsg("Adding columns with volatile defaults to replication table is prohibited due to data inconsistency risk.")));
	/*
...
RemoveStatistics(Oid relid, AttrNumber attnum)
{
	...
	if (attnum == 0)
	{
		LocalStatisticRemoveByRelid(relid);
	}
	else
	{
		LocalStatisticRemoveAttrStats(relid, attnum);
	}
}
...
heap_truncate_one_rel(Relation rel)
{
	...
	LocalStatisticSetRelStats(rel, 0, 0, 0, 0, false);
}
...
void ValidateStrOptCompression(char *val)
{
    if (pg_strcasecmp(val, COMPRESSION_NO) != 0 &&
        pg_strcasecmp(val, COMPRESSION_LOW) != 0 && pg_strcasecmp(val, COMPRESSION_MIDDLE) != 0 &&
        pg_strcasecmp(val, COMPRESSION_HIGH) != 0)
        ereport(ERROR,
                (errcode(ERRCODE_INVALID_PARAMETER_VALUE), errmsg("Invalid string for  \"COMPRESSION\" option."),
                 errdetail("Valid string are \"no\", \"low\", \"middle\", \"high\" for column compress table. ")));
}

#endif

void
ValidateStrOptParallelDml(char * val)
{
	if (!IS_CENTRALIZED_DATANODE)
		ereport(ERROR, (errcode(ERRCODE_SYNTAX_ERROR),
			errmsg("parameter \"rel_parallel_dml\" is not supported in cluster mode")));

	if (pg_strcasecmp(val, REL_PARALLEL_DML_DEFAULT) != 0 &&
		pg_strcasecmp(val, REL_PARALLEL_DML_SAFE) != 0 &&
		pg_strcasecmp(val, REL_PARALLEL_DML_UNSAFE) != 0)
		ereport(ERROR, (errcode(ERRCODE_SYNTAX_ERROR),
				errmsg("parameter \"rel_parallel_dml\" must be SAFE, UNSAFE or DEFAULT")));
	return;
}


List*
RelationGetConstraintList(Oid relOid, char constrType)
{
	Relation pg_constraint_table;
	HeapTuple pg_constraint_tuple;
	HeapTuple scan_result_tuple;
	SysScanDesc scan;
	ScanKeyData skey[2];
	int nkeys;
	List* result;
	...
	result = NIL;
	pg_constraint_table = heap_open(ConstraintRelationId, AccessShareLock);
	ScanKeyInit(&skey[0], Anum_pg_constraint_conrelid, BTEqualStrategyNumber,
		F_OIDEQ, ObjectIdGetDatum(relOid));

	if (constrType != CONSTRAINT_INVALID)
	{
	    ScanKeyInit(&skey[1], Anum_pg_constraint_contype, BTEqualStrategyNumber,
			F_CHAREQ, CharGetDatum(constrType));
	    nkeys = 2;
	    scan = systable_beginscan(pg_constraint_table, ConstraintRelidIndexId, false, NULL,
					nkeys, skey);
	...
	{
	    nkeys = 1;
	    scan = systable_beginscan(pg_constraint_table, ConstraintRelidIndexId, true, NULL,
					nkeys, skey);
	}

	while (HeapTupleIsValid(pg_constraint_tuple = systable_getnext(scan)))
	{
	    scan_result_tuple = heap_copytuple(pg_constraint_tuple);
	    result = lappend(result, scan_result_tuple);
	}

	systable_endscan(scan);
	heap_close(pg_constraint_table, AccessShareLock);
