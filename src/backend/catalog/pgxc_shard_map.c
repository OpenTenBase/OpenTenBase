...
void InitShardMap(int32 nShardGroup, int32 nNodes, Oid *nodes, int16 extend, Oid nodeGroup)
{
	...
	{		
		...
		values[Anum_pgxc_shard_map_shardclusterid - 1] = UInt16GetDatum((iShard / nNodes) % shard_cluster_num);
#endif
...
#ifdef __OPENTENBASE_C__
	new_record_repl[Anum_pgxc_shard_map_shardclusterid - 1] = true;
#endif
...
#ifdef __OPENTENBASE_C__
		new_record[Anum_pgxc_shard_map_shardclusterid - 1] = Int16GetDatum(scid);
#endif
