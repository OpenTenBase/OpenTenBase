...
/* inplace upgrade */
Oid			inplace_upgrade_next_index_pg_class_oid = InvalidOid;
						 ...
						 Oid *classObjectId)
{
	...
	for (i = 0; i < numatts; i++)
	{
		...
		if (atnum != 0)
		{
			...
			to->atttranscomp = COMPRESSION_TYPE_NO;
			to->attuniquestore	= false;
		}
		else
		{
			/* Expressional index */
			 ...
			 IndexStmt *indexstmt)
{
	...
	char		relkind;
	ObjectAddress	indexTable = InvalidObjectAddress;
	ObjectAddress	indexTableIndex = InvalidObjectAddress;
	char		gtt_type = 0;
	 ...
	 */
	if (shared_relation && !IsBootstrapProcessingMode() && !IsInplaceUpgrade)
		ereport(ERROR,
	...
	if (indexstmt && indexstmt->cross_node)
	{
		index_table_create(heapRelation,
			indexTupDesc,
			indexRelationName,
			indexInfo->ii_Unique,
			tableSpaceId,
			...
			&indexTableIndex,
			indexstmt);
		indexRelation = heap_open(indexTable.objectId, NoLock);
		indexRelationId = indexTable.objectId;
	}
	else
	{
		/*
		...
		if (!OidIsValid(indexRelationId))
		{
			...
			if (IsBinaryUpgrade)
			{
				...
				binary_upgrade_next_index_pg_class_oid = InvalidOid;
					...
					OidIsValid(inplace_upgrade_next_index_pg_class_oid))
			{
				indexRelationId = inplace_upgrade_next_index_pg_class_oid;
				inplace_upgrade_next_index_pg_class_oid = InvalidOid;
			}
			else
			{
				indexRelationId =
...
#ifdef __OPENTENBASE_C__
	indexRelation->rd_rel->relstore = RELSTORE_ROW;
#endif
...
BuildIndexInfo(Relation index)
{
	...
	ii->ii_AmCache = NULL;
	ii->eu_update_ctid = NULL;
	ii->ii_Context = CurrentMemoryContext;
	ii->gindex_oid = RelationIsCrossNodeIndex(index->rd_rel) ? 
					RelationGetRelid(index) : InvalidOid;
	return ii;
}
				 ...
				 AttrNumber *attmap, int maplen)
{
	...
	for (i = 0; i < info1->ii_NumIndexAttrs; i++)
	{
		...
		if (info2->ii_KeyAttrNumbers[i] - 1 >= 0)
		{
			if (attmap[info2->ii_KeyAttrNumbers[i] - 1] !=
				info1->ii_KeyAttrNumbers[i])
				return false;
		}
		else if (info2->ii_KeyAttrNumbers[i] != info1->ii_KeyAttrNumbers[i])
		{
			/*
			 * index uses a virtual column.
			 */
			return false;
		}
 ...
 *
 * This is used during CREATE/DROP INDEX CONCURRENTLY, ENABLE/DISABLE CONSTRAINT
 * to adjust the pg_index flags that denote the index's state.
 * Because the update is not transactional and will not roll back on error,
 * this must only be used as the last step in a transaction that has not made
 * any transactional catalog updates!
 *
...
index_set_state_flags(Oid indexId, IndexStateFlagsAction action)
{
    ...
    Assert(action == INDEX_ENABLE_SET_ALIVE ||
           action == INDEX_DISABLE_SET_DEAD ||
	...
	switch (action)
	{
		case INDEX_CREATE_SET_READY:
			...
			break;
		case INDEX_CREATE_SET_VALID_WITH_UPDATE:
		...
		case INDEX_DROP_SET_DEAD:
			...
			break;
#ifdef __OPENTENBASE__
	    case INDEX_ENABLE_SET_ALIVE:

            /*
             * Set indisready/indislive for ENABLE CONSTRAINT
             *
             * We set both indisready and indislive and reindex later.
             */
            indexForm->indisready = true;
            indexForm->indislive = true;
            break;
        case INDEX_DISABLE_SET_DEAD:

            /*
             * Clear indisready/indislive for DISABLE CONSTRAINT
             *
             * We clear both indisready and indislive, because we not only
             * want to stop updates, we want to prevent sessions from touching
             * the index at all.
			  ...
			  int options)
{
	...
	/*
	 * Avoid deadlock cause by "reindex index pg_index_indexrelid_index".
	 * Scan pg_index_indexrelid_index and load the necessary mapping to
	 * the system cache before add a AccessExclusiveLock lock for reindex.
	 * So there is no need to add a AccessShareLock lock again during
	 * two-phase commit.
	 */
	if (indexId == IndexRelidIndexId)
	{
		Oid relationId = IndexGetRelation(PgxcNodeNodeNameIndexId, false);
		if (!OidIsValid(relationId))
		{
			elog(ERROR, "get relation failed for index %u", PgxcNodeNodeNameIndexId);
		}
	}
...
reindex_relation(Oid relid, int flags, int options)
{
	...
	foreach(indexId, indexIds)
	{
		...
		bool 		isCrossNodeIndex = false;

		isCrossNodeIndex = RelationIsCrossNodeIndex(indexRel->rd_rel);
		...
		if (isCrossNodeIndex)
			reindex_relation(indexOid, flags, options);
		else
