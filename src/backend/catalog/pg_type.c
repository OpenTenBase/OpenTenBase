...
/* inplace upgrade */
Oid			inplace_upgrade_next_pg_type_oid = InvalidOid;
Oid			inplace_upgrade_next_array_pg_type_oid = InvalidOid;
char		TypeCreateType = '\0';
...
TypeShellMake(const char *typeName, Oid typeNamespace, Oid ownerId)
{
	...
	if (IsBinaryUpgrade)
	{
		...
		OidIsValid(inplace_upgrade_next_pg_type_oid))
	{
		HeapTupleSetOid(tup, inplace_upgrade_next_pg_type_oid);
		inplace_upgrade_next_pg_type_oid = InvalidOid;
	}
	 ...
	 */
	if (!IsBootstrapProcessingMode() && !IsInplaceUpgrade)
		   ...
		   Oid typeCollation)
{
	...
	isDependentType = isImplicitArray ||
		(OidIsValid(relationOid) && relationKind != RELKIND_COMPOSITE_TYPE && relationKind != RELKIND_NESTED_TABLE_TYPE);
	...
	if (IsInplaceUpgrade && TypeCreateType)
	{
		typeType = TypeCreateType;
		TypeCreateType = '\0';
	}
	...
	else
	{
		...
		else if (IsInplaceUpgrade && OidIsValid(inplace_upgrade_next_pg_type_oid))
		{
			HeapTupleSetOid(tup, inplace_upgrade_next_pg_type_oid);
			inplace_upgrade_next_pg_type_oid = InvalidOid;
		}
						 ...
						 bool rebuild)
{
	...
	if (OidIsValid(typeForm->typrelid))
	{
		...
		if (relationKind != RELKIND_COMPOSITE_TYPE 
			&& relationKind != RELKIND_NESTED_TABLE_TYPE)
	...
	if (OidIsValid(typeForm->typelem))
	{
		...
		recordDependencyOn(&myself, &referenced,
						  		isImplicitArray ? DEPENDENCY_INTERNAL : 
						   			                    DEPENDENCY_NORMAL);
	}
