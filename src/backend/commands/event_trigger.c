...
check_ddl_tag(const char *tag)
{
	...
	if (pg_strcasecmp(tag, "CREATE TABLE AS") == 0 ||
		...
		pg_strcasecmp(tag, "CREATE RESOURCE GROUP") == 0 ||
		pg_strcasecmp(tag, "DROP RESOURCE GROUP") == 0 ||
		pg_strcasecmp(tag, "AUDIT") == 0 ||
		pg_strcasecmp(tag, "NO AUDIT") == 0 ||
		pg_strcasecmp(tag, "CLEAN AUDIT") == 0 ||
		pg_strncasecmp(tag, "AUTO XACT ", 10) == 0 ||
#endif
						...
						EventTriggerData *trigdata)
{
	...
	/*
	 * Event triggers from connection type other than REMOTE_CONN_APP will be
	 * skipped to avoid duplicated execution.
	 */
	if (!IsConnFromApp())
		return NIL;
...
#ifdef USE_ASSERT_CHECKING
	{
		...
		if (event == EVT_DDLCommandStart ||
			...
			event == EVT_SQLDrop)
		{
			// TODO: delete strcmp(dbtag, "ALTER RESOURCE GROUP") != 0 when the issue is resolved.
			if (strcmp(dbgtag, "ALTER RESOURCE GROUP") != 0 && check_ddl_tag(dbgtag) != EVENT_TRIGGER_COMMAND_TAG_OK)
