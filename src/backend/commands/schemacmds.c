					...
					int stmt_location, int stmt_len)
{
	...
	if (ORA_MODE && IsSystemNamespaceName(schemaName))
		schemaName = asc_tolower(schemaName, strlen(schemaName));
	...
	if (!allowSystemTableMods && !IsInplaceUpgrade && IsReservedName(schemaName))
		ereport(ERROR,
...
RenameSchema(const char *oldname, const char *newname)
{
	...
	if (ORA_MODE && IsSystemNamespaceName(oldname))
		oldname = asc_tolower(oldname, strlen(oldname));

	if (ORA_MODE && IsSystemNamespaceName(newname))
...
AlterSchemaOwner(const char *name, Oid newOwnerId)
{
	...
	if (ORA_MODE && IsSystemNamespaceName(name))
		name = asc_tolower(name, strlen(name));
