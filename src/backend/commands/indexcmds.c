			...
			TupleDesc oldDesc)
{
	...
	*/
	...
	if (rel_is_external_table(relationId) || rel_is_hdfsfdw_table(relationId))
		ereport(ERROR,
	...
	if (rel->rd_rel->relkind != RELKIND_RELATION &&
		...
		rel->rd_rel->relkind != RELKIND_PARTITIONED_GLOBAL_INDEX &&
	...
	*/
	 ...
	 *
	 * Note：When specifying the primary key constraint name, the primary
	 * key constraint name is the created index name, and the table name
	 * is the same as the index name, resulting in an error when the same
	 * name is found in the system table pg_class when creating the index
	 * name, need to generate new index name. Maintain behavior consistent
	 * with opentenbase_ora.
	 */
	indexRelationName = stmt->idxname;

	if (indexRelationName != NULL && ORA_MODE &&
		ConstraintNameIsUsedInObjNamespace(CONSTRAINT_RELATION,
				namespaceId, indexRelationName))
	{
		ereport(ERROR,
			(errcode(ERRCODE_DUPLICATE_OBJECT),
				errmsg("constraint \"%s\" for relation \"%s\" already used by an existing constraint",
	...
	if (indexRelationName == NULL ||
			(ORA_MODE &&
			 stmt->isconstraint &&
					(strcmp(RelationGetRelationName(rel), indexRelationName) == 0 ||
						(indexRelationName != NULL && get_relname_relid(indexRelationName, namespaceId)))))
	{
	...
	if (!IsInplaceUpgrade && checksum_ddl_default)
	...
	if (IS_PGXC_COORDINATOR && (stmt->primary || stmt->unique))
	{
		...
		if (rel->rd_locator_info && !loose_constraints)
		{
			if (IsLocatorReplicated(rel->rd_locator_info->locatorType))
			{
				/* always safe */
				isSafe = true;
			}
			
			if (rel->rd_locator_info->locatorType == LOCATOR_TYPE_RROBIN)
			{
				ereport(ERROR,
		...
		if (!isSafe)
		{
			if (loose_constraints)
			{
				ereport(WARNING,
					(errcode(ERRCODE_INVALID_COLUMN_REFERENCE),
					 errmsg("Unique index of distributed table must contain "
	...
	indexInfo->ii_AmCache = NULL;
	indexInfo->ii_Context = CurrentMemoryContext;
	indexInfo->skip_scan = stmt->skip_scan;
	 ...
	 */
	if (stmt->primary && !stmt->skip_scan)
	...
	if (skip_build || stmt->concurrent || partitioned || stmt->cross_node)
	...
	indexRelationId =
		index_create(rel, indexRelationName, indexRelationId, parentIndexId,
					 ...
					 flags, constr_flags,
					 allowSystemTableMods || IsInplaceUpgrade, !check_rights,
					 &createdConstraintId,
					 stmt);

	ObjectAddressSet(address, RelationRelationId, indexRelationId);

	if (retCreatedConstraintId)
		*retCreatedConstraintId = createdConstraintId;
	...
	*/
		...
		if (!stmt->relation || stmt->relation->inh)
		{
			...
			for (i = 0; i < nparts; i++)
			{
				...
				if (!found)
				{
					...
					Oid         childCreateConstraintId = InvalidOid;
					...
					DefineIndex(childRelid, childStmt,
								...
								skip_build, quiet, &childCreateConstraintId, oldDesc);
				}
	...
	else 
	{
		...
		index_set_state_flags(indexRelationId, INDEX_CREATE_SET_VALID_WITH_UPDATE);
					  ...
					  int options)
{
	...
	while ((tuple = heap_getnext(scan, ForwardScanDirection)) != NULL)
	{
		...
		/* Skip (global) temp tables of other backends; we can't reindex them at all */
