...
DefineType(ParseState *pstate, List *names, List *parameters)
{
	 ...
	 *
	 * During upgrade, we may create pseudo type which doesn't have array type.
	 */
	if (!IsInplaceUpgrade || !TypeCreateType || TypeCreateType != TYPTYPE_PSEUDO)
...
DefineRange(CreateRangeStmt *stmt)
{
	...
	if (type_is_collatable(rangeSubtype))
	{
		...
		{
			if (ORA_MODE)
				rangeCollation = get_special_ora_obj_oid(rangeCollationName, get_collation_oid, NULL, 0);
			else
...
findRangeSubOpclass(List *opcname, Oid subtype)
{
	...
	{
		if (ORA_MODE)
			opcid = get_special_ora_obj_oid(opcname, NULL, get_opclass_oid, BTREE_AM_OID);
		else
...
AssignTypeArrayOid(void)
{
	...
	if (IsBinaryUpgrade)
	{
		...
		binary_upgrade_next_array_pg_type_oid = InvalidOid;
			...
			OidIsValid(inplace_upgrade_next_array_pg_type_oid))
	{
		type_array_oid = inplace_upgrade_next_array_pg_type_oid;
		inplace_upgrade_next_array_pg_type_oid = InvalidOid;
	}
	else
	{
...
DefineCompositeType(RangeVar *typevar, List *coldeflist)
{
	 ...
	 */
	DefineRelation(createStmt, RELKIND_COMPOSITE_TYPE, InvalidOid, &address, NULL, InvalidOid);
...
replace_domain_constraint_value(ParseState *pstate, ColumnRef *cref)
{
	...
	if (list_length(cref->fields) == 1)
	{
		...
		if (strcmp(colname, (ORA_MODE) ? "VALUE" : "value") == 0)
		{
...
AlterTypeOwner_oid(Oid typeOid, Oid newOwnerId, bool hasDependEntry)
{
	 ...
	 */
	if (typTup->typtype == TYPTYPE_COMPOSITE)
		ATExecChangeOwner(typTup->typrelid, newOwnerId, true, AccessExclusiveLock, NULL);
	else
