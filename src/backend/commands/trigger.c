...
int         g_trig_exec_mode = TRIG_EXECMODE_AUTO;

/* if trigger is on table level */
bool		trig_on_table = false;
			  ...
			  bool isInternal)
{
	...
	/* if stmt->relation is not NULL, set trig_on_table to true */
	trig_on_table = !(stmt->row);
		...
		rel->rd_rel->relkind == RELKIND_PARTITIONED_TABLE)
	{
		...
		if ((stmt->row || !IS_CENTRALIZED_MODE) &&
			rel->rd_rel->relkind == RELKIND_PARTITIONED_TABLE)
			ereport(ERROR,
					(errcode(ERRCODE_WRONG_OBJECT_TYPE),
		...
		foreach(lc, varList)
		{
			...
			switch (var->varno)
			{
				case PRS2_OLD_VARNO:
					...
					if (!ORA_MODE && TRIGGER_FOR_INSERT(tgtype))
						ereport(ERROR,
								(errcode(ERRCODE_INVALID_OBJECT_DEFINITION),
					...
					if (!TRIGGER_FOR_ROW(tgtype))
						ereport(ERROR,
								(errcode(ERRCODE_INVALID_OBJECT_DEFINITION),
								 errmsg("statement trigger's WHEN condition cannot reference column values"),
								 parser_errposition(pstate, var->location)));
					if (!ORA_MODE && TRIGGER_FOR_DELETE(tgtype))
						ereport(ERROR,
								(errcode(ERRCODE_INVALID_OBJECT_DEFINITION),
	...
	if (!IS_CENTRALIZED_DATANODE &&
		g_trig_exec_mode == TRIG_EXECMODE_AUTO &&
		(rel->rd_rel->relkind == RELKIND_RELATION ||
		 rel->rd_rel->relkind == RELKIND_PARTITIONED_TABLE) &&
		RelationGetLocInfo(rel) != NULL &&
		RelationGetLocInfo(rel)->locatorType == LOCATOR_TYPE_SHARD)
	{
		FuncCheckData trigckdata = {.type = T_FuncCheckData,
									.pullup = false,
									.relid = RelationGetRelid(rel),
									.tgtype = tgtype};
		(void)CheckFuncPushdown(funcoid, (Node *)&trigckdata);
	}
								 ...
								 void *arg)
{
	...
	if (!allowSystemTableMods && !IsInplaceUpgrade && IsSystemClass(relid, form))
		ereport(ERROR,
				(errcode(ERRCODE_INSUFFICIENT_PRIVILEGE),
				 errmsg("permission denied: \"%s\" is a system catalog",
						rv->relname)));
...
SetTriggerFlags(TriggerDesc *trigdesc, Trigger *trigger)
{
	...
	{
		trigdesc->trig_pullup |= tgtype;
	}
	if (!trigdesc->trig_pullup)
	{
		trigdesc->trig_pullup = contain_user_defined_functions_checker(trigger->tgfoid, NULL, NULL);
	}
}
...
ExecBSInsertTriggers(EState *estate, ResultRelInfo *relinfo)
{
	...
	if (!should_pgxc_fire_trigger(trigdesc, relinfo->ri_RelationDesc, CMD_INSERT))
		return;
#endif
...
#ifdef __OPENTENBASE__
	/* should I fire the trigger? */
	if (!should_pgxc_fire_trigger(trigdesc, relinfo->ri_RelationDesc, CMD_DELETE))
		return;
#endif
	LocTriggerData.type = T_TriggerData;
					 ...
					 TupleTableSlot **epqslot)
{
	...
	if (!should_pgxc_fire_trigger(trigdesc, relinfo->ri_RelationDesc, CMD_DELETE))
		return true;
#endif
	/* opentenbase always send fdw_trigtuple as trigtuple */
...
#ifdef __OPENTENBASE__
	/* should I fire the trigger? */
	if (!should_pgxc_fire_trigger(trigdesc, relinfo->ri_RelationDesc, CMD_UPDATE))
		return;
#endif
					 ...
					 TupleTableSlot *slot)
{
	...
	if (newSlot != NULL)
	{
		ExecStoreHeapTuple(trigtuple, oldslot, false);
		slot = ExecGetUpdateNewTuple(relinfo, newSlot, oldslot);
	...
	/*
	 * If trigger return old, the newtuple is equal to trigtuple, 
	 * so add a check before free trigtuple.
	 */
	if (trigtuple != fdw_trigtuple && newtuple != trigtuple)
		heap_freetuple(trigtuple);
...
#ifdef __OPENTENBASE__
	/* should I fire the trigger? */
	if (!should_pgxc_fire_trigger(trigdesc, relinfo->ri_RelationDesc, CMD_UTILITY))
		return;
#endif
	LocTriggerData.type = T_TriggerData;
...
{
	/* Maybe already cleaned in AfterTriggerEndXact */
	if (afterTriggers.query_depth == -1 && afterTriggers.maxquerydepth == 0)
	{
		return;
	}
	...
	if (row_trigger && transition_capture != NULL)
	{
		...
		*/
		    ...
		    (event == TRIGGER_EVENT_UPDATE && update_old_table)))
		{
			...
			if (map != NULL)
			{
				HeapTuple	converted = execute_attr_map_tuple(oldtup, map, NULL);
            ...
            (event == TRIGGER_EVENT_UPDATE && update_new_table)))
		{
			...
			{
				HeapTuple	converted = execute_attr_map_tuple(newtup, map, NULL);
...
/*
  ...
  */
bool
should_pgxc_fire_trigger(TriggerDesc *trigdesc, Relation rel, CmdType cmd)
{
	RelationLocInfo *rd_locator_info = rel->rd_locator_info;
	int16   tgtype = pgxc_get_trigevent(cmd);
	bool    pullup = trigdesc && ((trigdesc->trig_pullup & tgtype) != 0);
		
	if (IS_ACCESS_NODE)
		return true;
	
	if (g_trig_exec_mode == TRIG_EXECMODE_PULLUP && IS_DISTRIBUTED_DATANODE)
		return false;
	...
	/*
	 * It is a logical replication worker in a data migration
	 * task, it should not fire the trigger.
	 */
	...
	if (g_trig_exec_mode == TRIG_EXECMODE_AUTO &&
	    trigdesc && !trigdesc->trig_pullup)
		return true;
