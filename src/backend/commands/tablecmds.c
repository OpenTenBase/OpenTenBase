...
/*inplace upgrade */
bool		new_catalog_isshared = false;
...
static HTAB *active_temp_table_hash = NULL;
...
#ifdef _PG_ORCL_
	Oid			newPartRel; /* New added partition relation */
#endif
} AlteredTableInfo;
...
static const struct dropmsgstrings dropmsgstringarray[] = {
	...
	{RELKIND_PARTITIONED_INDEX,
		...
		gettext_noop("\"%s\" is not an index"),
	gettext_noop("Use DROP INDEX to remove an index.")},
	{RELKIND_PARTITIONED_GLOBAL_INDEX,
		ERRCODE_UNDEFINED_OBJECT,
		gettext_noop("global index \"%s\" does not exist"),
		gettext_noop("global index \"%s\" does not exist, skipping"),
		gettext_noop("\"%s\" is not an index"),
	gettext_noop("Use DROP INDEX to remove an global index.")},
...
#define ESTORE_SUPPORT_AT_CMD(cmd)                                                             \
	((cmd) == AT_AddConstraint || (cmd) == AT_DropConstraint || (cmd) == AT_AddColumn ||       \
		(cmd) == AT_DropColumn || (cmd) == AT_AlterColumnType || (cmd) == AT_ColumnDefault ||  \
		(cmd) == AT_SetTableSpace || (cmd) == AT_SetOptions || (cmd) == AT_ResetOptions ||     \
		(cmd) == AT_ChangeOwner || (cmd) == AT_AddIndex || (cmd) == AT_AddIndexConstraint ||   \
		(cmd) == AT_ReplicaIdentity || (cmd) == AT_DropNotNull || (cmd) == AT_SetNotNull ||	   \
        (cmd) == AT_SetRelOptions || (cmd) == AT_ResetRelOptions || (cmd) == AT_AttachPartition || \
		(cmd) == AT_DetachPartition || (cmd) == AT_SetStatistics || (cmd) == AT_ClusterOn || \
		(cmd) == AT_DropCluster || (cmd) == AT_AddIdentity)
...
static void StoreCatalogInheritance1(Oid relationId, Oid parentOid,
						 ...
						 bool child_is_partition);
#ifdef __OPENTENBASE_C__
static void RecordChildAliasDepend(Oid childreloid, Oid childaliasoid);
static void StoreCatalogChildAlias(Relation	relation, Oid relationId, List *supers, char* aliasname);
static void ATPrepSetAlias(Relation rel, char *aliasname);
#endif
...
static ObjectAddress ATExecValidateConstraint(Relation rel, char *constrName,
						 bool recurse, bool recursing, LOCKMODE lockmode, bool updatevalidate);
...
static ObjectAddress ATExecAddColumn(List **wqueue, AlteredTableInfo *tab,
				Relation rel, AlterTableCmd *cmd, ColumnDef *colDef, bool isOid,
...
static void RemoveActiveGlobalTempRel(Oid relid);
#endif
static int64 ATCheckColumnValues( Relation rel, AlterTableCmd *cmd);

#ifdef __OPENTENBASE__
static void CheckCatalogInheritance(char relkind, List *supers, bool ispart);

static ObjectAddress ATExecEnableConstraint(Relation rel, char *constrName, ValidateBehavior behavior,
					   bool recurse, bool recursing, LOCKMODE lockmode);
static ObjectAddress ATExecDisableConstraint(Relation rel, char *constrName, ValidateBehavior behavior,
						bool recurse, bool recursing, LOCKMODE lockmode);
static struct ObjectAddress UpdateConstraintCatalog(Relation rel, char *constrName,
													bool enable, bool validated);
static List *UpdateTriggerCatalog(Relation rel, Oid tgOid, char fires_when);
#endif

static OnCommitAction gtt_oncommit_option(List *options);
static bool RelationSupportChecksum(char relkind);

static bool
RelationSupportChecksum(char relkind)
{
	switch (relkind)
	{
		...
		case RELKIND_PARTITIONED_GLOBAL_INDEX:
			return true;
	}
...
static char
GetForeignDistributeType(char relkind, CreateStmt *stmt)
{
	ListCell   *option;

	if (relkind != RELKIND_FOREIGN_TABLE ||
		strcmp(((CreateForeignTableStmt *) stmt)->servername, EXTTABLE_SERVER_NAME) == 0)
		return 0;

	foreach (option, ((CreateForeignTableStmt *) stmt)->options)
	{
		DefElem *defel = lfirst_node(DefElem, option);

		if ((pg_strcasecmp(defel->defname, "distribute") == 0))
		{
			if (IsA(defel->arg, String) && strcasecmp(strVal(defel->arg), "replication") == 0)
				return LOCATOR_TYPE_FOREIGN;
			if (IsA(defel->arg, String) && strcasecmp(strVal(defel->arg), "shard") == 0)
			   ...
			   Oid viewId)
{
...
#endif
#ifdef __OPENTENBASE_C__
	char		relstore   = RELSTORE_ROW;
	int 		compress_id = 0;
#endif
	bool relisshared = IsInplaceUpgrade && new_catalog_isshared;
	...
	if (stmt->oncommit != ONCOMMIT_NOOP
		...
		ereport(ERROR,
				(errcode(ERRCODE_INVALID_TABLE_DEFINITION),
				 errmsg("ON COMMIT can only be used on global/temporary tables")));

#ifdef _PG_ORCL_
	if (stmt->partspec != NULL || (stmt->merge_partspec != NULL))
#else
	...
	if ((stmt->relation->relpersistence == RELPERSISTENCE_TEMP ||
	 /* global temp table same as temp table */
		 stmt->relation->relpersistence == RELPERSISTENCE_GLOBAL_TEMP)
...
#ifdef __OPENTENBASE_C__
	/* Add checksum options for relation if need */
	if (!IsInplaceUpgrade && RelationSupportChecksum(relkind) && 
		checksum_ddl_default)
		stmt->options = CheckAndAddChecksumOptions(stmt->options);
#endif
	...
	/* Parse 'on commit' option for global temp table */
		...
		(relkind == RELKIND_RELATION || relkind == RELKIND_PARTITIONED_TABLE))
	{
		...
		if (inheritOids)
		{
			...
			Relation relation = heap_open(parent, NoLock);
		...
		{
			if (stmt->oncommit != ONCOMMIT_NOOP)
				elog(ERROR, "can not define global temp table with on commit and with clause at same time");
		...
		elog(ERROR, "regular table cannot specify on_commit_delete_rows");
	...
	if (inheritOids != NULL)
	{
		CheckCatalogInheritance(relkind, inheritOids, stmt->partbound != NULL);
	}
	 ...
	 */
	descriptor = BuildDescForRelation(stmt->tableElts, relstore, compress_id, 
								(inheritOids != NULL || stmt->like_found));
	...
	if(IS_PGXC_DATANODE)
	{
		...
		if (has_extent)
		{
			elog(ERROR, "do not support extent");
		}
#endif
	}
#endif
	...
	relationId = heap_create_with_catalog(relname,
										  ...
										  reloptions,
										  true,
										  allowSystemTableMods || IsInplaceUpgrade,
										  false,
#ifdef _SHARDING_
										  has_extent,
#endif
...
#ifdef __OPENTENBASE__
	if (NULL != stmt->aliasname)
		StoreCatalogChildAlias(NULL, relationId, inheritOids, stmt->aliasname);
#endif
	 ...
	 */
#ifdef _MIGRATE_
	if ((!IsInplaceUpgrade || !IsSystemNamespace(namespaceId)) &&
		((stmt->distributeby) ||
			(isRestoreMode && stmt->distributeby != NULL)))
#else			
	if ((!IsInplaceUpgrade || !IsSystemNamespace(namespaceId))
		&& ((IS_PGXC_COORDINATOR && stmt->distributeby) ||
			(isRestoreMode && stmt->distributeby != NULL)))
#endif
	{
#ifdef __FDW__
		/* We do not need to add the foreign table to pgxc_class. */
	char foreignDistributeType = GetForeignDistributeType(relkind, stmt);
	if (relkind != RELKIND_FOREIGN_TABLE ||
		strcmp(((CreateForeignTableStmt *) stmt)->servername, EXTTABLE_SERVER_NAME) == 0 ||
		foreignDistributeType != 0)
		{
			AddRelationDistribution(relationId, stmt->distributeby,
									stmt->subcluster,
#ifdef __OPENTENBASE_C__
									foreignDistributeType,
#endif
...
#ifdef _PG_ORCL_
		if (stmt->istransformed)
			bound = stmt->partbound;
		else
#endif
...
#ifdef _PG_ORCL_
			/*
			 * Delay check bound for opentenbase_ora mode.
			 */
			if (!ORA_MODE)
#endif
				check_default_partition_contents(parent, defaultRel,
#ifdef _PG_ORCL_
									rel,
#endif
									bound);
	...
	if (partitioned)
	{
...
#ifdef _PG_ORCL_
		if (stmt->istransformed && stmt->merge_partspec)
			partnatts = stmt->merge_partspec->partnatts;
		else
#endif
		...
		if (stmt->istransformed)
		{
			PartitionMergeSpec	*spc = stmt->merge_partspec;

			Assert(spc != NULL);
			strategy = spc->strategy;
		}
		else
#endif
		...
		if (stmt->istransformed)
		{
			...
			StorePartitionKey(rel, spc->strategy, spc->partnatts, spc->partattrs, spc->partexprs,
						  spc->partopclass, spc->partcollation);
		}
		else
		{
#endif
	...
	*/
	if (stmt->partbound)
	{
	   ...
	   foreach(cell, idxlist)
	   {
		   ...
		   IndexStmt  *idxstmt;
			Oid			constraintOid;
			Oid         childConstraintOid = InvalidOid;
		   ...
		   idxstmt =
			   generateClonedIndexStmt(NULL, RelationGetRelid(rel), idxRel,
										...
										&constraintOid);
			idxstmt->skip_scan = true;
			DefineIndex(RelationGetRelid(rel),
					   idxstmt,
					   InvalidOid,
					   RelationGetRelid(idxRel),
						constraintOid,
					   false, false, false, false, false, &childConstraintOid, NULL);
	...
	/* Move the tuple in default into newly added partition finally. */
	if (stmt->partbound && ORA_MODE)
	{
		Oid			parentId = linitial_oid(inheritOids),
					defaultPartOid;
		Relation	parent;
		...
		if (OidIsValid(defaultPartOid))
		{
			ParseState *pstate;
			PartitionBoundSpec *bound;
			Relation	defaultRel = heap_open(defaultPartOid, AccessExclusiveLock);
			int	reindex_flags;
			char	relpersist = rel->rd_rel->relpersistence;
			...
			if (!bound->is_default)
			{
				check_default_partition_contents(parent, defaultRel, rel, bound);

				/* Reindex the indexes on the new partition */
				reindex_flags = REINDEX_REL_SUPPRESS_INDEX_USE |
									REINDEX_REL_CHECK_CONSTRAINTS;
				if (relpersist == RELPERSISTENCE_UNLOGGED)
					reindex_flags |= REINDEX_REL_FORCE_INDEXES_UNLOGGED;
				else if (relpersist == RELPERSISTENCE_PERMANENT)
					reindex_flags |= REINDEX_REL_FORCE_INDEXES_PERMANENT;
				reindex_relation(RelationGetRelid(rel), reindex_flags, 0);
			}
...
#endif
{
	...
	foreach(cell, drop->objects)
	{
		...
		if (RELKIND_RELATION == relkind)
		{
			...
			if (IsInplaceUpgrade && IsSystemRelation(relation))
				toast_oid = relation->rd_rel->reltoastrelid;

			report_utility_time(NULL, relation, InvalidOid);

			RemoveResultCacheForRelation(relation);
			heap_close(relation, NoLock);
		}
#endif
...
#ifdef __OPENTENBASE_C__
		if (RELKIND_RELATION == relkind)
			lreloids = list_append_unique_oid(lreloids, relOid);
#endif
								...
								void *arg)
{
	...
	if (classform->relkind == RELKIND_PARTITIONED_TABLE)
		...
		expected_relkind = RELKIND_INDEX;
	else if (classform->relkind == RELKIND_PARTITIONED_GLOBAL_INDEX)
		expected_relkind = RELKIND_INDEX;
	else if (classform->relkind == RELKIND_GLOBAL_INDEX_OF_PARTITIONED)
		expected_relkind = RELKIND_INDEX;
	else
	...
	if (!allowSystemTableMods && !IsInplaceUpgrade && IsSystemClass(relOid, classform))
		ereport(ERROR,
				(errcode(ERRCODE_INSUFFICIENT_PRIVILEGE),
...
ExecuteTruncate(TruncateStmt *stmt)
{
	...
	foreach(cell, stmt->relations)
	{
		...
		/*
		 * Support opentenbase_ora compatible syntax:
		 * -- alter table <table_name> truncate partition (aliasname)...
		 */
		if (stmt->paretablename)
		{
			Oid child_table_oid = InvalidOid;
			HeapTuple    childaliastup = NULL;

			parent_table_oid = RangeVarGetRelid(stmt->paretablename, AccessShareLock, false);

			childaliastup = SearchSysCache2(CHIILDASNAME, ObjectIdGetDatum(parent_table_oid),
											CStringGetDatum(rv->relname));
			...
			if (HeapTupleIsValid(childaliastup))
			{
				...
				if (isNull)
				{
					elog(ERROR, "null child oid for child alias name %s", rv->relname);
				}

				child_table_oid = DatumGetObjectId(tmp);
				ReleaseSysCache(childaliastup);

				rel = heap_open(child_table_oid, lockmode);
				pfree(rv->relname);
				rv->relname = RelationGetRelationName(rel);
			}
		}
		...
		if (NULL == rel)
		{
			...
			if(!OidIsValid(relid))
				elog(ERROR, "Cannot get the the valid Oid of relation: %s", 
						rv->relname? rv->relname: "NULL");

			if (IsGlobalTempRelId(relid))
				rel = heap_open(relid, AccessShareLock);
			else
				rel = heap_open(relid, lockmode);
		}
		if (RelationIsCrossNodeIndex(rel->rd_rel))
			elog(ERROR, "truncate a global index table is forbidden, truncate a table is allowed");
		...
		if (stmt->paretablename)
		{
			...
			/* Make true the relation is the subpartition/partition of the parent table. */
			if (stmt->trunc_subpart)
				partrel_oid = get_partition_parent(myrelid);
			else
				partrel_oid = myrelid;

			depend_rel = heap_open(DependRelationId, AccessShareLock);
			ScanKeyInit(&skey[0],
			...
			scan = systable_beginscan(depend_rel, InvalidOid, false, NULL, 4, skey);

			/*  check if the child table inherit the parent table */
			if (!HeapTupleIsValid(tup = systable_getnext(scan)))
			{
				elog(ERROR, "truncate relation %s is not a child table of %s .",
							rv->relname, stmt->paretablename->relname);
			}
			
			stmt->sc_id = InvalidShardClusterId;

			/* Cleanup */
			systable_endscan(scan);
			heap_close(depend_rel, AccessShareLock);
		}
		...
		/*Ignore the truncate command on meta global temp table */
		if (RELATION_IS_ACTIVE_GLOBAL_TEMP(rel))
			gtt_relids = lappend_oid(gtt_relids, myrelid);
	
#ifdef __OPENTENBASE_C__
		RemoveResultCacheForRelation(rel);
#endif
	...
	{
		List *index_rels   = NIL;
		List *index_relids = NIL;

		foreach(cell, rels)
		{
			Relation rel = (Relation) lfirst(cell);
			List     *global_indexes = RelationGetGlobalIndexInfoList(rel);
			ListCell *lc;

			foreach(lc, global_indexes)
			{
				IndexInfo *index = (IndexInfo *) lfirst(lc);
				Oid  index_table = index->gindex_oid;
				Relation index_rel;

				Assert(OidIsValid(index_table));

				index_rel = heap_open(index_table, AccessExclusiveLock);

				truncate_check_rel(index_rel);

				index_rels = lappend(index_rels, index_rel);
				index_relids = lappend_oid(index_relids, index_table);
			}
			list_free_ext(global_indexes);
		}

		rels = list_concat(rels, index_rels);
		relids = list_concat(relids, index_relids);
	}
	...
	foreach(cell, rels)
	{
		...
		/* Skip partitioned tables, cn only need record the truncate info */
		...
		/* Only Need to truncate global temp tables on datanode */
		...
		else
		{
			...
			if (stmt->sc_id != InvalidShardClusterId)
			{
				if (!RelationIsSharded(rel))
				{
					elog(ERROR, "Truncate ShardCluster is only for sharded table");
				}
				elog(ERROR, "TRUNCATE ShardCluster is not permitted");
			}
			else
			{
				RelationSetNewRelfilenode(rel, rel->rd_rel->relpersistence,
										  GetRecentXmin(), minmulti);
			}
	...
	foreach(cell, rels)
	{
		...
		report_utility_time(NULL, rel, parent_table_oid);

		heap_close(rel, NoLock);
	}	

	/* just remove the active relation for global temp table */
	foreach(cell, gtt_relids)
	{
		Oid	relid = lfirst_oid(cell);
		RemoveActiveGlobalTempRel(relid);
	}
}
...
truncate_check_rel(Relation rel)
{
	...
	if (rel->rd_rel->relkind != RELKIND_RELATION &&
		...
		!RelationIsCrossNodeIndex(rel->rd_rel))
		ereport(ERROR,
				(errcode(ERRCODE_WRONG_OBJECT_TYPE),
	...
	if (!allowSystemTableMods && !IsInplaceUpgrade && IsSystemRelation(rel))
		ereport(ERROR,
				(errcode(ERRCODE_INSUFFICIENT_PRIVILEGE),
				 errmsg("permission denied: \"%s\" is a system catalog",
						RelationGetRelationName(rel))));
	...
	foreach(entry, supers)
	{
			 ...
			 parent_attno++)
		{
			...
			if (exist_attno > 0)
			{
				...
				typenameTypeIdAndMod(NULL, def->typeName, &defTypeId, &deftypmod, true);
	...
	if (inhSchema != NIL)
	{
		...
		foreach(entry, schema)
		{
			...
			if (exist_attno > 0)
			{
				...
				def = (ColumnDef *) list_nth(inhSchema, exist_attno - 1);
				typenameTypeIdAndMod(NULL, def->typeName, &defTypeId, &deftypmod, true);
				typenameTypeIdAndMod(NULL, newdef->typeName, &newTypeId, &newtypmod, true);
...
#ifdef __OPENTENBASE__
/*
 * CheckCatalogInheritance
 *		Check the system catalogs with proper inheritance information.
 *
 * supers is a list of the OIDs of the new relation's direct ancestors.
 */
static void
CheckCatalogInheritance(char relkind, List *supers, bool ispart)
{
	Relation	relation;
	ListCell   *entry;
	...
	foreach(entry, supers)
	{
		...
		relation = heap_open(parentOid, AccessShareLock);
			...
			relation->rd_rel->relkind != relkind)
		{
			ereport(ERROR,
				(errcode(ERRCODE_INVALID_TABLE_DEFINITION),
					errmsg("parent relation \"%s\" should also be foreign table",
					RelationGetRelationName(relation))));
		}
...
#ifdef __OPENTENBASE__
char*
format_childalias(Oid oid)
{
	HeapTuple tup;
	char* aname = NULL;

	tup = SearchSysCache1(CHIILDASOID, ObjectIdGetDatum(oid));
	if (!HeapTupleIsValid(tup))
		elog(ERROR, "cache lookup failed for child alias %u", oid);

	aname = NameStr(((Form_pg_childalias) GETSTRUCT(tup))->aname);
	ReleaseSysCache(tup);
...
void
RemoveChildAliasById(Oid oid)
{
	Relation rel;
	HeapTuple tup;

	rel = heap_open(ChildAliasRelationId, RowExclusiveLock);
...
static void
RecordChildAliasDepend(Oid childreloid, Oid childaliasoid)
{
	ObjectAddress myself;
	ObjectAddress referenced;

	/* Make dependency entries */
	myself.classId = ChildAliasRelationId;
	myself.objectId = childaliasoid;
	myself.objectSubId = 0;

	/* Dependency on relation */
	referenced.classId = RelationRelationId;
	referenced.objectId = childreloid;
...
/*
 * StoreCatalogChildAlias
 *		Save the system catalogs with child alias information.
 *
 * supers is a list of the OIDs of the new relation's direct ancestors.
 */
static void
StoreCatalogChildAlias(Relation	relation, Oid relationId, List *supers, char* aliasname)
{
	ListCell   *entry;
	NameData	aliasdname;
	...
	namestrcpy(&aliasdname, aliasname);
	...
	{
		Oid			parentOid = lfirst_oid(entry);
		Datum		values[Natts_pg_childas];
		bool		nulls[Natts_pg_childas];
		Oid         childaliasoid;
		HeapTuple	tuple;

		/*
		 * Make the pg_childalias entry
		 */
		values[Anum_pg_childas_poid - 1] = ObjectIdGetDatum(parentOid);
		values[Anum_pg_childas_aname - 1] = NameGetDatum(&aliasdname);
		values[Anum_pg_childas_coid - 1] = ObjectIdGetDatum(relationId);

		memset(nulls, 0, sizeof(nulls));

		tuple = heap_form_tuple(RelationGetDescr(rel), values, nulls);

		childaliasoid = CatalogTupleInsert(rel, tuple);

		heap_freetuple(tuple);

		RecordChildAliasDepend(relationId, childaliasoid);
	}
...
renameatt_check(Oid myrelid, Form_pg_class classform, bool recursing)
{
	...
	if (!pg_class_ownercheck(myrelid, GetUserId())&&!is_mls_user())
		aclcheck_error(ACLCHECK_NOT_OWNER, ACL_KIND_CLASS,
					   NameStr(classform->relname));
	if (!allowSystemTableMods && !IsInplaceUpgrade && IsSystemClass(myrelid, classform))
		ereport(ERROR,
				(errcode(ERRCODE_INSUFFICIENT_PRIVILEGE),
				 errmsg("permission denied: \"%s\" is a system catalog",
						NameStr(classform->relname))));
...
RenameRelationInternal(Oid myrelid, const char *newrelname, bool is_internal)
{
	...
	{
#ifdef _PG_ORCL_
		bool	islocal = IsLocalSequence(targetrelation->rd_id);
		...
		if (RenameSequenceGTM(seqname, newseqname) < 0)
 		...
 		RegisterSeqRename(newseqname, seqname);
...
static void
ATVerifyExternalTable(AlterTableStmt *stmt, Relation rel)
{
	...
	/*
	 * Check the ALTER command type is supported for EXTERNAL TABLE.
	 */
...
{
	Oid	relid = InvalidOid;
		relid = RangeVarGetRelidExtended(stmt->relation, lockmode, stmt->missing_ok,
										 false, RangeVarCallbackForAlterRelation,
										 (void *)stmt);
	return relid;
}
/* END_OPENTENBASE_ORA */
...
AlterTable(Oid relid, LOCKMODE lockmode, AlterTableStmt *stmt)
{
	...
	ATVerifyExternalTable(stmt, rel);
		  ...
		  bool recurse, bool recursing, LOCKMODE lockmode)
{
	...
	switch (cmd->subtype)
	{
		...
		case AT_DropColumn:		/* DROP COLUMN */
			ATSimplePermissions(rel,
								ATT_TABLE | ATT_COMPOSITE_TYPE | ATT_FOREIGN_TABLE |
								(IsInplaceUpgrade ? ATT_VIEW : ATT_NULL));
		...
		case AT_SetUnLogged:	/* SET UNLOGGED */
			...
			pass = AT_PASS_MISC;
			break;
		case AT_SetAlias:	/* SET AS */
			ATSimplePermissions(rel, ATT_TABLE);
			tab->chgPersistence = ATPrepChangePersistence(rel, false);

			ATPrepSetAlias(rel, cmd->name);
			pass = AT_PASS_MISC;
			break;
	...
	for (pass = 0; pass < AT_NUM_PASSES; pass++)
	{
		...
		foreach(ltab, *wqueue)
		{
			...
			foreach(lcmd, subcmds)
			{
				...
				ATExecCmd(wqueue, tab, rel, cmd, lockmode);
			}
		  ...
		  AlterTableCmd *cmd, LOCKMODE lockmode)
{
	...
	switch (cmd->subtype)
	{
		case AT_AddColumn:		/* ADD COLUMN */
		case AT_AddColumnToView:	/* add column via CREATE OR REPLACE VIEW */
			address = ATExecAddColumn(wqueue, tab, rel, cmd, (ColumnDef *) cmd->def,
									  false, false, false,
									  false, lockmode);
			break;
		...
		case AT_AddOids:		/* SET WITH OIDS */
			...
			if (cmd->def != NULL)
				address =
					ATExecAddColumn(wqueue, tab, rel, cmd, (ColumnDef *) cmd->def,
									true, false, false,
									cmd->missing_ok, lockmode);
			break;
		...
		case AT_ReplaceRelOptions:	/* replace entire option list */
			ATExecSetRelOptions(rel, (List *) cmd->def, cmd->subtype, lockmode, false);
			break;
		...
		case AT_DetachPartition:
			...
			break;
#ifdef __OPENTENBASE__
		case AT_EnableConstraint:  /* ALTER TABLE ENABLE CONSTRAINT */
			address = ATExecEnableConstraint(rel, cmd->name, cmd->validate, false, false,
											 lockmode);
			break;
		case AT_EnableConstraintRecurse:	/* ALTER TABLE ENABLE CONSTRAINT with
											 * recursion */
			address = ATExecEnableConstraint(rel, cmd->name, cmd->validate, true, false,
											 lockmode);
			break;
		case AT_DisableConstraint: /* ALTER TABLE DISABLE CONSTRAINT */
			address = ATExecDisableConstraint(rel, cmd->name, cmd->validate, false, false,
											  lockmode);
			break;
		case AT_DisableConstraintRecurse:	/* ALTER TABLE DISABLE CONSTRAINT with
											 * recursion */
			address = ATExecDisableConstraint(rel, cmd->name, cmd->validate, true, false,
											 lockmode);
			break;
#endif
		default:				/* oops */
...
static bool
CheckFKColumnIfNewAdded(List **wqueue, Constraint *fkconstraint, Oid fkrelid)
{
	ListCell *ltab;
	...
	{
		AlteredTableInfo *tab = (AlteredTableInfo *)lfirst(ltab);
		List *subcmds = tab->subcmds[AT_PASS_ADD_COL];
		Oid relid = tab->relid;
		ListCell *lcmd;
		...
		{
			AlterTableCmd *cmd = (AlterTableCmd *)lfirst(lcmd);
			if (cmd->subtype == AT_AddColumnRecurse)
			{
				char *addColName = ((ColumnDef *)cmd->def)->colname;
				ListCell *l;
				char *attname;
				...
				foreach (l, fkconstraint->fk_attrs)
				{
					...
					if (strcmp(attname, addColName) == 0)
					{
						if ((fkconstraint->fk_matchtype == FKCONSTR_MATCH_FULL) &&
							(list_length(fkconstraint->fk_attrs) > 1))
						{
							ereport(ERROR, (errmsg("adding a new column and creating a MATCH FULL multi-column "
							                       "foreign key that includes new column in a single operation is not supported."),
												   errhint("Please add the column and create the foreign key in separate operations.")));
						}
						return true;
					}
				}
...
#ifdef _PG_ORCL_
				if (ORA_MODE && tab->validate_default &&
								OidIsValid(tab->newPartRel))
				{
					int		reindex_flags = REINDEX_REL_SUPPRESS_INDEX_USE;
					char	relpersis = get_rel_persistence(tab->newPartRel);

					if (relpersis == RELPERSISTENCE_UNLOGGED)
						reindex_flags |= REINDEX_REL_FORCE_INDEXES_UNLOGGED;
					else if (relpersis == RELPERSISTENCE_PERMANENT)
						reindex_flags |= REINDEX_REL_FORCE_INDEXES_PERMANENT;

					/*
					 * Move rows in default partition into new added partition.
					 * And reindex the newly added partition.
					 */
					ATRewriteTable(tab, tab->newPartRel, lockmode, false);
					reindex_relation(tab->newPartRel, reindex_flags, 0);
				}
				else
#endif
	...
	foreach(ltab, *wqueue)
	{
		...
		foreach(lcon, tab->constraints)
		{
			...
			if (con->contype == CONSTR_FOREIGN)
			{
				...
				/*
				 * Check if the column of foreign key is newly added. If the
				 * column is newly added, constraint checks are not required.
				 * Otherwise, it will report an error on the datanode because
				 * the new column does not exist yet.
				 */
				if (CheckFKColumnIfNewAdded(wqueue, fkconstraint, tab->relid))
				{
					continue;
				}
...
#ifdef _PG_ORCL_
						, bool do_insert /* Insert into new heap? */
#endif
						)
{
	Relation	oldrel;
...
#ifdef _PG_ORCL_
	/*
	 * When rewriting table, the newly inserting tuples into temporary
	 * relation use rowid sequence of old relation for ROWID.
	 */
	if (newrel)
		newrel->rd_rowidseqid = oldrel->rd_rel->relrowidseq;
#endif
...
#ifdef _SHARDING_
		discolnums = get_newheap_diskeys(oldrel, newrel, &ndiscols);
#endif
	}
	else
	{
	...
	if (newrel || needscan)
	{
		...
		{
#ifdef _PG_ORCL_
			ItemPointerData	old_tupid;

			ItemPointerCopy(&tuple->t_self, &old_tupid);
#endif
			if (tab->rewrite > 0)
			{
				...
				if (RelationIsSharded(oldrel))
					tuple = heap_form_tuple_plain(newTupDesc, values, isnull, discolnums, ndiscols, RelationGetRelid(newrel));
				else
#endif
			...
			if (partqualstate && !ExecCheck(partqualstate, econtext))
			{
				...
				{
#ifdef _PG_ORCL_
					if (ORA_MODE && newrel != NULL)
					{
						Assert(tab->rewrite == 0);
						MoveDefaultPartitionRows(oldrel, newrel, tuple, hi_options,
								bistate, mycid);
					}
					else
#endif
...
#ifdef _PG_ORCL_
		newrel->rd_rowidseqid = InvalidOid;
#endif
				...
				bool is_view, AlterTableCmd *cmd, LOCKMODE lockmode)
{
	...
	type_tuple = typenameType(NULL, ((ColumnDef*)cmd->def)->typeName, &typemod);
	type_oid = HeapTupleGetOid(type_tuple);
	ReleaseSysCache(type_tuple);

	if (is_numericfd_type(type_oid))
	{
		ereport(ERROR,
				(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
					errmsg("type \"%s\" is not supported.",
					format_type_with_typemod(type_oid, typemod))));
	}
...
ATExecAddColumn(List **wqueue, AlteredTableInfo *tab, Relation rel,
				AlterTableCmd *cmd, ColumnDef *colDef, bool isOid,
				bool recurse, bool recursing,
				bool if_not_exists, LOCKMODE lockmode)
{
	...
	if (colDef->inhcount > 0)
	{
		...
		if (HeapTupleIsValid(tuple))
		{
			...
			typenameTypeIdAndMod(NULL, colDef->typeName, &ctypeId, &ctypmod, true);
...
#ifdef __OPENTENBASE_C__
	/* we handle column store rewrite here. */
	if (RelationIsRowStore(rel) && colDef->encoding)
	{
		ereport(ERROR,
				(errcode(ERRCODE_TOO_MANY_COLUMNS),
				 errmsg("encoding not supported in row store table.")));
	}

	relOptions = heap_getattr(reltup, Anum_pg_class_reloptions, RelationGetDescr(pgclass), &isNull);
	rel_options = untransformRelOptions(relOptions);
	list_free_deep(rel_options);
	rel_options = NULL;
	ret = GetColumnEncodingInfo(colDef->encoding, &attribute.atttranscomp);
	if (!ret)
	{
		attribute.atttranscomp = compress_id;
	}	
#endif
...
#ifdef __OPENTENBASE_C__
	attribute.attcollen 		= tform->typlen;
	attribute.attuniquestore    = false;
#endif
	...
	{
		RawColumnDefault *rawEnt;
		bool need_check_volatile = false;

		/*
		 * When adding a column to a replication table, it is necessary to
		 * check if the default value contains any volatile functions. If
		 * volatile are present, values generated by volatile function will
		 * differ on each DN, leading to inconsistencies replication data.
		 */
		if (!IS_CENTRALIZED_MODE &&
			rel->rd_locator_info &&
			IsLocatorReplicated(rel->rd_locator_info->locatorType))
			need_check_volatile = true;
		...
		AddRelationNewConstraints(rel, list_make1(rawEnt), NIL,
								  false, true, false, need_check_volatile);
				 ...
				 ObjectAddresses *addrs)
{
...
#ifdef _PG_ORCL_
					&& attnum != ORCL_RowIdAttributeNumber
#endif
							)
		ereport(ERROR,
				(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
	...
	*/
	...
	if (IsDistributedColumn(attnum, rel->rd_locator_info) && !IS_CENTRALIZED_MODE)
	{
		ereport(ERROR,
				(errcode(ERRCODE_INVALID_TABLE_DEFINITION),
	...
	if (children)
	{
		...
		foreach(child, children)
		{
			...
			if (recurse)
			{
				/* The partition rowid has the same sequence with its father, so drop it */
				if ((childatt->attinhcount == 1 && !childatt->attislocal) || attnum == ORCL_RowIdAttributeNumber)
				{
				/*
...
#ifdef __OPENTENBASE_C__
					/* Find or create work queue entry for this table */
					ATGetQueueEntry(wqueue, childrel);
#endif
			   ...
			   IndexStmt *stmt, bool is_rebuild, LOCKMODE lockmode)
{
	...
	address = DefineIndex(RelationGetRelid(rel),
						  ...
						  quiet,
						  &createConstraintId,
						  tab->oldDesc);	/* will to be used by RelationForceStashOntoSilos  */
	...
	switch (newConstraint->contype)
	{
		...
		case CONSTR_FOREIGN:
			...
			if (rel->rd_locator_info &&
				rel->rd_locator_info->locatorType ==  LOCATOR_TYPE_REPLICATED &&
				!IS_CENTRALIZED_MODE)
			{
	...
	if (RELATION_IS_GLOBAL_TEMP(pkrel))
		ereport(ERROR,
				(errcode(ERRCODE_INVALID_FOREIGN_KEY),
				 errmsg("attempt to reference temporary table \"%s\" in a referential integrity constraint",
						RelationGetRelationName(pkrel))));

	if (!allowSystemTableMods && !IsInplaceUpgrade && IsSystemRelation(pkrel))
		ereport(ERROR,
				(errcode(ERRCODE_INSUFFICIENT_PRIVILEGE),
				 errmsg("permission denied: \"%s\" is a system catalog",
						RelationGetRelationName(pkrel))));
...
#ifdef __OPENTENBASE__
/*
 * Update enable and validated flag for pg_constraint.
 */
static struct ObjectAddress
UpdateConstraintCatalog(Relation rel, char *constrName, bool enable, bool validated)
{
	Relation	conrel;
	SysScanDesc scan;
	ScanKeyData key;
	...
	copy_con = (Form_pg_constraint) GETSTRUCT(copyTuple);
	copy_con->conisenable = enable;
	copy_con->convalidated = validated;
	CatalogTupleUpdate(conrel, &copyTuple->t_self, copyTuple);
...
/*
 * Update tgenabled for pg_constraint.
 */
static List *
UpdateTriggerCatalog(Relation rel, Oid tgOid, char fires_when)
{
	HeapTuple   copyTuple;
	HeapTuple   tgtuple;
	ScanKeyData tgkey;
	...
	ScanKeyInit(&tgkey, Anum_pg_trigger_tgconstraint, BTEqualStrategyNumber, F_OIDEQ, tgOid);
 ...
 *
 * Return value is the address of the enabled constraint.  If the constraint
 * was already enabled, InvalidObjectAddress is returned.
 */
static ObjectAddress
ATExecEnableConstraint(Relation rel, char *constrName, ValidateBehavior behavior,
					   bool recurse, bool recursing, LOCKMODE lockmode)
{
	Relation           conrel;
	SysScanDesc        scan;
	ScanKeyData        key;
	HeapTuple          tuple;
	Form_pg_constraint oldcon = NULL;
	bool               found = false;
	ObjectAddress      address = InvalidObjectAddress;
	bool               do_validated = false;
	/* If user doesn't specify validate clause,then default is validated */
	bool               expect_validated = true;
	List              *children = NIL;
	ListCell          *child;
	Oid                conid = InvalidOid;
	List              *other_relids = NIL;
	ListCell          *other_relid;
	...
	while (HeapTupleIsValid(tuple = systable_getnext(scan)))
	{
		oldcon = (Form_pg_constraint) GETSTRUCT(tuple);
		if (strcmp(NameStr(oldcon->conname), constrName) == 0)
		{
			found = true;
	...
	if (oldcon->contype != CONSTRAINT_FOREIGN &&
		oldcon->contype != CONSTRAINT_CHECK &&
		oldcon->contype != CONSTRAINT_UNIQUE &&
		oldcon->contype != CONSTRAINT_PRIMARY )
	{
		ereport(ERROR,
				(errcode(ERRCODE_WRONG_OBJECT_TYPE),
						errmsg("constraint \"%s\" of relation \"%s\" is not a foreign key or check or unique or primary constraint",
							   constrName, RelationGetRelationName(rel))));
	}

	/* Calculate expected validate state. */
	if (behavior == ALTER_NOVALIDATE)
	{
	 ...
	 */
	if (!recursing && !oldcon->connoinherit)
	{
	...
	/*
	 * We must ensure that we only mark the
	 * constraint as enabled on the parent if it's already enabled
	 * on the children.
	 *
	 * We recurse before enabling on the parent, to reduce risk of
	 * deadlocks.
	 */
	foreach(child, children)
	{
		...
		ATExecEnableConstraint(childrel, constrName, behavior,
								false, true, lockmode);

		heap_close(childrel, NoLock);
	}

	/* Nothing changed, Fast return */
	if (oldcon->conisenable && oldcon->convalidated == expect_validated)
	{
		systable_endscan(scan);
		heap_close(conrel, RowExclusiveLock);
		return InvalidObjectAddress;
	...
	/* NoValidate -> Validate : go validate the constraint */
	if (!oldcon->convalidated && expect_validated)
	{
		(void) ATExecValidateConstraint(rel, constrName, false, true, lockmode, false);
		do_validated = true;
	}

	/* If not validated, ReIndex for unique and primary constraint */
	if (!oldcon->conisenable && !do_validated &&
		(oldcon->contype == CONSTRAINT_UNIQUE ||
		 oldcon->contype == CONSTRAINT_PRIMARY ))
	{
		/* Make index reusable */
		index_set_state_flags(oldcon->conindid, INDEX_ENABLE_SET_ALIVE);
		reindex_index(oldcon->conindid, !do_validated, rel->rd_rel->relpersistence, 0);
	}

	systable_endscan(scan);
	heap_close(conrel, RowExclusiveLock);

	address = UpdateConstraintCatalog(rel, constrName, true, expect_validated);

	other_relids = UpdateTriggerCatalog(rel, conid, TRIGGER_FIRES_ON_ORIGIN);
	 ...
	 */
	CacheInvalidateRelcache(rel);
	foreach (other_relid, other_relids)
	{
		CacheInvalidateRelcacheByRelid(lfirst_oid(other_relid));
	}
 ...
 * doing so at that level.
 *
 * Return value is the address of the disabled constraint.  If the constraint
 * was already disabled, InvalidObjectAddress is returned.
 */
static struct ObjectAddress
ATExecDisableConstraint(Relation rel, char *constrName, ValidateBehavior behavior,
						bool recurse, bool recursing, LOCKMODE lockmode)
{
	Relation           conrel;
	SysScanDesc        scan;
	...
	bool               found = false;
	ObjectAddress      address = InvalidObjectAddress;
	/* If user doesn't specify validate clause,then default is novalidate */
	bool               expected_validated = false;
	List              *children = NIL;
	ListCell          *child;
	Oid                conid = InvalidOid;
	List              *other_relids = NIL;
	...
	/*
	 * We must ensure that we only mark the
	 * constraint as disabled on the parent if it's already disabled
	 * on the children.
	 *
	 * We recurse before disabling on the parent, to reduce risk of
	 * deadlocks.
	 */
	foreach(child, children)
	{
		...
		ATExecDisableConstraint(childrel, constrName, behavior,
								 false, true, lockmode);
	...
	if (!oldcon->conisenable && oldcon->convalidated == expected_validated)
	{
		systable_endscan(scan);
		heap_close(conrel, RowExclusiveLock);
		return InvalidObjectAddress;
	...
	/*  NoValidate -> Validate : go validate the constraint */
	if (!oldcon->convalidated && expected_validated)
	{
		(void) ATExecValidateConstraint(rel, constrName, false, true, lockmode, false);
	}

	/* Disable index for unique and primary constraint. */
	if (oldcon->contype == CONSTRAINT_UNIQUE || oldcon->contype == CONSTRAINT_PRIMARY)
	{
		index_set_state_flags(oldcon->conindid, INDEX_DISABLE_SET_DEAD);
	}

	systable_endscan(scan);
	heap_close(conrel, RowExclusiveLock);

	address = UpdateConstraintCatalog(rel, constrName, false, expected_validated);

	other_relids = UpdateTriggerCatalog(rel, conid, TRIGGER_DISABLED);
...
ATExecValidateConstraint(Relation rel, char *constrName, bool recurse,
						 bool recursing, LOCKMODE lockmode, bool updatevalidate)
{
	Relation	conrel;
	SysScanDesc scan;
	ScanKeyData key;
	...
	if (!con->convalidated)
	{
		...
		else if (con->contype == CONSTRAINT_PRIMARY || con->contype == CONSTRAINT_UNIQUE)
		{
			List	   *children = NIL;
			ListCell   *child;
			...
			/*
			 * For index constraints, we must ensure that we only mark the
			 * constraint as validated on the parent if it's already validated
			 * on the children.
			 *
			 * We recurse before validating on the parent, to reduce risk of
			...
			/* Check unique or primary constraint by reindexing */
			index_set_state_flags(con->conindid, INDEX_ENABLE_SET_ALIVE);

			Assert(OidIsValid(con->conindid));
			reindex_index(con->conindid, false, rel->rd_rel->relpersistence , 0);
		}
					 ...
					 bool missing_ok, LOCKMODE lockmode)
{
	...
	foreach(child, children)
	{
		...
		while (HeapTupleIsValid(tuple = systable_getnext(scan)))
		{
			 ...
			 * Note: CLUSTER constraints can also be inherited now.
			 */
			if (con->contype != CONSTRAINT_CHECK && con->contype != CONSTRAINT_CLUSTER)
				continue;
...
static void
get_numeric_typmod(int32 typmod, int32 *precision, int *scale)
{
	if (typmod < 0)
	{
					  ...
					  AlterTableCmd *cmd, LOCKMODE lockmode)
{
...
#ifndef _PG_REGRESS_
	/* could not alter distribute column except in centralized mode */
	if (IsDistributedColumn(attnum, rel->rd_locator_info) && !IS_CENTRALIZED_MODE)
	{
		ereport(ERROR,
				(errcode(ERRCODE_INVALID_TABLE_DEFINITION),
	...
	typenameTypeIdAndMod(NULL, typeName, &targettype, &targettypmod, true);

	if (is_numericfd_type(targettype))
	{
		ereport(ERROR,
				(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
					errmsg("type \"%s\" is not supported.",
...
#ifdef __OPENTENBASE_C__
	/* we handle column store rewrite here. */
	if (RelationIsRowStore(rel) && def->encoding)
	{
		ereport(ERROR,
				(errcode(ERRCODE_TOO_MANY_COLUMNS),
				 errmsg("encoding not supported in row store table.")));
	...
	/*check the type if it may be loss of accuracy*/
	if (ORA_MODE &&
		NUMERICOID == targettype &&
		NUMERICOID == attTup->atttypid &&
		targettypmod != attTup->atttypmod)
	{
		int tgt_precision = ((targettypmod - VARHDRSZ) >> 16) & 0xffff;
		int tgt_scale = (targettypmod - VARHDRSZ) & 0xffff;
		int att_precision= ((attTup->atttypmod - VARHDRSZ) >> 16) & 0xffff;
		int att_scale = (attTup->atttypmod - VARHDRSZ) & 0xffff;
		int delta_precision = 0;
		int delta_scale = 0;

		get_numeric_typmod(targettypmod, &tgt_precision, &tgt_scale);
		get_numeric_typmod(attTup->atttypmod, &att_precision, &att_scale);

		delta_precision = tgt_precision - att_precision;
		delta_scale = tgt_scale - att_scale;
		is_shrink = (delta_scale < 0) || (delta_precision == 0 && delta_scale != 0) ||
		            (delta_scale == 0 && delta_precision < 0) || (delta_precision < delta_scale);
	}
...
#ifdef _PG_ORCL_
	if (ORA_MODE && is_shrink && IS_LOCAL_ACCESS_NODE)
	{
		/*
		 * if it decreases the column's  accuracy, but this column is not null for each row.
		 * It reports error and can't alter column type. 
		 */
		if (ATCheckColumnValues(rel, cmd) > 0)
		{
			elog(ERROR, "column to be modified must be empty to decrease precision or scale. rel:%s, column:%s",
 ...
 */
void
ATExecChangeOwner(Oid relationOid, Oid newOwnerId, bool recursing, LOCKMODE lockmode, void *context)
{
	Relation	target_rel;
	Relation	class_rel;
	HeapTuple	tuple;
...
/* BEGIN_OPENTENBASE_ORA */
		{
			if (tuple_class->reltype != InvalidOid)
...
/* END_OPENTENBASE_ORA */
			...
			tuple_class->relkind == RELKIND_TOASTVALUE)
		{
			...
			foreach(i, index_oid_list)
				ATExecChangeOwner(lfirst_oid(i), newOwnerId, true, lockmode, context);

			list_free(index_oid_list);
		}

		if (tuple_class->relkind == RELKIND_RELATION ||
		    (enable_lightweight_ora_syntax && tuple_class->relkind == RELKIND_PARTITIONED_TABLE) ||
		    tuple_class->relkind == RELKIND_MATVIEW)
		{
		...
		/* In LIGHTWEIGHT_ORA mode, permission changes need to take effect simultaneously on the sub-partitions of partitioned tables. */
		if (enable_lightweight_ora_syntax && tuple_class->relkind == RELKIND_PARTITIONED_TABLE && context == NULL)
		{
			List     *inheritor_oids = find_all_inheritors(relationOid, AccessShareLock, NULL);
			ListCell *i;

			/*
			 * The find_all_inheritors function can retrieve all child partition OIDs at once, 
			 * eliminating the need for repetitive traversal.
			 */
			context = (void *) &relationOid;

			/* recording child oids. */
			inheritor_oids = list_delete_oid(inheritor_oids, relationOid);
			foreach(i, inheritor_oids)
				ATExecChangeOwner(lfirst_oid(i), newOwnerId, recursing, lockmode, context);
		}
	}
...
change_owner_recurse_to_sequences(Oid relationOid, Oid newOwnerId, LOCKMODE lockmode)
{
	...
	while (HeapTupleIsValid(tup = systable_getnext(scan)))
	{
		...
		ATExecChangeOwner(depForm->objid, newOwnerId, true, lockmode, NULL);
...
ATExecSetRelOptions(Relation rel, List *defList, AlterTableType operation,
					LOCKMODE lockmode, bool internal)
{
	Oid			relid;
	Relation	pgclass;
	HeapTuple	tuple;
	...
	if (rel->rd_rel->relkind == RELKIND_VIEW)
	{
		...
		if (check_option)
		{
			...
			view_query_is_auto_updatable(view_query, CMD_UTILITY);
...
/*
 * ALTER TABLE SET AS name
 */
static void
ATPrepSetAlias(Relation rel, char *aliasname)
{
	HeapTuple   tup;
	ScanKeyData entry[1];
	...
	if (!OidIsValid(parentoid))
		elog(ERROR, "the table %s is not child table of partition", RelationGetRelationName(rel));

	relation = heap_open(ChildAliasRelationId, RowExclusiveLock);
	ScanKeyInit(&entry[0],
				Anum_pg_childas_coid,
				BTEqualStrategyNumber, F_OIDEQ,
				ObjectIdGetDatum(RelationGetRelid(rel)));

	scandesc = heap_beginscan_catalog(relation, 1, entry);
	tup = heap_getnext(scandesc, ForwardScanDirection);

	if (!HeapTupleIsValid(tup))
		StoreCatalogChildAlias(relation, RelationGetRelid(rel), list_make1_oid(parentoid), aliasname);
	else
	{
		Datum       values[Natts_pg_childas];
		bool        nulls[Natts_pg_childas];
		bool        replaces[Natts_pg_childas];
		HeapTuple   new_tup;
		NameData	aliasdname;
		...
		replaces[Anum_pg_childas_aname - 1] = true;
		values[Anum_pg_childas_aname - 1] = NameGetDatum(&aliasdname);

		new_tup = heap_modify_tuple(tup, RelationGetDescr(relation), values, nulls, replaces);

		CatalogTupleUpdate(relation, &new_tup->t_self, new_tup);
	}
				   ...
				   ForkNumber forkNum, char relpersistence)
{
	...
	for (blkno = 0; blkno < nblocks; blkno++)
	{
		...
		if (!PageIsVerified(page, blkno, src->smgr_haschecksum))
		{
			WritePageCheckSumErrorFile(src->smgr_rnode, forkNum, blkno);
			ereport(FATAL,
		 ...
		 * space.
		 */
		PageSetChecksumFlag(page, dst->smgr_haschecksum);

		if (use_wal)
			log_newpage(&dst->smgr_rnode.node, forkNum, blkno, page, false, 
						dst->smgr_haschecksum);
...
CreateInheritance(Relation child_rel, Relation parent_rel)
{
	...
	if (((child_rel->rd_rel->relkind == RELKIND_FOREIGN_TABLE)
			|| (parent_rel->rd_rel->relkind == RELKIND_FOREIGN_TABLE))
		&& (child_rel->rd_rel->relkind != parent_rel->rd_rel->relkind))
		ereport(ERROR,
			(errcode(ERRCODE_INVALID_TABLE_DEFINITION),
			errmsg("parent relation \"%s\" and child relation \"%s\" would be both foreign table",
...
bool ConstraintSatisfyAutoIncrement(HeapTuple tuple, TupleDesc desc, 
									AttrNumber attrnum, char contype)
{
	...
	conkey = heap_getattr(tuple, Anum_pg_constraint_conkey, desc, &isnull);
	
	if (isnull)
		return false;
		
	deconstruct_array(DatumGetArrayTypeP(conkey), INT2OID, sizeof(int16), true, 
					  's', &keys, NULL, &nKeys);
...
MergeDistributionIntoExisting(Relation child_rel, Relation parent_rel)
{
	 ...
	 * cases.
	 */
	if (parent_locinfo->nDisAttrs != child_locinfo->nDisAttrs)
		ereport(ERROR,
				(errcode(ERRCODE_WRONG_OBJECT_TYPE),
						errmsg("table \"%s\" is distributed on \"%d\" columns, but the "
							   "parent table \"%s\" is distributed on \"%d\" columns",
							   RelationGetRelationName(child_rel),
							   child_locinfo->nDisAttrs,
							   RelationGetRelationName(parent_rel),
							   parent_locinfo->nDisAttrs),
						errdetail("Distribution column for the child must be same as the parent")));
	
	for (i = 0; i < parent_locinfo->nDisAttrs; ++i)
	{
		char *p_colname = get_attname(parent_locinfo->relid, parent_locinfo->disAttrNums[i]);
		char *c_colname = get_attname(child_locinfo->relid, child_locinfo->disAttrNums[i]);
		
		if (parent_locinfo->disAttrNums[i] != child_locinfo->disAttrNums[i])
			ereport(ERROR,
					(errcode(ERRCODE_WRONG_OBJECT_TYPE),
...
#ifdef __OPENTENBASE_C__
	int ndiscols = 0;
	AttrNumber discolnums[MaxHeapAttributeNumber];
#endif
...
ATCheckCmd(Relation rel, AlterTableCmd *cmd)
{
	...
	switch (cmd->subtype)
	{
		case AT_DropColumn:
			/* Distribution column can be dropped in centralized mode */
			if (IsRelationDistribColumn(rel->rd_locator_info, cmd->name) && !IS_CENTRALIZED_MODE)
				ereport(ERROR,
						(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
						 errmsg("Distribution column cannot be dropped")));
#ifdef _MLS_
		...
		switch (cmd->subtype)
		{
			case AT_DistributeBy:
				...
				GetRelationDistributionItems(redistribState->relid,
											 ...
											 NULL,
											 NULL,
											 (int *)&(newLocInfo->nDisAttrs),
											 newLocInfo->disAttrNums,
											 0
											);
				break;
			case AT_SubCluster:
...
/* BEGIN_OPENTENBASE_ORA */
	/*
	 * InvalidOid of reltype needs not to change the schema, such as force
	 * view.
	 */
...
/* END_OPENTENBASE_ORA */
		...
		(oldNspOid != nspOid))
	{
		...
		is_local = IsLocalSequence(rel->rd_id);
		if (!is_local)
		{
#endif
				   ...
				   LOCKMODE lockmode)
{
	...
	while (HeapTupleIsValid(tup = systable_getnext(scan)))
	{
		...
		{
#ifdef _PG_ORCL_
			bool	islocal = IsLocalSequence(seqRel->rd_id);
	...
	foreach(l, on_commits)
	{
		...
		switch (oc->oncommit)
		{
			...
			case ONCOMMIT_DELETE_ROWS:
				...
				if ((MyXactFlags & XACT_FLAGS_ACCESSEDTEMPREL) && is_temp_table_active(oc->relid))
				{
					if (IsGlobalTempRelId(oc->relid))
								 ...
								 void *arg)
{
	...
	else
	{
		...
		if ((aclresult != ACLCHECK_OK || has_partition_ancestor_privs(relid, GetUserId(), ACL_MAINTAIN)) 
			&& !mls_policy_check(stmt, relid))
	...
	if (!allowSystemTableMods && !IsInplaceUpgrade && IsSystemClass(relid, classform))
		ereport(ERROR,
				(errcode(ERRCODE_INSUFFICIENT_PRIVILEGE),
				 errmsg("permission denied: \"%s\" is a system catalog",
						rv->relname)));
	...
	switch (relid)
	{
		...
		case TypeRelationId:
			...
			if (!IsInplaceUpgrade)
				ereport(ERROR,
					(errcode(ERRCODE_INSUFFICIENT_PRIVILEGE),
						errmsg("permission denied: system catalog \"%s\" can not be altered",
								rv->relname)));
			break;
		default:
			break;
...
#ifdef PGXC
/*
 * IsShardRelOrMainTableIsShardRelOrCnRepRel
 *
 * Check whether a table is a shard relation or whether the main table of
 * an index is a shard relation.
 */
bool
IsShardRelOrMainTableIsShardRelOrRepRel(Oid relid)
{
	Oid heapOid;
	Relation	rel;
	bool		res;
	...
	if (rel == NULL)
		return false;
	else if (RelationIsIndex(rel->rd_rel))
	{
		heapOid = IndexGetRelation(relid, false);
		relation_close(rel, NoLock);
		rel = relation_open(heapOid, NoLock);
	}
	res = RelationIsSharded(rel) ||
		  RelationIsReplication(rel);
	relation_close(rel, NoLock);
	return res;
}
...
/*
 * IsIndexUsingGlobalTempTable
 *
 * Check if given index relation uses global temporary tables.
 */
bool
IsIndexUsingGlobalTempTable(Oid relid)
{
	bool res = false;
	HeapTuple   tuple;
	Oid parent_id = InvalidOid;
	...
	if (HeapTupleIsValid(tuple))
	{
		...
		res = IsGlobalTempRelId(parent_id);
	}
	else
		res = false; /* Default case */
	...
	foreach(cell, idxes)
	{
		...
		if (!found)
		{
			...
			DefineIndex(RelationGetRelid(attachrel), stmt, InvalidOid,
						RelationGetRelid(idxRel),
						constraintOid,
						true, false, false, false, false, &childConstraintOid, NULL);
		}
...
ATExecDetachPartition(Relation rel, RangeVar *name)
{
	...
	if (mls_check_relation_permission(partoid, &schema_bound))
	{
		if (!schema_bound && !is_mls_user())
			elog(ERROR,
				"could not detach partition for table:%s, cause mls poilcy is bound",
				NameStr(rel->rd_rel->relname));
	}
	else if (is_mls_user())
	{
		elog(ERROR,
			"must be owner of relation %s", NameStr(rel->rd_rel->relname));
	}
#endif
...
#ifdef _MLS_
static bool
mls_allow_add_cls_col(Node * stmt, Oid relid)
{
...
static bool
mls_allow_detach_parition(Node * stmt, Oid relid)
{
	ObjectType	reltype;
	Oid         partoid;
...
static bool
mls_policy_check(Node * stmt, Oid relid)
{
...
#ifdef _PG_ORCL_
void
MoveDefaultPartitionRows(Relation def_rel, Relation newRel, HeapTuple tuple,
					int hi_options, BulkInsertState bistate, CommandId mycid)
{
	Oid		tupOid = InvalidOid;
	Datum	values[MaxTupleAttributeNumber];
	bool	isnull[MaxTupleAttributeNumber];
	TupleDesc	defTupDesc = RelationGetDescr(def_rel);
	TupleDesc	newTupDesc = RelationGetDescr(newRel);

	heap_deform_tuple(tuple, defTupDesc, values, isnull);
	simple_heap_delete(def_rel, &tuple->t_self);

	if (defTupDesc->tdhasoid)
		tupOid = HeapTupleGetOid(tuple);

	if(RelationIsSharded(newRel))
	{
		tuple = heap_form_tuple_plain(newTupDesc, values, isnull, RelationGetDisKeys(newRel),
										RelationGetNumDisKeys(newRel),
	...
	tuple->t_tableOid = RelationGetRelid(newRel);
	heap_insert(newRel, tuple, mycid, hi_options, bistate);
}


static int64
ATCheckColumnByScan(Relation rel, AlterTableCmd *cmd,
					TupleDesc tupdesc, int attnum)
{
	...
	scan = heap_beginscan(rel, GetLatestSnapshot(), 0, NULL);
	while (HeapTupleIsValid(tup = heap_getnext(scan, ForwardScanDirection)))
	{
		if (!heap_attisnull(tup, attnum, tupdesc))
		{
			elog(DEBUG1, "[%s] tup attnum(%d, %s) in not null",
				__FUNCTION__, attnum, cmd->name);
			nums++;
			break;
		}
	}
...
static int64
ATCheckColumnValues( Relation rel, AlterTableCmd *cmd)
{
	char				*nspname = NULL;
	char				*relname = NULL;
	...
	RemoteQueryState	*node = NULL;
	TupleTableSlot		*result = NULL;

	/* Number tuples from data nodes */
	int64		nums = 0;
	...
	if (IS_CENTRALIZED_DATANODE)
	{
		...
		{
			elog(DEBUG1, "[%s] relname %s, attname[%d]: %s",
				__FUNCTION__, relname, i, NameStr(tupdesc->attrs[i].attname));
			if (strcmp(cmd->name, NameStr(tupdesc->attrs[i].attname)) == 0)
			{
				attnum = i + 1;
				elog(DEBUG1, "[%s] attnum: %d", __FUNCTION__, attnum);
				break;
			}
		}
		...
		if (rel->rd_rel->relkind == RELKIND_PARTITIONED_TABLE)
		{
			...
			inh = find_all_inheritors(RelationGetRelid(rel), NoLock, NULL);
			...
			for_each_cell(cell, lnext(list_head(inh)))
			{
				...
				/* find_all_inheritors already got lock */
				childrel = heap_open(lfirst_oid(cell), NoLock);
				nums += ATCheckColumnByScan(childrel, cmd, tupdesc, attnum);
				heap_close(childrel, NoLock);
			...
			nums = ATCheckColumnByScan(rel, cmd, tupdesc, attnum);
		elog(DEBUG1, "[%s] attnum:%d, nums: %ld", __FUNCTION__, attnum, nums);
	...
	/* gen select to check the value in each tuple  */
	appendStringInfo(&query, "SELECT \"%s\" FROM \"%s\".\"%s\" WHERE \"%s\" IS NOT NULL LIMIT 1",
							cmd->name, nspname, relname, cmd->name);
	...
	elog(DEBUG1, "%s: %s.%s %s not null rows(%ld)", __FUNCTION__, nspname, relname, cmd->name, nums);
	return nums;
}
...
/*
 * change tablespace for stash relation.
 */
static inline void 
ChangeTableSpaceForStashRelation(Oid stashIdxOid, Oid stashOid, 
									Oid targetTableSpace, LOCKMODE lockmode)
{
	if (OidIsValid(stashOid))
	{
		Relation stashRel;
		/* ATExecSetTableSpace() requires that targetTableSpace is not InvalidOid */
		targetTableSpace = (InvalidOid == targetTableSpace) ? MyDatabaseTableSpace : targetTableSpace;
		Assert(OidIsValid(targetTableSpace));

		/* lock stash relation with lockmode */
		stashRel = heap_open(stashOid, lockmode);

		/* change tablespace for stash Relation */
		ATExecSetTableSpace(stashOid, targetTableSpace, lockmode);

		/* unlock until committed */
		relation_close(stashRel, NoLock);

		/* change tablespace for stash Index Relation */
		if (OidIsValid(stashIdxOid))
		{
			ATExecSetTableSpace(stashIdxOid, targetTableSpace, lockmode);
		}
	}
}

/*
 * change tablespace for registry and its index relation.
 */
static inline void 
ChangeTableSpaceForRegistryRelation(Oid registryIdxOid, Oid registryOid, 
										Oid targetTableSpace, LOCKMODE lockmode)
{
	...
	/* change tablespace for registry Relation */
	Assert(OidIsValid(registryOid));
	ATExecSetTableSpace(registryOid, targetTableSpace, lockmode);

	/* change tablespace for registry Index Relation */
	Assert(OidIsValid(registryIdxOid));
	ATExecSetTableSpace(registryIdxOid, targetTableSpace, lockmode);
}
 ...
 * ----------------------------------------------------------------
 */
void report_utility_time(RangeVar* relation, Relation objRel, Oid parentOid)
{
    ...
    if (!enable_save_datachanged_timestamp)
        return;
    ...
    PG_TRY();
    {
		...
		{
			rel = heap_open(parentOid, AccessShareLock);
		}
		...
		{
			/* use its parent's entry */
			if (!objRel->rd_rel->relispartition)
			{
				rel = objRel;
				needClose = false;
			}
			...
			{
				parentOid = get_partition_parent(objRel->rd_id);

				while (get_rel_relispartition(parentOid))
					parentOid = get_partition_parent(parentOid);
        ...
        if (rid >= FirstNormalObjectId)
		{
            if (rel->rd_rel->relpersistence == RELPERSISTENCE_PERMANENT ||
                rel->rd_rel->relpersistence == RELPERSISTENCE_UNLOGGED)
			{
                pgstat_report_data_changed(rid, rel->rd_rel->relisshared);
            }
        }
...
/* 
 * record the oid of a temp table in a hashtable if it is 
 * accessed in this transaction
 */
void
record_acitve_temp_table(Oid oid)
{
	Oid  *find_oid;
	bool found = false;
	...
	if (!active_temp_table_hash)
	{
		...
		active_temp_table_hash = hash_create("active transaction level temp table info",
											 512,
											 &ctl, HASH_ELEM | HASH_BLOBS);
	}

	find_oid = hash_search(active_temp_table_hash, (void *) &oid, HASH_ENTER, &found);
	
	if (!found)
		*find_oid = oid;
}
/* 
 * check whether the temp table is accessed during the transaction
 *  true  -  accessed 
 *  false -  not accessed
 */ 
static bool 
is_temp_table_active(Oid oid)
{
	Oid *find_oid;
	long nentries = 0;
	...
	nentries = hash_get_num_entries(active_temp_table_hash);

	elog(DEBUG1, "Active hash table count: %ld", nentries);

	if (nentries <= 0)
		return false;

	find_oid = hash_search(active_temp_table_hash, (void *) &oid, HASH_FIND, NULL);

	return find_oid != NULL;
}
/*
...
destory_active_table_hash(void)
{
	if (active_temp_table_hash)
	{
		hash_destroy(active_temp_table_hash);
		active_temp_table_hash = NULL;
	}
}
/*
 * RemoveActiveGlobalTempRel
 * 
 *  perfrom a simpele deletion on an acitve global temp table, a strong lock 
 *  is not neccessary, because the active global temp table is private.
 * 
 */
static void
RemoveActiveGlobalTempRel(Oid relid)
{
	ObjectAddress object;
