...
void lazy_vacuum_global_index_table(Relation *rels, int nrel, int options, VacuumParams *params,
...
#ifdef __OPENTENBASE__
/*
 *	lazy_vacuum_global_index_table() -- perform LAZY VACUUM for one globale index relation
 */
void
lazy_vacuum_global_index_table(Relation *rels, int nrel, int options, VacuumParams *params,
				BufferAccessStrategy bstrategy)
{
	int i;
	for (i = 0; i < nrel; i ++)
	{
		if (RelationIsCrossNodeIndex(rels[i]->rd_rel))
			lazy_vacuum_rel(rels[i], options, params, bstrategy);
	}
				...
				BufferAccessStrategy bstrategy)
{
	...
	lazy_vacuum_global_index_table(Irel, nindexes, options, params, bstrategy);
...
#define FORCE_CHECK_PAGE() \
		...
		if (PageIsEmpty(page))
		{
			...
			if (!PageIsAllVisible(page))
			{
				...
				{
					log_newpage_buffer(buf, true, RelationHasChecksum(onerel));
				}
...
#ifdef __SUPPORT_DISTRIBUTED_TRANSACTION__
						{
                            CommitSeqNo committs = HeapTupleHeaderGetXminTimestampAtomic(tuple.t_data);

                            if (!CSN_IS_COMMITTED(committs))
                            {
                                committs = TransactionIdGetCSN(xmin);
                            }
				  ...
				  LVRelStats *vacrelstats)
{
	...
	PGRUsage	ru0;
	if (RelationIsCrossNodeIndex(indrel->rd_rel))
		return;
	pg_rusage_init(&ru0);
							...
							int *deleted_tuples)
{
	...
	for(blkno = from_blk; blkno < to_blk; blkno++)
	{
		...
		if(cleanpage)
		{
			...
			PageInit(BufferGetPage(buf), BufferGetPageSize(buf), 0, RelationHasChecksum(onerel));
			MarkBufferDirty(buf);
			UnlockReleaseBuffer(buf);
		}
#endif
...
reinit_extent_pages(Relation rel, ExtentID eid)
{
	...
	for(blk = from_blk; blk < to_blk; blk++)
	{
		...
		LockBuffer(buf, BUFFER_LOCK_EXCLUSIVE);
		PageClean(BufferGetPage(buf), BufferGetPageSize(buf));
		PageInit(BufferGetPage(buf), BufferGetPageSize(buf), 0, RelationHasChecksum(rel));
		MarkBufferDirty(buf);
		UnlockReleaseBuffer(buf);
...
void
xlog_reinit_extent_pages(RelFileNode rnode, ExtentID eid, bool checksum_enabled)
{
	BlockNumber blk;
	Buffer		buf;
	BlockNumber	from_blk;
	...
	for(blk = from_blk; blk < to_blk; blk++)
	{
		buf = XLogReadBufferExtended(rnode, MAIN_FORKNUM, blk, RBM_NORMAL, checksum_enabled);
		...
		LockBuffer(buf, BUFFER_LOCK_EXCLUSIVE);
		PageClean(BufferGetPage(buf), BufferGetPageSize(buf));
		PageInit(BufferGetPage(buf), BufferGetPageSize(buf), 0, checksum_enabled);
		freespace = PageGetHeapFreeSpace(BufferGetPage(buf));
		MarkBufferDirty(buf);
		UnlockReleaseBuffer(buf);

		XLogRecordPageWithFreeSpace_extent(rnode, blk, freespace, true, checksum_enabled);
	}
			...
			lineoff++, itemid++)
	{
			...
			!HeapTupleHeaderXminFrozen(tuphdr))
		{
			...
			if (GlobalTimestampIsValid(tuple_xmin_gts)
                ...
                && TransactionIdGetCSNAndCommitStat(xmin, &tlog_xmin_gts, NULL)
			...
			HeapTupleHeaderXmaxCommitted(tuphdr))
		{
			...
			if (GlobalTimestampIsValid(tuple_xmax_gts)
                ...
                && TransactionIdGetCSNAndCommitStat(xmax, &tlog_xmax_gts, NULL)
