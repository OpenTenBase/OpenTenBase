...
#define PG_EXT_SQL_MODE "postgresql"
#define ORA_EXT_SQL_MODE "opentenbase_ora"
#define ALL_EXT_SQL_MODE "all"
#define DEFAULT_EXT_SQL_MODE PG_EXT_SQL_MODE
...
typedef struct ExtensionControlFile
{
	...
	char	   *sql_mode;			/* The supported sql mode for the extension */
...
static bool check_extension_in_right_mode(ExtensionControlFile *control, bool report_error);
...
{
	/* postgis with opentenbase_ora=on is not supported */
	if (ORA_MODE &&
		extension_name &&
		 ...
		 (strcmp(extension_name, "postgis_topology") == 0)))
	{
		ereport(ERROR,
				...
				errmsg("In opentenbase_ora compatible mode, the postgis extension is not supported")));
	}
}
							 ...
							 const char *version)
{
	...
	for (item = head; item != NULL; item = item->next)
	{
		...
		else if (strcmp(item->name, "sql_mode") == 0)
			control->sql_mode = pstrdup(item->value);
		else
			ereport(ERROR,
					(errcode(ERRCODE_SYNTAX_ERROR),
...
read_extension_control_file(const char *extname)
{
	...
	control->encoding = -1;
	/* The supported sql mode for the extension is postgres in default */
	control->sql_mode = DEFAULT_EXT_SQL_MODE;
						   ...
						   const char *filename)
{
	...
	pg_verify_mbstr_len(src_encoding, src_str, len, false, false);
	 ...
	 * valid as is.
	 */
	dest_str = pg_any_to_server(src_str, len, src_encoding, false, false);
 ...
 */
static void
execute_extension_script(Oid extensionOid,
						 ExtensionControlFile *control,
						 ...
						 bool support_gis_extension)
{
	...
	(void) set_config_option("log_statement", "none", PGC_SUSET, PGC_S_SESSION,
	                         GUC_ACTION_SAVE, true, 0, false);
	...
	/*
	 * For performance reasons, the sql that needs to be executed to load
	 * the extension is not recorded
	 */
	(void)set_config_option("pg_stat_statements.track",
							"none",
							PGC_SUSET,
							PGC_S_SESSION,
							GUC_ACTION_SAVE,
							...
							DEBUG3,
							false);
	(void)set_config_option("pg_stat_log.track",
							"none",
							PGC_SUSET,
							PGC_S_SESSION,
							GUC_ACTION_SAVE,
						...
						bool is_create)
{
	...
	ListCell   *lc;
	bool save_enable_auto_pushdown_function;

	/* Don't auto pushdown function when creating extension. */
	save_enable_auto_pushdown_function = enable_auto_pushdown_function;
	enable_auto_pushdown_function = false;
	/*
	...
	/* Check the extension in right sql mode */
	check_extension_in_right_mode(pcontrol, true);
	...
	if (control->schema != NULL)
	{
		...
		if (schemaName && strcmp(control->schema, schemaName) != 0 &&
			!(ORA_MODE && strcmp(asc_toupper(control->schema,
			  strlen(control->schema)), schemaName) == 0) &&
			!cascade)
			ereport(ERROR,
					(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
		...
		/*
		 * In opentenbase_ora mode, we convert the schema name to uppercase, otherwise
		 * later it will fail when create object on this schema, because it can
		 * not find uppercase schema.
		 */
		if (ORA_MODE)
			schemaName = asc_toupper(schemaName, strlen(schemaName));
	 ...
	 */
	execute_extension_script(extensionOid,
							 control,
							 oldVersionName,
							 versionName,
							 requiredSchemas,
	...
	/* Reset the guc of enable_auto_pushdown_function. */
	enable_auto_pushdown_function = save_enable_auto_pushdown_function;
	...
	else
	{
		while ((de = ReadDir(dir, location)) != NULL)
		{
			...
			*strrchr(extname, '.') = '\0';
			...
			/* Skip the unsupported extensions in this sql mode */
			if (!check_extension_in_right_mode(control, false))
				continue;
...
ExecAlterExtensionStmt(ParseState *pstate, AlterExtensionStmt *stmt)
{
	...
	/* Check the extension in right sql mode */
	check_extension_in_right_mode(control, true);
						...
						bool is_create)
{
	...
	if (control->schema != NULL)
	{
		...
		/* Find or create the schema in case it does not exist. */
		if (ORA_MODE)
			schemaName = str_toupper(control->schema, strlen(control->schema), DEFAULT_COLLATION_OID);
		...
		foreach(lc2, stmt_list)
		{
			...
			else
			{
				...
				if (create_replica_table && IS_PGXC_LOCAL_COORDINATOR) 
				{
					...
					/*
					 * When shorter utility_sql is used, stmt_location needs to be updated synchronously to prevent the subsequent
					 * mismatch between pgss_store/pgsl_store detection query and stmt_location.
					 */
...
/*
 * The postgresql function name in the extension script is lowercase in default
 * and it is uppercase in opentenbase_ora mode. However, the C language function name is still
 * lowercase in default. So we need to convert it to lowercase to find it in
 * dynamic link library.
 */
char *
get_c_function_symbol_name(char *funcname)
{
	if (ORA_MODE && creating_extension)
		return get_lowercase_ora_ident_str(funcname);
	else
		return funcname;
}
...
/*
 * Check the extension whether it is in the right sql mode.
 *
 * Postgresql sql mode does not support a few special extensions only for opentenbase_ora mode,
 * while opentenbase_ora mode requires extra work to support the extensions, so extensions support
 * postgresql sql mode in default.
 */
static bool
check_extension_in_right_mode(ExtensionControlFile *control, bool report_error)
{
	if (strcmp(control->sql_mode, ALL_EXT_SQL_MODE) == 0 ||
		(ORA_MODE && strcmp(control->sql_mode, ORA_EXT_SQL_MODE) == 0) ||
		(PG_MODE && strcmp(control->sql_mode, PG_EXT_SQL_MODE) == 0))
		return true;
	else if (report_error)
		ereport(ERROR, (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
				errmsg("The extension \"%s\" is not supported in %s sql mode",
				control->name, ORA_MODE? "opentenbase_ora":"postgresql")));
	else
		return false;
}
