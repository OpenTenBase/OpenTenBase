...
static int32 RemoveCurrentDbTempSchema(void);
...
bool
RemoveTempSchema(Oid namespaceId)
{
	ObjectAddresses *objects;
	ObjectAddress obj;
	HeapTuple	tuple;
	Form_pg_namespace spcForm;

	if (!ConditionalLockDatabaseObject(NamespaceRelationId, namespaceId, 0,
									   AccessExclusiveLock))
	{
		elog(WARNING, "can not acquire lock of namespace, ID is %u", namespaceId);
		return false;
	}

	/* hold the lock, check again */
	tuple = SearchSysCacheCopy1(NAMESPACEOID, ObjectIdGetDatum(namespaceId));

	if (!HeapTupleIsValid(tuple))
	{
		elog(LOG, "the schema not exists, skip remove, schema id is %u", namespaceId);
		return false;
	}

	if (!PgTempSchemaCanClean(namespaceId, NULL))
	{
		UnlockDatabaseObject(NamespaceRelationId, namespaceId, 0, AccessExclusiveLock);
		pfree(tuple);
		return false;
	}
	objects = new_object_addresses();

	obj.classId = NamespaceRelationId;
	obj.objectId = namespaceId;
	obj.objectSubId = InvalidOid;
	add_exact_object_address(&obj, objects);

	performMultipleDeletions(objects, DROP_CASCADE, 0);
	free_object_addresses(objects);

	spcForm = (Form_pg_namespace) GETSTRUCT(tuple);

	elog(LOG, "drop temp schema: database: %s schema: %s",
				get_database_name(MyDatabaseId), NameStr(spcForm->nspname));
	pfree(tuple);
	return true;
}

static int32
RemoveCurrentDbTempSchema(void)
{
	List		*spc_list = NIL;
	ListCell	*cell = NULL;
	HeapTuple spcTuple;
	...
	while ((spcTuple = systable_getnext(scan)) != NULL)
	{
		Form_pg_namespace spcForm = (Form_pg_namespace) GETSTRUCT(spcTuple);
		char* nspname = NameStr(spcForm->nspname);

		if ((strncmp(nspname, "pg_temp_", 8) == 0) ||
				(strncmp(nspname, "pg_toast_temp_", 14) == 0))
		{
			spc_list = lappend_oid(spc_list, HeapTupleGetOid(spcTuple));
		}
	}
...
Datum
pg_clean_unused_temp_schema(PG_FUNCTION_ARGS)
{
	Name		schemaName = NULL;

	schemaName = PG_GETARG_NAME_IF_NULL(0);

	if (schemaName == NULL)
	{
		int32		del_num;
		del_num = RemoveCurrentDbTempSchema();
		if (del_num > 0)
			PG_RETURN_BOOL(true);
		else
	...
	{
		Oid	namespaceId = GetSysCacheOid1(NAMESPACENAME, CStringGetDatum(schemaName));
		
		if (namespaceId != InvalidOid)
		{
			if (RemoveTempSchema(namespaceId))
				PG_RETURN_BOOL(true);
			else
				PG_RETURN_BOOL(false);
		}
		...
		{
			elog(WARNING, "The schema cannot be cleaned up as the temporary table is still active.");
			PG_RETURN_BOOL(false);
		}
	}
	PG_RETURN_BOOL(true);
