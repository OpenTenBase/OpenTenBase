...
void
assign_track_context_name(const char *newval, void *extra)
{
	if (track_memory_inited && !ParseTrackCxtlist(newval))
	{
		GUC_check_errmsg("Incorrect input format, a correct example is " 
			"set track_context_name = 'CacheMemoryContext,TopMemoryContext'.");
	}
}

void
assign_spm_plan_capture_rules(const char *newval, void *extra)
{
	if (!ParseSPMPlanCaptureRules(newval))
	{
		elog(ERROR,
		     "Incorrect input format: %s. A correct example is 'set "
		     "optimizer_capture_sql_plan_baselines_rules = \'time="
		     "10s\''. The unit 'time' can be configured in milliseconds (ms), seconds (s), minutes "
		     "(min), or hours (h). If no unit is specified, it defaults to seconds (s).",
		     newval);
	}
}
...
check_XactIsoLevel(char **newval, void **extra, GucSource source)
{
	...
	if (newXactIsoLevel != XACT_READ_COMMITTED && !enable_experiment_isolation)
	{
		GUC_check_errcode(ERRCODE_FEATURE_NOT_SUPPORTED);
		GUC_check_errmsg("transaction mode is restricted to READ COMMITTED.");
		GUC_check_errhint("Isolation modes other than READ COMMITTED can be enabled by setting "
		                  "enable_experiment_isolation to on, but as experimental feature they are "
...
check_global_session(char **newval, void **extra, GucSource source)
{
	...
	else
	{
		...
		if (!IsTransactionState())
		{
			 ...
			 */
			GUC_check_errmsg("not in transaction state");
			return false;
		}

		coordname = palloc(strlen(*newval) + 1);
		r23 = sscanf(*newval, "%[^_]_%d_%lu", coordname, &coordpid, &coordtimestamp);
		if (r23 != 2 && r23 != 3)
		{
			GUC_check_errmsg("malformed Global Session identifier: \"%s\" %d", *newval, r23);
			return false;
		}
#if 0
		...
		{
			/*In the centralized deployment mode, the CN is allowed to access the standby DNs of other planes across the plane*/
			coordTup = SearchSysCache2(PGXCNODENAME, PointerGetDatum(coordname), PointerGetDatum(PGXCMainClusterName));
		}
		else
		{
			/* CN can only access the DN on the same plane */
			coordTup = SearchSysCache2(PGXCNODENAME, PointerGetDatum(coordname), PointerGetDatum(PGXCClusterName));
		}
#endif

		coordTup = SearchSysCache2(PGXCNODENAME, PointerGetDatum(coordname), PointerGetDatum(pgxc_plane_name(PGXCPlaneNameID)));
		...
		if (OidIsValid(coordid))
			strncpy(MyCoordName, coordname, NAMEDATALEN);
	...
	myextra->coordtimestamp = coordtimestamp;
	*extra = (void *) myextra;
...
assign_global_session(const char *newval, void *extra)
{
	...
	if (strcmp(newval, "none") == 0 && IsConnFromCoord())
	...
	SetGlobalSession(myextra->coordid, myextra->coordpid, myextra->coordtimestamp);
}
#endif
