...
DefineTSDictionary(List *names, List *parameters)
{
	...
	foreach(pl, parameters)
	{
		...
		{
			List	*template_names = defGetQualifiedName(defel);

			/* Process the template argument value like postgres */
			if (ORA_MODE)
				templId = get_special_ora_obj_oid(template_names, get_ts_template_oid, NULL, 0);
			else
				templId = get_ts_template_oid(template_names, false);
		}
		else
		{
...
DefineTSConfiguration(List *names, List *parameters, ObjectAddress *copied)
{
	...
	foreach(pl, parameters)
	{
		...
		{
			List	*copy_names = defGetQualifiedName(defel);

			if (ORA_MODE)
				sourceOid = get_special_ora_obj_oid(copy_names, get_ts_config_oid, NULL, 0);
			else
				sourceOid = get_ts_config_oid(copy_names, false);
		}
		else
			ereport(ERROR,
					(errcode(ERRCODE_SYNTAX_ERROR),
...
serialize_deflist(List *deflist)
{
	...
	foreach(l, deflist)
	{
		...
		appendStringInfo(&buf, "%s = ",
						 quote_identifier_as_pg(defel->defname));
