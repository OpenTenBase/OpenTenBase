				   ...
				   ParamListInfo params, char *completionTag)
{
	...
	if (!experiment_feature)
		elog(ERROR, "materialized view can only be used when the experimental feature is enabled");
						 ...
						 )
{
	...
	plan = pg_plan_query(query, 0, NULL, 0, false);
...
transientrel_receive(TupleTableSlot *slot, DestReceiver *self)
{
	...
	/* deform datarow in server encoding to ensure we store correct values */
	if (slot->tts_datarow)
		slot_getallattrs_encoding(slot);
					   ...
					   int save_sec_context)
{
	...
	/*
	 * It doesn't send down the ANALYZE on the remote nodes because the
	 * temporary table was not created there to start with. We could have
	 * invented a special option for ANALYZE to only run locally. But we
	 * could instead just cook up a VacuumStmt and call vacuum (with
	 * VACOPT_ANALYZE option of course) directly
	 */
	{
		char *tmp_rel_name = NULL;
		char *tmp_ns_name = NULL;
		char *tmp_split = NULL;
		VacuumStmt *stmt = makeNode(VacuumStmt);
		RangeVar *rv = NULL;

		tmp_split = strchr(diffname, '.');
		if (NULL == tmp_split)
		{
			tmp_rel_name = diffname;
			tmp_ns_name = NULL;
		}
		else
		{
			char tmp_namespace[NAMEDATALEN] = {0};
			int len_ns_name =  tmp_split - diffname;
			len_ns_name = (len_ns_name < NAMEDATALEN -1) ? len_ns_name : (NAMEDATALEN -1);
			strncpy(&tmp_namespace[0], diffname, len_ns_name);
			tmp_ns_name = pstrdup(&tmp_namespace[0]);
			tmp_rel_name = tmp_split + 1;
		}

		rv = makeRangeVar(tmp_ns_name, tmp_rel_name, -1);
		stmt->relation = rv;
		stmt->options = VACOPT_ANALYZE;
		ExecVacuum(stmt, true);
	}
...
refresh_by_heap_swap(Oid matviewOid, Oid OIDNewHeap, char relpersistence)
{
	finish_heap_swap(matviewOid, OIDNewHeap, false, false, true, true,
					 GetRecentXmin(), ReadNextMultiXactId(), relpersistence);
}
