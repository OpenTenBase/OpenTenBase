...
rel_is_external_table(Oid relid)
{
	...
	result = (tableform->ftserver == get_foreign_server_oid(EXTTABLE_SERVER_NAME, true));
...
/*
 * Is the given table a hdfs fdw foreign table?
 */
bool
rel_is_hdfsfdw_table(Oid relid)
{
	Form_pg_foreign_table tableform;
	HeapTuple	tp;
	bool		result = false;
	...
	if (strcasecmp(fdw->fdwname, "hdfs_fdw") == 0)
		result = true;
	else
		result = false;
...
get_foreign_server_name(Oid serverid)
{
	...
	serverform = (Form_pg_foreign_server) GETSTRUCT(tp);
	result = pstrdup(NameStr(serverform->srvname));
