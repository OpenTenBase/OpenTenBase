...
extern bool createTempNamespaceSuccess;
	...
	switch (dest)
	{
		...
		case DestRemoteSimple:
			...
			if (enable_distri_print)
				elog(LOG, "pid:%d send CommandComplete internal_cmd_sequence %u", MyProcPid, internal_cmd_sequence);
			if(!IsConnFromApp())
...
ReadyForQuery(CommandDest dest)
{
	switch (dest)
	{
		...
		{
			if (enable_distri_print)
				elog(LOG, "pid %d send ReadyForQuery, BlockStatus %c internal_cmd_sequence %u",
					 MyProcPid, TransactionBlockStatusCode(), internal_cmd_sequence);

			if (PG_PROTOCOL_MAJOR(FrontendProtocol) >= 3)
			{
				StringInfoData buf;
				if (!IsConnFromApp() && feedbackGts)
				{
					feedbackGts = false;
					pq_beginmessage(&buf, '8');
					pq_sendint64(&buf, pg_atomic_barrier_read_u64(&(ShmemVariableCache->localCacheGts.u64[0])));
					pq_endmessage(&buf);
				}

				if (!IsConnFromApp())
				{
					if (createTempNamespaceSuccess)
					{
						/* 
						 * current connection should be session hold, if it creates 
						 * tempoarary namespace space 
						 */
						pq_putemptymessage('F');
						createTempNamespaceSuccess = false;
					}
					pq_putmessage('9', (const char *)&internal_cmd_sequence, 1);
				}
