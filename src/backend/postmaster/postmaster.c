...
#define BACKEND_TYPE_2PCWORKER	0x0020	/* 2pc clean worker process */

#define BACKEND_TYPE_PGJOB		0x0040	/* pg job worker process */

#define BACKEND_TYPE_ALL		0x007F	/* OR of all the above */

#define BACKEND_TYPE_WORKER		(BACKEND_TYPE_AUTOVAC | BACKEND_TYPE_PGJOB | BACKEND_TYPE_BGWORKER | BACKEND_TYPE_2PCWORKER)
...
/* The forward process socket number we are listening for connections on */
...
bool backend_cn = false;
int savedRemoteConn = 0;
...
#define IsPGXCNodeBelongToPlane() ((pmState != PM_HOT_STANDBY) || PGXCPlaneNameID != PGXCMainPlaneNameID)
...
/* received START_AUTOVAC_LAUNCHER signal */
static volatile sig_atomic_t start_pgjob_launcher = false;

/* the launcher needs to be signalled to communicate some condition */
static volatile bool avlauncher_needs_signal = false;

static volatile bool pgjoblauncher_needs_signal = false;

/* received START_AUTOSM_LAUNCHER signal */
static volatile sig_atomic_t start_autosm_launcher = false;

/* asm launcher needs to be signalled to communicate some condition with worker */
static volatile bool asmlauncher_needs_signal = false;
...
#ifdef PGXC
int 			PGXCPlaneNameID = OpenTenBase_Cluster;
int 			PGXCMainPlaneNameID = OpenTenBase_Cluster;
int 			PGXCDefaultPlaneNameID = OpenTenBase_Cluster;
bool			IsPGXCMainPlane = false;

char			*PGXCNodeName = NULL;

/* 
 * use for global stat view, query_id used only for cn, 
 * and PGXCQueryId save in every backendnode during transaction active 
 */
char			PGXCQueryId[NAMEDATALEN] = {0};
uint64			PGXCDigitalQueryId = 0;
volatile pg_atomic_uint64* query_id;

int			PGXCNodeId = 0;
bool		PGXCNodeTypeCoordinator = false;
/* 
 * the first char indicate whether regenerate traceid; 1: y, 0: n
 * the last char 'X' means pg_clean set this traceid.
 * caution: set the 'NAMEDATALEN *2' character to zero to avoid memory corrupt
 */
char   GlobalTraceIdHistory[TRACEID_LEN];
char   GlobalTraceId[TRACEID_LEN];
char   LocalTraceId[TRACEID_LEN];
char  *GlobalTraceIdHistoryPointer = GlobalTraceIdHistory;
TransactionId GlobalTraceVxid = InvalidLocalTransactionId;
...
#define StartSpBgWriter()		StartChildProcess(SpBgWriterProcess)
#define StartCheckpointer()		StartChildProcess(CheckpointerProcess)
#define StartWalWriter()		StartChildProcess(WalWriterProcess)
#define StartWalReceiver()		StartChildProcess(WalReceiverProcess)
#define StartForwardSender()    StartChildProcess(ForwardSendProcess)
#define StartForwardRecver()    StartChildProcess(ForwardRecvProcess)
...
PostmasterMain(int argc, char *argv[])
{
	...
	pqsignal_pm(SIGHUP, SIGHUP_handler);	/* reread config file and
	...
	pqsignal_pm(SIGUSR1, sigusr1_handler);	/* message from child
													 * process */
	pqsignal_pm(SIGUSR2, dummy_handler);	/* unused, reserve for
													 * children */
	 ...
	 */
	while ((opt = getopt(argc, argv, "B:bc:C:D:d:EeFf:h:ijk:lN:nOo:Pp:r:S:sTt:u:W:-:")) != -1)
	{
		switch (opt)
		{
			case 'B':
			...
			case 'u':
				pg_atomic_write_u32((pg_atomic_uint32*)(&WorkingGrandVersionNum), (uint32)strtoul(optarg, NULL, 10));
				if (pg_atomic_read_u32((pg_atomic_uint32*)(&WorkingGrandVersionNum)) > GRAND_VERSION_NUM)
				{
			...
			case '-':
				{
					...
					if (strcmp(name, "coordinator") == 0 && !value)
					{
						isPGXCCoordinator = true;
						isPGXCDataNode = false;
						PGXCNodeTypeCoordinator = true;
					}
					else if (strcmp(name, "datanode") == 0 && !value)
					{
						isPGXCDataNode = true;
						isPGXCCoordinator = false;
						PGXCNodeTypeCoordinator = false;
					}
	...
	if(IS_PGXC_COORDINATOR)
	{
		...
		ret = GetLimitsFromLicenseFile(&max_conn_lic, &max_dn, &max_cn, &expired_date);
		if(ret < 0)
		{		
			elog(WARNING, "read license failed. cluster can only be read now.");
			elog(WARNING, "read license failed. maximum of datanodes is 4; maximum of coordinators is 2.");
			elog(WARNING, "read license failed. Max connections is set to default value %d. ",
							DefaultMaxConnections);
			
			MaxConnections = DefaultMaxConnections + ReservedBackends + max_wal_senders;
		}
		else
		{
			if(max_conn_lic + ReservedBackends + max_wal_senders < MaxConnections)
			{
				elog(WARNING, "max_connection(%d) cannot greater than max connection(%d) from license aggrement.",
						MaxConnections, max_conn_lic);
				MaxConnections = max_conn_lic + ReservedBackends + max_wal_senders;
			}
		}		
	}
#endif
...
#ifdef __OPENTENBASE__
    /*
    * Register GlobalDeadLockDetector worker
    */
    if (IS_PGXC_COORDINATOR)
    {
	...
	/*
	 * Previously, the probe and bless steps are performed when GUC initalization, which
	 * may be problematic, e.g., resource_group_name is not initalized yet. So move here.
	 * TODO we may init default system group here later.
	 */
	...
	if (HaveCrashedWorker)
	{
		...
		slist_foreach_modify(siter, &BackgroundWorkerList)
		{
			...
			if (rw->rw_worker.bgw_restart_time == BGW_NEVER_RESTART
				|| rw->rw_terminate)
			{
				elog(DEBUG1, "rw rw_terminate bgw_notify_pid %d, rw->rw_worker.bgw_restart_time %d, rw->rw_terminate %d", 
...
ServerLoop(void)
{
	...
	for (;;)
	{
		...
		if (IS_PGXC_COORDINATOR && Clean2pcPID == 0 && upgrade_mode != INPLACE_UPGRADE &&
		...
		/* If we have lost the fnsender, try to start a new one */
		if (FnSenderPID == 0 && !IS_CENTRALIZED_MODE &&
			((pmState == PM_RUN || pmState == PM_HOT_STANDBY) && IsPGXCNodeBelongToPlane()))
			FnSenderPID = StartForwardSender();
...
#ifdef __OPENTENBASE_C__
		/* If we have lost the fn recv, try to start a new one */
		if (FnRecvPID == 0 && !IS_CENTRALIZED_MODE &&
			(pmState == PM_RUN || pmState == PM_HOT_STANDBY) && IsPGXCNodeBelongToPlane())
		{
			FnRecvPID = StartForwardRecver();
			ereport(LOG,
				(errmsg("[%s:%d] start forward recv process in ServerLoop [FnRecvPID == 0 && pmState == PM_RUN], "
						"pmState = %d, FnRecvPID = %d", 
		...
		/* If we need to signal the job launcher, do so now */
		if (pgjoblauncher_needs_signal)
		{
			pgjoblauncher_needs_signal = false;
			if (PgJobPID != 0)
				kill(PgJobPID, SIGUSR2);
		}

		/* If we need to start a WAL receiver, try to do that now */
		if (WalReceiverRequested)
        {
            elog(DEBUG1, "try to start walreceiver 1");
...
#ifdef __OPENTENBASE_C__
	else if (proto == END_QUERY_REQUEST_CODE)
	{
		...
		while (offset < len)
		{
			...
			else
			{
				...
				if (strcmp(nameptr, "application_name") == 0)
				{
					...
					port->remote_nodename = pstrdup(valptr + len);
				}
			}
...
SIGHUP_handler(SIGNAL_ARGS)
{
	...
	if (Shutdown <= SmartShutdown)
	{
		...
		if (SpBgWriterPID != 0)
			signal_child(SpBgWriterPID, SIGHUP);
		if (CheckpointerPID != 0)
		...
		if (PgJobPID != 0)
			signal_child(PgJobPID, SIGHUP);
		if (AutoSMPID != 0)
			signal_child(AutoSMPID, SIGHUP);
		if (Clean2pcPID != 0)
...
#endif
		if (FnSenderPID != 0)
			signal_child(FnSenderPID, SIGHUP);
		if (FnRecvPID != 0)
			signal_child(FnRecvPID, SIGHUP);
	...
	switch (postgres_signal_arg)
	{
		case SIGTERM:
				...
				pmState == PM_HOT_STANDBY || pmState == PM_STARTUP)
			{
				...
				/* and bgworkers, 2pcworkers too; does this need tweaking? */
				SignalSomeChildren(SIGTERM, BACKEND_TYPE_WORKER);
				/* and the autovac launcher too */
				if (AutoVacPID != 0)
					signal_child(AutoVacPID, SIGTERM);
				/* and the job launcher too */
				if (PgJobPID != 0)
					signal_child(PgJobPID, SIGTERM);
				...
				/* and the invalid buffer bgwriter too */
				if (SpBgWriterPID != 0)
					signal_child(SpBgWriterPID, SIGTERM);
				/* and the walwriter too */
				if (WalWriterPID != 0)
					signal_child(WalWriterPID, SIGTERM);
				/* and the fnsender too */
				if (FnSenderPID != 0)
					signal_child(FnSenderPID, SIGTERM);
				if (FnRecvPID != 0)
					signal_child(FnRecvPID, SIGTERM);
...
reaper(SIGNAL_ARGS)
{
	...
	while ((pid = waitpid(-1, &exitstatus, WNOHANG)) > 0)
	{
		...
		if (pid == StartupPID)
		{
			...
			if (WalWriterPID == 0)
				WalWriterPID = StartWalWriter();
			if (FnSenderPID == 0 && IsPGXCNodeBelongToPlane() && !IS_CENTRALIZED_MODE)
				FnSenderPID = StartForwardSender();
			...
			if (!IsBinaryUpgrade && AutoVacuumingActive() && AutoVacPID == 0)
				AutoVacPID = StartAutoVacLauncher();
			if (upgrade_mode != INPLACE_UPGRADE && IS_PGXC_COORDINATOR && Clean2pcPID == 0 &&
					pmState == PM_RUN && enable_clean_2pc_launcher)
				Clean2pcPID = StartClean2pcLauncher();
...
#ifdef __OPENTENBASE_C__
			/* Was it the forward manager process?	If so, try to start a new one */
			if (FnRecvPID == 0 && IsPGXCNodeBelongToPlane() && !IS_CENTRALIZED_MODE)
			{
				FnRecvPID = StartForwardRecver();
				ereport(LOG,
					(errmsg("[%s:%d] start forward manager process in reaper [pid == FnRecvPID], "
							"pmState = %d, FnRecvPID = %d", 
							__FUNCTION__, __LINE__,
							pmState, FnRecvPID)));
			}
		...
		/*
		 * Was it the invalid buffer bgwriter?  Normal exit can be ignored; we'll 
		 * start a new one at the next iteration of the postmaster's main loop, if
		 * necessary.  Any other exit condition is treated as a crash.
		 */
		if (pid == SpBgWriterPID)
		{
			SpBgWriterPID = 0;
			if (!EXIT_STATUS_0(exitstatus))
				HandleChildCrash(pid, exitstatus,
								 _("invalid buffer bgwriter process"));
			continue;
		}
		...
		/*
		 * Was it the fnsender?  Normal exit can be ignored; we'll start a new
		 * one at the next iteration of the postmaster's main loop, if
		 * necessary.  Any other exit condition is treated as a crash.
		 */
		if (pid == FnSenderPID)
		{
		    FnSenderPID = 0;
			if (!EXIT_STATUS_0(exitstatus))
			    HandleChildCrash(pid, exitstatus, _("forward sender process"));
			continue;
		}
		...
		if (pid == FnRecvPID)
		{
		    FnRecvPID = 0;
			if (!EXIT_STATUS_0(exitstatus))
			    HandleChildCrash(pid, exitstatus, _("forward recv process"));
			continue;
		}
#ifdef __OPENTENBASE__
...
#ifdef __OPENTENBASE_C__
static void
NotifyFnBackendCrash(bool take_action, int pid, int exitstatus)
{
    if (!take_action)
    {
        return;
    ...
    IsProcCrash = true;
    elog(LOG, "notify postmaster dprocess crash, pid %d", pid);
}
#endif
...
HandleChildCrash(int pid, int exitstatus, const char *procname)
{
...
#ifdef __OPENTENBASE_C__
            NotifyFnBackendCrash(take_action, pid, exitstatus);
#endif			
		}
		else
		{
	...
	/* Take care of the invalid buffer bgwriter too */
	if (pid == SpBgWriterPID)
		SpBgWriterPID = 0;
	else if (SpBgWriterPID != 0 && take_action)
	{
		ereport(DEBUG2,
				(errmsg_internal("sending %s to process %d",
								 (SendStop ? "SIGSTOP" : "SIGQUIT"),
								 (int) SpBgWriterPID)));
		signal_child(SpBgWriterPID, (SendStop ? SIGSTOP : SIGQUIT));
	}
	...
	if (pid == FnSenderPID)
		FnSenderPID = 0;
	else if (FnSenderPID != 0 && take_action)
	{
		ereport(DEBUG2,
				(errmsg_internal("sending %s to process fn sender(%d)",
								 (SendStop ? "SIGSTOP" : "SIGQUIT"),
								 (int) FnSenderPID)));
		signal_child(FnSenderPID, (SendStop ? SIGSTOP : SIGQUIT));
	}

	if (pid == FnRecvPID)
		FnRecvPID = 0;
	else if (FnRecvPID != 0 && take_action)
	{
		ereport(DEBUG2,
				(errmsg_internal("sending %s to process fn receiver(%d)",
								 (SendStop ? "SIGSTOP" : "SIGQUIT"),
								 (int) FnRecvPID)));
		signal_child(FnRecvPID, (SendStop ? SIGSTOP : SIGQUIT));
	}
	...
	/* Take care of the job launcher too */
	if (pid == PgJobPID)
		PgJobPID = 0;
	else if (PgJobPID != 0 && take_action)
	{
		ereport(DEBUG2,
				(errmsg_internal("sending %s to process %d",
								 (SendStop ? "SIGSTOP" : "SIGQUIT"),
								 (int) PgJobPID)));
		signal_child(PgJobPID, (SendStop ? SIGSTOP : SIGQUIT));
	}
...
PostmasterStateMachine(void)
{
	...
	if (FatalError && pmState == PM_NO_CHILDREN)
	{
		...
		if (IS_PGXC_COORDINATOR || IS_PGXC_DATANODE)
		{
			ResQUsageSetTotalPriorities(0);
			ResQUsageSetTotalNetworkBytesLimit(0);
		}
#endif
...
TerminateChildren(int signal)
{
	...
	if (SpBgWriterPID != 0)
		signal_child(SpBgWriterPID, signal);
	if (CheckpointerPID != 0)
	...
	if (PgJobPID != 0)
		signal_child(PgJobPID, signal);
	if (AutoSMPID != 0)
		signal_child(AutoSMPID, signal);
	if (Clean2pcPID != 0)
	...
	if (FnSenderPID != 0)
		signal_child(FnSenderPID, signal);
	if (FnRecvPID != 0)
		signal_child(FnRecvPID, signal);
#ifdef __AUDIT__
	if (AuditLoggerPID != 0)
	{
		ereport(LOG,
...
SubPostmasterMain(int argc, char *argv[])
{
	...
	if (strcmp(argv[1], "--forkbackend") == 0 ||
		...
		strcmp(argv[1], "--forkpgjoblauncher") == 0 ||
		strcmp(argv[1], "--forkpgjobworker") == 0 ||
		strcmp(argv[1], "--forkasmlauncher") == 0 ||
		strcmp(argv[1], "--forkasmworker") == 0 ||
		strcmp(argv[1], "--forkboot") == 0 ||
#ifdef __AUDIT__
		strcmp(argv[1], "--forkalog") == 0 ||
#endif
#ifdef __OPENTENBASE_C__
		strcmp(argv[1], "--forkforward") == 0 ||
#endif
	...
	if (strcmp(argv[1], "--forkpgjoblauncher") == 0)
		PgJobLauncherIAm();
	if (strcmp(argv[1], "--forkpgjobworker") == 0)
		PgJobWorkerIAm();

#ifdef __OPENTENBASE_C__
	if (strcmp(argv[1], "--forkforward") == 0)
		PGXCForwardMgrProcessIam();
#endif
	...
	if (strcmp(argv[1], "--forkpgjoblauncher") == 0)
	{
		...
		JobScheduleMain(argc - 2, argv + 2);	/* does not return */
	}
	if (strcmp(argv[1], "--forkpgjobworker") == 0)
	{
		/* Restore basic shared memory pointers */
		...
		JobExecuteWorkerMain(argc - 2, argv + 2);	/* does not return */
	}
	if (strcmp(argv[1], "--forkasmlauncher") == 0)
	{
		/* Restore basic shared memory pointers */
		InitShmemAccess(UsedShmemSegAddr);
		...
		AutoSMLauncherMain(argc - 2, argv + 2);	/* does not return */
	}
	if (strcmp(argv[1], "--forkasmworker") == 0)
	{
		/* Restore basic shared memory pointers */
		InitShmemAccess(UsedShmemSegAddr);
		...
		AutoSMWorkerMain(argc - 2, argv + 2);	/* does not return */
	}
	if (strcmp(argv[1], "--forkclean2pclauncher") == 0)
	{
		/* Restore basic shared memory pointers */
	...
	if (strcmp(argv[1], "--forkforward") == 0)
	{
		...
		(void)ForwardManagerMain();	/* does not return */
	}
#endif
...
StartClean2pcWorker(void)
{
	...
	if (bn)
	{
		...
		/* 2pc clean workers are not dead_end and need a child slot */
		bn->dead_end = false;
		bn->child_slot = MyPMChildSlot = AssignPostmasterChildSlot();
		bn->bgworker_notify = false;

		bn->pid = StartCln2pcWorker();
		if (bn->pid > 0)
		{
			bn->bkend_type = BACKEND_TYPE_2PCWORKER;
			dlist_push_head(&BackendList, &bn->elem);
#ifdef EXEC_BACKEND
			ShmemBackendArrayAdd(bn);
#endif
...
sigusr1_handler(SIGNAL_ARGS)
{
	...
	if (CheckPostmasterSignal(PMSIGNAL_START_PGJOB_LAUNCHER) &&
		Shutdown == NoShutdown)
	{
		start_pgjob_launcher = true;
	}
	...
	{
        elog(DEBUG1, "try to start walreceiver");
 ...
 * usr2 signal handler
 *
 * We use in the postmaster for notify
 */
static void
	...
	if (pid < 0)
	{
		...
		switch (type)
		{
			...
			case SpBgWriterProcess:
				ereport(LOG,
						(errmsg("could not fork invalid buffer bgwriter process: %m")));
				break;
			...
			case ForwardSendProcess:
				ereport(LOG,
						(errmsg("could not fork forward sender process: %m")));
			case ForwardRecvProcess:
				ereport(LOG,
						(errmsg("could not fork forward recver process: %m")));
				break;
			default:
				ereport(LOG,
...
{
	return 2 * (MaxConnections + autovacuum_max_workers + 1 + pgjob_max_workers + 1 +
...
do_start_bgworker(RegisteredBgWorker *rw)
{
	...
	ereport(DEBUG1,
			(errmsg("starting background worker process \"%s\", bgw_notify_pid %d",
					rw->rw_worker.bgw_name, rw->rw_worker.bgw_notify_pid)));
...
#endif
	{
		...
		case 0:
			...
			/* in postmaster child ... */
			ereport(DEBUG1,
					(errmsg("worker, starting background worker process \"%s\", bgw_notify_pid %d, getpid %d",
							rw->rw_worker.bgw_name, rw->rw_worker.bgw_notify_pid, MyProcPid)));
		...
		default:
			...
			ereport(DEBUG1,
					(errmsg("after fork starting background worker process \"%s\", bgw_notify_pid %d, worker_pid %d",
							rw->rw_worker.bgw_name, rw->rw_worker.bgw_notify_pid, worker_pid)));
...
#define MAX_BGWORKERS_TO_LAUNCH 100
	...
	slist_foreach_modify(iter, &BackgroundWorkerList)
	{
		...
		if (rw->rw_terminate)
		{
			elog(DEBUG1, "rw rw_terminate bgw_notify_pid %d", rw->rw_worker.bgw_notify_pid);
		...
		if (rw->rw_crashed_at != 0)
		{
			if (rw->rw_worker.bgw_restart_time == BGW_NEVER_RESTART)
			{
				...
				elog(DEBUG1, "rw BGW_NEVER_RESTART bgw_notify_pid %d", rw->rw_worker.bgw_notify_pid);
		...
		if (bgworker_should_start_now(rw->rw_worker.bgw_start_time))
		{
			...
			if (upgrade_mode == INPLACE_UPGRADE &&
				(strcmp("ApplyLauncherMain", rw->rw_worker.bgw_function_name) == 0 ||
				strcmp("ApplyWorkerMain", rw->rw_worker.bgw_function_name) == 0))
			{
				elog(DEBUG1, "ApplyLauncher or ApplyWorker can't run during inplace upgrade");
				return;
			}
