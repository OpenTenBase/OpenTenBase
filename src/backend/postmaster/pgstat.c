...
bool		enable_save_datachanged_timestamp = false;
int			pgstat_track_functions = TRACK_FUNC_OFF;
int			pgstat_track_activity_query_size = 1024;
int			pgstat_track_statement_query_size = 1024;
int			pgstat_inq_interval = 640;
/* ----------
...
extern uint32 PGXCLocalSessionId;
...
HTAB *xactRelStatHash = NULL;
HTAB *analyzeRelHash = NULL;
XactRelStatsArray *xactRelStatList = NULL;
...
static XactRelStats* get_relstat_entry(Oid rel_id);
...
static const char *pgstat_get_wait_fn(WaitEventFn w);
...
static void generate_pgstat_targetlist(List **tagert, TupleDesc tdesc);
static void wait_statfile_fresh(Oid inquiry_db);
static void xact_update_relstats(Oid relid, int32 relpages, float4 reltuples);
static bool xact_update_attstats(Oid relid, int16 attnum, bool inh, HeapTuple statTup);
/* ------------------------------------------------------------
...
pgstat_report_stat(bool force)
{
	...
	for (tsa = pgStatTabList; tsa != NULL; tsa = tsa->tsa_next)
	{
		for (i = 0; i < tsa->tsa_used; i++)
		{
			...
			Assert(entry->trans == NULL || shmem_exit_inprogress);
 ...
 * ---------
 */
void pgstat_report_data_changed(Oid tableoid, bool shared)
{
...
void
relstat_initstats(Relation rel)
{
	Oid rel_id = rel->rd_id;
	char relkind = rel->rd_rel->relkind;

	if (!ENABLE_AUTO_ANALYZE_IN_XACT() || IsParallelWorker() || IsSystemRelation(rel) ||
		IS_DISTRIBUTED_DATANODE)
	{
		rel->xact_relstat_info = NULL;
		return;
	}
	...
	if (rel->xact_relstat_info != NULL &&
		rel->xact_relstat_info->relid == rel_id)
		return;

	rel->xact_relstat_info = get_relstat_entry(rel_id);
}

static XactRelStats*
get_relstat_entry(Oid rel_id)
{
	RelStatHashEntry *hash_entry;
	XactRelStatsArray *rel_array;
	XactRelStats	*entry;
	bool		found = false;
	...
	if (xactRelStatHash == NULL)
	{
		...
		ctl.entrysize = sizeof(RelStatHashEntry);

		xactRelStatHash = hash_create("xact lookup hash table",
									XACT_RELSTAT_INIT_NUM,
									&ctl,
									HASH_ELEM | HASH_BLOBS);
	}
	 ...
	 */
	hash_entry = hash_search(xactRelStatHash, &rel_id, HASH_ENTER, &found);
	if (!found)
	{
		/* initialize new entry */
		hash_entry->stats = NULL;
	}

	/*
	 * If entry is already valid, we're done.
	 */
	if (hash_entry->stats != NULL)
		return hash_entry->stats;

	/*
	 * Locate the first xactRelStatList entry with free space, making a new list
	 ...
	 */
	if (xactRelStatList == NULL)
	{
		/* Set up first pgStatTabList entry */
		xactRelStatList = (XactRelStatsArray *)
			MemoryContextAllocZero(TopMemoryContext,
								   sizeof(XactRelStatsArray));
	}

	rel_array = xactRelStatList;
	while (rel_array->used >= XACT_RELSTAT_INIT_NUM)
	{
		if (rel_array->next_array == NULL)
			rel_array->next_array = (XactRelStatsArray *)
				MemoryContextAllocZero(TopMemoryContext,
										sizeof(XactRelStatsArray));
		rel_array = rel_array->next_array;
	}

	/*
	 * Allocate a XactRelStats entry within this list entry.  We assume
	 * the entry was already zeroed, either at creation or after last use.
	 */
	entry = &rel_array->rel_entries[rel_array->used++];
	entry->relid = rel_id;

	/*
	 * Now we can fill the entry in xactRelStatHash.
	 */
	hash_entry->stats = entry;

	return entry;
}

void
clean_relstat_list(void)
{
	XactRelStatsArray *next_array;
	XactRelStatsArray *array;

	next_array = xactRelStatList->next_array;

	while (next_array != NULL)
	{
		array = next_array;
		next_array = array->next_array;
		if (array->used > 0)
			MemSet(array->rel_entries, 0, array->used * sizeof(XactRelStats));
		array->next_array = NULL;
		array->used = 0;
		pfree(array);
	}
	if (xactRelStatList->used > 0)
		MemSet(xactRelStatList->rel_entries, 0, xactRelStatList->used * sizeof(XactRelStats));

	xactRelStatList->used = 0;
	xactRelStatList->next_array = NULL;
}
...
pgstat_initstats(Relation rel)
{
	...
	if (!(relkind == RELKIND_RELATION ||
		  ...
		  /* create entry for parent table on cn */
		  (IS_PGXC_COORDINATOR && relkind == RELKIND_PARTITIONED_TABLE) ||
#endif
		  relkind == RELKIND_MATVIEW ||
...
pgstat_count_heap_insert(Relation rel, PgStat_Counter n)
{
	...
	/* centralized mode, need record partition changes of the partitioned table */
	if (relstat_info != NULL && rel->rd_rel->relispartition)
	{
		relstat_info->insert_tuples += n;
	}
}
...
pgstat_count_heap_delete(Relation rel)
{
	...
	if (relstat_info != NULL && rel->rd_rel->relispartition)
	{
		relstat_info->delete_tuples++;
	}
}

/*
 * pgstat_count_estore_insert - count a tuple insertion of n tuples
 */
void
pgstat_count_estore_insert(Relation rel, PgStat_Counter n)
{
	PgStat_TableStatus *pgstat_info = rel->pgstat_info;
	XactRelStats *relstat_info = rel->xact_relstat_info;
...
/*
 * pgstat_count_estore_update - count tuples update
 */
void
pgstat_count_estore_update(Relation rel, PgStat_Counter n)
{
	PgStat_TableStatus *pgstat_info = rel->pgstat_info;
	XactRelStats *relstat_info = rel->xact_relstat_info;
	...
	if (relstat_info != NULL && rel->rd_rel->relispartition)
	{
		relstat_info->update_tuples += n;
	}
}

/*
 * pgstat_count_estore_delete - count tuples deletion
 */
void
pgstat_count_estore_delete(Relation rel, PgStat_Counter n)
{
	PgStat_TableStatus *pgstat_info = rel->pgstat_info;
	XactRelStats *relstat_info = rel->xact_relstat_info;
...
pgstat_count_truncate(Relation rel)
{
	...
	{
		relstat_info->insert_tuples = 0;
		relstat_info->update_tuples = 0;
		relstat_info->delete_tuples = 0;
	}
}
...
pgstat_count_remote_insert(Relation rel, int n)
{
	...
	/*
	 * This function is only called in distributed mode, need record the parent
	 * table info and normal table.
	 */
	if (relstat_info != NULL && !rel->rd_rel->relispartition)
		relstat_info->insert_tuples += n;
...
pgstat_count_remote_delete(Relation rel, int n)
{
	...
	/* Should be only applied to distributed table or updatable view */
	...
	/*
	 * This function is only called in distributed mode, need record the parent
	 * table info. Other tables are recorded in the ExecutePlan function.
	 */
	if (relstat_info != NULL)
		relstat_info->delete_tuples += n;
}
	...
	if (xact_state != NULL)
	{
		...
		for (trans = xact_state->first; trans != NULL; trans = trans->next)
		{
			...
			if (isCommit)
			{
				...
				{
					bool found = false;
					RelStatHashEntry *entry = hash_search(xactRelStatHash, &(tabstat->t_id), HASH_FIND, &found);

					if (found && entry->stats->update_relstat)
					{
						entry->stats->changes_since_analyze = entry->stats->insert_tuples +
																entry->stats->update_tuples +
																entry->stats->delete_tuples;
						tabstat->t_counts.t_changed_tuples = entry->stats->changes_since_analyze;
					}
				}
			}
			else
...
static void
xact_update_relstats(Oid relid, int32 relpages, float4 reltuples)
{
	Relation pg_class;
	HeapTuple tuple;
...
static bool
xact_update_attstats(Oid relid, int16 attnum, bool inh, HeapTuple statTup)
{
	Relation	stat_rel;
	HeapTuple	oldtup;
	bool		success = true;
	...
	{
		/* Yes, replace it */
		statTup->t_data->t_ctid = oldtup->t_data->t_ctid;
		statTup->t_self = oldtup->t_self;
		statTup->t_tableOid = oldtup->t_tableOid;
#ifdef PGXC
		statTup->t_xc_node_id = oldtup->t_xc_node_id;
#endif
		ReleaseSysCache(oldtup);
		PG_TRY();
		{
			CatalogTupleUpdate(stat_rel, &statTup->t_self, statTup);
		}
		PG_CATCH();
		{
			success = false;
	...
	{
		/* No, insert new tuple */
		CatalogTupleInsert(stat_rel, statTup);
	}
...
void
xact_update_relstat_attstat(void)
{
	HASH_SEQ_STATUS status;
	RelStatHashEntry *rel_entry;
	XactRelStats *stats;
	bool found = false;
	local_statistic_hash_entry *localstat_entry;
	RelStatFileAnalyzedEntry	*statfile_entry;
	Relation 	rel;
	int i;
	bool update_catalog = false;
	bool pop_snapshot = false;

	if (!ENABLE_AUTO_ANALYZE_IN_XACT())
		return;

	/* This transaction did not trigger an analyze */
	if (local_statistic_hash == NULL || xactRelStatHash == NULL)
		return;
	...
	pgstat_read_statsfiles_analyzed();
	...
	while ((localstat_entry = (local_statistic_hash_entry*)hash_seq_search(&status)) != NULL)
	{
		int64	statfile_tuples;
		int64	xact_tuples;

		rel_entry = (RelStatHashEntry*) hash_search(xactRelStatHash, &(localstat_entry->relid), HASH_FIND, &found);
		// Assert(found);
		if (found)
		{
			stats = rel_entry->stats;
			statfile_entry = hash_search(analyzeRelHash, &(rel_entry->relid), HASH_FIND, &found);
			if (found)
			{
				stats->changes_since_analyze = stats->insert_tuples +
												stats->update_tuples + stats->delete_tuples;
				statfile_tuples = statfile_entry->n_live_tuples +
										statfile_entry->n_dead_tuples +
										statfile_entry->changes_since_analyze;
				xact_tuples = localstat_entry->livetuples +
									localstat_entry->deadtuples +
									stats->changes_since_analyze;

				if (xact_tuples > statfile_tuples)
				{
					update_catalog = true;
				}
			}

			/* stat file not contain this table, need update catalog */
			if (!found || update_catalog)
			{
				bool success = false;
				if (ConditionalLockRelationOid(rel_entry->relid, ShareUpdateExclusiveLock))
				{
					rel = relation_open(rel_entry->relid, NoLock);
					if (rel != NULL)
					{
						xact_update_relstats(rel_entry->relid, localstat_entry->relpages, localstat_entry->livetuples);
						for (i = 0; i < localstat_entry->natts; i++)
						{
							if (localstat_entry->att_stat_tups[i] != NULL)
							{
								success = xact_update_attstats(rel_entry->relid, localstat_entry->attnum[i],
									false, localstat_entry->att_stat_tups[i]);
							}
							if (localstat_entry->att_stat_tups_inh[i] != NULL)
							{
								success = xact_update_attstats(rel_entry->relid, localstat_entry->attnum[i],
									true, localstat_entry->att_stat_tups_inh[i]);
							}
						}
					}
					if (success)
						pgstat_report_analyze(rel, localstat_entry->livetuples, localstat_entry->deadtuples, true);
					relation_close(rel, ShareUpdateExclusiveLock);
					rel_entry->stats->update_relstat = success;
				}
			}
		}
	}
...
static char *BackendStatementBuffer = NULL;
static Size BackendActivityBufferSize = 0;
static Size BackendStatementBufferSize = 0;
#ifdef USE_SSL
...
BackendStatusShmemSize(void)
{
	...
	size = add_size(size,
					mul_size(pgstat_track_statement_query_size, NumBackendStatSlots));
#ifdef USE_SSL
...
CreateSharedBackendStatus(void)
{
	...
	/* Create or attach to the shared quer id buffer */
	size = mul_size(NAMEDATALEN, NumBackendStatSlots);
	BackendAppnameBuffer = (char *)
		ShmemInitStruct("Backend Query Id Buffer", size, &found);

	if (!found)
	{
		MemSet(BackendAppnameBuffer, 0, size);

		/* Initialize query_id pointers. */
		buffer = BackendAppnameBuffer;
		for (i = 0; i < NumBackendStatSlots; i++)
		{
			BackendStatusArray[i].st_query_id = buffer;
			buffer += NAMEDATALEN;
		}
	}
	...
	/* Create or attach to the shared statement buffer */
	BackendStatementBufferSize = mul_size(pgstat_track_statement_query_size,
										 NumBackendStatSlots);
	BackendStatementBuffer = (char *)
		ShmemInitStruct("Backend Statement Buffer",
						BackendStatementBufferSize,
						&found);

	if (!found)
	{
		MemSet(BackendStatementBuffer, 0, size);

		/* Initialize st_activity pointers. */
		buffer = BackendStatementBuffer;
		for (i = 0; i < NumBackendStatSlots; i++)
		{
			BackendStatusArray[i].st_statement = buffer;
			buffer += pgstat_track_statement_query_size;
		}
	}
...
/*
 * use userid to set the MyBEEntry's st_userid
 */
void
pgstat_set_userid(Oid userid)
{
	if (MyBEEntry)
		MyBEEntry->st_userid = userid;
}
...
pgstat_bestart(void)
{
	...
	else
	{
		...
		switch (MyAuxProcType)
		{
			...
			case BgWriterProcess:
				...
				break;
			case SpBgWriterProcess:
				beentry->st_backendType = B_SPBG_WRITER;
				break;
			...
			case ClusterMonitorProcess:
				...
				break;
			case ForwardSendProcess:
				beentry->st_backendType = B_FN_SENDER;
				break;
			case ForwardRecvProcess:
				beentry->st_backendType = B_FN_RECVER;
				break;
			default:
	...
	beentry->st_query_id[0] = '\0';
	beentry->st_activity[0] = '\0';
	beentry->st_statement[0] = '\0';
	/* Also make sure the last byte in each string area is always 0 */
	beentry->st_clienthostname[NAMEDATALEN - 1] = '\0';
	beentry->st_appname[NAMEDATALEN - 1] = '\0';
	beentry->st_activity[pgstat_track_activity_query_size - 1] = '\0';
	beentry->st_statement[pgstat_track_statement_query_size - 1] = '\0';
	beentry->st_progress_command = PROGRESS_COMMAND_INVALID;
	beentry->st_progress_command_target = InvalidOid;
	beentry->st_rsgid = InvalidOid;
	beentry->st_cputime = getProcCputime(MyProcPid);
	beentry->st_memory = getProcPss(MyProcPid);
	getProcIO(MyProcPid, &read, &write);
	beentry->st_ioread = read;
	beentry->st_iowrite = write;
	LWLockInitialize((LWLock *) (&(beentry->st_memstat.mem_track_lock)), LWMEM_TRACK);
...
pgstat_report_activity(BackendState state, const char *cmd_str)
{
	...
	if (strlen(PGXCQueryId) != 0 && strcmp(beentry->st_query_id, PGXCQueryId) != 0)
	{
		memcpy((char *) beentry->st_query_id, PGXCQueryId, strlen(PGXCQueryId) + 1);
		beentry->st_query_id[strlen(PGXCQueryId)] = 0;
	}
...
void
pgstat_report_statement(const char *cmd_str)
{
       volatile PgBackendStatus *beentry = MyBEEntry;
       int                     len = 0;
       ...
       {
               len = pg_mbcliplen(cmd_str, strlen(cmd_str),
                                                  pgstat_track_statement_query_size - 1);
       }
       ...
       if (cmd_str != NULL)
       {
               memcpy((char *) beentry->st_statement, cmd_str, len);
               beentry->st_statement[len] = '\0';
       }
 ...
 *
 * refresh PGXCQueryId in beentry
 * ----------
 */
void
pgstat_refresh_queryid(void)
{
	volatile PgBackendStatus *beentry = MyBEEntry;
...
/* ----------
 * pgstat_refresh_fn_queryid() -
 *
 * refresh st_fn_queryid in beentry
 * ----------
 */
void
pgstat_refresh_fn_queryid(FNQueryId fn_queryid)
{
	volatile PgBackendStatus *beentry = MyBEEntry;
	...
	if (beentry->st_fn_queryid.timestamp_nodeid != fn_queryid.timestamp_nodeid ||
		beentry->st_fn_queryid.sequence != fn_queryid.sequence)
	{
		beentry->st_fn_queryid = fn_queryid;
	}
...
/*
 * Report the current execute queryid,userid
 */
char *
pgstat_report_queryid(int *st_userid)
{
	volatile PgBackendStatus *beentry = MyBEEntry;

	if (!beentry || beentry->st_backendType != B_BACKEND || 
			strlen(beentry->st_query_id) == 0 || strlen(beentry->st_activity) == 0)
		return NULL;

	if (st_userid)
		*st_userid = beentry->st_userid;
	return pstrdup(beentry->st_query_id);
}


QueryBackendStat *
GetQueryInfoFromPgBackendStatus(void)
{
	QueryBackendStat *query_backend_st = NULL;
	volatile PgBackendStatus *beentry = MyBEEntry;
	...
	query_backend_st = (QueryBackendStat *)palloc0(sizeof(QueryBackendStat));
	query_backend_st->st_procpid = beentry->st_procpid;
	query_backend_st->st_backendType = beentry->st_backendType;
	query_backend_st->st_proc_start_timestamp = beentry->st_proc_start_timestamp;
	query_backend_st->st_xact_start_timestamp = beentry->st_xact_start_timestamp;
	query_backend_st->st_activity_start_timestamp = beentry->st_activity_start_timestamp;
	query_backend_st->st_state_start_timestamp = beentry->st_state_start_timestamp;
	query_backend_st->st_databaseid = beentry->st_databaseid;
	query_backend_st->st_userid = beentry->st_userid;
	query_backend_st->st_clientaddr = beentry->st_clientaddr;
	query_backend_st->st_clienthostname = pstrdup(beentry->st_clienthostname);
	query_backend_st->st_state = beentry->st_state;
	query_backend_st->st_appname = pstrdup(beentry->st_appname);
	query_backend_st->st_activity = pstrdup(beentry->st_activity);
	query_backend_st->st_query_id = pstrdup(beentry->st_query_id);
	query_backend_st->st_rsgid = beentry->st_rsgid;

	return query_backend_st;
}
...
pgstat_read_current_status(void)
{
	...
	localstatement = (char *)
		MemoryContextAllocHuge(pgStatLocalContext,
							   pgstat_track_statement_query_size * NumBackendStatSlots);
#ifdef USE_SSL
	...
	for (i = 1; i <= NumBackendStatSlots; i++)
	{
		...
		for (;;)
		{
			...
			if (localentry->backendStatus.st_procpid > 0)
			{
				...
				strcpy(queryid, (char *) beentry->st_query_id);
				localentry->backendStatus.st_query_id = queryid;
				
				strcpy(localactivity, (char *) beentry->st_activity);
				localentry->backendStatus.st_activity = localactivity;
				strcpy(localstatement, (char *) beentry->st_statement);
				localentry->backendStatus.st_statement = localstatement;
		...
		if (localentry->backendStatus.st_procpid > 0)
		{
			...
			localstatement += pgstat_track_statement_query_size;
#ifdef USE_SSL
			localsslstatus++;
#endif
			localNumBackends++;
 ...
 *
 *	Return a string representing the current wait event information, backend is
 *	waiting on.
 */
void
pgstat_get_wait_event_info(uint32 wait_event_info, PGPROC *proc, StringInfo event_info)
{
	uint32			classId;
	...
	{
		case PG_WAIT_FN:
			appendStringInfo(event_info, "%d", proc->fn_fid);
			break;
		default:
			break;
	}
...
pgstat_get_wait_event(uint32 wait_event_info)
{
	...
	switch (classId)
	{
		...
		case PG_WAIT_FN:
			{
				WaitEventFn w = (WaitEventFn) wait_event_info;

				event_name = pgstat_get_wait_fn(w);
				break;
			}
		default:
...
pgstat_get_wait_activity(WaitEventActivity w)
{
	...
	switch (w)
	{
		...
		case WAIT_EVENT_AUTOVACUUM_MAIN:
			...
			break;
		case WAIT_EVENT_PGJOB_MAIN:
			event_name = "PgJobMain";
			break;
        ...
        case WAIT_EVENT_GLOBAL_DEADLOCK_DETECTOR_MAIN:
            ...
            break;
#endif
		case WAIT_EVENT_FNSENDER_HIBERNATE:
			event_name = "FnSenderHibernate";
			break;
...
pgstat_get_wait_ipc(WaitEventIPC w)
{
	...
	switch (w)
	{
		...
		case WAIT_EVENT_HASH_GROW_BUCKETS_REINSERTING:
			...
			break;
		case WAIT_EVENT_SIP_ELECT_RECEIVER:
			event_name = "Sip/Receiver/Electing";
			break;
		case WAIT_EVENT_SIP_FILTER_FINISHED:
			event_name = "Hash/Receiver/Finished";
			break;
...
pgstat_get_wait_io(WaitEventIO w)
{
	...
	switch (w)
	{
		...
		case WAIT_EVENT_DATA_FILE_TRUNCATE:
			...
			break;
		case WAIT_EVENT_DATA_FILE_EXTENTION:
			event_name = "DataFileExtention";
			break;
#ifdef _SHARDING_
 ...
 *  pgstat_get_wait_fn() -
 *
 *  Convert WaitEventFn to string.
 * ----------
 */
static const char *
pgstat_get_wait_fn(WaitEventFn w)
{
	const char *event_name = "unknown wait event";

	switch (w)
	{
		case WAIT_EVENT_SEND_DATA:
			event_name = "FnSendData";
			break;
		case WAIT_EVENT_RECEIVE_DATA:
			event_name = "FnRecieveData";
			break;
		case WAIT_EVENT_RECEIVE_MSG:
			event_name = "FnReceiveMsg";
			break;
		case WAIT_EVENT_ALLOCATE_FNPAGE:
			event_name = "FnAllocatePage";
			break;
		case WAIT_EVENT_SHAREDCTE_READ:
			event_name = "SharedCTERead";
			break;
		default:
			break;
			/* no default case, so that compiler will warn */
...
pgstat_get_backend_desc(BackendType backendType)
{
	...
	switch (backendType)
	{
		...
		case B_AUTOVAC_WORKER:
			...
			break;
		case B_PGJOB_LAUNCHER:
			backendDesc = "pg job launcher";
			break;
		case B_PGJOB_WORKER:
			backendDesc = "pg job worker";
			break;
		case B_BACKEND:
		...
		case B_BG_WRITER:
			...
			break;
		case B_SPBG_WRITER:
			backendDesc = "invalidate buffer bgwriter";
			break;
		case B_CHECKPOINTER:
		...
		case B_CLEAN_2PC_WORKER:
			...
			break;
		case B_FN_SENDER:
			backendDesc = "forward sender";
			break;
		case B_FN_RECVER:
			backendDesc = "forward recver";
			break;
			case B_PROXY_FOR_DN:
...
reset_dbentry_counters(PgStat_StatDBEntry *dbentry)
{
	...
	dbentry->n_tuples_stashed = 0;
	dbentry->last_autovac_time = 0;
	dbentry->last_autosm_time = 0;
...
void
pgstat_read_statsfiles_analyzed(void)
{
	HASHCTL		hash_ctl;
	HTAB	   *table_hash;
	HASH_SEQ_STATUS status;
	PgStat_StatTabEntry	*temp_entry;
	bool		found;

	wait_statfile_fresh(MyDatabaseId);
	...
	hash_ctl.entrysize = sizeof(PgStat_StatTabEntry);
	hash_ctl.hcxt = pgStatLocalContext;
	table_hash = hash_create("DB table stat hash", PGSTAT_TAB_HASH_SIZE, &hash_ctl,
						 HASH_ELEM | HASH_BLOBS | HASH_CONTEXT);

	memset(&hash_ctl, 0, sizeof(hash_ctl));
	hash_ctl.keysize = sizeof(Oid);
	hash_ctl.entrysize = sizeof(RelStatFileAnalyzedEntry);
	hash_ctl.hcxt = TopMemoryContext;
	analyzeRelHash = hash_create("Rel stat changes hash", PGSTAT_TAB_HASH_SIZE, &hash_ctl,
						 HASH_ELEM | HASH_BLOBS | HASH_CONTEXT);

	pgstat_read_db_statsfile(MyDatabaseId, table_hash, NULL, false);
	hash_seq_init(&status, table_hash);
	while ((temp_entry = (PgStat_StatTabEntry*)hash_seq_search(&status)) != NULL)
	{
		RelStatFileAnalyzedEntry *entry = hash_search(analyzeRelHash, &(temp_entry->tableid), HASH_ENTER, &found);
		if (!found)
		{
			entry->changes_since_analyze = temp_entry->changes_since_analyze;
			entry->n_live_tuples = temp_entry->n_live_tuples;
			entry->n_dead_tuples = temp_entry->n_dead_tuples;
		}
	}
	hash_destroy(table_hash);
...
static void
wait_statfile_fresh(Oid inquiry_db)
{
...
backend_read_statsfile(void)
{
	...
	wait_statfile_fresh(inquiry_db);
...
pgstat_recv_tabstat(PgStat_MsgTabstat *msg, int len)
{
	...
	for (i = 0; i < msg->m_nentries; i++)
	{
		...
		if (tabmsg->t_counts.t_stashed)
			dbentry->n_tuples_stashed += tabmsg->t_counts.t_tuples_inserted;
	}
}
...
/* ----------
 * pg_stat_get_global_activity
 * 
 *  Execute pg_stat_get_global_activity query remotely and save
 *  results in tuplestore.
 * ----------
 */
void
pg_stat_get_global_activity(const char *query_id, bool coord, ReturnSetInfo *rsinfo)
{
...
void
pg_stat_get_global_stat(const char *query, bool coord, ReturnSetInfo *rsinfo)
{
	...
	if (coord)
	{
		...
		plan->exec_nodes->nodeList = GetAllCoordNodes(true);
	}
	else
	{
	...
	generate_pgstat_targetlist(&plan->scan.plan.targetlist, rsinfo->setDesc);
	/* prepare to execute */
...
double getProcCputime(int procid)
{
	double cputime = 0;
	...
	snprintf(filename, BUF_SIZE, "/proc/%d/stat", procid);
	file = fopen(filename, "r");
	if (file == NULL)
	{
		elog(LOG, "open file:%s failed for %s", filename, strerror(errno));
		return cputime;
	}

	if (fgets(buf, BUF_SIZE, file) == NULL)
	{
		elog(LOG, "fgets file:%s failed for %s", filename, strerror(errno));
		return cputime;
	}
	fclose(file);
	...
	cputime += atof(token) / sysconf(_SC_CLK_TCK);
	token = strtok(NULL, " ");
	cputime += atof(token) / sysconf(_SC_CLK_TCK);
...
long long getProcPss(int procid)
{
	long long	total_pss = 0;
	char filename[BUF_SIZE];
	FILE *file = NULL;
	char buf[BUF_SIZE];

	snprintf(filename, BUF_SIZE, "/proc/%d/smaps", procid);
	file = fopen(filename, "r");
	if (file == NULL)
	{
		elog(LOG, "open file:%s failed for %s", filename, strerror(errno));
	...
	while (fgets(buf, BUF_SIZE, file) != NULL)
	{
		...
		if (sscanf(buf, "Pss: %lld kB", &pss) == 1)
		{
			total_pss += pss;
		}
	}
...
void getProcIO(int procid, long long *total_read, long long *total_write)
{
	char filename[BUF_SIZE];
	FILE *file = NULL;
	char buf[BUF_SIZE];

	snprintf(filename, BUF_SIZE, "/proc/%d/io", procid);
	file = fopen(filename, "r");
	if (file == NULL)
	{
		elog(LOG, "open file:%s failed for %s", filename, strerror(errno));
	...
	while (fgets(buf, BUF_SIZE, file) != NULL)
	{
		...
		if (sscanf(buf, "read_bytes: %lld kB", &read) == 1)
		{
			*total_read += read;
		}
		else if (sscanf(buf, "write_bytes: %lld kB", &write) == 1)
		{
			*total_write += write;
		}
	}
...
void
pg_stat_get_global_query_cputime(const char *queryid, bool coord, ReturnSetInfo *rsinfo)
{
	char    query[QUERY_LEN];

	if (queryid == NULL)
	{
		sprintf(query, "select queryid, nodename, sum(cputime) as cputime from pg_stat_get_query_cputime(NULL) group by queryid, nodename order by queryid, nodename;");
	} else
	{
		sprintf(query, "select queryid, nodename, sum(cputime) as cputime from pg_stat_get_query_cputime('%s') group by queryid, nodename order by queryid, nodename;", queryid);
	}
...
void
pg_stat_get_global_query_memory(const char *queryid, bool coord, ReturnSetInfo *rsinfo)
{
	char    query[QUERY_LEN];

	if (queryid == NULL)
	{
		sprintf(query, "select queryid, nodename, sum(memory) as memory_sum, max(memory) as memory_max from pg_stat_get_query_memory(NULL) group by queryid, nodename order by queryid, nodename;");
	} else
	{
		sprintf(query, "select queryid, nodename, sum(memory) as memory_sum, max(memory) as memory_max from pg_stat_get_query_memory('%s') group by queryid, nodename order by queryid, nodename;", queryid);
	}
...
void
pg_stat_get_global_query_workfile_usage(const char *queryid, bool coord, ReturnSetInfo *rsinfo)
{
	char    query[QUERY_LEN];

	if (queryid == NULL)
	{
		sprintf(query, "select queryid, nodename, sum(numfiles) as total_numfiles, sum(size) / 1024 as total_size from pg_stat_get_query_workfile_usage(NULL) group by queryid, nodename order by queryid, nodename;");
	} else
	{
		sprintf(query, "select queryid, nodename, sum(numfiles) as total_numfiles, sum(size) / 1024 as total_size from pg_stat_get_query_workfile_usage('%s') group by queryid, nodename order by queryid, nodename;", queryid);
	}
...
void
pg_stat_get_global_workfile_diskspace(bool coord, ReturnSetInfo *rsinfo)
{
	char		query[QUERY_LEN];

	sprintf(query, "select * from pg_stat_get_workfile_diskspace();");

	pg_stat_get_global_stat(query, coord, rsinfo);
}

void
pg_stat_get_global_query_io(const char *queryid, bool coord, ReturnSetInfo *rsinfo)
{
	char    query[QUERY_LEN];

	if (queryid == NULL)
	{
		sprintf(query, "select queryid, nodename, sum(read) as read, sum(write) as write from pg_stat_get_query_io(NULL) group by queryid, nodename order by queryid, nodename;");
	} else
	{
		sprintf(query, "select queryid, nodename, sum(read) as read, sum(write) as write from pg_stat_get_query_io('%s') group by queryid, nodename order by queryid, nodename;", queryid);
	}
...
void 
generate_pgstat_targetlist(List **target, TupleDesc tdesc)
{
	Var		*dummy;
	int i;
	for (i = 0; i < tdesc->natts; i ++)
	{
		...
		attr = &tdesc->attrs[i];
		dummy = makeVar(1, i, attr->atttypid, 0, InvalidOid, 0);
		
		*target = lappend(*target,
				(void *)makeTargetEntry((Expr *) dummy, i, attr->attname.data, false));
	}
}

/*
 * The main function of DoMemTrackInfoAchieveInternal is to trigger archiving when users query memory views
 * (pg_stat_session_memory_detail/pg_stat_context_memory_detail), ensuring that the memory usage data in 
 * MyBEEntry->st_memstat is more accurate. At the same time, is_print is supported for the function 
 * pg_stat_node_memory_detail to print full information on memory allocation.
 */
static void
DoMemTrackInfoAchieveInternal(bool is_print)
{
	HTAB                 *cxt_detail_hash;
	HASH_SEQ_STATUS       hash_seq;
	MemCxtDetail         *entry_cxt_total = NULL;
	struct MemCxtDetail   mem_detail_tmp[TOP50_MEMDETAIL_LEN] = {{{0}}};
	int                   mem_detail_tmp_size = 0;
	int                   track_mode = track_memory_mode;
#ifdef MEMORY_ALLOC_TRACKING
	TrackMemCxtEle       *entry_cxt;
	bool                  found = false;
	HASH_SEQ_STATUS       hash_seq1;
	HTAB                 *alloc_detail_hash = NULL;
	MemAllocEle          *entry_alloc = NULL;
	MemAllocDetail       *entry_alloc_total = NULL;
	struct MemAllocDetail mem_cxt_detail_tmp[TOP50_MEMDETAIL_LEN] = {{{0}}};
	int                   mem_cxt_detail_tmp_size = 0;
#endif

	if (!track_memory_inited || track_mode < TRACK_MEM_SESSION)
	{
		return;
	}

	cxt_detail_hash = CreateTrackMemCxtDetailHash();

#ifdef MEMORY_ALLOC_TRACKING
	if (track_mode == TRACK_MEM_CONTEXT)
	{
		alloc_detail_hash = CreateTrackMemAllocDetailHash();

		/* get the sum of same alloc location */
		hash_seq_init(&hash_seq, mem_cxt_stats_hash);
		while ((entry_cxt = (TrackMemCxtEle *) hash_seq_search(&hash_seq)) != NULL)
		{
			const char *name = entry_cxt->key->ident ? entry_cxt->key->ident : entry_cxt->key->name;

			hash_seq_init(&hash_seq1, (HTAB *) entry_cxt->mem_alloc_stats_hash);
			while ((entry_alloc = (MemAllocEle *) hash_seq_search(&hash_seq1)) != NULL)
			{
				MemAllocDetailKey entry_key;

				entry_key.file = entry_alloc->key.file;
				entry_key.line = entry_alloc->key.line;
				strncpy(entry_key.cxt_name, name, NAMEDATALEN - 1);

				entry_alloc_total =
					(MemAllocDetail *) hash_search(alloc_detail_hash, &entry_key, HASH_ENTER, &found);
				if (entry_alloc_total)
				{
					if (found)
					{
						entry_alloc_total->total_size += entry_alloc->total_size;
					}
					else
					{
						entry_alloc_total->total_size = entry_alloc->total_size;
				...
				{
					elog(ERROR, "DoMemTrackInfoAchieveInternal: alloc_detail_hash HASH_ENTER failed");
				}
			}
		}
	}
	...
	/* Calculate the sum of memory usage for contexts with the same context name. */
	(void)SummarizeCxtMemUsage(cxt_detail_hash);

	hash_seq_init(&hash_seq, cxt_detail_hash);
	while ((entry_cxt_total = (MemCxtDetail *) hash_seq_search(&hash_seq)) != NULL)
	{
		GetTopnMemcxtEles(entry_cxt_total, mem_detail_tmp, &mem_detail_tmp_size);
	}
	hash_destroy(cxt_detail_hash);

#ifdef MEMORY_ALLOC_TRACKING
	if (track_mode == TRACK_MEM_CONTEXT)
	{
		hash_seq_init(&hash_seq, alloc_detail_hash);
		while ((entry_alloc_total = (MemAllocDetail *) hash_seq_search(&hash_seq)) != NULL)
		{
			if (is_print)
				fprintf(stderr,
				        "dump_memory: %s, %s:%d, %lu\n",
				        entry_alloc_total->key.cxt_name,
				        entry_alloc_total->key.file,
				        entry_alloc_total->key.line,
				        entry_alloc_total->total_size);
			GetTopnMemAllocEles(entry_alloc_total, mem_cxt_detail_tmp, &mem_cxt_detail_tmp_size);
		}
		hash_destroy(alloc_detail_hash);
	}
#endif

	/* achieve top50 */
	LWLockAcquire(&MyBEEntry->st_memstat.mem_track_lock, LW_EXCLUSIVE);

	if (track_mode >= TRACK_MEM_SESSION)
	{
		memcpy(MyBEEntry->st_memstat.mem_cxt_detail,
		       mem_detail_tmp,
		       TOP50_MEMDETAIL_LEN * sizeof(struct MemCxtDetail));
		MyBEEntry->st_memstat.mem_cxt_detail_len = mem_detail_tmp_size;
	}

#ifdef MEMORY_ALLOC_TRACKING
	if (track_mode == TRACK_MEM_CONTEXT)
	{
		memcpy(MyBEEntry->st_memstat.mem_alloc_detail,
		       mem_cxt_detail_tmp,
		       TOP50_MEMDETAIL_LEN * sizeof(struct MemAllocDetail));
		MyBEEntry->st_memstat.mem_alloc_detail_len = mem_cxt_detail_tmp_size;
	}
#endif

	LWLockRelease(&MyBEEntry->st_memstat.mem_track_lock);
	return;
}

void
DoMemTrackInfoAchieve(void)
{
	DoMemTrackInfoAchieveInternal(false);
}

void
DoMemTrackAllocInfoPrint(void)
{
	DoMemTrackInfoAchieveInternal(true);
}

/*
 * The function pg_stat_context_memory_detail is called to query the top 50 memory allocation situations for the function.
 */
SessionMemAllocDetailList *
GetSessionMemAllocDetail(void)
{
#ifdef MEMORY_ALLOC_TRACKING
	SessionMemAllocDetailList result;
	...
	/*
	 * Here we are not canceling the business process, but sending a memory monitoring archive signal
	 * to notify the archiving of memory usage statistics.
	 */
	CancelDBBackends(MyDatabaseId, PROCSIG_REFRESH_MEMORY_TRACKINFO, false);
	...
	for (curr_backend = 1; curr_backend <= num_backends; curr_backend++)
	{
		...
		PgBackendStatus *          beentry;
		int                        loopcnt = 0;
		SessionMemAllocDetailList *res_entry;
		...
		st_memstat = &beentry->st_memstat;

		tmp = (MemAllocDetail *) st_memstat->mem_alloc_detail;
		mem_track_lock = (LWLock) st_memstat->mem_track_lock;
		loopcnt = st_memstat->mem_alloc_detail_len;

		LWLockAcquire(&mem_track_lock, LW_SHARED);
		for (i = 0; i < loopcnt; i++)
		{
			res_entry = (SessionMemAllocDetailList *) palloc0(sizeof(SessionMemAllocDetailList));
			memcpy(&res_entry->entry.entry, &tmp[i], sizeof(MemAllocDetail));
			res_entry->entry.pid = beentry->st_procpid;

			if (strlen(beentry->st_query_id) != 0)
			{
				strcpy(res_entry->entry.sessionid, beentry->st_query_id);
			}

			res_entry->next = result.next;
			result.next = res_entry;
		}
		LWLockRelease(&mem_track_lock);
	}
	return result.next;
#else
	ereport(ERROR,
	        (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
	         errmsg("The current version of the function does not support this feature.")));
#endif
}

/*
 * The function pg_stat_session_memory_detail is called to query the top 50 memory usage situations for session context.
 */
SessionMemCxtDetailList *
GetSessionMemCxtDetail(void)
{
	SessionMemCxtDetailList result;
	int                     num_backends = pgstat_fetch_stat_numbackends();
	int                     curr_backend;
	PgBackendMemStat *      st_memstat;
	LWLock                  mem_track_lock;
	...
	for (curr_backend = 1; curr_backend <= num_backends; curr_backend++)
	{
		...
		PgBackendStatus *        beentry;
		int                      loopcnt = 0;
		SessionMemCxtDetailList *res_entry;
		...
		tmp = (MemCxtDetail *) st_memstat->mem_cxt_detail;
		mem_track_lock = (LWLock) st_memstat->mem_track_lock;
		loopcnt = st_memstat->mem_cxt_detail_len;

		LWLockAcquire(&mem_track_lock, LW_SHARED);
		for (i = 0; i < loopcnt; i++)
		{
			res_entry = (SessionMemCxtDetailList *) palloc0(sizeof(SessionMemCxtDetailList));
			strcpy(res_entry->entry.cxt_name, tmp[i].cxt_name);
			res_entry->entry.pid = beentry->st_procpid;
			...
			res_entry->entry.total_size = tmp[i].total_size;
			res_entry->next = result.next;
			result.next = res_entry;
		}
		LWLockRelease(&mem_track_lock);
...
void
DoMemoryDetailBooking(void)
{
	struct MemoryDetail   mem_ctx_detail[TOP50_MEMDETAIL_LEN] = {{{0}}};
	int                   mem_ctx_detail_size = 0;
	volatile PgBackendStatus *beentry = MyBEEntry;

	GetMemoryDetail(mem_ctx_detail, &mem_ctx_detail_size);
	
	pgstat_increment_changecount_before(beentry);

	memcpy(MyBEEntry->memstat.memory_detail,
		    mem_ctx_detail,
		    TOP50_MEMDETAIL_LEN * sizeof(struct MemoryDetail));
	MyBEEntry->memstat.memory_detail_len = mem_ctx_detail_size;

	pgstat_increment_changecount_after(beentry);
}

void
DoMemCtxDetailBooking(const char *ctxname)
{
	instr_time	start;
	instr_time	duration;
	struct MemCtxDetail memctx_detail[TOP50_MEMDETAIL_LEN] = {{{0}}};
	int                   memctx_alloc_detail_size = 0;
	volatile PgBackendStatus *beentry = MyBEEntry;

	INSTR_TIME_SET_CURRENT(start);

	GetMemoryContextDetail(memctx_detail, &memctx_alloc_detail_size, ctxname);

	INSTR_TIME_SET_CURRENT(duration);
	INSTR_TIME_SUBTRACT(duration, start);

	elog(LOG, "GetMemoryContextDetail on %s takes %.3f ms", ctxname, INSTR_TIME_GET_MILLISEC(duration));

	pgstat_increment_changecount_before(beentry);

	memcpy(MyBEEntry->memstat.memctx_detail,
		    memctx_detail,
		    TOP50_MEMDETAIL_LEN * sizeof(struct MemCtxDetail));
	MyBEEntry->memstat.memctx_detail_len = memctx_alloc_detail_size;
