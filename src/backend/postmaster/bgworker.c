...
ReportBackgroundWorkerExit(slist_mutable_iter *cur)
{
	 ...
	 */
	elog(DEBUG1, "ResetBackgroundWorkerCrashTimes rw_terminate bgw_notify_pid %d, rw->rw_worker.bgw_restart_time %d, rw->rw_terminate %d",
...
bgworker_die(SIGNAL_ARGS)
{
	...
	/*
	 * Calling backstrace() which in turn calls malloc() is not safe inside
	 * signal handler, disable the stacktrace is ok since we're exiting soon.
	 */
	error_stacktrace_level = STACKTRACE_OFF;
...
StartBackgroundWorker(void)
{
	...
	if (upgrade_mode == INPLACE_UPGRADE &&
		(strcmp("ApplyLauncherMain", worker->bgw_function_name) == 0 ||
		strcmp("ApplyWorkerMain", worker->bgw_function_name) == 0))
	{
		elog(FATAL, "ApplyLauncher or ApplyWorker can't run during inplace upgrade");
	}
...
void
EndQueryBackgroundWorker(BackgroundWorkerHandle *handle)
{
	BackgroundWorkerSlot *slot;
	...
	/* Set terminate flag in shared memory, unless slot has been reused. */
	LWLockAcquire(BackgroundWorkerLock, LW_EXCLUSIVE);
	if (handle->generation == slot->generation && slot->pid > 0)
	{
		kill(slot->pid, SIGUSR2);
	}
	LWLockRelease(BackgroundWorkerLock);
}
