...
static List *addTargetToGroupList(ParseState *pstate, TargetEntry *tle,
					 List *grouplist, List *targetlist, int location, bool is_agg);
static WindowClause *findWindowClause(List *wclist, const char *name);
static Node *transformFrameOffset(ParseState *pstate, int frameOptions,
					 Oid rangeopfamily, Oid rangeopcintype, Oid *inRangeFunc,
					 Node *clause, bool *contain_vars);
extern Oid OraGetFirstDisValConstraint(Relation relation);

static void opentenbaseOraCheckJoinForNclob(Node *joinqual);

typedef struct
{
	List	*targetlist;
	bool	 contain_invalid_var;
} sort_expr_context;
...
/* dblink supports DML:insert/update/delete */
	...
	/* Disable Validate, the current table does not allow insertions, deletions, or modifications. */
	if (ORA_MODE && pstate->p_target_relation->cons_disablevalidate)
	{
		Oid conid = OraGetFirstDisValConstraint(pstate->p_target_relation);
		ereport(ERROR,
		        (errcode(ERRCODE_CHECK_VIOLATION),
		         errmsg("No insert/update/delete on table with constraint \"%s\" disabled and "
		                "validated",
		                get_constraint_name(conid))));
	}

#ifdef _PG_ORCL_
	/*
	 * When relation is replaced via synonym, using synonym name as alias to avoid
	 * failed to lookup refname.
	 */
	if (ORA_MODE && relation->alias == NULL)
		rte = addRangeTableEntryForRelation(pstate, pstate->p_target_relation,
										makeAlias(relation->relname, NIL), inh, false);
	else
#endif
	/*
	 * Now build an RTE.
	...
	/* dblink supports DML:insert/update/delete */
#ifdef _PG_ORCL_
	/* set rte as RET_FOREIGN_TABLE */
	if (relation->dblinkname != NULL)
	{
...
/*
 * opentenbase_ora prohibit nclob type join, so check whether join quals
 * include nclob args.
 */
static void
opentenbaseOraCheckJoinForNclob(Node *joinqual)
{
	List		*args = NULL;
	ListCell	*lc = NULL;

	if (!ORA_MODE || !joinqual || !IsA(joinqual, OpExpr))
		return;

	args = ((OpExpr *)joinqual)->args;
	foreach(lc, args)
	{
		if (IsA(lfirst(lc), CoerceViaIO))
		{
			CoerceViaIO *coerce_node = (CoerceViaIO*) lfirst(lc);
			if (TEXTOID == coerce_node->resulttype &&
				COERCE_IMPLICIT_CAST == coerce_node->coerceformat &&
				coerce_node->arg && IsA(coerce_node->arg, Var))
			{
				Var *coerce_arg = (Var *) coerce_node->arg;
				if (coerce_arg->vartype == NCLOBOID)
					elog(ERROR, "NCLOB do not support join");
			}
		}
	}
}
	...
	foreach(lc, r->functions)
	{
		...
		if (IsA(fexpr, FuncCall))
		{
			...
			if (list_length(fc->funcname) == 1 &&
				downcase_ora_ident_strcmp(strVal(linitial(fc->funcname)), "unnest") == 0 &&
...
transformRangeTableFunc(ParseState *pstate, RangeTableFunc *rtf)
{
	...
	foreach(col, rtf->columns)
	{
		...
		else
		{
			...
			typenameTypeIdAndMod(pstate, rawc->typeName, &typid, &typmod, true);
		}
						...
						List **namespace, bool isMergeInto)
{
	...
	else if (IsA(n, RangeTableSample))
	{
		...
		rel = transformFromClauseItem(pstate, rts->relation,
									  top_rte, top_rti, namespace, false);
	...
	else if (IsA(n, JoinExpr))
	{
        ...
        else
        {
            ...
            l_namespace = list_make1(makeDefaultNSItem(l_rte));
        }
		...
		opentenbaseOraCheckJoinForNclob(j->quals);
					 ...
					 ParseExprKind exprKind, const char *constructName)
{
	...
	if (!IS_CENTRALIZED_MODE && contain_plpgsql_functions((Node *) qual))
	{
					 ...
					 LimitOption limitOption)
{
	...
	qual = coerce_to_specific_type(pstate, qual, target_typeoid, constructName);
	 ...
	 */
	if (!ORA_MODE &&
	    exprKind == EXPR_KIND_LIMIT &&
	    (limitOption == LIMIT_OPTION_WITH_TIES || limitOption == LIMIT_OPTION_PERCENT_WITH_TIES) &&
	    IsA(clause, A_Const) &&
	    ((A_Const *) clause)->val.type == T_Null)
		ereport(ERROR,
...
checkExprIsVarFree(ParseState *pstate, Node *n, const char *constructName)
{
	...
	if (contain_cn_expr(n))
	{
		ereport(ERROR,
				(errcode(ERRCODE_INVALID_COLUMN_REFERENCE),
					/* translator: %s is name of a SQL construct, eg LIMIT */
					errmsg("argument of %s must not contain pseudo column",
						   constructName),
					parser_errposition(pstate,
									   locate_var_of_level(n, 0))));
	}
						 ...
						 ParseExprKind exprKind)
{
		...
		IsA(linitial(((ColumnRef *) node)->fields), String))
	{
		...
		/*
		 * For ORDER BY, give it another try
		 */
		if (exprKind == EXPR_KIND_ORDER_BY && name != NULL && ORA_MODE)
		{
			TargetEntry *target_result = NULL;

			target_result = scanNameSpaceForTle(pstate, name, *tlist, location); 
			if (target_result != NULL)
				return target_result;
		}
	}
	if (IsA(node, A_Const))
	{
		...
		/*
		 * If not a INT position, treat it as a regular expression and transform it
		 * using SQL99 rules.
		 */
#endif
	}
					...
					bool useSQL99)
{
	...
	if (ORA_MODE && exprKind == EXPR_KIND_ORDER_BY)
	{
		pstate->order_list = targetlist;
	}
						   ...
						   List **targetlist)
{
	...
	foreach(lc, windowdefs)
	{
		...
		wc->startOffset = transformFrameOffset(pstate, wc->frameOptions,
											   ...
											   &wc->startInRangeFunc,
											   windef->startOffset,
											   &wc->start_contain_vars);
		if (wc->start_contain_vars)
		{
			ListCell *lc;
			bool      existed = false;
			...
			foreach (lc, *targetlist)
			{
				...
				if (equal(wc->startOffset, tle->expr))
				{
					existed = true;
					break;
				}
			...
			if (!existed)
			{
				...
				tle = makeTargetEntry((Expr *) wc->startOffset,
		...
		if (wc->end_contain_vars)
		{
			...
			foreach (lc, *targetlist)
			{
				...
				if (equal(wc->endOffset, tle->expr))
				{
					existed = true;
					break;
				}
			...
			if (!existed)
			{
				...
				tle = makeTargetEntry((Expr *) wc->endOffset,
						list_length(*targetlist) + 1,
						NULL,
						true);
				*targetlist = lappend(*targetlist, tle);
...
/*
 * check_rowexpr_expand -
 *	Check whether the rowexpr in distinct clause could be expand.
 */
static bool
check_rowexpr_expand(ParseState *pstate, TargetEntry *tle,
					 List *grouplist, List *targetlist, int location)
{
	...
	/*
	 * Check whether columns are supported for distinct expand.
	 */
	foreach(lc, ((RowExpr *)tle->expr)->args)
	{
		...
		if (!targetIsInSortList(tle, InvalidOid, grouplist))
		{
			...
			/* If the expr is not hashable, the rowexpr should not be expanded.*/
			if (!hashable)
			{
				return false;
			}
						...
						List **targetlist, List *sortClause, bool is_agg)
{
	...
	/*
	 * In opentenbase_ora, the expr including the target column as the 'order by ' 
	 * sorting expr is allowed
	 */
	foreach(slitem, sortClause)
	{
		...
		if (tle->resjunk && (!ORA_MODE || !enable_distinct_order_by || IsA(tle->expr, Var)))
			ereport(ERROR,
					(errcode(ERRCODE_INVALID_COLUMN_REFERENCE),
						is_agg ?
	...
	foreach(tlitem, *targetlist)
	{
		...
		if (check_rowexpr_expand(pstate, tle,
								 result, *targetlist,
								 exprLocation((Node *) tle->expr)))
		{
			ListCell	*rlc = NULL;
			List		*tle_list = list_copy(*targetlist);
			AttrNumber	attrno = list_length(*targetlist);

			foreach(rlc, ((RowExpr*)tle->expr)->args)
			{
				/*
				 * RowExpr node should be expaned.
				 */
				TargetEntry * targetEntry = makeTargetEntry((Expr *) lfirst(rlc),
										++attrno,
										NULL,
										true);
				tle_list = lappend(tle_list, targetEntry);

				result = addTargetToGroupList(pstate, targetEntry,
										result, tle_list,
										exprLocation((Node *) targetEntry->expr), is_agg);
			}
		...
		else
		{
			result = addTargetToGroupList(pstate, tle,
										result, *targetlist,
										exprLocation((Node *) tle->expr), is_agg);
		}
	}
...
#ifdef _PG_ORCL_
	/* block sort by blob or clob */
    if (restype == BLOBOID)
        ereport(ERROR,
                (errcode(ERRCODE_WRONG_OBJECT_TYPE),
                 errmsg("There exists unexpected data type of BLOB for sort")));

    if (restype == CLOBOID)
        ereport(ERROR,
                (errcode(ERRCODE_WRONG_OBJECT_TYPE),
                 errmsg("There exists unexpected data type of CLOB for sort")));
#endif
					 ...
					 List *grouplist, List *targetlist, int location, bool is_agg)
{
	...
	if (!targetIsInSortList(tle, InvalidOid, grouplist))
	{
		...
		/* determine the eqop and optional sortop */
		get_sort_group_operators_impl(restype,
								 false, true, false,
								 &sortop, &eqop, NULL,
								 &hashable, is_agg);
...
#ifdef _PG_ORCL_
/*
 * transformWithFunctionClause
 *    Transform CreateFunctionStmt in 'with_funcs' into a List of Const node
 *  constaining a pg_proc tuple as bytea type.
 */
#endif
