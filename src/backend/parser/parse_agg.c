...
typedef struct
{
	List	*group_exprs;
	Node	*ungroup_expr;
} replace_ungrouped_columns_context;
					   ...
					   List *args, List *aggorder, bool agg_distinct)
{
	...
	foreach(lc, args)
	{
		...
		Oid   attoid = exprType((Node *) arg);

		/* opentenbase_ora: long type no support count */
		if (attoid == LRAWOID || attoid == LONGOID)
		{
			elog(ERROR, "illegal use of LONG datatype");
		}
...
#ifdef PGXC
	/* Set for pgxc_shippability_walker */
...
transformGroupingFunc(ParseState *pstate, GroupingFunc *p)
{
	...
	if (ORA_MODE && p->kind == GROUPING_FUNC &&
		list_length(args) > 1)
		ereport(ERROR,
				(errcode(ERRCODE_TOO_MANY_ARGUMENTS),
				 errmsg("invalid number of arguments, GROUPING must contain only one argument"),
				 parser_errposition(pstate, p->location)));
	...
	*p_levelsup = min_varlevel;
	...
	switch (pstate->p_expr_kind)
	{
		...
		case EXPR_KIND_MERGE_WHEN:
			if (isAgg)
				err = _("aggregate functions are not allowed in MERGE WHEN "
						"conditions");
			else
				err = _("grouping operations are not allowed in MERGE WHEN "
						"conditions");
					...
					Expr *filter)
{
	...
	if (ORA_MODE)
	{
		ParseState	*p_pstate = pstate;

		while (p_pstate)
		{
			if (p_pstate->force_agg_local)
				return 0;
			p_pstate = p_pstate->parentParseState;
		}
	}
	/* end ora_compatible */
							   ...
							   check_ungrouped_columns_context *context)
{
	...
	if ((IsA(node, CBRootExpr) ||
		 ...
		 IsA(node, LevelExpr) ||
		 IsA(node, PriorExpr) ||
		 IsA(node, RownumExpr)) && context->sublevels_up == 0)
	{
		...
		ereport(ERROR,
				(errcode(ERRCODE_GROUPING_ERROR),
					errmsg("pseudo column must appear in the GROUP BY clause or be used in an aggregate function"),
	...
	if (IsA(node, Query))
	{
		...
		sub_qry = (Query *) node;
		if (sub_qry->limitOffset || sub_qry->limitCount)
			context->has_limit = true;
		context->sublevels_up++;
		result = query_tree_walker((Query *) sub_qry,
...
/*
 * Like build_aggregate_transfn_expr, but creates an expression tree for the
 * init function of an aggregate, rather than the transition function.
 */
void
build_aggregate_initfn_expr(Oid agg_state_type,
							Oid agg_input_collation,
							Oid initfn_oid,
							Expr **initfnexpr)
{
	...
	fexpr = makeFuncExpr(initfn_oid,
						 agg_state_type,
						 args,
						 InvalidOid,
						 agg_input_collation,
						 COERCE_EXPLICIT_CALL);
	*initfnexpr = (Expr *) fexpr;
}
...
static Node *
replace_ungrouped_columns_mutator(Node *node, void *context)
{
	ListCell	*lc;
	replace_ungrouped_columns_context *ctx =
		(replace_ungrouped_columns_context *) context;
	...
	foreach(lc, ctx->group_exprs)
	{
		if (equal(node, lfirst(lc)))
			return node;
	}

	if (equal(node, ctx->ungroup_expr))
	{
	...
	return expression_tree_mutator(node, replace_ungrouped_columns_mutator,
								   (void *) context);
}

List *
replace_ungrouped_columns(List *exprs, List *grouped, Node *ungrouped)
{
	replace_ungrouped_columns_context ctx;
	ctx.group_exprs = grouped;
	ctx.ungroup_expr = ungrouped;

	return (List *) replace_ungrouped_columns_mutator((Node *) exprs, &ctx);
}
#endif
