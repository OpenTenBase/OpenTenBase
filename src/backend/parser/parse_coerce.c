...
static bool can_bytea_coercion_to(Oid targetTypeId);
static bool can_clob_coercion_to(Oid targetTypeId);
static bool ora_can_coerce_rowtype(Oid	inputTypeId, Oid targetTypeId);
#define MAX_LIGHTWEIGHT_ORA_IMPLICIT_NUM (9)
#define MAX_ORA_IMPLICIT_NUM (7)
const ora_implicit_coercion_rec ora_implicit_coercion_recs[] =
	  {
			{NUMERICOID, INT4OID},
			{FLOAT8OID, FLOAT4OID},
			{FLOAT4OID, INT4OID},
			...
			{VARCHAROID, CLOBOID},
			{VARCHAR2OID, CLOBOID},
			/* below are extra coercion rules for LIGHTWEIGHT_ORA */
			{TIMESTAMPOID, TEXTOID},
			{TIMESTAMPTZOID, TEXTOID}
	  };
#define ORA_DIFF_TYPES_LEN (4)
const Oid ora_diff_types[ORA_DIFF_TYPES_LEN] = {INT2OID, INT4OID, FLOAT4OID, FLOAT8OID};
					  ...
					  int location)
{
	...
	if (!can_coerce_type(1, &exprtype, &targettype, ccontext, false))
		return NULL;
			...
			CoercionContext ccontext, CoercionForm cformat, int location)
{
		...
		targetTypeId == ANYNONARRAYOID)
	{
		...
		{
			if (type_is_nestedtable(inputTypeId))
			{
	...
	if (inputTypeId == UNKNOWNOID && IsA(node, Const))
	{
		...
		if (baseTypeId == NUMERICDOID || baseTypeId == NUMERICFOID)
			baseTypeId = NUMERICOID;
		 ...
		 */
		if (baseTypeId == INTERVALOID || (ORA_MODE && baseTypeId == NUMERICOID))
		...
		if (pstate && pstate->p_coerce_empty_string_hook &&
			pstate->p_coerce_empty_string_hook(pstate, (Node *) con))
			newcon->constlen = NULL_TYPE_STR_LEN;
	...
	{
		/*
		 * char(n) and text/varchar(n) are binary-compatible.
		 * Under opentenbase_ora_compatible mode, we coercion char(n) to text/varchar(n)
		 * by binary-compatible cast instead of function rtrim(bpchar).
		 */
		if (pathtype == COERCION_PATH_FUNC && funcId == BPCHAR_TO_TEXT_OID)
		{
	...
	if (pathtype != COERCION_PATH_NONE)
	{
		if (pathtype != COERCION_PATH_RELABELTYPE)
		{
			...
			if (ORA_MODE && pathtype == COERCION_PATH_FUNC)
			{
				/* Pass interval type's typmod to coerce to text */
				if (funcId == INTERVAL_TO_TEXT_OID)
				{
					funcId = INTERVAL_TYPMOD_TO_TEXT_OID;
					baseTypeMod = exprTypmod(node);
				}
			}
	...
	/*
	 * When targetTypeMod >= 0, it indicates that the target tupdesc exists
	 * and does not support the corresponding conversion.
	 */
	if (targetTypeId == RECORDOID && ISCOMPLEX(inputTypeId) &&
		!(ORA_MODE && targetTypeMod >= 0))
	{
	...
	if (ora_can_coerce_rowtype(inputTypeId, targetTypeId))
	{
		/*
		 * If the composite types corresponding to two tables/views
		 * are identical except for the name, no conversion
		 * is required.
		 */
		return node;
	}
...
bool
is_ora_implicit_coercion(Oid targetTypeId, Oid sourceTypeId)
{
	int i;
	int max_implicit_num;
	TYPCATEGORY	scat, tcat;

	/* must in opentenbase_ora or LIGHTWEIGHT_ORA mode */
	if (!ORA_MODE && !enable_lightweight_ora_syntax)
		return false;

	max_implicit_num = ORA_MODE ? MAX_ORA_IMPLICIT_NUM : MAX_LIGHTWEIGHT_ORA_IMPLICIT_NUM;
	for (i = 0; i < max_implicit_num; i++)
	{
		if (ora_implicit_coercion_recs[i].source_typoid == sourceTypeId &&
			ora_implicit_coercion_recs[i].target_typoid == targetTypeId)
			return true;
	}

	/* Lift the restriction of assignment cast between STRING and NUMBER */
	scat = TypeCategory(sourceTypeId);
	tcat = TypeCategory(targetTypeId);

	if ((scat == TYPCATEGORY_STRING && tcat == TYPCATEGORY_NUMERIC) ||
		(scat == TYPCATEGORY_NUMERIC && tcat == TYPCATEGORY_STRING))
		return true;

	/*
	 * For some ansi data types, we can't implement exactly
	 * the same as opentenbase_ora, like:
	 * INTEGER, INT, SMALLINT in opentenbase_ora are NUMBER, but are
	 * INT4, INT4, INT2 in our opentenbase_ora mode.
	 * DOUBLE PRECISION, REAL in opentenbase_ora are NUMBER (FLOAT is
	 * a subtype of NUMBER), but are FLOAT8, FLOAT4 in our opentenbase_ora
	 * mode.
	 * So we will return true to let the caller use the special
	 * coercion or assignment coercion as implicit coercion
	 * for the INT4, INT2, FLOAT8, FLOAT4 data types.
	 *
	 * the coercion is enabled in none-opentenbase_ora mode if enable_lightweight_ora_syntax is on,
	 * but it must not override enable_opentenbase_ora_implicit_coercion in opentenbase_ora mode
	 */
	if (ENABLE_ORA_IMPLICIT_COERCION)
	{
		for (i = 0; i < ORA_DIFF_TYPES_LEN; i++)
		{
			if (sourceTypeId == ora_diff_types[i] ||
				targetTypeId == ora_diff_types[i])
				return true;
		}
	}
...
static bool
ora_can_coerce_rowtype(Oid	inputTypeId, Oid targetTypeId)
{
	bool	res = false;

	/*
	 * In opentenbase_ora PL/SQL, Table%rowtype or View%rowtype can
	 * be converted if the field types in the two types are
	 * exactly the same.
	 */
	if (ORA_MODE && OidIsValid(inputTypeId) &&
		OidIsValid(targetTypeId) && InPlpgsqlFunc())
	{
		char		ityptype, ttyptype;
		char		ireltype, treltype;

		ityptype = get_typtype(inputTypeId);
		ttyptype = get_typtype(targetTypeId);
		ireltype = get_rel_relkind(typeidTypeRelid(inputTypeId));
		treltype = get_rel_relkind(typeidTypeRelid(targetTypeId));
		if (ityptype == ttyptype && ityptype == TYPTYPE_COMPOSITE &&
			(ireltype == RELKIND_RELATION || ireltype == RELKIND_VIEW ||
			 ireltype == RELKIND_MATVIEW || ireltype == RELKIND_PARTITIONED_TABLE) &&
			(treltype == RELKIND_RELATION || treltype == RELKIND_VIEW ||
			 treltype == RELKIND_MATVIEW || treltype == RELKIND_PARTITIONED_TABLE))
		{
			TupleDesc	stupledesc = NULL;
			TupleDesc	ttupledesc = NULL;

			stupledesc = lookup_rowtype_tupdesc(inputTypeId, -1);
			ttupledesc = lookup_rowtype_tupdesc(targetTypeId, -1);

			if (stupledesc && ttupledesc &&
				tupleDescIsSame(stupledesc, ttupledesc, true))
				res = true;

			ReleaseTupleDesc(stupledesc);
			ReleaseTupleDesc(ttupledesc);
		}
	}
				...
				CoercionContext ccontext, bool use_ora_implicit)
{
	...
	for (i = 0; i < nargs; i++)
	{
		...
		if (inputTypeId == UNKNOWNOID)
			continue;
		if (use_ora_implicit && is_ora_implicit_coercion(targetTypeId, inputTypeId) &&
				   ...
				   bool hideInputCoercion)
{
	...
	if (ORA_MODE && INTERVALOID == targetTypeId && IsA(node, Const))
	{
		Const *expr = (Const*) node;
		if (expr->consttype == targetTypeId && expr->consttypmod != targetTypMod)
		{
			int32 exprTypMod = expr->consttypmod;
			CheckIntervalTypmod(exprTypMod, targetTypMod);
		}
	}
						  ...
						  int location)
{
	...
	else if (pathtype == COERCION_PATH_COERCEVIAIO)
	{
        ...
        iocoerce->resulttypmod = targetTypMod;
			...
			else if (ncategory != pcategory)
			{
				...
				/*------
				  translator: first %s is name of a SQL construct, eg CASE */
					 ...
					 can_coerce_type(1, &ptype, &ntype, COERCION_IMPLICIT, false) &&
					 !can_coerce_type(1, &ntype, &ptype, COERCION_IMPLICIT, false))
			{
				/*
...
Node *
ora_coerce_to_common_type(ParseState *pstate, Node *node,
					  Oid targetTypeId, const char *context)
{
	...
	if (inputTypeId == targetTypeId)
		return node;			/* no work */
	if (can_coerce_type(1, &inputTypeId, &targetTypeId, COERCION_IMPLICIT, false))
	{
		int32 typmod = -1;


		node = coerce_type(pstate, node, inputTypeId, targetTypeId, typmod,
						   COERCION_IMPLICIT, COERCE_IMPLICIT_CAST, -1);
	}
	else if (can_coerce_type(1, &inputTypeId, &targetTypeId, COERCION_EXPLICIT, false))
								 ...
								 bool allow_poly)
{
	...
	if (rettype == ANYARRAYOID)
	{
		...
		{
			array_typeid = get_array_type(elem_typeid);
			if (!OidIsValid(array_typeid) && ORA_MODE)
			{
				Oid typrelid = get_typ_typrelid(elem_typeid);

				if (OidIsValid(typrelid) &&
					get_rel_relkind(typrelid) == RELKIND_NESTED_TABLE_TYPE)
					array_typeid = elem_typeid;
			}
					  ...
					  Oid *funcid)
{
	...
	*funcid = InvalidOid;
	...
	if (!enable_lightweight_ora_syntax && HeapTupleIsValid(tuple))
	{
		const char coercionCtxType = ((Form_pg_cast) GETSTRUCT(tuple))->castcontext;

		if (coercionCtxType == COERCION_CODE_LIGHTWEIGHT_ORA_IMPLICIT ||
			coercionCtxType == COERCION_CODE_LIGHTWEIGHT_ORA_ASSIGNMENT)
		{
			ReleaseSysCache(tuple);
			tuple = NULL;
		}
	...
	if (HeapTupleIsValid(tuple))
	{
		...
		{
			case COERCION_CODE_ORA_IMPLICIT:
				if (enable_opentenbase_ora_implicit_coercion)
					castcontext = COERCION_IMPLICIT;
				else
					/* Ouch, we can't use the opentenbase_ora implicit, so give a bigger castcontext */
					castcontext = COERCION_EXPLICIT + 1;
				break;
			case COERCION_CODE_IMPLICIT:
			case COERCION_CODE_LIGHTWEIGHT_ORA_IMPLICIT:
				castcontext = COERCION_IMPLICIT;
				break;
			/*
			 * Note: Some of our type implementations differ from opentenbase_ora, like
			 * int, float and so on. And mark these coercions related to these
			 * types special.
			 */
			case COERCION_CODE_ORA_SPECIAL:
			case COERCION_CODE_ASSIGNMENT:
			case COERCION_CODE_LIGHTWEIGHT_ORA_ASSIGNMENT:
	...
	else
	{
		...
		if (result == COERCION_PATH_NONE)
		{
			if (ORA_MODE)
			{
				if (sourceTypeId == BYTEAOID && !can_bytea_coercion_to(targetTypeId))
					return result;
				if (sourceTypeId == CLOBOID && !can_clob_coercion_to(targetTypeId))
					return result;
			}
...
#ifdef _PG_ORCL_
		if (result == COERCION_PATH_NONE && (ORA_MODE || enable_lightweight_ora_syntax))
		{
			if (ccontext >= COERCION_ASSIGNMENT &&
				TypeCategory(sourceTypeId) == TYPCATEGORY_STRING &&
				TypeCategory(targetTypeId) == TYPCATEGORY_NUMERIC)
				result = COERCION_PATH_COERCEVIAIO;
		}
#endif
	}
...
/*
 * Check whether bytea can convert to target type
 */
static bool
can_bytea_coercion_to(Oid targetTypeId)
{
	switch (targetTypeId)
	{
		case NCLOBOID:
...
/*
 * Check whether clob can convert to target type
 */
static bool
can_clob_coercion_to(Oid targetTypeId)
{
	switch (targetTypeId)
	{
		case BYTEAOID:
...
#ifdef _PG_ORCL_
/*
 * Like select_common_type() to check if the two type is same catagory.
 */
static bool
is_same_typecatalog(Oid ltype, Oid rtype)
{
	Oid		pltype;
	TYPCATEGORY pcategory;
	bool		pispreferred;
	...
	pltype = getBaseType(ltype);
	get_type_category_preferred(pltype, &pcategory, &pispreferred);

	ntype = getBaseType(rtype);
	get_type_category_preferred(ntype, &ncategory, &nispreferred);

	return pcategory == ncategory;
}

/*
 * setOpTypeCanImplicitCast - for SetOp type coerce.
 */
bool
setOpTypeCanImplicitCast(ParseState *pstate, Node *node, Oid ltype, Oid rtype)
{
	if (IsA(node, Const))
	{
		Const	*c = (Const *) node;

		if (c->constisnull && ltype == TEXTOID && rtype != ltype)
		{
			if (!is_same_typecatalog(ltype, rtype))
				return false;
		}
	}
...
/*
 * Get the cast context between source and target and
 * return '\0' while there are no coercion.
 */
char
getCastContext(Oid sourceTypeId, Oid targetTypeId)
{
	HeapTuple	tuple;
	char 		context = '\0';
