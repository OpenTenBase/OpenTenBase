...
static void seq_nextcall_clean(ParseState *pstate);
...
make_parsestate(ParseState *parentParseState)
{
	...
	pstate->parse_type_object = false;
	pstate->parse_pkg_func_header = false;
	pstate->parse_typobj_fun_info = false;

	pstate->has_outer_join_operator = false;
	pstate->need_ignore_outer_join = false;
	...
	if (parentParseState)
	{
		...
		pstate->p_collectref_hook = parentParseState->p_collectref_hook;
		pstate->p_vartypmod_hook = parentParseState->p_vartypmod_hook;
		pstate->p_coerce_param_hook = parentParseState->p_coerce_param_hook;
		pstate->p_ref_hook_state = parentParseState->p_ref_hook_state;
		pstate->p_coerce_empty_string_hook = parentParseState->p_coerce_empty_string_hook;
		/* query environment stays in context for the whole parse analysis */
		pstate->p_queryEnv = parentParseState->p_queryEnv;

		/*
		 * For quick source identification,
		 * each PState stores a copy of state information to facilitate O(1) complexity determination.
		 * transformParamRef processing does not need to be repeated in CreateTableAsStmt mode.
		 */
		pstate->fromCTAS = parentParseState->fromCTAS;

		/* isDeparsedQuery stays in context for the whole parse analysis */
		pstate->isDeparsedQuery = parentParseState->isDeparsedQuery;
	}
...
free_parsestate(ParseState *pstate)
{
	...
	{
		seq_nextcall_clean(pstate);
		pfree(pstate->hash_seqnextval_calls);
	}
...
/*
 * set_with_local_funcs
 *		Set with_funcs and local_funcs in pstate via SPI
 */
void
set_with_local_funcs(ParseState *pstate)
{
}
						 ...
						 Oid indexTypOid)
{
	...
	{
		Oid	disguiseType = InvalidOid;

		disguiseType = NestedTableGetDisguiseType(arrayType);
		if (disguiseType != InvalidOid)
			arrayType = disguiseType;
	...
	foreach(idx, indirection)
	{
		...
		if (ai->uidx)
		{
			...
			/*
			 * The message comes from CN or DN, and the query is 
			 * deparse_query which has been converted to the expected type. No
			 * further conversion is required.
			 */
			if (ORA_MODE && !pstate->isDeparsedQuery)
			{
				if (firstDim && arrayBase &&
					IsA(arrayBase, Param) && OidIsValid(indexTypOid))
				{
					if (indexTypOid != exprType(subexpr))
					{
						char	*indexTypName = NULL;

						indexTypName = get_typename(indexTypOid);
						if (is_integer_type(indexTypName))
						{
							/*
							 * PLS_INTEGER/BINARY_INTEGER is the mapping type of INTEGER,
							 * and the storage structure is the same: type conversion can
							 * be achieved through subexprType->INTEGER->PLS_INTEGER/
							 * BINARY_INTEGER.
							 */
							subexpr = coerce_to_target_type(pstate,
										subexpr, exprType(subexpr),
			...
			if (subexpr == NULL)
				ereport(ERROR,
						(errcode(ERRCODE_DATATYPE_MISMATCH),
							errmsg("array subscript must have type"),
...
make_const(ParseState *pstate, Value *value, int location)
{
	...
	switch (nodeTag(value))
	{
		...
		case T_String:
			...
			if (strVal(value)[0] == '\0' && enable_lightweight_ora_syntax)
				is_null = true;
		...
		case T_Null:
			typelen = NULL_TYPE_NULL_LEN;
			if ((ORA_MODE || enable_lightweight_ora_syntax) && value->val.str != NULL)
				typelen = NULL_TYPE_STR_LEN;
...
static void
seq_nextcall_clean(ParseState *pstate)
{
	HASH_SEQ_STATUS hseq_status;
	SeqNextCallItem *item = NULL;

	if (pstate->hash_seqnextval_calls == NULL)
		return;

	hash_seq_init(&hseq_status, pstate->hash_seqnextval_calls);
	while ((item = (SeqNextCallItem *) hash_seq_search(&hseq_status)) != NULL)
		hash_search(pstate->hash_seqnextval_calls, &item->seqId, HASH_REMOVE, NULL);
}
