...
PLy_commit(PyObject *self, PyObject *args)
{
	...
	bool is_trans_blk = false;
	List    *nsp_stack = NULL;
	int     *spi_levels = NULL;
	int		 plsub_count = 0;
	MemoryContext orig_ctxt = CurrentMemoryContext;

	if (ORA_MODE)
	{
		spi_levels = GetPLpgSQLSubTransactionNum(&plsub_count);

		nsp_stack = SaveOverrideStack(NULL);

		/*
		 * Before commit, will pop the subtrasactions, later will re-create them.
		 */
		if (plsub_count > 0)
		{
			while (IsPLpgSQLSubTransaction() || IsPlsqlImplicitSavepoint())
			{
				MemoryContext   old_context;

				old_context = MemoryContextSwitchTo(orig_ctxt);
				MemoryContextSwitchTo(old_context);

				SPI_internal_xact(true);
				ReleaseCurrentSubTransaction();
				SPI_internal_xact(false);
			}
		}
		is_trans_blk = IsTransactionBlock();
	}
	...
	if (ORA_MODE)
	{
		SaveOverrideStack(nsp_stack);
		rebuild_subtransaction(orig_ctxt, plsub_count, spi_levels);
	}
