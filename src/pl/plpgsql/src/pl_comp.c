		   ...
		   bool forValidator)
{
	...
	switch (function->fn_is_trigger)
	{
		...
		case PLPGSQL_DML_TRIGGER:
			...
			rec = plpgsql_build_record(ORA_MODE ? "NEW" : "new", 0, RECORDOID, true);
			function->new_varno = rec->dno;

			/* Add the record for referencing OLD ROW */
			rec = plpgsql_build_record(ORA_MODE ? "OLD" : "old", 0, RECORDOID, true);
			function->old_varno = rec->dno;

			/* Add the variable tg_name */
			var = plpgsql_build_variable(ORA_MODE ? "TG_NAME" : "tg_name", 0,
			...
			var = plpgsql_build_variable(ORA_MODE ? "TG_WHEN" : "tg_when", 0,
			...
			var = plpgsql_build_variable(ORA_MODE ? "TG_LEVEL" : "tg_level", 0,
										 plpgsql_build_datatype(TEXTOID,
																-1,
																function->fn_input_collation),
										 true);
			...
			var = plpgsql_build_variable(ORA_MODE ? "TG_OP" : "tg_op", 0,
										 plpgsql_build_datatype(TEXTOID,
																-1,
																function->fn_input_collation),
										 true);
			...
			var = plpgsql_build_variable(ORA_MODE ? "TG_RELID" : "tg_relid", 0,
			...
			var = plpgsql_build_variable(ORA_MODE ? "TG_RELNAME" : "tg_relname", 0,
										 plpgsql_build_datatype(NAMEOID,
																-1,
																InvalidOid),
										 true);
			...
			var = plpgsql_build_variable(ORA_MODE ? "TG_TABLE_NAME" : "tg_table_name", 0,
										 plpgsql_build_datatype(NAMEOID,
																-1,
																InvalidOid),
										 true);
			...
			var = plpgsql_build_variable(ORA_MODE ? "TG_TABLE_SCHEMA" : "tg_table_schema", 0,
										 plpgsql_build_datatype(NAMEOID,
																-1,
																InvalidOid),
										 true);
			...
			var = plpgsql_build_variable(ORA_MODE ? "TG_NARGS" : "tg_nargs", 0,
			...
			var = plpgsql_build_variable(ORA_MODE ? "TG_ARGV" : "tg_argv", 0,
		...
		case PLPGSQL_EVENT_TRIGGER:
			...
			var = plpgsql_build_variable(ORA_MODE ? "TG_EVENT" : "tg_event", 0,
										 plpgsql_build_datatype(TEXTOID,
																-1,
																function->fn_input_collation),
										 true);
			...
			var = plpgsql_build_variable(ORA_MODE ? "TG_TAG" : "tg_tag", 0,
										 plpgsql_build_datatype(TEXTOID,
																-1,
																function->fn_input_collation),
										 true);
	 ...
	 */
	var = plpgsql_build_variable(ORA_MODE ? "FOUND" : "found", 0,
...
plpgsql_param_ref(ParseState *pstate, ParamRef *pref)
{
	...
	/*
	 * plpgsql_ns_lookup processing does not need to be repeated in CreateTableAsStmt mode.
