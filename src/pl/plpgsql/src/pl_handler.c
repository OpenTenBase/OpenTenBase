...
plpgsql_call_handler(PG_FUNCTION_ARGS)
{
	...
	SPI_set_and_check_pl_conflict(PG_PLSQL);
	...
	PG_TRY();
	{
		...
		{
			retval = plpgsql_check_function(func, fcinfo,
										   NULL, NULL,
										   procedure_resowner,
										   !nonatomic, (FuncCheckData *) fcinfo->context);
		}
...
plpgsql_inline_handler(PG_FUNCTION_ARGS)
{
	...
	PG_CATCH();
	{
		 ...
		 * rollback failed, then the plpgsql_subxact_cb at line 413 is handling
		 * the subtransaction of plpgsql_exec_function. However, it is necessary
		 * to also handle the subtransaction of plpgsql_call_handler to avoid
		...
		if (curr_sub_xid > TopSubTransactionId && curr_sub_xid != GetCurrentSubTransactionId())
			plpgsql_subxact_cb(SUBXACT_EVENT_ABORT_SUB, curr_sub_xid, 0, NULL);
