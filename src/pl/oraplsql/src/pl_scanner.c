...
IdentifierLookup oraplsql_IdentifierLookup = IDENTIFIER_LOOKUP_NORMAL;
 ...
 * isn't a known variable name.  If oraplsql_IdentifierLookup is DECLARE then
...
/* Current token's length (corresponds to oraplsql_yylval and oraplsql_yylloc) */
static int	oraplsql_yyleng;

/* Current token's code (corresponds to oraplsql_yylval and oraplsql_yylloc) */
static int	oraplsql_yytoken;
...
/* State for oraplsql_location_to_lineno() */
 ...
 * is turned off by oraplsql_IdentifierLookup, it is returned as
...
oraplsql_yylex(void)
{
	...
	if (tok1 == IDENT || tok1 == PARAM)
	{
		...
		if (tok2 == '.')
		{
			...
			if (tok3 == IDENT)
			{
				...
				if (tok4 == '.')
				{
					...
					if (tok5 == IDENT)
					{
						if (oraplsql_parse_tripword(aux1.lval.str,
												   aux3.lval.str,
												   aux5.lval.str,
												   &aux1.lval.wdatum,
												   &aux1.lval.cword))
					...
					else
					{
						...
						if (oraplsql_parse_dblword(aux1.lval.str,
												  aux3.lval.str,
												  &aux1.lval.wdatum,
												  &aux1.lval.cword))
							tok1 = T_DATUM;
			...
			else
			{
				...
				if (oraplsql_parse_word(aux1.lval.str,
		...
		else
		{
			 ...
			 * unreserved keywords.
			 */
			if (AT_STMT_START(oraplsql_yytoken) &&
				!(tok2 == '=' || tok2 == COLON_EQUALS || tok2 == '['))
			{
				...
				else if (oraplsql_parse_word(aux1.lval.str,
											core_yy.scanbuf + aux1.lloc,
											&aux1.lval.wdatum,
											&aux1.lval.word))
					tok1 = T_DATUM;
	...
	else
	{
		 ...
		 * oraplsql_yytoken in the is-this-start-of-statement test above.
		 */
	}

	oraplsql_yylval = aux1.lval;
	oraplsql_yylloc = aux1.lloc;
	oraplsql_yyleng = aux1.leng;
	oraplsql_yytoken = tok1;
	return tok1;
}
...
internal_yylex(TokenAuxData *auxdata)
{
	...
	{
		token = pg_core_yylex(&auxdata->lval.core_yystype,
						   &auxdata->lloc,
						   yyscanner);
...
/*
 * Push back a single token to be re-read by next oraplsql_yylex() call.
 *
 ...
 */
void
oraplsql_push_back_token(int token)
{
	TokenAuxData auxdata;

	auxdata.lval = oraplsql_yylval;
	auxdata.lloc = oraplsql_yylloc;
	auxdata.leng = oraplsql_yyleng;
 ...
 */
bool
oraplsql_token_is_unreserved_keyword(int token)
{
	int			i;
 ...
 */
void
oraplsql_append_source_text(StringInfo buf,
 ...
 */
void
oraplsql_peek2(int *tok1_p, int *tok2_p, int *tok1_loc, int *tok2_loc)
{
	int			tok1,
				tok2;
	TokenAuxData aux1,
 ...
 */
int
oraplsql_scanner_errposition(int location)
{
	int			pos;
 ...
 * last returned by oraplsql_yylex()).
 ...
 */
void
oraplsql_yyerror(const char *message)
{
	char	   *yytext = core_yy.scanbuf + oraplsql_yylloc;
	...
	if (*yytext == '\0')
	{
		...
		/* translator: %s is typically the translation of "syntax error" */
				 ...
				 oraplsql_scanner_errposition(oraplsql_yylloc)));
	}
	else
	{
		/*
 ...
 */
int
oraplsql_location_to_lineno(int location)
{
	const char *loc;
...
/* initialize or reset the state for oraplsql_location_to_lineno */
static void
 ...
 *
 * Note: the passed "str" must remain valid until oraplsql_scanner_finish().
 * Although it is not fed directly to flex, we need the original string
 * to cite in error messages.
 */
void
oraplsql_scanner_init(const char *str)
{
...
/*
 * Called after parsing is done to clean up after oraplsql_scanner_init()
 */
void
oraplsql_scanner_finish(void)
{
	/* release storage */
