...
/* ----------
 * oraplsql_ns_init			Initialize namespace processing for a new function
 * ----------
 */
void
oraplsql_ns_init(void)
{
	ns_top = NULL;
}
...
/* ----------
 * oraplsql_ns_push			Create a new namespace level
 * ----------
 */
void
oraplsql_ns_push(const char *label, PLpgSQL_label_type label_type)
{
	if (label == NULL)
		label = "";
	oraplsql_ns_additem(PLPGSQL_NSTYPE_LABEL, (int) label_type, label);
}


/* ----------
 * oraplsql_ns_pop			Pop entries back to (and including) the last label
 * ----------
 */
void
oraplsql_ns_pop(void)
{
	Assert(ns_top != NULL);
...
/* ----------
 * oraplsql_ns_top			Fetch the current namespace chain end
 * ----------
 */
PLpgSQL_nsitem *
oraplsql_ns_top(void)
{
	return ns_top;
}
...
/* ----------
 * oraplsql_ns_additem		Add an item to the current namespace chain
 * ----------
 */
void
oraplsql_ns_additem(PLpgSQL_nsitem_type itemtype, int itemno, const char *name)
{
	PLpgSQL_nsitem *nse;
...
/*
 * Based on scan.l, the function is used to validate identifiers that start with ident_start.
...
{
	return isascii(c) && (isalpha(c) || c == '_' || (c >= '\200' && c <= '\377'));
}

/* ----------
 * oraplsql_ns_lookup		Lookup an identifier in the given namespace chain
 *
 ...
 */
PLpgSQL_nsitem *
oraplsql_ns_lookup(PLpgSQL_nsitem *ns_cur, bool localmode,
				  const char *name1, const char *name2, const char *name3,
				  int *names_used)
{
	/* adapt lightweight_ora, handle :params */
	if (name1 && name1[0] == ':' && is_ident_start(name1[1]))
		name1++;
...
/* ----------
 * oraplsql_ns_lookup_label		Lookup a label in the given namespace chain
 * ----------
 */
PLpgSQL_nsitem *
oraplsql_ns_lookup_label(PLpgSQL_nsitem *ns_cur, const char *name)
{
	while (ns_cur != NULL)
	{
...
/* ----------
 * oraplsql_ns_find_nearest_loop		Find innermost loop label in namespace chain
 * ----------
 */
PLpgSQL_nsitem *
oraplsql_ns_find_nearest_loop(PLpgSQL_nsitem *ns_cur)
{
	while (ns_cur != NULL)
	{
		if (ns_cur->itemtype == PLPGSQL_NSTYPE_LABEL &&
 ...
 */
const char *
oraplsql_stmt_typename(PLpgSQL_stmt *stmt)
{
 ...
 */
const char *
oraplsql_getdiag_kindname(PLpgSQL_getdiag_kind kind)
{
	switch (kind)
	{
...
void
oraplsql_free_function_memory(PLpgSQL_function *func)
{
	int			i;
...
dump_getdiag(PLpgSQL_stmt_getdiag *stmt)
{
	...
	foreach(lc, stmt->diag_items)
	{
		...
		printf("{var %d} = %s", diag_item->target,
			   oraplsql_getdiag_kindname(diag_item->kind));
	}
	printf("\n");
}
...
void
oraplsql_dumptree(PLpgSQL_function *func)
{
	int			i;
	PLpgSQL_datum *d;
