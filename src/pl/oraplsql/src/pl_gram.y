...
#define parser_errposition(pos)  oraplsql_scanner_errposition(pos)
...
static	void			 check_sql_expr(const char *stmt, int location,
										int leaderlen);
static	void			 oraplsql_sql_error_callback(void *arg);
...
/*
 * Other tokens recognized by oraplsql's lexer interface layer (pl_scanner.c).
 */
...
pl_function		: comp_options pl_block opt_semi
					{
						oraplsql_parse_result = (PLpgSQL_stmt_block *) $2;
					}
				;
...
comp_option		: '#' K_OPTION K_DUMP
					{
						oraplsql_DumpExecTree = true;
					}
				| '#' K_PRINT_STRICT_PARAMS option_value
					{
						if (downcase_ora_ident_strcmp($3, "on") == 0)
							oraplsql_curr_compile->print_strict_params = true;
						else if (downcase_ora_ident_strcmp($3, "off") == 0)
							oraplsql_curr_compile->print_strict_params = false;
						else
							elog(ERROR, "unrecognized print_strict_params option %s", $3);
					}
				| '#' K_VARIABLE_CONFLICT K_ERROR
					{
						oraplsql_curr_compile->resolve_option = PLPGSQL_RESOLVE_ERROR;
					}
				| '#' K_VARIABLE_CONFLICT K_USE_VARIABLE
					{
						oraplsql_curr_compile->resolve_option = PLPGSQL_RESOLVE_VARIABLE;
					}
				| '#' K_VARIABLE_CONFLICT K_USE_COLUMN
					{
						oraplsql_curr_compile->resolve_option = PLPGSQL_RESOLVE_COLUMN;
					}
				;
...
pl_block		: decl_sect K_BEGIN proc_sect exception_sect K_END opt_label
					{
						...
						new->lineno		= oraplsql_location_to_lineno(@2);
						new->label		= $1.label;
...
decl_sect		: opt_block_label
					{
						...
						oraplsql_IdentifierLookup = IDENTIFIER_LOOKUP_NORMAL;
						$$.label	  = $1;
						$$.n_initvars = 0;
						$$.initvarnos = NULL;
					}
				...
				| opt_block_label decl_start decl_stmts
					{
						...
						$$.n_initvars = oraplsql_add_initdatums(&($$.initvarnos));
					}
				;
...
decl_start		: K_DECLARE
					{
						...
						oraplsql_add_initdatums(NULL);
						/*
						 * Disable scanner lookup of identifiers while
						 * we process the decl_stmts
						 */
						oraplsql_IdentifierLookup = IDENTIFIER_LOOKUP_DECLARE;
					}
				;
...
decl_statement	: decl_varname decl_const decl_datatype decl_collate decl_notnull decl_defval
					{
						...
						var = oraplsql_build_variable($1.name, $1.lineno,
													 $3, true);
						if ($2)
						{
				...
				| decl_varname K_ALIAS K_FOR decl_aliasitem ';'
					{
						oraplsql_ns_additem($4->itemtype,
										   $4->itemno, $1.name);
					}
				| decl_varname opt_scrollable K_CURSOR
					{ oraplsql_ns_push($1.name, PLPGSQL_LABEL_OTHER); }
				  decl_cursor_args decl_is_for decl_cursor_query
					{
						...
						new = (PLpgSQL_var *)
							oraplsql_build_variable($1.name, $1.lineno,
												   oraplsql_build_datatype(REFCURSOROID,
																		  -1,
																		  InvalidOid),
												   true);
				...
				| '(' decl_cursor_arglist ')'
					{
						...
						new->lineno = oraplsql_location_to_lineno(@1);
						new->rowtupdesc = NULL;
...
decl_is_for		:	K_IS |		/* opentenbase_ora */
					K_FOR;		/* SQL standard */
...
decl_aliasitem	: T_WORD
					{
						...
						nsi = oraplsql_ns_lookup(oraplsql_ns_top(), false,
												$1.ident, NULL, NULL,
												NULL);
						if (nsi == NULL)
							ereport(ERROR,
...
decl_varname	: T_WORD
					{
						$$.name = $1.ident;
						$$.lineno = oraplsql_location_to_lineno(@1);
						/*
						...
						if (oraplsql_curr_compile->extra_warnings & PLPGSQL_XCHECK_SHADOWVAR ||
							oraplsql_curr_compile->extra_errors & PLPGSQL_XCHECK_SHADOWVAR)
						{
							PLpgSQL_nsitem *nsi;
							nsi = oraplsql_ns_lookup(oraplsql_ns_top(), false,
													$1.ident, NULL, NULL, NULL);
							if (nsi != NULL)
								ereport(oraplsql_curr_compile->extra_errors & PLPGSQL_XCHECK_SHADOWVAR ? ERROR : WARNING,
...
stmt_call		: K_CALL
					{
						...
						oraplsql_curr_compile->requires_procedure_resowner = true;
										...
										ereport(ERROR,
												(errcode(ERRCODE_SYNTAX_ERROR),
												 errmsg("diagnostics item %s is not allowed in GET STACKED DIAGNOSTICS",
														oraplsql_getdiag_kindname(ditem->kind)),
												 parser_errposition(@1)));
									break;
...
opt_expr_until_when	:
					{
						...
						if (tok != K_WHEN)
						{
							oraplsql_push_back_token(tok);
							expr = read_sql_expression(K_WHEN, "WHEN");
						}
						oraplsql_push_back_token(K_WHEN);
						$$ = expr;
					}
				;
...
for_control		: for_variable K_IN
					{
						...
						if (tok == K_EXECUTE)
						{
							...
							else
							{
								...
								{
									/* Not found a defined loop variable */
									PLpgSQL_var	*lp_var;

									lp_var = (PLpgSQL_var *)
										oraplsql_build_variable($1.name,
													   $1.lineno,
													   oraplsql_build_datatype(RECORDOID,
																  -1,
																  InvalidOid),
													   true);
									new->var = (PLpgSQL_variable *) lp_var;
									check_assignable((PLpgSQL_datum *) lp_var, @1);
								}
								else
									ereport(ERROR,
										(errcode(ERRCODE_DATATYPE_MISMATCH),
...
stmt_exit		: exit_type opt_label opt_exitcond
					{
						...
						if ($2)
						{
							...
							label = oraplsql_ns_lookup_label(oraplsql_ns_top(), $2);
							if (label == NULL)
								ereport(ERROR,
										(errcode(ERRCODE_SYNTAX_ERROR),
						...
						else
						{
							 ...
							 */
							if (oraplsql_ns_find_nearest_loop(oraplsql_ns_top()) == NULL)
								ereport(ERROR,
										(errcode(ERRCODE_SYNTAX_ERROR),
										 new->is_exit ?
						...
						if (tok != ';')
						{
							...
							else if (tok != K_USING)
							{
								...
								else
								{
									...
									else if (oraplsql_token_is_unreserved_keyword(tok))
										new->condname = pstrdup(yylval.keyword);
									else
										yyerror("syntax error");
									oraplsql_recognize_err_condition(new->condname,
																	false);
								}
								tok = yylex();
								if (tok != ';' && tok != K_USING)
...
/*
 * T_WORD+T_CWORD match any initial identifier that is not a known oraplsql
 ...
 * word is a SQL statement keyword that isn't also a oraplsql keyword.
...
stmt_fetch		: K_FETCH opt_fetch_direction cursor_variable K_INTO
					{
						...
						fetch->lineno = oraplsql_location_to_lineno(@1);
						fetch->target	= target;
						fetch->curvar	= $3->dno;
						fetch->is_move	= false;
				...
				| K_EXCEPTION
					{
						 ...
						 * current block.
						 */
						int			lineno = oraplsql_location_to_lineno(@1);
						PLpgSQL_exception_block *new = palloc(sizeof(PLpgSQL_exception_block));
						PLpgSQL_variable *var;

						var = oraplsql_build_variable(ORA_MODE ? "SQLSTATE" : "sqlstate", lineno,
						...
						var = oraplsql_build_variable(ORA_MODE ? "SQLERRM" : "sqlerrm", lineno,
													 oraplsql_build_datatype(TEXTOID,
																			-1,
																			oraplsql_curr_compile->fn_input_collation),
													 true);
...
proc_condition	: any_identifier
						{
							if (downcase_ora_ident_strcmp($1, "sqlstate") != 0)
							{
								$$ = oraplsql_parse_err_condition($1);
							}
							else
							{
...
opt_block_label	:
					{
						oraplsql_ns_push(NULL, PLPGSQL_LABEL_BLOCK);
						$$ = NULL;
					}
				| LESS_LESS any_identifier GREATER_GREATER
					{
						oraplsql_ns_push($2, PLPGSQL_LABEL_BLOCK);
						$$ = $2;
					}
				;
...
opt_loop_label	:
					{
						oraplsql_ns_push(NULL, PLPGSQL_LABEL_LOOP);
						$$ = NULL;
					}
				| LESS_LESS any_identifier GREATER_GREATER
					{
						oraplsql_ns_push($2, PLPGSQL_LABEL_LOOP);
						$$ = $2;
					}
				;
				   ...
				   int *endtoken)
{
	...
	save_IdentifierLookup = oraplsql_IdentifierLookup;
	oraplsql_IdentifierLookup = IDENTIFIER_LOOKUP_EXPR;
	...
	oraplsql_IdentifierLookup = save_IdentifierLookup;
	...
	oraplsql_append_source_text(&ds, startlocation, yylloc);
...
read_datatype(int tok)
{
	...
	Assert(oraplsql_IdentifierLookup == IDENTIFIER_LOOKUP_DECLARE);
	...
	/* Anonymous blocks support a new way of using variables. eg :param */
	if (tok == ':')
	{
		newparam = true;
		tok = yylex();
	...
	if (tok == T_WORD)
	{
		char   *dtname = newparam ? StrConcat(":", (char *) yylval.word.ident) : yylval.word.ident;
		...
		if (tok == '%')
		{
			...
			if (tok_is_keyword(tok, &yylval,
							   K_TYPE, "type"))
			{
				result = oraplsql_parse_wordtype(dtname);
				if (result)
					return result;
			}
			else if (tok_is_keyword(tok, &yylval,
									K_ROWTYPE, "rowtype"))
			{
				result = oraplsql_parse_wordrowtype(dtname);
				if (result)
					return result;
			}
		}
	...
	{
		char   *dtname = newparam ? StrConcat(":", (char *) yylval.keyword) : pstrdup(yylval.keyword);
	...
	/*
	 * Same as LIGHTWEIGHT_ORA implementation, custom types or table types do not support the
	 * :param syntax for variable declaration.
	 */
	else if (tok == T_CWORD && !newparam)
	{
		...
		if (tok == '%')
		{
			...
			if (tok_is_keyword(tok, &yylval,
							   K_TYPE, "type"))
			{
				result = oraplsql_parse_cwordtype(dtnames);
				if (result)
					return result;
			}
			else if (tok_is_keyword(tok, &yylval,
									K_ROWTYPE, "rowtype"))
			{
				result = oraplsql_parse_cwordrowtype(dtnames);
				if (result)
					return result;
			}
		}
...
make_execsql_stmt(int firsttoken, int location)
{
	...
	if (have_into)
	{
		 ...
		 */
		oraplsql_append_source_text(&ds, location, into_start_loc);
		appendStringInfoSpaces(&ds, into_end_loc - into_start_loc);
		oraplsql_append_source_text(&ds, into_end_loc, yylloc);
	}
	else
		oraplsql_append_source_text(&ds, location, yylloc);
	...
	execsql->lineno  = oraplsql_location_to_lineno(location);
...
make_return_stmt(int location)
{
	...
	new->lineno   = oraplsql_location_to_lineno(location);
	new->expr	  = NULL;
	new->retvarno = -1;

	if (oraplsql_curr_compile->fn_retset)
	{
		if (yylex() != ';')
			ereport(ERROR,
					(errcode(ERRCODE_DATATYPE_MISMATCH),
	...
	else if (oraplsql_curr_compile->fn_rettype == VOIDOID)
	{
		if (yylex() != ';')
		{
			if (oraplsql_curr_compile->fn_prokind == PROKIND_PROCEDURE)
				ereport(ERROR,
						(errcode(ERRCODE_SYNTAX_ERROR),
	...
	else if (oraplsql_curr_compile->out_param_varno >= 0)
	{
		if (yylex() != ';')
			ereport(ERROR,
					(errcode(ERRCODE_DATATYPE_MISMATCH),
					 ...
					 parser_errposition(yylloc)));
		new->retvarno = oraplsql_curr_compile->out_param_varno;
	}
	else
	{
		/*
...
make_return_next_stmt(int location)
{
	...
	if (oraplsql_curr_compile->out_param_varno >= 0)
	{
		if (yylex() != ';')
			ereport(ERROR,
					(errcode(ERRCODE_DATATYPE_MISMATCH),
					  ...
					  int initial_location)
{
	...
	row->lineno = oraplsql_location_to_lineno(initial_location);
	row->rowtupdesc = NULL;
	row->nfields = nfields;
...
check_sql_expr(const char *stmt, int location, int leaderlen)
{
	...
	if (!oraplsql_check_syntax)
		return;

	cbarg.location = location;
	cbarg.leaderlen = leaderlen;

	syntax_errcontext.callback = oraplsql_sql_error_callback;
...
static void
oraplsql_sql_error_callback(void *arg)
{
 ...
 * cursor pointing into the oraplsql function source, if necessary.
...
parse_datatype(const char *string, int location)
{
	...
	return oraplsql_build_datatype(type_id, typmod,
								  oraplsql_curr_compile->fn_input_collation);
}
...
read_cursor_args(PLpgSQL_var *cursor, int until, const char *expected)
{
	 ...
	 */
	row = (PLpgSQL_row *) oraplsql_Datums[cursor->cursor_explicit_argrow];
	...
	for (argc = 0; argc < row->nfields; argc++)
	{
		...
		oraplsql_peek2(&tok1, &tok2, &arglocation, NULL);
		  ...
		  List *case_when_list, List *else_stmts)
{
	...
	if (t_expr)
	{
		 ...
		 */
		t_var = (PLpgSQL_var *)
			oraplsql_build_variable(varname, new->lineno,
								   oraplsql_build_datatype(INT4OID,
														  -1,
														  InvalidOid),
								   true);
