 ...
 * oraplsql_inline_handler().  DO blocks still use the simple_econtext_stack,
 ...
 * EState will be cleaned up when we get back to oraplsql_inline_handler.  This
...
} oraplsql_CastHashKey;

typedef struct					/* cast_hash table entry */
{
	oraplsql_CastHashKey key;	/* hash key --- MUST BE FIRST */
...
static void coerce_function_result_tuple(PLpgSQL_execstate *estate,
							 TupleDesc tupdesc);
static void oraplsql_exec_error_callback(void *arg);
static void copy_oraplsql_datums(PLpgSQL_execstate *estate,
...
static void oraplsql_estate_setup(PLpgSQL_execstate *estate,
								 PLpgSQL_function *func,
								 ReturnSetInfo *rsi,
...
static ParamListInfo setup_param_list(PLpgSQL_execstate *estate,
				 PLpgSQL_expr *expr);
static ParamExternData *oraplsql_param_fetch(ParamListInfo params,
					int paramid, bool speculative,
					ParamExternData *workspace);
static void oraplsql_param_compile(ParamListInfo params, Param *param,
					  ExprState *state,
					  Datum *resv, bool *resnull);
static void oraplsql_param_eval_var(ExprState *state, ExprEvalStep *op,
					   ExprContext *econtext);
static void oraplsql_param_eval_var_ro(ExprState *state, ExprEvalStep *op,
						  ExprContext *econtext);
static void oraplsql_param_eval_recfield(ExprState *state, ExprEvalStep *op,
							ExprContext *econtext);
static void oraplsql_param_eval_generic(ExprState *state, ExprEvalStep *op,
						   ExprContext *econtext);
static void oraplsql_param_eval_generic_ro(ExprState *state, ExprEvalStep *op,
							  ExprContext *econtext);
...
static Datum do_cast_value(PLpgSQL_execstate *estate,
						   ...
						   Oid valtype, int32 valtypmod,
						   Oid reqtype, int32 reqtypmod);
static oraplsql_CastHashEntry *get_cast_hashentry(PLpgSQL_execstate *estate,
				   Oid srctype, int32 srctypmod,
				   Oid dsttype, int32 dsttypmod);
static void exec_init_tuple_store(PLpgSQL_execstate *estate);
static void exec_set_found(PLpgSQL_execstate *estate, bool state);
static void oraplsql_create_econtext(PLpgSQL_execstate *estate);
static void oraplsql_destroy_econtext(PLpgSQL_execstate *estate);
...
/* ----------
 * oraplsql_exec_function	Called by the call handler for
 *				function execution.
 *
 ...
 */
Datum
oraplsql_exec_function(PLpgSQL_function *func, FunctionCallInfo fcinfo,
					  EState *simple_eval_estate,
					  ResourceOwner simple_eval_resowner,
					  ...
					  bool atomic, FuncCheckData *tgchkdata)
{
	...
	int			i;
	int			rc;
	int oraplsql_level = SetEnterPlpgsqlFunc();

	/*
	 * Setup the execution state
	 */
	oraplsql_estate_setup(&estate, func, (ReturnSetInfo *) fcinfo->resultinfo,
	 ...
	 */
	plerrcontext.callback = oraplsql_exec_error_callback;
	...
	copy_oraplsql_datums(&estate, func);
	 ...
	 */
	if (*oraplsql_plugin_ptr && (*oraplsql_plugin_ptr)->func_beg)
		((*oraplsql_plugin_ptr)->func_beg) (&estate, func);
	...
	if (estate.check_pullup)
	{
		SetExitPlpgsqlFunc(oraplsql_level);
		return (Datum) 0;
	}
	 ...
	 * Let the instrumentation plugin peek at this function
	 */
	if (*oraplsql_plugin_ptr && (*oraplsql_plugin_ptr)->func_end)
		((*oraplsql_plugin_ptr)->func_end) (&estate, func);

	/* Clean up any leftover temporary memory */
	oraplsql_destroy_econtext(&estate);
	exec_eval_cleanup(&estate);
	...
	if (PlpgsqlDebugPrint)
		elog(LOG,
			"[PLPGSQL] oraplsql_exec_function end g_in_plpgsql_exec_fun:%d",
...
/*
 * Helper for oraplsql_exec_function: coerce composite result to the specified
...
/* ----------
 * oraplsql_exec_trigger		Called by the call handler for
 *				trigger execution.
 * ----------
 */
HeapTuple
oraplsql_exec_trigger(PLpgSQL_function *func,
					 TriggerData *trigdata)
{
	 ...
	 * Setup the execution state
	 */
	oraplsql_estate_setup(&estate, func, NULL, NULL, NULL);
	/* TODO */
	...
	if (PlpgsqlDebugPrint)
		elog(LOG,
			 "[PLPGSQL] oraplsql_exec_trigger end g_in_plpgsql_exec_fun:%d",
			 g_in_plpgsql_exec_fun);
...
/* ----------
 * oraplsql_exec_event_trigger		Called by the call handler for
 *				event trigger execution.
 * ----------
 */
void
oraplsql_exec_event_trigger(PLpgSQL_function *func, EventTriggerData *trigdata)
{
	PLpgSQL_execstate estate;
	ErrorContextCallback plerrcontext;
	int			rc;
 ...
 */
static void
oraplsql_exec_error_callback(void *arg)
{
	...
	else if (estate->err_stmt != NULL)
	{
		...
		if (plpgsql_contenxt_line_num_print)
		{
			errcontext("PL/pgSQL function %s line %d at %s",
			           estate->func->fn_signature,
			           estate->err_stmt->lineno,
			           oraplsql_stmt_typename(estate->err_stmt));
		}
		else
		{
 ...
 */
static void
copy_oraplsql_datums(PLpgSQL_execstate *estate,
					PLpgSQL_function *func)
{
	...
	for (i = 0; i < ndatums; i++)
	{
		...
		/* This must agree with oraplsql_finish_datums on what is copiable */
...
exec_toplevel_block(PLpgSQL_execstate *estate, PLpgSQL_stmt_block *block)
{
	...
	if (*oraplsql_plugin_ptr && (*oraplsql_plugin_ptr)->stmt_beg)
		((*oraplsql_plugin_ptr)->stmt_beg) (estate, (PLpgSQL_stmt *) block);
	...
	if (*oraplsql_plugin_ptr && (*oraplsql_plugin_ptr)->stmt_end)
		((*oraplsql_plugin_ptr)->stmt_end) (estate, (PLpgSQL_stmt *) block);
...
exec_stmt_block(PLpgSQL_execstate *estate, PLpgSQL_stmt_block *block)
{
	...
	for (i = 0; i < block->n_initvars; i++)
	{
		...
		/*
		 * The set of dtypes handled here must match oraplsql_add_initdatums().
		 */
		switch (datum->dtype)
		{
			case PLPGSQL_DTYPE_VAR:
	...
	if (block->exceptions)
	{
		...
		oraplsql_level = SetEnterPlpgsqlFunc();
		...
		PG_TRY();
		{
			 ...
			 */
			oraplsql_create_econtext(estate);
...
exec_stmts(PLpgSQL_execstate *estate, List *stmts)
{
	...
	foreach(s, stmts)
	{
		...
		/* Let the plugin know that we are about to execute this statement */
		if (*oraplsql_plugin_ptr && (*oraplsql_plugin_ptr)->stmt_beg)
			((*oraplsql_plugin_ptr)->stmt_beg) (estate, stmt);
		...
		/* Let the plugin know that we have finished executing this statement */
		if (*oraplsql_plugin_ptr && (*oraplsql_plugin_ptr)->stmt_end)
			((*oraplsql_plugin_ptr)->stmt_end) (estate, stmt);
...
exec_stmt_case(PLpgSQL_execstate *estate, PLpgSQL_stmt_case *stmt)
{
	...
	if (stmt->t_expr != NULL)
	{
		...
		if (t_var->datatype->typoid != t_typoid ||
			...
			t_var->datatype = oraplsql_build_datatype(t_typoid,
													 t_typmod,
...
exec_stmt_foreach_a(PLpgSQL_execstate *estate, PLpgSQL_stmt_foreach_a *stmt)
{
		...
		loop_var->dtype == PLPGSQL_DTYPE_ROW)
	{
		...
		loop_var_elem_type = get_element_type(oraplsql_exec_get_datum_type(estate,
																		  loop_var));
			...
			case PLPGSQL_DTYPE_VAR:
				{
					 ...
					 * is NULL.  (Note that oraplsql_exec_trigger assumes that
...
exec_stmt_raise(PLpgSQL_execstate *estate, PLpgSQL_stmt_raise *stmt)
{
	...
	if (stmt->condname)
	{
		err_code = oraplsql_recognize_err_condition(stmt->condname, true);
				...
				if (err_code)
					ereport(ERROR,
							(errcode(ERRCODE_SYNTAX_ERROR),
							 errmsg("RAISE option already specified: %s",
									"ERRCODE")));
				err_code = oraplsql_recognize_err_condition(extval, true);
 ...
 */
static void
oraplsql_estate_setup(PLpgSQL_execstate *estate,
					 PLpgSQL_function *func,
					 ReturnSetInfo *rsi,
					 EState *simple_eval_estate,
					 ResourceOwner simple_eval_resowner)
{
	...
	/* this link will be restored at exit from oraplsql_call_handler */
	...
	estate->datums = NULL;
	/* the datums array will be filled by copy_oraplsql_datums() */
	...
	estate->paramLI->paramFetch = oraplsql_param_fetch;
	estate->paramLI->paramFetchArg = (void *) estate;
	estate->paramLI->paramCompile = oraplsql_param_compile;
	estate->paramLI->paramCompileArg = NULL;	/* not needed */
	estate->paramLI->parserSetup = (ParserSetupHook) oraplsql_parser_setup;
	...
	if (simple_eval_estate)
	{
		...
		ctl.keysize = sizeof(oraplsql_CastHashKey);
		ctl.entrysize = sizeof(oraplsql_CastHashEntry);
	 ...
	 */
	if (*oraplsql_plugin_ptr)
	{
		(*oraplsql_plugin_ptr)->error_callback = oraplsql_exec_error_callback;
		(*oraplsql_plugin_ptr)->assign_expr = exec_assign_expr;

		if ((*oraplsql_plugin_ptr)->func_setup)
			((*oraplsql_plugin_ptr)->func_setup) (estate, func);
	}
}
				  ...
				  PLpgSQL_expr *expr, int cursorOptions)
{
	...
	plan = SPI_prepare_params(expr->query,
							  (ParserSetupHook) oraplsql_parser_setup,
							  (void *) expr,
							  cursorOptions);
	if (plan == NULL)
...
/*
 * oraplsql_exec_get_datum_type				Get datatype of a PLpgSQL_datum
 *
 ...
 */
Oid
oraplsql_exec_get_datum_type(PLpgSQL_execstate *estate,
							PLpgSQL_datum *datum)
{
	Oid			typeid;
...
/*
 * oraplsql_exec_get_datum_type_info			Get datatype etc of a PLpgSQL_datum
 *
 * An extended version of oraplsql_exec_get_datum_type, which also retrieves the
 ...
 */
void
oraplsql_exec_get_datum_type_info(PLpgSQL_execstate *estate,
								 PLpgSQL_datum *datum,
...
/*
 * oraplsql_param_fetch		paramFetch callback for dynamic parameter fetch
 *
...
oraplsql_param_fetch(ParamListInfo params,
					int paramid, bool speculative,
					ParamExternData *prm)
{
	int			dno;
...
/*
 * oraplsql_param_compile		paramCompile callback for plpgsql parameters
 */
static void
oraplsql_param_compile(ParamListInfo params, Param *param,
					  ExprState *state,
					  Datum *resv, bool *resnull)
{
	PLpgSQL_execstate *estate;
	...
	if (datum->dtype == PLPGSQL_DTYPE_VAR)
	{
		if (dno != expr->rwparam &&
			...
			scratch.d.cparam.paramfunc = oraplsql_param_eval_var_ro;
		else
			scratch.d.cparam.paramfunc = oraplsql_param_eval_var;
	}
	else if (datum->dtype == PLPGSQL_DTYPE_RECFIELD)
		scratch.d.cparam.paramfunc = oraplsql_param_eval_recfield;
	else if (datum->dtype == PLPGSQL_DTYPE_REC &&
			 dno != expr->rwparam)
		scratch.d.cparam.paramfunc = oraplsql_param_eval_generic_ro;
	else
		scratch.d.cparam.paramfunc = oraplsql_param_eval_generic;
...
/*
 * oraplsql_param_eval_var		evaluation of EEOP_PARAM_CALLBACK step
 *
 * This is specialized to the case of DTYPE_VAR variables for which
 * we do not need to invoke MakeExpandedObjectReadOnly.
 */
static void
oraplsql_param_eval_var(ExprState *state, ExprEvalStep *op,
					   ExprContext *econtext)
{
...
/*
 * oraplsql_param_eval_var_ro		evaluation of EEOP_PARAM_CALLBACK step
 *
 * This is specialized to the case of DTYPE_VAR variables for which
 * we need to invoke MakeExpandedObjectReadOnly.
 */
static void
oraplsql_param_eval_var_ro(ExprState *state, ExprEvalStep *op,
						  ExprContext *econtext)
{
	ParamListInfo params;
	PLpgSQL_execstate *estate;
...
/*
 * oraplsql_param_eval_recfield		evaluation of EEOP_PARAM_CALLBACK step
 *
 * This is specialized to the case of DTYPE_RECFIELD variables, for which
 * we never need to invoke MakeExpandedObjectReadOnly.
 */
static void
oraplsql_param_eval_recfield(ExprState *state, ExprEvalStep *op,
							ExprContext *econtext)
{
	ParamListInfo params;
	PLpgSQL_execstate *estate;
...
/*
 * oraplsql_param_eval_generic		evaluation of EEOP_PARAM_CALLBACK step
 *
 * This handles all variable types, but assumes we do not need to invoke
 * MakeExpandedObjectReadOnly.
 */
static void
oraplsql_param_eval_generic(ExprState *state, ExprEvalStep *op,
						   ExprContext *econtext)
{
	ParamListInfo params;
	PLpgSQL_execstate *estate;
...
/*
 * oraplsql_param_eval_generic_ro	evaluation of EEOP_PARAM_CALLBACK step
 *
 ...
 */
static void
oraplsql_param_eval_generic_ro(ExprState *state, ExprEvalStep *op,
							  ExprContext *econtext)
{
	ParamListInfo params;
	PLpgSQL_execstate *estate;
...
do_cast_value(PLpgSQL_execstate *estate,
			  ...
			  Oid reqtype, int32 reqtypmod)
{
	oraplsql_CastHashEntry *cast_entry;
 ...
 *
 * Returns a oraplsql_CastHashEntry if an expression has to be evaluated,
 ...
 * ----------
 */
static oraplsql_CastHashEntry *
get_cast_hashentry(PLpgSQL_execstate *estate,
				   Oid srctype, int32 srctypmod,
				   Oid dsttype, int32 dsttypmod)
{
	oraplsql_CastHashKey cast_key;
	oraplsql_CastHashEntry *cast_entry;
	bool		found;
	...
	cast_entry = (oraplsql_CastHashEntry *) hash_search(estate->cast_hash,
													   (void *) &cast_key,
													   HASH_FIND, NULL);
...
/*
 * oraplsql_create_econtext --- create an eval_econtext for the current function
 *
 ...
 */
static void
oraplsql_create_econtext(PLpgSQL_execstate *estate)
{
	 ...
	 * required EState was already made by oraplsql_inline_handler.  However,
...
/*
 * oraplsql_destroy_econtext --- destroy function's econtext
 *
 * We check that it matches the top stack entry, and destroy the stack
 * entry along with the context.
 */
static void
oraplsql_destroy_econtext(PLpgSQL_execstate *estate)
{
...
/*
 * oraplsql_xact_cb --- post-transaction-commit-or-abort cleanup
 *
 * If a simple-expression EState was created in the current transaction,
 * it has to be cleaned up.  The same for the simple-expression resowner.
 */
void
oraplsql_xact_cb(XactEvent event, void *arg)
{
	/*
...
/*
 * oraplsql_subxact_cb --- post-subtransaction-commit-or-abort cleanup
 *
 ...
 */
void
oraplsql_subxact_cb(SubXactEvent event, SubTransactionId mySubid,
...
Datum
oraplsql_check_function(PLpgSQL_function *func, FunctionCallInfo fcinfo,
							EState *simple_eval_estate,
							ResourceOwner simple_eval_resowner,
							ResourceOwner procedure_resowner,
