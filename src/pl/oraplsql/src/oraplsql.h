...
typedef struct PLpgSQL_stmt_execsql
{
	...
	bool		mod_stmt;		/* is the stmt INSERT/UPDATE/DELETE/MERGE?  Note:
 ...
 *
 * The position of fn_oid needs to be aligned in the corresponding
 * structures of oraplsql.h and plpgsql.h.
 */
 ...
 *
 * The position of func needs to be aligned in the corresponding
 * structures of oraplsql.h and plpgsql.h.
 */
 ...
 * oraplsql_exec_error_callback and exec_assign_expr functions.  This is
...
typedef struct PLpgSQL_plugin
{
	...
	void		(*assign_expr) (PLpgSQL_execstate *estate, PLpgSQL_datum *target,
								PLpgSQL_expr *expr);
	HeapTuple	(*make_tuple) (PLpgSQL_execstate *estate, PLpgSQL_row *row,
								TupleDesc tupdesc);
	char		*(*value_to_string) (PLpgSQL_execstate *estate, Datum value,
									 Oid valtype, int typmod);
} PLpgSQL_plugin;
...
extern IdentifierLookup oraplsql_IdentifierLookup;

extern int	oraplsql_variable_conflict;

extern bool oraplsql_print_strict_params;

extern bool oraplsql_check_asserts;
...
extern int	oraplsql_extra_warnings;
extern int	oraplsql_extra_errors;

extern bool oraplsql_check_syntax;
extern bool oraplsql_DumpExecTree;

extern PLpgSQL_stmt_block *oraplsql_parse_result;

extern int	oraplsql_nDatums;
extern PLpgSQL_datum **oraplsql_Datums;

extern char *oraplsql_error_funcname;

extern PLpgSQL_function *oraplsql_curr_compile;
extern MemoryContext oraplsql_compile_tmp_cxt;

extern PLpgSQL_plugin **oraplsql_plugin_ptr;
 ...
 */
extern PLpgSQL_function *oraplsql_compile(FunctionCallInfo fcinfo,
				bool forValidator);
extern PLpgSQL_function *oraplsql_compile_inline(char *proc_source);
extern void oraplsql_parser_setup(struct ParseState *pstate,
					 PLpgSQL_expr *expr);
extern bool oraplsql_parse_word(char *word1, const char *yytxt,
				   PLwdatum *wdatum, PLword *word);
extern bool oraplsql_parse_dblword(char *word1, char *word2,
					  PLwdatum *wdatum, PLcword *cword);
extern bool oraplsql_parse_tripword(char *word1, char *word2, char *word3,
					   PLwdatum *wdatum, PLcword *cword);
extern PLpgSQL_type *oraplsql_parse_wordtype(char *ident);
extern PLpgSQL_type *oraplsql_parse_cwordtype(List *idents);
extern PLpgSQL_type *oraplsql_parse_wordrowtype(char *ident);
extern PLpgSQL_type *oraplsql_parse_cwordrowtype(List *idents);
extern PLpgSQL_type *oraplsql_build_datatype(Oid typeOid, int32 typmod,
					   Oid collation);
extern PLpgSQL_variable *oraplsql_build_variable(const char *refname, int lineno,
					   PLpgSQL_type *dtype,
					   bool add2namespace);
extern PLpgSQL_rec *oraplsql_build_record(const char *refname, int lineno,
					 Oid rectypeid, bool add2namespace);
extern PLpgSQL_recfield *oraplsql_build_recfield(PLpgSQL_rec *rec,
					   const char *fldname);
extern int oraplsql_recognize_err_condition(const char *condname,
								bool allow_sqlstate);
extern PLpgSQL_condition *oraplsql_parse_err_condition(char *condname);
extern void oraplsql_adddatum(PLpgSQL_datum *new);
extern int	oraplsql_add_initdatums(int **varnos);
extern void oraplsql_HashTableInit(void);
 ...
 */
extern Datum oraplsql_exec_function(PLpgSQL_function *func,
								   FunctionCallInfo fcinfo,
								   EState *simple_eval_estate,
								   ResourceOwner simple_eval_resowner,
								   ResourceOwner procedure_resowner,
								   bool atomic, FuncCheckData *tgchkdata);
extern HeapTuple oraplsql_exec_trigger(PLpgSQL_function *func,
					 TriggerData *trigdata);
extern void oraplsql_exec_event_trigger(PLpgSQL_function *func,
						   EventTriggerData *trigdata);
extern void oraplsql_xact_cb(XactEvent event, void *arg);
extern void oraplsql_subxact_cb(SubXactEvent event, SubTransactionId mySubid,
				   SubTransactionId parentSubid, void *arg);
extern Oid oraplsql_exec_get_datum_type(PLpgSQL_execstate *estate,
							PLpgSQL_datum *datum);
extern void oraplsql_exec_get_datum_type_info(PLpgSQL_execstate *estate,
								 PLpgSQL_datum *datum,
								 Oid *typeid, int32 *typmod, Oid *collation);
extern Datum oraplsql_check_function(PLpgSQL_function *func, FunctionCallInfo fcinfo,
							EState *simple_eval_estate,
							ResourceOwner simple_eval_resowner,
							ResourceOwner procedure_resowner,
 ...
 */
extern void oraplsql_ns_init(void);
extern void oraplsql_ns_push(const char *label,
				PLpgSQL_label_type label_type);
extern void oraplsql_ns_pop(void);
extern PLpgSQL_nsitem *oraplsql_ns_top(void);
extern void oraplsql_ns_additem(PLpgSQL_nsitem_type itemtype, int itemno, const char *name);
extern PLpgSQL_nsitem *oraplsql_ns_lookup(PLpgSQL_nsitem *ns_cur, bool localmode,
				  const char *name1, const char *name2,
				  const char *name3, int *names_used);
extern PLpgSQL_nsitem *oraplsql_ns_lookup_label(PLpgSQL_nsitem *ns_cur,
						const char *name);
extern PLpgSQL_nsitem *oraplsql_ns_find_nearest_loop(PLpgSQL_nsitem *ns_cur);

/*
 * Other functions in pl_funcs.c
 */
extern const char *oraplsql_stmt_typename(PLpgSQL_stmt *stmt);
extern const char *oraplsql_getdiag_kindname(PLpgSQL_getdiag_kind kind);
extern void oraplsql_free_function_memory(PLpgSQL_function *func);
extern void oraplsql_dumptree(PLpgSQL_function *func);
/*
 * Scanner functions in pl_scanner.c
 */
extern int	oraplsql_base_yylex(void);
extern int	oraplsql_yylex(void);
extern void oraplsql_push_back_token(int token);
extern bool oraplsql_token_is_unreserved_keyword(int token);
extern void oraplsql_append_source_text(StringInfo buf,
						   int startlocation, int endlocation);
extern int	oraplsql_peek(void);
extern void oraplsql_peek2(int *tok1_p, int *tok2_p, int *tok1_loc,
			  int *tok2_loc);
extern int	oraplsql_scanner_errposition(int location);
extern void oraplsql_yyerror(const char *message) pg_attribute_noreturn();
extern int	oraplsql_location_to_lineno(int location);
extern int	oraplsql_latest_lineno(void);
extern void oraplsql_scanner_init(const char *str);
extern void oraplsql_scanner_finish(void);

/*
 * Externs in gram.y
 */
extern int	oraplsql_yyparse(void);
