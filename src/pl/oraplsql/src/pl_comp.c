 ...
 * ----------
 */
PLpgSQL_stmt_block *oraplsql_parse_result;

static int	datums_alloc;
int			oraplsql_nDatums;
PLpgSQL_datum **oraplsql_Datums;
static int	datums_last;

char	   *oraplsql_error_funcname;
bool		oraplsql_DumpExecTree = false;
bool		oraplsql_check_syntax = false;

PLpgSQL_function *oraplsql_curr_compile;

/* A context appropriate for short-term allocs during compilation */
MemoryContext oraplsql_compile_tmp_cxt;

/* ----------
 * Hash table for compiled functions
 * ----------
 */
static HTAB *oraplsql_HashTable = NULL;

typedef struct oraplsql_hashent
{
...
static PLpgSQL_function *do_compile(FunctionCallInfo fcinfo,
		   ...
		   bool forValidator);
static void oraplsql_compile_error_callback(void *arg);
static void add_parameter_name(PLpgSQL_nsitem_type itemtype, int itemno, const char *name);
static void add_dummy_return(PLpgSQL_function *function);
static Node *oraplsql_pre_column_ref(ParseState *pstate, ColumnRef *cref);
static Node *oraplsql_post_column_ref(ParseState *pstate, ColumnRef *cref, Node *var);
static Node *oraplsql_param_ref(ParseState *pstate, ParamRef *pref);
...
static void oraplsql_start_datums(void);
static void oraplsql_finish_datums(PLpgSQL_function *function);
static void compute_function_hashkey(FunctionCallInfo fcinfo,
						 Form_pg_proc procStruct,
						 PLpgSQL_func_hashkey *hashkey,
						 bool forValidator);
static void oraplsql_resolve_polymorphic_argtypes(int numargs,
									 Oid *argtypes, char *argmodes,
									 Node *call_expr, bool forValidator,
									 const char *proname);
static PLpgSQL_function *oraplsql_HashTableLookup(PLpgSQL_func_hashkey *func_key);
static void oraplsql_HashTableInsert(PLpgSQL_function *function,
						PLpgSQL_func_hashkey *func_key);
static void oraplsql_HashTableDelete(PLpgSQL_function *function);
static void delete_function(PLpgSQL_function *func);

/* ----------
 * oraplsql_compile		Make an execution tree for a PL/pgSQL function.
 *
 ...
 */
PLpgSQL_function *
oraplsql_compile(FunctionCallInfo fcinfo, bool forValidator)
{
...
recheck:
	if (!function)
	{
		...
		/* And do the lookup */
		function = oraplsql_HashTableLookup(&hashkey);
	}
...
/*
 * This is the slow part of oraplsql_compile().
 *
 ...
 * allocations is the oraplsql_compile_tmp_cxt.
 *
		   ...
		   bool forValidator)
{
	...
	bool		is_dml_trigger = CALLED_AS_TRIGGER(fcinfo) || CALLED_AS_TRIGGER_CHECK(fcinfo);
	...
	oraplsql_scanner_init(proc_source);

	oraplsql_error_funcname = pstrdup(NameStr(procStruct->proname));

	/*
	 * Setup error traceback support for ereport()
	 */
	plerrcontext.callback = oraplsql_compile_error_callback;
	 ...
	 * reasons.
	 */
	oraplsql_check_syntax = forValidator;
	...
	oraplsql_curr_compile = function;
	...
	oraplsql_compile_tmp_cxt = MemoryContextSwitchTo(func_cxt);
	...
	function->resolve_option = oraplsql_variable_conflict;
	function->print_strict_params = oraplsql_print_strict_params;
	/* only promote extra warnings and errors at CREATE FUNCTION time */
	function->extra_warnings = forValidator ? oraplsql_extra_warnings : 0;
	function->extra_errors = forValidator ? oraplsql_extra_errors : 0;
	 ...
	 */
	oraplsql_ns_init();
	oraplsql_ns_push(NameStr(procStruct->proname), PLPGSQL_LABEL_BLOCK);
	oraplsql_DumpExecTree = false;
	oraplsql_start_datums();
	...
	switch (function->fn_is_trigger)
	{
		case PLPGSQL_NOT_TRIGGER:
			 ...
			 */
			MemoryContextSwitchTo(oraplsql_compile_tmp_cxt);

			numargs = get_func_arg_info(procTup,
										&argtypes, &argnames, &argmodes);

			oraplsql_resolve_polymorphic_argtypes(numargs, argtypes, argmodes,
												 fcinfo->flinfo->fn_expr,
												 forValidator,
												 oraplsql_error_funcname);
			...
			for (i = 0; i < numargs; i++)
			{
				...
				argdtype = oraplsql_build_datatype(argtypeid,
												  -1,
				...
				argvariable = oraplsql_build_variable(buf, 0,
													 argdtype, false);
				...
				if (ORA_MODE && numargs == procStruct->pronargs)
				{
					/* plpgsql function established in opentenbase_ora library */
					if (argmode == PROARGMODE_IN ||
						...
						in_arg_varnos[i] = -1;	/* out parameter */
				}
				else if (argmode == PROARGMODE_IN ||
						 argmode == PROARGMODE_INOUT ||
						 (argmode == PROARGMODE_OUT && function->fn_prokind == PROKIND_PROCEDURE) ||
						 argmode == PROARGMODE_VARIADIC)
				...
				(num_out_args == 1 && function->fn_prokind == PROKIND_PROCEDURE))
			{
				...
				oraplsql_adddatum((PLpgSQL_datum *) row);
			...
			if (IsPolymorphicType(procStruct->prorettype) &&
				num_out_args == 0)
			{
				(void) oraplsql_build_variable("$0", 0,
		...
		case PLPGSQL_DML_TRIGGER:
			...
			rec = oraplsql_build_record(ORA_MODE ? "NEW" : "new", 0, RECORDOID, true);
			function->new_varno = rec->dno;

			/* Add the record for referencing OLD ROW */
			rec = oraplsql_build_record(ORA_MODE ? "OLD" : "old", 0, RECORDOID, true);
			function->old_varno = rec->dno;

			/* Add the variable tg_name */
			var = oraplsql_build_variable(ORA_MODE ? "TG_NAME" : "tg_name", 0,
										 oraplsql_build_datatype(NAMEOID,
																-1,
																InvalidOid),
										 true);
			...
			var = oraplsql_build_variable(ORA_MODE ? "TG_WHEN" : "tg_when", 0,
										 oraplsql_build_datatype(TEXTOID,
																-1,
																function->fn_input_collation),
										 true);
			...
			var = oraplsql_build_variable(ORA_MODE ? "TG_LEVEL" : "tg_level", 0,
										 oraplsql_build_datatype(TEXTOID,
																-1,
																function->fn_input_collation),
										 true);
			...
			var = oraplsql_build_variable(ORA_MODE ? "TG_OP" : "tg_op", 0,
										 oraplsql_build_datatype(TEXTOID,
																-1,
																function->fn_input_collation),
										 true);
			...
			var = oraplsql_build_variable(ORA_MODE ? "TG_RELID" : "tg_relid", 0,
										 oraplsql_build_datatype(OIDOID,
																-1,
																InvalidOid),
										 true);
			...
			var = oraplsql_build_variable(ORA_MODE ? "TG_RELNAME" : "tg_relname", 0,
										 oraplsql_build_datatype(NAMEOID,
																-1,
																InvalidOid),
										 true);
			...
			var = oraplsql_build_variable(ORA_MODE ? "TG_TABLE_NAME" : "tg_table_name", 0,
										 oraplsql_build_datatype(NAMEOID,
																-1,
																InvalidOid),
										 true);
			...
			var = oraplsql_build_variable(ORA_MODE ? "TG_TABLE_SCHEMA" : "tg_table_schema", 0,
										 oraplsql_build_datatype(NAMEOID,
																-1,
																InvalidOid),
										 true);
			...
			var = oraplsql_build_variable(ORA_MODE ? "TG_NARGS" : "tg_nargs", 0,
										 oraplsql_build_datatype(INT4OID,
																-1,
																InvalidOid),
										 true);
			...
			var = oraplsql_build_variable(ORA_MODE ? "TG_ARGV" : "tg_argv", 0,
										 oraplsql_build_datatype(TEXTARRAYOID,
																-1,
																function->fn_input_collation),
										 true);
		...
		case PLPGSQL_EVENT_TRIGGER:
			...
			var = oraplsql_build_variable(ORA_MODE ? "TG_EVENT" : "tg_event", 0,
										 oraplsql_build_datatype(TEXTOID,
																-1,
																function->fn_input_collation),
										 true);
			...
			var = oraplsql_build_variable(ORA_MODE ? "TG_TAG" : "tg_tag", 0,
										 oraplsql_build_datatype(TEXTOID,
																-1,
																function->fn_input_collation),
										 true);
	 ...
	 */
	var = oraplsql_build_variable(ORA_MODE ? "FOUND" : "found", 0,
								 oraplsql_build_datatype(BOOLOID,
														-1,
														InvalidOid),
								 true);
	 ...
	 */
	parse_rc = oraplsql_yyparse();
	if (parse_rc != 0)
		elog(ERROR, "plpgsql parser returned %d", parse_rc);
	function->action = oraplsql_parse_result;

	oraplsql_scanner_finish();
	pfree(proc_source);
	...
	oraplsql_finish_datums(function);

	/* Debug dump for completed functions */
	if (oraplsql_DumpExecTree)
		oraplsql_dumptree(function);

	/*
	 * add it to the hash table
	 */
	oraplsql_HashTableInsert(function, hashkey);

	/*
	 * Pop the error context stack
	 */
	error_context_stack = plerrcontext.previous;
	oraplsql_error_funcname = NULL;

	oraplsql_check_syntax = false;

	MemoryContextSwitchTo(oraplsql_compile_tmp_cxt);
	oraplsql_compile_tmp_cxt = NULL;
	return function;
}

/* ----------
 * oraplsql_compile_inline	Make an execution tree for an anonymous code block.
 *
 ...
 */
PLpgSQL_function *
oraplsql_compile_inline(char *proc_source)
{
	...
	oraplsql_error_funcname = func_name;
	...
	oraplsql_check_syntax = check_function_bodies;
	...
	oraplsql_ns_init();
	oraplsql_ns_push(func_name, PLPGSQL_LABEL_BLOCK);
	oraplsql_DumpExecTree = false;
	oraplsql_start_datums();
 ...
 */
static void
oraplsql_compile_error_callback(void *arg)
{
	if (arg)
	{
		/*
	...
	if (oraplsql_error_funcname)
		errcontext("compilation of PL/pgSQL function \"%s\" near line %d",
				   oraplsql_error_funcname, oraplsql_latest_lineno());
}
...
add_parameter_name(PLpgSQL_nsitem_type itemtype, int itemno, const char *name)
{
	 ...
	 * disambiguate.
	 */
	if (oraplsql_ns_lookup(oraplsql_ns_top(), true,
						  name, NULL, NULL,
						  NULL) != NULL)
		ereport(ERROR,
				(errcode(ERRCODE_INVALID_FUNCTION_DEFINITION),
	...
	/* OK, add the name */
	oraplsql_ns_additem(itemtype, itemno, name);
}
...
/*
 * oraplsql_parser_setup		set up parser hooks for dynamic parameters
 *
 ...
 */
void
oraplsql_parser_setup(struct ParseState *pstate, PLpgSQL_expr *expr)
{
	pstate->p_pre_columnref_hook = oraplsql_pre_column_ref;
	pstate->p_post_columnref_hook = oraplsql_post_column_ref;
	pstate->p_paramref_hook = oraplsql_param_ref;
...
/*
 * oraplsql_pre_column_ref		parser callback before parsing a ColumnRef
 */
static Node *
oraplsql_pre_column_ref(ParseState *pstate, ColumnRef *cref)
{
...
/*
 * oraplsql_post_column_ref		parser callback after parsing a ColumnRef
 */
static Node *
oraplsql_post_column_ref(ParseState *pstate, ColumnRef *cref, Node *var)
{
	PLpgSQL_expr *expr = (PLpgSQL_expr *) pstate->p_ref_hook_state;
	Node	   *myvar;
	...
	{
		/*
		 * Use table column as this current resolution.
		 */
		return NULL;
	}
...
/*
 * oraplsql_param_ref		parser callback for ParamRefs ($n symbols)
 */
static Node *
oraplsql_param_ref(ParseState *pstate, ParamRef *pref)
{
	PLpgSQL_expr *expr = (PLpgSQL_expr *) pstate->p_ref_hook_state;
	char		pname[32];
	PLpgSQL_nsitem *nse;
	...
	/*
	 * oraplsql_ns_lookup processing does not need to be repeated in CreateTableAsStmt mode.
	 * Because the param resolution was completed before QueryRewriteCTAS.
	 */
	if (!pstate->fromCTAS)
	{
		snprintf(pname, sizeof(pname), "$%d", pref->number);
		
		nse = oraplsql_ns_lookup(expr->ns, false,
								pname, NULL, NULL,
								NULL);
	...
	return make_datum_param(expr, pno, pref->location);
}
...
make_datum_param(PLpgSQL_expr *expr, int dno, int location)
{
	...
	param->paramid = dno + 1;
	oraplsql_exec_get_datum_type_info(estate,
									 datum,
									 &param->paramtype,
									 &param->paramtypmod,
									 &param->paramcollid);
...
/* ----------
 * oraplsql_parse_word		The scanner calls this to postparse
 ...
 */
bool
oraplsql_parse_word(char *word1, const char *yytxt,
				   PLwdatum *wdatum, PLword *word)
{
	 ...
	 */
	if (oraplsql_IdentifierLookup == IDENTIFIER_LOOKUP_NORMAL)
	{
		/*
		 * Do a lookup in the current namespace stack
		 */
		ns = oraplsql_ns_lookup(oraplsql_ns_top(), false,
							   word1, NULL, NULL,
							   NULL);
		...
		if (ns != NULL)
		{
			switch (ns->itemtype)
			{
				case PLPGSQL_NSTYPE_VAR:
				case PLPGSQL_NSTYPE_REC:
					wdatum->datum = oraplsql_Datums[ns->itemno];
					wdatum->ident = word1;
				...
				default:
					/* oraplsql_ns_lookup should never return anything else */
...
/* ----------
 * oraplsql_parse_dblword		Same lookup for two words
 *					separated by a dot.
 * ----------
 */
bool
oraplsql_parse_dblword(char *word1, char *word2,
					  PLwdatum *wdatum, PLcword *cword)
{
	 ...
	 * needed.
	 */
	if (oraplsql_IdentifierLookup != IDENTIFIER_LOOKUP_DECLARE)
	{
		/*
		 * Do a lookup in the current namespace stack
		 */
			...
			switch (ns->itemtype)
			{
				...
				case PLPGSQL_NSTYPE_REC:
					if (nnames == 1)
					{
						...
						rec = (PLpgSQL_rec *) (oraplsql_Datums[ns->itemno]);
						new = oraplsql_build_recfield(rec, word2);
...
/* ----------
 * oraplsql_parse_tripword		Same lookup for three words
 *					separated by dots.
 * ----------
 */
bool
oraplsql_parse_tripword(char *word1, char *word2, char *word3,
					   PLwdatum *wdatum, PLcword *cword)
{
	PLpgSQL_nsitem *ns;
	List	   *idents;
				...
				case PLPGSQL_NSTYPE_REC:
					{
						...
						rec = (PLpgSQL_rec *) (oraplsql_Datums[ns->itemno]);
						new = oraplsql_build_recfield(rec, word3);
...
/* ----------
 * oraplsql_parse_wordtype	The scanner found word%TYPE. word can be
 ...
 */
PLpgSQL_type *
oraplsql_parse_wordtype(char *ident)
{
	PLpgSQL_type *dtype;
	PLpgSQL_nsitem *nse;
	HeapTuple	typeTup;
	 ...
	 * Do a lookup in the current namespace stack
	 */
	nse = oraplsql_ns_lookup(oraplsql_ns_top(), false,
							ident, NULL, NULL,
							NULL);
	...
	if (nse != NULL)
	{
		...
		{
			case PLPGSQL_NSTYPE_VAR:
				return ((PLpgSQL_var *) (oraplsql_Datums[nse->itemno]))->datatype;
	...
	if (typeTup)
	{
		...
		dtype = build_datatype(typeTup, -1,
							   oraplsql_curr_compile->fn_input_collation);
...
/* ----------
 * oraplsql_parse_cwordtype		Same lookup for compositeword%TYPE
 * ----------
 */
PLpgSQL_type *
oraplsql_parse_cwordtype(List *idents)
{
	...
	oldCxt = MemoryContextSwitchTo(oraplsql_compile_tmp_cxt);
	...
	if (list_length(idents) == 2)
	{
		...
		{
			dtype = ((PLpgSQL_var *) (oraplsql_Datums[nse->itemno]))->datatype;
			goto done;
		}
...
/* ----------
 * oraplsql_parse_wordrowtype		Scanner found word%ROWTYPE.
 *					So word must be a table name.
 * ----------
 */
PLpgSQL_type *
oraplsql_parse_wordrowtype(char *ident)
{
	Oid			classOid;
	...
	return oraplsql_build_datatype(get_rel_type_id(classOid), -1, InvalidOid);
}

/* ----------
 * oraplsql_parse_cwordrowtype		Scanner found compositeword%ROWTYPE.
 *			So word must be a namespace qualified table name.
 * ----------
 */
PLpgSQL_type *
oraplsql_parse_cwordrowtype(List *idents)
{
	Oid			classOid;
	RangeVar   *relvar;
	MemoryContext oldCxt;
...
/*
 * oraplsql_build_variable - build a datum-array entry of a given
 * datatype
 *
 ...
 */
PLpgSQL_variable *
oraplsql_build_variable(const char *refname, int lineno, PLpgSQL_type *dtype,
					   bool add2namespace)
{
		...
		case PLPGSQL_TTYPE_SCALAR:
			{
				...
				oraplsql_adddatum((PLpgSQL_datum *) var);
				if (add2namespace)
					oraplsql_ns_additem(PLPGSQL_NSTYPE_VAR,
									   var->dno,
									   refname);
		...
		case PLPGSQL_TTYPE_REC:
			{
				...
				rec = oraplsql_build_record(refname, lineno, dtype->typoid,
										   add2namespace);
 ...
 */
PLpgSQL_rec *
oraplsql_build_record(const char *refname, int lineno, Oid rectypeid,
					 bool add2namespace)
{
	PLpgSQL_rec *rec;
	...
	rec->firstfield = -1;
	rec->erh = NULL;
	oraplsql_adddatum((PLpgSQL_datum *) rec);
	if (add2namespace)
		oraplsql_ns_additem(PLPGSQL_NSTYPE_REC, rec->dno, rec->refname);
 ...
 */
PLpgSQL_recfield *
oraplsql_build_recfield(PLpgSQL_rec *rec, const char *fldname)
{
	...
	while (i >= 0)
	{
		PLpgSQL_recfield *fld = (PLpgSQL_recfield *) oraplsql_Datums[i];
	...
	oraplsql_adddatum((PLpgSQL_datum *) recfield);
 ...
 */
PLpgSQL_type *
oraplsql_build_datatype(Oid typeOid, int32 typmod, Oid collation)
{
	HeapTuple	typeTup;
	PLpgSQL_type *typ;
...
/*
 *	oraplsql_recognize_err_condition
 ...
 */
int
oraplsql_recognize_err_condition(const char *condname, bool allow_sqlstate)
{
	int			i;
 ...
 */
PLpgSQL_condition *
oraplsql_parse_err_condition(char *condname)
{
	int			i;
	 ...
	 */
	if (downcase_ora_ident_strcmp(condname, "others") == 0)
	{
...
/* ----------
 * oraplsql_start_datums			Initialize datum list at compile startup.
 * ----------
 */
static void
oraplsql_start_datums(void)
{
	datums_alloc = 128;
	oraplsql_nDatums = 0;
	/* This is short-lived, so needn't allocate in function's cxt */
	oraplsql_Datums = MemoryContextAlloc(oraplsql_compile_tmp_cxt,
										sizeof(PLpgSQL_datum *) * datums_alloc);
	/* datums_last tracks what's been seen by oraplsql_add_initdatums() */
	datums_last = 0;
}

/* ----------
 * oraplsql_adddatum			Add a variable, record or row
 *					to the compiler's datum list.
 * ----------
 */
void
oraplsql_adddatum(PLpgSQL_datum *new)
{
	if (oraplsql_nDatums == datums_alloc)
	{
		datums_alloc *= 2;
		oraplsql_Datums = repalloc(oraplsql_Datums, sizeof(PLpgSQL_datum *) * datums_alloc);
	}

	new->dno = oraplsql_nDatums;
	oraplsql_Datums[oraplsql_nDatums++] = new;
}

/* ----------
 * oraplsql_finish_datums	Copy completed datum info into function struct.
 * ----------
 */
static void
oraplsql_finish_datums(PLpgSQL_function *function)
{
	Size		copiable_size = 0;
	int			i;

	function->ndatums = oraplsql_nDatums;
	function->datums = palloc(sizeof(PLpgSQL_datum *) * oraplsql_nDatums);
	for (i = 0; i < oraplsql_nDatums; i++)
	{
		function->datums[i] = oraplsql_Datums[i];

		/* This must agree with copy_oraplsql_datums on what is copiable */
...
/* ----------
 * oraplsql_add_initdatums		Make an array of the datum numbers of
 ...
 */
int
oraplsql_add_initdatums(int **varnos)
{
	int			i;
	int			n = 0;
	 ...
	 */
	for (i = datums_last; i < oraplsql_nDatums; i++)
	{
		switch (oraplsql_Datums[i]->dtype)
		{
			case PLPGSQL_DTYPE_VAR:
			case PLPGSQL_DTYPE_REC:
				n++;
			...
			*varnos = (int *) palloc(sizeof(int) * n);
			...
			for (i = datums_last; i < oraplsql_nDatums; i++)
			{
				switch (oraplsql_Datums[i]->dtype)
				{
					case PLPGSQL_DTYPE_VAR:
					case PLPGSQL_DTYPE_REC:
						(*varnos)[n++] = oraplsql_Datums[i]->dno;
						 ...
						 bool forValidator)
{
	...
	if (procStruct->pronargs > 0)
	{
		...
		oraplsql_resolve_polymorphic_argtypes(procStruct->pronargs,
											 hashkey->argtypes,
											 NULL,
											 fcinfo->flinfo->fn_expr,
											 forValidator,
 ...
 */
static void
oraplsql_resolve_polymorphic_argtypes(int numargs,
									 Oid *argtypes, char *argmodes,
									 Node *call_expr, bool forValidator,
									 const char *proname)
{
...
/* exported so we can call it from oraplsql_init() */
void
oraplsql_HashTableInit(void)
{
	HASHCTL		ctl;

	/* don't allow double-initialization */
	Assert(oraplsql_HashTable == NULL);

	memset(&ctl, 0, sizeof(ctl));
	ctl.keysize = sizeof(PLpgSQL_func_hashkey);
	ctl.entrysize = sizeof(oraplsql_HashEnt);
	oraplsql_HashTable = hash_create("PLpgSQL function hash",
									FUNCS_PER_USER,
									&ctl,
...
static PLpgSQL_function *
oraplsql_HashTableLookup(PLpgSQL_func_hashkey *func_key)
{
	oraplsql_HashEnt *hentry;

	hentry = (oraplsql_HashEnt *) hash_search(oraplsql_HashTable,
											 (void *) func_key,
											 HASH_FIND,
											 NULL);
	if (hentry)
...
static void
oraplsql_HashTableInsert(PLpgSQL_function *function,
						PLpgSQL_func_hashkey *func_key)
{
	oraplsql_HashEnt *hentry;
	bool		found;
...
static void
oraplsql_HashTableDelete(PLpgSQL_function *function)
{
	oraplsql_HashEnt *hentry;
