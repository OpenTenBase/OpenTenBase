...
static bool completion_with_uppercase_in_ora;	/* need to convert the string to uppercase in opentenbase_ora mode */
...
do { \
	char *text_new = (char *)text; \
	bool need_process = false; \
	need_process = ORA_MODE && strlen(text) != 0 && strstr(query, "quote_ident"); \
	if (need_process) \
		text_new = process_string_in_ora(text_new, completion_with_uppercase_in_ora); \
	completion_charp = query; \
	matches = completion_matches(text_new, complete_from_query);                   \
	if (need_process) \
		pg_free(text_new); \
} while (0)
...
do { \
	char *text_new = (char *)text; \
	if (ORA_MODE && strlen(text) != 0) \
		text_new = process_string_in_ora(text_new, completion_with_uppercase_in_ora); \
	completion_squery = &(query); \
	completion_charp = addon; \
	matches = completion_matches(text_new, complete_from_schema_query); \
	if (ORA_MODE && strlen(text) != 0) \
		pg_free(text_new); \
} while (0)
...
"   AND (substring(pg_catalog.quote_ident(attname),1,%d)='%s' or %d=0) "\
...
" WHERE (substring(pg_catalog.quote_ident(d.datname),1,%d)='%s' or %d=0) "\
"   AND (d.datistemplate OR pg_catalog.pg_has_role(d.datdba, 'USAGE'))"

#define Query_for_list_of_databases \
"SELECT pg_catalog.quote_ident(datname) FROM pg_catalog.pg_database "\
" WHERE (substring(pg_catalog.quote_ident(datname),1,%d)='%s' or %d=0) "

#define Query_for_list_of_tablespaces \
"SELECT pg_catalog.quote_ident(spcname) FROM pg_catalog.pg_tablespace "\
" WHERE (substring(pg_catalog.quote_ident(spcname),1,%d)='%s' or %d=0)"

#define Query_for_list_of_encodings \
" SELECT DISTINCT pg_catalog.pg_encoding_to_char(conforencoding) "\
"   FROM pg_catalog.pg_conversion "\
"  WHERE (substring(pg_catalog.pg_encoding_to_char(conforencoding),1,%d)=UPPER('%s') or %d=0)"

#define Query_for_list_of_languages \
"SELECT pg_catalog.quote_ident(lanname) "\
"  FROM pg_catalog.pg_language "\
" WHERE lanname != 'internal' "\
"   AND (substring(pg_catalog.quote_ident(lanname),1,%d)='%s' or %d=0)"

#define Query_for_list_of_schemas \
"SELECT pg_catalog.quote_ident(nspname) FROM pg_catalog.pg_namespace "\
" WHERE (substring(pg_catalog.quote_ident(nspname),1,%d)='%s' or %d=0)"
...
" WHERE (substring(name,1,%d)='%s' or %d=0)"
...
"  WHERE (substring(pg_catalog.quote_ident(rsgname),1,%d)='%s' or %d=0)"

#define Query_for_list_of_roles \
" SELECT pg_catalog.quote_ident(rolname) "\
"   FROM pg_catalog.pg_roles "\
"  WHERE (substring(pg_catalog.quote_ident(rolname),1,%d)='%s' or %d=0)"
...
"  FROM pg_catalog.pg_class c1, pg_catalog.pg_class c2, pg_catalog.pg_index i"\
" WHERE c1.oid=i.indrelid and i.indexrelid=c2.oid"\
"       and (%d = pg_catalog.length('%s') or %d=0)"\
...
" WHERE c1.oid=conrelid and (%d = pg_catalog.length('%s') or %d=0)"\
"       and pg_catalog.quote_ident(c1.relname)='%s'"\
...
" WHERE t.oid=contypid and (%d = pg_catalog.length('%s') or %d=0)"\
...
" WHERE (%d = pg_catalog.length('%s') or %d=0)"\
"   AND oid IN "\
...
" WHERE c1.oid=ev_class and (%d = pg_catalog.length('%s') or %d=0)"\
"       and pg_catalog.quote_ident(c1.relname)='%s'"\
"       and pg_catalog.pg_table_is_visible(c1.oid)"
...
" WHERE c1.oid=tgrelid and (%d = pg_catalog.length('%s') or %d=0)"\
"       and pg_catalog.quote_ident(c1.relname)='%s'"\
"       and pg_catalog.pg_table_is_visible(c1.oid)"\
...
"  WHERE (substring(pg_catalog.quote_ident(s.subname),1,%d)='%s' or %d=0) "\
...
"  WHERE (substring(pg_catalog.quote_ident(extname),1,%d)='%s' or %d=0) "

#define Query_for_list_of_available_extensions \
" SELECT pg_catalog.quote_ident(name) "\
"   FROM pg_catalog.pg_available_extensions "\
"  WHERE (substring(pg_catalog.quote_ident(name),1,%d)='%s' or %d=0) AND installed_version IS NULL"
...
"  WHERE (substring(pg_catalog.quote_ident(name),1,%d)='%s' or %d=0)"
...
"  WHERE (substring(pg_catalog.quote_ident(evtname),1,%d)='%s' or %d=0)"
...
"        (substring(pg_catalog.quote_ident(proname),1,%d)='%s' or %d=0)"

#define Query_for_list_of_policies \
" SELECT pg_catalog.quote_ident(polname) "\
"   FROM pg_catalog.pg_policy "\
"  WHERE (substring(pg_catalog.quote_ident(polname),1,%d)='%s' or %d=0)"
...
"    UNION ALL " \
"   SELECT 'DEFAULT' ) ss "\
"  WHERE (pg_catalog.substring(name,1,%%d)='%%s' or %%d=0)"
...
#define THING_LOWER_IN_ORA	(1 << 3)	/* should not be converted to uppercase */
#define THING_NO_SHOW		(THING_NO_CREATE | THING_NO_DROP | THING_NO_ALTER)

static const pgsql_thing_t words_after_create[] = {
	{"ACCESS METHOD", NULL, NULL, THING_NO_ALTER | THING_LOWER_IN_ORA},
	{"AGGREGATE", NULL, &Query_for_list_of_aggregates},
	{"BARRIER", NULL, NULL},	/* Comes barrier name next, so skip it */
	{"CAST", NULL, NULL},		/* Casts have complex structures for names, so
								 * skip it */
	{"COLLATION", "SELECT pg_catalog.quote_ident(collname) FROM pg_catalog.pg_collation WHERE collencoding IN (-1, pg_catalog.pg_char_to_encoding(pg_catalog.getdatabaseencoding())) AND (substring(pg_catalog.quote_ident(collname),1,%d)='%s' and %d=0)", NULL, THING_LOWER_IN_ORA},

	/*
	 * CREATE CONSTRAINT TRIGGER is not supported here because it is designed
	 * to be used only by pg_dump.
	 */
	{"CONFIGURATION", Query_for_list_of_ts_configurations, NULL, THING_NO_SHOW | THING_LOWER_IN_ORA},
	{"CONVERSION", "SELECT pg_catalog.quote_ident(conname) FROM pg_catalog.pg_conversion WHERE (substring(pg_catalog.quote_ident(conname),1,%d)='%s' and %d=0)"},
	{"DATABASE", Query_for_list_of_databases},
	{"DEFAULT PRIVILEGES", NULL, NULL, THING_NO_CREATE | THING_NO_DROP},
	{"DICTIONARY", Query_for_list_of_ts_dictionaries, NULL, THING_NO_SHOW | THING_LOWER_IN_ORA},
	{"DOMAIN", NULL, &Query_for_list_of_domains},
	{"EVENT TRIGGER", NULL, NULL},
	{"EXTENSION", Query_for_list_of_extensions, NULL, THING_LOWER_IN_ORA},
	{"FUNCTION", NULL, &Query_for_list_of_functions},
	{"GROUP", Query_for_list_of_roles},
	{"INDEX", NULL, &Query_for_list_of_indexes},
	{"LANGUAGE", Query_for_list_of_languages, NULL, THING_LOWER_IN_ORA},
	...
	{"PARSER", Query_for_list_of_ts_parsers, NULL, THING_NO_SHOW | THING_LOWER_IN_ORA},
	{"POLICY", NULL, NULL},
	...
	{"RULE", "SELECT pg_catalog.quote_ident(rulename) FROM pg_catalog.pg_rules WHERE (substring(pg_catalog.quote_ident(rulename),1,%d)='%s' or %d=0)"},
	{"SCHEMA", Query_for_list_of_schemas},
	{"SEQUENCE", NULL, &Query_for_list_of_sequences},
	{"SERVER", Query_for_list_of_servers},
	{"STATISTICS", NULL, &Query_for_list_of_statistics},
	{"SUBSCRIPTION", Query_for_list_of_subscriptions},
	{"SYSTEM", NULL, NULL, THING_NO_CREATE | THING_NO_DROP | THING_LOWER_IN_ORA},
	{"TABLE", NULL, &Query_for_list_of_tables},
	{"TABLESPACE", Query_for_list_of_tablespaces},
	{"TEMP", NULL, NULL, THING_NO_DROP | THING_NO_ALTER},	/* for CREATE TEMP TABLE
															 * ... */
	{"TEMPLATE", Query_for_list_of_ts_templates, NULL, THING_NO_SHOW | THING_LOWER_IN_ORA},
	{"TEMPORARY", NULL, NULL, THING_NO_DROP | THING_NO_ALTER},	/* for CREATE TEMPORARY
																 * TABLE ... */
	{"TEXT SEARCH", NULL, NULL, THING_LOWER_IN_ORA},
	{"TRANSFORM", NULL, NULL},
	{"TRIGGER", "SELECT pg_catalog.quote_ident(tgname) FROM pg_catalog.pg_trigger WHERE (substring(pg_catalog.quote_ident(tgname),1,%d)='%s' and %d=0) AND NOT tgisinternal"},
...
static char *process_string_in_ora(char *str, bool upper_case);
...
#define HeadMatches3(p1, p2, p3) \
	...
	static const char *const sql_commands[] = {
		"ABORT", "ALTER", "ANALYZE", "BEGIN", "CALL", "CHECKPOINT", "CLEAN CONNECTION", "CLOSE", "CLUSTER",
	...
	/* Need to convert to uppercase in opentenbase_ora mode */
	sql_mode = get_sql_mode();
	completion_with_uppercase_in_ora = true;
...
/* ALTER */
	...
	{
		completion_info_charp = process_string_in_ora(prev2_wd, false);
	...
	{
		completion_info_charp = process_string_in_ora(prev3_wd, false);
...
/* COMMENT */
	...
	{
		completion_with_uppercase_in_ora = false;
...
/* DROP */
	...
	/* help completing some of the variants */
		...
		COMPLETE_WITH_LIST2("DATA WRAPPER", "TABLE");
	else if (Matches3("DROP", "DATABASE", MatchAny))
		COMPLETE_WITH_CONST("WITH (");
 ...
 */
	...
	{
		if (TailMatches1("SELECT|INSERT|UPDATE|DELETE|TRUNCATE|REFERENCES|TRIGGER|CREATE|CONNECT|TEMPORARY|TEMP|EXECUTE|USAGE|MAINTAIN|ALL"))
			COMPLETE_WITH_CONST("ON");
...
/* NOTIFY --- can be inside EXPLAIN, RULE, etc */
		...
		COMPLETE_WITH_QUERY("SELECT pg_catalog.quote_ident(channel) FROM pg_catalog.pg_listening_channels() AS channel WHERE (substring(pg_catalog.quote_ident(channel),1,%d)='%s' or %d=0)");
...
/* UNLISTEN */
		...
		COMPLETE_WITH_QUERY("SELECT pg_catalog.quote_ident(channel) FROM pg_catalog.pg_listening_channels() AS channel WHERE (substring(pg_catalog.quote_ident(channel),1,%d)='%s' or %d=0) UNION SELECT '*'");
...
/* TODO:  \dc \dd \dl */
	...
	else
	{
		...
		for (i = 0; words_after_create[i].name; i++)
		{
			...
			{
				/* If it is a special object in opentenbase_ora, no need to convert it to uppercase */
				if (words_after_create[i].flags & THING_LOWER_IN_ORA)
					completion_with_uppercase_in_ora = false;
...
_complete_from_query(int is_schema_query, const char *text, int state)
{
	...
	if (state == 0)
	{
		...
		if (is_schema_query)
		{
			...
			appendPQExpBuffer(&query_buffer, "(substring(%s,1,%d)='%s' or %d=0)",
							  completion_squery->result,
							  char_length, e_text, char_length);
			...
			if (strcmp(completion_squery->catname,
					   "pg_catalog.pg_class c") == 0 &&
						...
						!(ORA_MODE && strncmp(text, "PG_", 3) ==0) &&
						!(ORA_MODE && strncmp(text, "\"pg_", 4) ==0) &&
					!(ORA_MODE && strncmp(text, "\"PG_", 4) ==0))
			{
			...
			appendPQExpBuffer(&query_buffer, "\nUNION\n"
							  ...
							  "WHERE (substring(pg_catalog.quote_ident(n.nspname) || '.',1,%d)='%s' or %d=0)",
							  char_length, e_text, char_length);
			appendPQExpBuffer(&query_buffer,
							  " AND (SELECT count(*)"
							  " FROM pg_catalog.pg_namespace"
							  " WHERE (substring(pg_catalog.quote_ident(nspname) || '.',1,%d) ="
							  " substring('%s',1,pg_catalog.length(pg_catalog.quote_ident(nspname))+1) or %d=0)) > 1",
							  char_length, e_text, char_length);

			/* Add special namespace in opentenbase_ora mode */
			if (ORA_MODE && char_length != 0)
			{
				/*
				 * Add in matching schema names, but only if there is more than
				 * one potential match among schema names.
				...
				appendPQExpBuffer(&query_buffer, "\nUNION\n"
								  "SELECT n.nspname || '.' "
								  "FROM pg_catalog.pg_namespace n "
								  "WHERE n.nspname in ('pg_catalog','pg_totast','public','opentenbase_ora','pg_slice')"
								  " AND (substring(n.nspname || '.',1,%d)=lower('%s') or %d=0)",
								  char_length, e_text, char_length);
				appendPQExpBuffer(&query_buffer,
								  " AND (SELECT count(*)"
								  " FROM pg_catalog.pg_namespace"
								  " WHERE (upper(substring(nspname || '.',1,%d)) ="
								  " substring('%s',1,pg_catalog.length(nspname)+1) or %d=0)) > 1",
								  char_length, e_text, char_length);
				...
				if (completion_squery->selcondition)
					appendPQExpBuffer(&query_buffer, "%s AND ",
									  completion_squery->selcondition);
				appendPQExpBuffer(&query_buffer, "(substring(upper(n.nspname) || '.' || %s,1,%d)='%s' or %d=0)",
								  qualresult,
								  char_length, e_text, char_length);
				 ...
				 */
				appendPQExpBuffer(&query_buffer,
								  " AND (upper(substring(n.nspname || '.',1,%d)) ="
								  " substring('%s',1,pg_catalog.length(n.nspname)+1) or %d=0)",
								  char_length, e_text, char_length);
				appendPQExpBuffer(&query_buffer,
								  " AND (SELECT count(*)"
								  " FROM pg_catalog.pg_namespace"
			...
			if (completion_squery->selcondition)
				appendPQExpBuffer(&query_buffer, "%s AND ",
								  completion_squery->selcondition);
			appendPQExpBuffer(&query_buffer, "(substring(pg_catalog.quote_ident(n.nspname) || '.' || %s,1,%d)='%s' or %d=0)",
							  qualresult,
							  char_length, e_text, char_length);
			...
			appendPQExpBuffer(&query_buffer,
							  ...
							  " substring('%s',1,pg_catalog.length(pg_catalog.quote_ident(n.nspname))+1) or %d=0)",
							  char_length, e_text, char_length);
			appendPQExpBuffer(&query_buffer,
							  " AND (SELECT count(*)"
							  " FROM pg_catalog.pg_namespace"
...
static char *
process_string_in_ora(char *str, bool upper_case)
{
	char	*result;
	...
	else if (upper_case)
	{
		...
		cp = result = pg_strdup(str);

		while (*cp)
		{
			*cp = pg_toupper((unsigned char) *cp);
			cp++;
		}
	}
	else
	{
		...
		result = pg_malloc(strlen(str) + 2);
		/* Convert the string 'abc' to '"abc' if it is a special object */
		result[0] = '"';

		for (i = 0; i < strlen(str); i++)
			result[i+1] = pg_tolower((unsigned char) str[i]);
