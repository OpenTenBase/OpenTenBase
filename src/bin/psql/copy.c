...
parse_slash_copy(const char *args)
{
	...
	if (pg_strcasecmp(token, "binary") == 0)
	{
		...
		token = strtokx(NULL, whitespace, ".,()", "\"",
						0, false, false, pset.encoding);
		/* Handle {COPY table_name [ ( column_name [, ...] ) ] TO/FROM ...} case where table_name or schema_name is 'binary'. */
		if (token[0] == '.' || token[0] == '(' || pg_strcasecmp(token, "from") == 0 || pg_strcasecmp(token, "to") == 0)
			goto has_binary_ident;
		if (!token)
			goto error;
	}
...
has_binary_ident:
	...
	if (token[0] == '(')
	{
		/* handle parenthesized column list (including column name and column position).*/
		int depth = 1;
		for (;;)
		{
			xstrcat(&result->before_tofrom, " ");
