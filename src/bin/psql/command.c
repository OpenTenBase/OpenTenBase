...
static backslashResult exec_command_send_extra_lines(PsqlScanState scan_state, bool active_branch);
...
static backslashResult exec_command_nl(PsqlScanState scan_state, bool active_branch);
static backslashResult exec_command_nc(PsqlScanState scan_state, bool active_branch);
			 ...
			 PQExpBuffer previous_buf)
{
	...
	if (strcmp(cmd, "a") == 0)
	...
	else if (strcmp(cmd, "send_extra_lines") == 0)
		status = exec_command_send_extra_lines(scan_state, active_branch);
		...
		status = exec_command_nl(scan_state, active_branch);
	else if (strcmp(cmd, "nc") == 0)
		status = exec_command_nc(scan_state, active_branch);
#endif	/*__OPENTENBASE__*/
	else
...
exec_command_connect(PsqlScanState scan_state, bool active_branch)
{
	...
	/* Connect to new database success, so change the keyword in here */
	if (success)
	{
		if (strcmp(PQSqlMode(pset.db) , "postgresql") == 0)
			PgKeyWordSwitch();
		else
			OraKeyWordSwitch();
	}
	...
	/*
	 * opentenbase-ora-compatible
	 * set sql_mode when use \c to connect to another database
	 */
		...
		switch (cmd[1])
		{
			...
			case 'M':
				switch (cmd[2])
				{
					...
					case '+':
						{
							char	*p_cmd = "p"; /* like \dfp[+] */
							success = describeFunctions(p_cmd, pattern, show_verbose, show_system);
						}
						break;
					default:
						status = PSQL_CMD_UNKNOWN;
			...
			case 'E':
				...
				break;
			case 'K':
				success = listPackages(pattern, show_verbose);
				break;
			case 'r':
				if (cmd[2] == 'd' && cmd[3] == 's')
				{
...
/*
 * \send_extra_lines -- enable/disable send_extra_lines of queries
 */
static backslashResult
exec_command_send_extra_lines(PsqlScanState scan_state, bool active_branch)
{
	bool		success = true;
	...
	if (active_branch)
	{
		...
		if (opt)
			success = ParseVariableBool(opt, "\\send_extra_lines", &pset.send_extra_lines);
		else
			pset.send_extra_lines = !pset.send_extra_lines;
		if (!pset.quiet)
		{
			if (pset.send_extra_lines)
				puts(_("Send_extra_lines is on."));
			else
				puts(_("Send_extra_lines is off."));
		}
		free(opt);
	}
	else
...
#define PARAMS_ARRAY_SIZE	8
	...
	/*query and set the node_name from server*/
	if (NULL == pset.node_name)
	{
		pset.node_name = queryNodeName();
	}
...
/*
 * \nl -- list node info(only master node except GTM)
 */
static backslashResult
exec_command_nl(PsqlScanState scan_state, bool active_branch)
{
	bool		success = listPgxcNode();
	return success ? PSQL_CMD_SKIP_LINE : PSQL_CMD_ERROR;
}

/*
 * \nc -- connect to new node
 */
static backslashResult
exec_command_nc(PsqlScanState scan_state, bool active_branch)
{
		bool		success = true;
		...
		if (active_branch)
		{
			...
			if (opt1 != NULL)
			{
				if (NULL != pset.node_name && 0 == strcmp(opt1, pset.node_name))
				{
					printf(_("You are now connected to database \"%s\" as user \"%s\" on node \"%s\".\n"),
							PQdb(pset.db), PQuser(pset.db), pset.node_name);

				}

				success = queryHostAddrByName(opt1, &opt2, &opt3);
				if (!success)
				{
					printf(_("You can not connect to database \"%s\" as user \"%s\" on node \"%s\".\n"),
							PQdb(pset.db), PQuser(pset.db), opt1);
				}
				else
				{
					if (NULL != pset.node_name)
					{
						free(pset.node_name);
						pset.node_name = strdup(opt1);
					}

					success = do_connect(TRI_DEFAULT, PQdb(pset.db), PQuser(pset.db), opt2, opt3);
					free(opt2);
					free(opt3);
				}
			}
