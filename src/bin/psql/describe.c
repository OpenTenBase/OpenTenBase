...
static void remove_checksum_option(char *reloption);
...
describeAggregates(const char *pattern, bool verbose, bool showSystem)
{
	...
	processSQLNamePattern(pset.db, &buf, pattern, true, false,
						  ...
						  "pg_catalog.pg_function_is_visible(p.oid)", false);
...
describeFunctions(const char *functypes, const char *pattern, bool verbose, bool showSystem)
{
	...
	if (showProcedure && pset.sversion < 100000)
	{
		char		sverbuf[32];
...
describeTypes(const char *pattern, bool verbose, bool showSystem)
{
	...
	processSQLNamePattern(pset.db, &buf, pattern, true, false,
						  ...
						  "pg_catalog.pg_type_is_visible(t.oid)", false);
...
describeOperators(const char *pattern, bool verbose, bool showSystem)
{
	...
	processSQLNamePattern(pset.db, &buf, pattern, !showSystem && !pattern, true,
						  ...
						  "pg_catalog.pg_operator_is_visible(o.oid)", false);
...
listAllDbs(const char *pattern, bool verbose)
{
	...
	printfPQExpBuffer(&buf,
					  ...
					  gettext_noop("opentenbase_ora"),
					  gettext_noop("postgres"),
					  gettext_noop("Sqlmode"));
	if (pset.sversion >= 80400)
		appendPQExpBuffer(&buf,
						  "       d.datcollate as \"%s\",\n"
						  "       d.datctype as \"%s\",\n",
	...
	if (pattern)
		processSQLNamePattern(pset.db, &buf, pattern, false, false,
							  NULL, "d.datname", NULL, NULL, false);
...
permissionsList(const char *pattern)
{
	...
	processSQLNamePattern(pset.db, &buf, pattern, true, false,
						  ...
						  "n.nspname !~ '^pg_' AND pg_catalog.pg_table_is_visible(c.oid)", false);
...
objectDescription(const char *pattern, bool showSystem)
{
	...
	if (pset.sversion >= 80300)
	{
		...
		processSQLNamePattern(pset.db, &buf, pattern, true, false,
							  ...
							  "pg_catalog.pg_opclass_is_visible(o.oid)", true);
	}
	...
	if (pset.sversion >= 90200)
	{
		...
		processSQLNamePattern(pset.db, &buf, pattern, true, false,
							  ...
							  "pg_catalog.pg_opfamily_is_visible(opf.oid)", true);
	}
...
/*
 * Remove "checksum=on" from reloption.
 */
static void
remove_checksum_option(char *reloption)
{
#define CHECKSUM_RELOPTION_NUM 6
	const char *sub[CHECKSUM_RELOPTION_NUM] = {"checksum=on, ", 
												", checksum=on", 
												"checksum=on",
												"checksum='on', ",
												", checksum='on'",
	...
	for (i = 0; i < CHECKSUM_RELOPTION_NUM; i++)
	{
		char	*p = reloption;
		size_t	len = strlen(sub[i]);
		bool	dealed = false;;

		while ((p = strstr(p, sub[i])) != NULL)
		{
...
#ifdef __OPENTENBASE__
						  "c.relpersistence, c.relreplident, c.relstore, c.relhasclusterkey\n"
#else
						  "c.relpersistence, c.relreplident\n"
#endif
						  ", c.relrowidseq, (select seqlocal from %s where seqrelid = c.relrowidseq)\n"
						  "FROM pg_catalog.pg_class c\n "
...
#ifdef __OPENTENBASE__
	tableinfo.relstorage = (pset.sversion >= 90500)?
		*PQgetvalue(res, 0, 14) : 'n';
#endif
	tableinfo.hasPartialClusterKey = strcmp(PQgetvalue(res, 0, 15), "t") == 0;
#ifdef _PG_ORCL_
	tableinfo.hasrowid = atooid(PQgetvalue(res, 0, 16)) != 0;
	tableinfo.rid_seq_local = strcmp(PQgetvalue(res, 0, 17), "t") == 0;
#endif
	PQclear(res);
	res = NULL;
...
/* BEGIN_OPENTENBASE_ORA */
	if (tableinfo.relkind == RELKIND_VIEW)
	{
		...
		printfPQExpBuffer(&buf, "SELECT 1 from pg_catalog.pg_class "
								"where oid = '%s'::pg_catalog.oid and "
								"relpartbound is not null", oid);
...
/* END_OPENTENBASE_ORA */
	...
	for (i = 0; i < numrows; i++)
	{
		...
		if (show_column_details)
		{
			...
			if (tableinfo.relstorage == 'e')
			{
				...
				compress_type = atoi(PQgetvalue(res, i, 9));
				printTableAddCell(&cont, g_compress_method_name[(int32)compress_type], false, false);
			}
	...
	if (tableinfo.ispartition)
	{
		...
		if (verbose)
			appendPQExpBufferStr(&buf,
                                 ...
                                 ",\n (select aname from pg_childalias as pca where pca.childoid = c.oid) as aname");
		appendPQExpBuffer(&buf,
						  "\nFROM pg_catalog.pg_class c"
		...
		if (PQntuples(result) > 0)
		{
			...
			if (verbose)
			{
                ...
                alias_value = PQgetvalue(result, 0, 3);
                if (alias_value != NULL && alias_value[0] != '\0')
                {
                    printfPQExpBuffer(&tmpbuf, _("Partition alias name: %s"),
                                      alias_value);
                    printTableAddFooter(&cont, tmpbuf.data);
                }
            }
			 ...
			 tableinfo.relkind == RELKIND_PARTITIONED_TABLE)
	{
		...
		/* print table partial cluster constraints */
		if (tableinfo.hasPartialClusterKey && tableinfo.relstorage == 'e')
		{
			printfPQExpBuffer(&buf,
							  "SELECT r.conname, "
			...
			if (tuples > 0)
			{
				printTableAddFooter(&cont, _("Partial cluster constraints:"));
				for (i = 0; i < tuples; i++)
				{
...
/* BEGIN_OPENTENBASE_ORA */
	...
	{
		if (fv_invalid)
			printTableAddFooter(&cont, _("View object contains some errors"));
	}
/* END_OPENTENBASE_ORA */
		...
		tableinfo.relkind == RELKIND_PARTITIONED_TABLE)
	{
		...
		if (tableinfo.relkind == RELKIND_FOREIGN_TABLE)
		{
			 ...
			 *
			 * Notice that for some services like tdx, foreign table options
			 * might contain a string of an escape character, such as "\n" or
			 * "\r", as an eol field, which need to be transformed to avoid
			 * displaying incorrectly.
			 */
			ftoptions = replace_eol_char(PQgetvalue(result, 0, 1));
		...
		else
		{
			...
			for (i = 0; i < tuples; i++)
			{
				...
				else
				{
                    ...
                    /* 4 is length of " AS " */
                    char alias_name[4 + NAMEDATALEN] = {0};
                    char *alias_value = PQgetvalue(result, i, 3);
		...
		if (verbose && (tableinfo.relkind == 'r' || tableinfo.relkind == 'p'))
		{
			...
			printfPQExpBuffer(&buf,
                    ...
                    "(SELECT string_agg(attname, ', ') FROM pg_catalog.pg_attribute WHERE attrelid = c.pcrelid AND c.discolnums::int2[] @> ARRAY[attnum] AND attname NOT LIKE '%%pg.dropped%%') as discolumn, "
					...
					"CASE array_length(discolnums, 1) WHEN 0 THEN null ELSE array_to_string(ARRAY( select attname from pg_attribute, (select discolnum, row_number() over() from (select unnest(discolnums) as discolnum FROM pg_catalog.pgxc_class WHERE pcrelid = '%s')) where attrelid = '%s' and attnum = discolnum and attname NOT LIKE '%%pg.dropped%%' order by row_number), ',') END as dis_cols "
					"FROM pg_catalog.pg_attribute a right join pg_catalog.pgxc_class c on a.attrelid = c.pcrelid and c.discolnums::int2[] @> ARRAY[a.attnum], (SELECT count(*) AS dn_cn FROM pg_catalog.pgxc_node WHERE node_type = 'D') as nc WHERE pcrelid = '%s' "
					, LOCATOR_TYPE_RROBIN
			...
			if (tuples > 0)
			{
				...
				idiscols   = PQfnumber(result, "dis_cols");
		...
		tableinfo.reloptions && tableinfo.reloptions[0] != '\0')
	{
		...
		/*
 		 * By default, checksum is enabled, so we don't display the "checksum=on" option.
 		 * Therefore, we remove the "checksum=on" field from the reloption.
 		 */
		remove_checksum_option(tableinfo.reloptions);
...
describeRoles(const char *pattern, bool verbose, bool showSystem)
{
	...
	if (pset.sversion >= 80100)
	{
		...
		processSQLNamePattern(pset.db, &buf, pattern, false, false,
							  NULL, "r.rolname", NULL, NULL, false);
	}
	else
	{
		printfPQExpBuffer(&buf,
		...
		processSQLNamePattern(pset.db, &buf, pattern, false, false,
							  NULL, "u.usename", NULL, NULL, false);
	}
...
/*
 * listPackages
 *    List packages for \dK \dK+
 */
bool
listPackages(const char *pattern, bool verbose)
{
	PQExpBufferData buf;
	PGresult   *res;
	printQueryOpt myopt = pset.popt;
	...
	{
		appendPQExpBuffer(&buf,
				",\n pg_catalog.pg_get_userbyid(pkgowner) as \"%s\""
				",\n CASE WHEN pkgsecdef THEN '%s' ELSE '%s' END AS \"%s\""
				",\n pkgheadersrc as \"%s\""
				",\n pkgbodysrc as \"%s\"",
				gettext_noop("Owner"),
				gettext_noop("definer"),
	...
	processSQLNamePattern(pset.db, &buf, pattern, false, false,
			"n.nspname", "p.pkgname", NULL, NULL, false);
...
listConversions(const char *pattern, bool verbose, bool showSystem)
{
	...
	processSQLNamePattern(pset.db, &buf, pattern, true, false,
						  ...
						  "pg_catalog.pg_conversion_is_visible(c.oid)", false);
...
listCasts(const char *pattern, bool verbose)
{
	...
	processSQLNamePattern(pset.db, &buf, pattern, true, false,
						  ...
						  "pg_catalog.pg_type_is_visible(ts.oid)", false);
	...
	processSQLNamePattern(pset.db, &buf, pattern, true, false,
						  ...
						  "pg_catalog.pg_type_is_visible(tt.oid)", false);
...
listCollations(const char *pattern, bool verbose, bool showSystem)
{
	...
	processSQLNamePattern(pset.db, &buf, pattern, true, false,
						  ...
						  "pg_catalog.pg_collation_is_visible(c.oid)", true);
	...
	if (!showSystem && !pattern)
	{
		if (ORA_MODE)
			appendPQExpBufferStr(&buf,
							 ...
							 "AND oid not in (select p.PKGCRRSPNDINGNS from pg_catalog.pg_package p"
							 " union all select o.OBJCREATENSOID from pg_catalog.pg_type_object o)\n");
		else
			appendPQExpBufferStr(&buf,
							 "WHERE n.nspname !~ '^pg_' AND n.nspname <> 'information_schema'\n");
	}
...
listTSParsers(const char *pattern, bool verbose)
{
	...
	processSQLNamePattern(pset.db, &buf, pattern, false, false,
						  ...
						  "pg_catalog.pg_ts_parser_is_visible(p.oid)", true);
...
listTSDictionaries(const char *pattern, bool verbose)
{
	...
	processSQLNamePattern(pset.db, &buf, pattern, false, false,
						  ...
						  "pg_catalog.pg_ts_dict_is_visible(d.oid)", true);
...
listTSTemplates(const char *pattern, bool verbose)
{
	...
	processSQLNamePattern(pset.db, &buf, pattern, false, false,
						  ...
						  "pg_catalog.pg_ts_template_is_visible(t.oid)", true);
...
listTSConfigs(const char *pattern, bool verbose)
{
	...
	processSQLNamePattern(pset.db, &buf, pattern, false, false,
						  ...
						  "pg_catalog.pg_ts_config_is_visible(c.oid)", true);
...
listForeignServers(const char *pattern, bool verbose)
{
	...
	processSQLNamePattern(pset.db, &buf, pattern, false, false,
						  NULL, "s.srvname", NULL, NULL, false);
...
listUserMappings(const char *pattern, bool verbose)
{
	...
	processSQLNamePattern(pset.db, &buf, pattern, false, false,
						  NULL, "um.srvname", "um.usename", NULL, false);
...
queryNodeName(void)
{
	...
	if (PQopentenbaseVersion(pset.db) < 1)
	{
		return NULL;
	}
	...
	printfPQExpBuffer(&buf, "SELECT node_name FROM PGXC_NODE WHERE node_host = '%s' AND node_port = %s",
					  gettext_noop(PQhost(pset.db)), gettext_noop(PQport(pset.db)));
	...
	{
		/*parse the result in tuple*/
		if (!PQgetisnull(res, 0, 0))
		{
			node_name = PQgetvalue(res, 0, 0);
			if (node_name)
			{
...
/*
 * query the current node's host and port by node_name
 */
bool
queryHostAddrByName(const char *node_name, char **host, char **port)
{
	PQExpBufferData buf;
	PGresult   *res = NULL;
	char *str = NULL;
	...
	*port = NULL;
	...
	printfPQExpBuffer(&buf, "SELECT node_host, node_port FROM PGXC_NODE WHERE node_name = '%s' AND node_cluster_name='opentenbase_cluster' ",
					  gettext_noop(node_name));
...
/*
 * query pgxc node and promot it as "db_name@node_name"
 */
bool listPgxcNode(void)
{
	PQExpBufferData buf;
	PGresult   *res;
	printQueryOpt myopt = pset.popt;
	...
	initPQExpBuffer(&buf);
	printfPQExpBuffer(&buf, "SELECT NODE_NAME, NODE_TYPE, NODE_PORT, NODE_HOST, NODE_CLUSTER_NAME FROM PGXC_NODE WHERE NODE_TYPE <> 'G' ORDER BY NODE_NAME ");
