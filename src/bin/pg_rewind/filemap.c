...
extern XLogSegNo copylogSegNo;

static char *datasegpath(RelFileNode rnode, ForkNumber forknum,
						uint32 attnum, BlockNumber segno);
static int	path_cmp(const void *a, const void *b);
static int	final_filemap_cmp(const void *a, const void *b);
static void filemap_list_to_array(filemap_t *map);
static bool isOldWal(const char* path);
/*
 ...
 */
void
process_source_file(const char * parentpath, const char *path, file_type_t type, 
					size_t newsize, const char *link_target)
{
	...
	/*
	 * Skip other node tablespace files or dir.
	 * if the path contain tablespace flag, but
	 * not has the nodeName flag.
	 */
	if (strstr(path, "/" TABLESPACE_VERSION_DIRECTORY) != NULL &&
			strstr(path, pgxcNodeName) == NULL)
	{
		pg_log(PG_DEBUG, "source dir skip the path %s", path);
		return;
	}
	...
	switch (type)
	{
		...
		case FILE_TYPE_REGULAR:
			...
			if (!exists || !isRelDataFile(path))
			{
                ...
                else if (pg_str_startwith(path, "pg_wal") && isOldWal(path))
                {
                    action = FILE_ACTION_NONE;
                    oldsize = 0;
                }
	...
	entry->pagemap.estorearray = NULL;
	entry->pagemap.estorearray_size = 0;
 ...
 */
void
process_target_file(const char *parentpath, const char *path, file_type_t type, size_t oldsize,
					const char *link_target)
{
	...
	if (strstr(path, "/" TABLESPACE_VERSION_DIRECTORY) != NULL &&
			strstr(path, pgxcNodeName) == NULL)
	{
		pg_log(PG_DEBUG, "target dir skip the path %s", path);
		return;
	}
	/*
 ...
 */
void
process_block_change(bool estorefile, ForkNumber forknum, RelFileNode rnode, 
						BlockNumber blkno, uint32 attnum, uint32 esotre_offset, 
						uint32 size)
{
	char	   *path;
	file_entry_t key;
	...
	offset = (blkno_inseg + 1) * BLCKSZ;

	path = datasegpath(rnode, forknum, attnum, segno);
...
isRelDataFile(const char *path)
{
	...
	nmatch = sscanf(path, "global/%u.%u", &rnode.relNode, &sliceNo);
	...
	else
	{
		nmatch = sscanf(path, "base/%u/%u.%u",
						&rnode.dbNode, &rnode.relNode, &sliceNo);
		if (nmatch == 2 || nmatch == 3)
		{
			...
			if (nmatch == 2)
			{
				sscanf(path, "base/%u/%u_%u.%u", &rnode.dbNode, &rnode.relNode, &attnum, &sliceNo);
			}
	...
	if (matched)
	{
		char	   *check_path = datasegpath(rnode, MAIN_FORKNUM, attnum, sliceNo);
 ...
 */
static char *
datasegpath(RelFileNode rnode, ForkNumber forknum, uint32 attnum, BlockNumber segno)
{
	char	   *path;
	char	   *segpath;
	...
	if (attnum > 0)
	{
		segpath = psprintf("%s_%u.%u", path, attnum, segno);
		pfree(path);
		return segpath;
	}
...
/*
 * Wal files that created before
 * min(diverge checkpoint, minimal replication slot lsn) 
 * can be skipped to avoid unnecessary file transfer.
 */
bool
isOldWal(const char* path)
{
    ...
    if (sscanf(path, "pg_wal/%s", xlogName) != 1)
    {
        pg_log(PG_WARNING, "unrecognized result \"%s\" for current XLOG name\n", path);
        return false;
    }

    /* exclude archive_status and other files in pg_wal */
    if (!IsXLogFileName(xlogName))
        return false;

    XLogFromFileName(xlogName, &tli, &segno);
    if (segno < copylogSegNo)
    {
        printf("%s skip\n", xlogName);
        return true;
    }
