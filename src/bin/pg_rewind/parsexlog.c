...
/*
 * Reads to the end WAL record. Returns the end position of the record, without
 * doing anything with the record itself.
 */
XLogRecPtr
readEndRecord(const char *datadir, XLogRecPtr startptr, int tliIndex)
{
	XLogRecord *record;
	XLogReaderState *xlogreader;
	char	   *errormsg;
	...
	while (true)
	{
		...
		startptr = InvalidXLogRecPtr; /* continue reading at next record */
		endptr = xlogreader->EndRecPtr;
	}

	if (startptr != InvalidXLogRecPtr)
	{
		if (errormsg)
			pg_fatal("could not read WAL record at %X/%X: %s\n",
			         (uint32) (startptr >> 32), (uint32) (startptr), errormsg);
		else
			pg_fatal("could not read WAL record at %X/%X\n",
...
extractPageInfo(XLogReaderState *record)
{
	...
	for (block_id = 0; block_id <= record->max_block_id; block_id++)
	{
		...
		process_block_change(false, forknum, rnode, blkno, 0, 0, 0);
	}
}
