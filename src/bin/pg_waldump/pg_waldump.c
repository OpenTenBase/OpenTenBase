...
static void WriteFPWPage(XLogReaderState *record, RelFileNode *rnode, BlockNumber blk, int block_id);
	...
	if (!config->bkp_details)
	{
		...
		for (block_id = 0; block_id <= record->max_block_id; block_id++)
		{
			...
			if (XLogRecHasBlockImage(record, block_id))
			{
                ...
                {
                    WriteFPWPage(record, &rnode, blk, block_id);
                }
...
static void WriteFPWPage(XLogReaderState *record, RelFileNode *rnode, BlockNumber blk, int block_id)
{
    ...
    fd = open(fpath, O_RDWR | O_CREAT | O_EXCL, S_IRUSR | S_IWUSR);
    if (fd < 0)
        printf("open file error: %s", fpath);


    if (write(fd, page,  XLOG_BLCKSZ) != XLOG_BLCKSZ)
    {
