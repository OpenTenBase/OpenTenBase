...
#endif
static PQExpBuffer with_table_oids_set = NULL;
...
#define fmtQualifiedDumpableLikePG(obj) \
    fmtQualifiedId(fout->remoteVersion, \
                   (obj)->dobj.namespace->dobj.name, \
                   (obj)->dobj.name, true)
...
#ifdef _PG_ORCL_
static int	no_extensions = 0;

#define OPENTENBASE_ORA_DEFAULT_MINSEQUENCE "-999999999999999999999999999"

#define OPENTENBASE_ORA_DEFAULT_MAXSEQUENCE "9999999999999999999999999999"
#endif

/* Set it to true while the search_path is '' */
static bool empty_search_path = false;
...
#ifdef _PG_ORCL_
static void dumpSynonym(Archive *fout, SynoInfo *synoinfo);
#endif
...
static void getSequenceLocalInfo(Archive *fout, TableInfo *tbinfo);
...
static PQExpBuffer createFilterTableOids(char *colname);
static bool isTypeObject(Archive *fout, TypeInfo *typinfo);
static bool isTypeObjectMetaExist(Archive *fout);
static char *getFormattedPkgFunc(Archive *fout, Oid funcoid, bool pkgheader);
...
main(int argc, char **argv)
{
...
#ifdef _PG_ORCL_
		{"no-extensions", no_argument, &no_extensions, 1},
#endif
	...
	/*
	 * Because some keyword have different type in opentenbase_ora and pg mode. So we have
	 * to use proper keyword list here.
	 * For example, LEFT in opentenbase_ora mode is unreserved keyword, and in pg mode it
	 * is function name keyword.
	 */
	if (PQinOraMode())
		OraKeyWordSwitch();
	else
...
#ifdef _PG_ORCL_
	printf(_("  --no-extensions              do not dump extensions\n"));
#endif
				 ...
				 const char *dumpsnapshot, char *use_role)
{
	...
	ExecuteSqlStatement(AH, ALWAYS_SECURE_SEARCH_PATH_SQL);
	...
	if (AH->remoteVersion >= 90100)
	{
		...
		{
			/* pg_dump dependency, enable support for the default RR isolation level. */
			ExecuteSqlStatement(AH, "set enable_experiment_isolation to on");
							...
							bool strict_names)
{
	...
	for (cell = patterns->head; cell; cell = cell->next)
	{
		...
		processSQLNamePattern(GetConnection(fout), query, cell->val, false,
							  false, NULL, "n.nspname", NULL, NULL, false);
						   ...
						   bool strict_names)
{
	...
	for (cell = patterns->head; cell; cell = cell->next)
	{
		...
		processSQLNamePattern(GetConnection(fout), query, cell->val, true,
							  ...
							  "pg_catalog.pg_table_is_visible(c.oid)", false);
		...
		ExecuteSqlStatement(fout, ALWAYS_SECURE_SEARCH_PATH_SQL);
...
selectDumpableNamespace(NamespaceInfo *nsinfo, Archive *fout)
{
	 ...
	 */
	if (table_include_oids.head != NULL || nsinfo->pkg_nsp)
...
#endif
			 (PQinOraMode() ? strcmp(nsinfo->dobj.name, "INFORMATION_SCHEMA") == 0 :
								 strcmp(nsinfo->dobj.name, "information_schema") == 0) ||
			 strcmp(nsinfo->dobj.name, "estore") == 0 ||
			 (PQinOraMode() ? strcmp(nsinfo->dobj.name, "DBMS_RANDOM") == 0 :
								 strcmp(nsinfo->dobj.name, "dbms_random") == 0) ||
			(PQinOraMode() && strcmp(nsinfo->dobj.name, "PG_DBMS_JOB") == 0) ||
			(PQinOraMode() && strcmp(nsinfo->dobj.name, "SYS") == 0) ||
			(PQinOraMode() && strcmp(nsinfo->dobj.name, "SYSTEM") == 0) ||
			(PQinOraMode() ? strcmp(nsinfo->dobj.name, "SQUEEZE") == 0 :
									strcmp(nsinfo->dobj.name, "squeeze") == 0) ||
			(PQinOraMode() ? strcmp(nsinfo->dobj.name, "HINT_PLAN") == 0 :
									strcmp(nsinfo->dobj.name, "hint_plan") == 0))
	{
...
selectDumpableType(TypeInfo *tyinfo, Archive *fout)
{
	...
	if (OidIsValid(tyinfo->typrelid) &&
		...
		tyinfo->typrelkind != RELKIND_NESTED_TABLE_TYPE)
	{
...
dumpTableData(Archive *fout, TableDataInfo *tdinfo)
{
	...
	/* Skip the global temporary table */
	if (tdinfo->tdtable->relpersistence == RELPERSISTENCE_GLOBAL_TEMP)
		return;
...
dumpDatabase(Archive *fout)
{
	...
	if (fout->remoteVersion >= 90300)
	{
		appendPQExpBuffer(dbQry, "SELECT tableoid, oid, "
						  ...
						  "datcollate, datctype, datfrozenxid, datminmxid, datasmxid, datsqlmode, "
	...
	else if (fout->remoteVersion >= 80400)
	{
		appendPQExpBuffer(dbQry, "SELECT tableoid, oid, "
						  "(%s datdba) AS dba, "
						  "pg_encoding_to_char(encoding) AS encoding, "
						  "datcollate, datctype, datfrozenxid, 0 AS datminmxid, 0 AS datasmxid, "
						  "(SELECT spcname FROM pg_tablespace t WHERE t.oid = dattablespace) AS tablespace, "
						  "shobj_description(oid, 'pg_database') AS description "
	...
	else if (fout->remoteVersion >= 80200)
	{
		appendPQExpBuffer(dbQry, "SELECT tableoid, oid, "
						  "(%s datdba) AS dba, "
						  "pg_encoding_to_char(encoding) AS encoding, "
						  "NULL AS datcollate, NULL AS datctype, datfrozenxid, 0 AS datminmxid, 0 AS datasmxid, "
						  "(SELECT spcname FROM pg_tablespace t WHERE t.oid = dattablespace) AS tablespace, "
						  "shobj_description(oid, 'pg_database') AS description "
	...
	i_asmxid = PQfnumber(res, "datasmxid");
	i_sqlmode = PQfnumber(res, "datsqlmode");
	...
	asmxid = atooid(PQgetvalue(res, 0, i_asmxid));
	sqlmode = PQgetvalue(res, 0, i_sqlmode);
	tablespace = PQgetvalue(res, 0, i_tablespace);

	/* using different template for different sql mode */
	if (strcmp(sqlmode, "p") == 0)
		appendPQExpBuffer(creaQry, "CREATE DATABASE %s WITH TEMPLATE = template0",
						  fmtId(datname));
	else
		appendPQExpBuffer(creaQry, "CREATE DATABASE %s WITH TEMPLATE = template0_ora",
						  fmtId(datname));
	...
	{
		appendPQExpBufferStr(creaQry, "\n-- For binary upgrade, "
							 "set datfrozenxid, datminmxid and datasmxid.\n");
		appendPQExpBuffer(creaQry, "UPDATE pg_catalog.pg_database\n"
						  "SET datfrozenxid = '%u', datminmxid = '%u', datasmxid = '%u'\n"
						  "WHERE datname = ",
						  frozenxid, minmxid, asmxid);
	...
	{
		dumpComment(fout, "DATABASE", fmtId(datname), NULL, dba, dbCatId, 0, dbDumpId);
	}
...
static void dumpSearchPath(Archive *AH)
{
	...
	/*
	 * For opentenbase_ora compatibility, we directly use 'set search_path' instead of 'set_config' to
	 * prevent opentenbase_ora from treating empty strings as null. This avoids unexpected reset behavior.
	 * To standardize behavior between PostgreSQL and opentenbase_ora, we set the format to 'set search_path'.
	 */
	appendPQExpBufferStr(qry, "SET search_path to ");
...
dumpSubscription(Archive *fout, SubscriptionInfo *subinfo)
{
	...
	if (strcmp(subinfo->subsynccommit, "off") != 0)
		appendPQExpBuffer(query, ", synchronous_commit = %s", fmtId_internal(subinfo->subsynccommit, true));
								...
								const char *objnamespace)
{
	...
	appendPQExpBuffer(upgrade_buffer, "ALTER EXTENSION %s ADD %s ",
					  fmtId_internal(extobj->name, true), objtype);
...
getNamespaces(Archive *fout, int *numNamespaces)
{
	...
	int			i_initrnspacl;
	int			i_pkgnsp;
	bool		typobjExist = false;
	char		*pkg_or_typobj_ns = NULL;

	query = createPQExpBuffer();

	typobjExist = isTypeObjectMetaExist(fout);
	if (!typobjExist)
		pkg_or_typobj_ns = ", n.oid in (SELECT pkgcrrspndingns FROM pg_catalog.pg_package) as pkg_nsp ";
	else
		pkg_or_typobj_ns = ", n.oid in (SELECT pkgcrrspndingns FROM pg_catalog.pg_package union SELECT objcreatensoid FROM pg_catalog.pg_type_object) as pkg_nsp ";
	...
	i_pkgnsp = PQfnumber(res, "pkg_nsp");
	...
	for (i = 0; i < ntups; i++)
	{
		...
		if (strcmp(PQgetvalue(res, i, i_pkgnsp), "t") == 0)
			nsinfo[i].pkg_nsp = true;
		else
			nsinfo[i].pkg_nsp = false;
...
getTypes(Archive *fout, int *numTypes)
{
	...
	if (fout->remoteVersion >= 90600)
	{
		...
		PQExpBuffer filter_oid_query = createFilterTableOids("t.typrelid");
		...
		appendPQExpBuffer(query, "SELECT t.tableoid, t.oid, t.typname, "
						  ...
						  "AND pip.objsubid = 0) %s",
						  acl_subquery->data,
						  racl_subquery->data,
						  initacl_subquery->data,
						  initracl_subquery->data,
						  username_subquery,
						  filter_oid_query->data);
		...
		destroyPQExpBuffer(filter_oid_query);
	}
	...
	for (i = 0; i < ntups; i++)
	{
		...
		tyinfo[i].isTypObj = isTypeObject(fout, &(tyinfo[i]));
...
#ifdef _PG_ORCL_
static char *
findNextNonQuoteChar(char *str, char ch)
{
	int		i = 0;
	bool	inQuote = false;
...
getPackages(Archive *fout)
{
	...
	appendPQExpBuffer(query, "SELECT tableoid, oid, pkgname, "
					  "pkgnamespace, "
	...
	pkginfo = (PkgInfo *) pg_malloc(ntups * sizeof(PkgInfo));
	...
	for (i = 0; i < ntups; i++)
	{
		...
		pkginfo[i].dobj.namespace = findNamespace(fout,
						  atooid(PQgetvalue(res, i, i_pkgnamespace)));
		pkginfo[i].rolname = pg_strdup(PQgetvalue(res, i, i_rolname));

		pkginfo[i].dobj.dump = DUMP_COMPONENT_DEFINITION;
 ...
 */
static void
dumpPackage(Archive *fout, PkgInfo *pkginfo)
{
	DumpOptions *dopt = fout->dopt;
	PQExpBuffer query;
	PQExpBuffer q;
	PQExpBuffer delq;
	PQExpBuffer rewriteq_header;
	PQExpBuffer rewriteq_body;
	PGresult   *res;
	int		i_pkgheader,
			i_pkgbody,
			i_pkgfuncspec,
	...
	char	*pkgheader,
			...
			*pkgfuncspec,
			*pkgfunc,
			*pkgbodyfunc,	/* public/private functions in the package */
			*pkgprivvarsrc,
			*pkginitsrc;
	...
	q = createPQExpBuffer();
	delq = createPQExpBuffer();
	rewriteq_header = createPQExpBuffer();
	rewriteq_body = createPQExpBuffer();

	appendPQExpBuffer(query, "SELECT "
					  "pkgheadersrc, "
					  "pkgbodysrc, "
					  "pkgfuncspec, "
					  "pg_catalog.array_to_string(pkgfunc, ' ') AS pkgfunc, "
					  "pg_catalog.array_length(pkgfunc, 1) AS npkgfunc, "
					  "(select pg_catalog.array_to_string(pg_catalog.array_agg(p.oid), ' ') "
					  "from pg_catalog.pg_proc p where p.pronamespace = c.pkgcrrspndingns) pkgbodyfunc, "
					  "(select pg_catalog.array_length(pg_catalog.array_agg(p.oid), 1) "
					  "from pg_catalog.pg_proc p where p.pronamespace = c.pkgcrrspndingns) npkgbodyfunc, "
					  "pkgprivvarsrc, "
					  "pkginitsrc "
					  "FROM pg_catalog.pg_package c "
					  "WHERE c.oid = '%u'::pg_catalog.oid",
					  pkginfo->dobj.catId.oid);

	res = ExecuteSqlQueryForSingleRow(fout, query->data);

	i_pkgheader = PQfnumber(res, "pkgheadersrc");
	i_pkgbody = PQfnumber(res, "pkgbodysrc");
	i_pkgfuncspec = PQfnumber(res, "pkgfuncspec");
	i_pkgfunc = PQfnumber(res, "pkgfunc");
	i_npkgfunc = PQfnumber(res, "npkgfunc");
	i_pkgbodyfunc = PQfnumber(res, "pkgbodyfunc");
	i_npkgbodyfunc = PQfnumber(res, "npkgbodyfunc");
	i_pkgprivvarsrc = PQfnumber(res, "pkgprivvarsrc");
	i_pkginitsrc = PQfnumber(res, "pkginitsrc");

	if (PQgetisnull(res, 0, i_pkgheader))
		pkgheader = NULL;
	else
		pkgheader = PQgetvalue(res, 0, i_pkgheader);

	if (PQgetisnull(res, 0, i_pkgbody))
		pkgbody = NULL;
	else
		pkgbody = PQgetvalue(res, 0, i_pkgbody);

	if (PQgetisnull(res, 0, i_pkgfuncspec))
		pkgfuncspec = NULL;
	else
		pkgfuncspec = PQgetvalue(res, 0, i_pkgfuncspec);

	if (PQgetisnull(res, 0, i_pkgfunc))
		pkgfunc = NULL;
	else
		pkgfunc = PQgetvalue(res, 0, i_pkgfunc);

	if (PQgetisnull(res, 0, i_npkgfunc))
		npkgfunc = 0;
	else
		npkgfunc = atoi(PQgetvalue(res, 0, i_npkgfunc));

	if (PQgetisnull(res, 0, i_pkgbodyfunc))
		pkgbodyfunc = NULL;
	else
		pkgbodyfunc = PQgetvalue(res, 0, i_pkgbodyfunc);

	if (PQgetisnull(res, 0, i_npkgbodyfunc))
		npkgbodyfunc = 0;
	else
		npkgbodyfunc = atoi(PQgetvalue(res, 0, i_npkgbodyfunc));

	if (PQgetisnull(res, 0, i_pkgprivvarsrc))
		pkgprivvarsrc = NULL;
	else
		pkgprivvarsrc = PQgetvalue(res, 0, i_pkgprivvarsrc);

	if (PQgetisnull(res, 0, i_pkginitsrc))
		pkginitsrc = NULL;
	else
		pkginitsrc = PQgetvalue(res, 0, i_pkginitsrc);

	/*
	 * Process the schema of the object in the funcspec in the package header.
	 */
	if (pkgheader != NULL && pkgfuncspec != NULL &&
		pkgfunc != NULL && npkgfunc != 0)
	{
		int				i = 0;
		Oid				*funcids;
		char			*pkgheader_loc = pkgheader;
		char			*pkgfuncspec_loc = pkgfuncspec;
		char			*splitfunc = NULL;
		bool			success = true;

		funcids = palloc(npkgfunc * sizeof(Oid));
		parseOidArray(pkgfunc, funcids, npkgfunc);

		splitfunc = findNextNonQuoteChar(pkgfuncspec_loc, ';');
		while (splitfunc != NULL)
		{
			char   *func = NULL;
			size_t	func_len;

			*splitfunc = '\0';
			func = strstr(pkgheader_loc, pkgfuncspec_loc);
			*splitfunc = ';';
			...
			/* Add the part in front of the function in the pkgheader string */
			if (pkgheader_loc < func)
				appendBinaryPQExpBuffer(rewriteq_header, pkgheader_loc, func - pkgheader_loc);

			/* Rewrite the function parameter type to "schema.type" */
			if (i < npkgfunc)
			{
				appendPQExpBufferStr(rewriteq_header, getFormattedPkgFunc(fout, funcids[i], true));
				i++;
			}
			else
			{
			...
			/* pkgheader go ahead */
			func_len = splitfunc - pkgfuncspec_loc + 1;
			if (func + func_len <= pkgheader + strlen(pkgheader))
				pkgheader_loc = func + func_len;
			else
			{
				success = false;
				break;
			...
			/* pkgfuncspec_loc go ahead */
			pkgfuncspec_loc = splitfunc + 1;
			splitfunc = findNextNonQuoteChar(pkgfuncspec_loc, ';');
		}

		if (success)
		{
			if (pkgheader_loc < pkgheader + strlen(pkgheader))
				appendPQExpBufferStr(rewriteq_header, pkgheader_loc);

			pkgheader = rewriteq_header->data;
		}
	...
	/*
	 * Process the schema of the object in the funcspec in the package body.
	 */
	if (pkgbody && npkgbodyfunc != 0 && pkgbodyfunc)
	{
		int		i;
		Oid		*funcids;

		funcids = palloc(npkgbodyfunc * sizeof(Oid));
		parseOidArray(pkgbodyfunc, funcids, npkgbodyfunc);

		if (pkgprivvarsrc)
			appendPQExpBufferStr(rewriteq_body, pkgprivvarsrc);

		for (i = 0; i < npkgbodyfunc; i++)
		{
			appendPQExpBufferStr(rewriteq_body, getFormattedPkgFunc(fout, funcids[i], false));
		}
		...
		if (pkginitsrc)
		{
			...
			appendPQExpBufferStr(rewriteq_body, pkginitsrc);

			ptr = pkginitsrc + strlen(pkginitsrc) - 1;
			while (ptr >= pkginitsrc && isspace((unsigned char) *ptr))
				ptr--;

			if (ptr >= pkginitsrc && *ptr != ';')
				appendPQExpBufferChar(rewriteq_body, ';');
		}

		pkgbody = rewriteq_body->data;
		free(funcids);
	}
	else if (pkgbody && pkginitsrc)
	{
		/* The package init block needs to be followed by a ';' at the end */
		char	*ptr;

		ptr = pkgbody + strlen(pkgbody) - 1;
		while (ptr >= pkgbody && isspace((unsigned char) *ptr))
			ptr--;

		if (ptr >= pkgbody && *ptr != ';')
		{
			appendPQExpBufferStr(rewriteq_body, pkgbody);
			appendPQExpBufferChar(rewriteq_body, ';');
			pkgbody = rewriteq_body->data;
		}
	}
	...
	/* Reset search_path while restore package with a empty search_path */
	if (empty_search_path)
		appendPQExpBuffer(q, "RESET search_path;\n");

	/* package header */
	appendPQExpBuffer(q, "CREATE PACKAGE %s",
						fmtId(pkginfo->dobj.namespace->dobj.name));
	appendPQExpBuffer(q, ".%s AS $p$\n", fmtId(pkginfo->dobj.name));
	...
	if (pkgbody)
	{
		appendPQExpBuffer(q, "CREATE PACKAGE BODY %s",
						  fmtId(pkginfo->dobj.namespace->dobj.name));
		appendPQExpBuffer(q, ".%s AS $p$\n", fmtId(pkginfo->dobj.name));
		appendPQExpBuffer(q, "%s\n", pkgbody);
		appendPQExpBuffer(q, "\n$p$;");
	...
	if (pkginfo->dobj.dump & DUMP_COMPONENT_DEFINITION)
		ArchiveEntry(fout, pkginfo->dobj.catId, pkginfo->dobj.dumpId,
					 ...
					 NULL,
					 pkginfo->rolname,
					 false, "PACKAGE", SECTION_PRE_DATA,
	...
	destroyPQExpBuffer(q);
	destroyPQExpBuffer(delq);
	destroyPQExpBuffer(rewriteq_header);
	destroyPQExpBuffer(rewriteq_body);
}
...
getSynonyms(Archive *fout)
{
	...
	appendPQExpBuffer(query, "SELECT tableoid, oid, synname, "
					  "synnamespace, "
					  "(%s synowner) AS rolname "
					  "FROM pg_synonym WHERE OBJNAMESPACE not in ('SYS', 'SYSTEM')",
					  username_subquery);
	...
	synoinfo = (SynoInfo *) pg_malloc(ntups * sizeof(SynoInfo));

	i_tableoid = PQfnumber(res, "tableoid");
	i_oid = PQfnumber(res, "oid");
	i_synoname = PQfnumber(res, "synname");
	i_synonamespace = PQfnumber(res, "synnamespace");
	i_rolname = PQfnumber(res, "rolname");

	for (i = 0; i < ntups; i++)
	{
		synoinfo[i].dobj.objType = DO_SYNONYM;
		synoinfo[i].dobj.catId.tableoid = atooid(PQgetvalue(res, i, i_tableoid));
		synoinfo[i].dobj.catId.oid = atooid(PQgetvalue(res, i, i_oid));
		AssignDumpId(&synoinfo[i].dobj);
		synoinfo[i].dobj.name = pg_strdup(PQgetvalue(res, i, i_synoname));
		synoinfo[i].dobj.namespace = findNamespace(fout,
						  atooid(PQgetvalue(res, i, i_synonamespace)));
		synoinfo[i].rolname = pg_strdup(PQgetvalue(res, i, i_rolname));

		/* Decide whether we want to dump it */
		selectDumpableObject(&(synoinfo[i].dobj), fout);
		synoinfo[i].dobj.dump &= DUMP_COMPONENT_DEFINITION;
	}
...
/*
 * dumpSynonym
 *	  write out a single synonym definition
 */
static void
dumpSynonym(Archive *fout, SynoInfo *synoinfo)
{
	DumpOptions *dopt = fout->dopt;
	PQExpBuffer query;
	PQExpBuffer q;
	...
	/* Skip if not to be dumped */
	if (!synoinfo->dobj.dump || dopt->dataOnly)
		return;
	...
	appendPQExpBuffer(query, "SELECT "
					  "objnamespace, "
					  "objname, objdblink "
					  "FROM pg_catalog.pg_synonym c "
					  "WHERE c.oid = '%u'::pg_catalog.oid",
					  synoinfo->dobj.catId.oid);

	res = ExecuteSqlQueryForSingleRow(fout, query->data);

	i_objnspname = PQfnumber(res, "objnamespace");
	i_objname = PQfnumber(res, "objname");
	i_objdblink = PQfnumber(res, "objdblink");

	if (PQgetisnull(res, 0, i_objnspname))
		objnspname = NULL;
	else
		objnspname = PQgetvalue(res, 0, i_objnspname);
	objname = PQgetvalue(res, 0, i_objname);
	if (PQgetisnull(res, 0, i_objdblink))
		objdblink = NULL;
	else
		objdblink = PQgetvalue(res, 0, i_objdblink);

	/*
	 * DROP must be fully qualified in case same name appears in pg_catalog
	 */
	appendPQExpBuffer(delq, "DROP SYNONYM %s",
					  fmtId(synoinfo->dobj.namespace->dobj.name));
	appendPQExpBuffer(delq, ".%s;\n",
					  fmtId(synoinfo->dobj.name));

	appendPQExpBuffer(q, "CREATE SYNONYM %s",
						fmtId(synoinfo->dobj.namespace->dobj.name));
	appendPQExpBuffer(q, ".%s FOR ", fmtId(synoinfo->dobj.name));

	if (objnspname != NULL)
		appendPQExpBuffer(q, "%s.", objnspname);
	appendPQExpBuffer(q, "%s", objname);
	if (objdblink != NULL)
		appendPQExpBuffer(q, " @ %s;\n", objdblink);
	else
		appendPQExpBuffer(q, ";\n");

	if (synoinfo->dobj.dump & DUMP_COMPONENT_DEFINITION)
		ArchiveEntry(fout, synoinfo->dobj.catId, synoinfo->dobj.dumpId,
					 synoinfo->dobj.name,
					 synoinfo->dobj.namespace->dobj.name,
					 NULL,
					 synoinfo->rolname,
					 false, "SYNONYM", SECTION_PRE_DATA,
					 q->data, delq->data, NULL,
					 NULL, 0,
					 NULL, NULL);
...
getAggregates(Archive *fout, int *numAggs)
{
	...
	if (fout->remoteVersion >= 90600)
	{
		...
		appendPQExpBuffer(query, "SELECT p.tableoid, p.oid, "
						  ...
						  "LEFT JOIN pg_init_privs pip ON "
						  "(p.oid = pip.objoid "
						  "AND pip.classoid = 'pg_catalog.pg_proc'::regclass "
						  "AND pip.objsubid = 0) "
						  "WHERE %s AND ("
						  "p.pronamespace != "
		...
		if (dopt->binary_upgrade)
			appendPQExpBufferStr(query,
								 ...
								 "classid = 'pg_catalog.pg_proc'::regclass AND "
								 "objid = p.oid AND "
...
getFuncs(Archive *fout, int *numFuncs)
{
	...
	if (fout->remoteVersion >= 90600)
	{
		...
		appendPQExpBuffer(query,
						  ...
						  "WHERE classid = 'pg_catalog.pg_proc'::regclass AND "
						  "objid = p.oid AND deptype = 'i')"
#ifdef _PG_ORCL_
						  "\n  AND pronamespace NOT IN"
						  "(SELECT pkgcrrspndingns FROM pg_package)"
#endif
						  "\n  AND ("
						  "\n  pronamespace != "
						  "(SELECT oid FROM pg_namespace "
...
void createWithTableOidsSet(InhInfo *inhinfo, int numInherits)
{
	SimpleOidListCell *cell;
	bool isFirstCell = true;

	with_table_oids_set = createPQExpBuffer();

	if (table_include_oids.head == NULL)
		return;

	for (cell = table_include_oids.head; cell; cell = cell->next)
	{
		Oid			oid = cell->val;
		int			i;
		...
		if (isFirstCell)
		{
			appendPQExpBuffer(with_table_oids_set, "%d", oid);
			isFirstCell = false;
		}
		else
			appendPQExpBuffer(with_table_oids_set, ",%d", oid);
		...
		for (i = 0; i < numInherits; i++)
		{
			if (inhinfo[i].inhrelid == oid)
				appendPQExpBuffer(with_table_oids_set, ",%d", inhinfo[i].inhparent);
		}
	}
}

void clearWithTableOidsSet()
{
	destroyPQExpBuffer(with_table_oids_set);
}

static PQExpBuffer
createFilterTableOids(char *colname)
{
	PQExpBuffer query = createPQExpBuffer();
	...
	appendPQExpBuffer(query, "where %s in (%s)", colname, with_table_oids_set->data);

	return query;
}

static PQExpBuffer
createFilterTableOids4GetTables()
{
	PQExpBuffer query = createPQExpBuffer();
	if (table_include_oids.head == NULL)
		return query;

	appendPQExpBuffer(query, "(c.oid in (%s) OR d.refobjid in (%s)) AND ", with_table_oids_set->data, with_table_oids_set->data);
...
getTables(Archive *fout, int *numTables)
{
	...
	if (fout->remoteVersion >= 90600)
	{
		...
		PQExpBuffer filter_oid_query = createFilterTableOids4GetTables();
		...
		if (fout->remoteVersion >= 100000)
		{
            ...
            aliasname = "(select aname from pg_childalias as pca where pca.childoid = c.oid)";
		}
		...
		appendPQExpBuffer(query,
						  ...
						  "AND pip.classoid = '\"pg_class\"'::regclass "
						  "AND pip.objsubid = 0) "
						  "WHERE %s c.relkind in ('%c', '%c', '%c', '%c', '%c', '%c', '%c') "
						  "AND c.relname NOT LIKE 'estore_registry_%%' "
						  "AND c.relname NOT LIKE 'estore_stash_%%' "
						  "ORDER BY c.oid",
						  acl_subquery->data,
						  racl_subquery->data,
						  initacl_subquery->data,
						  ...
						  fout->isPostgresXL
							  ...
							  "(WITH discolnums_cte AS (SELECT unnest(discolnums) AS discolnum FROM pg_catalog.pgxc_class WHERE pcrelid = c.oid) SELECT CASE WHEN EXISTS (SELECT 1 FROM pg_attribute WHERE attrelid = c.oid AND attnum = ANY(SELECT discolnum FROM discolnums_cte) AND attisdropped = true) THEN null WHEN array_length(discolnums, 1) = 0 THEN null ELSE array_to_string(ARRAY( select quote_ident(attname) from pg_attribute, (select discolnum, row_number() over() from discolnums_cte) where attrelid = c.oid and attnum = discolnum order by row_number), ',') end from pg_catalog.pgxc_class WHERE pcrelid = c.oid) as discols, "
							  : "",
                          aliasname,
						  RELKIND_SEQUENCE,
	...
	*numTables = ntups;
...
#ifdef _PG_ORCL_
	i_relrowidseq = PQfnumber(res, "relrowidseq");
#endif
	...
	for (i = 0; i < ntups; i++)
	{
...
#ifdef _PG_ORCL_
		tblinfo[i].hasrowid = (atooid(PQgetvalue(res, i, i_relrowidseq)) != 0);
		tblinfo[i].seqislocal = false;
#endif
		...
		if (fout->isPostgresXL)
		{
			...
			if (PQgetisnull(res, i, i_discols))
			{
				tblinfo[i].discols = NULL;
			}
			else
			{
				tblinfo[i].discols = pg_strdup(PQgetvalue(res, i, i_discols));
			}
		}
#endif
#ifdef __OPENTENBASE__
        ...
        if (PQgetisnull(res, i, i_aliasname))
            tblinfo[i].aliasname = NULL;
        else
            tblinfo[i].aliasname = pg_strdup(PQgetvalue(res, i, i_aliasname));
#endif
...
getRules(Archive *fout, int *numRules)
{
	...
	if (fout->remoteVersion >= 80300)
	{
		PQExpBuffer filter_oid_query = createFilterTableOids("ev_class");
		appendPQExpBuffer(query, "SELECT "
...
get_language_name(Archive *fout, Oid langid)
{
	...
	res = ExecuteSqlQueryForSingleRow(fout, query->data);
	lanname = pg_strdup(fmtId_internal(PQgetvalue(res, 0, 0), true));
	destroyPQExpBuffer(query);
	PQclear(res);
...
getTableAttrs(Archive *fout, TableInfo *tblinfo, int numTables)
{
	...
	for (i = 0; i < numTables; i++)
	{
...
#ifdef __OPENTENBASE__
		tbinfo->encoding = (AttrEncodingInfo **) pg_malloc0(ntups * sizeof(AttrEncodingInfo *));
#endif
		hasdefaults = false;
		...
		/* get table encoding info . */
		{
			AttrEncodingInfo *attrdecode;
			int			      numDecoding;

			if (g_verbose)
				write_msg(NULL, "finding encoding infomation of table \"%s.%s\"\n",
						  tbinfo->dobj.namespace->dobj.name,
						  tbinfo->dobj.name);
			resetPQExpBuffer(q);
			printfPQExpBuffer(q, "select attnum,atttranscomp::int,attuniquestore::int from pg_catalog.pg_attribute " 
								 "where attrelid = %u::pg_catalog.oid and (attuniquestore != false);",
							      tbinfo->dobj.catId.oid);

			res = ExecuteSqlQuery(fout, q->data, PGRES_TUPLES_OK);

			numDecoding = PQntuples(res);
			if (numDecoding)
			{
				attrdecode  = (AttrEncodingInfo *) pg_malloc(numDecoding * sizeof(AttrEncodingInfo));

				for (j = 0; j < numDecoding; j++)
				{
					int			attrnum     = 0;
					attrnum					   = atoi(PQgetvalue(res, j, 0));
					attrdecode[j].tc_id        = atoi(PQgetvalue(res, j, 1));
					attrdecode[j].unique_store = atoi(PQgetvalue(res, j, 2));

					tbinfo->encoding[attrnum - 1] = &attrdecode[j];
				}
			}
			PQclear(res);
		}
...
#ifdef _PG_ORCL_
		case DO_SYNONYM:
			dumpSynonym(fout, (SynoInfo *) dobj);
			break;
		case DO_PACKAGE:
			dumpPackage(fout, (PkgInfo *) dobj);
			break;
#endif
...
dumpExtension(Archive *fout, ExtensionInfo *extinfo)
{
	...
	qextname = pg_strdup(fmtId_internal(extinfo->dobj.name, true));
...
dumpRangeType(Archive *fout, TypeInfo *tyinfo)
{
	...
	if (PQgetvalue(res, 0, PQfnumber(res, "opcdefault"))[0] != 't')
	{
		...
		appendPQExpBuffer(q, ",\n    subtype_opclass = %s.",
						  fmtId(nspname));
		appendPQExpBufferStr(q, fmtId_internal(opcname, true));
	}
	...
	if (OidIsValid(collationOid))
	{
		...
		if (coll)
			appendPQExpBuffer(q, ",\n    collation = %s",
							  fmtQualifiedDumpableLikePG(coll));
	}
...
dumpBaseType(Archive *fout, TypeInfo *tyinfo)
{
	...
	if (tyinfo->typrelkind == RELKIND_NESTED_TABLE_TYPE)
	{
		char	*elemType;
		...
		appendPQExpBuffer(q,
					"CREATE TYPE %s IS TABLE OF %s;",
					qualtypname, elemType);
		free(elemType);
	}
	else
	{
...
dumpDomain(Archive *fout, TypeInfo *tyinfo)
{
	...
	if (OidIsValid(typcollation))
	{
		...
		if (coll)
			appendPQExpBuffer(q, " COLLATE %s", fmtQualifiedDumpableLikePG(coll));
	}
	...
	for (i = 0; i < tyinfo->nDomChecks; i++)
	{
		...
		appendPQExpBuffer(conprefix, "CONSTRAINT %s ",
...
static bool
isTypeObjectMetaExist(Archive *fout)
{
	PQExpBuffer query = createPQExpBuffer();
	PGresult	*res;
	int			ntups;

	appendPQExpBuffer(query, "SELECT relname from pg_class where oid = 4192");
	...
	return ntups == 1 ? true : false;
}

static bool
isTypeObject(Archive *fout, TypeInfo *typinfo)
{
	PQExpBuffer query = NULL;
	PGresult	*res;
	char		*query_val  = NULL;
	int			nsOid;
	bool	    is_type_obj = false;

	if (!isTypeObjectMetaExist(fout) || typinfo == NULL)
		return false;

	if (typinfo->typtype != 'c')
		return false;

	query = createPQExpBuffer();
	appendPQExpBuffer(query, "select setting from pg_catalog.pg_settings where name = 'sql_mode'");
	res = ExecuteSqlQuery(fout, query->data, PGRES_TUPLES_OK);
	query_val = PQgetvalue(res, 0, 0);
	if (strcasecmp(query_val, "opentenbase_ora") != 0)
	{
		PQclear(res);
		destroyPQExpBuffer(query);
		return false;
	...
	nsOid = typinfo->dobj.namespace->dobj.catId.oid;
	resetPQExpBuffer(query);
	appendPQExpBuffer(query, "select count(1) cnt "
						"from pg_catalog.pg_type_object pt"
						" where pt.objnamespace = %d and "
						"pt.objname = '%s'",
						nsOid, typinfo->dobj.name);

	res = ExecuteSqlQuery(fout, query->data, PGRES_TUPLES_OK);
	query_val = PQgetvalue(res, 0, 0);
	is_type_obj = (strcmp(query_val, "0") == 0) ? false : true;
	PQclear(res);
	destroyPQExpBuffer(query);
...
/*
 * Dump type object, both spec and body
 */
void
dumpTypeObject(Archive *fout, TypeInfo *tyinfo)
{
	PQExpBuffer query = createPQExpBuffer();
	PQExpBuffer q = createPQExpBuffer();
	PQExpBuffer nsq = createPQExpBuffer();
	PQExpBuffer delq = createPQExpBuffer();
	PGresult	*res;
	int			ntups;
	int			i_objname;
	...
	if (!isTypeObjectMetaExist(fout))
		return;

	appendPQExpBuffer(query, "select objname, objisspec, "
						"objheadersrc, objfuncspec, objbodysrc "
						"from pg_catalog.pg_type_object pt"
						" where pt.objcrrspndingns = '%u'::pg_catalog.oid", 
						tyinfo->dobj.catId.oid);
	...
	i_objname = PQfnumber(res, "objname");
	i_spec = PQfnumber(res, "objisspec");
	i_objheader = PQfnumber(res, "objheadersrc");
	i_objbody = PQfnumber(res, "objbodysrc");
	...
	for (i = 0; i < ntups; i++)
	{
		...
		headerq = createPQExpBuffer();

		objname = PQgetvalue(res, i, i_objname);
		isspec = (PQgetvalue(res, i, i_spec)[0] == 't');
		header = PQgetvalue(res, i, i_objheader);
		body = PQgetvalue(res, i, i_objbody);
		...
		if (isspec)
		{
			if (header != NULL)
			{
				char   *quotes_left = NULL;
				char   *quotes_right = NULL;
				char   *parenthesis_left = NULL;

				parenthesis_left = strchr(header, '(');

				/* ignore ( in " " */
				quotes_left = strchr(header, '\"');
				if (quotes_left != NULL)
					quotes_right = strchr(quotes_left + 1, '\"');

				while (quotes_left != NULL && quotes_right != NULL &&
					   parenthesis_left != NULL && quotes_left < parenthesis_left &&
					   quotes_right > parenthesis_left)
				{
					parenthesis_left = strchr(quotes_right + 1, '(');
					quotes_left = strchr(quotes_right + 1, '\"');
				...
				if (parenthesis_left != NULL)
				{
					appendPQExpBuffer(headerq, "CREATE TYPE %s AS OBJECT %s",
									fmtQualifiedDumpable(tyinfo), parenthesis_left);
		...
		else
		{
			...
			if (ah->format != archNull)
				appendPQExpBuffer(q, "CREATE TYPE BODY %s AS $p$ \n %s\n$p$;\n",
								  fmtQualifiedDumpable(tyinfo), body);
			else
		...
		if (tyinfo->dobj.dump & DUMP_COMPONENT_DEFINITION)
			ArchiveEntry(fout, tyinfo->dobj.catId, tyinfo->dobj.dumpId,
						...
						NULL,
						tyinfo->rolname,
						false, "TYPE OBJECT", SECTION_PRE_DATA,
						q->data,
						delq->data, NULL,
						NULL, 0,
						NULL, NULL);

		destroyPQExpBuffer(headerq);
	}
...
dumpProcLang(Archive *fout, ProcLangInfo *plang)
{
	...
	qlanname = pg_strdup(fmtId_internal(plang->dobj.name, true));
...
/*
 * Remove OUT option from argument list.
 * For example, the original string is: foo(a int, OUT b int, c int);
 * after removed, it become foo(a int, b int, c int);
 *
 * The reason do this is becuase opentenbase kernel will not count OUT argument
 * when do ALTER/DROP etc(see extractArgTypes). But pronargs in pg_proc
 * contain it.
 */
static char *
_Remove_OUT_Option(const char *lines)
{
	int i = 0, len;
	char *result;
	bool quoted = false;
	...
	while (i < len)
	{
		/* Remove " OUT " or "(OUT " that not in double quote */
		if (!quoted && i + 3 < len && i >= 1 &&
			(lines[i - 1] == ' ' || lines[i - 1] == '(') &&
			memcmp(lines + i, "OUT ", 4) == 0)
			i += 4;
...
dumpFunc(Archive *fout, FuncInfo *finfo)
{
	...
	char	   *procsig = NULL;	/* identity signature */
	...
	if (prokind[0] == PROKIND_PROCEDURE && !PQinOraMode())
		procsig = _Remove_OUT_Option(funcsig);

	appendPQExpBuffer(delqry, "DROP %s %s.%s;\n",
					  keyword,
					  fmtId(finfo->dobj.namespace->dobj.name),
					  procsig == NULL ? funcsig : procsig);
	...
	appendPQExpBuffer(q, "\n    LANGUAGE %s", fmtId_internal(lanname, true));
	...
	if (provolatile[0] != PROVOLATILE_VOLATILE)
	{
		if (provolatile[0] == PROVOLATILE_IMMUTABLE)
			...
			appendPQExpBufferStr(q, " RESULT_CACHE");
	...
	for (i = 0; i < nconfigitems; i++)
	{
		...
		if (pos == NULL)
			continue;
		*pos++ = '\0';
		appendPQExpBuffer(q, "\n    SET %s TO ", fmtId_internal(configitem, true));
	...
	if (dopt->binary_upgrade)
		binary_upgrade_extension_member(q, &finfo->dobj,
										keyword, procsig == NULL ? funcsig : procsig,
...
dumpTransform(Archive *fout, TransformInfo *transform)
{
	...
	appendPQExpBuffer(transformargs, "FOR %s LANGUAGE %s", transformType,
					  lanname);
...
dumpAccessMethod(Archive *fout, AccessMethodInfo *aminfo)
{
	...
	qamname = pg_strdup(fmtId_internal(aminfo->dobj.name, true));
...
dumpOpclass(Archive *fout, OpclassInfo *opcinfo)
{
	...
	appendPQExpBuffer(delq, "DROP OPERATOR CLASS %s",
					  fmtQualifiedDumpableLikePG(opcinfo));
	appendPQExpBuffer(delq, " USING %s;\n", fmtId_internal(amname, true));
	...
	if (strlen(opcfamilyname) > 0)
	{
		...
		appendPQExpBufferStr(q, fmtId_internal(opcfamilyname, true));
	}
	...
	for (i = 0; i < ntups; i++)
	{
		...
		if (strlen(sortfamily) > 0)
		{
			...
			appendPQExpBufferStr(q, fmtId_internal(sortfamily, true));
		}
	...
	if (dopt->binary_upgrade)
		binary_upgrade_extension_member(q, &opcinfo->dobj, "OPERATOR CLASS",
										nameusing->data,
...
dumpOpfamily(Archive *fout, OpfamilyInfo *opfinfo)
{
	...
	appendPQExpBuffer(delq, "DROP OPERATOR FAMILY %s",
					  fmtQualifiedDumpableLikePG(opfinfo));
	appendPQExpBuffer(delq, " USING %s;\n", fmtId_internal(amname, true));
	...
	appendPQExpBuffer(nameusing, " USING %s", fmtId_internal(amname, true));
...
dumpCollation(Archive *fout, CollInfo *collinfo)
{
	...
	q = createPQExpBuffer();
	delq = createPQExpBuffer();
	qcollname = pg_strdup(fmtId_internal(collinfo->dobj.name, true));
	...
	appendPQExpBuffer(delq, "DROP COLLATION %s;\n",
					  fmtQualifiedDumpableLikePG(collinfo));
...
dumpAgg(Archive *fout, AggInfo *agginfo)
{
	...
	bool		opentenbase_ora_style = false;
	const char *funcret;
	...
	if (fout->remoteVersion >= 110000)
	{
		appendPQExpBuffer(query, "SELECT aggtransfn, "
						  ...
						  "pg_catalog.pg_get_function_result(p.oid) AS funcret, "
						  "p.proparallel "
	...
	/*
	 * If initval starts as "opentenbase_" it is most likely an opentenbase_ora style aggregate.
	 */
	if (agginitval && pg_strncasecmp(agginitval, INITVAL_MAGIC_HEADER,
									 strlen(INITVAL_MAGIC_HEADER)) == 0)
	{
		opentenbase_ora_style = true;
		funcret = PQgetvalue(res, 0, PQfnumber(res, "funcret"));

		appendPQExpBuffer(details, "RETURN %s%s AGGREGATE USING %s",
						  funcret, 
						  (proparallel && proparallel[0] != PROPARALLEL_UNSAFE ? " PARALLEL_ENABLE" : ""),
						  aggtranstype);
	}
	else
	{
	...
	if (opentenbase_ora_style)
	{
		appendPQExpBuffer(q, "set enable_opentenbase_ora_dist_udaf to on;\n");
		appendPQExpBuffer(q, "CREATE FUNCTION %s.%s %s;\n",
...
dumpTSParser(Archive *fout, TSParserInfo *prsinfo)
{
	...
	q = createPQExpBuffer();
	delq = createPQExpBuffer();
	qprsname = pg_strdup(fmtId_internal(prsinfo->dobj.name, true));

	appendPQExpBuffer(q, "CREATE TEXT SEARCH PARSER %s (\n",
					  fmtQualifiedDumpableLikePG(prsinfo));
...
dumpTSDictionary(Archive *fout, TSDictInfo *dictinfo)
{
	...
	delq = createPQExpBuffer();
	query = createPQExpBuffer();
	qdictname = pg_strdup(fmtId_internal(dictinfo->dobj.name, true));
	...
	appendPQExpBuffer(q, "CREATE TEXT SEARCH DICTIONARY %s (\n",
					  fmtQualifiedDumpableLikePG(dictinfo));

	appendPQExpBufferStr(q, "    TEMPLATE = ");
	appendPQExpBuffer(q, "%s.", fmtId(nspname));
	appendPQExpBufferStr(q, fmtId_internal(tmplname, true));
...
dumpTSTemplate(Archive *fout, TSTemplateInfo *tmplinfo)
{
	...
	q = createPQExpBuffer();
	delq = createPQExpBuffer();
	qtmplname = pg_strdup(fmtId_internal(tmplinfo->dobj.name, true));

	appendPQExpBuffer(q, "CREATE TEXT SEARCH TEMPLATE %s (\n",
					  fmtQualifiedDumpableLikePG(tmplinfo));
...
dumpTSConfig(Archive *fout, TSConfigInfo *cfginfo)
{
	...
	delq = createPQExpBuffer();
	query = createPQExpBuffer();
	qcfgname = pg_strdup(fmtId_internal(cfginfo->dobj.name, true));
	...
	appendPQExpBuffer(q, "CREATE TEXT SEARCH CONFIGURATION %s (\n",
					  fmtQualifiedDumpableLikePG(cfginfo));

	appendPQExpBuffer(q, "    PARSER = %s.", fmtId(nspname));
	appendPQExpBuffer(q, "%s );\n", fmtId_internal(prsname, true));
	...
	for (i = 0; i < ntups; i++)
	{
			...
			strcmp(tokenname, PQgetvalue(res, i - 1, i_tokenname)) != 0)
		{
			...
			appendPQExpBuffer(q, "    ADD MAPPING FOR %s WITH %s",
							  fmtId_internal(tokenname, true), dictname);
		}
		else
...
dumpForeignDataWrapper(Archive *fout, FdwInfo *fdwinfo)
{
	...
	if (fdwinfo->dobj.dump & DUMP_COMPONENT_ACL)
		dumpACL(fout, fdwinfo->dobj.catId, fdwinfo->dobj.dumpId,
				...
				fdwinfo->rolname, fdwinfo->fdwacl, fdwinfo->rfdwacl,
			 ...
			 CatalogId catalogId, int subid, DumpId dumpId)
{
	...
	for (i = 0; i < nlabels; i++)
	{
		...
		appendPQExpBuffer(query,
						  ...
						  fmtId_internal(labels[i].provider, true), type);
		if (namespace && *namespace)
			appendPQExpBuffer(query, "%s.", fmtId(namespace));
		appendPQExpBuffer(query, "%s IS ", name);
...
dumpTableSecLabel(Archive *fout, TableInfo *tbinfo, const char *reltypename)
{
	...
	for (i = 0; i < nlabels; i++)
	{
		...
		appendPQExpBuffer(query, "SECURITY LABEL FOR %s ON %s IS ",
						  fmtId_internal(provider, true), target->data);
...
dumpTable(Archive *fout, TableInfo *tbinfo)
{
	 ...
	 * Skip estore registry/stash column ACLs.
	 */
...
createViewAsClause(Archive *fout, TableInfo *tbinfo)
{
	...
	if (len == 0)
	{
		PQclear(res);
		destroyPQExpBuffer(query);
		write_msg(NULL, "WARNING: definition of view \"%s\" appears to be empty (length zero)\n", tbinfo->dobj.name);
		return result;
	}

	/* Strip off the trailing semicolon so that other things may follow. */
	Assert(PQgetvalue(res, 0, 0)[len - 1] == ';');
	result = createPQExpBuffer();
...
dumpTableSchema(Archive *fout, TableInfo *tbinfo)
{
	...
	if (tbinfo->relkind == RELKIND_VIEW)
	{
		...
		if (PQinOraMode() && ((ArchiveHandle *) fout)->format == archNull)
		{
			/*
			 * The opentenbase-ora-compatible mode view may contain a with function
			 * and needs to be terminated by a '/' mark.
			 */
			appendPQExpBufferStr(q, ";\n/\n");
		}
		else
...
#ifdef __OPENTENBASE_C__
					if (tbinfo->encoding[j])
					{
						bool need_sep = false;
						appendPQExpBuffer(q, " encoding (");

						if (tbinfo->encoding[j]->tc_id)
						{							
							appendPQExpBuffer(q, " compression = %s",
											  g_compress_method_name[tbinfo->encoding[j]->tc_id]);
							need_sep = true;
						}

						if (tbinfo->encoding[j]->unique_store)
						{
							if (need_sep)
							{
								appendPQExpBuffer(q, ",");								
							}
							appendPQExpBuffer(q, " uniquestore = true");						
						}
...
#ifdef PGXC
        if (tbinfo->ispartition && !dopt->binary_upgrade && tbinfo->aliasname)
        {
            appendPQExpBuffer(q, " AS %s", tbinfo->aliasname);
        }

		if (fout->isPostgresXL && !tbinfo->ispartition)
		{
			...
			if (tbinfo->pgxclocatortype != 'E')
			{
				...
				else if (tbinfo->pgxclocatortype == 'H' && tbinfo->discols)
				{
					appendPQExpBuffer(q, "\nDISTRIBUTE BY HASH (%s)",
									tbinfo->discols);
				}
				else if (tbinfo->pgxclocatortype == 'M' && tbinfo->discols)
				{
...
#ifdef __OPENTENBASE__
				else if(tbinfo->pgxclocatortype == 'S' && !tbinfo->ispartition && tbinfo->discols)
				{
...
getAttrName(int attrnum, TableInfo *tblInfo)
{
	...
	switch (attrnum)
	{
		...
		case XC_NodeIdAttributeNumber:
			return "xc_node_id";
#endif
		case ORCL_RowIdAttributeNumber:
			return "ROWID";
	}
...
dumpIndex(Archive *fout, IndxInfo *indxinfo)
{
	...
	/* Skip if only has rowid column index */
	if (tbinfo->hasrowid &&
		indxinfo->indnkeys == 1 &&
		((int)(indxinfo->indkeys[0])) == ORCL_RowIdAttributeNumber)
		return;
...
dumpSequence(Archive *fout, TableInfo *tbinfo)
{
	...
	if (fout->remoteVersion >= 100000)
	{
		appendPQExpBuffer(query,
						  ...
						  "seqmax, seqmin, "
						  "seqcache, seqcycle "
						  ", seqlocal or (select count(*) > 0 from pg_class where relrowidseq = seqrelid) "
						  " %s "
						  "FROM pg_class c "
						  "JOIN %s s ON (s.seqrelid = c.oid) "
						  "WHERE c.oid = '%u'::oid",
						  PQinOraMode() ? ",seqoptions" : " ",
						  PQinOraMode() ? "pg_ora_sequence" : "pg_sequence",
						  tbinfo->dobj.catId.oid);
	}
	else if (fout->remoteVersion >= 80400)
	{
	...
	tbinfo->seqislocal = seqlocal = (strcmp(PQgetvalue(res, 0, 7), "t") == 0);
	if (PQinOraMode())
		seqoptions = atoll(PQgetvalue(res, 0, 8));
	else
		seqoptions = 0;

	seqorder = seqoptions & SeqOrderOption;

	if (seqlocal)
	{
		/* Skip the local query dump */
		PQclear(res);
	...
	else if (strcmp(seqtype, "NUMERIC") == 0)
	{
		if (!is_ascending && strcmp(minv, OPENTENBASE_ORA_DEFAULT_MINSEQUENCE) == 0)
			minv = NULL;
		if (is_ascending && strcmp(maxv, OPENTENBASE_ORA_DEFAULT_MAXSEQUENCE) == 0)
			maxv = NULL;
	}
	/*
	...
	{
		if (seqorder)
			appendPQExpBuffer(query, "    ORDER\n");
		else
			appendPQExpBuffer(query, "    NOORDER\n");
	}
...
static void
getSequenceLocalInfo(Archive *fout, TableInfo *tbinfo)
{
    PGresult   *res;
    PQExpBuffer query = createPQExpBuffer();
    ...
    {
        appendPQExpBuffer(query,
                          "SELECT seqlocal or (select count(*) > 0 from pg_class where relrowidseq = seqrelid)"
                          "FROM pg_class c "
						  "JOIN %s s ON (s.seqrelid = c.oid) "
                          "WHERE c.oid = '%u'::oid",
						  PQinOraMode() ? "pg_ora_sequence" : "pg_sequence",
    ...
    tbinfo->seqislocal = (strcmp(PQgetvalue(res, 0, 0), "t") == 0);
    PQclear(res);
    destroyPQExpBuffer(query);
}
...
dumpSequenceData(Archive *fout, TableDataInfo *tdinfo)
{
	...
	/* No need dump for local sequence. */
    getSequenceLocalInfo(fout, tbinfo);
	if (tbinfo->seqislocal)
		return;
	...
	if (fout->isPostgresXL)
	{
		...
		resetPQExpBuffer(query);
		if (PQinOraMode())
			appendPQExpBufferStr(query, "SELECT opentenbase_ora.nextval(");
		else
	...
	resetPQExpBuffer(query);
	if (PQinOraMode())
		appendPQExpBufferStr(query, "SELECT opentenbase_ora.setval(");
	else
...
dumpTrigger(Archive *fout, TriggerInfo *tginfo)
{
	...
	appendPQExpBuffer(trigprefix, "TRIGGER %s ", fmtId(tginfo->dobj.name));
 ...
 * getFormattedPkgFunc
 *
 * Get the function in the package and format the function parameter/return
 * value type as "schema.type".
 * 
 * pkgheader: If pkgheader is true, it means that the function in the package
 * header is being processed, and if false, it means that the function in the
 * package body is being processed.
 */
static char *
getFormattedPkgFunc(Archive *fout, Oid funcoid, bool pkgheader)
{
	int			i;
	int			nconfigitems = 0;
	Oid			prorettype;
	char	   *formattedfunc = NULL;
	char	   *funcfullsig = NULL; /* full signature */
	char	   *proname;
	char	   *proretset;
	char	   *prosrc;
	...
	appendPQExpBuffer(query,
					  ...
					  "pg_catalog.pg_get_function_result(oid) AS funcresult, "
					  "array_to_string(protrftypes, ' ') AS protrftypes, "
					  "prokind, provolatile, proisstrict, "
					  "proleakproof, proconfig, procost, prorows, "
					  "proparallel, proname, prorettype, "
					  "(SELECT lanname FROM pg_catalog.pg_language WHERE oid = prolang) AS lanname "
					  "FROM pg_catalog.pg_proc "
					  "WHERE oid = '%u'::pg_catalog.oid",
					  funcoid);
	...
	initPQExpBuffer(&fn);
	appendPQExpBufferStr(&fn, fmtId(proname));
	appendPQExpBuffer(&fn, "(%s)", funcargs);
	funcfullsig = fn.data;
	...
	/*
	 * The function name does not require schema information
	...
	{
		rettypename = getFormattedTypeName(fout, prorettype,
										   zeroAsOpaque);
		appendPQExpBuffer(q, " RETURNS %s%s",
						  (proretset[0] == 't') ? "SETOF " : "",
						  rettypename);
	...
	/* Functions in package body need to export asPart */
	if (!pkgheader)
	{
		/*
		 * See backend/commands/functioncmds.c for details of how the 'AS' clause
