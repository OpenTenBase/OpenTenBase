...
#ifdef _PG_ORCL_
	dopt->with_default_rowid = ropt->with_default_rowid;
#endif
	return dopt;
}
...
_doSetFixedOutputState(ArchiveHandle *AH)
{
	...
	/* Make sure function body checking is disabled */
	ahprintf(AH, "SET check_function_bodies = false;\n");

	/* Make sure function pludt checking is disabled */
	ahprintf(AH, "SET check_function_pludt = false;\n");
...
#ifdef _PG_ORCL_
	if (ropt && ropt->with_default_rowid)
		ahprintf(AH, "SET default_with_rowid = on;\n");
	else
		ahprintf(AH, "SET default_with_rowid = off;\n");
#endif

	/* Dump the empty string and not change it to null while it is in opentenbase_ora mode */
	if(PQinOraMode())
		ahprintf(AH, "SET disable_empty_to_null = on;\n");
...
_getObjectDescription(PQExpBuffer buf, TocEntry *te, ArchiveHandle *AH)
{
...
#ifdef _PG_ORCL_
	if (strcmp(type, "SYNONYM") == 0)
	{
		if (te->namespace == NULL)
		...
		strcmp(type, "USER MAPPING") == 0)
	{
		...
		if (strcmp(type, "COLLATION") == 0 ||
			...
			strcmp(type, "TEXT SEARCH CONFIGURATION") == 0 ||
			strcmp(type, "PROCEDURAL LANGUAGE") == 0)
			appendPQExpBufferStr(buf, fmtId_internal(te->tag, true));
		else
