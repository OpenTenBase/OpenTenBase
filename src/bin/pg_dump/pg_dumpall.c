...
static void buildShSecLabels(PGconn *conn, const char *catalog_name,
							 Oid objectId, const char *objtype,
							 const char *objname, PQExpBuffer buffer);
...
#ifdef _PG_ORCL_
static SimpleStringList exclude_users = {NULL, NULL};
#endif
...
#ifdef _PG_ORCL_
		{"dump-dbs", no_argument, NULL, 'D'},
		{"exclude-users", required_argument, NULL, 'E'},
#endif
...
#ifdef _PG_ORCL_
	while ((c = getopt_long(argc, argv, "acd:DE:f:gh:l:oOp:rsS:tuU:vwWx", long_options, &optindex)) != -1)
#else
...
#endif
	{
		switch (c)
		{
			...
			case 'D':
				...
				break;
			case 'E':
				simple_string_list_append(&exclude_users, optarg);
				break;
#endif
...
#ifdef _PG_ORCL_
	printf(_("  -D, --dump-dbs               dump database create DDL\n"));
	printf(_("  -E, --exclude-users          excludng users/roles to dump\n"));
#endif
#ifdef __OPENTENBASE__
	...
	for (i = 0; i < PQntuples(res); i++)
	{
		...
		else
		{
			...
			printfPQExpBuffer(buf, "CREATE RESOURCE GROUP %s WITH "
							  "concurrency %s %s %s "
							  "memory_limit %s;\n",
							  fmtId_internal(groupname, true), concurrency, cpu_prop, cpu_setting,
							  memory_limit);
		}
...
#ifdef _PG_ORCL_
		if (simple_string_list_member(&exclude_users, rolename))
			continue;
#endif
...
dumpRoles(PGconn *conn)
{
	...
	"  pg_resgroup.rsgname = rolresgroup) AS rolgroupname " : "";
	...
	if (server_version >= 90600)
		printfPQExpBuffer(buf,
						  ...
						  "FROM %s "
						  "WHERE rolname !~ '^pg_' "
						  "ORDER BY 2", role_catalog, resgroup_col, role_catalog);
...
dumpCreateDB(PGconn *conn)
{
	...
	if (server_version >= 90600)
		...
		printfPQExpBuffer(buf,
						  ...
						  "coalesce(rolname, (select rolname from %s where oid=(select datdba from pg_database where datname='template0'))), "
						  "pg_encoding_to_char(d.encoding), "
						  "datcollate, datctype, datfrozenxid, datminmxid, 0 AS datasmxid, "
		...
		printfPQExpBuffer(buf,
						  ...
						  "coalesce(rolname, (select rolname from %s where oid=(select datdba from pg_database where datname='template0'))), "
						  "pg_encoding_to_char(d.encoding), "
						  "null::text AS datcollate, null::text AS datctype, datfrozenxid, 0 AS datminmxid, 0 AS datasmxid, "
						  "datistemplate, datacl, '' as rdatacl, "
						  "datconnlimit, "
						  "(SELECT spcname FROM pg_tablespace t WHERE t.oid = d.dattablespace) AS dattablespace "
						  "FROM pg_database d LEFT JOIN %s u ON (datdba = u.oid) "
	...
	for (i = 0; i < PQntuples(res); i++)
	{
		...
		uint32		dbasmxid = atooid(PQgetvalue(res, i, 7));
		char       *dbsqlmode = PQgetvalue(res, i, 8);
		char	   *dbistemplate = PQgetvalue(res, i, 9);
		char	   *dbacl = PQgetvalue(res, i, 10);
		char	   *rdbacl = PQgetvalue(res, i, 11);
		char	   *dbconnlimit = PQgetvalue(res, i, 12);
		char	   *dbtablespace = PQgetvalue(res, i, 13);
		char	   *fdbname;
		 ...
		 */
		if (strcmp(dbname, "template1") != 0 &&
			strcmp(dbname, "template1_ora") != 0 &&
			strcmp(dbname, "postgres") != 0)
		{
			appendPQExpBuffer(buf, "CREATE DATABASE %s", fdbname);

			/* postgres mode, 'o' for opentenbase_ora ,null and 'p' for postgres */
			if (dbsqlmode == NULL || strcmp(dbsqlmode, "p") == 0)
				appendPQExpBufferStr(buf, " WITH TEMPLATE = template0");
			else
				appendPQExpBufferStr(buf, " WITH TEMPLATE = template0_ora");
					   ...
					   const char *type2, const char *name2)
{
	...
	*pos = 0;
	...
	appendPQExpBuffer(buf, "SET %s TO ", fmtId_internal(mine, true));
				...
				trivalue prompt_password, bool fail_on_error)
{
	...
	executeCommand(conn, ALWAYS_SECURE_SEARCH_PATH_SQL);
...
dumpNodes(PGconn *conn)
{
	...
	appendPQExpBuffer(query, "select 'CREATE NODE ' || node_name || '"
					...
					" then 'coordinator' when node_type='D' then 'datanode' else 'forward' end) || chr(39)"
					...
					" ||', CLUSTER= ' || chr(39) || node_plane_name || chr(39) ||');' "
					" as node_query from pg_catalog.pgxc_node where node_type in ('C', 'D') order by node_name, node_plane_name");
	...
	appendPQExpBuffer(query, "select 'CREATE NODE ' || node_name || '"
					...
					" then 'coordinator' when node_type='D' then 'datanode' else 'forward' end) || chr(39)"
					" || ' , HOST = ' || chr(39) || node_host || chr(39)"
					" || ', PORT = ' || node_port || ', FORWARD = ' || node_forward_port || ');' "
					" as node_query from pg_catalog.pgxc_node where node_type in ('F') order by node_name, node_plane_name");
