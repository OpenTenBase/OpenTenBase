...
#ifdef _PGPAL_
extern int InfoReloadHoldoffCount;
extern int InfoReloadPending;
extern bool g_alter_setting_wait_for_commit_or_abort;
extern void HandleInfoReload(void);

#define CHECK_FOR_INFO_RELOAD() \
do { \
	if (InfoReloadPending) \
		HandleInfoReload(); \
} while(0)

#define HOLD_INFO_RELOAD()  (InfoReloadHoldoffCount++)

#define RESUME_INFO_RELOAD() \
do { \
	Assert(InfoReloadHoldoffCount > 0); \
	if(InfoReloadHoldoffCount > 0)	\
		InfoReloadHoldoffCount--; \
} while(0)
