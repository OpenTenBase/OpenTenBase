...
{
	my $lsn = shift;
	my $size_of_segment = $WAL_SEGMENT_SIZE * $WAL_BLOCK_SIZE;
	return ($lsn / $size_of_segment, $lsn % $size_of_segment);
}
...
my $node = get_new_node("node", 'datanode');
$node->init(extra => ['--master_gtm_nodename', 'no_gtm',
...
print "WAL_SEGMENT_SIZE: $WAL_SEGMENT_SIZE WAL_BLOCK_SIZE: $WAL_BLOCK_SIZE TLI: $TLI\n";
my $end_lsn;
my $prev_lsn;
...
$log_size = -s $node->logfile;
$node->start;
ok($node->log_contains("record length 2147483648 at .* too long", $log_size),
	"xlp_magic zero");
...
$node->start;
ok( $node->log_contains(
		"record length 2147483648 at .* too long", $log_size),
	"xlp_pageaddr bad");
