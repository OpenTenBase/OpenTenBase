...
#ifdef __OPENTENBASE_C__
static int	co1_fn_port = -1;
static int	co2_fn_port = -1;
static int	dn1_fn_port = -1;
static int	dn2_fn_port = -1;
#endif

static bool single_cn_dn = false;
/*
...
bool g_bEnablePerfDataPrint = true;
...
#define REGR_PRINT_ELAPSED_TIME_TEMP(i)                                \
    {                                                                  \
        if (g_bEnablePerfDataPrint) {                                  \
            status(_("elapsed: [%9.3lf]"), g_dOneRegrTotalTime[i]);	   \
        }                                                              \
    }
...
/* Attempt to get the node port by the environment variable. */
static void
get_env_port(int *port, char *envname)
{
	char *pgport = getenv(envname);
...
/*
 * Get forward node port number
 */
static int
get_forward_port(PGXCNodeTypeNum node)
{
	switch (node)
	{
		case PGXC_COORD_1:
...
initdb_node(PGXCNodeTypeNum node)
{
	...
	else
	{
		...
		snprintf(buf, sizeof(buf),
				 ...
				 "--master_gtm_nodename %s --master_gtm_ip %s --master_gtm_port %d  --user %s > \"%s/log/initdb.log\" 2>&1",
				 bindir ? bindir : "",
				 bindir ? "/" : "",
...
#ifdef __LICENSE__
#ifndef NOLIC
		snprintf(buf, sizeof(buf), "cp \"%s/license.info\" \"%s/%s/pgxz_license/\"",
			inputdir, temp_instance, data_folder);
		if (system(buf))
		{
			fprintf(stderr, _("\n%s: cp failed\nExamine %s/log/initdb.log for the reason.\nCommand was: %s\n"), progname, outputdir, buf);
			exit(2);
...
set_node_config_file(PGXCNodeTypeNum node)
{
	...
	snprintf(buf, sizeof(buf), "forward_port = %d\n", get_forward_port(node));
	fputs(buf, pg_conf);
	...
	fputs("pool_maintenance_timeout = 1\n", pg_conf);
...
setup_connection_information(void)
{
	...
	psql_command_node("postgres", PGXC_COORD_1, "ALTER NODE %s WITH (HOST = 'localhost',"
					  " type = 'coordinator', PORT = %d, FORWARD = %d);",
					  (char *)get_node_name(PGXC_COORD_1),
					  get_port_number(PGXC_COORD_1),
					  get_forward_port(PGXC_COORD_1));
	...
	if (!single_cn_dn)
		psql_command_node("postgres", PGXC_COORD_1, "CREATE NODE %s WITH (HOST = 'localhost',"
						  ...
						  get_forward_port(PGXC_COORD_2));
	/* Datanodes on Coordinator 1 */
	psql_command_node("postgres", PGXC_COORD_1, "CREATE NODE %s WITH (HOST = 'localhost',"
					  " type = 'datanode', PORT = %d, FORWARD = %d, PRIMARY, PREFERRED);",
					  (char *)get_node_name(PGXC_DATANODE_1),
					  get_port_number(PGXC_DATANODE_1),
					  get_forward_port(PGXC_DATANODE_1));
	if (!single_cn_dn)
		psql_command_node("postgres", PGXC_COORD_1, "CREATE NODE %s WITH (HOST = 'localhost',"
						  " type = 'datanode', PORT = %d, FORWARD = %d);",
						  (char *)get_node_name(PGXC_DATANODE_2),
						  get_port_number(PGXC_DATANODE_2),
						  get_forward_port(PGXC_DATANODE_2));
	...
	{/* -----coordinator 2 ------- */
		...
		psql_command_node("postgres", PGXC_COORD_2,
	...
	{/* -----datanode 2 ------- */
		...
		psql_command_node("postgres", PGXC_COORD_1,
		                  "EXECUTE DIRECT ON ( %s ) $$CREATE NODE %s WITH (HOST = 'localhost',"
		                  " type = 'coordinator', PORT = %d, FORWARD = %d)$$;",
		                  (char *) get_node_name(PGXC_DATANODE_2),
		                  (char *) get_node_name(PGXC_COORD_1),
		                  get_port_number(PGXC_COORD_1),
		...
		psql_command_node("postgres", PGXC_COORD_1,
		                  "EXECUTE DIRECT ON ( %s ) $$ ALTER NODE %s WITH (HOST = 'localhost',"
		                  " type = 'datanode', PORT = %d, FORWARD = %d)$$;",
		                  (char *) get_node_name(PGXC_DATANODE_2),
		                  (char *) get_node_name(PGXC_DATANODE_2),
		                  get_port_number(PGXC_DATANODE_2),
	...
	{
		psql_command_node("postgres", PGXC_COORD_1,
		                  "create DEFAULT node group default_group with (%s, %s);",
		                  (char *) get_node_name(PGXC_DATANODE_1),
		                  (char *) get_node_name(PGXC_DATANODE_2));
		psql_command_node("postgres", PGXC_COORD_1,
	...
	{
		psql_command_node("postgres", PGXC_COORD_1,
		                  "create DEFAULT node group default_group with (%s);",
		                  (char *) get_node_name(PGXC_DATANODE_1));
		psql_command_node("postgres", PGXC_COORD_1,
		                  "create sharding group to group default_group;");
		psql_command_node("postgres", PGXC_COORD_1, "clean sharding;");
...
convert_sourcefiles_in(char *source_subdir, char *dest_dir, char *dest_subdir, char *suffix)
{
	...
	for (name = names; *name; name++)
	{
		...
		snprintf(destfile, MAXPGPATH, "%s/%s/%s.%s", dest_dir, dest_subdir,
				 prefix, suffix);
		snprintf(coord_port_str, 8, "%d", port);
		...
		while (fgets(line, sizeof(line), infile))
		{
			...
			replace_string(line, "@dblink_port@", coord_port_str);
			fputs(line, outfile);
		}
		fclose(infile);
		fclose(outfile);
...
#define MAX_DBNAME 256
	char ora_dbname[MAX_DBNAME];

    snprintf(ora_dbname,MAX_DBNAME,"%s_ora",dbname);
	header(_("dropping database \"%s\""), dbname);
	psql_command("postgres", "DROP DATABASE IF EXISTS \"%s\"", dbname);
	psql_command("postgres", "DROP DATABASE IF EXISTS \"%s\"", ora_dbname);
}
...
#define MAX_DBNAME 256
	...
	if (encoding)
    {
        ...
        psql_command("postgres", "CREATE DATABASE \"%s\" TEMPLATE=template0_ora ENCODING='%s'%s", ora_dbname, encoding,
                     (nolocale) ? " LC_COLLATE='C' LC_CTYPE='C'" : "");
    }
	else
    {
        psql_command("postgres", "CREATE DATABASE \"%s\" TEMPLATE=template0 %s", dbname,
                     (nolocale) ? " LC_COLLATE='C' LC_CTYPE='C'" : "");
        psql_command("postgres", "CREATE DATABASE \"%s\" TEMPLATE=template0_ora %s", ora_dbname,
                     (nolocale) ? " LC_COLLATE='C' LC_CTYPE='C'" : "");
    }
	psql_command(dbname,
    ...
    psql_command(ora_dbname,
                 ...
                 "ALTER DATABASE \"%s\" SET bytea_output TO 'hex';"
                 "ALTER DATABASE \"%s\" SET timezone_abbreviations TO 'Default';",
                 ora_dbname, ora_dbname, ora_dbname, ora_dbname, ora_dbname, ora_dbname);
	...
	for (sl = loadlanguage; sl != NULL; sl = sl->next)
	{
        ...
        psql_command(ora_dbname, "CREATE OR REPLACE LANGUAGE \"%s\"", sl->str);
	}
	...
	for (sl = loadextension; sl != NULL; sl = sl->next)
	{
        ...
        psql_command(ora_dbname, "CREATE EXTENSION IF NOT EXISTS \"%s\"", sl->str);
	}
}
...
regression_main(int argc, char *argv[], init_function ifunc, test_function tfunc)
{
	static struct option long_options[] = {
		...
		{"single-cn-dn", no_argument, NULL, 26},
		{NULL, 0, NULL, 0}
	};
	...
	if ((temp_instance && !port_specified_by_user) || port == -1)
...
#ifdef __OPENTENBASE_C__
	co1_fn_port = (0xC000 | (PG_VERSION_NUM & 0x3FFF)) + 9;
	co2_fn_port = (0xC000 | (PG_VERSION_NUM & 0x3FFF)) + 10;
	dn1_fn_port = (0xC000 | (PG_VERSION_NUM & 0x3FFF)) + 11;
	dn2_fn_port = (0xC000 | (PG_VERSION_NUM & 0x3FFF)) + 12;
#endif

#endif

	header("************* get_env_port set local port from env *******************");
	get_env_port(&port, "MY_LOCAL_CN1PORT");
	get_env_port(&port_coord2, "MY_LOCAL_CN2PORT");
	get_env_port(&port_dn1, "MY_LOCAL_DN1PORT");
	get_env_port(&port_dn2, "MY_LOCAL_DN2PORT");
	get_env_port(&port_gtm, "MY_LOCAL_GTMPORT");
	get_env_port(&co1_pooler_port, "MY_LOCAL_CN1POOLERPORT");
	get_env_port(&co2_pooler_port, "MY_LOCAL_CN2POOLERPORT");
	get_env_port(&dn1_pooler_port, "MY_LOCAL_DN1POOLERPORT");
	get_env_port(&dn2_pooler_port, "MY_LOCAL_DN2POOLERPORT");
	get_env_port(&co1_fn_port, "MY_LOCAL_CN1FNPORT");
	get_env_port(&co2_fn_port, "MY_LOCAL_CN2FNPORT");
	get_env_port(&dn1_fn_port, "MY_LOCAL_DN1FNPORT");
	get_env_port(&dn2_fn_port, "MY_LOCAL_DN2FNPORT");
...
#error Platform has no means to secure the test installation.
		...
		for (i = 0; i < wait_seconds; i++)
		{
			...
			if (check_node_running(PGXC_COORD_1) &&
				(single_cn_dn || check_node_running(PGXC_COORD_2)) &&
				check_node_running(PGXC_DATANODE_1) &&
				(single_cn_dn || check_node_running(PGXC_DATANODE_2)))
				break;
