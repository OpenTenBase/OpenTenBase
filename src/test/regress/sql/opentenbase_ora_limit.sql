...
-- test LIMIT ALL clause
CREATE TABLE t_limit(a int);
INSERT INTO t_limit VALUES(1), (2), (3);
SELECT * FROM t_limit ORDER BY 1 LIMIT ALL;
SELECT * FROM t_limit ORDER BY 1 LIMIT ALL OFFSET 1;
DROP TABLE t_limit;
...
drop table t1;
drop table t2;
create table t1(id int primary key, name varchar2(20), age int, sex int, point number) distribute by shard(id);
insert into t1 values(1, '张三', 15, 1, 93);
insert into t1 values(2, '李四', 20, 2, 90);
insert into t1 values(3, '王五', 19, 2, 85);
insert into t1 values(4, '李六', 30, 1, 73);
insert into t1 values(5, '李六', 20, 1, 88);
insert into t1 values(6, '赵六', 19, 2, 63);

-- replication表
create table t2(id int primary key, name varchar2(20), age int, sex int, point number) distribute by replication;
insert into t2 values(1, '张三', 15, 1, 93);
insert into t2 values(2, '李四', 20, 2, 90);
insert into t2 values(3, '王五', 19, 2, 85);
insert into t2 values(4, '李六', 30, 1, 73);
insert into t2 values(5, '李六', 20, 1, 88);
insert into t2 values(6, '赵六', 19, 2, 63);

select * from t1 order by id offset 2 row fetch first null rows only;
select * from t1 order by id offset 2 row fetch first -3 percent row only;
select * from t1 order by id offset 2 row fetch first null percent row only;
select * from t1 order by id offset 2 row fetch first 100.0 percent row only;

explain (verbose, nodes off, costs off)select * from t2 order by id offset 2 row fetch first null rows only;
explain (verbose, nodes off, costs off)select * from t2 order by id offset 2 row fetch first -3 percent row only;
explain (verbose, nodes off, costs off)select * from t2 order by id offset 2 row fetch first null percent row only;
explain (verbose, nodes off, costs off)select * from t2 order by id offset 2 row fetch first 100.0 percent row only;

select * from t2 order by id offset 2 row fetch first null rows only;
select * from t2 order by id offset 2 row fetch first -3 percent row only;
select * from t2 order by id offset 2 row fetch first null percent row only;
select * from t2 order by id offset 2 row fetch first 100.0 percent row only;
drop table t1;
drop table t2;
