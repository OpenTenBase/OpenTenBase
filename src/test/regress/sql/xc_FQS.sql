...
-- Some scenarios do not support FQS, for example, when one query contains multiple statements.
set default_transaction_isolation to 'repeatable read';
set enable_fast_query_shipping to on;
drop table if exists i_table;
create table i_table (f1 int) Distribute By SHARD(f1);
...
explain (costs off, verbose on) select from (select a from ab) group by a;
select from (select a from ab) group by a;
drop table ab;

set enable_fast_query_shipping to on;
create table fqs_t1 (c1 int, c2 int, c3 int) distribute by shard(c1, c2);
explain (costs off) insert into fqs_t1(c1) values(random());
explain (costs off) insert into fqs_t1(c1, c2) values(random(), 1);
drop table fqs_t1;
reset enable_fast_query_shipping;
drop table if exists fqst1;
create table fqst1(c0 int, c1 int, c2 int, c3 text) distribute by shard(c3, c1);
prepare sh(text) as select * from fqst1 where c3=$1 and c0 > 5 and c0 < 100 order by c1;
explain (costs off)execute sh('test');
deallocate sh;
drop table if exists fqst1;
