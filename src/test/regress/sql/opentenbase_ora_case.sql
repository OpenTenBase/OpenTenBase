...
SELECT vala.column1, vala.column2, vala.column3
FROM ( VALUES ( 1, NULL :: text,  NULL :: text ), ( NULL :: integer, NULL :: text, NULL :: text)) vala;
...
-- test group by / having clause order
drop table if exists test_group_having;
create table test_group_having(i int, j int);
insert into test_group_having values (1,2),(1,3),(2,4),(2,5),(3,6);
select sum(j) as sumj from test_group_having having sum(j) > 6 group by i order by i;
select sum(j) as sumj from test_group_having  group by i having sum(j) > 6 order by i;
select sum(j) as sumj from test_group_having  group by i order by i;
select distinct i, sum(j) from test_group_having having sum(j) > 0 group by j,i order by j,i;
select distinct i, sum(j) from test_group_having  group by j,i having sum(j) > 0 order by j,i;
select distinct i, sum(j) from test_group_having group by j,i order by j,i;
select distinct i, j  from test_group_having order by 1;
drop table test_group_having;
