...
result timestamp(0);
begin
result := to_date('1970-01-01','yyyy-mm-dd') + (timest + 28800) / 86400;
return result;
end
$function$
;
...
create function cnf1(in i int, out j int) returns setof int as $$
begin
  j := i+1;
  return next;
  j := i+2;
...
create or replace function cnf2(i int) returns int  as $$
begin
  i := i+1;
  return i;
end$$ language plpgsql  not pushdown;

explain (verbose, costs off, nodes off)
with g as(select cnf1(cnf2(42)) j from generate_series(1, 2) )
select 'aaa', j, count(*) from g where g.j > 10 group by j order by j;
