...
CREATE TYPE "TYPE_UPPER" AS  ("X" INT, "Y" INT);
ALTER TYPE TYPE_UPPER RENAME TO "TY_UPPER";
CREATE TABLE T_UPPER(ID INT, L_HOME TY_UPPER, L_COMPANY "TY_UPPER");
INSERT INTO T_UPPER VALUES(1, (124, 53), (77, 89));
SELECT * FROM T_UPPER;
DROP TABLE T_UPPER;
DROP TYPE TY_UPPER;
-- create, alter, use, drop trigger
CREATE TABLE T_UPPER(ID INT, NAME TEXT);
CREATE TABLE T_LOG(ID INT, CREATE_T TIMESTAMP);
CREATE OR REPLACE FUNCTION LOG_INSERT() RETURNS TRIGGER
IS 
BEGIN
INSERT INTO T_LOG VALUES(new.ID, current_timestamp);
END;
/
SET enable_datanode_row_triggers TO on;
CREATE TRIGGER TRIG_UPPER AFTER INSERT ON T_UPPER  
FOR EACH ROW EXECUTE PROCEDURE LOG_INSERT();
DELETE FROM T_LOG;
INSERT INTO T_UPPER VALUES(1,'Tom');
SELECT COUNT(*) FROM T_LOG;
ALTER TRIGGER "TRIG_UPPER" ON T_UPPER RENAME TO "TR_UPPER";
RESET enable_datanode_row_triggers;
DROP TRIGGER "TR_UPPER" ON T_UPPER;
DROP TABLE T_UPPER;
DROP TABLE T_LOG;
...
SELECT LAST_VALUE from S_UPPER;
...
create table t_search_path_20240117(id int);
insert into t_search_path_20240117 values (1);
for c in (select id from t_search_path_20240117) LOOP
raise notice '%', c.id;
END LOOP;
END;
$$
