...
select tmplname,tmpltrusted,tmpldbacreate,tmplhandler,tmplinline,tmplvalidator,tmpllibrary,tmplacl from pg_pltemplate order by 1;
select oid, spcname, spcowner, spcacl from pg_tablespace where oid < 10000 order by 1;
select dbid,classid,objid,objsubid,refclassid,refobjid,deptype from pg_shdepend where dbid = 0 and refobjid < 10000 order by 5,6,7;
select oid,typname,typnamespace, typowner, typbyval, typtype, typcategory, typispreferred, typisdefined, typdelim,typinput,typoutput,typreceive,typsend,typmodin,typmodout,typanalyze,typalign,typstorage,typnotnull,typbasetype,typtypmod,typndims,typcollation,typdefaultbin,typdefault,typacl from pg_type where oid < 10000 order by 1;
...
select roleid,member,grantor,admin_option from pg_auth_members where roleid < 10000 order by 1;
...
select oid,umuser,umserver,umoptions from pg_user_mapping where oid < 16384;
select objoid,classoid,description from pg_shdescription where objoid < 10000 order by 1;
select classoid, description from pg_shdescription where objoid > 10000 and objoid < 16384 order by 1, 2;
select aggfnoid,aggkind,aggnumdirectargs,aggtransfn,aggfinalfn,aggcombinefn,aggserialfn,aggdeserialfn,aggmtransfn,aggminvtransfn,aggmfinalfn,aggfinalextra,aggmfinalextra,aggfinalmodify,aggmfinalmodify,aggsortop,aggtranstype,aggtransspace,aggmtranstype,aggmtransspace,agginitval,aggminitval from pg_aggregate order by 1,2,3,4;
select oid,amname,amhandler,amtype from pg_am order by 1;
select amopfamily,amoplefttype,amoprighttype,amopstrategy,amoppurpose,amopopr,amopmethod,amopsortfamily from pg_amop order by 1,2,3,4;
select amprocfamily,amproclefttype,amprocrighttype,amprocnum,amproc from pg_amproc order by 1,2,3,4;
select adrelid,adnum,adbin,adsrc from pg_attrdef where adrelid < 16384;
select castsource,casttarget,castfunc,castcontext,castmethod from pg_cast where castsource < 10000 and casttarget < 10000 order by 1,2,3,4;
...
select conname,connamespace,conowner,conforencoding,contoencoding,conproc,condefault from pg_conversion where oid < 16384 order by 1;
select objoid,classoid,objsubid,description from pg_description where objoid < 10000 order by 2,1;
select indexrelid,indrelid,indnatts,indisunique,indisprimary,indisexclusion,indimmediate,indisclustered,indisvalid,indcheckxmin,indisready,indislive,indisreplident,indkey,indcollation,indclass,indexprs,indpred from pg_index where indrelid < 10000 order by 1,2;
select oid,lanname,lanowner,lanispl,lanpltrusted,lanplcallfoid,laninline,lanvalidator,lanacl from pg_language where oid < 10000 order by 1;
select lanname,lanowner,lanispl,lanpltrusted,lanplcallfoid,laninline,lanvalidator,lanacl from pg_language where oid > 10000 and oid < 16384;
...
select opcmethod,opcname,opcnamespace,opcowner,opcfamily,opcintype,opcdefault,opckeytype from pg_opclass order by 1,2;
select oid,oprname,oprnamespace,oprowner,oprkind,oprcanmerge,oprcanhash,oprleft,oprright,oprresult,oprcom,oprnegate,oprcode,oprrest,oprjoin from pg_operator where oid < 10000 order by 1,7,8,9;
select oid,opfmethod,opfname,opfnamespace,opfowner from pg_opfamily order by 1;
select oid,extname,extowner,extnamespace,extrelocatable,extversion,extconfig,extcondition from pg_extension where oid < 16384;
select oid,collname,collnamespace,collowner,collprovider,collencoding,collcollate,collctype,collversion from pg_collation where oid < 10000 order by 1;
select rngtypid,rngsubtype,rngcollation,rngsubopc,rngcanonical,rngsubdiff from pg_range order by 1;
select dictname,dictnamespace,dictowner,dictinitoption from pg_ts_dict order by 1;
select oid,prsname,prsnamespace,prsstart,prstoken,prstoken,prsend,prsheadline,prslextype from pg_ts_parser order by 1;
select cfgname,cfgnamespace,cfgowner,cfgparser from pg_ts_config order by 1;
select mapcfg,maptokentype,mapseqno,mapdict from pg_ts_config_map where mapcfg < 10000 order by 1,2;
select oid,tmplname,tmplnamespace,tmplinit,tmpllexize from pg_ts_template where oid < 10000;
select oid,rsgname,parent from pg_resgroup order by 1;
select resgroupid,reslimittype,value from pg_resgroupcapability order by 1;
select schemaname,viewname from pg_views  where schemaname = 'pg_catalog' or schemaname = 'information_schema' order by 1,2;
...
select schemaname,viewname from pg_views where schemaname = 'information_schema' order by 1,2 limit 1;
...
select schemaname,relname from pg_stat_user_tables order by 1,2 limit 1;
select schemaname,relname from pg_stat_xact_user_tables  order by 1,2 limit 1;
...
select schemaname,relname from pg_statio_user_tables order by 1,2 limit 1;
...
select schemaname,relname,indexrelname from pg_stat_user_indexes order by 1,2,3 limit 1;
...
select * from information_schema.constraint_column_usage order by 1,2,3 limit 1;
select * from information_schema.constraint_table_usage order by 1,2,3 limit 1;
select * from information_schema.domain_constraints limit 1;
select * from information_schema.domain_udt_usage order by 1,2,3 limit 1;
select * from information_schema.domains order by 1,2,3 limit 1;
select constraint_catalog,constraint_schema,constraint_name from information_schema.key_column_usage where constraint_schema = 'SQUEEZE' limit 1;
...
select specific_catalog,specific_schema from information_schema.routine_privileges where specific_schema = 'DBMS_ALERT' order by 1,2 limit 1;
select specific_catalog,specific_schema from information_schema.role_routine_grants where specific_schema = 'DBMS_ALERT' order by 1,2 limit 1;
select * from information_schema.routines limit 1;
select catalog_name, schema_name from information_schema.schemata where schema_name = 'DBMS_ALERT' order by 1,2 limit 1;
...
select object_catalog,object_schema,object_type from information_schema.data_type_privileges where object_schema = 'DBMS_ALERT' order by 1,2,3 limit 1;
select object_catalog,object_schema,object_type from information_schema.element_types order by 1,2,3 limit 1;
...
select schema_name,table_name from all_col_comments order by 1,2 limit 1;
select * from user_col_comments order by 1,2,3,4 limit 1;
select constraint_schema from all_cons_columns order by 1 limit 1;
select constraint_schema,table_schema,table_name,column_name from user_cons_columns order by 1,2,3,4 limit 1;
select constraint_schema from all_constraints order by 1 limit 1;
select constraint_schema,constraint_type,table_schema,table_name from user_constraints order by 1,2,3,4 limit 1;
select table_schema,table_name from all_tab_columns order by 1,2 limit 1;
select table_schema,table_name,column_name from user_tab_columns order by 1,2,3 limit 1;
