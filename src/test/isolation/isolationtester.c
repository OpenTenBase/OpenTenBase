...
static PGconn *conn_checkwait = NULL;
static bool is_centralized_mode = false;
...
static void
setup_conn_checkwait(const char *conninfo)
{
	PGresult   *res;

	conn_checkwait = PQconnectdb(conninfo);
	if (PQstatus(conn_checkwait) != CONNECTION_OK)
	{
		fprintf(stderr, "Connection failed: %s", PQerrorMessage(conn_checkwait));
		exit(1);
	}

	/* set default_locator_type to shard */
	res = PQexec(conn_checkwait, "set default_locator_type=shard");
	if (PQresultStatus(res) != PGRES_COMMAND_OK)
	{
		fprintf(stderr,
				"Set default param default_locator_type to shard failed: %s",
				PQerrorMessage(conn_checkwait));
		exit(1);
	}
	PQclear(res);

	res = PQexec(conn_checkwait, "show is_centralized_mode;");
	if (PQresultStatus(res) == PGRES_TUPLES_OK && PQntuples(res) == 1 && PQnfields(res) == 1)
	{
		...
		{
			fprintf(stderr, "Invalid value for is_centralized_mode: %s", value);
			exit(1);
		}
	}
	else
	{
		fprintf(stderr, "show is_centralized_mode failed: %s", PQerrorMessage(conn_checkwait));
		exit(1);
	}
	PQclear(res);
}

/* Construct the SQL query based on the mode */
static void
construct_query(char *query, const char *pid)
{
	...
	{
		sprintf(query,
		        "SELECT wait_event_type, wait_event FROM pg_stat_cluster_activity WHERE queryid = "
		        "(SELECT queryid "
		        "FROM pg_stat_cluster_activity WHERE pid=%s)",
		        pid);
	}
}

/* Process the query results to check if the process is waiting or needs retry */
static bool
process_query_results(PGresult *res, int try_cnt, bool *need_retry)
{
	bool waiting = false;
	int  i;
	...
	{
		/* Check for "Lock" in the wait_event_type column */
		if (strstr(PQgetvalue(res, i, 0), "Lock") != NULL)
		{
			/* Check if we need to retry for non-blocking events */
			if (try_cnt > 0 && strcmp(PQgetvalue(res, i, 1), "virtualxid") == 0)
			{
				/* Flag for retrying */
				*need_retry = true;
				break;
...
check_waiting(const char *pid)
{
	...
	construct_query(query, pid);

	/* Retry loop for checking if waiting */
	while (try_cnt > 0 && (need_retry || try_cnt == 10))
	{
		need_retry = false;
		waiting = false;
		try_cnt--;

		/* Execute the query and check result status */
		res = PQexec(conn_checkwait, query);
		if (PQresultStatus(res) != PGRES_TUPLES_OK || PQntuples(res) < 0)
		{
			fprintf(stderr, "Query execution failed: %s", PQerrorMessage(conn_checkwait));
			PQclear(res);
			exit(1);
		}

		/* Process each result tuple */
		waiting = process_query_results(res, try_cnt, &need_retry);

		PQclear(res);

		/* If retry is needed, sleep for a while */
		if (need_retry)
		{
			usleep(100000);
		}
...
main(int argc, char **argv)
{
	...
	for (i = 0; i < nconns; i++)
	{
		...
		/* set default_locator_type to shard */
		res = PQexec(conns[i].conn, "set default_locator_type=shard");
		if (PQresultStatus(res) != PGRES_COMMAND_OK)
		{
			fprintf(stderr,
			        "Set default param default_locator_type to shard failed: %s",
...
try_complete_step(TestSpec *testspec, PermutationStep *pstep, int flags)
{
	...
	while (PQisBusy(conn))
	{
		...
		else if (ret == 0)		/* select() timeout: check for lock wait */
		{
			...
			{
				bool waiting = check_waiting(conns[step->session + 1].backend_pid_str);
