...
SELECT '' AS five, f.f1, abs(f.f1) AS abs_f1 FROM FLOAT4_TBL f ORDER BY f1;
...
CREATE TABLE FLOAT4_TBL2(f1 float4) DISTRIBUTE BY SHARD(f1);
insert into FLOAT4_TBL2 values('1.2345618901234e-20');
select * from FLOAT4_TBL2;
drop table FLOAT4_TBL2;
...
-- https://tapd.woa.com/20385652/prong/stories/view/1020385652886695821
CREATE OR REPLACE FUNCTION get_random_ng_3() RETURNS integer AS
$BODY$
BEGIN
RETURN floor(random() * (3) -1);
END;
$BODY$
LANGUAGE plpgsql;

create table row_float4(c1 float4, c2 float4);
ALTER FUNCTION get_random_ng_3() PUSHDOWN;

drop table row_float4;
drop function get_random_ng_3;
