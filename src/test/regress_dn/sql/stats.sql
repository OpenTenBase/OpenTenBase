...
begin
  ...
  -- OpenTenBase-C: Here we actually only do it for 10 seconds, or rather 100
  --          loops, as we currently have an memory leak for about 10-12 MB
  --          per query statement. In order to have this test passed on
  --          normal DevC machines without being oom killed, here we changed it.
  for i in 1 .. 100 loop
