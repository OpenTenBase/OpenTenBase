...
begin
    ...
    ilink := mytype || substr(blname, 1, 2);
    if ilink = ''PLPL'' then
        raise exception
		''backlink between two phone lines does not make sense'';
    end if;
    if ilink in (''PLWS'', ''WSPL'') then
        raise exception
		''direct ilink of phone line to wall slot not permitted'';
    end if;
    if mytype = ''PS'' then
...
begin
    ...
    if liink in (''PHHS'', ''HSPH'') then
        raise exception
		''ilink of phone to hub does not make sense'';
    end if;
    if ilink in (''PHIF'', ''IFPH'') then
        raise exception
		''ilink of phone to hub does not make sense'';
    end if;
    if ilink in (''PSWS'', ''WSPS'') then
        raise exception
...
-- by opentenbase_ora's PL/SQL but was handled differently by PG before 9.1.
...
create temp table foo (f1 int, f2 int) distribute by replication;
...
(
      trs_level int NOT NULL,
      node_no int NOT NULL,
      status int,
       PRIMARY KEY (trs_level, node_no)
) WITHOUT OIDS;
...
INSERT INTO transition_table_status (trs_level, node_no, status)
...
DROP FUNCTION IF EXISTS return10();
CREATE FUNCTION return10()
RETURNS INTEGER AS $$
BEGIN
    RETURN 10;
END $$ LANGUAGE plpgsql;
SELECT return10();
CALL return10();
DROP FUNCTION return10();

DROP FUNCTION IF EXISTS returnlist(int, int);
CREATE FUNCTION returnlist(int, int)
RETURNS SETOF INT AS $$
DECLARE
    i INT;
BEGIN
    ...
    RETURN;
END $$ LANGUAGE plpgsql;
SELECT returnlist(1, 2);
CALL returnlist(1, 2);
DROP FUNCTION returnlist(int, int);
...
DROP FUNCTION IF EXISTS calculate_total_price(quantity INTEGER, unit_price INTEGER);
DROP FUNCTION IF EXISTS multiply_numbers(a INTEGER, b INTEGER);
CREATE FUNCTION calculate_total_price(quantity INTEGER, unit_price INTEGER)
RETURNS INTEGER AS $$
BEGIN
    RETURN multiply_numbers(quantity, unit_price);
END $$ LANGUAGE plpgsql;
...
BEGIN
    RETURN a * b;
END $$ LANGUAGE plpgsql;
SELECT calculate_total_price(5, 10);
CALL calculate_total_price(5, 10);
DROP FUNCTION calculate_total_price(quantity INTEGER, unit_price INTEGER);
DROP FUNCTION multiply_numbers(a INTEGER, b INTEGER);

DROP FUNCTION IF EXISTS print2();
DROP FUNCTION IF EXISTS print1();
...
SELECT print2();
CALL print2();
-- Calling functions within anonymous blocks is not supported.
DO $$
BEGIN
	CALL print1();
END;
$$ LANGUAGE plpgsql;
DROP FUNCTION print2();
DROP FUNCTION print1();

-- adapt to lightweight_ora newparam usage
-- Usage 1, input parameters with the same name.
drop table if exists tb1;
drop table if exists tb1_f;
create table tb1(id int, a character varying);
create table tb1_f(id int, a character varying);
insert into tb1 values(1,'a'),(1,'b');
insert into tb1_f values(2,'a'),(2,'b');
select * from tb1 order by 1;
CREATE FUNCTION job_imp_tb1(a character varying)
RETURNS void
LANGUAGE plpgsql
AS $$
DECLARE
...
insert into tb1
select * from tb1_f where a =:a;
END;
$$;
select job_imp_tb1('a');
select * from tb1 order by 1;
drop function job_imp_tb1(a character varying);
drop table tb1;
drop table tb1_f;
-- Usage 2, parameters with the same name in anonymous blocks.
drop table if exists tb1;
drop table if exists tb1_f;
create table tb1(id int, a character varying);
create table tb1_f(id int, a character varying);
...
insert into tb1_f values(2,'a'),(2,'b');
select * from tb1 order by 1;
CREATE FUNCTION job_imp_tb1_2(b character varying)
RETURNS void
LANGUAGE plpgsql
AS $$
DECLARE
...
END;
$$;
select job_imp_tb1_2('a');
select * from tb1 order by 1;
drop function job_imp_tb1_2(b character varying);
drop table tb1;
drop table tb1_f;
-- Usage 3, other cases
drop table if exists tb1;
...
a := 'a';
action := :a;
delete from tb1 where a = :action;
insert into tb1
select * from tb1_f where a =:action;
END;
$$;
select job_imp_tb1_2('a');
select * from tb1 order by 1;
...
-- test something in opentenbase_ora mode
\c regression_ora
-- cherry-pick from f1631ddd95c7219383591be86ee8b89b8089463b
declare
v1 number;
v2 number;
v3 number;
begin
v1:=3;v2:=4; -- test comment
v3:=5;
raise notice '%,%,%', v1,v2,v3;
end;
/
-- 
-- check oralce compatible anonymous code block
-- 

create table oralce_pl_t1(f1 int not null,f2 varchar2(100),primary key (f1));
INSERT INTO oralce_pl_t1 VALUES (1, 'ONE');  
INSERT INTO oralce_pl_t1 VALUES (2, 'TWO');  
INSERT INTO oralce_pl_t1 VALUES (3, 'THREE');

declare
    v_f1 int;
    v_rec record;
begin
    insert into oralce_pl_t1 values(4,'four') returning f1 into v_f1;   
    raise notice 'v_f1 = %',v_f1;
    update oralce_pl_t1 set f2='ONE' WHERE f2='ONE' returning f1 into v_f1;  
    raise notice 'v_f1 = %',v_f1;  
    DELETE FROM oralce_pl_t1 WHERE f2='THREE' returning f1 into v_f1;  
    raise notice 'v_f1 = %',v_f1;
end;
/

select * from oralce_pl_t1 order by f1;

begin 
    update oralce_pl_t1 set f2='TWOOOOOOOOO' where f1=2;
    insert into oralce_pl_t1 values(5,'five');
    delete from oralce_pl_t1 where f1=4;
end;
/
select * from oralce_pl_t1 order by f1;
...
-- 
-- check oralce compatible function/procedure head and tail format
-- 
CREATE OR REPLACE function opentenbase_ora_func_1() returns void as 
    i int:= 0;
begin
    <<top>>
    LOOP
...
select opentenbase_ora_func_1();


CREATE OR REPLACE function opentenbase_ora_func_2(i integer) returns void as
begin
    <<top>>
    LOOP
        i = i + 1;  
...
select opentenbase_ora_func_2(2);
...
call opentenbase_ora_proc_1();



CREATE OR REPLACE procedure opentenbase_ora_proc_2(i integer) as
begin
    <<top>>
    LOOP
        i = i + 1;  
...
call opentenbase_ora_proc_2(4);

-- test create interval partition table in function/procedure
...
insert into employees values('d', 4, 'deptcurtyp', 1458.256);
...
-- test min/max in plpgsql function
create or replace function cib_get_datemark(v_mark_id character varying)
returns date
...
declare
 ret_date date;
begin
 select nvl(max(t.date_mark),trunc(sysdate -1))
 into ret_date
 from cib_datemark_tbl t 
 where t.mark_id = v_mark_id;
 return ret_date;
end CIB_GET_DATEMARK;
$function$
/

create table cib_datemark_tbl (
mark_id varchar(100),
date_mark date
) ;

insert into cib_datemark_tbl(
mark_id,
date_mark
)values ('VTM1','2021-01-01');

create table cib_rp_15_stat_tbl (
time               timestamp(0) without time zone,
object_id          character varying(27),
av_t_answered      numeric,
av_t_abandoned     numeric,
av_t_distributed   numeric,
max_t_abandoned    integer,
max_t_answered     integer,
pc_n_abandoved     numeric,
pc_n_answered      numeric,
service_factor     numeric,
n_abandoned        integer,
n_answered         integer,
n_distributed      integer,
n_distrib_in_tr    integer,
n_entered          integer,
n_abandoned_in_tr  integer,
t_abandoned        integer,
t_answered         integer,
t_distributed      integer,
pc_n_distrib       numeric
) distribute by shard(object_id);

insert into cib_rp_15_stat_tbl 
(
time               ,
object_id          ,
av_t_answered      ,
...
create table cib_cti_rp_tbl (
object_id          varchar(27),
confserver_id      varchar(3),
...
confserver_obj_id  numeric,
add_time           timestamp(0) without time zone,
delete_time        timestamp(0) without time zone,
add_time_gmt       timestamp(0) without time zone,
delete_time_gmt    timestamp(0) without time zone
)
distribute by shard(object_id);

insert into cib_cti_rp_tbl(
object_id          ,
confserver_id      ,
object_name        ,
object_type_id     ,
...
create or replace function convert_time(in_date timestamp without time zone)
...
begin
  case 
    when to_char(in_date,'MI') = '15'
    or to_char(in_date,'MI') = '45'
    then result := in_date - 1/ 96;
    else result := in_date;
    end case;
    return result;
end convert_time;
...
explain
select convert_time(a."time") as start_time,
  sum(a.n_entered) as n_entered,
  sum(a.n_answered) as n_answered,
  sum(a.t_answered) as t_answered,
  sum(a.n_abandoned) as n_abandoned,
  sum(a.t_abandoned) as t_abandoned,
  sum(a.n_distrib_in_tr) as n_distrib_in_tr,
  sum(a.n_abandoned_in_tr) as n_abandoned_in_tr
from cib_rp_15_stat_tbl a,cib_cti_rp_tbl b 
where a.object_id::text = b.object_id::text and b.object_name::text = '201121@FZ_Switch'::text and a."time" >= cib_get_datemark('VTM1'::character varying)::timestamp with time zone 
group by (convert_time(a."time"));


-- Fix anonymous block replacement comment area recognition
/*
 comment...
 */
create or replace procedure ref_pro2
as
begin
    null;
end;
...
-- 
--TestPoint:1.TAPD
create or replace function func_func1_20230423(var int) return varchar2 result_cache
is
begin
return 'dadada';
end;
...
select func_func1_20230423(1);
drop function func_func1_20230423;
--TestPoint:2.return void
create or replace function func_func1_20230423(var int) return void result_cache
is
begin
raise notice 'dadada';
end;
...
--TestPoint:3. return_cache in package function
create or replace package pkg_return_cache_20230423 is
function f() return varchar2 result_cache;
end;
/

create or replace package body pkg_return_cache_20230423 is
function f() return varchar2 result_cache
is
begin
return 'dadada';
...
select pkg_return_cache_20230423.f();
drop package pkg_return_cache_20230423;

--TestPoint:4. result_cache and authid
CREATE OR REPLACE FUNCTION fibonacci_20230511 (n NUMBER)
  RETURN NUMBER
  RESULT_CACHE
  AUTHID DEFINER
IS
BEGIN
  IF (n =0) OR (n =1) THEN
    RETURN 1;
  ELSE
    RETURN fibonacci_20230511(n - 1) + fibonacci_20230511(n - 2);
  END IF;
END;
/

select fibonacci_20230511(2);

drop function fibonacci_20230511;

-- 
-- Support sys_refcursor
-- 

--TestPoint:1. anonymous block
declare
a sys_refcursor;
begin
...
--TestPoint:2.function param using sys_refcursor
create or replace function f_test_refcursor_20230418(a sys_refcursor)
return integer
is
begin
open a for select * from dual;
...
declare
	...
	res int;
begin
  res := f_test_refcursor_20230418(a);
end;
/

drop function f_test_refcursor_20230418;

--TestPoint:3. function return type
create or replace function f_test_refcursor_20230418(a sys_refcursor)
...
declare
	...
	res int;
begin
  a := f_test_refcursor_20230418(a);
end;
/
drop function f_test_refcursor_20230418;

--TestPoint:4. procedure out param
create or replace procedure f_test_refcursor_20230418(a out sys_refcursor)
is
begin
open a for select * from dual;
close a;
...
declare
	...
	res int;
begin
 call f_test_refcursor_20230418(a);
end;
/

drop procedure f_test_refcursor_20230418;
-- 
-- Over
--
