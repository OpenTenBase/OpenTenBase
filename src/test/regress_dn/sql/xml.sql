   ...
   FROM (SELECT data FROM xmldata) x,
        LATERAL XMLTABLE('/ROWS/ROW'
                         ...
                         COLUMNS id int PATH '@id',
                                  ...
                                  size float PATH 'SIZE',
                                  unit text PATH 'SIZE/@unit',
                                  premier_name text PATH 'PREMIER_NAME' DEFAULT 'not specified') ORDER BY id;
...
SELECT xmltable.* FROM xmltest2, LATERAL xmltable('/d/r' PASSING x COLUMNS a int PATH '' || lower(_path) || 'c') order by 1;
SELECT xmltable.* FROM xmltest2, LATERAL xmltable(('/d/r/' || lower(_path) || 'c') PASSING x COLUMNS a int PATH '.') order by 1;
SELECT xmltable.* FROM xmltest2, LATERAL xmltable(('/d/r/' || lower(_path) || 'c') PASSING x COLUMNS a int PATH 'x' DEFAULT ascii(_path) - 54) order by 1;

-- test select xmltype null
\c regression_ora
drop table if exists test_XMLType;
create table test_XMLType(f1 int,f2 XMLType);
insert into test_XMLType values(1,'<ROW>XMLType2</ROW>');
select f1, extractvalue(x.f2,'/ROW/text()') as A from test_XMLType x order by f1;
insert into test_XMLType values(null,null);
select f1, extractvalue(x.f2,'/ROW/text()') as A from test_XMLType x order by f1;
drop table test_XMLType;

-- test xmltable without COLUMNS
select a.* from xmltable('/ROWS/ROW' passing xmltype('<ROWS><ROW><id>1</id></ROW><ROW><id>2</id></ROW></ROWS>')) a;
select COLUMN_VALUE from xmltable('/ROWS/ROW' passing xmltype('<ROWS><ROW id="1"><name>"JIM"</name></ROW><ROW id="2"><name>"TOM"</name></ROW></ROWS>')) a;
select column_value from xmltable('/ROWS/ROW/name' passing xmltype('<ROWS><ROW id="1" type="student"><name>"JIM"</name></ROW><ROW id="2" type="teacher"><name>"TOM"</name></ROW></ROWS>'));
CREATE TABLE r_xmltable_20220308(data xml);
INSERT INTO r_xmltable_20220308 VALUES('<ROWS>
    <ROW id="1">
        <COUNTRY_ID>AU</COUNTRY_ID>
        <COUNTRY_NAME>Australia</COUNTRY_NAME>
        <REGION_ID>3</REGION_ID>
...
select b.* from r_xmltable_20220308, xmltable('/ROWS/ROW' passing r_xmltable_20220308.data) as b;
select b.* from r_xmltable_20220308, xmltable('/ROWS/ROW/COUNTRY_NAME' passing r_xmltable_20220308.data) as b;
select b.* from r_xmltable_20220308, xmltable('/ROWS/ROW[COUNTRY_NAME="Japan" or COUNTRY_NAME="India"]' passing r_xmltable_20220308.data) as b;
select a.* from xmltable('/ROWS/ROW' passing xmltype('<ROWS><ROW><id>1</id></ROW><ROW><id>2</id></ROW></ROWS>')) a;
select COLUMN_VALUE from xmltable('/ROWS/ROW' passing xmltype('<ROWS><ROW id="1"><name>"JIM"</name></ROW><ROW id="2"><name>"TOM"</name></ROW></ROWS>')) a;
select b.* from r_xmltable_20220308, xmltable('/ROWS/ROW' passing r_xmltable_20220308.data) as b;
drop table r_xmltable_20220308;
