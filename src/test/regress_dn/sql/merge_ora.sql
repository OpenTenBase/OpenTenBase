...
WHEN MATCHED THEN
UPDATE SET t1.col7 = to_date(test_date_tb6.col2::text,'yyyymmdd'), t1.col3=cast(test_date_tb6.col4 as int) 
WHEN NOT MATCHED THEN
...
WHEN MATCHED THEN
UPDATE SET t1.col7 = to_date(test_date_tb5.col2::text,'yyyymmdd'), t1.col3=cast(test_date_tb5.col4 as int) 
WHEN NOT MATCHED THEN
...
insert into s_shard_20220815 select n, n + 200, 'f3 r'|| floor(n/5),'f4 r' || n, to_date('2022-08-10', 'YYYY-MM-DD') + n from generate_series(1,1) n;
insert into s_shard_20220815 select n, n + 200, 'f3 r'|| floor(n/5),'f4 r' || n, to_date('2022-08-10', 'YYYY-MM-DD') + n  from generate_series(2,2) n;
insert into s_shard_20220815 select n, n + 200, 'f3 r'|| floor(n/5),'f4 r' || n, to_date('2022-08-10', 'YYYY-MM-DD') + n  from generate_series(3,3) n;
select * from s_shard_20220815 order by f1, f2, f3, f4, f5;

insert into t_shard_20220815 select n, n+100, 'f3 r'|| floor(n/5),'f4 r' || n, to_date('2022-08-10', 'YYYY-MM-DD') + n from generate_series(1,1) n;
select * from t_shard_20220815 order by f1, f2, f3, f4, f5;

insert into t_shard_20220815 overriding user value select n, n+100, 'f3 r'|| floor(n/5),'f4 r' || n, to_date('2022-08-10', 'YYYY-MM-DD') + n from generate_series(2,2) n;
select * from t_shard_20220815 order by f1, f2, f3, f4, f5;

insert into t_shard_20220815_2 select n, n+100, 'f3 r'|| floor(n/5),'f4 r' || n, to_date('2022-08-10', 'YYYY-MM-DD') + n from generate_series(1,1) n;
select * from t_shard_20220815_2 order by f1, f2, f3, f4, f5;

insert into t_shard_20220815_2 (f1, f3, f4, f5) select n, 'f3 r'|| floor(n/5),'f4 r' || n, to_date('2022-08-10', 'YYYY-MM-DD') + n from generate_series(1,1) n;
select * from t_shard_20220815_2 order by f1, f2, f3, f4, f5;

insert into t_shard_20220815_2 overriding system value select n, n+100, 'f3 r'|| floor(n/5),'f4 r' || n, to_date('2022-08-10', 'YYYY-MM-DD') + n from generate_series(2,2) n;
select * from t_shard_20220815_2 order by f1, f2, f3, f4, f5;
...
create table tv_t (f1 int, f2 name, f3 name, f4 varchar2(40), f5 timestamp without time zone) distribute by shard(f1);
create table tv_t2 (f1 int, f2 name, f3 name, f4 varchar2(40), f5 timestamp without time zone) distribute by shard(f1);
create table tv_rp (f1 int, f2 name, f3 name, f4 varchar2(40), f5 timestamp without time zone) distribute by replication;
create table tv_s (f1 int, f2 name, f3 name, f4 varchar2(40), f5 timestamp without time zone) distribute by shard(f1);
create table tv_s2 (f1 int, f2 name, f3 name, f4 varchar2(20), f5 timestamp without time zone) distribute by shard(f1);
insert into tv_s select n, n, 'f3 r'|| floor(n/5),'f4 r' || n, to_date('2022-08-10', 'YYYY-MM-DD') + n from generate_series(1,10) n;
insert into tv_s2 select n, n - 4, 'f3 r'|| floor(n/5),'f4 r' || (n - 4), to_date('2022-08-10', 'YYYY-MM-DD') + n - 4 from generate_series(5,10) n;
...
-- ok
merge into (select f1 ff1, f1 + f2::numeric ff12, f2 ff2, substr(f3, 5) as ff3, f4 ff4, f5 ff5 from tv_t t where f2 = (select max(f2) from tv_s2 s where t.f4 = s.f4)) using tv_s s on ff12 = s.f1 * 2 when matched then update set ff5 = to_date('2022-10-10', 'YYYY-MM-DD') where ff1 % 2 = 0;
select * from tv_t order by 1,2,3,4,5;
...
explain (costs off) select f2 from t1 t where create_date >= pg_catalog.date'2023-04-01' and create_date < pg_catalog.date'2023-04-02' order by f2;
select f2 from t1 t where create_date >= pg_catalog.date'2023-04-01' and create_date < pg_catalog.date'2023-04-02' order by f2;

select distinct f2 from t1 t where create_date >= pg_catalog.date'2023-04-01' and create_date < pg_catalog.date'2023-04-02' order by f2;
...
merge into t1 using t2 on t1.f1 = t2.f1 when matched then update set t1.f2 = t2.f2 when not matched then insert(f1, f2, create_date) values (99, '999', '2023-03-20');
...
select * from merge_trg_rel_20230808;
select * from new_merge_trg_rel_20230808;
...
-- test merge into bfile
drop table if exists m_bfile;
drop table if exists s_bfile;
drop table if exists s_test;
create table m_bfile (i int, b bfile); --no locator
create table s_bfile (i int, b bfile); --no locator
create table s_text(i int, b text); --dist
insert into m_bfile values (1, 'tmp1');
insert into s_bfile values (1, 'tmpx'),(2, 'tmp2');
insert into s_text values(1, 'tmps'),(2, 'tmp2');

begin;
merge into m_bfile using s_bfile on (m_bfile.i=s_bfile.i) when matched then update set m_bfile.b=s_bfile.b when not matched then insert values (s_bfile.i, s_bfile.b);
explain (costs off, nodes off, summary off) merge into m_bfile using s_bfile on (m_bfile.i=s_bfile.i) when matched then update set m_bfile.b=s_bfile.b when not matched then insert values (s_bfile.i, s_bfile.b);select * from m_bfile;
rollback;

begin;
merge into m_bfile using s_text on (m_bfile.i=s_text.i) when matched then update set m_bfile.b=s_text.b when not matched then insert values (s_text.i, s_text.b);
explain (costs off, nodes off, summary off) merge into m_bfile using s_text on (m_bfile.i=s_text.i) when matched then update set m_bfile.b=s_text.b when not matched then insert values (s_text.i, s_text.b);
select * from m_bfile;
rollback;
