...
-- fix core in procedure
drop procedure if exists crf_pro;
drop table if exists t_dst;
drop table if exists t_src;
CREATE TABLE t_src
       (EMPNO NUMBER(4) CONSTRAINT pk_t_src PRIMARY KEY,
        ENAME VARCHAR2(10),
        JOB VARCHAR2(9),
        MGR NUMBER(4),
        HIREDATE DATE,
...
create or replace procedure crf_pro
as
DECLARE
        CURSOR
        crs_testAvg IS select EMPNO,ENAME,JOB,SAL,deptno,AVG(SAL) OVER (PARTITION BY deptno ) AS DEP_AVG FROM t_dst order by EMPNO;
        r_testAvg crs_testAvg%rowtype;
        salInfo t_dst.sal%type;
begin
        --perform dbms_output.serveroutput('t');
        for r_testAvg in crs_testAvg 
        loop
                if r_testAvg.SAL>=r_testAvg.DEP_AVG then
                        salInfo:=r_testAvg.SAL-50;
                end if;
                --perform dbms_output.put_line('EMPNO:'||r_testAvg.EMPNO||' '||'SAL:'||r_testAvg.SAL||' '||'avg:'||salInfo);
                update t_dst set SAL=salInfo where EMPNO=r_testAvg.EMPNO;
        end loop;
end;
/
call crf_pro();
drop procedure crf_pro;
drop table t_dst;
drop table t_src;
...
create or replace type create_type_as_table_of_date as table of date;
create table create_type_as_table_of(id int,v_date create_type_as_table_of_date);

insert into create_type_as_table_of(
select i,array[date(current_timestamp+random()*10* interval '1 day')] from generate_series(1,5) i);
...
select * from tb1@t_link_ora;
insert into tb1 @ss values(1);
update a @xxx set a=1 from tb1 @ t_sss;
select * from a @ xxx;
delete from a @xxx where age =1;
SELECT 'a:1 b:2'::tsvector @@ 'a <1> b'::tsquery AS "true";
SELECT '{"a":"b", "b":1, "c":null}'::jsonb @> '{"a":"b"}';
SELECT '{"a":"b", "b":1, "c":null}'::jsonb <@ '{"a":"b"}';
create or replace public synonym s_test for re_x @ dbk;
drop synonym s_test;
-- 1020421696875393133
/*
...
-- in/out
create or replace procedure bproc_88(a in out number) is
begin
  a = 12;
end;
/

create or replace function bfunc_88(a in out number) return int is
begin
  a = 12.23;
  return 12;
end;
...
/
drop function bfunc_88;
drop procedure bproc_88;
...
--
-- Fixed the issue of ambiguous error when VAR(i) does not exist.
--
...
--TestPoint:2.unreserved key word
begin
bulk(1):=100;
end;
/
...
--TestPoint:3.pkg.var
create or replace package pkg_pkg1_20240603
is
var int;
end;
/
...
begin
pkg_pkg1_20240603.var1(1):=100;
end;
/

--TestPoint:4.schema.pkg.var
begin
public.pkg_pkg1_20240603.var1(1):=100;
end;
/

drop package pkg_pkg1_20240603;
