--Distribution column definition can be changed, however values should be within limits of the changed data type. Distribution column can be dropped in centralized mode. This works for HASH and not for MODULO now (bug). So failing test for now. 
...
ALTER TABLE xl_at2h DROP COLUMN product_identifier; --distribution column be dropped.
...
ALTER TABLE xl_at2m DROP COLUMN product_identifier; --distribution column be dropped in centralized mode.
