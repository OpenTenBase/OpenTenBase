...
-- fix transition table with fk core
drop table if exists test_runner cascade;
drop table if exists user_test cascade;
drop table if exists loose_test cascade;
create table test_runner(
id serial primary key,
runnername varchar)
distribute by shard(id);
insert into test_runner(id,runnername)values(1001,'runner1001'),(1002,'runner1002'),(1003,'runner1003'),(1004,'runner1004'),(1005,'runner1005'),(1006,'runner1006');

create table user_test(
id serial primary key,
name varchar,
runner_id int REFERENCES test_runner(id) ON DELETE CASCADE
)
distribute by shard(id);

insert into user_test(name,runner_id)values('a0001',1001),('a0002',1002),('a0003',1003),('a0004',1004),('a0005',1005);

create table loose_test(
id serial primary key,
fully_qualified_table_name varchar,
primary_key_value int not null)
distribute by shard(id);

CREATE or replace FUNCTION testfuc() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
    DECLARE
    myvara varchar :='chushizhi';
    myvarb varchar :='chushizhi';
BEGIN
  select TG_TABLE_SCHEMA || '.' || TG_TABLE_NAME  into myvara FROM old_table;
  raise notice 'this is table name ----->%',myvara;
  select COALESCE(old_table.id::varchar,'this is null_valu'::varchar) into myvarb FROM old_table;
  raise notice 'this is tables id ----->%',myvarb;
  RETURN NULL;
END
$$;

CREATE TRIGGER user_test_loose_fk_trigger AFTER DELETE ON user_test REFERENCING OLD TABLE AS old_table FOR EACH STATEMENT EXECUTE FUNCTION testfuc();
delete from test_runner where id=1001;
delete from test_runner where id=1002;
drop table test_runner cascade;
drop table user_test cascade;
drop table loose_test cascade;
reset enable_datanode_row_triggers;
