...
BEGIN
	FOR r IN SELECT colname, oper, typ, value[ordinality], matches[ordinality] FROM brinopers, unnest(op) WITH ORDINALITY AS oper LOOP
		...
		num := array_length(idx_ctids, 1);

		IF NOT (num = array_length(ss_ctids, 1) AND
				idx_ctids @> ss_ctids AND
				idx_ctids <@ ss_ctids) THEN
			-- report the results of each scan to make the differences obvious
			RAISE WARNING 'something not right in %: num %', r, num;
			SET enable_seqscan = 1;
			SET enable_bitmapscan = 0;
		...
		IF num != r.matches THEN RAISE WARNING 'unexpected number of results % for %', num, r; END IF;
	END LOOP;
END;
$x$;
