...
{
	const char *data_folder = dn_folder;
	FILE	   *pg_conf;
	...
	fputs("pool_session_max_lifetime = 10\n", pg_conf);
...
/*
 * Setup connection information for Datanode running regression
 */
static void
setup_connection_information(void)
{
	...
	psql_command("postgres", "ALTER NODE %s WITH (HOST = 'localhost',"
				" type = 'datanode', PORT = %d, PRIMARY, PREFERRED);",
				"datanode_1", port);

	psql_command("postgres", "create DEFAULT node group default_group with (datanode_1);");
	psql_command("postgres", "create sharding group to group default_group;");
	psql_command("postgres", "clean sharding;");
#endif
}
...
regression_main(int argc, char *argv[], init_function ifunc, test_function tfunc)
{
	...
	if (temp_instance)
	{
        ...
        snprintf(buf, sizeof(buf),
				 "\"%s%sinitdb\" -D \"%s/%s\" --no-clean --no-sync%s%s --nodename %s --nodetype %s "
		...
		fputs("allow_dml_on_datanode = on\n", pg_conf);
		fputs("is_centralized_mode = on\n", pg_conf);
...
#error Platform has no means to secure the test installation.
        ...
        snprintf(buf, sizeof(buf),
				 "\"%s%spostgres\" --datanode -i -D \"%s/%s\" %s "
				 "-k \"%s\" "
				 "> \"%s/log/postmaster.log\" 2>&1",
				 bindir ? bindir : "",
				 bindir ? "/" : "",
				 temp_instance, dn_folder, debug ? " -d 5" : "",
				 sockdir ? sockdir : "",
				 outputdir);
