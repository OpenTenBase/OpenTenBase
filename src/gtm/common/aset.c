	...
	if (size > set->allocChunkLimit)
	{
		...
		if (block == NULL)
		{
			...
			ereport(ERROR,
					(ENOMEM,
					 errmsg("out of memory"),
					 errdetail("Failed on request of size %lu in file %s line %d.",
...
AllocSetRealloc(MemoryContext context, void *pointer, Size size, const char* file, int line)
{
	...
	if (chunk->requested_size < oldsize)
		if (((char *) pointer)[chunk->requested_size] != 0x7E)
			elog(WARNING, "detected write past chunk end in %s %p in file %s line %d",
				 set->header.name, chunk, file, line);
#endif
	...
	if (oldsize > set->allocChunkLimit)
	{
		...
		if (block == NULL)
		{
			if (MemoryContextIsShared(context))
				MemoryContextUnlock(context);
			elog(ERROR, "could not find block containing chunk %p in file %s line %d", chunk, file, line);
		}
		/* let's just make sure chunk is the only one in the block */
		Assert(block->freeptr == ((char *) block) +
			   (chunk->size + ALLOC_BLOCKHDRSZ + ALLOC_CHUNKHDRSZ));
