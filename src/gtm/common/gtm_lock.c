...
GTM_RWLockAcquire(GTM_RWLock *lock, GTM_LockMode mode)
{
	...
	int i = 0;
	if(my_threadinfo && my_threadinfo->write_locks_hold &&
	        my_threadinfo->xlog_inserting && lock->lock_flag & GTM_RWLOCK_FLAG_STORE)
	{
	...
	if (my_threadinfo && my_threadinfo->locks_hold && my_threadinfo->max_lock_number != -1)
	{
...
#ifdef __XLOG__
		if(my_threadinfo->write_locks_hold && mode == GTM_LOCKMODE_WRITE && 
				my_threadinfo->xlog_inserting && lock->lock_flag&GTM_RWLOCK_FLAG_STORE)
		{
