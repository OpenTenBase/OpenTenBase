					...
					const char *name)
{
	...
	else
	{
		...
		node = (MemoryContext) pg_malloc(needed);
		Assert(node != NULL);
	}
...

{
	...
	if (!AllocSizeIsValid(size))
		elog(ERROR, "invalid memory alloc request size %lu in file %s line %d",
			 (unsigned long) size, file, line);

	return (*context->methods->alloc) (context, size, file, line);
}
