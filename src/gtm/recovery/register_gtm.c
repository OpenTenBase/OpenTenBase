...
ProcessPGXCNodeRegister(Port *myport, StringInfo message, bool is_backup)
{
	...
	if (type == GTM_NODE_GTM)
	{
		...
		if (ret)
		{
			elog(LOG, "Standby GTM %s is not exist, no need unregister.", node_name);
		}
		/*
...
ProcessGTMBeginBackup(Port *myport, StringInfo message)
{
	...
	else if (remote_sysid && remote_syslsn && remote_sysgts)
	{
		...
		else if ((remote_syslsn > local_syslsn) || (remote_sysgts > local_sysgts))
		{
			elog(WARNING, "remote_syslsn or remote_sysgts is larger than the local, local lsn:%ld local gts:%ld, remote lsn:%ld, remote gts:%ld",
				 local_syslsn, local_sysgts, remote_syslsn, remote_sysgts);
		}
	}
