...
#define GTM_DEFAULT_PORT		6666
	...
	snprintf(connstr, sizeof(connstr),
			 "host=localhost port=%s remote_type=%d connect_timeout=5 node_name=one", portstr, GTM_NODE_GTM_CTL);
...
do_reconnect(void)
{
	 ...
	 */
	reconnect_point_file_nam = pg_malloc(strlen(gtm_data) + 9);
...
do_stat(void)
{
    ...
    if (!ret)
    {
        ...
        strftime(time_buff, sizeof(time_buff),
                 "%Y-%m-%d %H:%M:%S",
                 localtime_r((time_t *)&result->start_time, &timeinfo));
        printf(_("statistics start time: %s\n"), time_buff);

        strftime(time_buff, sizeof(time_buff),
                 "%Y-%m-%d %H:%M:%S",
                 localtime_r((time_t *)&result->end_time, &timeinfo));
...
do_errlog(void)
{
    ...
    if (!ret)
    {
        ...
        while (len)
        {
            ...
            err_info->proc_id = pg_ntoh32(err_info->proc_id);
            err_info->error_no = pg_ntoh32(err_info->error_no);
            err_info->log_time = be64toh(err_info->log_time);
            err_info->err_level = pg_ntoh32(err_info->err_level);
            err_info->errmsg_len = pg_ntoh32(err_info->errmsg_len);

            strftime(time_buff, sizeof(time_buff),
                        "%Y-%m-%d %H:%M:%S",
                     localtime_r((time_t *)&err_info->log_time, &timeinfo));
...
{
    /* At present, for gtm, the processing of these signals is the same */
...
main(int argc, char **argv)
{
		...
		ctl_command == RESTART_COMMAND)
	{
		...
		if (strcmp(gtm_app, "gtm_proxy") == 0)
		{
			...
			if (nodename)
			{
				strcat(gtmdata_opt, " -i ");
				strcat(gtmdata_opt, nodename);
			}
			else
				strcat(gtmdata_opt, " ");
		}
