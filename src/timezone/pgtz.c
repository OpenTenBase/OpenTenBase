 ...
 */
pg_tz *
pg_tzset(const char *name, bool ora_strick_check)
{
	pg_tz_cache *tzp;
	...
	*p = '\0';
	...
	{
		if (!tzparse(uppername, &tzstate, true, ora_strick_check))
		{
	...
	{
		if (uppername[0] == ':' || !tzparse(uppername, &tzstate, false, ora_strick_check))
		{
...
pg_timezone_initialize(void)
{
	 ...
	 */
	session_timezone = pg_tzset("GMT", false);
