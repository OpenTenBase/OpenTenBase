		   ...
		   union local_storage *lsp)
{
		...
		sp->typecnt + 2 <= TZ_MAX_TYPES)
	{
		...
		up->buf[nread - 1] = '\0';
		if (tzparse(&up->buf[1], ts, false, false)
			&& ts->typecnt == 2)
		{
			/*
 ...
 */
bool
tzparse(const char *name, struct state *sp, bool lastditch, bool ora_strict_check)
{
	const char *stdname;
...
struct pg_tm *
pg_localtime_r(const pg_time_t *timep, const pg_tz *tz, struct pg_tm *pg_tm)
{
	return localsub(&tz->state, timep, pg_tm);
}
