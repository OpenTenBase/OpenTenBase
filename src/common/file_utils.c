...
{
	const static char zbuffer_raw[XLOG_BLCKSZ + MAXIMUM_ALIGNOF] = {0};
	char *zbuffer = (char *) MAXALIGN(zbuffer_raw);
	...
	while (remaining_size > 0)
	{
		...
		for (; iovcnt < PG_IOV_MAX && remaining_size > 0; iovcnt++)
		{
			...
			iov[iovcnt].iov_base = zbuffer;

			if (remaining_size < XLOG_BLCKSZ)
				this_iov_size = remaining_size;
			else
				this_iov_size = XLOG_BLCKSZ;
