...
select am, round(sum(weight), 4) = 1.0000 as ok_sum,
       bool_and(source = 'measured') as ok_measured,
       max(dispersion) as max_dispersion
from vecdiag.stage_weight group by am order by am;

\echo '== 2. 每条采样序列的加权进度单调非递减 =='
with runs as (
...
c as (
  ...
  from runs r, lateral vecdiag.progress_curve(r.run_id, r.am) pc
)
