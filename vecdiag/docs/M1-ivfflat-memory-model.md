...
## 补充（2026-08-27，T4.3 保守方向专项测试发现）：报错里的 MB 不是你该设的内存

做 T4.3 时把 `maintenance_work_mem` 设成报错消息里的数字再建一次，**照样报错**。原因清楚：

`predicted_mb` 等于报错消息里的数字，而报错来自**最先越界的那个检查点**。把内存抬到这个值，
只是让第一个检查点放行，接着就撞上更大的 C2 或 C3。三个检查点的量级差得很远：

```sql
select min_mwm_kb, min_mwm_mb, binding, c1_kb, c2_kb, c3_kb
  from vecdiag.ivfflat_min_mwm_kb(100000, 128, 500);
--  63459 | 62 | C3 | 253 | 12949 | 63459
```

C1 只要 253 kB，C3 要 63 459 kB —— 差 250 倍。所以两个问题必须分开回答：

| 问题 | 用哪个 |
|---|---|
| "报错会写多少 MB" | `vecdiag.ivfflat_predict().predicted_mb`（用于核对模型与实际是否一致） |
| "我该把内存设多少" | `vecdiag.ivfflat_min_mwm_kb()`（三个检查点的最大值） |

触发条件是 `totalSize / 1024 > maintenance_work_mem`（整数除法，`ivfutils.c:124`），
所以可行下界是 `max_i floor(C_i / 1024)`，取等号即可通过。

**边界精度实测（run `t44-20260827`，B* 组）**：三个配置分别在 `need` kB 处建成、
`need − 1` kB 处报错，**边界精确到 1 kB**，两个方向都对。这也是这条模型"不偏乐观"的直接证据：
异常矩阵 18 个用例里 `DANGEROUS`（模型说能建、实际建不起来）为 **0**。
