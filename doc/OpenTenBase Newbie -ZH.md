...
CN 节点不持久化任何用户数据，因此增加 CN 节点无需数据迁移，可直接提升集群并发处理能力。

### 5. 主数据节点（DN-主）

分片组内负责接收读写请求的主节点，是数据服务的核心。

### 6. 备数据节点（DN-备）

通过流复制实时同步主节点数据，在主节点故障时接管服务的备份节点。
...
GTM 代理是部署在 GTM 主节点前的“专属秘书”。它负责两件事：

**转发请求**：把 CN 的事务 ID 请求转发给 GTM 主节点。

**健康监测与切换**：持续监控 GTM 主节点是否存活，一旦主节点故障，自动将请求切到备节点。
...
<img title="" src="file:///C:/Users/victor/Pictures/2026-07-08-14-10-24-opentenbase架构图.png" alt="" width="764">
