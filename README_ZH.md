...
一个标准的 OpenTenBase 分布式集群由 **CoordinatorNode (CN，协调节点)**、**DataNode (DN，数据节点)** 和 **GTM (全局事务管理节点)** 组成。所有用户业务数据经哈希切分存储在底层各个 DataNode 中，CoordinatorNode 负责全网元数据、连接解析与分布式查询路由，GTM 负责全局单调递增时间戳 (GTS) 与全局快照分配。

### 1. 核心角色与查询流转关系

| 角色 / 组件 | 核心职能说明 | 与用户 / 其它角色的协作关系 |
| :--- | :--- | :--- |
| **客户端 (Client)** | 业务应用与 SQL 客户端 | **始终只直接连接 CoordinatorNode (CN)**，无需感知底层真实节点分布。 |
| **协调节点 (CN)** | SQL 统一接入与执行计划大脑 | 接收并解析用户 SQL；向 GTM 申请全局事务时间戳与快照；生成分布式执行计划并将算子下推给具体 DN；合并排序后返回给用户。 |
| **数据节点 (DN)** | 真正的数据分片存储与计算单元 | 保存用户的表分片数据；根据 CN 下发的分片计划进行本地过滤、局部聚合；参与 CN 发起的 2PC 两阶段提交。 |
| **全局事务管理 (GTM)** | 分布式时钟与事务快照中心 | 发放全局唯一事务序列号与时间戳（GTS），保证多 DN 上分布式读写的强一致与 MVCC 隔离。 |

```mermaid
graph LR
    User["应用层 / SQL 客户端"] ==>|直连 SQL 请求| CN["协调节点 Coordinator (CN)"]
    CN <==>|申请全局快照/GTS| GTM["全局事务控制 (GTM)"]
    CN ==>|查询下推/2PC 提交| DN1[("数据节点 DataNode (DN 1)")]
    CN ==>|查询下推/2PC 提交| DN2[("数据节点 DataNode (DN 2)")]
```

> [!TIP]
> **新手必读完整指南**：关于 Coordinator、DataNode、GTM、分布式模式、集中式模式、Node Group 以及 `opentenbase_ctl` 工具的详细解释、工作机制与新手进阶 FAQ，请参阅：  
> **👉 [OpenTenBase 核心术语表与新手架构导览 (doc/GLOSSARY_ZH.md)](doc/GLOSSARY_ZH.md)**
