			...
			else if (action == 'c')
			{
				if (is_update == 1)
				{
					...
					for (k = 1; k <= nkeys; k++)
					{
						...
						if ((strcmp(type, (ORA_MODE) ? "TEXT" : "text") &&
							strcmp(type, (ORA_MODE) ? "VARCHAR" : "varchar") &&
							 strcmp(type, (ORA_MODE) ? "CHAR" : "char") &&
							 strcmp(type, (ORA_MODE) ? "BPCHAR" : "bpchar") &&
							 strcmp(type, (ORA_MODE) ? "DATE" : "date") &&
							 strcmp(type, (ORA_MODE) ? "TIMESTAMP" : "timestamp")) == 0)
							is_char_type = 1;
#ifdef	DEBUG_QUERY
