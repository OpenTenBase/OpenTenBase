...
-- IVFFlat adaptive parameter recommendation functions

CREATE FUNCTION ivfflat_recommend_params(
	rows bigint,
	dim integer,
	...
	OUT lists integer,
	OUT probes integer,
	OUT adaptive_scan boolean,
	OUT target_recall float8,
	OUT min_probes integer,
	OUT adaptive_threshold float8,
	OUT sql_index_clause text,
	OUT sql_set_commands text
)
AS 'MODULE_PATHNAME', 'ivfflat_recommend_params'
LANGUAGE C VOLATILE CALLED ON NULL INPUT;

COMMENT ON FUNCTION ivfflat_recommend_params(bigint, integer, text, float8, boolean) IS 'recommend Pareto-optimal IVFFlat index and runtime parameters, and optionally apply to current session';

CREATE FUNCTION ivfflat_auto_tune(
	target_recall float8 DEFAULT 0.95,
	probes integer DEFAULT 10
) RETURNS text
AS 'MODULE_PATHNAME', 'ivfflat_auto_tune'
LANGUAGE C VOLATILE STRICT;

COMMENT ON FUNCTION ivfflat_auto_tune(float8, integer) IS 'automatically apply adaptive scan GUC configuration to session';

CREATE FUNCTION ivfflat_recommend_table(
	table_name text,
	column_name text,
	metric text DEFAULT 'l2',
	target_recall float8 DEFAULT 0.95,
	...
	lists integer,
	probes integer,
	adaptive_scan boolean,
	target_recall float8,
	min_probes integer,
	adaptive_threshold float8,
	sql_index_clause text,
	sql_set_commands text
) AS $$
DECLARE
	row_count bigint;
	col_dim integer;
BEGIN
	...
	EXECUTE format('SELECT vector_dims(%I) FROM %I WHERE %I IS NOT NULL LIMIT 1', column_name, table_name, column_name) INTO col_dim;
	IF col_dim IS NULL THEN
		col_dim := 128;
	END IF;
	RETURN QUERY SELECT * FROM ivfflat_recommend_params(row_count, col_dim, metric, target_recall, auto_apply);
END;
