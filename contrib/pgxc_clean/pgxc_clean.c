...
getDatabaseList(PGconn *conn)
{
	...
	for(ii = 0; ii < database_count; ii++)
	{
		...
		if (strcmp(dbname, "template0") == 0 || strcmp(dbname, "template0_ora") == 0)
...
parse_pgxc_clean_options(int argc, char *argv[])
{
	...
	while (argc - optind >= 1)
	{
		if (head_database_names == NULL)
		{
			if (strcmp(argv[optind], "template0") == 0 || strcmp(argv[optind], "template0_ora") == 0)
			{
				fprintf(stderr, "%s: You should not clean template0 or template0_ora database.\n",
						progname);
				exit(1);
			}
