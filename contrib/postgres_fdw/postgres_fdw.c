...
bool pgfdw_pushdown_non_volatile_func;
int pgfdw_join_pushdown;
double pgfdw_join_startup_cost;
double pgfdw_join_total_cost;
double pgfdw_join_rows;
/*
...
extern bool is_load_postgres_fdw_guc;
void _PG_init()
{
	if (!is_load_postgres_fdw_guc)
	{
		DefineCustomBoolVariable("pgfdw_pushdown_non_volatile_func",
		                         gettext_noop(
			                           "pushdown postgres fdw none valatile function"),
		                         NULL,
		                         &pgfdw_pushdown_non_volatile_func,
		                         false,
		                         PGC_USERSET, 0,
		                         NULL, NULL, NULL);
		DefineCustomIntVariable("pgfdw_join_pushdown",
		                        gettext_noop(
			                          "pushdown postgres fdw simple relation count limit"),
		                        NULL,
		                        &pgfdw_join_pushdown,
		                        8, 1, INT32_MAX,
		                        PGC_USERSET, 0,
		                        NULL, NULL, NULL);
		DefineCustomRealVariable("pgfdw_join_startup_cost",
		                        gettext_noop(
			                          "pushdown postgres fdw startup cost"),
		                        NULL,
		                        &pgfdw_join_startup_cost,
		                        -1.0, -1.0, DBL_MAX,
		                        PGC_USERSET, 0,
		                        NULL, NULL, NULL);
		DefineCustomRealVariable("pgfdw_join_total_cost",
		                        gettext_noop(
			                          "pushdown postgres fdw total cost"),
		                        NULL,
		                        &pgfdw_join_total_cost,
		                        -1.0, -1.0, DBL_MAX,
		                        PGC_USERSET, 0,
		                        NULL, NULL, NULL);
		DefineCustomRealVariable("pgfdw_join_rows",
		                        gettext_noop(
			                          "pushdown postgres fdw join estimate rows"),
		                        NULL,
		                        &pgfdw_join_rows,
		                        1.0, 0.0, DBL_MAX,
		                        PGC_USERSET, 0,
		                        NULL, NULL, NULL);
		is_load_postgres_fdw_guc = true;
	}
}
/*
...
#ifdef __FDW__
static bool postgresIsForeignRelTransCompitable(void);
#endif
...
#ifdef __FDW__
    routine->IsForeignRelTransCompitable = postgresIsForeignRelTransCompitable;
#endif
...
postgresIterateForeignScan(ForeignScanState *node)
{
	...
	if (ps->instrument && ps->instrument->need_timer)
	{
		if (INSTR_TIME_IS_ZERO(node->f_start_time))
			INSTR_TIME_SET_CURRENT(node->f_start_time);
	}
	/*
	...
	{
		if (INSTR_TIME_IS_ZERO(node->f_start_time))
			elog(ERROR, "Foreign scan called without start");
		INSTR_TIME_SET_CURRENT(endtime);
		INSTR_TIME_ACCUM_DIFF(node->f_counter, endtime, node->f_start_time);
		INSTR_TIME_SET_ZERO(node->f_start_time);
	}
	/*
	...
	PG_TRY();
	{
		...
		for (i = 0; i < numrows;)
		{
			...
			appendStringInfo(&buf, "CREATE FOREIGN TABLE %s.%s (\n",
						stmt->local_schema, quote_identifier(tablename));
							...
							JoinPathExtraData *extra)
{
	...
	if ((bms_num_members(joinrel->relids) > pgfdw_join_pushdown) || !foreign_join_ok(root, joinrel, jointype, outerrel, innerrel, extra))
	{
	...
	if (pgfdw_join_startup_cost > -1.0)
		startup_cost = pgfdw_join_startup_cost;
	if (pgfdw_join_total_cost > -1.0)
		total_cost = pgfdw_join_total_cost;
	if (pgfdw_join_rows > 0.0)
		rows = pgfdw_join_rows;
	fpinfo->rows = rows;
	fpinfo->width = width;
...
#ifdef __FDW__
	if (fsstate != NULL && (fsstate->ss.ps.state->es_result_relations != NULL) 
	    && (is_shard_table = RelationIsSharded(fsstate->ss.ps.state->es_result_relations->ri_RelationDesc)))
	{
		discolnums = RelationGetDisKeys(fsstate->ss.ps.state->es_result_relations->ri_RelationDesc);
		ndiscols = RelationGetNumDisKeys(fsstate->ss.ps.state->es_result_relations->ri_RelationDesc);
	    tuple = heap_form_tuple_plain(tupdesc, values, nulls, discolnums, ndiscols,
									  fsstate->ss.ps.state->es_result_relations->ri_RelationDesc->rd_locator_info->relid);
	}
	else
	{
