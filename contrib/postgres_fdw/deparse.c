...
{
	foreign_glob_cxt glob_cxt;
	foreign_loc_cxt loc_cxt = {.collation=InvalidOid,.state=FDW_COLLATE_NONE};
	...
	if (loc_cxt.state == FDW_COLLATE_UNSAFE)
		return false;
	if (pgfdw_pushdown_non_volatile_func && !contain_volatile_functions((Node *) expr))
		return true;
	/*
		...
		case T_FuncExpr:
			{
				 ...
				 */
				if (fe->inputcollid == InvalidOid || fe->inputcollid == DEFAULT_COLLATION_OID)
