...
heap_page_items(PG_FUNCTION_ARGS)
{
	...
	if (fctx->call_cntr < fctx->max_calls)
	{
			...
			lp_offset + lp_len <= raw_page_size)
		{
			...
			values[8] = UInt32GetDatum(tuphdr->t_infomask3);
			values[9] = UInt32GetDatum(tuphdr->t_infomask2);
			values[10] = UInt32GetDatum(tuphdr->t_infomask);
#ifdef _MIGRATE_
			values[11] = Int32GetDatum(tuphdr->t_shardid);
				...
				tuphdr->t_hoff == MAXALIGN(tuphdr->t_hoff))
			{
				...
				if (tuphdr->t_infomask & HEAP_HASOID)
					values[14] = HeapTupleHeaderGetOid(tuphdr);
				else
					nulls[14] = true;
			}
			else
