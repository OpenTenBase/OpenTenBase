...
/*
 * Remove multi cluster 
 */
static void emptyMultiCluster(void)
{
	reset_var_val(VAR_multiCluster, "n");
	reset_var(VAR_coordMasterCluster);
	reset_var(VAR_coordSlaveCluster);
	reset_var(VAR_datanodeMasterCluster);
	reset_var(VAR_datanodeSlaveCluster);
}

static void checkMultiCluster(void)
{
	...
	if (strcasecmp(opts[0], sval(VAR_pgxcMainClusterName)) != 0)
	{
		elog(ERROR, "ERROR: The only valid value of pgxcMainClusterName is '%s', But here got an invalid input '%s'",
					opts[0], sval(VAR_pgxcMainClusterName));
		return;
	}
	
	for (ii = 0; aval(VAR_coordMasterCluster)[ii]; ii++)
	{
		if (strcasecmp(aval(VAR_coordMasterCluster)[ii], sval(VAR_pgxcMainClusterName)) != 0)
		{
			elog(ERROR, "ERROR: The only valid value in coordMasterCluster is '%s', But here got an invalid input '%s'",
						sval(VAR_pgxcMainClusterName), aval(VAR_coordMasterCluster)[ii]);
			return;
		}
	}

	for (ii = 0; aval(VAR_coordSlaveCluster)[ii]; ii++)
	{		
		int jj = 1;
		for (jj = 1; jj < sizeof(opts)/sizeof(opts[0]); jj++)
		{
			if (strcasecmp(aval(VAR_coordSlaveCluster)[ii], opts[jj]) == 0)
			{
				break;
			}
		}

		if (jj == sizeof(opts)/sizeof(opts[0]))
		{
			elog(ERROR, "ERROR: Found invalid value '%s' in coordSlaveCluster, "
						"valid values are \"opentenbase_cluster_01/opentenbase_cluster_02/.../opentenbase_cluster_14/opentenbase_cluster_15\"",
						aval(VAR_coordSlaveCluster)[ii]);
			return;
		}
	}

	for (ii = 0; aval(VAR_datanodeMasterCluster)[ii]; ii++)
	{
		if (strcasecmp(aval(VAR_datanodeMasterCluster)[ii], sval(VAR_pgxcMainClusterName)) != 0)
		{
			elog(ERROR, "ERROR: The only valid value in datanodeMasterCluster is '%s', But here got an invalid input '%s'",
						sval(VAR_pgxcMainClusterName), aval(VAR_datanodeMasterCluster)[ii]);
		}
	}

	for (ii = 0; aval(VAR_datanodeSlaveCluster)[ii]; ii++)
	{
		int jj = 1;
		for (jj = 1; jj < sizeof(opts)/sizeof(opts[0]); jj++)
		{
			if (strcasecmp(aval(VAR_datanodeSlaveCluster)[ii], opts[jj]) == 0)
			{
				break;
			}
		}
		...
		if (jj == sizeof(opts)/sizeof(opts[0]))
		{
			elog(ERROR, "ERROR: Found invalid value '%s' in datanodeSlaveCluster, "
						"valid values are \"opentenbase_cluster_01/opentenbase_cluster_02/.../opentenbase_cluster_14/opentenbase_cluster_15\"",
						aval(VAR_datanodeSlaveCluster)[ii]);
			return;
		}
	}
}
