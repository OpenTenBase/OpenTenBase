...
/* Extend four digits to AES key length */
static void expand_key_from_number();

/* Perform hexadecimal encoding */
static void base64_encode(unsigned char *in, int in_len, char *out);

/* Convert string to int32-bit integer, if failed, return 0  */
static int32 convert_string_to_int32(char *string);
...
cmd_t *prepare_initDatanodeMaster(char *nodeName)
{
	...
	fprintf(f,
			...
			aval(VAR_datanodeFwdServerPorts)[idx]);
...
cmd_t *prepare_initDatanodeSlave(char *nodeName)
{
	...
	fprintf(f,
			...
			aval(VAR_datanodeSlaveFwdServerPorts)[idx]);
...
#    define checkRc() do{if(WEXITSTATUS(rc_local) > rc) rc = WEXITSTATUS(rc_local);}while(0)
	...
	rc_local = doImmediate(aval(VAR_datanodeSlaveServers)[datanodeIdx], NULL,
						   "%spg_ctl restart -w -Z datanode -D %s -o -i; sleep 1",
						   pwdParam, aval(VAR_datanodeSlaveDirs)[datanodeIdx]);
	checkRc();
	/*
	...
	var_assign(&(aval(VAR_datanodeFwdServerPorts)[datanodeIdx]), Strdup(aval(VAR_datanodeSlaveFwdServerPorts)[datanodeIdx]));
	...
	var_assign(&(aval(VAR_datanodeSlaveFwdServerPorts)[datanodeIdx]), Strdup("none"));
	/*
	...
	fprintf(f,
			...
			listValue(VAR_datanodeFwdServerPorts),
			...
			listValue(VAR_datanodeSlaveFwdServerPorts),
...
static void
expand_key_from_number()
{
	int i;
    /* num is a four digit number, we will copy it to the beginning of the key array */
    snprintf((char *)key, AES_BLOCK_SIZE, "%04d", num_key);
...
static void
base64_encode(unsigned char *input, int in_len, char *output)
{
	int i;
	for (i = 0; i < in_len; i++) {
...
static int32
convert_string_to_int32(char *string)
{
	int32_t result;
	// used to store the end position of the converted string
	char *endptr;
 ...
 *
 *-----------------------------------------------------------------------*/
int add_datanodeMaster(char *name, char *host, int port, int pooler, int fwdserver, char *dir,
		char *waldir, char *extraConf, char *extraPgHbaConf)
{
	...
	if ((extendVar(VAR_datanodeNames, idx + 1, "none") != 0) ||
		...
		(extendVar(VAR_datanodeFwdServerPorts, idx + 1, "none")  != 0) ||
	...
	if (isVarYes(VAR_datanodeSlave))
	{
		if ((extendVar(VAR_datanodeSlaveServers, idx + 1, "none")  != 0) ||
			...
			(extendVar(VAR_datanodeSlaveFwdServerPorts, idx + 1, "none")  != 0) ||
	...
	snprintf(fwdserver_s, MAXTOKEN, "%d", fwdserver);
	...
	assign_arrayEl(VAR_datanodeFwdServerPorts, idx, fwdserver_s, "-1");
 	...
 	if (isVarYes(VAR_datanodeSlave))
	{
		...
		assign_arrayEl(VAR_datanodeSlaveFwdServerPorts, idx, "-1", NULL);
 	...
 	if (isVarYes(VAR_datanodeSlave))
	{
		...
		fprintAval(f, VAR_datanodeSlaveFwdServerPorts);
...
int add_datanodeSlave(char *name, char *host, int port, int pooler,  int fwdserver, char *dir,
		char *walDir, char *archDir)
{
	...
	replace_arrayEl(VAR_datanodeSlaveFwdServerPorts, idx, fwdserver_s, NULL);
	...
	/* pg_basebackup */
	doImmediate(host, NULL, "%spg_basebackup -U %s -p %s -h %s -D %s --wal-method=stream %s %s",
                pwdParam, aval(VAR_pgxcOwner)[0],
				aval(VAR_datanodePorts)[idx],
	...
	fprintf(f,
            ...
            "max_wal_senders = 0\n"        /* Minimum WAL senders */
			"# End of Addition\n",
			timeStampString(date, MAXTOKEN), aval(VAR_datanodeSlavePorts)[idx], aval(VAR_datanodeSlavePoolerPorts)[idx], aval(VAR_datanodeFwdServerPorts)[idx]);
	pclose(f);
	...
	pclose(f);
	/* Start the slave */
	doImmediate(host, NULL, "%spg_ctl start -w -Z datanode -D %s", pwdParam, dir);
	return 0;
}
...
cmd_t *prepare_cleanDatanodeMaster(char *nodeName)
{
	...
	snprintf(newCommand(cmd), MAXLINE,
			 "rm -rf %s; %s %s %s mkdir -p %s; chmod 0700 %s; rm -f /tmp/.s.*%d*;  rm -f /tmp/.s.*%d*",
			 aval(VAR_datanodeMasterDirs)[idx],
			 wal ? "rm -rf " : "",
			 wal ? aval(VAR_datanodeMasterWALDirs)[idx] : "",
			 wal ? ";" : "",
			 ...
			 atoi(aval(VAR_datanodeFwdServerPorts)[idx]));
	return(cmd);
}
