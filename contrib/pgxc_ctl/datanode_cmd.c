...
cmd_t *prepare_initDatanodeMaster(char *nodeName)
{
	...
	fprintf(f,
			...
			aval(VAR_datanodeForwardPorts)[idx],
 ...
 *
 *-----------------------------------------------------------------------*/
int add_datanodeMaster(char *name, char *host, int port, int forwardPort, int pooler, char *dir,
...
int add_datanodeSlave(char *name, char *host, int port, int forwardPort, int pooler, char *dir,
		char *walDir, char *archDir)
{
	...
	fprintf(f,
			...
			"max_wal_senders = 0\n"		/* Minimum WAL senders */
			"# End of Addition\n",
			timeStampString(date, MAXTOKEN), aval(VAR_datanodeSlavePorts)[idx], aval(VAR_datanodeSlaveForwardPorts)[idx], aval(VAR_datanodeSlavePoolerPorts)[idx]);
	pclose(f);
