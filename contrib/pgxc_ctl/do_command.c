...
static void fork_execute(int (*func)(void))
{
	if (fork() == 0)
		exit(func());
	return ;
...
static void fork_execute_char(int (*func)(char *), char *par)
{
	if (fork() == 0)
		exit(func(par));
	return ;
...
static void wait_all_child(void)
{
	while(wait(NULL) != -1) ;
}
...
static void start_all(void)
{
	fork_execute(start_gtm_master);
	if (isVarYes(VAR_gtmSlave))
		fork_execute(start_gtm_slave);
	fork_execute(start_coordinator_master_all);
	if (isVarYes(VAR_coordSlave))
		fork_execute(start_coordinator_slave_all);
	fork_execute(start_datanode_master_all);
	if (isVarYes(VAR_datanodeSlave))
		fork_execute(start_datanode_slave_all);
	wait_all_child();
}
...
{
	if (isVarYes(VAR_coordSlave))
		fork_execute_char(stop_coordinator_slave_all, immediate);
	fork_execute_char(stop_coordinator_master_all, immediate);
	if (isVarYes(VAR_datanodeSlave))
		fork_execute_char(stop_datanode_slave_all, immediate);
	fork_execute_char(stop_datanode_master_all, immediate);
	if (isVarYes(VAR_gtmSlave))
		fork_execute(stop_gtm_slave);
	if (!is_none(sval(VAR_gtmMasterServer)))
		fork_execute(stop_gtm_master);
	wait_all_child();
}
...
static void do_add_command(char *line)
{
	...
	else if (TestToken("coordinator"))
	{
		...
		if (TestToken("master"))
		{
			...
			GetAndSet(forward, "ERROR: please specify the forward port number for the coordinator master\n");
			...
			add_coordinatorMaster(name, host, atoi(port), atoi(forward), atoi(pooler), dir,
		...
		else
		{
			...
			GetAndSet(forward, "ERROR: please specify the forward port number for the coordinator slave\n");
			...
			add_coordinatorSlave(name, host, atoi(port), atoi(forward), atoi(pooler), dir, archDir);
			freeAndReset(name);
			freeAndReset(host);
			freeAndReset(dir);
		}
	...
	else if (TestToken("datanode"))
	{
		...
		if (TestToken("master"))
		{
			...
			GetAndSet(forward, "ERROR: please specify the forward port number for the datanode master\n");
			...
			add_datanodeMaster(name, host, atoi(port), atoi(forward), atoi(pooler), dir,
		...
		else
		{
			...
			GetAndSet(forward, "ERROR: please specify the forward port number for the datanode slave\n");
			...
			add_datanodeSlave(name, host, atoi(port), atoi(forward), atoi(pooler), dir,
					walDir, archDir);
			freeAndReset(name);
			freeAndReset(host);
			freeAndReset(port);
...
static void do_clean_command(char *line)
{
	...
	if (TestToken("all"))
	{
		...
		if (!is_none(sval(VAR_gtmMasterServer)))
			fork_execute(clean_gtm_master);
		if (isVarYes(VAR_gtmSlave))
			fork_execute(clean_gtm_slave);
		fork_execute(clean_coordinator_master_all);
		if (isVarYes(VAR_coordSlave))
			fork_execute(clean_coordinator_slave_all);
		fork_execute(clean_datanode_master_all);
		if (isVarYes(VAR_datanodeSlave))
			fork_execute(clean_datanode_slave_all);
		wait_all_child();
	}
	else if (TestToken("gtm"))
	{
...
static void do_configure_command(char *line)
{
	...
	{
		int node_type = NodeType_UNDEF;

		if (TestToken("datanode"))
			node_type = NodeType_DATANODE;
		else if (TestToken("coordinator"))
			node_type = NodeType_COORDINATOR;
		else
		{
			elog(ERROR, "ERROR: must specify to coordinator or datanode\n");
			return;
		}
		...
		if (node_type == NodeType_DATANODE)
...
static int show_Resource(char *datanodeName, char *databasename, char *username)
{
	...
	fprintf(f,
			...
			"              and pgxc_class.discolnums::int[] @> ARRAY[pg_attribute.attnum]\n"
			...
			"       WHERE pg_class.oid = pgxc_class.pcrelid\n"
			"             and pg_class.oid = pg_attribute.attrelid\n"
			"             and pgxc_class.discolnums::int[] @> ARRAY[0]\n"
			"             and pgxc_node.node_name = '%s'\n"
			"             and pgxc_node.oid = ANY (pgxc_class.nodeoids)\n"
			"             ;\n",
