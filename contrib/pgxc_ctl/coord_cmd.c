...
cmd_t *prepare_initCoordinatorMaster(char *nodeName)
{
	...
	fprintf(f,
			...
			atoi(aval(VAR_coordForwardPorts)[jj]),
	...
	for (ii = 0; aval(VAR_coordNames)[ii]; ii++)
	{
		...
		if (!is_none(aval(VAR_coordMasterServers)[ii]))
		{
			if (idx != targetIdx)
				...
				fprintf(f, "CREATE NODE %s WITH (TYPE='coordinator', HOST='%s', PORT=%d, FORWARD=%d);\n",
						aval(VAR_coordNames)[ii], 
						aval(VAR_coordMasterServers)[ii],
						atoi(aval(VAR_coordPorts)[ii]),
						atoi(aval(VAR_coordForwardPorts)[ii]));
			else
				/* Update myself */
				fprintf(f, "ALTER NODE %s WITH (HOST='%s', PORT=%d, FORWARD=%d);\n",
						aval(VAR_coordNames)[ii],
						aval(VAR_coordMasterServers)[ii],
						atoi(aval(VAR_coordPorts)[ii]),
						atoi(aval(VAR_coordForwardPorts)[ii]));
	...
	for (ii = 0; aval(VAR_datanodeNames)[ii]; ii++)
	{
		...
		if (sval(VAR_primaryDatanode) && (strcmp(sval(VAR_primaryDatanode), aval(VAR_datanodeNames)[dnIdx]) == 0))
		{
			...
			if (strcmp(aval(VAR_coordMasterServers)[idx], aval(VAR_datanodeMasterServers)[dnIdx]) == 0)
				...
				fprintf(f, "CREATE NODE %s WITH (TYPE='datanode', HOST='%s', PORT=%d, FORWARD=%d, PRIMARY, PREFERRED);\n",
						aval(VAR_datanodeNames)[dnIdx], aval(VAR_datanodeMasterServers)[dnIdx],
						atoi(aval(VAR_datanodePorts)[dnIdx]), atoi(aval(VAR_datanodeForwardPorts)[dnIdx]));
			else
				/* Primary but not prefereed node */
				fprintf(f, "CREATE NODE %s WITH (TYPE='datanode', HOST='%s', PORT=%d, FORWARD=%d, PRIMARY);\n",
						aval(VAR_datanodeNames)[dnIdx], aval(VAR_datanodeMasterServers)[dnIdx],
						atoi(aval(VAR_datanodePorts)[dnIdx]), atoi(aval(VAR_datanodeForwardPorts)[dnIdx]));
		}
		else
		{
			...
			if (strcmp(aval(VAR_coordMasterServers)[idx], aval(VAR_datanodeMasterServers)[dnIdx]) == 0)
				/* Preferred node */
				fprintf(f, "CREATE NODE %s WITH (TYPE='datanode', HOST='%s', PORT=%d, FORWARD=%d, PREFERRED);\n",
						aval(VAR_datanodeNames)[dnIdx], aval(VAR_datanodeMasterServers)[dnIdx],
						atoi(aval(VAR_datanodePorts)[dnIdx]), atoi(aval(VAR_datanodeForwardPorts)[dnIdx]));
			else
				/* non-Preferred node */
				fprintf(f, "CREATE NODE %s WITH (TYPE='datanode', HOST='%s', PORT=%d, FORWARD=%d);\n",
						aval(VAR_datanodeNames)[dnIdx], aval(VAR_datanodeMasterServers)[dnIdx],
						atoi(aval(VAR_datanodePorts)[dnIdx]), atoi(aval(VAR_datanodeForwardPorts)[dnIdx]));
		}
	}
...
static cmd_t *prepare_configureDataNode(char *nodeName)
{
	...
	for (ii = 0; aval(VAR_coordNames)[ii]; ii++)
	{
		...
		{
			/* Register outside coordinator */
			fprintf(f, "EXECUTE DIRECT ON (%s) 'CREATE NODE %s WITH (TYPE=''coordinator'', HOST=''%s'', PORT=%d, FORWARD=%d)';\n",
	...
	for (ii = 0; aval(VAR_datanodeNames)[ii]; ii++)
	{
		...
		if (sval(VAR_primaryDatanode) && (strcmp(sval(VAR_primaryDatanode), aval(VAR_datanodeNames)[dnIdx]) == 0))
		{
			if (idx != dnIdx)
			{
				...
				{
					/* Primay and preferred node */
					fprintf(f, "EXECUTE DIRECT ON (%s) 'CREATE NODE %s WITH (TYPE=''datanode'', HOST=''%s'', PORT=%d, FORWARD=%d, PRIMARY, PREFERRED)';\n",
							aval(VAR_datanodeNames)[idx],
							aval(VAR_datanodeNames)[dnIdx], aval(VAR_datanodeMasterServers)[dnIdx],
							atoi(aval(VAR_datanodePorts)[dnIdx]), atoi(aval(VAR_datanodeForwardPorts)[dnIdx]));
				}
					...
					/* Primary but not prefereed node */
					fprintf(f, "EXECUTE DIRECT ON (%s) 'CREATE NODE %s WITH (TYPE=''datanode'', HOST=''%s'', PORT=%d, FORWARD=%d, PRIMARY)';\n",
							aval(VAR_datanodeNames)[idx],
							aval(VAR_datanodeNames)[dnIdx], aval(VAR_datanodeMasterServers)[dnIdx],
							atoi(aval(VAR_datanodePorts)[dnIdx]), atoi(aval(VAR_datanodeForwardPorts)[dnIdx]));
			}
			else
			{
				...
				if (is_preferred)
					/* Primay and preferred node */
					fprintf(f, "EXECUTE DIRECT ON (%s) 'ALTER NODE %s WITH (TYPE=''datanode'', HOST=''%s'', PORT=%d, FORWARD=%d, PRIMARY, PREFERRED)';\n",
							aval(VAR_datanodeNames)[idx],
							aval(VAR_datanodeNames)[dnIdx], aval(VAR_datanodeMasterServers)[dnIdx],
							atoi(aval(VAR_datanodePorts)[dnIdx]), atoi(aval(VAR_datanodeForwardPorts)[dnIdx]));
				else
					/* Primary but not prefereed node */
					fprintf(f, "EXECUTE DIRECT ON (%s) 'ALTER NODE %s WITH (TYPE=''datanode'', HOST=''%s'', PORT=%d, FORWARD=%d, PRIMARY)';\n",
							aval(VAR_datanodeNames)[idx],
							aval(VAR_datanodeNames)[dnIdx], aval(VAR_datanodeMasterServers)[dnIdx],
							atoi(aval(VAR_datanodePorts)[dnIdx]), atoi(aval(VAR_datanodeForwardPorts)[dnIdx]));
			}
		...
		else
		{
			if (idx != dnIdx)
			{
				...
				if (is_preferred)
					/* Preferred node */
					fprintf(f, "EXECUTE DIRECT ON (%s) 'CREATE NODE %s WITH (TYPE=''datanode'', HOST=''%s'', PORT=%d, FORWARD=%d, PREFERRED)';\n",
							aval(VAR_datanodeNames)[idx],
							aval(VAR_datanodeNames)[dnIdx], aval(VAR_datanodeMasterServers)[dnIdx],
							atoi(aval(VAR_datanodePorts)[dnIdx]), atoi(aval(VAR_datanodeForwardPorts)[dnIdx]));
				else
					/* non-Preferred node */
					fprintf(f, "EXECUTE DIRECT ON (%s) 'CREATE NODE %s WITH (TYPE=''datanode'', HOST=''%s'', PORT=%d, FORWARD=%d)';\n",
							aval(VAR_datanodeNames)[idx],
							aval(VAR_datanodeNames)[dnIdx], aval(VAR_datanodeMasterServers)[dnIdx],
							atoi(aval(VAR_datanodePorts)[dnIdx]), atoi(aval(VAR_datanodeForwardPorts)[dnIdx]));
			}
			else
			{
				...
				if (is_preferred)
					/* Preferred node */
					fprintf(f, "EXECUTE DIRECT ON (%s) 'ALTER NODE %s WITH (TYPE=''datanode'', HOST=''%s'', PORT=%d, FORWARD=%d, PREFERRED)';\n",
							aval(VAR_datanodeNames)[idx],
							aval(VAR_datanodeNames)[dnIdx], aval(VAR_datanodeMasterServers)[dnIdx],
							atoi(aval(VAR_datanodePorts)[dnIdx]), atoi(aval(VAR_datanodeForwardPorts)[dnIdx]));
				else
					/* non-Preferred node */
					fprintf(f, "EXECUTE DIRECT ON (%s) 'ALTER NODE %s WITH (TYPE=''datanode'', HOST=''%s'', PORT=%d, FORWARD=%d)';\n",
							aval(VAR_datanodeNames)[idx],
							aval(VAR_datanodeNames)[dnIdx], aval(VAR_datanodeMasterServers)[dnIdx],
							atoi(aval(VAR_datanodePorts)[dnIdx]), atoi(aval(VAR_datanodeForwardPorts)[dnIdx]));
			}
...
cmd_t *prepare_configureNode_multicluster(char *nodeName)
{
	...
	for (ii = 0; aval(VAR_coordNames)[ii]; ii++)
	{
		...
		if (!is_none(aval(VAR_coordMasterServers)[ii]))
		{
			if (idx != targetIdx)
				/* Register outside coordinator */
				fprintf(f, "CREATE NODE %s WITH (TYPE='coordinator', HOST='%s', PORT=%d, FORWARD=%d, CLUSTER='%s');\n",
						aval(VAR_coordNames)[ii], 
						aval(VAR_coordMasterServers)[ii],
						atoi(aval(VAR_coordPorts)[ii]),
						atoi(aval(VAR_coordForwardPorts)[ii]),
		...
		if (!is_none(aval(VAR_coordSlaveServers)[ii]))
		{
			fprintf(f, "CREATE NODE %s WITH (TYPE='coordinator', HOST='%s', PORT=%d, FORWARD=%d, CLUSTER='%s');\n",
						...
						atoi(aval(VAR_coordSlaveForwardPorts)[ii]),
	...
	for (ii = 0; aval(VAR_datanodeNames)[ii]; ii++)
	{
		...
		if (sval(VAR_primaryDatanode) && (strcmp(sval(VAR_primaryDatanode), aval(VAR_datanodeNames)[dnIdx]) == 0))
		{
			...
			if (strcmp(aval(VAR_coordMasterServers)[idx], aval(VAR_datanodeMasterServers)[dnIdx]) == 0)
				/* Primay and preferred node */
				fprintf(f, "CREATE NODE %s WITH (TYPE='datanode', HOST='%s', PORT=%d, FORWARD=%d, CLUSTER='%s', PRIMARY, PREFERRED);\n",
						aval(VAR_datanodeNames)[dnIdx], aval(VAR_datanodeMasterServers)[dnIdx],
						atoi(aval(VAR_datanodePorts)[dnIdx]), atoi(aval(VAR_datanodeForwardPorts)[dnIdx]),
						aval(VAR_datanodeMasterCluster)[dnIdx]);
			else
				/* Primary but not prefereed node */
				fprintf(f, "CREATE NODE %s WITH (TYPE='datanode', HOST='%s', PORT=%d, FORWARD=%d, CLUSTER='%s', PRIMARY);\n",
						aval(VAR_datanodeNames)[dnIdx], aval(VAR_datanodeMasterServers)[dnIdx],
						atoi(aval(VAR_datanodePorts)[dnIdx]), atoi(aval(VAR_datanodeForwardPorts)[dnIdx]),
						aval(VAR_datanodeMasterCluster)[dnIdx]);
		}
		else
		{
			...
			if (strcmp(aval(VAR_coordMasterServers)[idx], aval(VAR_datanodeMasterServers)[dnIdx]) == 0)
				/* Preferred node */
				fprintf(f, "CREATE NODE %s WITH (TYPE='datanode', HOST='%s', PORT=%d, FORWARD=%d, CLUSTER='%s', PREFERRED);\n",
						aval(VAR_datanodeNames)[dnIdx], aval(VAR_datanodeMasterServers)[dnIdx],
						atoi(aval(VAR_datanodePorts)[dnIdx]), atoi(aval(VAR_datanodeForwardPorts)[dnIdx]),
						aval(VAR_datanodeMasterCluster)[dnIdx]);
			else
				/* non-Preferred node */
				fprintf(f, "CREATE NODE %s WITH (TYPE='datanode', HOST='%s', PORT=%d, FORWARD=%d, CLUSTER='%s');\n",
						aval(VAR_datanodeNames)[dnIdx], aval(VAR_datanodeMasterServers)[dnIdx],
						atoi(aval(VAR_datanodePorts)[dnIdx]), atoi(aval(VAR_datanodeForwardPorts)[dnIdx]),
						aval(VAR_datanodeMasterCluster)[dnIdx]);
		}
		...
		{			
			fprintf(f, "CREATE NODE %s WITH (TYPE='datanode', HOST='%s', PORT=%d, FORWARD=%d, CLUSTER='%s');\n",
				...
				atoi(aval(VAR_datanodeSlavePorts)[dnIdx]), atoi(aval(VAR_datanodeSlaveForwardPorts)[dnIdx]),
...
static cmd_t *prepare_configureDataNode_multicluster(char *nodeName)
{
	...
	for (ii = 0; aval(VAR_coordNames)[ii]; ii++)
	{
		...
		{
			/* Register outside coordinator */
			fprintf(f, "EXECUTE DIRECT ON (%s) 'CREATE NODE %s WITH (TYPE=''coordinator'', HOST=''%s'', PORT=%d, FORWARD=%d, CLUSTER=''%s'')';\n",
					aval(VAR_datanodeNames)[idx],
					aval(VAR_coordNames)[ii],
					aval(VAR_coordMasterServers)[ii],
					atoi(aval(VAR_coordPorts)[ii]),
	...
	for (ii = 0; aval(VAR_datanodeNames)[ii]; ii++)
	{
		...
		if (sval(VAR_primaryDatanode) && (strcmp(sval(VAR_primaryDatanode), aval(VAR_datanodeNames)[dnIdx]) == 0))
		{
			if (idx != dnIdx)
			{
				...
				{
					/* Primay and preferred node */
					fprintf(f, "EXECUTE DIRECT ON (%s) 'CREATE NODE %s WITH (TYPE=''datanode'', HOST=''%s'', PORT=%d, FORWARD=%d, CLUSTER=''%s'', PRIMARY, PREFERRED)';\n",
							aval(VAR_datanodeNames)[idx],
							aval(VAR_datanodeNames)[dnIdx], aval(VAR_datanodeMasterServers)[dnIdx],
							atoi(aval(VAR_datanodePorts)[dnIdx]), atoi(aval(VAR_datanodeForwardPorts)[dnIdx]),
							aval(VAR_datanodeMasterCluster)[dnIdx]);
					...
					/* Primary but not prefereed node */
					fprintf(f, "EXECUTE DIRECT ON (%s) 'CREATE NODE %s WITH (TYPE=''datanode'', HOST=''%s'', PORT=%d, FORWARD=%d, CLUSTER=''%s'', PRIMARY)';\n",
							aval(VAR_datanodeNames)[idx],
							aval(VAR_datanodeNames)[dnIdx], aval(VAR_datanodeMasterServers)[dnIdx],
							atoi(aval(VAR_datanodePorts)[dnIdx]), atoi(aval(VAR_datanodeForwardPorts)[dnIdx]),
							aval(VAR_datanodeMasterCluster)[dnIdx]);
		...
		else
		{
			if (idx != dnIdx)
			{
				...
				if (is_preferred)
					/* Preferred node */
					fprintf(f, "EXECUTE DIRECT ON (%s) 'CREATE NODE %s WITH (TYPE=''datanode'', HOST=''%s'', PORT=%d, FORWARD=%d, CLUSTER=''%s'',PREFERRED)';\n",
							aval(VAR_datanodeNames)[idx],
							aval(VAR_datanodeNames)[dnIdx], aval(VAR_datanodeMasterServers)[dnIdx],
							atoi(aval(VAR_datanodePorts)[dnIdx]), atoi(aval(VAR_datanodeForwardPorts)[dnIdx]),
							aval(VAR_datanodeMasterCluster)[dnIdx]);
					...
					/* non-Preferred node */
					fprintf(f, "EXECUTE DIRECT ON (%s) 'CREATE NODE %s WITH (TYPE=''datanode'', HOST=''%s'', PORT=%d, FORWARD=%d, CLUSTER=''%s'')';\n",
							aval(VAR_datanodeNames)[idx],
							aval(VAR_datanodeNames)[dnIdx], aval(VAR_datanodeMasterServers)[dnIdx],
							atoi(aval(VAR_datanodePorts)[dnIdx]), atoi(aval(VAR_datanodeForwardPorts)[dnIdx]),
							aval(VAR_datanodeMasterCluster)[dnIdx]);
 ...
 *
 *-----------------------------------------------------------------------*/
int add_coordinatorMaster(char *name, char *host, int port, int forwardPort, int pooler,
...
int add_coordinatorSlave(char *name, char *host, int port, int forwardPort, int pooler_port, char *dir, char *archDir)
{
	int idx;
	FILE *f;
	char port_s[MAXTOKEN+1];
	...
	fprintf(f,
			...
			atoi(aval(VAR_coordSlaveForwardPorts)[idx]),
