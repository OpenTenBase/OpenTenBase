...
select pg_typeof(sys_guid()), pg_typeof(pg_catalog.sys_guid());

select pg_typeof(opentenbase_ora.sys_guid());

create database regression_ora_uuidossp sql mode opentenbase_ora;
\c regression_ora_uuidossp

CREATE EXTENSION "uuid-ossp";
-- use sys_guid in opentenbase_ora first
select pg_typeof(sys_guid()), pg_typeof(pg_catalog.sys_guid()),pg_typeof(opentenbase_ora.sys_guid()) from dual;
SELECT uuid_nil();
SELECT uuid_ns_dns();
SELECT uuid_ns_url();
SELECT uuid_ns_oid();
