...
sys_guid(PG_FUNCTION_ARGS)
{
	...
	uuid = DatumGetUUIDP(uuid_generate_internal(UUID_MAKE_V1, NULL, NULL, 0));
	initStringInfo(&buf);
	...
	for (i = 0; i < UUID_LEN; i++)
	{
		...
		appendStringInfoChar(&buf, hex_chars_upper[hi]);
		appendStringInfoChar(&buf, hex_chars_upper[lo]);
	}
	buf.data[UUID_LEN_STR] = '\0';
