...
static bool auto_explain_log_plan_warning = true;
...
_PG_init(void)
{
	...
	DefineCustomBoolVariable("auto_explain.log_plan_warning",
							 "Check and record plans with suspicious points.",
							 NULL,
							 &auto_explain_log_plan_warning,
							 true,
							 PGC_SUSET,
							 0,
							 NULL,
...
explain_ExecutorStart(QueryDesc *queryDesc, int eflags)
{
	...
	if (auto_explain_enabled())
	{
		...
		if (auto_explain_log_plan_warning)
			queryDesc->instrument_options |= INSTRUMENT_SQL_WARN;
	}
...
static uint32
gen_planinfo(QueryDesc *queryDesc)
{
	...
	ExplainEndOutput(es);
	return hash_any((const unsigned char*)es->str->data, es->str->len);
}
...
explain_ExecutorEnd(QueryDesc *queryDesc)
{
	if (queryDesc->totaltime && auto_explain_enabled())
	{
		...
		if (msec >= auto_explain_log_min_duration)
		{
			...
			if (compute_query_id)
				ExplainPropertyInteger("Query Id", NULL, queryDesc->plannedstmt->queryId, es);
			
			ExplainPropertyInteger("Plan Id", NULL, SPM_ENABLED ? pre_spmplan->plan_id : gen_planinfo(queryDesc), es);
			
			ExplainQueryText(es, queryDesc);
			ExplainQueryParameters(es, queryDesc->params, auto_explain_log_parameter_max_length);
			ExplainPropertyText("Query Plan", "", es);
			ExplainPrintPlan(es, queryDesc);
			...
			if (es->warning_str)
			{
				...
				if (auto_explain_log_format == EXPLAIN_FORMAT_JSON)
				{
					es->warning_str->data[0] = '{';
					es->warning_str->data[es->warning_str->len - 1] = '}';
				}

				ereport(LOG,
						(errmsg("duration: %.3f ms  plan:\n%s\n%s",
								msec, es->str->data, es->warning_str->data),
						errhidestmt(true)));
