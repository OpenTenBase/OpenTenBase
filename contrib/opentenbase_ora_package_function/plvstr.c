...
PG_FUNCTION_INFO_V1(opentenbase_ora_substr2);
PG_FUNCTION_INFO_V1(opentenbase_ora_substr3);

PG_FUNCTION_INFO_V1(plvstr_instrb2);
PG_FUNCTION_INFO_V1(plvstr_instrb3);
PG_FUNCTION_INFO_V1(plvstr_instrb4);

PG_FUNCTION_INFO_V1(orcl_chr_1);
PG_FUNCTION_INFO_V1(orcl_chr_2);
PG_FUNCTION_INFO_V1(orcl_soundex);
PG_FUNCTION_INFO_V1(orcl_soundex_rowid);
...
int
ora_instr(text *txt, text *pattern, int start, int nth, bool in_byte)
{
	int			len_txt, len_pat;
	const char *str_txt, *str_pat;
	int			beg, end, i, dx;
	...
	if (nth <= 0)
		PARAMETER_ERROR("The 4th parameter is out of range.");

	/* OpenTenBase_Ora treat 0 as invalid start parameter */
	if (ORA_MODE && start == 0)
		return 0;
...
plvstr_instr2 (PG_FUNCTION_ARGS)
{
	...
	PG_RETURN_INT32(ora_instr(arg1, arg2, 1, 1, false));
}
...
plvstr_instr3 (PG_FUNCTION_ARGS)
{
	...
	PG_RETURN_INT32(ora_instr(arg1, arg2, arg3, 1, false));
}
...
plvstr_instr4 (PG_FUNCTION_ARGS)
{
	...
	PG_RETURN_INT32(ora_instr(arg1, arg2, arg3, arg4, false));
}
 ...
 *
 * Syntax:
 *   FUNCTION plvstr.instrb (string_in VARCHAR, pattern VARCHAR)
 *   FUNCTION plvstr.instrb (string_in VARCHAR, pattern VARCHAR,
 *            start_in INTEGER)
 *   FUNCTION plvstr.instrb (string_in VARCHAR, pattern VARCHAR,
 *            start_in INTEGER, nth INTEGER)
 ...
 ****************************************************************/
Datum
plvstr_instrb2 (PG_FUNCTION_ARGS)
{
	text *arg1 = PG_GETARG_TEXT_PP(0);
	text *arg2 = PG_GETARG_TEXT_PP(1);

	PG_RETURN_INT32(ora_instr(arg1, arg2, 1, 1, true));
}

Datum
plvstr_instrb3 (PG_FUNCTION_ARGS)
{
	text *arg1 = PG_GETARG_TEXT_PP(0);
	text *arg2 = PG_GETARG_TEXT_PP(1);
	int arg3 = PG_GETARG_INT32(2);

	PG_RETURN_INT32(ora_instr(arg1, arg2, arg3, 1, true));
}

Datum
plvstr_instrb4 (PG_FUNCTION_ARGS)
{
	text *arg1 = PG_GETARG_TEXT_PP(0);
	text *arg2 = PG_GETARG_TEXT_PP(1);
	int arg3 = PG_GETARG_INT32(2);
	...
	PG_RETURN_INT32(ora_instr(arg1, arg2, arg3, arg4, true));
}
...
plvstr_rvrs(PG_FUNCTION_ARGS)
{
	...
	/*
	 * Normalize the [start,end] range for negtive number,
	 * e.g. [-1, -10] on a 12 byte string to [3, 12]
	 */
	new_start = start;
	new_end = end;
	if (start < 0)
	{
		new_start = MAX(len + end + 1, 0);
		new_end = MAX(len + start + 1, 0);
	}

	/* Treat index 0 the same as index 1 */
	if (new_start == 0)
		new_start = 1;

	/* Trim end index beyond the last character */
	new_end = MIN(new_end, len);
	...
	if (mb_encode)
	{
		...
		cur_size = 0;
		p = VARDATA_ANY(str);
		for (i = new_end - 1; i>= new_start - 1; i--)
		{
			for (j=0; j<sizes[i]; j++)
...
plvstr_lpart (PG_FUNCTION_ARGS)
{
	...
	loc = ora_instr(str, div, start, nth, false);
	if (loc == 0)
	{
		if (all_if_notfound)
...
{
	return opentenbase_ora_substr2(fcinfo);
}
...
{
	return opentenbase_ora_substr3(fcinfo);
}
 ...
 *
 * Purpouse:
 *   Returns len chars from start_in position, compatible with OpenTenBase_Ora
 *
 ****************************************************************/

Datum
opentenbase_ora_substr3(PG_FUNCTION_ARGS)
{
...
Datum
opentenbase_ora_substr2(PG_FUNCTION_ARGS)
{
	PG_RETURN_TEXT_P(ora_substr(PG_GETARG_DATUM(0), PG_GETARG_INT32(1), -1));
}
...
plvstr_betwn_c(PG_FUNCTION_ARGS)
{
	...
	{
		v_start = 1;
		v_end = ora_instr(string_in, end_in, 1, endnth_in, false);
	}
	else
	{
		v_start = ora_instr(string_in, start_in, 1, startnth_in, false);
		v_end = ora_instr(string_in, end_in, v_start + 1, endnth_in, false);
	}
...
Datum
orcl_chr_1(PG_FUNCTION_ARGS)
{
	return DirectFunctionCall1(orcl_chr, PG_GETARG_DATUM(0));
}

Datum
orcl_chr_2(PG_FUNCTION_ARGS)
{
	return DirectFunctionCall2(orcl_chr, PG_GETARG_DATUM(0), PG_GETARG_DATUM(1));
}
...
static bool
orcl_is_lower_alphabet_and_get_diff(wchar_t wchar, int* diff)
{
	static const wchar_t lower_alphabet_min = (wchar_t)('a');
	static const wchar_t lower_alphabet_max = (wchar_t)('z');

	if (wchar >= lower_alphabet_min && wchar <= lower_alphabet_max)
	{
		*diff = wchar - lower_alphabet_min;
		return true;
	}
	return false;
}
...
static bool
orcl_is_upper_alphabet_and_get_diff(wchar_t wchar,  int* diff)
{
	static const wchar_t upper_alphabet_min = (wchar_t)('A');
	static const wchar_t upper_alphabet_max = (wchar_t)('Z');

	if (wchar >= upper_alphabet_min && wchar <= upper_alphabet_max)
	{
		*diff = wchar - upper_alphabet_min;
		return true;
	}
	return false;
}
...
static int8
orcl_soundex_get_character_code(const wchar_t wchar)
{
	/* alphabet num */
	static const int8 convert_map[26] =
	{
		0, 1, 2, 3, 0, 1, 2,  /* a ~ g */
		0, 0, 2, 2, 4, 5, 5,  /* h ~ n */
		0, 1, 2, 6, 2, 3,     /* o ~ t */
	...
	if (orcl_is_lower_alphabet_and_get_diff(wchar, &index))
		res = convert_map[index];
	else if (orcl_is_upper_alphabet_and_get_diff(wchar, &index))
		res = convert_map[index];
	return res;
}
...
/*
 * char soundex(char)
 * The phonetic representation is defined in
 * The Art of Computer Programming, Volume 3: Sorting and Searching, 
 * by Donald E. Knuth, as follows:
 ...
 * 	  m, n = 5
 * 	  r = 6
 * 3. If two or more letters with the same number were adjacent in the original name (before step 1),
 *    or adjacent except for any intervening h and w, then retain the first letter and
 *    omit rest of all the adjacent letters with same number.
 * 4. Return the first four bytes padded with 0.
 *    Assign numbers to the remaining letters (after the first) as follows:
 */

Datum
orcl_soundex(PG_FUNCTION_ARGS)
{
	text	 	*in_str = PG_GETARG_TEXT_PP_IF_NULL(0);
	char		*str = NULL;
	int			str_len = -1;
	int32		str_cursor = 0;
	bool		is_first = true;
	...
	if (VARSIZE_ANY_EXHDR(in_str) == 0)
		PG_RETURN_NULL();
	
	str = text_to_cstring(in_str);
	str_len = VARSIZE_ANY_EXHDR(in_str);
	while((str_cursor < str_len) && (pos < fix_res_len))
	{
		mb_len = mblen(str + str_cursor, str_len - str_cursor);
		/* illegal charset omit */
		if (mb_len == -1)
			++str_cursor;
		else
		{
			char	*mbstr = pnstrdup(str + str_cursor, mb_len);
			if (mbstowcs(&wchar, mbstr, 1) == -1)
			{
				ereport(ERROR,
			...
			pfree(mbstr);
			soundex_code = orcl_soundex_get_character_code(wchar);
			if (is_first)
			{
				if (soundex_code >= 0)
				{
	...
	while(pos < fix_res_len)
		buf[pos++] = '0';

	PG_RETURN_TEXT_P(cstring_to_text_with_len(buf, fix_res_len));
}

Datum
orcl_soundex_rowid(PG_FUNCTION_ARGS)
{
	return DirectFunctionCall1(orcl_soundex, DirectFunctionCall1(rowid_to_text, PG_GETARG_DATUM(0)));
}
