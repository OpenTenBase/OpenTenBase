 ...
 *-------------------------------------------------------------------------
 */
#ifndef DBMS_MD_PG_BACKUP_ARCHIVE__
#define DBMS_MD_PG_BACKUP_ARCHIVE__
...
#define WRITE_ERROR_EXIT \
	do { \
		elog(ERROR, "could not write to output file: %s\n", \
					  strerror(errno)); \
	} while (0)
...
typedef struct
{
	...
	StringInfo curCmd;			/* incomplete line (NULL if not created) */
} sqlparseInfo;
...
struct _archiveHandle
{
	...
	DBMS_MD_ArchiveMode mode;			/* File mode - r or w */
...
extern void InitArchiveFmt_String(ArchiveHandle *AH);
#if 0
...
#endif			/*DBMS_MD_PG_BACKUP_ARCHIVE__*/
