...
/* 
 *
 * OpenTenBase_Ora Database Real Application Security SQL Functions
 * 
 */
...
ora_asciistr(PG_FUNCTION_ARGS)
{
	...
	/* Convert to utf8 */
	str = text_to_cstring(string);
	utf8_str = pg_server_to_any(str, len, PG_UTF8);
	utf8_data = (unsigned char *) utf8_str;
	/* Update the length */
	len = strlen(utf8_str);
	/*
	 * The max size of utf16 is len * 3 + 1:
	 * 1 byte in utf8 -> 1 byte in utf16
	 * 2 bytes in utf8 -> 5 bytes in utf16
	 * 3 bytes in utf8 -> 5 bytes in utf16
	 * 4 bytes in utf8 -> 10 bytes in utf16
	 */
	utf16_str = palloc(len * 3 + 1);
	utf16_data = (unsigned char *) utf16_str;
	...
	for (; len > 0; len -= l)
	{
		...
		l = pg_utf_mblen(utf8_data);
		if (len < l)
			break;

		if (!pg_utf8_islegal(utf8_data, l))
			break;
		...
		{
			/* 110xxxxx 10xxxxxx */
			code_point = ((*utf8_data++) & 0x1F) << 6;
			code_point |= (*utf8_data++) & 0x3F;
		}
		else if (l == 3)
		{
			/* 1110xxxx 10xxxxxx 10xxxxxx */
			code_point = ((*utf8_data++) & 0x0F) << 12;
			code_point |= ((*utf8_data++) & 0x3F) << 6;
			code_point |= (*utf8_data++) & 0x3F;
		}
		else if (l == 4)
		{
			/* 11110xxx 10xxxxxx 10xxxxxx 10xxxxxx */
			code_point = ((*utf8_data++) & 0x07) << 18;
			code_point |= ((*utf8_data++) & 0x3F) << 12;
			code_point |= ((*utf8_data++) & 0x3F) << 6;
			code_point |= (*utf8_data++) & 0x3F;
		}
		else
		...
		/* Get the hexadecimal utf16 code */
		Assert(code_point);
		*utf16_data++ = '\\';

		if (code_point <= 0xFFFF)
		{
			/* Process code points in BMP */
			*utf16_data++ = hex_chars[code_point >> 12];
			*utf16_data++ = hex_chars[(code_point >> 8) & 0x0F];
			*utf16_data++ = hex_chars[(code_point >> 4) & 0x0F];
			*utf16_data++ = hex_chars[code_point & 0x0F];
		}
		else
		{
			/* Process code points in supplementary planes */
			uint32 y_x;
			uint32 high;
			uint32 low;

			/* y_x like yy yyyyyyyy xx xxxxxxxx */
			y_x = code_point - 0x10000;
			/* High byte like 110110yy yyyyyyyy */
			high = (y_x >> 10) | 0xD800;
			*utf16_data++ = 'D';
			*utf16_data++ = hex_chars[(high >> 8) & 0x0F];
			*utf16_data++ = hex_chars[(high >> 4) & 0x0F];
			*utf16_data++ = hex_chars[high & 0x0F];
			/* Low byte like 110111xx xxxxxxxx */
			low = (y_x & 0x03FF) | 0xDC00;
			*utf16_data++ = '\\';
			*utf16_data++ = 'D';
			*utf16_data++ = hex_chars[(low >> 8) & 0x0F];
			*utf16_data++ = hex_chars[(low >> 4) & 0x0F];
			*utf16_data++ = hex_chars[low & 0x0F];
		}
	}

	/* if we broke out of loop early, must be invalid input */
	if (len > 0)
		report_invalid_encoding(PG_UTF8, (const char *) utf8_data, len);

	*utf16_data = '\0';
	result = cstring_to_text_with_len(utf16_str, strlen(utf16_str));
	/* Free the memory */
	if (utf8_str != (char *) str)
		pfree(utf8_str);
	pfree(utf16_str);
	pfree(str);
	PG_RETURN_TEXT_P(result);
