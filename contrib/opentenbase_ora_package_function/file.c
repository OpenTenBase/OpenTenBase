...
utl_file_fopen(PG_FUNCTION_ARGS)
{
	 ...
	 * for opentenbase_ora compatibility.
	 */
#if NOT_USED
	...
	if (!file_exist)
	{
		int ret = chmod(fullname, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH);
		if (ret < 0)
		{
			ereport(ERROR, (errcode_for_file_access(),
				errmsg("could not chmod file[%s] %d:%s", fullname, errno, strerror(errno))));
		}
	}
...
get_line(FILE *f, size_t max_linesize, int encoding, bool *iseof)
{
	...
	if (!eof)
	{
		...
		pg_verify_mbstr(encoding, buffer, size2int(csize), false, false);
 ...
 *          RETURNS NULL
 *
 * Close an open file. This function reset file handle to NULL on OpenTenBase_Ora platform.
