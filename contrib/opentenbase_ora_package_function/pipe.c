	...
	if (pipes == NULL)
	{
		...
		if (!found)
		{
...

			{
				...
				LWLockRegisterTranche(sh_mem->tranche_id, "opentenbase_dbms_pipe");

#else

				static LWLockTranche tranche;

				tranche.name = "opentenbase_dbms_pipe";
			...
			pipes = sh_mem->pipes = ora_salloc(max_pipes*sizeof(db_pipe));
			sid = sh_mem->sid = 1;
...
static void*
remove_first(db_pipe *p, bool *found)
{
		...
		else
		{
			...
			ereport(ERROR,
						(errcode(ERRCODE_PROGRAM_LIMIT_EXCEEDED),
						 errmsg("pipe creation error"),
						 errdetail("Maybe pipe num exceed MAX_PIPES: %d.", MAX_PIPES)));
		}
	}
	WATCH_POST(timeout, endtime, cycle);
	LOCK_ERROR();
