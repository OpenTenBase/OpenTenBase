...
dbms_alert_defered_signal(PG_FUNCTION_ARGS)
{
	...
	if (strcmp((relname = SPI_getrelname(trigdata->tg_relation)),
			   (ORA_MODE) ? "ORA_ALERTS" : "ora_alerts") != 0)
		ereport(ERROR,
			(errcode(ERRCODE_TRIGGERED_ACTION_EXCEPTION),
	...
	if (SPI_ERROR_NOATTRIBUTE == (event_col = SPI_fnumber(tupdesc, (ORA_MODE) ? "EVENT" : "event")))
		ereport(ERROR,
			(errcode(ERRCODE_TRIGGERED_ACTION_EXCEPTION),
			 errmsg("attribute event not found")));


	if (SPI_ERROR_NOATTRIBUTE == (message_col = SPI_fnumber(tupdesc, (ORA_MODE) ? "MESSAGE" : "message")))
		ereport(ERROR,
			(errcode(ERRCODE_TRIGGERED_ACTION_EXCEPTION),
