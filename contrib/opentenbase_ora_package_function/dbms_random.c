 ...
 * number generator used in OpenTenBase_Ora. So the results of these
 * functions should be different then native OpenTenBase_Ora functions!
...
dbms_random_random(PG_FUNCTION_ARGS)
{
	int result;
	/*
	 * OpenTenBase_Ora generator generates numebers from -2^31 and +2^31,
		...
		default:
			ereport(ERROR,
				(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
				 ...
				 errhint("available option: ['a', 'A', 'l', 'L', 'u', 'U', 'x', 'X', 'p', 'P'].")));
 ...
 *
 *     Alternatively, you can get a random OpenTenBase_Ora number x,
 *     where x is greater than or equal to low and less than high
 */
Datum
dbms_random_value_range(PG_FUNCTION_ARGS)
{
	float8 low = DatumGetFloat8(
		  DirectFunctionCall1(numeric_float8_no_overflow, PG_GETARG_DATUM(0)));
	float8 high = DatumGetFloat8(
		  DirectFunctionCall1(numeric_float8_no_overflow, PG_GETARG_DATUM(1)));
	float8 result;
