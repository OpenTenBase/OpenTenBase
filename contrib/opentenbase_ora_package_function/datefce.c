...
PG_FUNCTION_INFO_V1(new_time_orcl);
PG_FUNCTION_INFO_V1(extract_orcl);
...
next_day(PG_FUNCTION_ARGS)
{
	...
	/*
	 * OpenTenBase_Ora uses only 3 heading characters of the input.
...
/* next_day(date, integer) is not documented in OpenTenBase_Ora manual, but ... */
Datum
...
ora_to_date(PG_FUNCTION_ARGS)
{
	...
	if(nls_date_format && strlen(nls_date_format))
	{
		...
		newDate = DirectFunctionCall3(to_timestamp,
							CStringGetDatum(date_txt),
							CStringGetDatum(cstring_to_text(nls_date_format)),
							CStringGetDatum(cstring_to_text(disable_check)));
...
orafce_sysdate(PG_FUNCTION_ARGS)
{
	...
	/* necessary to cast to timestamp(0) to emulate OpenTenBase_Ora's date */
...
static NewTimeTimeZoneInfo new_time_timezones[] = {
	{"ADT", "-3:00"},
	{"AST", "-4:00"},
	{"BDT", "-10:00"},
	{"BST", "-11:00"},
...
const int newtime_timezone_conut = lengthof(new_time_timezones);
